...
using HIS.Desktop.Plugins.HisCashierAddConfig.Validtion;
using HIS.Desktop.LocalStorage.BackendData;

namespace HIS.Desktop.Plugins.HisCashierAddConfig.HisCashierAddConfig
{
    public partial class frmHisCashierAddConfig : HIS.Desktop.Utility.FormBase
    {
        #region Declare
        int rowCount = 0;
        int dataTotal = 0;
        ...
        MOS.EFMODEL.DataModels.V_HIS_CASHIER_ADD_CONFIG currentData;
        ...
        #region Construct
        public frmHisCashierAddConfig(Inventec.Desktop.Common.Modules.Module moduleData)
            : base(moduleData)
        {
            try
            {
        ...
        private void SetCaptionByLanguageKey()
        {
            try
            {
                ...
                Resources.ResourceLanguageManager.LanguageResource = new ResourceManager("HIS.Desktop.Plugins.HisCashierAddConfig.Resources.Lang", typeof(HIS.Desktop.Plugins.HisCashierAddConfig.HisCashierAddConfig.frmHisCashierAddConfig).Assembly);
                ...
                this.cboCashierRoom.Properties.NullText = Inventec.Common.Resource.Get.Value("frmHisSupclinicalRsAdo.cboResultRoom.Properties.NullText", Resources.ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
        ...
        private void SetDefaultValue()
        {
            try
            {
                ...
                dxValidationProviderEditorInfo.RemoveControlError(txtCashierRoomCode);
                dxValidationProviderEditorInfo.RemoveControlError(cboCashierRoom);
        ...
        private void InitComboCashierRoom()
        {
            try
            {
                ...
                {
                    cashierRooms = cashierRooms.Where(o => o.IS_ACTIVE == 1).ToList();
                }
                ...
                ControlEditorLoader.Load(cboCashierRoom, cashierRooms, controlEditorADO);
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Warn(ex);
        ...
        private void LoadPaging(object param)
        {
            try
            {
                ...
                Inventec.Core.ApiResultObject<List<MOS.EFMODEL.DataModels.V_HIS_CASHIER_ADD_CONFIG>> apiResult = null;
                HisCashierAddConfigViewFilter filter = new HisCashierAddConfigViewFilter();
                ...
                apiResult = new BackendAdapter(paramCommon).GetRO<List<MOS.EFMODEL.DataModels.V_HIS_CASHIER_ADD_CONFIG>>(HisRequestUriStore.MOSHIS_CASHIER_ROOM_GETVIEW, ApiConsumers.MosConsumer, filter, paramCommon);
                if (apiResult != null)
                {
                    var data = (List<MOS.EFMODEL.DataModels.V_HIS_CASHIER_ADD_CONFIG>)apiResult.Data;
                    if (data != null)
                    {
        ...
        private void SetFilterNavBar(ref HisCashierAddConfigViewFilter filter)
        {
            try
            {
        ...
        private void txtKeyword_KeyUp(object sender, KeyEventArgs e)
        {
            try
            {
                ...
                else if (e.KeyCode == Keys.Down)
                {
                    ...
                    var rowData = (MOS.EFMODEL.DataModels.V_HIS_CASHIER_ADD_CONFIG)gridviewFormList.GetFocusedRow();
                    if (rowData != null)
                    {
        ...
        private void gridviewFormList_CustomUnboundColumnData(object sender, DevExpress.XtraGrid.Views.Base.CustomColumnDataEventArgs e)
        {
            try
            {
                ...
                {
                    MOS.EFMODEL.DataModels.V_HIS_CASHIER_ADD_CONFIG pData = (MOS.EFMODEL.DataModels.V_HIS_CASHIER_ADD_CONFIG)((IList)((BaseView)sender).DataSource)[e.ListSourceRowIndex];
                    ...
                    else if (e.Column.FieldName == "IS_NOT_PRIORITY_DISPLAY")
                    {
                        if (pData.IS_NOT_PRIORITY == 1)
                        {
                            e.Value = true;
                        }
                        else
        ...
        private void dnNavigation_PositionChanged(object sender, EventArgs e)
        {
            try
            {
                this.currentData = (MOS.EFMODEL.DataModels.V_HIS_CASHIER_ADD_CONFIG)(gridControlFormList.DataSource as List<MOS.EFMODEL.DataModels.V_HIS_CASHIER_ADD_CONFIG>)[dnNavigation.Position];
        ...
        private void ChangedDataRow(MOS.EFMODEL.DataModels.V_HIS_CASHIER_ADD_CONFIG data)
        {
            try
            {
                if (data != null)
        ...
        private void FillDataToEditorControl(MOS.EFMODEL.DataModels.V_HIS_CASHIER_ADD_CONFIG data)
        {
            try
            {
                if (data != null)
                {
                    ...
                    cboCashierRoom.EditValue = data.CASHIER_ROOM_ID;
                    chkIsNotPriority.Checked = data.IS_NOT_PRIORITY == 1;
                }
            }
            catch (Exception ex)
            {
        ...
        private void LoadCurrent(long currentId, ref MOS.EFMODEL.DataModels.HIS_CASHIER_ADD_CONFIG currentDTO)
        {
            try
            {
                CommonParam param = new CommonParam();
                ...
                filter.ID = currentId;
                currentDTO = new BackendAdapter(param).Get<List<MOS.EFMODEL.DataModels.HIS_CASHIER_ADD_CONFIG>>(HisRequestUriStore.MOSHIS_CASHIER_ROOM_GET, ApiConsumers.MosConsumer, filter, param).FirstOrDefault();
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Warn(ex);
            ...
            try
            {
                ...
                if (rowData != null)
                {
                    if (MessageBox.Show(LibraryMessage.MessageUtil.GetMessage(LibraryMessage.Message.Enum.HeThongTBCuaSoThongBaoBanCoMuonXoaDuLieuKhong), "", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        ...
                        {
                            FillDataToGridControl();
                            currentData = ((List<V_HIS_CASHIER_ADD_CONFIG>)gridControlFormList.DataSource).FirstOrDefault();
                        }
        ...
        private void SaveProcess()
        {
            ...
            try
            {
                ...
                if (!dxValidationProviderEditorInfo.Validate())
                    return;
                if (cboExecuteRoom.EditValue == null && cboRequestRoom.EditValue == null)
                {
                    MessageBox.Show("Bắt buộc truyền vào phòng yêu cầu hoặc phòng xử lý", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    txtRequestRoomCode.Focus();
                    txtRequestRoomCode.SelectAll();
                    return;
                }

                WaitingManager.Show();
                MOS.EFMODEL.DataModels.HIS_CASHIER_ADD_CONFIG updateDTO = new MOS.EFMODEL.DataModels.HIS_CASHIER_ADD_CONFIG();
                ...
                if (ActionType == GlobalVariables.ActionAdd)
                {
                    ...
                    var resultData = new BackendAdapter(param).Post<MOS.EFMODEL.DataModels.HIS_CASHIER_ADD_CONFIG>(HisRequestUriStore.MOSHIS_CASHIER_ROOM_CREATE, ApiConsumers.MosConsumer, updateDTO, param);
                    if (resultData != null)
                    {
                        success = true;
                        FillDataToGridControl();
                ...
                {
                    var resultData = new BackendAdapter(param).Post<MOS.EFMODEL.DataModels.HIS_CASHIER_ADD_CONFIG>(HisRequestUriStore.MOSHIS_CASHIER_ROOM_UPDATE, ApiConsumers.MosConsumer, updateDTO, param);
                    if (resultData != null)
                    {
                        success = true;
        ...
        private void UpdateRowDataAfterEdit(MOS.EFMODEL.DataModels.HIS_CASHIER_ADD_CONFIG data)
        {
            try
            {
                if (data == null)
                    throw new ArgumentNullException("data(MOS.EFMODEL.DataModels.V_HIS_CASHIER_ADD_CONFIG) is null");
                var rowData = (MOS.EFMODEL.DataModels.V_HIS_CASHIER_ADD_CONFIG)gridviewFormList.GetFocusedRow();
                if (rowData != null)
                {
                    Inventec.Common.Mapper.DataObjectMapper.Map<MOS.EFMODEL.DataModels.V_HIS_CASHIER_ADD_CONFIG>(rowData, data);
        ...
        private void UpdateDTOFromDataForm(ref MOS.EFMODEL.DataModels.HIS_CASHIER_ADD_CONFIG currentDTO)
        {
            try
            {
                ...
                if (cboCashierRoom.EditValue != null)
                    currentDTO.CASHIER_ROOM_ID = Inventec.Common.TypeConvert.Parse.ToInt64((cboCashierRoom.EditValue ?? "0").ToString());
                ...
                if (chkIsNotPriority.Checked)
                    currentDTO.IS_NOT_PRIORITY = 1;
                else
                    currentDTO.IS_NOT_PRIORITY = null;
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Warn(ex);
        ...
        private void ValidateForm()
        {
            try
            {
                ValidateGridLookupWithTextEdit(cboCashierRoom, txtCashierRoomCode);
        ...
        {
            CommonParam param = new CommonParam();
            HIS_CASHIER_ADD_CONFIG result = new HIS_CASHIER_ADD_CONFIG();
            bool success = false;
            try
            {

                V_HIS_CASHIER_ADD_CONFIG data = (V_HIS_CASHIER_ADD_CONFIG)gridviewFormList.GetFocusedRow();
                WaitingManager.Show();
                result = new Inventec.Common.Adapter.BackendAdapter(param).Post<HIS_CASHIER_ADD_CONFIG>(HisRequestUriStore.MOSHIS_CASHIER_ROOM_CHANGELOCK, ApiConsumers.MosConsumer, data.ID, param);
                WaitingManager.Hide();
                if (result != null)
                {
                    success = true;
        ...
        private void gridviewFormList_RowCellStyle(object sender, DevExpress.XtraGrid.Views.Grid.RowCellStyleEventArgs e)
        {
            try
            {
                ...
                if (e.RowHandle >= 0)
                {
                    V_HIS_CASHIER_ADD_CONFIG data = (V_HIS_CASHIER_ADD_CONFIG)((IList)((BaseView)sender).DataSource)[e.RowHandle];
        ...
        private void txtResultRoomCode_PreviewKeyDown(object sender, PreviewKeyDownEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keys.Enter)
                {
                    ...
                    {
                        List<V_HIS_CASHIER_ROOM> searchs = null;
                        var listData1 = BackendDataWorker.Get<V_HIS_CASHIER_ROOM>().Where(o => o.IS_ACTIVE == 1).Where(o => o.CASHIER_ROOM_CODE.ToUpper().Equals(txtCashierRoomCode.Text.ToUpper())).ToList();
                        if (listData1 != null && listData1.Count > 0)
                        {
                            searchs = (listData1.Count == 1) ? listData1 : (listData1.Where(o => o.CASHIER_ROOM_CODE.ToUpper() == txtCashierRoomCode.Text.ToUpper()).ToList());
                        }
                        if (searchs != null && searchs.Count == 1)
                        {
                            txtCashierRoomCode.Text = searchs[0].CASHIER_ROOM_CODE;
                            cboCashierRoom.EditValue = searchs[0].ID;
        ...
        private void cboResultRoom_Closed(object sender, DevExpress.XtraEditors.Controls.ClosedEventArgs e)
        {
            try
            {
                if (e.CloseMode == PopupCloseMode.Normal)
                {
                    if (cboCashierRoom.EditValue != null)
                    {
                        var data = BackendDataWorker.Get<V_HIS_CASHIER_ROOM>().SingleOrDefault(o => o.ID == Inventec.Common.TypeConvert.Parse.ToInt64((cboCashierRoom.EditValue ?? "").ToString()));
                        if (data != null)
                        {
                            txtCashierRoomCode.Text = data.CASHIER_ROOM_CODE;
        ...
        private void chkIsNotPriority_KeyUp(object sender, KeyEventArgs e)
        {
        ...
        private void chkIsNotPriority_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keys.Enter)
