...
namespace HIS.Desktop.Plugins.AssignPrescriptionKidney.AssignPrescription
{
    class AssignPrescriptionFactory
    {
        internal static IAssignPrescription MakeIAssignPrescription(CommonParam param, object[] data)
        {
            ...
            AssignPrescriptionKidneyADO AssignPrescriptionADO = null;
            try
            {
                if (data.GetType() == typeof(object[]))
                {
                    if (data != null && data.Count() > 0)
                    {
                        ...
                        {
                            if (data[i] is AssignPrescriptionKidneyADO)
                            {
                                AssignPrescriptionADO = (AssignPrescriptionKidneyADO)data[i];
                            }
                        ...
                        result = new AssignPrescriptionBehavior(moduleData, param, (AssignPrescriptionKidneyADO)AssignPrescriptionADO);
                    }
                }
