...
{
    public partial class UCListTYTKhhList : HIS.Desktop.Utility.UserControlBase
    {
        int rowCount = 0;
        int dataTotal = 0;
        int startPage = 0;
        ...
        public UCListTYTKhhList(Inventec.Desktop.Common.Modules.Module module)
            : base(module)
        {
            InitializeComponent();
        ...
        private void gridTYTKhhListList_CustomUnboundColumnData(object sender, DevExpress.XtraGrid.Views.Base.CustomColumnDataEventArgs e)
        {
            try
            {
                if (e.IsGetData)
                {
                    ...
                    var data = (TYT_KHH)((IList)((BaseView)sender).DataSource)[e.ListSourceRowIndex];
                    ...
                    gridViewTYTKhhList.RefreshData();
        ...
        private void FillDataToGrid()
        {
            try
            {
                ...
                ucPaging1.Init(GridPaging, param, pageSize, gridControlTYTKhhList);
                WaitingManager.Hide();
            }
            catch (Exception ex)
            {
        ...
        private void GridPaging(object param)
        {
            try
            {
                ...
                ApiResultObject<List<TYT.EFMODEL.DataModels.TYT_KHH>> apiResult = null;
                TytKhhFilter filter = new TytKhhFilter();

                SetFilter(ref filter);

                gridViewTYTKhhList.BeginUpdate();

                apiResult = new Inventec.Common.Adapter.BackendAdapter
                    (paramCommon).GetRO<List<TYT_KHH>>
                    ("api/TytKhh/Get", ApiConsumers.TytConsumer, filter, paramCommon);
                if (apiResult != null)
                {
                    var data = apiResult.Data;
                    if (data != null && data.Count > 0)
                    {
                        gridControlTYTKhhList.DataSource = data;
                        rowCount = (data == null ? 0 : data.Count);
                        dataTotal = (apiResult.Param == null ? 0 : apiResult.Param.Count ?? 0);
                    }
                    else
                    {
                        gridControlTYTKhhList.DataSource = null;
                        rowCount = (data == null ? 0 : data.Count);
                        dataTotal = (apiResult.Param == null ? 0 : apiResult.Param.Count ?? 0);
                    }
                }
                gridViewTYTKhhList.EndUpdate();
        ...
        private void SetFilter(ref TytKhhFilter filter)
        {
            try
            {
                if (!string.IsNullOrEmpty(txtTYTKhhListCode.Text))
                {
                    string code = txtTYTKhhListCode.Text.Trim();
                    if (code.Length < 10 && checkDigit(code))
                    {
                        code = string.Format("{0:0000000000}", Convert.ToInt64(code));
                        txtTYTKhhListCode.Text = code;
                    }
        ...
        private void btnGTYTKhhListEdit_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            try
            {
                var row = (TYT.EFMODEL.DataModels.TYT_KHH)gridViewTYTKhhList.GetFocusedRow();
                if (row != null)
                {
                    WaitingManager.Show();
                    ...
                    CallModule callModule = new CallModule(CallModule.TYTKhhListUpdate, this.currentModule.RoomId, this.currentModule.RoomTypeId, listArgs);
                    WaitingManager.Hide();
                }
            }
            catch (Exception ex)
        ...
        private void UCListTYTKhhList_Load(object sender, EventArgs e)
        {
            try
            {
                setDefaultControl();
        ...
        private void txtTYTKhhListCode_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                if (txtTYTKhhListCode.Text != "")
                {
                    if (e.KeyCode == Keys.Enter)
                    {
                        btnSearch_Click(null, null);
        ...
        private void gridViewTYTKhhList_CustomRowCellEdit(object sender, CustomRowCellEditEventArgs e)
        {
            try
            {
                if (e.RowHandle >= 0)
                {
                    string creator = (gridViewTYTKhhList.GetRowCellValue(e.RowHandle, "CREATOR") ?? "").ToString().Trim();
                    ...
                    {
                        if (creator == loginName || HIS.Desktop.IsAdmin.CheckLoginAdmin.IsAdmin(loginName))
                            e.RepositoryItem = btnGTYTKhhListEdit;
                        else
        ...
        private void repositoryItemButton__Delete_ButtonClick(object sender, ButtonPressedEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    var row = (TYT_KHH)gridViewTYTKhhList.GetFocusedRow();
                    if (row != null)
                    {
                        WaitingManager.Show();
                        var apiresul = new Inventec.Common.Adapter.BackendAdapter
                            (param).Post<bool>
                            ("/api/TytKhh/Delete", ApiConsumers.TytConsumer, row.ID, param);
                        if (apiresul)
                        {
                            success = true;
                            FillDataToGrid();
