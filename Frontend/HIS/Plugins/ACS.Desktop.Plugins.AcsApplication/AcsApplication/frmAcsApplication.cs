...
using ACS.Desktop.Plugins.AcsApplication.Validtion;
namespace ACS.Desktop.Plugins.AcsApplication
{
    public partial class frmAcsApplication : HIS.Desktop.Utility.FormBase
    {
        #region Declare
        int rowCount = 0;
        int dataTotal = 0;
        ...
        #region Construct
        public frmAcsApplication(Inventec.Desktop.Common.Modules.Module moduleData, Inventec.Common.WebApiClient.ApiConsumer sdaConsumer, Inventec.Common.WebApiClient.ApiConsumer acsConsumer, Action<Type> delegateRefresh, long numPageSize, string applicationCode, string iconPath)
            : base(moduleData)
        {
            try
            {
        ...
        private void frmAcsApplication_Load(object sender, EventArgs e)
        {
            try
            {
                MeShow();
        ...
        private void SetCaptionByLanguageKey()
        {
            try
            {
                ...
                Resources.ResourceLanguageManager.LanguageResource = new ResourceManager("ACS.Desktop.Plugins.AcsApplication.Resources.Lang", typeof(ACS.Desktop.Plugins.AcsApplication.frmAcsApplication).Assembly);
                ...
                this.grdColAllowVersion.ToolTip = Inventec.Common.Resource.Get.Value("frmAcsApplication.grdColAllowVesion.ToolTip", Resources.ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.grdColAllowVersion.Caption = Inventec.Common.Resource.Get.Value("frmAcsApplication.grdColAllowVesion.Caption", Resources.ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.grdColIsAllowVesion.ToolTip = Inventec.Common.Resource.Get.Value("frmAcsApplication.grdColIsAllowVesion.ToolTip", Resources.ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.grdColIsAllowVesion.Caption = Inventec.Common.Resource.Get.Value("frmAcsApplication.grdColIsAllowVesion.Caption", Resources.ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
        ...
        private void LoadPaging(object param)
        {
            try
            {
                ...
                apiResult = new BackendAdapter(paramCommon).GetRO<List<ACS_APPLICATION>>(AcsRequestUriStore.ACS_APPLICATION_GET, ApiConsumers.AcsConsumer, filter, paramCommon);
                if (apiResult != null)
                {
        ...
        private void SetFilterNavBar(ref AcsApplicationViewFilter filter)
        {
            try
            {
        ...
        private void txtKeyWord_PreviewKeyDown(object sender, PreviewKeyDownEventArgs e)
        {
            try
            {
                ...
                else if (e.KeyCode == Keys.Down)
                {
                    ...
                    var rowData = (ACS_APPLICATION)gridviewFormList.GetFocusedRow();
                    if (rowData != null)
                    {
        ...
        private void gridviewFormList_CustomUnboundColumnData(object sender, DevExpress.XtraGrid.Views.Base.CustomColumnDataEventArgs e)
        {
            try
            {
                ...
                {
                    ACS_APPLICATION pData = (ACS_APPLICATION)((IList)((BaseView)sender).DataSource)[e.ListSourceRowIndex];
                    ...
                    else if (e.Column.FieldName == "IS_ALLOW_VERSION_STR")
                    {
                        e.Value = pData.IS_CHECK_VERSION == 1 ? true : false;
        ...
        private void dnNavigation_PositionChanged(object sender, EventArgs e)
        {
            try
            {
                this.currentData = (ACS_APPLICATION)(gridControlFormList.DataSource as List<ACS_APPLICATION>)[dnNavigation.Position];
        ...
        private void ChangedDataRow(ACS_APPLICATION data)
        {
            try
            {
                if (data != null)
        ...
        private void FillDataToEditorControl(ACS_APPLICATION data)
        {
            try
            {
                if (data != null)
                {
                    txtApplicationCode.EditValue = data.APPLICATION_CODE;
                    txtApplicationName.EditValue = data.APPLICATION_NAME;
                    txtAllowVesion.EditValue = data.ALLOW_VERSION;
                    chkAllowVesion.Checked = (data.IS_CHECK_VERSION == 1 ? true : false);
                }
            }
            catch (Exception ex)
            {
                ...
                try
                {
                    foreach (DevExpress.XtraLayout.BaseLayoutItem item in lcEditorInfo.Items)
                    {
                        ...
                        if (lci != null && lci.Control != null && lci.Control is BaseEdit)
                        {
                            ...
                            chkAllowVesion.CheckState = CheckState.Unchecked;
                            fomatFrm.ResetText();
        ...
        private void LoadCurrent(long currentId, ref ACS_APPLICATION currentDTO)
        {
            try
            {
                CommonParam param = new CommonParam();
                ...
                filter.ID = currentId;
                currentDTO = new BackendAdapter(param).Get<List<ACS_APPLICATION>>(AcsRequestUriStore.ACS_APPLICATION_GET, ApiConsumers.AcsConsumer, filter, param).FirstOrDefault();
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Warn(ex);
            ...
            try
            {
                if (MessageBox.Show("Bạn có muốn xóa dữ liệu không", "", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    if (rowData != null)
                    {
                        bool success = false;
                        CommonParam param = new CommonParam();
                        success = new BackendAdapter(param).Post<bool>(AcsRequestUriStore.ACS_APPLICATION_DELETE, ApiConsumers.AcsConsumer, rowData, param);
                        if (success)
                        {
                            FillDataToGridControl();
                            currentData = ((List<ACS_APPLICATION>)gridControlFormList.DataSource).FirstOrDefault();
                            if (this.delegateRefresh != null)
                                this.delegateRefresh(new ACS_APPLICATION().GetType());
                        }
        ...
        private void SaveProcess()
        {
            ...
            try
            {
                ...
                if (!dxValidationProviderEditorInfo.Validate())
                    return;
                WaitingManager.Show();
                ACS_APPLICATION updateDTO = new ACS_APPLICATION();
                if (this.currentData != null && this.currentData.ID > 0)
                {
                    LoadCurrent(this.currentData.ID, ref updateDTO);
                }
                UpdateDTOFromDataForm(ref updateDTO);
                if (chkAllowVesion.Checked)
                    updateDTO.IS_CHECK_VERSION = 1;
                else
                    updateDTO.IS_CHECK_VERSION = null;
                if (ActionType == GlobalVariables.ActionAdd)
                {
                    updateDTO.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__TRUE;
                    var resultData = new BackendAdapter(param).Post<ACS_APPLICATION>(AcsRequestUriStore.ACS_APPLICATION_CREATE, ApiConsumers.AcsConsumer, updateDTO, param);
                    if (resultData != null)
                    {
                        success = true;
                        FillDataToGridControl();
                ...
                else
                {
                    ...
                    var resultData = new BackendAdapter(param).Post<ACS_APPLICATION>(AcsRequestUriStore.ACS_APPLICATION_UPDATE, ApiConsumers.AcsConsumer, updateDTO, param);
                    if (resultData != null)
                    {
                        success = true;
                        FillDataToGridControl();
        ...
        private void UpdateRowDataAfterEdit(ACS_APPLICATION data)
        {
            try
            {
                if (data == null)
                    throw new ArgumentNullException("data(ACS_APPLICATION) is null");
                var rowData = (ACS_APPLICATION)gridviewFormList.GetFocusedRow();
                if (rowData != null)
                {
                    Inventec.Common.Mapper.DataObjectMapper.Map<ACS_APPLICATION>(rowData, data);
        ...
        private void UpdateDTOFromDataForm(ref ACS_APPLICATION currentDTO)
        {
            try
            {

                currentDTO.APPLICATION_CODE = txtApplicationCode.Text.Trim();
                currentDTO.APPLICATION_NAME = txtApplicationName.Text.Trim();
                currentDTO.ALLOW_VERSION = txtAllowVesion.Text.Trim();
                currentDTO.IS_CHECK_VERSION = (chkAllowVesion.Checked ? (short?)1 : null);
        ...
        private void ValidateForm()
        {
            try
            {
                ...
                //ValidMaxlengthtxtApplicationCode();
                //ValidMaxlengthtxtApplicationName();
                //ValidMaxlengthtxtAllowVersion();
        ...
        //private void ValidationControlTextEditApplicationCode()
        //{
        //    try
        //    {
        //        ValidMaxlengthApplicationCode validRule = new ValidMaxlengthApplicationCode();
        //        validRule.txtApplicationCode = txtApplicationCode;
        //        validRule.ErrorText = MessageUtil.GetMessage(HIS.Desktop.LibraryMessage.Message.Enum.TruongDuLieuBatBuoc);
        //        validRule.ErrorType = ErrorType.Warning;
        //        dxValidationProviderEditorInfo.SetValidationRule(txtApplicationCode, validRule);
        //    }
        //    catch (Exception ex)
        //    {
        //        Inventec.Common.Logging.LogSystem.Warn(ex);
        //    }
        //}
        //private void ValidationControlTextEditApplicatioName()
        //{
        //    try
        //    {
        //        ValidMaxlengthApplicationName validRule = new ValidMaxlengthApplicationName();
        //        validRule.txtApplicationName = txtApplicationName;
        //        validRule.ErrorText = MessageUtil.GetMessage(HIS.Desktop.LibraryMessage.Message.Enum.TruongDuLieuBatBuoc);
        //        validRule.ErrorType = ErrorType.Warning;
        //        dxValidationProviderEditorInfo.SetValidationRule(txtApplicationName, validRule);
        //    }
        //    catch (Exception ex)
        //    {
        //        Inventec.Common.Logging.LogSystem.Warn(ex);
        ...
        //void ValidMaxlengthtxtApplicationCode()
        //{
        //    try
        //    {
        //        ValidateMaxLength validateMaxLength = new ValidateMaxLength();
        //        validateMaxLength.textEdit = txtApplicationCode;
        //        validateMaxLength.maxLength = 3;
        //        validateMaxLength.ErrorType = ErrorType.Warning;
        //        dxValidationProviderEditorInfo.SetValidationRule(txtApplicationCode, validateMaxLength);
        //    }
        //    catch (Exception ex)
        //    {
        //        Inventec.Common.Logging.LogSystem.Warn(ex);
        //    }
        //}
        //void ValidMaxlengthtxtApplicationName()
        //{
        //    try
        //    {
        //        ValidateMaxLength validateMaxLength = new ValidateMaxLength();
        //        validateMaxLength.textEdit = txtApplicationName;
        //        validateMaxLength.maxLength = 100;
        //        validateMaxLength.ErrorType = ErrorType.Warning;
        //        dxValidationProviderEditorInfo.SetValidationRule(txtApplicationName, validateMaxLength);
        //    }
        //    catch (Exception ex)
        //    {
        //        Inventec.Common.Logging.LogSystem.Warn(ex);
        //    }
        //}
        //void ValidMaxlengthtxtAllowVersion()
        //{
        //    try
        //    {
        //        ValidateMaxLength validateMaxLength = new ValidateMaxLength();
        //        validateMaxLength.textEdit = txtAllowVesion ;
        //        validateMaxLength.maxLength = 200;
        //        validateMaxLength.ErrorType = ErrorType.Warning;
        //        dxValidationProviderEditorInfo.SetValidationRule(txtAllowVesion, validateMaxLength);
        //    }
        //    catch (Exception ex)
        //    {
        //        Inventec.Common.Logging.LogSystem.Warn(ex);
        ...
        {
            CommonParam param = new CommonParam();
            ACS_APPLICATION success = new ACS_APPLICATION();
            //bool notHandler = false;
            try
            {

                ACS_APPLICATION data = (ACS_APPLICATION)gridviewFormList.GetFocusedRow();
                if (MessageBox.Show("Bạn có muốn khóa dữ liệu không", "", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ACS_APPLICATION data1 = new ACS_APPLICATION();
                    data1.ID = data.ID;
                    WaitingManager.Show();
                    success = new Inventec.Common.Adapter.BackendAdapter(param).Post<ACS_APPLICATION>(AcsRequestUriStore.ACS_APPLICATION_CHANGELOCK, ApiConsumers.AcsConsumer, data1, param);
                    WaitingManager.Hide();
                    if (success != null)
                    {
                        if (this.delegateRefresh != null)
        ...
        private void gridviewFormList_CustomRowCellEdit(object sender, DevExpress.XtraGrid.Views.Grid.CustomRowCellEditEventArgs e)
        {
            try
            {
                ...
                if (e.RowHandle >= 0)
                {
                    ...
                    ACS_APPLICATION data = (ACS_APPLICATION)((IList)((BaseView)sender).DataSource)[e.RowHandle];
        ...
        private void txtApplicationCode_KeyUp(object sender, KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keys.Enter)
        ...
        private void txtApplicationName_KeyUp(object sender, KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keys.Enter)
                {
                    txtAllowVesion.Focus();
                    txtAllowVesion.SelectAll();
                }//e.Handled = true;
            }
            catch (Exception ex)
            {
        ...
        private void txtAllowVesion_KeyUp(object sender, KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keys.Enter)
                {
                    chkAllowVesion.Focus();
                }//e.Handled = true;
            }
            catch (Exception ex)
            {
        ...
        private void chkAllowVesion_KeyUp(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
