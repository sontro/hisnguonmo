...
{
    class AcsModuleGroupBehavior : BusinessBase, IAcsModuleGroup
    {
        object[] entity;
        internal AcsModuleGroupBehavior(CommonParam param, object[] filter)
            : base()
        {
            this.entity = filter;
        }

        object IAcsModuleGroup.Run()
        {
            object result = null;
            try
            {
                ...
                result = new frmAcsModuleGroup(moduleData, listApiConsumer[0], listApiConsumer[1], listActionType[0], listLong[0], listString[0], listString[1]);
            }
            catch (Exception ex)
            {
