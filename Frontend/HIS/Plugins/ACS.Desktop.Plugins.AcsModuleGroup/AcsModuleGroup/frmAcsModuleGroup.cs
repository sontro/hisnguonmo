...
{
    public partial class frmAcsModuleGroup : HIS.Desktop.Utility.FormBase
    {
        #region Declare
        int rowCount = 0;
        int dataTotal = 0;
        ...
        #region Construct
        public frmAcsModuleGroup(Inventec.Desktop.Common.Modules.Module moduleData, Inventec.Common.WebApiClient.ApiConsumer sdaConsumer, Inventec.Common.WebApiClient.ApiConsumer acsConsumer, Action<Type> delegateRefresh, long numPageSize, string applicationCode, string iconPath)
            : base(moduleData)
        {
            try
            {
        ...
        private void frmAcsModuleGroup_Load(object sender, EventArgs e)
        {
            try
            {
                MeShow();
        ...
        private void SetCaptionByLanguageKey()
        {
            ...
            {  ////Khoi tao doi tuong resource
                Resources.ResourceLanguageManager.LanguageResource = new ResourceManager("ACS.Desktop.Plugins.AcsModuleGroup.Resources.Lang", typeof(ACS.Desktop.Plugins.AcsModuleGroup.frmAcsModuleGroup).Assembly);
        ...
        private void LoadPaging(object param)
        {
            try
            {
                ...
                Inventec.Core.ApiResultObject<List<ACS_MODULE_GROUP>> apiResult = null;
                ...
                apiResult = new BackendAdapter(paramCommon).GetRO<List<ACS_MODULE_GROUP>>(AcsRequestUriStore.ACS_MODULE_GROUP_GET, ApiConsumers.AcsConsumer, filter, paramCommon);
                if (apiResult != null)
                {
                    var data = (List<ACS_MODULE_GROUP>)apiResult.Data;
                    if (data != null)
                    {
        ...
        private void SetFilterNavBar(ref AcsModuleGroupFilter filter)
        {
            try
            {
        ...
        private void txtKeyWord_PreviewKeyDown(object sender, PreviewKeyDownEventArgs e)
        {
            try
            {
                ...
                else if (e.KeyCode == Keys.Down)
                {
                    ...
                    var rowData = (ACS_MODULE_GROUP)gridviewFormList.GetFocusedRow();
                    if (rowData != null)
                    {
        ...
        private void gridviewFormList_CustomUnboundColumnData(object sender, DevExpress.XtraGrid.Views.Base.CustomColumnDataEventArgs e)
        {
            try
            {
                ...
                {
                    ACS_MODULE_GROUP pData = (ACS_MODULE_GROUP)((IList)((BaseView)sender).DataSource)[e.ListSourceRowIndex];
        ...
        private void dnNavigation_PositionChanged(object sender, EventArgs e)
        {
            try
            {
                this.currentData = (ACS_MODULE_GROUP)(gridControlFormList.DataSource as List<ACS_MODULE_GROUP>)[dnNavigation.Position];
        ...
        private void ChangedDataRow(ACS_MODULE_GROUP data)
        {
            try
            {
                if (data != null)
        ...
        private void FillDataToEditorControl(ACS_MODULE_GROUP data)
        {
            try
            {
                if (data != null)
                {
                    txtModuleGroupCode.EditValue = data.MODULE_GROUP_CODE;
                    txtModuleGroupName.EditValue = data.MODULE_GROUP_NAME;
        ...
        private void LoadCurrent(long currentId, ref ACS_MODULE_GROUP currentDTO)
        {
            try
            {
                CommonParam param = new CommonParam();
                AcsModuleGroupFilter filter = new AcsModuleGroupFilter();
                filter.ID = currentId;
                currentDTO = new BackendAdapter(param).Get<List<ACS_MODULE_GROUP>>(AcsRequestUriStore.ACS_MODULE_GROUP_GET, ApiConsumers.AcsConsumer, filter, param).FirstOrDefault();
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Warn(ex);
            ...
            try
            {
                if (MessageBox.Show("Bạn có muốn xóa dữ liệu không", "", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    if (rowData != null)
                    {
                        bool success = false;
                        CommonParam param = new CommonParam();
                        success = new BackendAdapter(param).Post<bool>(AcsRequestUriStore.ACS_MODULE_GROUP_DELETE, ApiConsumers.AcsConsumer, rowData.ID, param);
                        if (success)
                        {
                            FillDataToGridControl();
                            btnRefesh_Click(null, null);
                            currentData = ((List<ACS_MODULE_GROUP>)gridControlFormList.DataSource).FirstOrDefault();
                        }
        ...
        private void SaveProcess()
        {
            ...
            try
            {
                ...
                WaitingManager.Show();
                ACS_MODULE_GROUP updateDTO = new ACS_MODULE_GROUP();
                ...
                if (ActionType == GlobalVariables.ActionAdd)
                {
                    ...
                    var resultData = new BackendAdapter(param).Post<ACS_MODULE_GROUP>(AcsRequestUriStore.ACS_MODULE_GROUP_CREATE, ApiConsumers.AcsConsumer, updateDTO, param);
                    if (resultData != null)
                    {
                        success = true;
                        FillDataToGridControl();
                ...
                else
                {
                    ...
                    var resultData = new BackendAdapter(param).Post<ACS_MODULE_GROUP>(AcsRequestUriStore.ACS_MODULE_GROUP_UPDATE, ApiConsumers.AcsConsumer, updateDTO, param);
                    if (resultData != null)
                    {
                        success = true;
                        FillDataToGridControl();
                ...
                if (success)
                {
                    if (this.delegateRefresh != null)
                        this.delegateRefresh(new ACS_MODULE_GROUP().GetType());
                    SetFocusEditor();
                }
        ...
        private void UpdateRowDataAfterEdit(ACS_MODULE_GROUP data)
        {
            try
            {
                if (data == null)
                    throw new ArgumentNullException("data(ACS_MODULE_GROUP) is null");
                var rowData = (ACS_MODULE_GROUP)gridviewFormList.GetFocusedRow();
                if (rowData != null)
                {
                    Inventec.Common.Mapper.DataObjectMapper.Map<ACS_MODULE_GROUP>(rowData, data);
        ...
        private void UpdateDTOFromDataForm(ref ACS_MODULE_GROUP currentDTO)
        {
            try
            {

                currentDTO.MODULE_GROUP_CODE = txtModuleGroupCode.Text.Trim();
                currentDTO.MODULE_GROUP_NAME = txtModuleGroupName.Text.Trim();
                ...
                {
                    currentDTO.NUM_ORDER = Inventec.Common.TypeConvert.Parse.ToInt64(txtNumOrder.Text.Trim());
                }
        ...
        {
            CommonParam param = new CommonParam();
            ACS_MODULE_GROUP success = new ACS_MODULE_GROUP();
            //bool notHandler = false;
            try
            {

                ACS_MODULE_GROUP data = (ACS_MODULE_GROUP)gridviewFormList.GetFocusedRow();
                if (MessageBox.Show("Bạn có muốn khóa dữ liệu không", "", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ACS_MODULE_GROUP data1 = new ACS_MODULE_GROUP();
                    data1.ID = data.ID;
                    WaitingManager.Show();
                    success = new Inventec.Common.Adapter.BackendAdapter(param).Post<ACS_MODULE_GROUP>(AcsRequestUriStore.ACS_MODULE_GROUP_CHANGELOCK, ApiConsumers.AcsConsumer, data1, param);
                    WaitingManager.Hide();
                    if (success != null)
                    {
                        if (this.delegateRefresh != null)
        ...
        private void gridviewFormList_CustomRowCellEdit(object sender, DevExpress.XtraGrid.Views.Grid.CustomRowCellEditEventArgs e)
        {
            try
            {
                ...
                if (e.RowHandle >= 0)
                {
                    ...
                    ACS_MODULE_GROUP data = (ACS_MODULE_GROUP)((IList)((BaseView)sender).DataSource)[e.RowHandle];
        ...
        private void txtModuleGroupName_PreviewKeyDown(object sender, PreviewKeyDownEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keys.Enter)
        ...
        private void txtIcon_PreviewKeyDown(object sender, PreviewKeyDownEventArgs e)
        {
