...
{
    class AcsControlBehavior : BusinessBase, IAcsControl
    {
        object[] entity;
        internal AcsControlBehavior(CommonParam param, object[] filter)
            : base()
        {
            this.entity = filter;
        }

        object IAcsControl.Run()
        {
            object result = null;
            try
            {
                ...
                result = new frmAcsControl(moduleData, listApiConsumer[0], listApiConsumer[1], listActionType[0], listLong[0], listString[0], listString[1]);
            }
            catch (Exception ex)
            {
