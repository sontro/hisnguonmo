...
using Inventec.Core;
using TYT.Desktop.Plugins.NervesList.Base;
    ...
    public partial class UCTYTNervesList
    {
        ...
        private void FillDataToControl()
        {
            try
            {
                ...
                ucPaging1.Init(FillDataToGrid, param, numPageSize, gridControlNerves);
            }
            catch (Exception ex)
            {
        ...
        internal void FillDataToGrid(object param)
        {
            try
            {
                ...
                Inventec.Core.ApiResultObject<List<TYT_NERVES>> apiResult = new ApiResultObject<List<TYT_NERVES>>();
                ...
                apiResult = new BackendAdapter(paramCommon)
                    .GetRO<List<TYT_NERVES>>("api/TytNerves/Get", ApiConsumers.TytConsumer, filter, paramCommon);
                WaitingManager.Hide();
                gridControlNerves.DataSource = null;
                ...
                if (apiResult != null)
                {
                    ...
                    if (rowCount > 0)
                    {
                        gridControlNerves.DataSource = apiResult.Data;
                    }
                }
                gridViewNerves.OptionsSelection.EnableAppearanceFocusedCell = false;
                gridViewNerves.OptionsSelection.EnableAppearanceFocusedRow = false;
