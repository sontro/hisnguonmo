...
namespace HIS.Desktop.Plugins.HisAccidentHurtType
{
    public partial class frmHisAccidentHurtType : HIS.Desktop.Utility.FormBase
    {
        public frmHisAccidentHurtType(Inventec.Desktop.Common.Modules.Module moduleData)
            : base(moduleData)
        {
            InitializeComponent();
        ...
        MOS.EFMODEL.DataModels.HIS_ACCIDENT_HURT_TYPE currentData;
        MOS.EFMODEL.DataModels.HIS_ACCIDENT_HURT_TYPE resultData;
        ...
        private void SaveProcess()
        {
            ...
            CommonParam param = new CommonParam();
            bool success = false;
            HIS_ACCIDENT_HURT_TYPE updateDTO = new HIS_ACCIDENT_HURT_TYPE();
            ...
            if (this.currentData != null && this.currentData.ID > 0)
            {
                ...
                updateDTO = new BackendAdapter(param).Get<List<HIS_ACCIDENT_HURT_TYPE>>
                    (HisRequestUriStore.HIS_ACCIDENT_HURT_TYPE_GET, ApiConsumers.MosConsumer, filter, param).FirstOrDefault();
            }

            updateDTO.ACCIDENT_HURT_TYPE_CODE = txtCode.Text.Trim();
            updateDTO.ACCIDENT_HURT_TYPE_NAME = txtName.Text.Trim();

            if (ActionType == GlobalVariables.ActionAdd)
            {
                HisAccidentHurtTypeFilter filter = new HisAccidentHurtTypeFilter();
                List<HIS_ACCIDENT_HURT_TYPE> obj = new List<HIS_ACCIDENT_HURT_TYPE>();
                obj = new BackendAdapter(param).Get<List<HIS_ACCIDENT_HURT_TYPE>>
                    (HisRequestUriStore.HIS_ACCIDENT_HURT_TYPE_GET, ApiConsumers.MosConsumer, filter, param);

                foreach (var item in obj)
                {
                    if (item.ACCIDENT_HURT_TYPE_CODE == txtCode.Text.Trim())
                    {
                        MessageBox.Show("Xử lý thất bại " + "Mã " + txtCode.Text + " đã tồn tại");
                        txtCode.Focus();
                        return;
                    }
                }
                updateDTO.IS_ACTIVE = IS_ACTIVE_TRUE;
                resultData = new BackendAdapter(param).Post<HIS_ACCIDENT_HURT_TYPE>
                    (HisRequestUriStore.HIS_ACCIDENT_HURT_TYPE_CREATE, ApiConsumers.MosConsumer, updateDTO, param);
                if (resultData != null)
                {
                    success = true;
            ...
            else if (updateDTO != null)
            {
                ...
                obj = obj.Where(o => o.ACCIDENT_HURT_TYPE_CODE != currentData.ACCIDENT_HURT_TYPE_CODE).ToList();
                ...
                resultData = new BackendAdapter(param).Post<HIS_ACCIDENT_HURT_TYPE>
                    (HisRequestUriStore.HIS_ACCIDENT_HURT_TYPE_UPDATE, ApiConsumers.MosConsumer, updateDTO, param);
                if (resultData != null)
                {
                    success = true;
                    FillDataToGridControl();
            ...
            if (success)
            {
                BackendDataWorker.Reset<HIS_ACCIDENT_HURT_TYPE>();
            }
            WaitingManager.Hide();
        ...
        {
            currentData = new HIS_ACCIDENT_HURT_TYPE();
            currentData = (HIS_ACCIDENT_HURT_TYPE)gridviewFormList.GetFocusedRow();
            if (currentData != null)
            {
                txtCode.Text = currentData.ACCIDENT_HURT_TYPE_CODE;
                txtName.Text = currentData.ACCIDENT_HURT_TYPE_NAME;
        ...
        private void SetCaptionByLanguageKey()
        {
            try
            {
                ...
                Resources.ResourceLanguageManager.LanguageResource = new ResourceManager("HIS.Desktop.Plugins.HisAccidentHurtType.Resources.Lang", typeof(HIS.Desktop.Plugins.HisAccidentHurtType.frmHisAccidentHurtType).Assembly);
        ...
        private void LoadPaging(object param)
        {
            ...
            MOS.Filter.HisAccidentHurtTypeFilter filterSearch = new HisAccidentHurtTypeFilter();
            filterSearch.KEY_WORD = txtSearch.Text.Trim();
            filterSearch.ORDER_DIRECTION = "DESC";
            filterSearch.ORDER_FIELD = "MODIFY_TIME";

            Inventec.Core.ApiResultObject<List<MOS.EFMODEL.DataModels.HIS_ACCIDENT_HURT_TYPE>> apiResult = null;
            apiResult = new BackendAdapter(paramCommon).GetRO<List<MOS.EFMODEL.DataModels.HIS_ACCIDENT_HURT_TYPE>>
                (HisRequestUriStore.HIS_ACCIDENT_HURT_TYPE_GET, ApiConsumers.MosConsumer, filterSearch, paramCommon);
            if (apiResult != null)
            {
                var data = (List<MOS.EFMODEL.DataModels.HIS_ACCIDENT_HURT_TYPE>)apiResult.Data;
        ...
        #region event
        private void frmHisAccidentHurtType_Load(object sender, EventArgs e)
        {
            try
            {
                FillDataToGridControl();
        ...
        private void gridviewFormList_CustomRowCellEdit(object sender, DevExpress.XtraGrid.Views.Grid.CustomRowCellEditEventArgs e)
        {
            try
            {
                if (e.RowHandle >= 0)
                {
                    HIS_ACCIDENT_HURT_TYPE data = (HIS_ACCIDENT_HURT_TYPE)gridviewFormList.GetRow(e.RowHandle);
        ...
        private void gridviewFormList_CustomUnboundColumnData(object sender, DevExpress.XtraGrid.Views.Base.CustomColumnDataEventArgs e)
        {
            try
            {
                HIS_ACCIDENT_HURT_TYPE data = (HIS_ACCIDENT_HURT_TYPE)gridviewFormList.GetRow(e.ListSourceRowIndex);
        ...
        private void btnLock_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            ...
            {
                CommonParam param = new CommonParam();
                HIS_ACCIDENT_HURT_TYPE resultLock = new HIS_ACCIDENT_HURT_TYPE();
                bool notHandler = false;

                HIS_ACCIDENT_HURT_TYPE currentLock = (HIS_ACCIDENT_HURT_TYPE)gridviewFormList.GetFocusedRow();
                    ...
                    "", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    WaitingManager.Show();
                    resultLock = new BackendAdapter(param).Post<HIS_ACCIDENT_HURT_TYPE>
                        (HisRequestUriStore.HIS_ACCIDENT_HURT_TYPE_CHANGELOCK, ApiConsumers.MosConsumer, resultLock, param);
                    BackendDataWorker.Reset<HIS_ACCIDENT_HURT_TYPE>();
                    notHandler = true;
        ...
        private void btnUnlock_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            ...
            {
                CommonParam param = new CommonParam();
                HIS_ACCIDENT_HURT_TYPE resultUnlock = new HIS_ACCIDENT_HURT_TYPE();
                bool notHandler = false;

                HIS_ACCIDENT_HURT_TYPE currentUnlock = (HIS_ACCIDENT_HURT_TYPE)gridviewFormList.GetFocusedRow();
                    ...
                    "", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    WaitingManager.Show();
                    resultUnlock = new BackendAdapter(param).Post<HIS_ACCIDENT_HURT_TYPE>(HisRequestUriStore.HIS_ACCIDENT_HURT_TYPE_CHANGELOCK, ApiConsumers.MosConsumer,
                        resultUnlock, param);
                    BackendDataWorker.Reset<HIS_ACCIDENT_HURT_TYPE>();
                    notHandler = true;
                    MessageManager.Show(this.ParentForm, param, notHandler);
        ...
        private void btnDelete_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            try
            {
                HIS_ACCIDENT_HURT_TYPE currentDelete = (HIS_ACCIDENT_HURT_TYPE)gridviewFormList.GetFocusedRow();
                if (currentDelete != null)
                {
                        ...
                        "", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        ...
                        (HisRequestUriStore.HIS_ACCIDENT_HURT_TYPE_DELETE, ApiConsumers.MosConsumer, currentDelete, param);
                        if (success)
                        {
                            FillDataToGridControl();
                            BackendDataWorker.Reset<HIS_ACCIDENT_HURT_TYPE>();
