...
using HIS.Desktop.Plugins.HisWorkPlace.Resources;
  ...
  public  class ValidateMaxLength : DevExpress.XtraEditors.DXErrorProvider.ValidationRule
    {
        ...
        public override bool Validate(Control control, object value)
        {
            ...
            try
            {
                ...
                {
                    this.ErrorText = String.Format(ResourceMessage.TruongDuLieuBatBuoc);
                    this.ErrorType = ErrorType.Warning;
                    return valid;
                }
                if (maxLength != null && !String.IsNullOrEmpty(textEdit.Text) && Inventec.Common.String.CountVi.Count(textEdit.Text) > maxLength)
                {
                    this.ErrorText = String.Format("Trường dữ liệu vượt quá kí tự cho phép ({0} kí tự) ",maxLength) ;
                    this.ErrorType = ErrorType.Warning;
                    return valid;
                }
                valid = true;
