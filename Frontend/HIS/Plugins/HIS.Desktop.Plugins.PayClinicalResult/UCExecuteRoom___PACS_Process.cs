...
using HIS.Desktop.Plugins.PayClinicalResult.PACS;
...
namespace HIS.Desktop.Plugins.PayClinicalResult
{
    public partial class UCExecuteRoom : HIS.Desktop.Utility.UserControlBase
    {
        ...
        private List<HIS.Desktop.Plugins.PayClinicalResult.PACS.ADO.ImagesADO> LoadImageFromPacsService(string soPhieu)
        {
            List<HIS.Desktop.Plugins.PayClinicalResult.PACS.ADO.ImagesADO> result = null;
            try
            {
                CommonParam param = new CommonParam();

                HIS.Desktop.Plugins.PayClinicalResult.PACS.ADO.ImageRequestADO layThongTinAnhInputADO = new HIS.Desktop.Plugins.PayClinicalResult.PACS.ADO.ImageRequestADO();
                ...
                var resultData = new Inventec.Common.Adapter.BackendAdapter(param).PostWithouApiParam<HIS.Desktop.Plugins.PayClinicalResult.PACS.ADO.ImageResponseADO>(RequestUriStore.PACS_SERIVCE__LAY_THONG_TIN_ANH, PacsApiConsumer.PacsConsumer, layThongTinAnhInputADO, null, param);

                if (resultData != null && resultData.TrangThai && resultData.Series != null)
                {
                    result = new List<HIS.Desktop.Plugins.PayClinicalResult.PACS.ADO.ImagesADO>();
