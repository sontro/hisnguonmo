...
{
  public partial class frmHisBornResult : HIS.Desktop.Utility.FormBase
  {
    public frmHisBornResult(Inventec.Desktop.Common.Modules.Module moduleData)
	:base(moduleData)
    {
      InitializeComponent();
    ...
    MOS.EFMODEL.DataModels.HIS_BORN_RESULT currentData;
    MOS.EFMODEL.DataModels.HIS_BORN_RESULT resultData;
    ...
    private void SaveProcess()
    {
      ...
      CommonParam param = new CommonParam();
      bool success = false;
      HIS_BORN_RESULT updateDTO = new HIS_BORN_RESULT();
      ...
      if (this.currentData != null && this.currentData.ID > 0)
      {
        ...
        updateDTO = new BackendAdapter(param).Get<List<HIS_BORN_RESULT>>
          (HisRequestUriStore.HIS_BORN_RESULT_GET, ApiConsumers.MosConsumer, filter, param).FirstOrDefault();
      }

      updateDTO.BORN_RESULT_CODE = txtCode.Text.Trim();
      updateDTO.BORN_RESULT_NAME = txtName.Text.Trim();

      if (ActionType == GlobalVariables.ActionAdd)
      {
        updateDTO.IS_ACTIVE = IS_ACTIVE_TRUE;
        resultData = new BackendAdapter(param).Post<HIS_BORN_RESULT>
          (HisRequestUriStore.HIS_BORN_RESULT_CREATE, ApiConsumers.MosConsumer, updateDTO, param);
        if (resultData != null)
        {
          success = true;
      ...
      {
        resultData = new BackendAdapter(param).Post<HIS_BORN_RESULT>
          (HisRequestUriStore.HIS_BORN_RESULT_UPDATE, ApiConsumers.MosConsumer, updateDTO, param);
        if (resultData != null)
        {
          success = true;
          FillDataToGridControl();
    ...
    {
      currentData = new HIS_BORN_RESULT();
      currentData = (HIS_BORN_RESULT)gridviewFormList.GetFocusedRow();
      if (currentData != null)
      {
        txtCode.Text = currentData.BORN_RESULT_CODE;
        txtName.Text = currentData.BORN_RESULT_NAME;
    ...
    private void LoadPaging(object param)
    {
      ...
      MOS.Filter.HisBornResultFilter filterSearch = new HisBornResultFilter();
      filterSearch.KEY_WORD = txtSearch.Text.Trim();

      Inventec.Core.ApiResultObject<List<MOS.EFMODEL.DataModels.HIS_BORN_RESULT>> apiResult = null;
      apiResult = new BackendAdapter(paramCommon).GetRO<List<MOS.EFMODEL.DataModels.HIS_BORN_RESULT>>
        (HisRequestUriStore.HIS_BORN_RESULT_GET, ApiConsumers.MosConsumer, filterSearch, paramCommon);
      if (apiResult != null)
      {
        var data = (List<MOS.EFMODEL.DataModels.HIS_BORN_RESULT>)apiResult.Data;
    ...
    #region event
    private void frmHisBornResult_Load(object sender, EventArgs e)
    {
      try
      {
        FillDataToGridControl();
    ...
    private void gridviewFormList_CustomRowCellEdit(object sender, DevExpress.XtraGrid.Views.Grid.CustomRowCellEditEventArgs e)
    {
      try
      {
        if (e.RowHandle >= 0)
        {
          HIS_BORN_RESULT data = (HIS_BORN_RESULT)gridviewFormList.GetRow(e.RowHandle);
    ...
    private void gridviewFormList_CustomUnboundColumnData(object sender, DevExpress.XtraGrid.Views.Base.CustomColumnDataEventArgs e)
    {
      try
      {
        HIS_BORN_RESULT data = (HIS_BORN_RESULT)gridviewFormList.GetRow(e.ListSourceRowIndex);
    ...
    private void btnLock_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
    {
      ...
      {
        CommonParam param = new CommonParam();
        HIS_BORN_RESULT resultLock = new HIS_BORN_RESULT();
        bool notHandler = false;

        HIS_BORN_RESULT currentLock = (HIS_BORN_RESULT)gridviewFormList.GetFocusedRow();
          ...
          "", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
        {
          ...
          WaitingManager.Show();
          resultLock = new BackendAdapter(param).Post<HIS_BORN_RESULT>
            (HisRequestUriStore.HIS_BORN_RESULT_CHANGELOCK, ApiConsumers.MosConsumer, currentLock.ID, param);
    ...
    private void btnUnlock_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
    {
      ...
      {
        CommonParam param = new CommonParam();
        HIS_BORN_RESULT resultUnlock = new HIS_BORN_RESULT();
        bool notHandler = false;

        HIS_BORN_RESULT currentUnlock = (HIS_BORN_RESULT)gridviewFormList.GetFocusedRow();
          ...
          "", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
        {
          ...
          WaitingManager.Show();
          resultUnlock = new BackendAdapter(param).Post<HIS_BORN_RESULT>(HisRequestUriStore.HIS_BORN_RESULT_CHANGELOCK, ApiConsumers.MosConsumer,
    ...
    private void btnDelete_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
    {
      try
      {
        HIS_BORN_RESULT currentDelete = (HIS_BORN_RESULT)gridviewFormList.GetFocusedRow();
        if (currentDelete != null)
        {
            ...
            "", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
          {
            ...
            (HisRequestUriStore.HIS_BORN_RESULT_DELETE, ApiConsumers.MosConsumer, currentDelete.ID, param);
            if (success)
              FillDataToGridControl();
            MessageManager.Show(this, param, success);
          }
