...
namespace HIS.Desktop.Plugins.Library.PrintOtherForm.MpsBehavior.Mps000416_1_
{
    public partial class Mps000416Behavior : MpsDataBase, ILoad
    {
        private V_HIS_TREATMENT treatment { get; set; }
        private HIS_DHST dhst { get; set; }

        public Mps000416Behavior(long _treatmentId)
            : base()
        {
            ...
            try
            {
                ...
                if (this.treatment != null)
                {
                    ...
                    if (this.treatment.CLINICAL_IN_TIME.HasValue)
                    {
                        ...
                        dicParamPlus.Add("CLINICAL_IN_TIME_STR", Inventec.Common.DateTime.Convert.TimeNumberToDateString(this.treatment.CLINICAL_IN_TIME.Value));
                    }
                    else
                    {
