    ...
    public partial class Mps000227Behavior : MpsDataBase, ILoad
    {
        ...
        private void SetDicParamSereServPTTT(ref Dictionary<string, object> dicParamPlus)
        {
            try
            {
                ...
                if (dicParamPlus.ContainsKey("BEFORE_PTTT_ICD_TEXT") && dicParamPlus["BEFORE_PTTT_ICD_TEXT"] != null)
                {
                    dicParamPlus["BEFORE_PTTT_ICD_NAME"] = dicParamPlus["BEFORE_PTTT_ICD_TEXT"];
                }

                if (dicParamPlus.ContainsKey("AFTER_PTTT_ICD_TEXT") && dicParamPlus["AFTER_PTTT_ICD_TEXT"] != null)
                {
        ...
        private void SetDicImagePlusSereServFile(ref Dictionary<string, System.Drawing.Image> dicImagePlus)
        {
            try
            {
                if (this.SereServFiles != null && this.SereServFiles.Count > 0)
                {
                    for (int i = 0; i < this.SereServFiles.Count; i++)
                    {
                        MemoryStream stream = Inventec.Fss.Client.FileDownload.GetFile(this.SereServFiles[i].URL);
                        Image Img = Image.FromStream(stream);
                        dicImagePlus.Add("DIAGRAM_IMAGE_" + (i + 1), Img);
                    }
                }
            }
            catch (Exception ex)
