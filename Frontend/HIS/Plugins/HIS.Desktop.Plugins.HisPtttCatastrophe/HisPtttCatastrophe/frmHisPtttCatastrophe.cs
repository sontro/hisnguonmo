...
{
    public partial class frmHisPtttCatastrophe : HIS.Desktop.Utility.FormBase
    {
        #region Declare
        int rowCount = 0;
        int dataTotal = 0;
        ...
        MOS.EFMODEL.DataModels.HIS_PTTT_CATASTROPHE currentData;
        ...
        #region Construct
        public frmHisPtttCatastrophe(Inventec.Desktop.Common.Modules.Module moduleData)
		:base(moduleData)
        {
            try
            {
        ...
        private void frmHisPtttCatastrophe_Load(object sender, EventArgs e)
        {
            try
            {
                MeShow();
        ...
        private void SetCaptionByLanguageKey()
        {
            try
            {
                ...
                Resources.ResourceLanguageManager.LanguageResource = new ResourceManager("HIS.Desktop.Plugins.HisPtttCatastrophe.Resources.Lang", typeof(HIS.Desktop.Plugins.HisPtttCatastrophe.frmHisPtttCatastrophe).Assembly);
        ...
        private void InitTabIndex()
        {
            try
            {
                dicOrderTabIndexControl.Add("txtPtttCatastropheCode", 0);
                dicOrderTabIndexControl.Add("txtPtttCatastropheName", 1);
        ...
        private void LoadPaging(object param)
        {
            try
            {
                ...
                Inventec.Core.ApiResultObject<List<MOS.EFMODEL.DataModels.HIS_PTTT_CATASTROPHE>> apiResult = null;
                ...
                apiResult = new BackendAdapter(paramCommon).GetRO<List<MOS.EFMODEL.DataModels.HIS_PTTT_CATASTROPHE>>(HisRequestUriStore.MOSHIS_PTTT_CATASTROPHE_GET, ApiConsumers.MosConsumer, filter, paramCommon);
                if (apiResult != null)
                {
                    var data = (List<MOS.EFMODEL.DataModels.HIS_PTTT_CATASTROPHE>)apiResult.Data;
                    if (data != null)
                    {
        ...
        private void SetFilterNavBar(ref HisPtttCatastropheFilter filter)
        {
            try
            {
        ...
        private void txtKeyWord_PreviewKeyDown(object sender, PreviewKeyDownEventArgs e)
        {
            try
            {
                ...
                else if (e.KeyCode == Keys.Down)
                {
                    ...
                    var rowData = (MOS.EFMODEL.DataModels.HIS_PTTT_CATASTROPHE)gridviewFormList.GetFocusedRow();
                    if (rowData != null)
                    {
        ...
        private void gridviewFormList_CustomUnboundColumnData(object sender, DevExpress.XtraGrid.Views.Base.CustomColumnDataEventArgs e)
        {
            try
            {
                ...
                {
                    MOS.EFMODEL.DataModels.HIS_PTTT_CATASTROPHE pData = (MOS.EFMODEL.DataModels.HIS_PTTT_CATASTROPHE)((IList)((BaseView)sender).DataSource)[e.ListSourceRowIndex];
        ...
        private void dnNavigation_PositionChanged(object sender, EventArgs e)
        {
            try
            {
                this.currentData = (MOS.EFMODEL.DataModels.HIS_PTTT_CATASTROPHE)(gridControlFormList.DataSource as List<MOS.EFMODEL.DataModels.HIS_PTTT_CATASTROPHE>)[dnNavigation.Position];
        ...
        private void ChangedDataRow(MOS.EFMODEL.DataModels.HIS_PTTT_CATASTROPHE data)
        {
            try
            {
                if (data != null)
        ...
        private void FillDataToEditorControl(MOS.EFMODEL.DataModels.HIS_PTTT_CATASTROPHE data)
        {
            try
            {
                if (data != null)
                {
                    txtPtttCatastropheCode.Text = data.PTTT_CATASTROPHE_CODE;
                    txtPtttCatastropheName.Text = data.PTTT_CATASTROPHE_NAME;
        ...
        private void LoadCurrent(long currentId, ref MOS.EFMODEL.DataModels.HIS_PTTT_CATASTROPHE currentDTO)
        {
            try
            {
                CommonParam param = new CommonParam();
                HisPtttCatastropheFilter filter = new HisPtttCatastropheFilter();
                filter.ID = currentId;
                currentDTO = new BackendAdapter(param).Get<List<MOS.EFMODEL.DataModels.HIS_PTTT_CATASTROPHE>>(HisRequestUriStore.MOSHIS_PTTT_CATASTROPHE_GET, ApiConsumers.MosConsumer, filter, param).FirstOrDefault();
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Warn(ex);
        ...
        private void btnGDelete_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            try
            {
                ...
                var data = new BackendAdapter(param).Get<List<HIS_PTTT_CATASTROPHE>>(HisRequestUriStore.MOSHIS_PTTT_CATASTROPHE_GET, ApiConsumers.MosConsumer, filter, param).FirstOrDefault();

                if (rowData != null)
                {
                    bool success = false;
                    success = new BackendAdapter(param).Post<bool>(HisRequestUriStore.MOSHIS_PTTT_CATASTROPHE_DELETE, ApiConsumers.MosConsumer, data, param);
                    if (success)
                    {
                        FillDataToGridControl();
                        currentData = ((List<HIS_PTTT_CATASTROPHE>)gridControlFormList.DataSource).FirstOrDefault();
                        BackendDataWorker.Reset<HIS_PTTT_CATASTROPHE>();
                    }
        ...
        private void SaveProcess()
        {
            ...
            try
            {
                ...
                WaitingManager.Show();
                MOS.EFMODEL.DataModels.HIS_PTTT_CATASTROPHE updateDTO = new MOS.EFMODEL.DataModels.HIS_PTTT_CATASTROPHE();
                ...
                if (ActionType == GlobalVariables.ActionAdd)
                {
                    ...
                    var resultData = new BackendAdapter(param).Post<MOS.EFMODEL.DataModels.HIS_PTTT_CATASTROPHE>(HisRequestUriStore.MOSHIS_PTTT_CATASTROPHE_CREATE, ApiConsumers.MosConsumer, updateDTO, param);
                    if (resultData != null)
                    {
                        success = true;
                        FillDataToGridControl();
                ...
                {
                    var resultData = new BackendAdapter(param).Post<MOS.EFMODEL.DataModels.HIS_PTTT_CATASTROPHE>(HisRequestUriStore.MOSHIS_PTTT_CATASTROPHE_UPDATE, ApiConsumers.MosConsumer, updateDTO, param);
                    if (resultData != null)
                    {
                        success = true;
                        FillDataToGridControl();
        ...
        private void UpdateRowDataAfterEdit(MOS.EFMODEL.DataModels.HIS_PTTT_CATASTROPHE data)
        {
            try
            {
                if (data == null)
                    throw new ArgumentNullException("data(MOS.EFMODEL.DataModels.HIS_PTTT_CATASTROPHE) is null");
                var rowData = (MOS.EFMODEL.DataModels.HIS_PTTT_CATASTROPHE)gridviewFormList.GetFocusedRow();
                if (rowData != null)
                {
                    Inventec.Common.Mapper.DataObjectMapper.Map<MOS.EFMODEL.DataModels.HIS_PTTT_CATASTROPHE>(rowData, data);
        ...
        private void UpdateDTOFromDataForm(ref MOS.EFMODEL.DataModels.HIS_PTTT_CATASTROPHE currentDTO)
        {
            try
            {
                currentDTO.PTTT_CATASTROPHE_CODE = txtPtttCatastropheCode.Text.Trim();
                currentDTO.PTTT_CATASTROPHE_NAME = txtPtttCatastropheName.Text.Trim();
        ...
        private void ValidateForm()
        {
            try
            {
                ValidationSingleControl(txtPtttCatastropheCode);
                ValidationSingleControl(txtPtttCatastropheName);
        ...
        private void btnGLock_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            ...
            CommonParam param = new CommonParam();
            HIS_PTTT_CATASTROPHE success = new HIS_PTTT_CATASTROPHE();
            //bool notHandler = false;
            try
            {

                HIS_PTTT_CATASTROPHE data = (HIS_PTTT_CATASTROPHE)gridviewFormList.GetFocusedRow();
                if (MessageBox.Show(LibraryMessage.MessageUtil.GetMessage(LibraryMessage.Message.Enum.HeThongTBCuaSoThongBaoBanCoMuonBoKhoaDuLieuKhong), "", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    HIS_PTTT_CATASTROPHE data1 = new HIS_PTTT_CATASTROPHE();
                    data1.ID = data.ID;
                    WaitingManager.Show();
                    success = new Inventec.Common.Adapter.BackendAdapter(param).Post<HIS_PTTT_CATASTROPHE>(HisRequestUriStore.MOSHIS_PTTT_CATASTROPHE_CHANGE_LOCK, ApiConsumers.MosConsumer, data1, param);
                    WaitingManager.Hide();
                    if (success != null)
                    {
                        BackendDataWorker.Reset<HIS_PTTT_CATASTROPHE>();
        ...
        private void gridviewFormList_CustomRowCellEdit(object sender, DevExpress.XtraGrid.Views.Grid.CustomRowCellEditEventArgs e)
        {
            try
            {
                ...
                if (e.RowHandle >= 0)
                {
                    HIS_PTTT_CATASTROPHE data = (HIS_PTTT_CATASTROPHE)((IList)((BaseView)sender).DataSource)[e.RowHandle];
