...
{
    public partial class frmHisFund : HIS.Desktop.Utility.FormBase
    {
        #region Declare
        int rowCount = 0;
        int dataTotal = 0;
        ...
        MOS.EFMODEL.DataModels.HIS_FUND currentData;
        ...
        #region Construct
        public frmHisFund(Inventec.Desktop.Common.Modules.Module moduleData)
            : base(moduleData)
        {
            try
            {
        ...
        private void frmHisFund_Load(object sender, EventArgs e)
        {
            try
            {
                MeShow();
        ...
        private void SetCaptionByLanguageKey()
        {
            try
            {
                ...
                Resources.ResourceLanguageManager.LanguageResource = new ResourceManager("HIS.Desktop.Plugins.HisFund.Resources.Lang", typeof(HIS.Desktop.Plugins.HisFund.frmHisFund).Assembly);
        ...
        private void InitTabIndex()
        {
            try
            {
                dicOrderTabIndexControl.Add("txtFundCode", 0);
                dicOrderTabIndexControl.Add("txtFundName", 1);
        ...
        private void LoadPaging(object param)
        {
            try
            {
                ...
                Inventec.Core.ApiResultObject<List<MOS.EFMODEL.DataModels.HIS_FUND>> apiResult = null;
                ...
                apiResult = new BackendAdapter(paramCommon).GetRO<List<MOS.EFMODEL.DataModels.HIS_FUND>>(HisRequestUriStore.MOSHIS_FUND_GET, ApiConsumers.MosConsumer, filter, paramCommon);
                if (apiResult != null)
                {
                    var data = (List<MOS.EFMODEL.DataModels.HIS_FUND>)apiResult.Data;
                    if (data != null)
                    {
        ...
        private void SetFilterNavBar(ref HisFundFilter filter)
        {
            try
            {
        ...
        private void txtKeyWord_PreviewKeyDown(object sender, PreviewKeyDownEventArgs e)
        {
            try
            {
                ...
                else if (e.KeyCode == Keys.Down)
                {
                    ...
                    var rowData = (MOS.EFMODEL.DataModels.HIS_FUND)gridviewFormList.GetFocusedRow();
                    if (rowData != null)
                    {
        ...
        private void gridviewFormList_CustomUnboundColumnData(object sender, DevExpress.XtraGrid.Views.Base.CustomColumnDataEventArgs e)
        {
            try
            {
                ...
                {
                    MOS.EFMODEL.DataModels.HIS_FUND pData = (MOS.EFMODEL.DataModels.HIS_FUND)((IList)((BaseView)sender).DataSource)[e.ListSourceRowIndex];
        ...
        private void dnNavigation_PositionChanged(object sender, EventArgs e)
        {
            try
            {
                this.currentData = (MOS.EFMODEL.DataModels.HIS_FUND)(gridControlFormList.DataSource as List<MOS.EFMODEL.DataModels.HIS_FUND>)[dnNavigation.Position];
        ...
        private void ChangedDataRow(MOS.EFMODEL.DataModels.HIS_FUND data)
        {
            try
            {
                if (data != null)
        ...
        private void FillDataToEditorControl(MOS.EFMODEL.DataModels.HIS_FUND data)
        {
            try
            {
                if (data != null)
                {
                    txtFundCode.Text = data.FUND_CODE;
                    txtFundName.Text = data.FUND_NAME;
        ...
        private void LoadCurrent(long currentId, ref MOS.EFMODEL.DataModels.HIS_FUND currentDTO)
        {
            try
            {
                CommonParam param = new CommonParam();
                HisFundFilter filter = new HisFundFilter();
                filter.ID = currentId;
                currentDTO = new BackendAdapter(param).Get<List<MOS.EFMODEL.DataModels.HIS_FUND>>(HisRequestUriStore.MOSHIS_FUND_GET, ApiConsumers.MosConsumer, filter, param).FirstOrDefault();
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Warn(ex);
        ...
        private void btnGDelete_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            try
            {
                ...
                var data = new BackendAdapter(param).Get<List<HIS_FUND>>(HisRequestUriStore.MOSHIS_FUND_GET, ApiConsumers.MosConsumer, filter, param).FirstOrDefault();

                if (rowData != null)
                {
                    bool success = false;
                    success = new BackendAdapter(param).Post<bool>(HisRequestUriStore.MOSHIS_FUND_DELETE, ApiConsumers.MosConsumer, data.ID, param);
                    if (success)
                    {
                        BackendDataWorker.Reset<HIS_FUND>();
                        FillDataToGridControl();
                        currentData = ((List<HIS_FUND>)gridControlFormList.DataSource).FirstOrDefault();
                    }
        ...
        private void SaveProcess()
        {
            ...
            try
            {
                ...
                WaitingManager.Show();
                MOS.EFMODEL.DataModels.HIS_FUND updateDTO = new MOS.EFMODEL.DataModels.HIS_FUND();
                ...
                if (ActionType == GlobalVariables.ActionAdd)
                {
                    ...
                    var resultData = new BackendAdapter(param).Post<MOS.EFMODEL.DataModels.HIS_FUND>(HisRequestUriStore.MOSHIS_FUND_CREATE, ApiConsumers.MosConsumer, updateDTO, param);
                    if (resultData != null)
                    {
                        success = true;
                        FillDataToGridControl();
                ...
                {
                    var resultData = new BackendAdapter(param).Post<MOS.EFMODEL.DataModels.HIS_FUND>(HisRequestUriStore.MOSHIS_FUND_UPDATE, ApiConsumers.MosConsumer, updateDTO, param);
                    if (resultData != null)
                    {
                        success = true;
                        FillDataToGridControl();
        ...
        private void UpdateRowDataAfterEdit(MOS.EFMODEL.DataModels.HIS_FUND data)
        {
            try
            {
                if (data == null)
                    throw new ArgumentNullException("data(MOS.EFMODEL.DataModels.HIS_FUND) is null");
                var rowData = (MOS.EFMODEL.DataModels.HIS_FUND)gridviewFormList.GetFocusedRow();
                if (rowData != null)
                {
                    Inventec.Common.Mapper.DataObjectMapper.Map<MOS.EFMODEL.DataModels.HIS_FUND>(rowData, data);
        ...
        private void UpdateDTOFromDataForm(ref MOS.EFMODEL.DataModels.HIS_FUND currentDTO)
        {
            try
            {
                currentDTO.FUND_CODE = txtFundCode.Text.Trim();
                currentDTO.FUND_NAME = txtFundName.Text.Trim();
        ...
        private void ValidateForm()
        {
            try
            {
                ValidationSingleControl(txtFundCode);
                ValidationSingleControl(txtFundName);
        ...
        private void btnGLock_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            ...
            CommonParam param = new CommonParam();
            HIS_FUND success = new HIS_FUND();
            //bool notHandler = false;
            try
            {

                HIS_FUND data = (HIS_FUND)gridviewFormList.GetFocusedRow();
                if (MessageBox.Show(LibraryMessage.MessageUtil.GetMessage(LibraryMessage.Message.Enum.HeThongTBCuaSoThongBaoBanCoMuonBoKhoaDuLieuKhong), "", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    HIS_FUND data1 = new HIS_FUND();
                    data1.ID = data.ID;
                    WaitingManager.Show();
                    success = new Inventec.Common.Adapter.BackendAdapter(param).Post<HIS_FUND>(HisRequestUriStore.MOSHIS_FUND_CHANGE_LOCK, ApiConsumers.MosConsumer, data1, param);
                    WaitingManager.Hide();
                    if (success != null)
                    {
                        BackendDataWorker.Reset<HIS_FUND>();
        ...
        private void gridviewFormList_CustomRowCellEdit(object sender, DevExpress.XtraGrid.Views.Grid.CustomRowCellEditEventArgs e)
        {
            try
            {
                ...
                if (e.RowHandle >= 0)
                {
                    HIS_FUND data = (HIS_FUND)((IList)((BaseView)sender).DataSource)[e.RowHandle];
        ...
        private void repositoryItembtnTreatment_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            try
            {
                WaitingManager.Show();
                List<object> listArgs = new List<object>();
                if (this.gridviewFormList.FocusedRowHandle >= 0)
                {
                    var data = (HIS_FUND)gridviewFormList.GetFocusedRow();

                    listArgs.Add((long)data.ID);
                }
                if (this.moduleData != null)
                {
                    CallModule callModule = new CallModule(CallModule.TreatmentFundList, moduleData.RoomId, moduleData.RoomTypeId, listArgs);
                }
                else
                {
                    CallModule callModule = new CallModule(CallModule.TreatmentFundList, 0, 0, listArgs);
                }
                WaitingManager.Hide();
            }
            catch (Exception ex)
