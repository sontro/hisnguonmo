...
using MOS.SDO;
using HIS.Desktop.Plugins.HisFuexServiceType.Resources;
...
namespace HIS.Desktop.Plugins.HisFuexServiceType.HisFuexServiceType
{
    public partial class frmHisSuimServiceType : HIS.Desktop.Utility.FormBase
    {
        ...
        MOS.EFMODEL.DataModels.V_HIS_FUEX_SERVICE_TYPE currentData;
        ...
        V_HIS_FUEX_SERVICE_TYPE currentDataSet = null;
        #endregion
        ...
        private void SetCaptionByLanguageKey()
        {
            try
            {
                ...
                Resources.ResourceLanguageManager.LanguageResource = new ResourceManager("HIS.Desktop.Plugins.HisFuexServiceType.Resources.Lang", typeof(HIS.Desktop.Plugins.HisFuexServiceType.HisFuexServiceType.frmHisSuimServiceType).Assembly);
                ...
                this.treeListColumn17.Caption = Inventec.Common.Resource.Get.Value("frmHisFuexServiceType.treeListColumn17.Caption", Resources.ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                ...
                this.layoutControlItem14.Text = Inventec.Common.Resource.Get.Value("frmHisFuexServiceType.layoutControlItem14.Text", Resources.ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Warn(ex);
        ...
        private void initComboCha()
        {
            ...
            try
            {
                ...
                HisFuexServiceTypeFilter filter = new HisFuexServiceTypeFilter();
                filter.IS_ACTIVE = 1;
                var serviceTypeParents = new BackendAdapter(param)
                    .Get<List<HIS_FUEX_SERVICE_TYPE>>(HisRequestUriStore.HIS_FUEX_SERVICE_TYPE_GET, ApiConsumers.MosConsumer, filter, param);
                List<ColumnInfo> columnInfos = new List<ColumnInfo>();
                columnInfos.Add(new ColumnInfo("FUEX_SERVICE_TYPE_CODE", "", 100, 1));
                columnInfos.Add(new ColumnInfo("FUEX_SERVICE_TYPE_NAME", "", 250, 2));
                ControlEditorADO controlEditorADO = new ControlEditorADO("FUEX_SERVICE_TYPE_NAME", "ID", columnInfos, false, 350);
        ...
        private void ChangedDataRow(MOS.EFMODEL.DataModels.V_HIS_FUEX_SERVICE_TYPE data)
        {
            try
            {
                if (data != null)
        ...
        private void FillDataToEditorControl(MOS.EFMODEL.DataModels.V_HIS_FUEX_SERVICE_TYPE data)
        {
            try
            {
                if (data != null)
                {
                    ...
                    suimId = data.ID;
                    txtSuimServiceTypeCode.Text = data.FUEX_SERVICE_TYPE_CODE;
                    txtSuimServiceTypeName.Text = data.FUEX_SERVICE_TYPE_NAME;
        ...
        private void LoadCurrent(long currentId, ref MOS.EFMODEL.DataModels.V_HIS_FUEX_SERVICE_TYPE currentDTO)
        {
            try
            {
                CommonParam param = new CommonParam();
                ...
                filter.ID = currentId;
                currentData = new BackendAdapter(param).Get<List<MOS.EFMODEL.DataModels.V_HIS_FUEX_SERVICE_TYPE>>(HisRequestUriStore.MOSHIS_SUIM_SERVICE_TYPE_GET, ApiConsumers.MosConsumer, filter, param).FirstOrDefault();
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Warn(ex);
        ...
        private void SaveProcess()
        {
            ...
            try
            {
                ...
                WaitingManager.Show();
                MOS.SDO.HisFuexServiceTypeSDO serviceSDO = new MOS.SDO.HisFuexServiceTypeSDO();
                MOS.SDO.HisFuexServiceTypeSDO suimServiceTypeResultSDO = new MOS.SDO.HisFuexServiceTypeSDO();

                serviceSDO.HisService = setDataService();
                serviceSDO.HisFuexServiceType = setDataFuexServiceType();
                if (ActionType == GlobalVariables.ActionAdd)
                {
                    suimServiceTypeResultSDO = new BackendAdapter(param).Post<HisFuexServiceTypeSDO>(HisRequestUriStore.MOSHIS_SUIM_SERVICE_TYPE_CREATE, ApiConsumers.MosConsumer, serviceSDO, param);
                }
                else
                {
                    if (serviceId > 0 && suimId > 0)
                    {
                        ...
                        serviceSDO.HisFuexServiceType.ID = suimId;
                        suimServiceTypeResultSDO = new BackendAdapter(param).Post<HisFuexServiceTypeSDO>(HisRequestUriStore.MOSHIS_SUIM_SERVICE_TYPE_UPDATE, ApiConsumers.MosConsumer, serviceSDO, param);
                    }
                }
        ...
        private void LoaddataToTreeList()
        {
            ...
            {
                CommonParam param = new CommonParam();
                MOS.Filter.HisFuexServiceTypeViewFilter filter = new MOS.Filter.HisFuexServiceTypeViewFilter();
                filter.KEY_WORD = txtKeyword.Text;
                var currentData = new BackendAdapter(param).Get<List<V_HIS_FUEX_SERVICE_TYPE>>("api/HisFuexServiceType/GetView", ApiConsumers.MosConsumer, filter, param).ToList();
                if (currentData != null)
                {
        ...
        private void TreeListSuimServiceType_CustomNodeCellEdit(object sender, DevExpress.XtraTreeList.GetCustomNodeCellEditEventArgs e)
        {
            try
            {
                ...
                if (data != null && data is V_HIS_FUEX_SERVICE_TYPE)
                {
                    V_HIS_FUEX_SERVICE_TYPE rowData = data as V_HIS_FUEX_SERVICE_TYPE;
        ...
        private void TreeListSuimServiceType_CustomUnboundColumnData(object sender, DevExpress.XtraTreeList.TreeListCustomColumnDataEventArgs e)
        {
            try
            {
                if (e.IsGetData)
                {
                    V_HIS_FUEX_SERVICE_TYPE pData = e.Row as V_HIS_FUEX_SERVICE_TYPE;
        ...
        private HIS_SERVICE setDataService()
        {
            ...
            try
            {
                ...
                service.SERVICE_TYPE_ID = HisServiceTypeCFG.SERVICE_TYPE_ID__FUEX;
                if (cboDV.EditValue != null && !String.IsNullOrEmpty(cboDV.Text)) service.HEIN_SERVICE_ID = Inventec.Common.TypeConvert.Parse.ToInt64((cboDV.EditValue).ToString());
        ...
        private HIS_FUEX_SERVICE_TYPE setDataFuexServiceType()
        {
            HIS_FUEX_SERVICE_TYPE fuexServiceType = new HIS_FUEX_SERVICE_TYPE();
            try
            {
                if (!String.IsNullOrEmpty(txtSuimServiceTypeCode.Text))
                    fuexServiceType.FUEX_SERVICE_TYPE_CODE = txtSuimServiceTypeCode.Text;
                if (!String.IsNullOrEmpty(txtSuimServiceTypeName.Text))
                    fuexServiceType.FUEX_SERVICE_TYPE_NAME = txtSuimServiceTypeName.Text;
                if (cboParent.EditValue != null) fuexServiceType.PARENT_ID = Inventec.Common.TypeConvert.Parse.ToInt64((cboParent.EditValue ?? "0").ToString());
                if (fuexServiceType.PARENT_ID == 0)
                {
                    fuexServiceType.PARENT_ID = null;
                }
                if (spNumOrder.EditValue != null)
                {
                    fuexServiceType.NUM_ORDER = (long)spNumOrder.Value;
                }
            }
            catch (Exception ex)
            {
        ...
        {
            CommonParam param = new CommonParam();
            V_HIS_FUEX_SERVICE_TYPE hisMediStock = new V_HIS_FUEX_SERVICE_TYPE();
            bool notHandler = false;
            try
            {
                WaitingManager.Show();
                var data = TreeListSuimServiceType.GetDataRecordByNode(TreeListSuimServiceType.FocusedNode);
                V_HIS_FUEX_SERVICE_TYPE dataMediStock = data as V_HIS_FUEX_SERVICE_TYPE;
                ...
                {
                    V_HIS_FUEX_SERVICE_TYPE data1 = new V_HIS_FUEX_SERVICE_TYPE();
                    data1.ID = dataMediStock.ID;
                    WaitingManager.Show();
                    hisMediStock = new BackendAdapter(param).Post<V_HIS_FUEX_SERVICE_TYPE>("api/HisFuexServiceType/ChangeLock", ApiConsumers.MosConsumer, data1, param);
                    WaitingManager.Hide();
            ...
            try
            {
                ...
                if (MessageBox.Show(String.Format(ResourceLanguageManager.BanCoMuonXoaDuLieu), "", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    ...
                    if (rowData != null)
                    {
                        bool success = false;
                        CommonParam param = new CommonParam();
                        success = new BackendAdapter(param).Post<bool>("api/HisFuexServiceType/Delete", ApiConsumers.MosConsumer, rowData, param);
                        if (success)
                        {
                            LoaddataToTreeList();
                        }
