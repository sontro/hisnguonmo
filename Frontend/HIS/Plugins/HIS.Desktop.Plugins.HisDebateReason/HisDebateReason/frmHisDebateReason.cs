...
{
    public partial class frmHisDebateReason : HIS.Desktop.Utility.FormBase
    {
        #region Declare
        int rowCount = 0;
        int dataTotal = 0;
        ...
        MOS.EFMODEL.DataModels.HIS_DEBATE_REASON currentData;
        Dictionary<string, int> dicOrderTabIndexControl = new Dictionary<string, int>();
        Inventec.Desktop.Common.Modules.Module moduleData;
        #endregion

        #region Construct
        public frmHisDebateReason(Inventec.Desktop.Common.Modules.Module moduleData)
            : base(moduleData)
        {
            try
            {
        ...
        private void SetDefaultProperties()
        {
            try
            {
                SetCustomSizeForGridView(ref gridviewFormList);
            }
            catch (Exception ex)
            {
        ...
        private void frmHisDebateReason_Load(object sender, EventArgs e)
        {
            try
            {
                MeShow();
        ...
        private void ChangedDataRow(MOS.EFMODEL.DataModels.HIS_DEBATE_REASON data)
        {
            try
            {
                if (data != null)
        ...
        private void FillDataToEditorControl(MOS.EFMODEL.DataModels.HIS_DEBATE_REASON data)
        {
            try
            {
                if (data != null)
                {
                    txtCode.Text = data.DEBATE_REASON_CODE;
                    txtName.Text = data.DEBATE_REASON_NAME;
                }
            }
            catch (Exception ex)
            {
        ...
        private void gridviewFormList_CustomUnboundColumnData(object sender, DevExpress.XtraGrid.Views.Base.CustomColumnDataEventArgs e)
        {
            try
            {
                ...
                {
                    MOS.EFMODEL.DataModels.HIS_DEBATE_REASON pData = (MOS.EFMODEL.DataModels.HIS_DEBATE_REASON)((IList)((BaseView)sender).DataSource)[e.ListSourceRowIndex];
                    ...
                    {
                        e.Value = pData.CREATE_TIME != null ? Inventec.Common.DateTime.Convert.TimeNumberToTimeString(pData.CREATE_TIME ?? 0) : "";
                    }
                    else if (e.Column.FieldName == "MODIFY_TIME_STR")
                    {
                        e.Value = pData.MODIFY_TIME != null ? Inventec.Common.DateTime.Convert.TimeNumberToTimeString(pData.MODIFY_TIME ?? 0) : "";
                    }
        ...
        private void gridviewFormList_RowCellStyle(object sender, DevExpress.XtraGrid.Views.Grid.RowCellStyleEventArgs e)
        {
            try
            {
                ...
                if (e.RowHandle >= 0)
                {
                    HIS_DEBATE_REASON data = (HIS_DEBATE_REASON)((IList)((BaseView)sender).DataSource)[e.RowHandle];
        ...
        private void gridControlFormList_Resize(object sender, EventArgs e)
        {
            try
            {
                SetCustomSizeForGridView(ref gridviewFormList);
