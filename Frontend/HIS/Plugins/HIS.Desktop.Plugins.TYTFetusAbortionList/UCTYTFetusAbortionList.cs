...
namespace TYT.Desktop.Plugins.FetusAbortionList
{
    public partial class UCTYTFetusAbortionList : HIS.Desktop.Utility.UserControlBase
    {
        ...
        public UCTYTFetusAbortionList(Inventec.Desktop.Common.Modules.Module _module)
            : base(_module)
        {
            InitializeComponent();
        ...
        private void gridViewFetusAbortion_CustomUnboundColumnData(object sender, DevExpress.XtraGrid.Views.Base.CustomColumnDataEventArgs e)
        {
            try
            {
                if (e.IsGetData && e.Column.UnboundType != UnboundColumnType.Bound)
                {
                    TYT_FETUS_ABORTION dataRow = (TYT_FETUS_ABORTION)((IList)((BaseView)sender).DataSource)[e.ListSourceRowIndex];
        ...
        private void UCTYTFetusAbortionList_Load(object sender, EventArgs e)
        {
            try
            {
                LoadDataToDefault();
        ...
        private void repositoryItemButtonDelete_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            try
            {
                TYT_FETUS_ABORTION fetusAbortion = gridViewFetusAbortion.GetFocusedRow() as TYT_FETUS_ABORTION;
                List<TYT_FETUS_ABORTION> listFetusAbortionTemp = gridControlFetusAbortion.DataSource as List<TYT_FETUS_ABORTION>;
                if (fetusAbortion != null && listFetusAbortionTemp != null)
                {
                    WaitingManager.Show();
                    CommonParam param = new CommonParam();
                    bool result = new BackendAdapter(param)
                    .Post<bool>("api/TytFetusAbortion/Delete", ApiConsumers.TytConsumer, fetusAbortion.ID, param);
                    WaitingManager.Hide();
                    if (result)
                    {
                        listFetusAbortionTemp.Remove(fetusAbortion);
                        gridControlFetusAbortion.RefreshDataSource();
                    }
                }
            }
            catch (Exception ex)
        ...
        private void repositoryItemButtonEdit_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            try
            {
                ...
                if (fetusAbortion != null)
                {
                    ...
                    if (moduleData == null) Inventec.Common.Logging.LogSystem.Error("khong tim thay moduleLink = TYT.Desktop.Plugins.FetusAbortion");
                    if (moduleData.IsPlugin && moduleData.ExtensionInfo != null)
                    {
                        List<object> listArgs = new List<object>();
                        listArgs.Add(fetusAbortion.ID);
        ...
        private void gridViewFetusAbortion_CustomRowCellEdit(object sender, DevExpress.XtraGrid.Views.Grid.CustomRowCellEditEventArgs e)
        {
            try
            {
                DevExpress.XtraGrid.Views.Grid.GridView View = sender as DevExpress.XtraGrid.Views.Grid.GridView;
                TYT_FETUS_ABORTION data = null;
                if (e.RowHandle > -1)
                {
                    var index = gridViewFetusAbortion.GetDataSourceRowIndex(e.RowHandle);
                    data = (TYT_FETUS_ABORTION)((IList)((BaseView)sender).DataSource)[index];
                }
                if (e.RowHandle >= 0)
                {
                    if (data != null)
