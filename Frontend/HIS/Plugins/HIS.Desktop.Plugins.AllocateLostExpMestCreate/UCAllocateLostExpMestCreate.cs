...
using HIS.Desktop.Plugins.AllocateLostExpMestCreate.Resources;
using HIS.Desktop.Plugins.AllocateLostExpMestCreate.Popup;
...
namespace HIS.Desktop.Plugins.AllocateLostExpMestCreate
{
    public partial class UCAllocateLostExpMestCreate : UserControl
    {
        ...
        HisLostExpMestResultSDO resultSdo = null;
        ...
        #region contructor
        public UCAllocateLostExpMestCreate(long roomtypeid, long roomid)
        {
            InitializeComponent();
            try
            {
        ...
        /// <summary>
        /// load dữ liệu vào combo medistock
        /// </summary>
        ...
        private void LoadMedicineInStock(string txtKeyword)
        {
            try
            {
                ...
                medicineInStockSDOs = new Inventec.Common.Adapter.BackendAdapter(param).Get<List<MOS.SDO.HisMedicineInStockSDO>>(HIS.Desktop.Plugins.AllocateLostExpMestCreate.HisRequestUriStore.MOSHIS_MEDICINE_GET_IN_STOCK_MEDICINE, ApiConsumer.ApiConsumers.MosConsumer, hisMedicineStockViewFilter, param);
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
        ...
        private void LoadMaterialInStock(string txtKeyword)
        {
            try
            {
                ...
                materialInStockSDOs = new Inventec.Common.Adapter.BackendAdapter(param).Get<List<MOS.SDO.HisMaterialInStockSDO>>(HIS.Desktop.Plugins.AllocateLostExpMestCreate.HisRequestUriStore.MOSHIS_MATERIAL_GET_IN_STOCK_MATERIAL, ApiConsumer.ApiConsumers.MosConsumer, hisMaterialStockViewFilter, param);
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
        ...
        /// <summary>
        /// set dữ liệu vào control dưới bên trái
        /// </summary>
        ...
        private void EnableControl(int actionType)
        {
            try
            {
                ...
                btnHoiDong.Enabled = (actionType == HIS.Desktop.LocalStorage.LocalData.GlobalVariables.ActionView);
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
        ...
        private void SaveProcess()
        {
            try
            {
                ...
                WaitingManager.Show();
                MOS.SDO.HisLostExpMestSDO lostExpMestSdo = new HisLostExpMestSDO();
                lostExpMestSdo.LostExpMest = new HIS_LOST_EXP_MEST();
                lostExpMestSdo.ExpMaterials = new List<HisMaterialAmountSDO>();
                lostExpMestSdo.ExpMedicines = new List<HisMedicineAmountSDO>();
                lostExpMestSdo.ExpMest = new HIS_EXP_MEST();
                lostExpMestSdo.ExpMest.REQ_ROOM_ID = roomId;
                ...
                foreach (var item in mediMatyInGridcontrol)
                {
                    if (item.IsMedicine)
                    {
                        ...
                        lostExpMestSdo.ExpMedicines.Add(hisMedicineAmountSDO);
                    }
                    else
                    {
                        ...
                        lostExpMestSdo.ExpMaterials.Add(hisMaterialAmountSDO);
                    }
                }
                lostExpMestSdo.ExpMest.DESCRIPTION = txtDescription.Text.Trim();
                if (cboMedistock.EditValue != null)
                {
                    lostExpMestSdo.ExpMest.MEDI_STOCK_ID = Inventec.Common.TypeConvert.Parse.ToInt64((cboMedistock.EditValue ?? "").ToString());
                }

                resultSdo = new BackendAdapter(param).Post<MOS.SDO.HisLostExpMestResultSDO>(HIS.Desktop.Plugins.AllocateLostExpMestCreate.HisRequestUriStore.MOSHIS_LOST_EXP_MEST_CREATE, ApiConsumers.MosConsumer, lostExpMestSdo, param);
                if (resultSdo != null)
                {
                    success = true;
                    OrderByExpMestMedicineExpMestMaterial(resultSdo.ExpMedicines, resultSdo.ExpMaterials);
        ...
        private void OrderByExpMestMedicineExpMestMaterial(List<V_HIS_EXP_MEST_MEDICINE> expMestMedicines, List<V_HIS_EXP_MEST_MATERIAL> expMestMaterials)
        {
            try
            {
                if (expMestMedicines != null && expMestMedicines.Count > 0)
                {
                    expMestMedicines = expMestMedicines.OrderBy(o => o.ID).ToList();
                }
        ...
        private void btnHoiDong_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (resultSdo != null)
                {
                    frmHoiDong frm = new frmHoiDong(resultSdo.ExpMedicines, resultSdo.ExpMaterials, resultSdo.ExpMest.ID, resultSdo.LostExpMest.ID);
                    frm.ShowDialog();
                }
                WaitingManager.Hide();
            }
