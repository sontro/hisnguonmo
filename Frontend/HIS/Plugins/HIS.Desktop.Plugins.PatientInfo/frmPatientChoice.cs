...
namespace HIS.Desktop.Plugins.PatientInfo
{
    ...
    {
        HIS.Desktop.Common.DelegateSelectData updatePatientInfo;
        List<HisPatientSDO> currentListPatient;
        Dictionary<long, string> dicGender;

        public frmPatientChoice(List<HisPatientSDO> currentListPatient, HIS.Desktop.Common.DelegateSelectData updatePatientInfo)
        {
        ...
        private void ProcessSelectedPatientSdo(ref HisPatientSDO patient)
        {
            try
            {
                ...
                HisPatientWarningSDO patientWarningSDO = new BackendAdapter(param).Get<List<HisPatientWarningSDO>>("api/HisPatient/GetPreviousWarning", ApiConsumers.MosConsumer, patient.ID, HIS.Desktop.Controls.Session.SessionManager.ActionLostToken, param).SingleOrDefault();
