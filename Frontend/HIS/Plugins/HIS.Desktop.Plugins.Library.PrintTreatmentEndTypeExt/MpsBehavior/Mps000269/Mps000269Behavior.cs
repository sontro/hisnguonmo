...
namespace HIS.Desktop.Plugins.Library.PrintTreatmentEndTypeExt.MpsBehavior.Mps000269
{
    public class Mps000269Behavior : PrintDataBase, ILoad
    {
        public HIS_SERE_SERV sereServ;

        public Mps000269Behavior(long treatmentId)
            : base(treatmentId)
        {
            try
            {
        ...
        bool ILoad.Run(string mpsCode, string fileName, bool printNow)
        {
            ...
            try
            {
                ...
                MPS.Processor.Mps000269.PDO.Mps000269PDO rdo = new MPS.Processor.Mps000269.PDO.Mps000269PDO(this.Treatment, this.PatientTypeAlter, this.Patient, this.sereServ);
                Inventec.Common.SignLibrary.ADO.InputADO inputADO = new HIS.Desktop.Plugins.Library.EmrGenerate.EmrGenerateProcessor().GenerateInputADOWithPrintTypeCode(this.Treatment != null ? this.Treatment.TREATMENT_CODE : "", mpsCode);
                if (printNow)
