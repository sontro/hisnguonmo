...
{
    public partial class frmWaitingScreenCashierTwo : HIS.Desktop.Utility.FormBase
    {
        int countTimer = 0;
        V_HIS_ROOM currentRoom;
        public frmWaitingScreenCashierTwo(Inventec.Desktop.Common.Modules.Module moduleData, V_HIS_ROOM room)
            : base(moduleData)
        {
            InitializeComponent();
            this.currentRoom = room;
            this.Name = "CALL_PATIENT_CASHIER_TWO_" + this.currentRoom.ROOM_CODE;
            SetIcon();
        ...
        private void LoadValueToControl()
        {
            try
            {
                ...
                {
                    lblNameClient.Appearance.Font = new Font(new FontFamily("Arial"), HisConfigCFG.NUM_ORDER_SIZE, FontStyle.Bold);
                }
            }
            catch (Exception ex)
            {
        ...
        public void SetPatient(string nameclient, long dateborn, string treatmentcode, int? counters )
        {
            try
            {
                if (!string.IsNullOrWhiteSpace(nameclient))
                {
                    lblNameClient.Invoke(new MethodInvoker(delegate
                    {
                        lblNameClient.Text = nameclient + "";
                ...
                {
                    lblRegisterRoom.Text = ("VÀO PHÒNG THU NGÂN :" + this.currentRoom.ROOM_NAME.ToUpper()).ToUpper();
                }
        ...
        private void ChangeColor()
        {
            try
            {
                ...
                {
                    lblNameClient.ForeColor = System.Drawing.Color.FromArgb(40, 255, 40);
                    timer.Start();
                }
                if (countTimer == 0 || countTimer == 2 || countTimer == 4 || countTimer == 6)
                {
                    lblNameClient.ForeColor = Color.Red;
                    timer.Start();
                }
                if (countTimer > 6)
                {
