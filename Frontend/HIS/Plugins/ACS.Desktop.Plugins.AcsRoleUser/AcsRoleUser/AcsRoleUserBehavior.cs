...
{
    class AcsRoleUserBehavior : BusinessBase, IAcsRoleUser
    {
        object[] entity;
        internal AcsRoleUserBehavior(CommonParam param, object[] filter)
            : base()
        {
            this.entity = filter;
        }

        object IAcsRoleUser.Run()
        {
            object result = null;
            try
            {
                ...
                result = new frmAcsRoleUser(moduleData, listApiConsumer[0], listApiConsumer[1], listLong[0], listString[0], listString[1], listLong[1]);
            }
            catch (Exception ex)
            {
