...
namespace HIS.Desktop.Plugins.ListPresBloodConfirm.Base
{
    class GlobalStore
    {
        ...
        private static List<MOS.EFMODEL.DataModels.HIS_EXP_MEST_STT> ImpMestStt;
        public static List<MOS.EFMODEL.DataModels.HIS_EXP_MEST_STT> HisImpMestStts
        {
            get
            {
                if (ImpMestStt == null || ImpMestStt.Count == 0)
                {
                    ImpMestStt = BackendDataWorker.Get<MOS.EFMODEL.DataModels.HIS_EXP_MEST_STT>().OrderByDescending(o => o.CREATE_TIME).ToList();
                }
                return ImpMestStt;
            }
            set
