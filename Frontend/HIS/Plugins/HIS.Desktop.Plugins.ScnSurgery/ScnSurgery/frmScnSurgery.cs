...
{
    public partial class frmScnSurgery : HIS.Desktop.Utility.FormBase
    {
        #region Declare
        int rowCount = 0;
        int dataTotal = 0;
        ...
        SCN.EFMODEL.DataModels.SCN_SURGERY currentData;
        ...
        #region Construct
        public frmScnSurgery(Inventec.Desktop.Common.Modules.Module moduleData, string PatientCode)
            : base(moduleData)
        {
            try
            {
        ...
        private void frmScnSurgery_Load(object sender, EventArgs e)
        {
            try
            {
                MeShow();
        ...
        private void SetCaptionByLanguageKey()
        {
            try
            {
                ...
                Resources.ResourceLanguageManager.LanguageResource = new ResourceManager("HIS.Desktop.Plugins.ScnSurgery.Resources.Lang", typeof(HIS.Desktop.Plugins.ScnSurgery.frmScnSurgery).Assembly);
        ...
        private void InitTabIndex()
        {
            try
            {
                dicOrderTabIndexControl.Add("dtSurgeryTime", 0);
                dicOrderTabIndexControl.Add("txtSurgeryPart", 1);
                dicOrderTabIndexControl.Add("txtMoTa", 2);
        ...
        private void LoadPaging(object param)
        {
            try
            {
                ...
                Inventec.Core.ApiResultObject<List<SCN.EFMODEL.DataModels.SCN_SURGERY>> apiResult = null;
                ScnSurgeryFilter filter = new ScnSurgeryFilter();
                SetFilterNavBar(ref filter);
                dnNavigation.DataSource = null;
                apiResult = new BackendAdapter(paramCommon).GetRO<List<SCN.EFMODEL.DataModels.SCN_SURGERY>>(HisRequestUriStore.MOSHIS_PROGRAM_GET, ApiConsumers.ScnConsumer, filter, paramCommon);
                if (apiResult != null)
                {
                    var data = (List<SCN.EFMODEL.DataModels.SCN_SURGERY>)apiResult.Data;
                    if (data != null)
                    {
        ...
        private void SetFilterNavBar(ref ScnSurgeryFilter filter)
        {
            try
            {
        ...
        private void txtKeyWord_PreviewKeyDown(object sender, PreviewKeyDownEventArgs e)
        {
            try
            {
                ...
                else if (e.KeyCode == Keys.Down)
                {
                    ...
                    var rowData = (SCN.EFMODEL.DataModels.SCN_SURGERY)gridviewFormList.GetFocusedRow();
                    if (rowData != null)
                    {
        ...
        private void gridviewFormList_CustomUnboundColumnData(object sender, DevExpress.XtraGrid.Views.Base.CustomColumnDataEventArgs e)
        {
            try
            {
                ...
                {
                    SCN.EFMODEL.DataModels.SCN_SURGERY pData = (SCN.EFMODEL.DataModels.SCN_SURGERY)((IList)((BaseView)sender).DataSource)[e.ListSourceRowIndex];
                    ...
                    else if (e.Column.FieldName == "SURGERY_TIME_STR")
                    {
                        try
                        {
                            e.Value = Inventec.Common.DateTime.Convert.TimeNumberToTimeString((long)pData.SURGERY_TIME);
                        }
                        catch (Exception ex)
                        {
                            Inventec.Common.Logging.LogSystem.Error(ex);
        ...
        private void dnNavigation_PositionChanged(object sender, EventArgs e)
        {
            try
            {
                this.currentData = (SCN.EFMODEL.DataModels.SCN_SURGERY)(gridControlFormList.DataSource as List<SCN.EFMODEL.DataModels.SCN_SURGERY>)[dnNavigation.Position];
        ...
        private void ChangedDataRow(SCN.EFMODEL.DataModels.SCN_SURGERY data)
        {
            try
            {
                if (data != null)
        ...
        private void FillDataToEditorControl(SCN.EFMODEL.DataModels.SCN_SURGERY data)
        {
            try
            {
                if (data != null)
                {
                    dtSurgeryTime.EditValue = Inventec.Common.DateTime.Convert.TimeNumberToSystemDateTime(data.SURGERY_TIME ?? 0);
                    txtSurgeryPart.Text = data.SURGERY_PART;
        ...
        private void LoadCurrent(long currentId, ref SCN.EFMODEL.DataModels.SCN_SURGERY currentDTO)
        {
            try
            {
                CommonParam param = new CommonParam();
                ScnSurgeryFilter filter = new ScnSurgeryFilter();
                filter.ID = currentId;
                currentDTO = new BackendAdapter(param).Get<List<SCN.EFMODEL.DataModels.SCN_SURGERY>>(HisRequestUriStore.MOSHIS_PROGRAM_GET, ApiConsumers.MosConsumer, filter, param).FirstOrDefault();
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Warn(ex);
        ...
        private void btnGDelete_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            try
            {
                ...
                var data = new BackendAdapter(param).Get<List<SCN_SURGERY>>(HisRequestUriStore.MOSHIS_PROGRAM_GET, ApiConsumers.ScnConsumer, filter, param).FirstOrDefault();

                if (rowData != null)
                {
                    bool success = false;
                    success = new BackendAdapter(param).Post<bool>(HisRequestUriStore.MOSHIS_PROGRAM_DELETE, ApiConsumers.ScnConsumer, data.ID, param);
                    if (success)
                    {
                        FillDataToGridControl();
                        currentData = ((List<SCN_SURGERY>)gridControlFormList.DataSource).FirstOrDefault();
                    }
        ...
        private void SaveProcess()
        {
            ...
            try
            {
                ...
                WaitingManager.Show();
                SCN.EFMODEL.DataModels.SCN_SURGERY updateDTO = new SCN.EFMODEL.DataModels.SCN_SURGERY();

                if (this.currentData != null && this.currentData.ID > 0)
                {
                    Inventec.Common.Mapper.DataObjectMapper.Map<SCN_SURGERY>(updateDTO, this.currentData);
                }
                UpdateDTOFromDataForm(ref updateDTO);
                if (ActionType == GlobalVariables.ActionAdd)
                {
                    updateDTO.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__TRUE;
                    var resultData = new BackendAdapter(param).Post<SCN.EFMODEL.DataModels.SCN_SURGERY>(HisRequestUriStore.MOSHIS_PROGRAM_CREATE, ApiConsumers.ScnConsumer, updateDTO, param);
                    if (resultData != null)
                    {
                        success = true;
                        FillDataToGridControl();
                ...
                {
                    var resultData = new BackendAdapter(param).Post<SCN.EFMODEL.DataModels.SCN_SURGERY>(HisRequestUriStore.MOSHIS_PROGRAM_UPDATE, ApiConsumers.ScnConsumer, updateDTO, param);
                    if (resultData != null)
                    {
                        success = true;
        ...
        private void UpdateRowDataAfterEdit(SCN.EFMODEL.DataModels.SCN_SURGERY data)
        {
            try
            {
                if (data == null)
                    throw new ArgumentNullException("data(SCN.EFMODEL.DataModels.SCN_SURGERY) is null");
                var rowData = (SCN.EFMODEL.DataModels.SCN_SURGERY)gridviewFormList.GetFocusedRow();
                if (rowData != null)
                {
                    Inventec.Common.Mapper.DataObjectMapper.Map<SCN.EFMODEL.DataModels.SCN_SURGERY>(rowData, data);
        ...
        private void UpdateDTOFromDataForm(ref SCN.EFMODEL.DataModels.SCN_SURGERY currentDTO)
        {
            try
            {
                if (dtSurgeryTime.DateTime != null && dtSurgeryTime.DateTime != DateTime.MinValue)
                {
                    currentDTO.SURGERY_TIME = Inventec.Common.TypeConvert.Parse.ToInt64(dtSurgeryTime.DateTime.ToString("yyyyMMddHHmm") + "00");
                }
                currentDTO.SURGERY_PART = txtSurgeryPart.Text.Trim();
                currentDTO.DESCRIPTION = txtMoTa.Text.Trim();
                currentDTO.PATIENT_CODE = patientCode;
        ...
        private void ValidateForm()
        {
            try
            {
                //ValidationSingleControl(txtSurgeryPart);
                //ValidationSingleControl(txtMoTa);
        ...
        private void btnGLock_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            ...
            CommonParam param = new CommonParam();
            SCN_SURGERY success = new SCN_SURGERY();
            //bool notHandler = false;
            try
            {

                SCN_SURGERY data = (SCN_SURGERY)gridviewFormList.GetFocusedRow();
                if (MessageBox.Show(LibraryMessage.MessageUtil.GetMessage(LibraryMessage.Message.Enum.HeThongTBCuaSoThongBaoBanCoMuonKhoaDuLieuKhong), "", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    SCN_SURGERY data1 = new SCN_SURGERY();
                    data1.ID = data.ID;
                    WaitingManager.Show();
                    success = new Inventec.Common.Adapter.BackendAdapter(param).Post<SCN_SURGERY>(HisRequestUriStore.MOSHIS_PROGRAM_CHANGE_LOCK, ApiConsumers.ScnConsumer, data1, param);
                    WaitingManager.Hide();
                    if (success != null)
                    {
                        FillDataToGridControl();
        ...
        private void gridviewFormList_CustomRowCellEdit(object sender, DevExpress.XtraGrid.Views.Grid.CustomRowCellEditEventArgs e)
        {
            try
            {
                ...
                if (e.RowHandle >= 0)
                {
                    SCN_SURGERY data = (SCN_SURGERY)((IList)((BaseView)sender).DataSource)[e.RowHandle];
        ...
        private void dtSurgeryTime_KeyUp(object sender, KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keys.Enter)
                {
                    ...
                    {
                        txtSurgeryPart.Focus();
                        txtSurgeryPart.SelectAll();
                    }
                    else
                    {
