...
{
    class HoldReturnFactory
    {
        internal static IHoldReturn MakeIAssignService(CommonParam param, object[] data)
        {
            IHoldReturn result = null;
            Inventec.Desktop.Common.Modules.Module moduleData = null;
            HoldReturnADO holdReturnADO = null;
            try
            {
                if (data.GetType() == typeof(object[]))
                {
                    if (data != null && data.Count() > 0)
                    {
                        for (int i = 0; i < data.Count(); i++)
                        {
                            ...
                            if (data[i] is HoldReturnADO)
                            {
                                holdReturnADO = (HoldReturnADO)data[i];
                            }
                        }
                        ...
                        result = new HoldReturnBehavior(param, moduleData, holdReturnADO);
                    }
                }
