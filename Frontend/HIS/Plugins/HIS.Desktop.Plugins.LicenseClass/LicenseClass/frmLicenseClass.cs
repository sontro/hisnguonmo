...
namespace HIS.Desktop.Plugins.LicenseClass
{
    public partial class frmLicenseClass : HIS.Desktop.Utility.FormBase
    {
        #region Declare
        int rowCount = 0;
        int dataTotal = 0;
        ...
        #region Construct
        public frmLicenseClass(Inventec.Desktop.Common.Modules.Module moduleData)
            : base(moduleData)
        {
            try
            {
        ...
        private void SetCaptionByLanguageKey()
        {
            try
            {
                ...
                Resources.ResourceLanguageManager.LanguageResource = new ResourceManager("HIS.Desktop.Plugins.LicenseClass.Resources.Lang", typeof(HIS.Desktop.Plugins.LicenseClass.frmLicenseClass).Assembly);
        ...
        private void LoadPaging(object param)
        {
            try
            {
                ...
                Inventec.Core.ApiResultObject<List<MOS.EFMODEL.DataModels.HIS_LICENSE_CLASS>> apiResult = null;
                HisLicenseClassFilter filter = new HisLicenseClassFilter();
                ...
                apiResult = new BackendAdapter(paramCommon).GetRO<List<MOS.EFMODEL.DataModels.HIS_LICENSE_CLASS>>("api/HisLicenseClass/Get", ApiConsumers.MosConsumer, filter, paramCommon);
                if (apiResult != null)
                {
                    var data = (List<MOS.EFMODEL.DataModels.HIS_LICENSE_CLASS>)apiResult.Data;
                    if (data != null)
                    {
        ...
        private void SetFilterNavBar(ref HisLicenseClassFilter filter)
        {
            try
            {
        ...
        private void FillDataToEditorControl(LisClsADO data)
        {
            try
            {
                if (data != null)
                {
                    ...
                    txtBedName.Text = data.LICENSE_CLASS_NAME;

                    txtBedCode.Text = data.LICENSE_CLASS_CODE;
        ...
        private void LoadCurrent(long currentId, ref MOS.EFMODEL.DataModels.HIS_LICENSE_CLASS currentDTO)
        {
            try
            {
                CommonParam param = new CommonParam();
                ...
                filter.ID = currentId;
                currentDTO = new BackendAdapter(param).Get<List<MOS.EFMODEL.DataModels.HIS_LICENSE_CLASS>>(HisRequestUriStore.MOSHIS_LICENSE_CLASS_GET, ApiConsumers.MosConsumer, filter, param).FirstOrDefault();
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Warn(ex);
        ...
        private void btnGDelete_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            try
            {
                if (MessageBox.Show(LibraryMessage.MessageUtil.GetMessage(LibraryMessage.Message.Enum.HeThongTBCuaSoThongBaoBanCoMuonXoaDuLieuKhong), "", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    var data = new BackendAdapter(param).Get<System.Collections.Generic.List<HIS_LICENSE_CLASS>>(HisRequestUriStore.MOSHIS_LICENSE_CLASS_GET, ApiConsumers.MosConsumer, filter, param).FirstOrDefault();
                    if (rowData != null)
                    {
                        bool success = false;

                        success = new BackendAdapter(param).Post<bool>(HisRequestUriStore.MOSHIS_LICENSE_CLASS_DELETE, ApiConsumers.MosConsumer, data.ID, param);
                        if (success)
                        {
                            FillDataToGridControl();
                            currentData = ((List<LisClsADO>)gridControlFormList.DataSource).FirstOrDefault();
                            BackendDataWorker.Reset<HIS_LICENSE_CLASS>();
                            BackendDataWorker.Reset<HIS_LICENSE_CLASS>();
                        }
        ...
        private void SaveProcess()
        {
            ...
            try
            {
                ...
                WaitingManager.Show();
                MOS.EFMODEL.DataModels.HIS_LICENSE_CLASS updateDTO = new MOS.EFMODEL.DataModels.HIS_LICENSE_CLASS();
                ...
                if (ActionType == GlobalVariables.ActionAdd)
                {
                    ...
                    var resultData = new MOS.EFMODEL.DataModels.HIS_LICENSE_CLASS();
                    //if (check(updateDTO))
                    //{
                    resultData = new BackendAdapter(param).Post<MOS.EFMODEL.DataModels.HIS_LICENSE_CLASS>(HisRequestUriStore.MOSHIS_LICENSE_CLASS_CREATE, ApiConsumers.MosConsumer, updateDTO, param);
                    if (resultData != null)
                    {
                        success = true;
                        FillDataToGridControl();
                ...
                else
                {
                    ...
                    //if (check(updateDTO))
                    //{
                    resultData = new BackendAdapter(param).Post<MOS.EFMODEL.DataModels.HIS_LICENSE_CLASS>(HisRequestUriStore.MOSHIS_LICENSE_CLASS_UPDATE, ApiConsumers.MosConsumer, updateDTO, param);
                    if (resultData != null)
                    {
                        success = true;
                        FillDataToGridControl();
        ...
        private bool check(HIS_LICENSE_CLASS updateDTO)
        {
            bool result = true;
            try
            {
                ...
                var Bed = new BackendAdapter(paramCommon).Get<List<MOS.EFMODEL.DataModels.HIS_LICENSE_CLASS>>
                  (HIS.Desktop.Plugins.LicenseClass.HisRequestUriStore.MOSHIS_LICENSE_CLASS_GET, ApiConsumers.MosConsumer, filterSearch, paramCommon);
                if (Bed != null && Bed.Count > 0)
                {
                    if (Bed.Exists(o => o.LICENSE_CLASS_CODE == updateDTO.LICENSE_CLASS_CODE))
                    {
                        if (ActionType == GlobalVariables.ActionAdd)
                        {
                            WaitingManager.Hide();
                        ...
                        else if (Bed.Exists(o => o.LICENSE_CLASS_CODE == updateDTO.LICENSE_CLASS_CODE && o.ID != updateDTO.ID))
                        {
                            WaitingManager.Hide();
                            MessageBox.Show("Dữ liệu đã tồn tại", "Thông báo");
                            result = false;
        ...
        private void UpdateRowDataAfterEdit(MOS.EFMODEL.DataModels.HIS_LICENSE_CLASS data)
        {
            try
            {
                if (data == null)
                    throw new ArgumentNullException("data(MOS.EFMODEL.DataModels.HIS_LICENSE_CLASS) is null");
                var rowData = (MOS.EFMODEL.DataModels.HIS_LICENSE_CLASS)gridviewFormList.GetFocusedRow();
                if (rowData != null)
                {
                    Inventec.Common.Mapper.DataObjectMapper.Map<MOS.EFMODEL.DataModels.HIS_LICENSE_CLASS>(rowData, data);
        ...
        private void UpdateDTOFromDataForm(ref MOS.EFMODEL.DataModels.HIS_LICENSE_CLASS currentDTO)
        {
            try
            {
                //currentDTO.SAMPLE_ROOM_CODE = txtBedCode.Text.Trim();
                currentDTO.LICENSE_CLASS_NAME = txtBedName.Text.Trim();
                currentDTO.LICENSE_CLASS_CODE = txtBedCode.Text.Trim();
        ...
        private void ValidateForm()
        {
            try
            {
                ...
                ValidationSingleControl(txtBedName);
        ...
        {
            CommonParam param = new CommonParam();
            HIS_LICENSE_CLASS success = new HIS_LICENSE_CLASS();
            ...
            try
            {
                ...
                {
                    HIS_LICENSE_CLASS data1 = new HIS_LICENSE_CLASS();
                    data1.ID = data.ID;
                    WaitingManager.Show();
                    success = new Inventec.Common.Adapter.BackendAdapter(param).Post<HIS_LICENSE_CLASS>(HisRequestUriStore.MOSHIS_LICENSE_CLASS_CHANGELOCK, ApiConsumers.MosConsumer, data1.ID, param);
                    WaitingManager.Hide();
                    if (success != null)
                    {
                        BackendDataWorker.Reset<HIS_LICENSE_CLASS>();
