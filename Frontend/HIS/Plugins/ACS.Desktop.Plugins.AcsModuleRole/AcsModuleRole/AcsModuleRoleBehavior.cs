...
using ACS.Desktop.Plugins.AcsModuleRole;
using ACS.EFMODEL.DataModels;

namespace ACS.Desktop.Plugins.AcsModuleRole.AcsModuleRole
{
    class AcsModuleRoleBehavior : Tool<IDesktopToolContext>, IAcsModuleRole
    {
        object[] entity;
        long treatmentId;
        Inventec.Desktop.Common.Modules.Module currentModule;

        internal AcsModuleRoleBehavior()
            : base()
        {

        }

        internal AcsModuleRoleBehavior(CommonParam param, object[] data)
            : base()
        {
            entity = data;
        }

        object IAcsModuleRole.Run()
        {
            object result = null;
            try
            {
                ...
                result = new UCAcsModuleRole(moduleData, listApiConsumer[0], listApiConsumer[1], listActionType[0], listLong[0], listString[0], listString[1], (List<ACS_APPLICATION>)listObj[0]);
            }
            catch (Exception ex)
            {
