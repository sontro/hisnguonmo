...
{
    public partial class frmOtherForms : HIS.Desktop.Utility.FormBase
    {
        ...
        public frmOtherForms()
        {
            // InitializeComponent();
        }

        public frmOtherForms(Inventec.Desktop.Common.Modules.Module currentModule, long treatmentId)
            : base(currentModule)
        {
            // InitializeComponent();
            try
        ...
        private void TaoBieuMauKhacClick()
        {
            try
            {
                ...
                {
                    dicParamPlus["VIR_ADDRESS_STR"] = currentPatient.VIR_ADDRESS;
                }
                else
                {
                    dicParamPlus["VIR_ADDRESS_STR"] = null;
                }

                if (currentPatient.CMND_DATE.HasValue)
                {
                    dicParamPlus["CMND_DATE_STR"] = Inventec.Common.DateTime.Convert.TimeNumberToDateString(currentPatient.CMND_DATE.Value);
                }
                else
                {
                    dicParamPlus["CMND_DATE_STR"] = null;
                }
                if (currentPatient.DOB != null)
                {
                    dicParamPlus["AGE_WITH_CAPTION"] = CalculateFullAge(currentPatient.DOB);
                }
                else
                {
                    dicParamPlus["AGE_WITH_CAPTION"] = null;
                }
                ...
                if (baby != null)
                {
                    ...
                    {
                        dicParamPlus["BABY_GENDER_NAME"] = baby.GENDER_NAME;
                    }
                    else
                    {
                        dicParamPlus["BABY_GENDER_NAME"] = null;
                    }
                    if (baby.WEIGHT.HasValue)
                    {
                        dicParamPlus["BABY_WEIGHT"] = baby.WEIGHT;
                    }
                    else
                    {
                        dicParamPlus["BABY_WEIGHT"] = null;
                    }
                    if (baby.BORN_TIME.HasValue)
                    {
                        dicParamPlus["BORN_TIME_STR"] = Inventec.Common.DateTime.Convert.TimeNumberToTimeStringWithoutSecond(baby.BORN_TIME.Value);
                        //dicParamPlus.Add("AGE_WITH_CAPTION", CalculateFullAge(baby.BORN_TIME.Value));
                        dicParamPlus["AGE_WITH_CAPTION"] = CalculateFullAge(baby.BORN_TIME.Value);
                        
                    }
                    else
                    {
                        dicParamPlus["BORN_TIME_STR"] = null;
                        dicParamPlus["AGE_WITH_CAPTION"] = null;
                ...
                //frmPrintEditor printEditor = new frmPrintEditor(printTemplate.FILE_PATTERN, "Biểu mẫu khác__", UpdateTreatmentJsonPrint, dicParamPlus, this.dicImagePlus, inputADO);//_SarPrintTypeAdo.FILE_NAME
        ...
        private void AddKeyIntoDictionaryPrint<T>(T data, Dictionary<string, object> dicParamPlus)
        {
            try
            {
                ...
                if (pis != null && pis.Length > 0)
                {
                    ...
                    {
                        if (!dicParamPlus.ContainsKey(pi.Name))
                        {
        ...
        private void addDataToBieuMauKhac(Dictionary<string, object> dicParamPlus)
        {
            try
            {
                ...
                if (currentTreatmentUpdate != null)
                {
                    ...
                    long ngayDieuTri = 0;
                    ngayDieuTri += HIS.Treatment.DateTime.Calculation.DayOfTreatment(currentTreatmentUpdate.IN_TIME, currentTreatmentUpdate.OUT_TIME ?? 0);
