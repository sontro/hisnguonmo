...
namespace HIS.Desktop.Plugins.CallPatientV3
{
    ...
    {
        frmWaitingScreen5 aFrmWaitingScreenQy = null;
        const string frmWaitingScreenStr = "frmWaitingScreen5"; 
        const string frmWaitingScreenQyStr = "frmWaitingScreen_QY5";
        ...
        private void tgExtendMonitor_Toggled(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!dxValidationProviderControl.Validate())
                    return;
                aFrmWaitingScreenQy = new frmWaitingScreen5(currentModule,HisServiceReq, serviceReqStts);
                if (this.room != null)
                {
                ...
                {
                    HIS.Desktop.Plugins.CallPatientV3.ChooseRoomForWaitingScreenProcess.ShowFormInExtendMonitor(aFrmWaitingScreenQy);
                    this.Close();
                }
                else
                {
                    HIS.Desktop.Plugins.CallPatientV3.ChooseRoomForWaitingScreenProcess.TurnOffExtendMonitor(aFrmWaitingScreenQy);
                }
            }
            catch (Exception ex)
            {
