...
{
    public partial class UCListTYTTuberculosisList : HIS.Desktop.Utility.UserControlBase
    {
        int rowCount = 0;
        int dataTotal = 0;
        int startPage = 0;
        ...
        public UCListTYTTuberculosisList(Inventec.Desktop.Common.Modules.Module module)
            : base(module)
        {
            InitializeComponent();
        ...
        private void gridTYTTuberculosisListList_CustomUnboundColumnData(object sender, DevExpress.XtraGrid.Views.Base.CustomColumnDataEventArgs e)
        {
            try
            {
                if (e.IsGetData)
                {
                    int[] selectRow = gridViewTYTTuberculosisList.GetSelectedRows();
                    foreach (var item in selectRow)
                    {
                        var row = gridViewTYTTuberculosisList.GetRow(item);
                    }


                    DevExpress.XtraGrid.Views.Grid.GridView view = sender as DevExpress.XtraGrid.Views.Grid.GridView;
                    var data = (TYT_TUBERCULOSIS)((IList)((BaseView)sender).DataSource)[e.ListSourceRowIndex];
                    ...
                    gridViewTYTTuberculosisList.RefreshData();
        ...
        private void setDefaultControl()
        {
            try
            {
                ...
                txtKeyWord.Text = "";
                txtMaYTeCode.Text = "";
                txtTYTTuberculosisListCode.Text = "";
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Warn(ex);
        ...
        private void FillDataToGrid()
        {
            try
            {
                ...
                ucPaging1.Init(GridPaging, param, pageSize, gridControlTYTTuberculosisList);
                WaitingManager.Hide();
            }
            catch (Exception ex)
            {
        ...
        private void GridPaging(object param)
        {
            try
            {
                ...
                ApiResultObject<List<TYT.EFMODEL.DataModels.TYT_TUBERCULOSIS>> apiResult = null;
                TytTuberculosisFilter filter = new TytTuberculosisFilter();
                filter.ORDER_FIELD = "CREATE_TIME";
                filter.ORDER_DIRECTION = "DESC";
                SetFilter(ref filter);

                gridViewTYTTuberculosisList.BeginUpdate();

                apiResult = new Inventec.Common.Adapter.BackendAdapter
                    (paramCommon).GetRO<List<TYT_TUBERCULOSIS>>
                    ("api/TytTuberculosis/Get", ApiConsumers.TytConsumer, filter, paramCommon);
                if (apiResult != null)
                {
                    var data = apiResult.Data;
                    if (data != null && data.Count > 0)
                    {
                        gridControlTYTTuberculosisList.DataSource = data;
                        rowCount = (data == null ? 0 : data.Count);
                        dataTotal = (apiResult.Param == null ? 0 : apiResult.Param.Count ?? 0);
                    }
                    else
                    {
                        gridControlTYTTuberculosisList.DataSource = null;
                        rowCount = (data == null ? 0 : data.Count);
                        dataTotal = (apiResult.Param == null ? 0 : apiResult.Param.Count ?? 0);
                    }
                }
                gridViewTYTTuberculosisList.EndUpdate();
        ...
        private void SetFilter(ref TytTuberculosisFilter filter)
        {
            try
            {
                if (!string.IsNullOrEmpty(txtTYTTuberculosisListCode.Text))
                {
                    string code = txtTYTTuberculosisListCode.Text.Trim();
                    if (code.Length < 10 && checkDigit(code))
                    {
                        code = string.Format("{0:0000000000}", Convert.ToInt64(code));
                        txtTYTTuberculosisListCode.Text = code;
                    }
        ...
        private void btnGTYTTuberculosisListEdit_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            try
            {
                var row = (TYT.EFMODEL.DataModels.TYT_TUBERCULOSIS)gridViewTYTTuberculosisList.GetFocusedRow();
                if (row != null)
                {
                    WaitingManager.Show();
                    ...
                    CallModule callModule = new CallModule(CallModule.TYTTuberculosisListUpdate, this.currentModule.RoomId, this.currentModule.RoomTypeId, listArgs);
                    WaitingManager.Hide();
                }
            }
            catch (Exception ex)
        ...
        private void UCListTYTTuberculosisList_Load(object sender, EventArgs e)
        {
            try
            {
                setDefaultControl();
        ...
        private void txtTYTTuberculosisListCode_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                if (txtTYTTuberculosisListCode.Text != "")
                {
                    if (e.KeyCode == Keys.Enter)
                    {
                        btnSearch_Click(null, null);
        ...
        private void gridViewTYTTuberculosisList_CustomRowCellEdit(object sender, CustomRowCellEditEventArgs e)
        {
            try
            {
                if (e.RowHandle >= 0)
                {
                    string creator = (gridViewTYTTuberculosisList.GetRowCellValue(e.RowHandle, "CREATOR") ?? "").ToString().Trim();
                    ...
                    {
                        if (creator == loginName || HIS.Desktop.IsAdmin.CheckLoginAdmin.IsAdmin(loginName))
                            e.RepositoryItem = btnGTYTTuberculosisListEdit;
                        else
        ...
        private void repositoryItemButton__Delete_ButtonClick(object sender, ButtonPressedEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    var row = (TYT_TUBERCULOSIS)gridViewTYTTuberculosisList.GetFocusedRow();
                    if (row != null)
                    {
                        WaitingManager.Show();
