    ...
    public partial class frmAssignService : HIS.Desktop.Utility.FormBase
    {
        ...
        private bool DelegateRunPrinter(string printTypeCode)
        {
            ...
            try
            {
                ...
                if (currentHisTreatment != null && serviceReqComboResultSDO != null && serviceReqComboResultSDO.ServiceReqs != null && serviceReqComboResultSDO.ServiceReqs.Count > 0)
                {
        ...
        private void InPhieuYeuCauDichVu()
        {
            try
            {
                ...
                {
                    List<V_HIS_BED_LOG> bedLogs = new List<V_HIS_BED_LOG>();
                    if (this.currentHisTreatment != null && serviceReqComboResultSDO.ServiceReqs != null && serviceReqComboResultSDO.ServiceReqs.Count > 0)
                    {
                        CommonParam param = new CommonParam();
                        MOS.Filter.HisBedLogViewFilter bedLogViewFilter = new MOS.Filter.HisBedLogViewFilter();
                        bedLogViewFilter.TREATMENT_ID = this.currentHisTreatment.ID;
