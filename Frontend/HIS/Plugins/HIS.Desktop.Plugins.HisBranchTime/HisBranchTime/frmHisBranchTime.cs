...
using HIS.Desktop.Plugins.HisBranchTime;

namespace HIS.Desktop.Plugins.HisBranchTime
{
    public partial class frmHisBranchTime : HIS.Desktop.Utility.FormBase
    {
        #region Declare
        int rowCount = 0;
        int dataTotal = 0;
        ...
        MOS.EFMODEL.DataModels.V_HIS_BRANCH_TIME currentData;
        ...
        #region Construct
        public frmHisBranchTime(Inventec.Desktop.Common.Modules.Module moduleData)
            : base(moduleData)
        {
            try
            {
        ...
        public frmHisBranchTime(Inventec.Desktop.Common.Modules.Module moduleData, HIS_BRANCH _executeRoom)
            : base(moduleData)
        {
            try
            {
                ...
                pagingGrid = new PagingGrid();
                this.moduleData = moduleData;
                this.currentBranch = _executeRoom;
                gridControlFormList.ToolTipController = toolTipControllerGrid;
        ...
        private void SetCaptionByLanguageKey()
        {
            try
            {
                ...
                Resources.ResourceLanguageManager.LanguageResource = new ResourceManager("HIS.Desktop.Plugins.HisBranchTime.Resources.Lang", typeof(HIS.Desktop.Plugins.HisBranchTime.frmHisBranchTime).Assembly);
        ...
        private void InitComboBranchId()
        {
            try
            {
                ...
                data = (data != null && data.Count > 0) ? data.Where(o => o.IS_USE_BRANCH_TIME == 1).ToList() : data;
        ...
        private void InitComboBranchSearch()
        {
            try
            {
                CommonParam param = new CommonParam();
                ...
                columnInfos.Add(new ColumnInfo("BRANCH_NAME", "", 250, 2));
                ControlEditorADO controlEditorADO = new ControlEditorADO("BRANCH_NAME", "ID", columnInfos, false, 350);
                ControlEditorLoader.Load(cboBranchSearch, data, controlEditorADO);
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Warn(ex);
        ...
        private void LoadPaging(object param)
        {
            try
            {
                ...
                Inventec.Core.ApiResultObject<List<MOS.EFMODEL.DataModels.V_HIS_BRANCH_TIME>> apiResult = null;
                HisBranchTimeViewFilter filter = new HisBranchTimeViewFilter();
                SetFilterNavBar(ref filter);
                filter.ORDER_DIRECTION = "DESC";
                filter.ORDER_FIELD = "MODIFY_TIME";
                dnNavigation.DataSource = null;
                gridviewFormList.BeginUpdate();
                apiResult = new BackendAdapter(paramCommon).GetRO<List<MOS.EFMODEL.DataModels.V_HIS_BRANCH_TIME>>(HisRequestUriStoreBranch.MOSHIS_BRANCH_TIME_GETVIEW, ApiConsumers.MosConsumer, filter, paramCommon);
                if (apiResult != null)
                {
                    var data = (List<MOS.EFMODEL.DataModels.V_HIS_BRANCH_TIME>)apiResult.Data;
                    if (data != null)
                    {
        ...
        private void SetFilterNavBar(ref HisBranchTimeViewFilter filter)
        {
            try
            {
                filter.KEY_WORD = txtKeyword.Text.Trim();
                if (this.cboBranchSearch.EditValue != null)
                {
                    filter.BRANCH_ID = Inventec.Common.TypeConvert.Parse.ToInt64(cboBranchSearch.EditValue.ToString());
                }
        ...
        private void txtKeyWord_PreviewKeyDown(object sender, PreviewKeyDownEventArgs e)
        {
            try
            {
                ...
                else if (e.KeyCode == Keys.Down)
                {
                    ...
                    var rowData = (MOS.EFMODEL.DataModels.V_HIS_BRANCH_TIME)gridviewFormList.GetFocusedRow();
                    if (rowData != null)
                    {
        ...
        private void gridviewFormList_CustomUnboundColumnData(object sender, DevExpress.XtraGrid.Views.Base.CustomColumnDataEventArgs e)
        {
            try
            {
                ...
                {
                    MOS.EFMODEL.DataModels.V_HIS_BRANCH_TIME pData = (MOS.EFMODEL.DataModels.V_HIS_BRANCH_TIME)((IList)((BaseView)sender).DataSource)[e.ListSourceRowIndex];
        ...
        private void dnNavigation_PositionChanged(object sender, EventArgs e)
        {
            try
            {
                this.currentData = (MOS.EFMODEL.DataModels.V_HIS_BRANCH_TIME)(gridControlFormList.DataSource as List<MOS.EFMODEL.DataModels.V_HIS_BRANCH_TIME>)[dnNavigation.Position];
        ...
        private void ChangedDataRow(MOS.EFMODEL.DataModels.V_HIS_BRANCH_TIME data)
        {
            try
            {
                if (data != null)
        ...
        private void FillDataToEditorControl(MOS.EFMODEL.DataModels.V_HIS_BRANCH_TIME data)
        {
            try
            {
                if (data != null)
        ...
        private void LoadCurrent(long currentId, ref MOS.EFMODEL.DataModels.HIS_BRANCH_TIME currentDTO)
        {
            try
            {
                CommonParam param = new CommonParam();
                HisBranchTimeFilter filter = new HisBranchTimeFilter();
                filter.ID = currentId;
                currentDTO = new BackendAdapter(param).Get<List<MOS.EFMODEL.DataModels.HIS_BRANCH_TIME>>(HisRequestUriStoreBranch.MOSHIS_BRANCH_TIME_GET, ApiConsumers.MosConsumer, filter, param).FirstOrDefault();
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Warn(ex);
        ...
        private void btnGDelete_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            try
            {
                CommonParam param = new CommonParam();
                var rowData = (MOS.EFMODEL.DataModels.V_HIS_BRANCH_TIME)gridviewFormList.GetFocusedRow();
                HIS_BRANCH_TIME data = new HIS_BRANCH_TIME();
                Inventec.Common.Mapper.DataObjectMapper.Map<HIS_BRANCH_TIME>(data, rowData);
                if (rowData != null)
                {
                    bool success = false;

                    success = new BackendAdapter(param).Post<bool>(HisRequestUriStoreBranch.MOSHIS_BRANCH_TIME_DELETE, ApiConsumers.MosConsumer, data.ID, param);
                    if (success)
                    {
                        FillDataToGridControl();
                        currentData = ((List<V_HIS_BRANCH_TIME>)gridControlFormList.DataSource).FirstOrDefault();
                    }
        ...
        private void SaveProcess()
        {
            ...
            try
            {
                ...
                MOS.EFMODEL.DataModels.HIS_BRANCH_TIME updateDTO = new MOS.EFMODEL.DataModels.HIS_BRANCH_TIME();
                ...
                //    HisBranchTimeFilter roomTimeFilter = new HisBranchTimeFilter();
                //    roomTimeFilter.BRANCH_ID = updateDTO.BRANCH_ID;
                //    var roomTime = new BackendAdapter(param).Get<List<HIS_BRANCH_TIME>>("api/HisBranchTime/Get", ApiConsumers.MosConsumer, roomTimeFilter, param);

                //    List<HIS_BRANCH_TIME> update = new List<HIS_BRANCH_TIME>();
                //    List<short> create = new List<short>();


                //    if (roomTime != null && roomTime.Count > 0)
                //    {
                //        update = roomTime.Where(o => _DaySelecteds.Select(p => p.Day).Contains(o.DAY)).ToList();

                //        var creates = _DaySelecteds.Where(o => !roomTime.Select(p => p.DAY).Contains(o.Day)).ToList();
                //        if (creates != null && creates.Count > 0)
                //        {
                //            create = creates.Select(o => o.Day).ToList();
                //        }
                ...
                //                    item.FROM_TIME = dtFromTime.DateTime.ToString("HHmmsss");
                //                else
                //                    item.FROM_TIME = "000000";

                //                if (dtToTime.EditValue != null)
                //                    item.TO_TIME = dtToTime.DateTime.ToString("HHmmsss");
                //                else
                //                    item.TO_TIME = "235959";
                //            }

                //            var resultData = new BackendAdapter(param).Post<List<HIS_BRANCH_TIME>>("api/HisBranchTime/UpdateList", ApiConsumers.MosConsumer, update, param);
                ...
                //        {
                //            List<HIS_BRANCH_TIME> createList = new List<HIS_BRANCH_TIME>();
                //            foreach (var item in create)
                //            {
                //                var cre = new HIS_BRANCH_TIME();
                //                cre.BRANCH_ID = updateDTO.BRANCH_ID;
                //                cre.DAY = item;
                //                if (dtFromTime.EditValue != null)
                //                    cre.FROM_TIME = dtFromTime.DateTime.ToString("HHmmsss");
                //                else
                //                    cre.FROM_TIME = "000000";

                //                if (dtToTime.EditValue != null)
                //                    cre.TO_TIME = dtToTime.DateTime.ToString("HHmmsss");
                //                else
                //                    cre.TO_TIME = "235959";
                ...
                //            var resultData = new BackendAdapter(param).Post<List<HIS_BRANCH_TIME>>("api/HisBranchTime/CreateList", ApiConsumers.MosConsumer, createList, param);
                //            if (resultData != null && resultData.Count > 0)
                //            {
                //                success = true;
                //            }
                ...
                //    var resultData = new BackendAdapter(param).Post<MOS.EFMODEL.DataModels.HIS_BRANCH_TIME>(HisRequestUriStoreBranch.MOSHIS_BRANCH_TIME_UPDATE, ApiConsumers.MosConsumer, updateDTO, param);
                //    if (resultData != null)
                //    {
                //        success = true;
                //    }
                ...
                if (this.ActionType == GlobalVariables.ActionAdd)
                {
                    ...
                    WaitingManager.Show();
                    List<HIS_BRANCH_TIME> roomTimes = new List<HIS_BRANCH_TIME>();
                    foreach (var item in _DaySelecteds)
                    {
                        HIS_BRANCH_TIME roomTime = new HIS_BRANCH_TIME();
                        roomTime.DAY = item.Day;
                        roomTime.BRANCH_ID = Inventec.Common.TypeConvert.Parse.ToInt64(cboBranch.EditValue.ToString() ?? "0");
                        if (dtFromTime.EditValue != null)
                    ...
                    var resultData = new BackendAdapter(param).Post<List<MOS.EFMODEL.DataModels.HIS_BRANCH_TIME>>("api/HisBranchTime/CreateList", ApiConsumers.MosConsumer, roomTimes, param);
                    if (resultData != null)
                    {
                        success = true;
                    }
                ...
                //    var resultData = new BackendAdapter(param).Post<MOS.EFMODEL.DataModels.HIS_BRANCH_TIME>(HisRequestUriStoreBranch.MOSHIS_BRANCH_TIME_CREATE, ApiConsumers.MosConsumer, updateDTO, param);
                //    if (resultData != null)
                //    {
                //        success = true;
                //        FillDataToGridControl();
        ...
        private void UpdateRowDataAfterEdit(MOS.EFMODEL.DataModels.V_HIS_BRANCH_TIME data)
        {
            try
            {
                if (data == null)
                    throw new ArgumentNullException("data(MOS.EFMODEL.DataModels.V_HIS_BRANCH_TIME) is null");
                var rowData = (MOS.EFMODEL.DataModels.V_HIS_BRANCH_TIME)gridviewFormList.GetFocusedRow();
                if (rowData != null)
                {
                    Inventec.Common.Mapper.DataObjectMapper.Map<MOS.EFMODEL.DataModels.V_HIS_BRANCH_TIME>(rowData, data);
        ...
        private void UpdateDTOFromDataForm(ref MOS.EFMODEL.DataModels.HIS_BRANCH_TIME currentDTO)
        {
            try
            {
                if (dtFromTime.EditValue != null)
        ...
        public void MeShow()
        {
            try
            {
                ...
                {
                    var room = BackendDataWorker.Get<HIS_BRANCH>().FirstOrDefault(o => o.ID == this.currentBranch.ID);
                    if (room != null)
                    {
                        cboBranch.EditValue = room.ID;
                        cboBranchSearch.EditValue = room.ID;
                    }
                }
        ...
        {
            CommonParam param = new CommonParam();
            HIS_BRANCH_TIME success = new HIS_BRANCH_TIME();
            //bool notHandler = false;
            try
            {

                V_HIS_BRANCH_TIME data = (V_HIS_BRANCH_TIME)gridviewFormList.GetFocusedRow();
                if (MessageBox.Show(LibraryMessage.MessageUtil.GetMessage(LibraryMessage.Message.Enum.HeThongTBCuaSoThongBaoBanCoMuonKhoaDuLieuKhong), "", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    HIS_BRANCH_TIME data1 = new HIS_BRANCH_TIME();
                    data1.ID = data.ID;
                    WaitingManager.Show();
                    success = new Inventec.Common.Adapter.BackendAdapter(param).Post<HIS_BRANCH_TIME>(HisRequestUriStoreBranch.MOSHIS_BRANCH_TIME_CHANGELOCK, ApiConsumers.MosConsumer, data1.ID, param);
                    WaitingManager.Hide();
                    if (success != null)
                    {
                        FillDataToGridControl();
        ...
        private void gridviewFormList_CustomRowCellEdit(object sender, DevExpress.XtraGrid.Views.Grid.CustomRowCellEditEventArgs e)
        {
            try
            {
                ...
                if (e.RowHandle >= 0)
                {
                    ...
                    V_HIS_BRANCH_TIME data = (V_HIS_BRANCH_TIME)((IList)((BaseView)sender).DataSource)[e.RowHandle];
                    if (e.Column.FieldName == "IS_LOCK")
                    {
                        e.RepositoryItem = (data.IS_ACTIVE == IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__TRUE ? ButtonEdit_Unlock : Lock);
        ...
        private void cboBranchSearch_Closed(object sender, ClosedEventArgs e)
        {
            try
            {
        ...
        private void cboBranchSearch_ButtonClick(object sender, ButtonPressedEventArgs e)
        {
            try
            {
                if (e.Button.Kind == ButtonPredefines.Delete)
                {
                    cboBranchSearch.EditValue = null;
                    btnSearch_Click(null, null);
                }
            }
            catch (Exception ex)
