...
{
    public partial class frmHisPatientCase : HIS.Desktop.Utility.FormBase
    {
        #region Declare
        int rowCount = 0;
        int dataTotal = 0;
        ...
        MOS.EFMODEL.DataModels.HIS_PATIENT_CASE currentData;
        ...
        #region Construct
        public frmHisPatientCase(Inventec.Desktop.Common.Modules.Module moduleData)
            : base(moduleData)
        {
            try
            {
        ...
        private void frmHisPatientCase_Load(object sender, EventArgs e)
        {
            try
            {
                MeShow();
        ...
        private void SetCaptionByLanguageKey()
        {
            try
            {
                ...
                Resources.ResourceLanguageManager.LanguageResource = new ResourceManager("HIS.Desktop.Plugins.HisPatientCase.Resources.Lang", typeof(HIS.Desktop.Plugins.HisPatientCase.HisPatientCase.frmHisPatientCase).Assembly);
        ...
        private void LoadPaging(object param)
        {
            try
            {
                ...
                Inventec.Core.ApiResultObject<List<MOS.EFMODEL.DataModels.HIS_PATIENT_CASE>> apiResult = null;
                HisPatientCaseFilter filter = new HisPatientCaseFilter();
                filter.ORDER_FIELD = "MODIFY_TIME";
                filter.ORDER_DIRECTION = "DESC";
                SetFilterNavBar(ref filter);
                dnNavigation.DataSource = null;
                gridviewFormList.BeginUpdate();
                apiResult = new BackendAdapter(paramCommon).GetRO<List<MOS.EFMODEL.DataModels.HIS_PATIENT_CASE>>(HisRequestUriStore.MOSHIS_PATIENT_CASE_GET, ApiConsumers.MosConsumer, filter, paramCommon);
                if (apiResult != null)
                {
                    var data = (List<MOS.EFMODEL.DataModels.HIS_PATIENT_CASE>)apiResult.Data;
                    if (data != null)
                    {
        ...
        private void SetFilterNavBar(ref HisPatientCaseFilter filter)
        {
            try
            {
        ...
        private void txtKeyword_KeyUp(object sender, KeyEventArgs e)
        {
            try
            {
                ...
                else if (e.KeyCode == Keys.Down)
                {
                    ...
                    var rowData = (MOS.EFMODEL.DataModels.HIS_PATIENT_CASE)gridviewFormList.GetFocusedRow();
                    if (rowData != null)
                    {
        ...
        private void gridviewFormList_CustomUnboundColumnData(object sender, DevExpress.XtraGrid.Views.Base.CustomColumnDataEventArgs e)
        {
            try
            {
                if (e.IsGetData && e.Column.UnboundType != UnboundColumnType.Bound)
                {
                    ...
                    MOS.EFMODEL.DataModels.HIS_PATIENT_CASE pData = (MOS.EFMODEL.DataModels.HIS_PATIENT_CASE)((IList)((BaseView)sender).DataSource)[e.ListSourceRowIndex];
        ...
        private void dnNavigation_PositionChanged(object sender, EventArgs e)
        {
            try
            {
                this.currentData = (MOS.EFMODEL.DataModels.HIS_PATIENT_CASE)(gridControlFormList.DataSource as List<MOS.EFMODEL.DataModels.HIS_PATIENT_CASE>)[dnNavigation.Position];
        ...
        private void ChangedDataRow(MOS.EFMODEL.DataModels.HIS_PATIENT_CASE data)
        {
            try
            {
                if (data != null)
        ...
        private void FillDataToEditorControl(MOS.EFMODEL.DataModels.HIS_PATIENT_CASE data)
        {
            try
            {
                if (data != null)
                {
                    Id = data.ID;
                    txtPatientCaseCode.Text = data.PATIENT_CASE_CODE;
                    txtPatientCaseName.Text = data.PATIENT_CASE_NAME;
                }
            }
            catch (Exception ex)
            {
        ...
        private void LoadCurrent(long currentId, ref MOS.EFMODEL.DataModels.HIS_PATIENT_CASE currentDTO)
        {
            try
            {
                CommonParam param = new CommonParam();
                HisPatientCaseFilter filter = new HisPatientCaseFilter();
                filter.ID = currentId;
                currentDTO = new BackendAdapter(param).Get<List<MOS.EFMODEL.DataModels.HIS_PATIENT_CASE>>(HisRequestUriStore.MOSHIS_PATIENT_CASE_GET, ApiConsumers.MosConsumer, filter, param).FirstOrDefault();
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Warn(ex);
            ...
            try
            {
                ...
                if (rowData != null)
                {
                    if (MessageBox.Show(LibraryMessage.MessageUtil.GetMessage(LibraryMessage.Message.Enum.HeThongTBCuaSoThongBaoBanCoMuonXoaDuLieuKhong), "", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        bool success = false;
                        CommonParam param = new CommonParam();
                        success = new BackendAdapter(param).Post<bool>(HisRequestUriStore.MOSHIS_PATIENT_CASE_DELETE, ApiConsumers.MosConsumer, rowData.ID, param);
                        if (success)
                        {
                            BackendDataWorker.Reset<HIS_PATIENT_CASE>();
                            FillDataToGridControl();
                        }
        ...
        private void SaveProcess()
        {
            ...
            try
            {
                ...
                WaitingManager.Show();
                MOS.EFMODEL.DataModels.HIS_PATIENT_CASE updateDTO = new MOS.EFMODEL.DataModels.HIS_PATIENT_CASE();
                ...
                {
                    var resultData = new BackendAdapter(param).Post<MOS.EFMODEL.DataModels.HIS_PATIENT_CASE>(HisRequestUriStore.MOSHIS_PATIENT_CASE_CREATE, ApiConsumers.MosConsumer, updateDTO, param);
                    if (resultData != null)
                    {
                        success = true;
                        FillDataToGridControl();
                ...
                else
                {
                    ...
                    {
                        updateDTO.ID = Id;
                        var resultData = new BackendAdapter(param).Post<MOS.EFMODEL.DataModels.HIS_PATIENT_CASE>(HisRequestUriStore.MOSHIS_PATIENT_CASE_UPDATE, ApiConsumers.MosConsumer, updateDTO, param);
                        if (resultData != null)
                        {
                            success = true;
                            FillDataToGridControl();
        ...
        private void LookPatientType()
        {
            ...
            try
            {
                bool success = false;
                WaitingManager.Show();
                var updateDTO = (MOS.EFMODEL.DataModels.HIS_PATIENT_CASE)gridviewFormList.GetFocusedRow();
        ...
        private void UpdateRowDataAfterEdit(MOS.EFMODEL.DataModels.HIS_PATIENT_CASE data)
        {
            try
            {
                if (data == null)
                    throw new ArgumentNullException("data(MOS.EFMODEL.DataModels.HIS_PATIENT_CASE) is null");
                var rowData = (MOS.EFMODEL.DataModels.HIS_PATIENT_CASE)gridviewFormList.GetFocusedRow();
                if (rowData != null)
                {
                    Inventec.Common.Mapper.DataObjectMapper.Map<MOS.EFMODEL.DataModels.HIS_PATIENT_CASE>(rowData, data);
        ...
        private void UpdateDTOFromDataForm(ref MOS.EFMODEL.DataModels.HIS_PATIENT_CASE currentDTO)
        {
            try
            {
                currentDTO.PATIENT_CASE_CODE = txtPatientCaseCode.Text.Trim();
                currentDTO.PATIENT_CASE_NAME = txtPatientCaseName.Text.Trim();
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Warn(ex);
        ...
        private void ValidateForm()
        {
            try
            {
                ValidationControlMaxLength(txtPatientCaseCode, 2, true);
                ValidationControlMaxLength(txtPatientCaseName, 100, true);
        ...
        private void gridviewFormList_CustomRowCellEdit(object sender, DevExpress.XtraGrid.Views.Grid.CustomRowCellEditEventArgs e)
        {
            try
            {
                ...
                if (e.RowHandle >= 0)
                {
                    ...
                    HIS_PATIENT_CASE data = (HIS_PATIENT_CASE)((IList)((BaseView)sender).DataSource)[e.RowHandle];
        ...
        private void txtPatientTypeCode_PreviewKeyDown(object sender, PreviewKeyDownEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keys.Enter)
                {
                    txtPatientCaseName.Focus();
                    txtPatientCaseName.SelectAll();
                }
            }
            catch (Exception ex)
            {
        ...
        private void LockUnlockPatientType()
        {
            try
            {
                ...
                bool success = false;
                CommonParam param = new CommonParam();
                var updateDTO = (HIS_PATIENT_CASE)gridviewFormList.GetFocusedRow();
                var result = new BackendAdapter(param).Post<HIS_PATIENT_CASE>("api/HisPatientCase/ChangeLock", ApiConsumers.MosConsumer, updateDTO.ID, param);
                if (result != null)
                {
                    success = true;
                    FillDataToGridControl();
