...
{
    class AcsModuleBehavior : BusinessBase, IAcsModule
    {
        object[] entity;
        internal AcsModuleBehavior(CommonParam param, object[] filter)
            : base()
        {
            this.entity = filter;
        }

        object IAcsModule.Run()
        {
            object result = null;
            try
            {
                ...
                result = new frmAcsModule(moduleData, listApiConsumer[0], listApiConsumer[1], listActionType[0], listLong[0], listString[0], listString[1], (List<ACS_APPLICATION>)listObj[0]);
            }
            catch (Exception ex)
            {
