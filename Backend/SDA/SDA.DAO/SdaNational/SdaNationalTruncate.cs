...
{
    partial class SdaNationalTruncate : EntityBase
    {
        public SdaNationalTruncate()
            : base()
        {
        ...
        public bool Truncate(SDA_NATIONAL data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Truncate(data.ID);
        }

        public bool TruncateList(List<SDA_NATIONAL> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<SDA_NATIONAL> listIdNotFound = new List<SDA_NATIONAL>();
                    ...
                    else
                    {
                        ...
                        {
                            List<SDA_NATIONAL> listRaw = ctx.SDA_NATIONAL.Where(o => listId.Contains(o.ID)).ToList();
