...
{
    partial class SdaNationalDelete : EntityBase
    {
        public SdaNationalDelete()
            : base()
        {
        ...
        public bool Delete(SDA_NATIONAL data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<SDA_NATIONAL> listData)
        {
            bool result = false;
            try
            {
