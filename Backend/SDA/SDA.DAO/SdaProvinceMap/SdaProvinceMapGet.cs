...
namespace SDA.DAO.SdaProvinceMap
{
    partial class SdaProvinceMapGet : EntityBase
    {
        ...
        {
            List<SDA_PROVINCE_MAP> list = new List<SDA_PROVINCE_MAP>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.SDA_PROVINCE_MAP.AsQueryable();
        ...
        public SDA_PROVINCE_MAP GetById(long id, SdaProvinceMapSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.SDA_PROVINCE_MAP.AsQueryable().Where(p => p.ID == id);
                        if (search.listSdaProvinceMapExpression != null && search.listSdaProvinceMapExpression.Count > 0)
                        {
                            foreach (var item in search.listSdaProvinceMapExpression)
                            {
