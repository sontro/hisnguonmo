...
{
    partial class SdaGroupTypeDelete : EntityBase
    {
        public SdaGroupTypeDelete()
            : base()
        {
        ...
        public bool Delete(SDA_GROUP_TYPE data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<SDA_GROUP_TYPE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<SDA_GROUP_TYPE> listIdNotFound = new List<SDA_GROUP_TYPE>();
                    ...
                    else
                    {
                        ...
                        {
                            List<SDA_GROUP_TYPE> listRaw = ctx.SDA_GROUP_TYPE.Where(o => listId.Contains(o.ID)).ToList();
