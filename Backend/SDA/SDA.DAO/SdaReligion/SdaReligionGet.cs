...
namespace SDA.DAO.SdaReligion
{
    partial class SdaReligionGet : EntityBase
    {
        ...
        {
            List<SDA_RELIGION> list = new List<SDA_RELIGION>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.SDA_RELIGION.AsQueryable();
                        if (search.listSdaReligionExpression != null && search.listSdaReligionExpression.Count > 0)
                        {
                            foreach (var item in search.listSdaReligionExpression)
                            {
        ...
        public SDA_RELIGION GetById(long id, SdaReligionSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.SDA_RELIGION.AsQueryable().Where(p => p.ID == id);
                        if (search.listSdaReligionExpression != null && search.listSdaReligionExpression.Count > 0)
                        {
                            foreach (var item in search.listSdaReligionExpression)
                            {
