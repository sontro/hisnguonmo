...
namespace SDA.DAO.SdaEthnic
{
    partial class SdaEthnicGet : EntityBase
    {
        ...
        {
            List<SDA_ETHNIC> list = new List<SDA_ETHNIC>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.SDA_ETHNIC.AsQueryable();
                        if (search.listSdaEthnicExpression != null && search.listSdaEthnicExpression.Count > 0)
                        {
                            foreach (var item in search.listSdaEthnicExpression)
                            {
        ...
        public SDA_ETHNIC GetById(long id, SdaEthnicSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.SDA_ETHNIC.AsQueryable().Where(p => p.ID == id);
                        if (search.listSdaEthnicExpression != null && search.listSdaEthnicExpression.Count > 0)
                        {
                            foreach (var item in search.listSdaEthnicExpression)
                            {
