...
namespace SDA.DAO.SdaTrouble
{
    partial class SdaTroubleGet : EntityBase
    {
        ...
        {
            List<SDA_TROUBLE> list = new List<SDA_TROUBLE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.SDA_TROUBLE.AsQueryable();
                        if (search.listSdaTroubleExpression != null && search.listSdaTroubleExpression.Count > 0)
                        {
                            foreach (var item in search.listSdaTroubleExpression)
                            {
        ...
        public SDA_TROUBLE GetById(long id, SdaTroubleSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.SDA_TROUBLE.AsQueryable().Where(p => p.ID == id);
                        if (search.listSdaTroubleExpression != null && search.listSdaTroubleExpression.Count > 0)
                        {
                            foreach (var item in search.listSdaTroubleExpression)
                            {
