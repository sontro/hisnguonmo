...
{
    partial class SdaDeleteDataGet : EntityBase
    {
        public List<SDA_DELETE_DATA> Get(SdaDeleteDataSO search, CommonParam param)
        {
            List<SDA_DELETE_DATA> list = new List<SDA_DELETE_DATA>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.SDA_DELETE_DATA.AsQueryable();
                        if (search.listSdaDeleteDataExpression != null && search.listSdaDeleteDataExpression.Count > 0)
                        {
                            foreach (var item in search.listSdaDeleteDataExpression)
                            {
        ...
        public SDA_DELETE_DATA GetById(long id, SdaDeleteDataSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.SDA_DELETE_DATA.AsQueryable().Where(p => p.ID == id);
                        if (search.listSdaDeleteDataExpression != null && search.listSdaDeleteDataExpression.Count > 0)
                        {
                            foreach (var item in search.listSdaDeleteDataExpression)
                            {
