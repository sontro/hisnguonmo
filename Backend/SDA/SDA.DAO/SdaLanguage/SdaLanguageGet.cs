...
namespace SDA.DAO.SdaLanguage
{
    partial class SdaLanguageGet : EntityBase
    {
        ...
        {
            List<SDA_LANGUAGE> list = new List<SDA_LANGUAGE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.SDA_LANGUAGE.AsQueryable();
                        if (search.listSdaLanguageExpression != null && search.listSdaLanguageExpression.Count > 0)
                        {
                            foreach (var item in search.listSdaLanguageExpression)
                            {
        ...
        public SDA_LANGUAGE GetById(long id, SdaLanguageSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.SDA_LANGUAGE.AsQueryable().Where(p => p.ID == id);
                        if (search.listSdaLanguageExpression != null && search.listSdaLanguageExpression.Count > 0)
                        {
                            foreach (var item in search.listSdaLanguageExpression)
                            {
