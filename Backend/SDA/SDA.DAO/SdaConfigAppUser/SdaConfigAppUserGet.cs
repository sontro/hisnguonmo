...
namespace SDA.DAO.SdaConfigAppUser
{
    partial class SdaConfigAppUserGet : EntityBase
    {
        ...
        {
            List<SDA_CONFIG_APP_USER> list = new List<SDA_CONFIG_APP_USER>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.SDA_CONFIG_APP_USER.AsQueryable();
        ...
        public SDA_CONFIG_APP_USER GetById(long id, SdaConfigAppUserSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.SDA_CONFIG_APP_USER.AsQueryable().Where(p => p.ID == id);
                        if (search.listSdaConfigAppUserExpression != null && search.listSdaConfigAppUserExpression.Count > 0)
                        {
                            foreach (var item in search.listSdaConfigAppUserExpression)
                            {
