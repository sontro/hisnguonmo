...
namespace SDA.DAO.SdaCustomizeButton
{
    partial class SdaCustomizeButtonGet : EntityBase
    {
        ...
        {
            List<SDA_CUSTOMIZE_BUTTON> list = new List<SDA_CUSTOMIZE_BUTTON>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.SDA_CUSTOMIZE_BUTTON.AsNoTracking().AsQueryable();
                        if (search.listSdaCustomizeButtonExpression != null && search.listSdaCustomizeButtonExpression.Count > 0)
                        {
                            foreach (var item in search.listSdaCustomizeButtonExpression)
                            {
        ...
        public SDA_CUSTOMIZE_BUTTON GetById(long id, SdaCustomizeButtonSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.SDA_CUSTOMIZE_BUTTON.AsQueryable().Where(p => p.ID == id);
                        if (search.listSdaCustomizeButtonExpression != null && search.listSdaCustomizeButtonExpression.Count > 0)
                        {
                            foreach (var item in search.listSdaCustomizeButtonExpression)
                            {
