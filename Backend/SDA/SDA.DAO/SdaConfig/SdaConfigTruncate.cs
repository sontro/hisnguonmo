...
{
    partial class SdaConfigTruncate : EntityBase
    {
        public SdaConfigTruncate()
            : base()
        {
        ...
        public bool Truncate(SDA_CONFIG data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Truncate(data.ID);
        }

        public bool TruncateList(List<SDA_CONFIG> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<SDA_CONFIG> listIdNotFound = new List<SDA_CONFIG>();
                    ...
                    else
                    {
                        ...
                        {
                            List<SDA_CONFIG> listRaw = ctx.SDA_CONFIG.Where(o => listId.Contains(o.ID)).ToList();
