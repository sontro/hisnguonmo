...
{
    partial class SdaSqlDelete : EntityBase
    {
        public SdaSqlDelete()
            : base()
        {
        ...
        public bool Delete(SDA_SQL data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<SDA_SQL> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<SDA_SQL> listIdNotFound = new List<SDA_SQL>();
                    ...
                    else
                    {
                        ...
                        {
                            List<SDA_SQL> listRaw = ctx.SDA_SQL.Where(o => listId.Contains(o.ID)).ToList();
