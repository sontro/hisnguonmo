...
{
    partial class SdaTranslateGet : EntityBase
    {
        public List<V_SDA_TRANSLATE> GetView(SdaTranslateSO search, CommonParam param)
        {
            List<V_SDA_TRANSLATE> list = new List<V_SDA_TRANSLATE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_SDA_TRANSLATE.AsQueryable();
                        if (search.listVSdaTranslateExpression != null && search.listVSdaTranslateExpression.Count > 0)
                        {
                            foreach (var item in search.listVSdaTranslateExpression)
                            {
