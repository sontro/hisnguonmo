...
namespace SDA.DAO.SdaModuleField
{
    partial class SdaModuleFieldGet : EntityBase
    {
        ...
        {
            List<SDA_MODULE_FIELD> list = new List<SDA_MODULE_FIELD>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.SDA_MODULE_FIELD.AsQueryable();
        ...
        public SDA_MODULE_FIELD GetById(long id, SdaModuleFieldSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.SDA_MODULE_FIELD.AsQueryable().Where(p => p.ID == id);
                        if (search.listSdaModuleFieldExpression != null && search.listSdaModuleFieldExpression.Count > 0)
                        {
                            foreach (var item in search.listSdaModuleFieldExpression)
                            {
