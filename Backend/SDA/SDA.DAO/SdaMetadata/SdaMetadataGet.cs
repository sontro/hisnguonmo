...
namespace SDA.DAO.SdaMetadata
{
    partial class SdaMetadataGet : EntityBase
    {
        ...
        {
            List<SDA_METADATA> list = new List<SDA_METADATA>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.SDA_METADATA.AsQueryable();
                        if (search.listSdaMetadataExpression != null && search.listSdaMetadataExpression.Count > 0)
                        {
                            foreach (var item in search.listSdaMetadataExpression)
                            {
        ...
        public SDA_METADATA GetById(long id, SdaMetadataSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.SDA_METADATA.AsQueryable().Where(p => p.ID == id);
                        if (search.listSdaMetadataExpression != null && search.listSdaMetadataExpression.Count > 0)
                        {
                            foreach (var item in search.listSdaMetadataExpression)
                            {
