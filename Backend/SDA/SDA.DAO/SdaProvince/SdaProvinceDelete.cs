...
{
    partial class SdaProvinceDelete : EntityBase
    {
        public SdaProvinceDelete()
            : base()
        {
        ...
        public bool Delete(SDA_PROVINCE data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<SDA_PROVINCE> listData)
        {
            bool result = false;
            try
            {
