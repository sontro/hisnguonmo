...
namespace SDA.DAO.SdaProvince
{
    ...
    {
        public List<SDA_PROVINCE> Get(SdaProvinceSO search, CommonParam param)
        {
            List<SDA_PROVINCE> list = new List<SDA_PROVINCE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.SDA_PROVINCE.AsQueryable();
                        if (search.listSdaProvinceExpression != null && search.listSdaProvinceExpression.Count > 0)
                        {
                            foreach (var item in search.listSdaProvinceExpression)
                            {
        ...
        public SDA_PROVINCE GetById(long id, SdaProvinceSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.SDA_PROVINCE.AsQueryable().Where(p => p.ID == id);
                        if (search.listSdaProvinceExpression != null && search.listSdaProvinceExpression.Count > 0)
                        {
                            foreach (var item in search.listSdaProvinceExpression)
                            {
