...
{
    partial class SdaProvinceTruncate : EntityBase
    {
        public SdaProvinceTruncate()
            : base()
        {
        ...
        public bool Truncate(SDA_PROVINCE data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Truncate(data.ID);
        }

        public bool TruncateList(List<SDA_PROVINCE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<SDA_PROVINCE> listIdNotFound = new List<SDA_PROVINCE>();
                    ...
                    else
                    {
                        ...
                        {
                            List<SDA_PROVINCE> listRaw = ctx.SDA_PROVINCE.Where(o => listId.Contains(o.ID)).ToList();
