...
{
    partial class SdaCommuneTruncate : EntityBase
    {
        public SdaCommuneTruncate()
            : base()
        {
        ...
        public bool Truncate(SDA_COMMUNE data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Truncate(data.ID);
        }

        public bool TruncateList(List<SDA_COMMUNE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<SDA_COMMUNE> listIdNotFound = new List<SDA_COMMUNE>();
                    ...
                    else
                    {
                        ...
                        {
                            List<SDA_COMMUNE> listRaw = ctx.SDA_COMMUNE.Where(o => listId.Contains(o.ID)).ToList();
