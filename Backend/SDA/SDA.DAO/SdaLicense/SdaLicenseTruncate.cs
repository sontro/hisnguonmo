...
{
    partial class SdaLicenseTruncate : EntityBase
    {
        public SdaLicenseTruncate()
            : base()
        {
        ...
        public bool Truncate(SDA_LICENSE data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Truncate(data.ID);
        }

        public bool TruncateList(List<SDA_LICENSE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<SDA_LICENSE> listIdNotFound = new List<SDA_LICENSE>();
                    ...
                    else
                    {
                        ...
                        {
                            List<SDA_LICENSE> listRaw = ctx.SDA_LICENSE.Where(o => listId.Contains(o.ID)).ToList();
