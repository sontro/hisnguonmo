...
namespace SDA.DAO.SdaDistrictMap
{
    partial class SdaDistrictMapGet : EntityBase
    {
        ...
        {
            List<SDA_DISTRICT_MAP> list = new List<SDA_DISTRICT_MAP>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.SDA_DISTRICT_MAP.AsQueryable();
        ...
        public SDA_DISTRICT_MAP GetById(long id, SdaDistrictMapSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.SDA_DISTRICT_MAP.AsQueryable().Where(p => p.ID == id);
                        if (search.listSdaDistrictMapExpression != null && search.listSdaDistrictMapExpression.Count > 0)
                        {
                            foreach (var item in search.listSdaDistrictMapExpression)
                            {
