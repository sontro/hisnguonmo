...
{
    partial class SdaDistrictTruncate : EntityBase
    {
        public SdaDistrictTruncate()
            : base()
        {
        ...
        public bool Truncate(SDA_DISTRICT data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Truncate(data.ID);
        }

        public bool TruncateList(List<SDA_DISTRICT> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<SDA_DISTRICT> listIdNotFound = new List<SDA_DISTRICT>();
                    ...
                    else
                    {
                        ...
                        {
                            List<SDA_DISTRICT> listRaw = ctx.SDA_DISTRICT.Where(o => listId.Contains(o.ID)).ToList();
