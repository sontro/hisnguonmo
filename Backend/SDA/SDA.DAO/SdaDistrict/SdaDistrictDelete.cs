...
{
    partial class SdaDistrictDelete : EntityBase
    {
        public SdaDistrictDelete()
            : base()
        {
        ...
        public bool Delete(SDA_DISTRICT data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<SDA_DISTRICT> listData)
        {
            bool result = false;
            try
            {
