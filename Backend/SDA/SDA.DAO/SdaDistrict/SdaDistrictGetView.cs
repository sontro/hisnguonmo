...
namespace SDA.DAO.SdaDistrict
{
    ...
    {
        public List<V_SDA_DISTRICT> GetView(SdaDistrictSO search, CommonParam param)
        {
            List<V_SDA_DISTRICT> list = new List<V_SDA_DISTRICT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_SDA_DISTRICT.AsQueryable();
                        if (search.listVSdaDistrictExpression != null && search.listVSdaDistrictExpression.Count > 0)
                        {
                            foreach (var item in search.listVSdaDistrictExpression)
                            {
