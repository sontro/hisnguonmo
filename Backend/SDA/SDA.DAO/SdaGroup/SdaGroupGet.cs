...
namespace SDA.DAO.SdaGroup
{
    partial class SdaGroupGet : EntityBase
    {
        ...
        {
            List<SDA_GROUP> list = new List<SDA_GROUP>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.SDA_GROUP.AsQueryable();
                        if (search.listSdaGroupExpression != null && search.listSdaGroupExpression.Count > 0)
                        {
                            foreach (var item in search.listSdaGroupExpression)
                            {
        ...
        public SDA_GROUP GetById(long id, SdaGroupSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.SDA_GROUP.AsQueryable().Where(p => p.ID == id);
                        if (search.listSdaGroupExpression != null && search.listSdaGroupExpression.Count > 0)
                        {
                            foreach (var item in search.listSdaGroupExpression)
                            {
