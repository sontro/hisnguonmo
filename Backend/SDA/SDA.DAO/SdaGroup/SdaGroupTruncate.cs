...
{
    partial class SdaGroupTruncate : EntityBase
    {
        public SdaGroupTruncate()
            : base()
        {
        ...
        public bool Truncate(SDA_GROUP data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Truncate(data.ID);
        }

        public bool TruncateList(List<SDA_GROUP> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<SDA_GROUP> listIdNotFound = new List<SDA_GROUP>();
                    ...
                    else
                    {
                        ...
                        {
                            List<SDA_GROUP> listRaw = ctx.SDA_GROUP.Where(o => listId.Contains(o.ID)).ToList();
