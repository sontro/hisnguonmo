...
namespace SDA.DAO.SdaNotify
{
    partial class SdaNotifyGet : EntityBase
    {
        ...
        {
            List<SDA_NOTIFY> list = new List<SDA_NOTIFY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.SDA_NOTIFY.AsQueryable();
                        if (search.listSdaNotifyExpression != null && search.listSdaNotifyExpression.Count > 0)
                        {
                            foreach (var item in search.listSdaNotifyExpression)
                            {
        ...
        public SDA_NOTIFY GetById(long id, SdaNotifySO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.SDA_NOTIFY.AsQueryable().Where(p => p.ID == id);
                        if (search.listSdaNotifyExpression != null && search.listSdaNotifyExpression.Count > 0)
                        {
                            foreach (var item in search.listSdaNotifyExpression)
                            {
