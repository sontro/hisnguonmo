...
{
    partial class SdaNotifyDelete : EntityBase
    {
        public SdaNotifyDelete()
            : base()
        {
        ...
        public bool Delete(SDA_NOTIFY data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<SDA_NOTIFY> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<SDA_NOTIFY> listIdNotFound = new List<SDA_NOTIFY>();
                    ...
                    else
                    {
                        ...
                        {
                            List<SDA_NOTIFY> listRaw = ctx.SDA_NOTIFY.Where(o => listId.Contains(o.ID)).ToList();
