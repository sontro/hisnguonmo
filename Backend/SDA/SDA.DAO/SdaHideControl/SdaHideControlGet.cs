...
namespace SDA.DAO.SdaHideControl
{
    partial class SdaHideControlGet : EntityBase
    {
        ...
        {
            List<SDA_HIDE_CONTROL> list = new List<SDA_HIDE_CONTROL>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.SDA_HIDE_CONTROL.AsQueryable();
        ...
        public SDA_HIDE_CONTROL GetById(long id, SdaHideControlSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.SDA_HIDE_CONTROL.AsQueryable().Where(p => p.ID == id);
                        if (search.listSdaHideControlExpression != null && search.listSdaHideControlExpression.Count > 0)
                        {
                            foreach (var item in search.listSdaHideControlExpression)
                            {
