...
using SDA.MANAGER.Core.SdaCommuneMap.Get.ListV;
using SDA.MANAGER.Core.SdaCommuneMap.Get.V;
using Inventec.Core;
using System;
    ...
    partial class SdaCommuneMapGet : BeanObjectBase, IDelegacyT
    {
        ...
        T IDelegacyT.Execute<T>()
        {
            ...
            try
            {
                ...
                else if (typeof(T) == typeof(List<V_SDA_COMMUNE_MAP>))
                {
                    ISdaCommuneMapGetListV behavior = SdaCommuneMapGetListVBehaviorFactory.MakeISdaCommuneMapGetListV(param, entity);
                    result = behavior != null ? (T)System.Convert.ChangeType(behavior.Run(), typeof(T)) : default(T);
                }
                else if (typeof(T) == typeof(V_SDA_COMMUNE_MAP))
                {
                    ISdaCommuneMapGetV behavior = SdaCommuneMapGetVBehaviorFactory.MakeISdaCommuneMapGetV(param, entity);
                    result = behavior != null ? (T)System.Convert.ChangeType(behavior.Run(), typeof(T)) : default(T);
                }
            }
            catch (Exception ex)
