...
using SDA.MANAGER.Core.SdaLicense.Get.ListV;
using SDA.MANAGER.Core.SdaLicense.Get.V;
using Inventec.Core;
using System;
    ...
    partial class SdaLicenseGet : BeanObjectBase, IDelegacyT
    {
        ...
        T IDelegacyT.Execute<T>()
        {
            ...
            try
            {
                ...
                else if (typeof(T) == typeof(List<V_SDA_LICENSE>))
                {
                    ISdaLicenseGetListV behavior = SdaLicenseGetListVBehaviorFactory.MakeISdaLicenseGetListV(param, entity);
                    result = behavior != null ? (T)System.Convert.ChangeType(behavior.Run(), typeof(T)) : default(T);
                }
                else if (typeof(T) == typeof(V_SDA_LICENSE))
                {
                    ISdaLicenseGetV behavior = SdaLicenseGetVBehaviorFactory.MakeISdaLicenseGetV(param, entity);
                    result = behavior != null ? (T)System.Convert.ChangeType(behavior.Run(), typeof(T)) : default(T);
                }
            }
            catch (Exception ex)
