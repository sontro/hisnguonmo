...
{
    public class SdaNotifyViewFilterQuery : SdaNotifyViewFilter
    {
        public SdaNotifyViewFilterQuery()
            : base()
        {

        }
        
        internal List<System.Linq.Expressions.Expression<Func<V_SDA_NOTIFY, bool>>> listExpression = new List<System.Linq.Expressions.Expression<Func<V_SDA_NOTIFY, bool>>>();
        ...
        internal SdaNotifySO Query()
        {
            ...
            try
            {
                ...
                search.listVSdaNotifyExpression.AddRange(listExpression);
                search.OrderField = OrderProcess.GetOrderField<V_SDA_NOTIFY>(ORDER_FIELD);
                search.OrderDirection = OrderProcess.GetOrderDirection(ORDER_DIRECTION);
            }
            catch (Exception ex)
            {
                LogSystem.Error(ex);
                search.listVSdaNotifyExpression.Clear();
                search.listVSdaNotifyExpression.Add(o => o.ID == NEGATIVE_ID);
            }
            return search;
        }
    }
