...
{
    public class SdaSqlParamFilterQuery : SdaSqlParamFilter
    {
        public SdaSqlParamFilterQuery()
            : base()
        {

        }

        internal List<System.Linq.Expressions.Expression<Func<SDA_SQL_PARAM, bool>>> listExpression = new List<System.Linq.Expressions.Expression<Func<SDA_SQL_PARAM, bool>>>();
            ...
            try
            {
                ...
                if (!String.IsNullOrEmpty(this.KEY_WORD))
                {
                    ...
                    listExpression.Add(o =>
                        ...
                        o.SQL_PARAM_NAME.ToLower().Contains(this.KEY_WORD)
                        );
                }
                if (this.SQL_ID.HasValue)
                {
                    listExpression.Add(o => o.SQL_ID == this.SQL_ID.Value);
                }
                if (this.SQL_IDs != null && this.SQL_IDs.Count > 0)
                {
                    listExpression.Add(o => this.SQL_IDs.Contains(o.SQL_ID));
                }

                search.listSdaSqlParamExpression.AddRange(listExpression);
                search.OrderField = OrderProcess.GetOrderField<SDA_SQL_PARAM>(ORDER_FIELD);
                search.OrderDirection = OrderProcess.GetOrderDirection(ORDER_DIRECTION);
            }
            catch (Exception ex)
            {
                LogSystem.Error(ex);
                search.listSdaSqlParamExpression.Clear();
                search.listSdaSqlParamExpression.Add(o => o.ID == NEGATIVE_ID);
            }
            return search;
        }
    }
