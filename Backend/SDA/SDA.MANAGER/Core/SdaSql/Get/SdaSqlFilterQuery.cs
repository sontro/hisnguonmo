...
namespace SDA.MANAGER.Core.SdaSql.Get
{
    public class SdaSqlFilterQuery : SdaSqlFilter
    {
        public SdaSqlFilterQuery()
            : base()
        {

        }

        internal List<System.Linq.Expressions.Expression<Func<SDA_SQL, bool>>> listExpression = new List<System.Linq.Expressions.Expression<Func<SDA_SQL, bool>>>();

        internal Inventec.Backend.MANAGER.OrderProcessorBase OrderProcess = new Inventec.Backend.MANAGER.OrderProcessorBase();

        internal SdaSqlSO Query()
        {
            SdaSqlSO search = new SdaSqlSO();
            try
            {
                ...
                if (!String.IsNullOrEmpty(this.KEY_WORD))
                {
                    ...
                    listExpression.Add(o =>
                        ...
                        o.SCHEMA_CODE.ToLower().Contains(this.KEY_WORD) ||
                        o.SQL_CODE.ToLower().Contains(this.KEY_WORD) ||
                        o.SQL_NAME.ToLower().Contains(this.KEY_WORD)
                        );
                }

                search.listSdaSqlExpression.AddRange(listExpression);
                search.OrderField = OrderProcess.GetOrderField<SDA_SQL>(ORDER_FIELD);
                search.OrderDirection = OrderProcess.GetOrderDirection(ORDER_DIRECTION);
            }
            catch (Exception ex)
            {
                LogSystem.Error(ex);
                search.listSdaSqlExpression.Clear();
                search.listSdaSqlExpression.Add(o => o.ID == NEGATIVE_ID);
            }
            return search;
        }
    }
