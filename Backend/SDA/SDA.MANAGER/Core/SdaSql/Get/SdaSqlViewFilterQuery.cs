...
{
    public class SdaSqlViewFilterQuery : SdaSqlViewFilter
    {
        public SdaSqlViewFilterQuery()
            : base()
        {

        }
        
        internal List<System.Linq.Expressions.Expression<Func<V_SDA_SQL, bool>>> listExpression = new List<System.Linq.Expressions.Expression<Func<V_SDA_SQL, bool>>>();
        ...
        internal SdaSqlSO Query()
        {
            ...
            try
            {
                ...
                search.listVSdaSqlExpression.AddRange(listExpression);
                search.OrderField = OrderProcess.GetOrderField<V_SDA_SQL>(ORDER_FIELD);
                search.OrderDirection = OrderProcess.GetOrderDirection(ORDER_DIRECTION);
            }
            catch (Exception ex)
            {
                LogSystem.Error(ex);
                search.listVSdaSqlExpression.Clear();
                search.listVSdaSqlExpression.Add(o => o.ID == NEGATIVE_ID);
            }
            return search;
        }
    }
