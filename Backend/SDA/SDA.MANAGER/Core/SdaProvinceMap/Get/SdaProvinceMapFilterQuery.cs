...
namespace SDA.MANAGER.Core.SdaProvinceMap.Get
{
    public class SdaProvinceMapFilterQuery : SdaProvinceMapFilter
    {
        public SdaProvinceMapFilterQuery()
            : base()
        {

        }

        internal List<System.Linq.Expressions.Expression<Func<SDA_PROVINCE_MAP, bool>>> listExpression = new List<System.Linq.Expressions.Expression<Func<SDA_PROVINCE_MAP, bool>>>();

        internal Inventec.Backend.MANAGER.OrderProcessorBase OrderProcess = new Inventec.Backend.MANAGER.OrderProcessorBase();

        internal SdaProvinceMapSO Query()
        {
            SdaProvinceMapSO search = new SdaProvinceMapSO();
            try
            {
                ...
                if (!String.IsNullOrEmpty(this.PROVINCE_CODE__EXACT))
                {
                    listExpression.Add(o => o.PROVINCE_CODE == this.PROVINCE_CODE__EXACT);
                }

                search.listSdaProvinceMapExpression.AddRange(listExpression);
                search.OrderField = OrderProcess.GetOrderField<SDA_PROVINCE_MAP>(ORDER_FIELD);
                search.OrderDirection = OrderProcess.GetOrderDirection(ORDER_DIRECTION);
            }
            catch (Exception ex)
            {
                LogSystem.Error(ex);
                search.listSdaProvinceMapExpression.Clear();
                search.listSdaProvinceMapExpression.Add(o => o.ID == NEGATIVE_ID);
            }
            return search;
        }
    }
