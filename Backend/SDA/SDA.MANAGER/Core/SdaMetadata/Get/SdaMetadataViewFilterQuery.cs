...
{
    public class SdaMetadataViewFilterQuery : SdaMetadataViewFilter
    {
        public SdaMetadataViewFilterQuery()
            : base()
        {

        }
        
        internal List<System.Linq.Expressions.Expression<Func<V_SDA_METADATA, bool>>> listExpression = new List<System.Linq.Expressions.Expression<Func<V_SDA_METADATA, bool>>>();
        ...
        internal SdaMetadataSO Query()
        {
            ...
            try
            {
                ...
                search.listVSdaMetadataExpression.AddRange(listExpression);
                search.OrderField = OrderProcess.GetOrderField<V_SDA_METADATA>(ORDER_FIELD);
                search.OrderDirection = OrderProcess.GetOrderDirection(ORDER_DIRECTION);
            }
            catch (Exception ex)
            {
                LogSystem.Error(ex);
                search.listVSdaMetadataExpression.Clear();
                search.listVSdaMetadataExpression.Add(o => o.ID == NEGATIVE_ID);
            }
            return search;
        }
    }
