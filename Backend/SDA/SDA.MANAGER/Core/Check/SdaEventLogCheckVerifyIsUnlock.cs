...
using SDA.MANAGER.Core.SdaEventLog.Get;
using Inventec.Core;
using System;
...
{
    class SdaEventLogCheckVerifyIsUnlock
    {
        ...
        internal static bool Verify(CommonParam param, SDA_EVENT_LOG data)
        {
            bool result = true;
            try
            {
        ...
        internal static bool Verify(CommonParam param, List<long> ids)
        {
            ...
            try
            {
                ...
                {
                    SdaEventLogFilterQuery filter = new SdaEventLogFilterQuery();
                    filter.IDs = ids;
                    List<SDA_EVENT_LOG> listData = new SdaEventLogBO().Get<List<SDA_EVENT_LOG>>(filter);
        ...
        internal static bool Verify(CommonParam param, List<SDA_EVENT_LOG> datas)
        {
            bool result = true;
            try
            {
        ...
        private static bool Check(CommonParam param, SDA_EVENT_LOG raw)
        {
            bool result = true;
            if (raw == null)
            {
