...
using SDA.MANAGER.Core.SdaConfigAppUser.Get;
using Inventec.Core;
using System;
...
{
    class SdaConfigAppUserCheckVerifyIsUnlock
    {
        ...
        internal static bool Verify(CommonParam param, List<long> ids)
        {
            ...
            try
            {
                ...
                {
                    SdaConfigAppUserFilterQuery filter = new SdaConfigAppUserFilterQuery();
                    filter.IDs = ids;
                    List<SDA_CONFIG_APP_USER> listData = new SdaConfigAppUserBO().Get<List<SDA_CONFIG_APP_USER>>(filter);
        ...
        private static bool Check(CommonParam param, SDA_CONFIG_APP_USER raw)
        {
            bool result = true;
            if (raw == null)
            {
