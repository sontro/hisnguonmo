...
using SDA.MANAGER.Core.SdaMetadata.Get;
using Inventec.Core;
using System;
...
{
    class SdaMetadataCheckVerifyIsUnlock
    {
        ...
        internal static bool Verify(CommonParam param, SDA_METADATA data)
        {
            bool result = true;
            try
            {
        ...
        internal static bool Verify(CommonParam param, List<long> ids)
        {
            ...
            try
            {
                ...
                {
                    SdaMetadataFilterQuery filter = new SdaMetadataFilterQuery();
                    filter.IDs = ids;
                    List<SDA_METADATA> listData = new SdaMetadataBO().Get<List<SDA_METADATA>>(filter);
        ...
        internal static bool Verify(CommonParam param, List<SDA_METADATA> datas)
        {
            bool result = true;
            try
            {
        ...
        private static bool Check(CommonParam param, SDA_METADATA raw)
        {
            bool result = true;
            if (raw == null)
            {
