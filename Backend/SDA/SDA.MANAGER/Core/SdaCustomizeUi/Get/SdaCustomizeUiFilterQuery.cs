...
{
    public class SdaCustomizeUiFilterQuery : SdaCustomizeUiFilter
    {
        public SdaCustomizeUiFilterQuery()
            : base()
        {

        }

        internal List<System.Linq.Expressions.Expression<Func<SDA_CUSTOMIZE_UI, bool>>> listExpression = new List<System.Linq.Expressions.Expression<Func<SDA_CUSTOMIZE_UI, bool>>>();
        ...
        internal SdaCustomizeUiSO Query()
        {
            ...
            try
            {
                ...
                search.listSdaCustomizeUiExpression.AddRange(listExpression);
                search.OrderField = OrderProcess.GetOrderField<SDA_CUSTOMIZE_UI>(ORDER_FIELD);
                search.OrderDirection = OrderProcess.GetOrderDirection(ORDER_DIRECTION);
            }
            catch (Exception ex)
            {
                LogSystem.Error(ex);
                search.listSdaCustomizeUiExpression.Clear();
                search.listSdaCustomizeUiExpression.Add(o => o.ID == NEGATIVE_ID);
            }
            return search;
        }
    }
