...
{
    public class SdaModuleFieldViewFilterQuery : SdaModuleFieldViewFilter
    {
        public SdaModuleFieldViewFilterQuery()
            : base()
        {

        }
        
        internal List<System.Linq.Expressions.Expression<Func<V_SDA_MODULE_FIELD, bool>>> listExpression = new List<System.Linq.Expressions.Expression<Func<V_SDA_MODULE_FIELD, bool>>>();
        ...
        internal SdaModuleFieldSO Query()
        {
            ...
            try
            {
                ...
                search.listVSdaModuleFieldExpression.AddRange(listExpression);
                search.OrderField = OrderProcess.GetOrderField<V_SDA_MODULE_FIELD>(ORDER_FIELD);
                search.OrderDirection = OrderProcess.GetOrderDirection(ORDER_DIRECTION);
            }
            catch (Exception ex)
            {
                LogSystem.Error(ex);
                search.listVSdaModuleFieldExpression.Clear();
                search.listVSdaModuleFieldExpression.Add(o => o.ID == NEGATIVE_ID);
            }
            return search;
        }
    }
