...
using SDA.MANAGER.Core.SdaEthnic.Get.ListV;
using SDA.MANAGER.Core.SdaEthnic.Get.V;
using Inventec.Core;
using System;
    ...
    partial class SdaEthnicGet : BeanObjectBase, IDelegacyT
    {
        ...
        T IDelegacyT.Execute<T>()
        {
            ...
            try
            {
                ...
                else if (typeof(T) == typeof(List<V_SDA_ETHNIC>))
                {
                    ISdaEthnicGetListV behavior = SdaEthnicGetListVBehaviorFactory.MakeISdaEthnicGetListV(param, entity);
                    result = behavior != null ? (T)System.Convert.ChangeType(behavior.Run(), typeof(T)) : default(T);
                }
                else if (typeof(T) == typeof(V_SDA_ETHNIC))
                {
                    ISdaEthnicGetV behavior = SdaEthnicGetVBehaviorFactory.MakeISdaEthnicGetV(param, entity);
                    result = behavior != null ? (T)System.Convert.ChangeType(behavior.Run(), typeof(T)) : default(T);
                }
            }
            catch (Exception ex)
