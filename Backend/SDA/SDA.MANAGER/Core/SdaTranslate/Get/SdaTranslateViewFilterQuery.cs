...
{
    public class SdaTranslateViewFilterQuery : SdaTranslateViewFilter
    {
        public SdaTranslateViewFilterQuery()
            : base()
        {

        }
        
        internal List<System.Linq.Expressions.Expression<Func<V_SDA_TRANSLATE, bool>>> listExpression = new List<System.Linq.Expressions.Expression<Func<V_SDA_TRANSLATE, bool>>>();
        ...
        internal SdaTranslateSO Query()
        {
            ...
            try
            {
                ...
                search.listVSdaTranslateExpression.AddRange(listExpression);
                search.OrderField = OrderProcess.GetOrderField<V_SDA_TRANSLATE>(ORDER_FIELD);
                search.OrderDirection = OrderProcess.GetOrderDirection(ORDER_DIRECTION);
            }
            catch (Exception ex)
            {
                LogSystem.Error(ex);
                search.listVSdaTranslateExpression.Clear();
                search.listVSdaTranslateExpression.Add(o => o.ID == NEGATIVE_ID);
            }
            return search;
        }
    }
