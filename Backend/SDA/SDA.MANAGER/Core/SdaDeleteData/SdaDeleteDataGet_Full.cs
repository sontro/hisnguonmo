...
using SDA.MANAGER.Core.SdaDeleteData.Get.ListV;
using SDA.MANAGER.Core.SdaDeleteData.Get.V;
using Inventec.Core;
using System;
    ...
    partial class SdaDeleteDataGet : BeanObjectBase, IDelegacyT
    {
        ...
        T IDelegacyT.Execute<T>()
        {
            ...
            try
            {
                ...
                else if (typeof(T) == typeof(List<V_SDA_DELETE_DATA>))
                {
                    ISdaDeleteDataGetListV behavior = SdaDeleteDataGetListVBehaviorFactory.MakeISdaDeleteDataGetListV(param, entity);
                    result = behavior != null ? (T)System.Convert.ChangeType(behavior.Run(), typeof(T)) : default(T);
                }
                else if (typeof(T) == typeof(V_SDA_DELETE_DATA))
                {
                    ISdaDeleteDataGetV behavior = SdaDeleteDataGetVBehaviorFactory.MakeISdaDeleteDataGetV(param, entity);
                    result = behavior != null ? (T)System.Convert.ChangeType(behavior.Run(), typeof(T)) : default(T);
                }
            }
            catch (Exception ex)
