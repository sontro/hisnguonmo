...
{
    public class SdaDeleteDataViewFilterQuery : SdaDeleteDataViewFilter
    {
        public SdaDeleteDataViewFilterQuery()
            : base()
        {

        }
        
        internal List<System.Linq.Expressions.Expression<Func<V_SDA_DELETE_DATA, bool>>> listExpression = new List<System.Linq.Expressions.Expression<Func<V_SDA_DELETE_DATA, bool>>>();
        ...
        internal SdaDeleteDataSO Query()
        {
            ...
            try
            {
                ...
                search.listVSdaDeleteDataExpression.AddRange(listExpression);
                search.OrderField = OrderProcess.GetOrderField<V_SDA_DELETE_DATA>(ORDER_FIELD);
                search.OrderDirection = OrderProcess.GetOrderDirection(ORDER_DIRECTION);
            }
            catch (Exception ex)
            {
                LogSystem.Error(ex);
                search.listVSdaDeleteDataExpression.Clear();
                search.listVSdaDeleteDataExpression.Add(o => o.ID == NEGATIVE_ID);
            }
            return search;
        }
    }
