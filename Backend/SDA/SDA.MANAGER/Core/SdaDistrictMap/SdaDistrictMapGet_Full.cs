...
using SDA.MANAGER.Core.SdaDistrictMap.Get.ListV;
using SDA.MANAGER.Core.SdaDistrictMap.Get.V;
using Inventec.Core;
using System;
    ...
    partial class SdaDistrictMapGet : BeanObjectBase, IDelegacyT
    {
        ...
        T IDelegacyT.Execute<T>()
        {
            ...
            try
            {
                ...
                else if (typeof(T) == typeof(List<V_SDA_DISTRICT_MAP>))
                {
                    ISdaDistrictMapGetListV behavior = SdaDistrictMapGetListVBehaviorFactory.MakeISdaDistrictMapGetListV(param, entity);
                    result = behavior != null ? (T)System.Convert.ChangeType(behavior.Run(), typeof(T)) : default(T);
                }
                else if (typeof(T) == typeof(V_SDA_DISTRICT_MAP))
                {
                    ISdaDistrictMapGetV behavior = SdaDistrictMapGetVBehaviorFactory.MakeISdaDistrictMapGetV(param, entity);
                    result = behavior != null ? (T)System.Convert.ChangeType(behavior.Run(), typeof(T)) : default(T);
                }
            }
            catch (Exception ex)
