...
{
    public class SdaConfigAppViewFilterQuery : SdaConfigAppViewFilter
    {
        public SdaConfigAppViewFilterQuery()
            : base()
        {

        }
        
        internal List<System.Linq.Expressions.Expression<Func<V_SDA_CONFIG_APP, bool>>> listExpression = new List<System.Linq.Expressions.Expression<Func<V_SDA_CONFIG_APP, bool>>>();
        ...
        internal SdaConfigAppSO Query()
        {
            ...
            try
            {
                ...
                search.listVSdaConfigAppExpression.AddRange(listExpression);
                search.OrderField = OrderProcess.GetOrderField<V_SDA_CONFIG_APP>(ORDER_FIELD);
                search.OrderDirection = OrderProcess.GetOrderDirection(ORDER_DIRECTION);
            }
            catch (Exception ex)
            {
                LogSystem.Error(ex);
                search.listVSdaConfigAppExpression.Clear();
                search.listVSdaConfigAppExpression.Add(o => o.ID == NEGATIVE_ID);
            }
            return search;
        }
    }
