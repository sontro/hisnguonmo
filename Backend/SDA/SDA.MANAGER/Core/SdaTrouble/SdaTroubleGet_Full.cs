...
using SDA.MANAGER.Core.SdaTrouble.Get.ListV;
using SDA.MANAGER.Core.SdaTrouble.Get.V;
using Inventec.Core;
using System;
    ...
    partial class SdaTroubleGet : BeanObjectBase, IDelegacyT
    {
        ...
        T IDelegacyT.Execute<T>()
        {
            ...
            try
            {
                ...
                else if (typeof(T) == typeof(List<V_SDA_TROUBLE>))
                {
                    ISdaTroubleGetListV behavior = SdaTroubleGetListVBehaviorFactory.MakeISdaTroubleGetListV(param, entity);
                    result = behavior != null ? (T)System.Convert.ChangeType(behavior.Run(), typeof(T)) : default(T);
                }
                else if (typeof(T) == typeof(V_SDA_TROUBLE))
                {
                    ISdaTroubleGetV behavior = SdaTroubleGetVBehaviorFactory.MakeISdaTroubleGetV(param, entity);
                    result = behavior != null ? (T)System.Convert.ChangeType(behavior.Run(), typeof(T)) : default(T);
                }
            }
            catch (Exception ex)
