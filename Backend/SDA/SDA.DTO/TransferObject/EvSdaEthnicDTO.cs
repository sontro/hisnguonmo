...
{
    public class EvSdaEthnicDTO : SDA_EV_SDA_ETHNIC, IDTO<EvSdaEthnicDTO, SDA_EV_SDA_ETHNIC>
    {
        public EvSdaEthnicDTO()
        {
            IS_ACTIVE = ((IS_ACTIVE == null) ? (short)1 : IS_ACTIVE);
            IS_DELETE = ((IS_DELETE == null) ? (short)0 : IS_DELETE);
        }
        
        public void ProcessNullActiveDelete(EvSdaEthnicDTO data)
        {
            try
            {
                if (data != null)
        ...
        public EvSdaEthnicDTO CreateDTO(SDA_EV_SDA_ETHNIC raw)
        {
            try
            {
                if (raw != null)
                {
                    EvSdaEthnicDTO dto = new EvSdaEthnicDTO();
                    PropertyInfo[] pi = Properties.Get<SDA_EV_SDA_ETHNIC>();
        ...
        public SDA_EV_SDA_ETHNIC CreateRaw(EvSdaEthnicDTO data)
        {
            try
            {
                if (data != null)
                {
                    SDA_EV_SDA_ETHNIC raw = new SDA_EV_SDA_ETHNIC();
                    PropertyInfo[] pi = Properties.Get<SDA_EV_SDA_ETHNIC>();
                    foreach (var item in pi)
                    {
        ...
        public void UpdateDTO(SDA_EV_SDA_ETHNIC raw, EvSdaEthnicDTO data)
        {
            try
            {
        ...
        public void UpdateRaw(EvSdaEthnicDTO data, SDA_EV_SDA_ETHNIC raw)
        {
            try
            {
                #region validate param
        ...
        private void SetNotChangeField(EvSdaEthnicDTO data, SDA_EV_SDA_ETHNIC raw)
        {
            try
            {
                #region validate param
