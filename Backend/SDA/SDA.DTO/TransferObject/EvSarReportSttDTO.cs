...
{
    public class EvSarReportSttDTO : SDA_EV_SAR_REPORT_STT, IDTO<EvSarReportSttDTO, SDA_EV_SAR_REPORT_STT>
    {
        public EvSarReportSttDTO()
        {
            IS_ACTIVE = ((IS_ACTIVE == null) ? (short)1 : IS_ACTIVE);
            IS_DELETE = ((IS_DELETE == null) ? (short)0 : IS_DELETE);
        }
        
        public void ProcessNullActiveDelete(EvSarReportSttDTO data)
        {
            try
            {
                if (data != null)
        ...
        public EvSarReportSttDTO CreateDTO(SDA_EV_SAR_REPORT_STT raw)
        {
            try
            {
                if (raw != null)
                {
                    EvSarReportSttDTO dto = new EvSarReportSttDTO();
                    PropertyInfo[] pi = Properties.Get<SDA_EV_SAR_REPORT_STT>();
        ...
        public SDA_EV_SAR_REPORT_STT CreateRaw(EvSarReportSttDTO data)
        {
            try
            {
                if (data != null)
                {
                    SDA_EV_SAR_REPORT_STT raw = new SDA_EV_SAR_REPORT_STT();
                    PropertyInfo[] pi = Properties.Get<SDA_EV_SAR_REPORT_STT>();
                    foreach (var item in pi)
                    {
        ...
        public void UpdateDTO(SDA_EV_SAR_REPORT_STT raw, EvSarReportSttDTO data)
        {
            try
            {
        ...
        public void UpdateRaw(EvSarReportSttDTO data, SDA_EV_SAR_REPORT_STT raw)
        {
            try
            {
                #region validate param
        ...
        private void SetNotChangeField(EvSarReportSttDTO data, SDA_EV_SAR_REPORT_STT raw)
        {
            try
            {
                #region validate param
