...
{
    public class EvSdaFeedbackDTO : SDA_EV_SDA_FEEDBACK, IDTO<EvSdaFeedbackDTO, SDA_EV_SDA_FEEDBACK>
    {
        public EvSdaFeedbackDTO()
        {
            IS_ACTIVE = ((IS_ACTIVE == null) ? (short)1 : IS_ACTIVE);
            IS_DELETE = ((IS_DELETE == null) ? (short)0 : IS_DELETE);
        }
        
        public void ProcessNullActiveDelete(EvSdaFeedbackDTO data)
        {
            try
            {
                if (data != null)
        ...
        public EvSdaFeedbackDTO CreateDTO(SDA_EV_SDA_FEEDBACK raw)
        {
            try
            {
                if (raw != null)
                {
                    EvSdaFeedbackDTO dto = new EvSdaFeedbackDTO();
                    PropertyInfo[] pi = Properties.Get<SDA_EV_SDA_FEEDBACK>();
        ...
        public SDA_EV_SDA_FEEDBACK CreateRaw(EvSdaFeedbackDTO data)
        {
            try
            {
                if (data != null)
                {
                    SDA_EV_SDA_FEEDBACK raw = new SDA_EV_SDA_FEEDBACK();
                    PropertyInfo[] pi = Properties.Get<SDA_EV_SDA_FEEDBACK>();
                    foreach (var item in pi)
                    {
        ...
        public void UpdateDTO(SDA_EV_SDA_FEEDBACK raw, EvSdaFeedbackDTO data)
        {
            try
            {
        ...
        public void UpdateRaw(EvSdaFeedbackDTO data, SDA_EV_SDA_FEEDBACK raw)
        {
            try
            {
                #region validate param
        ...
        private void SetNotChangeField(EvSdaFeedbackDTO data, SDA_EV_SDA_FEEDBACK raw)
        {
            try
            {
                #region validate param
