...
{
    public class EvSdaGroupTypeDTO : SDA_EV_SDA_GROUP_TYPE, IDTO<EvSdaGroupTypeDTO, SDA_EV_SDA_GROUP_TYPE>
    {
        public EvSdaGroupTypeDTO()
        {
            IS_ACTIVE = ((IS_ACTIVE == null) ? (short)1 : IS_ACTIVE);
            IS_DELETE = ((IS_DELETE == null) ? (short)0 : IS_DELETE);
        }
        
        public void ProcessNullActiveDelete(EvSdaGroupTypeDTO data)
        {
            try
            {
                if (data != null)
        ...
        public EvSdaGroupTypeDTO CreateDTO(SDA_EV_SDA_GROUP_TYPE raw)
        {
            try
            {
                if (raw != null)
                {
                    EvSdaGroupTypeDTO dto = new EvSdaGroupTypeDTO();
                    PropertyInfo[] pi = Properties.Get<SDA_EV_SDA_GROUP_TYPE>();
        ...
        public SDA_EV_SDA_GROUP_TYPE CreateRaw(EvSdaGroupTypeDTO data)
        {
            try
            {
                if (data != null)
                {
                    SDA_EV_SDA_GROUP_TYPE raw = new SDA_EV_SDA_GROUP_TYPE();
                    PropertyInfo[] pi = Properties.Get<SDA_EV_SDA_GROUP_TYPE>();
                    foreach (var item in pi)
                    {
        ...
        public void UpdateDTO(SDA_EV_SDA_GROUP_TYPE raw, EvSdaGroupTypeDTO data)
        {
            try
            {
        ...
        public void UpdateRaw(EvSdaGroupTypeDTO data, SDA_EV_SDA_GROUP_TYPE raw)
        {
            try
            {
                #region validate param
        ...
        private void SetNotChangeField(EvSdaGroupTypeDTO data, SDA_EV_SDA_GROUP_TYPE raw)
        {
            try
            {
                #region validate param
