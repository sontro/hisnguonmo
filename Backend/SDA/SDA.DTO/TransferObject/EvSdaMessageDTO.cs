...
{
    public class EvSdaMessageDTO : SDA_EV_SDA_MESSAGE, IDTO<EvSdaMessageDTO, SDA_EV_SDA_MESSAGE>
    {
        public EvSdaMessageDTO()
        {
            IS_ACTIVE = ((IS_ACTIVE == null) ? (short)1 : IS_ACTIVE);
            IS_DELETE = ((IS_DELETE == null) ? (short)0 : IS_DELETE);
        }
        
        public void ProcessNullActiveDelete(EvSdaMessageDTO data)
        {
            try
            {
                if (data != null)
        ...
        public EvSdaMessageDTO CreateDTO(SDA_EV_SDA_MESSAGE raw)
        {
            try
            {
                if (raw != null)
                {
                    EvSdaMessageDTO dto = new EvSdaMessageDTO();
                    PropertyInfo[] pi = Properties.Get<SDA_EV_SDA_MESSAGE>();
        ...
        public SDA_EV_SDA_MESSAGE CreateRaw(EvSdaMessageDTO data)
        {
            try
            {
                if (data != null)
                {
                    SDA_EV_SDA_MESSAGE raw = new SDA_EV_SDA_MESSAGE();
                    PropertyInfo[] pi = Properties.Get<SDA_EV_SDA_MESSAGE>();
                    foreach (var item in pi)
                    {
        ...
        public void UpdateDTO(SDA_EV_SDA_MESSAGE raw, EvSdaMessageDTO data)
        {
            try
            {
        ...
        public void UpdateRaw(EvSdaMessageDTO data, SDA_EV_SDA_MESSAGE raw)
        {
            try
            {
                #region validate param
        ...
        private void SetNotChangeField(EvSdaMessageDTO data, SDA_EV_SDA_MESSAGE raw)
        {
            try
            {
                #region validate param
