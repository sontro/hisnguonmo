...
{
    public class EvSdaTroubleDTO : SDA_EV_SDA_TROUBLE, IDTO<EvSdaTroubleDTO, SDA_EV_SDA_TROUBLE>
    {
        public EvSdaTroubleDTO()
        {
            IS_ACTIVE = ((IS_ACTIVE == null) ? (short)1 : IS_ACTIVE);
            IS_DELETE = ((IS_DELETE == null) ? (short)0 : IS_DELETE);
        }
        
        public void ProcessNullActiveDelete(EvSdaTroubleDTO data)
        {
            try
            {
                if (data != null)
        ...
        public EvSdaTroubleDTO CreateDTO(SDA_EV_SDA_TROUBLE raw)
        {
            try
            {
                if (raw != null)
                {
                    EvSdaTroubleDTO dto = new EvSdaTroubleDTO();
                    PropertyInfo[] pi = Properties.Get<SDA_EV_SDA_TROUBLE>();
        ...
        public SDA_EV_SDA_TROUBLE CreateRaw(EvSdaTroubleDTO data)
        {
            try
            {
                if (data != null)
                {
                    SDA_EV_SDA_TROUBLE raw = new SDA_EV_SDA_TROUBLE();
                    PropertyInfo[] pi = Properties.Get<SDA_EV_SDA_TROUBLE>();
                    foreach (var item in pi)
                    {
        ...
        public void UpdateDTO(SDA_EV_SDA_TROUBLE raw, EvSdaTroubleDTO data)
        {
            try
            {
        ...
        public void UpdateRaw(EvSdaTroubleDTO data, SDA_EV_SDA_TROUBLE raw)
        {
            try
            {
                #region validate param
        ...
        private void SetNotChangeField(EvSdaTroubleDTO data, SDA_EV_SDA_TROUBLE raw)
        {
            try
            {
                #region validate param
