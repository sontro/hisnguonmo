...
{
    public class EvSdaLicenseDTO : SDA_EV_SDA_LICENSE, IDTO<EvSdaLicenseDTO, SDA_EV_SDA_LICENSE>
    {
        public EvSdaLicenseDTO()
        {
            IS_ACTIVE = ((IS_ACTIVE == null) ? (short)1 : IS_ACTIVE);
            IS_DELETE = ((IS_DELETE == null) ? (short)0 : IS_DELETE);
        }
        
        public void ProcessNullActiveDelete(EvSdaLicenseDTO data)
        {
            try
            {
                if (data != null)
        ...
        public EvSdaLicenseDTO CreateDTO(SDA_EV_SDA_LICENSE raw)
        {
            try
            {
                if (raw != null)
                {
                    EvSdaLicenseDTO dto = new EvSdaLicenseDTO();
                    PropertyInfo[] pi = Properties.Get<SDA_EV_SDA_LICENSE>();
        ...
        public SDA_EV_SDA_LICENSE CreateRaw(EvSdaLicenseDTO data)
        {
            try
            {
                if (data != null)
                {
                    SDA_EV_SDA_LICENSE raw = new SDA_EV_SDA_LICENSE();
                    PropertyInfo[] pi = Properties.Get<SDA_EV_SDA_LICENSE>();
                    foreach (var item in pi)
                    {
        ...
        public void UpdateDTO(SDA_EV_SDA_LICENSE raw, EvSdaLicenseDTO data)
        {
            try
            {
        ...
        public void UpdateRaw(EvSdaLicenseDTO data, SDA_EV_SDA_LICENSE raw)
        {
            try
            {
                #region validate param
        ...
        private void SetNotChangeField(EvSdaLicenseDTO data, SDA_EV_SDA_LICENSE raw)
        {
            try
            {
                #region validate param
