...
{
    public class EvSdaDbLogDTO : SDA_EV_SDA_DB_LOG, IDTO<EvSdaDbLogDTO, SDA_EV_SDA_DB_LOG>
    {
        public EvSdaDbLogDTO()
        {
            IS_ACTIVE = ((IS_ACTIVE == null) ? (short)1 : IS_ACTIVE);
            IS_DELETE = ((IS_DELETE == null) ? (short)0 : IS_DELETE);
        }
        
        public void ProcessNullActiveDelete(EvSdaDbLogDTO data)
        {
            try
            {
                if (data != null)
        ...
        public EvSdaDbLogDTO CreateDTO(SDA_EV_SDA_DB_LOG raw)
        {
            try
            {
                if (raw != null)
                {
                    EvSdaDbLogDTO dto = new EvSdaDbLogDTO();
                    PropertyInfo[] pi = Properties.Get<SDA_EV_SDA_DB_LOG>();
        ...
        public SDA_EV_SDA_DB_LOG CreateRaw(EvSdaDbLogDTO data)
        {
            try
            {
                if (data != null)
                {
                    SDA_EV_SDA_DB_LOG raw = new SDA_EV_SDA_DB_LOG();
                    PropertyInfo[] pi = Properties.Get<SDA_EV_SDA_DB_LOG>();
                    foreach (var item in pi)
                    {
        ...
        public void UpdateDTO(SDA_EV_SDA_DB_LOG raw, EvSdaDbLogDTO data)
        {
            try
            {
        ...
        public void UpdateRaw(EvSdaDbLogDTO data, SDA_EV_SDA_DB_LOG raw)
        {
            try
            {
                #region validate param
        ...
        private void SetNotChangeField(EvSdaDbLogDTO data, SDA_EV_SDA_DB_LOG raw)
        {
            try
            {
                #region validate param
