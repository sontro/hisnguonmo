...
{
    public class EvSdaProvinceDTO : SDA_EV_SDA_PROVINCE, IDTO<EvSdaProvinceDTO, SDA_EV_SDA_PROVINCE>
    {
        public EvSdaProvinceDTO()
        {
            IS_ACTIVE = ((IS_ACTIVE == null) ? (short)1 : IS_ACTIVE);
            IS_DELETE = ((IS_DELETE == null) ? (short)0 : IS_DELETE);
        }
        
        public void ProcessNullActiveDelete(EvSdaProvinceDTO data)
        {
            try
            {
                if (data != null)
        ...
        public EvSdaProvinceDTO CreateDTO(SDA_EV_SDA_PROVINCE raw)
        {
            try
            {
                if (raw != null)
                {
                    EvSdaProvinceDTO dto = new EvSdaProvinceDTO();
                    PropertyInfo[] pi = Properties.Get<SDA_EV_SDA_PROVINCE>();
        ...
        public SDA_EV_SDA_PROVINCE CreateRaw(EvSdaProvinceDTO data)
        {
            try
            {
                if (data != null)
                {
                    SDA_EV_SDA_PROVINCE raw = new SDA_EV_SDA_PROVINCE();
                    PropertyInfo[] pi = Properties.Get<SDA_EV_SDA_PROVINCE>();
                    foreach (var item in pi)
                    {
        ...
        public void UpdateDTO(SDA_EV_SDA_PROVINCE raw, EvSdaProvinceDTO data)
        {
            try
            {
        ...
        public void UpdateRaw(EvSdaProvinceDTO data, SDA_EV_SDA_PROVINCE raw)
        {
            try
            {
                #region validate param
        ...
        private void SetNotChangeField(EvSdaProvinceDTO data, SDA_EV_SDA_PROVINCE raw)
        {
            try
            {
                #region validate param
