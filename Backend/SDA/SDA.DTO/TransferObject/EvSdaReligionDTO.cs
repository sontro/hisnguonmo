...
{
    public class EvSdaReligionDTO : SDA_EV_SDA_RELIGION, IDTO<EvSdaReligionDTO, SDA_EV_SDA_RELIGION>
    {
        public EvSdaReligionDTO()
        {
            IS_ACTIVE = ((IS_ACTIVE == null) ? (short)1 : IS_ACTIVE);
            IS_DELETE = ((IS_DELETE == null) ? (short)0 : IS_DELETE);
        }
        
        public void ProcessNullActiveDelete(EvSdaReligionDTO data)
        {
            try
            {
                if (data != null)
        ...
        public EvSdaReligionDTO CreateDTO(SDA_EV_SDA_RELIGION raw)
        {
            try
            {
                if (raw != null)
                {
                    EvSdaReligionDTO dto = new EvSdaReligionDTO();
                    PropertyInfo[] pi = Properties.Get<SDA_EV_SDA_RELIGION>();
        ...
        public SDA_EV_SDA_RELIGION CreateRaw(EvSdaReligionDTO data)
        {
            try
            {
                if (data != null)
                {
                    SDA_EV_SDA_RELIGION raw = new SDA_EV_SDA_RELIGION();
                    PropertyInfo[] pi = Properties.Get<SDA_EV_SDA_RELIGION>();
                    foreach (var item in pi)
                    {
        ...
        public void UpdateDTO(SDA_EV_SDA_RELIGION raw, EvSdaReligionDTO data)
        {
            try
            {
        ...
        public void UpdateRaw(EvSdaReligionDTO data, SDA_EV_SDA_RELIGION raw)
        {
            try
            {
                #region validate param
        ...
        private void SetNotChangeField(EvSdaReligionDTO data, SDA_EV_SDA_RELIGION raw)
        {
            try
            {
                #region validate param
