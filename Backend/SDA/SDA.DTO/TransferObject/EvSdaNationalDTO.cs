...
{
    public class EvSdaNationalDTO : SDA_EV_SDA_NATIONAL, IDTO<EvSdaNationalDTO, SDA_EV_SDA_NATIONAL>
    {
        public EvSdaNationalDTO()
        {
            IS_ACTIVE = ((IS_ACTIVE == null) ? (short)1 : IS_ACTIVE);
            IS_DELETE = ((IS_DELETE == null) ? (short)0 : IS_DELETE);
        }
        
        public void ProcessNullActiveDelete(EvSdaNationalDTO data)
        {
            try
            {
                if (data != null)
        ...
        public EvSdaNationalDTO CreateDTO(SDA_EV_SDA_NATIONAL raw)
        {
            try
            {
                if (raw != null)
                {
                    EvSdaNationalDTO dto = new EvSdaNationalDTO();
                    PropertyInfo[] pi = Properties.Get<SDA_EV_SDA_NATIONAL>();
        ...
        public SDA_EV_SDA_NATIONAL CreateRaw(EvSdaNationalDTO data)
        {
            try
            {
                if (data != null)
                {
                    SDA_EV_SDA_NATIONAL raw = new SDA_EV_SDA_NATIONAL();
                    PropertyInfo[] pi = Properties.Get<SDA_EV_SDA_NATIONAL>();
                    foreach (var item in pi)
                    {
        ...
        public void UpdateDTO(SDA_EV_SDA_NATIONAL raw, EvSdaNationalDTO data)
        {
            try
            {
        ...
        public void UpdateRaw(EvSdaNationalDTO data, SDA_EV_SDA_NATIONAL raw)
        {
            try
            {
                #region validate param
        ...
        private void SetNotChangeField(EvSdaNationalDTO data, SDA_EV_SDA_NATIONAL raw)
        {
            try
            {
                #region validate param
