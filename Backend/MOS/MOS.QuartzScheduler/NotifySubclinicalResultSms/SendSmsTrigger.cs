...
namespace MOS.QuartzScheduler.NotifySubclinicalResultSms
{
    class SendSmsTrigger
    {
        internal static void AddJob()
        {
            try
            {
                ...
                if (GetRepetitionIntervalTime(ref repeatTime))
                {
                    ...
                    IJobDetail job = JobBuilder.Create<SendSmsJob>().WithIdentity("SendSmsJob", "SendSms").Build();

                    ITrigger trigger = TriggerBuilder.Create()
                      .WithIdentity("SendSmsTrigger", "SendSms").StartNow().WithSimpleSchedule(x => x.WithInterval(repeatTime).RepeatForever()).Build();
        ...
        public static bool GetRepetitionIntervalTime(ref TimeSpan repeatTime)
        {
            try
            {
                int time = int.Parse(ConfigurationManager.AppSettings["MOS.API.Scheduler.NotifySubclinicalResultSms"]);
