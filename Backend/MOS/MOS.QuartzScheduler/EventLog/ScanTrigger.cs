...
namespace MOS.QuartzScheduler.EventLog
{
    internal class ScanTrigger
    {
        ...
        public static bool GetRepetitionIntervalTime(ref TimeSpan repeatTime)
        {
            try
            {
                //repeatTime = TimeSpan.FromMilliseconds(int.Parse(ConfigurationManager.AppSettings["MOS.QuartzScheduler.ScanEventLogJob"]));
                int time = int.Parse(ConfigurationManager.AppSettings["MOS.API.Scheduler.ScanEventLogJob"]);
