...
{
    partial class HisPtttCalendarLock : BusinessBase
    {
        internal HisPtttCalendarLock()
            : base()
        {

        }

        internal HisPtttCalendarLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_PTTT_CALENDAR resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_PTTT_CALENDAR data = new HisPtttCalendarGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisPtttCalendarDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_PTTT_CALENDAR resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_PTTT_CALENDAR resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisPtttCalendarCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisPtttCalendarDAO.Update(raw);
