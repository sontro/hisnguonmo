...
{
    partial class HisPrepareLock : BusinessBase
    {
        internal HisPrepareLock()
            : base()
        {

        }

        internal HisPrepareLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_PREPARE resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_PREPARE data = new HisPrepareGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisPrepareDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_PREPARE resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_PREPARE resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisPrepareCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisPrepareDAO.Update(raw);
