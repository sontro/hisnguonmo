...
namespace MOS.MANAGER.HisEmotionlessMethod
{
    ...
    /// </summary>
    partial class HisEmotionlessMethodCheck : BusinessBase
    {
        internal HisEmotionlessMethodCheck()
            : base()
        {

        }

        internal HisEmotionlessMethodCheck(CommonParam paramCheck)
            : base(paramCheck) 
        {

        }

        internal bool VerifyRequireField(HIS_EMOTIONLESS_METHOD data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisEmotionlessMethodGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_EMOTIONLESS_METHOD data)
        {
            bool valid = true;
            try
            {
                data = new HisEmotionlessMethodGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                if (listId != null && listId.Count > 0)
                {
                    ...
                    filter.IDs = listId;
                    List<HIS_EMOTIONLESS_METHOD> listData = new HisEmotionlessMethodGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_EMOTIONLESS_METHOD> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_EMOTIONLESS_METHOD data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisEmotionlessMethodDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_EMOTIONLESS_METHOD> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool ExistsCode(string code, long? id)
        {
            ...
            try
            {
                if (DAOWorker.HisEmotionlessMethodDAO.ExistsCode(code, id))
                {
        ...
        internal bool CheckConstraint(long id)
        {
            ...
            try
            {
                List<HIS_SERE_SERV_PTTT> hisSereServPttts = new HisSereServPtttGet().GetByEmotionlessMethodId(id);
