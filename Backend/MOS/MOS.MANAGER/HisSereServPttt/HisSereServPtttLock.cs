...
{
    partial class HisSereServPtttLock : BusinessBase
    {
        internal HisSereServPtttLock()
            : base()
        {

        }

        internal HisSereServPtttLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_SERE_SERV_PTTT data)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisSereServPtttCheck().VerifyId(data.ID, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisSereServPtttDAO.Update(raw);
