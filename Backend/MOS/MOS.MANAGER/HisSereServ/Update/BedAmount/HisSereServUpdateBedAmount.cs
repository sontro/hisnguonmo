...
{
    class HisSereServUpdateBedAmount : BusinessBase
    {
        private HIS_SERE_SERV before = null;

        internal HisSereServUpdateBedAmount()
            : base()
        {
            this.Init();
        }

        internal HisSereServUpdateBedAmount(CommonParam param)
            : base(param)
        {
            this.Init();
        }
        ...
        internal bool Run(UpdateBedAmountSDO data, ref HIS_SERE_SERV resultData)
        {
            ...
            try
            {
                ...
                valid = valid && checker.HasNoBill(sereServ);
                valid = valid && checker.HasNoDebt(new List<long>() { sereServ.ID });
                valid = valid && commonChecker.IsValidData(data, sereServ, workPlace, ref serviceReq, ref treatment, ref bedLog);
                ...
                if (valid)
                {
                    ...
                    result = true;
                    resultData = sereServ;
                    new EventLogGenerator(EventLog.Enum.HisSereServ_CapNhatSoLuongGiuong, this.before.AMOUNT, sereServ.AMOUNT)
