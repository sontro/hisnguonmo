...
namespace MOS.MANAGER.HisPatientProgram
{
    ...
    /// </summary>
    partial class HisPatientProgramCheck : BusinessBase
    {
        internal HisPatientProgramCheck()
            : base()
        {

        }

        internal HisPatientProgramCheck(CommonParam paramCheck)
            : base(paramCheck) 
        {

        }

        internal bool VerifyRequireField(HIS_PATIENT_PROGRAM data)
        {
            bool valid = true;
            try
            {
                ...
                if (data.PROGRAM_ID <= 0) throw new ArgumentNullException("data.PATIENT_ID");
            }
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisPatientProgramGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_PATIENT_PROGRAM data)
        {
            bool valid = true;
            try
            {
                data = new HisPatientProgramGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                if (listId != null && listId.Count > 0)
                {
                    ...
                    filter.IDs = listId;
                    List<HIS_PATIENT_PROGRAM> listData = new HisPatientProgramGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_PATIENT_PROGRAM> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_PATIENT_PROGRAM data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisPatientProgramDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_PATIENT_PROGRAM> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsNotExist(HIS_PATIENT_PROGRAM data)
        {
            bool valid = true;
            try
            {
                ...
                filter.PROGRAM_ID = data.PROGRAM_ID;
                filter.ID__NOT_EQUAL = data.ID;
                List<HIS_PATIENT_PROGRAM> list = new HisPatientProgramGet().Get(filter);
