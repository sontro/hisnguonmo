...
{
    partial class HisBcsMatyReqReqLock : BusinessBase
    {
        internal HisBcsMatyReqReqLock()
            : base()
        {

        }

        internal HisBcsMatyReqReqLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_BCS_MATY_REQ_REQ resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_BCS_MATY_REQ_REQ data = new HisBcsMatyReqReqGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisBcsMatyReqReqDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_BCS_MATY_REQ_REQ resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_BCS_MATY_REQ_REQ resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisBcsMatyReqReqCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisBcsMatyReqReqDAO.Update(raw);
