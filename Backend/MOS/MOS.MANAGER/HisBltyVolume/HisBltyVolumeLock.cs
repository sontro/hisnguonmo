...
{
    partial class HisBltyVolumeLock : BusinessBase
    {
        internal HisBltyVolumeLock()
            : base()
        {

        }

        internal HisBltyVolumeLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_BLTY_VOLUME resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_BLTY_VOLUME data = new HisBltyVolumeGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisBltyVolumeDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_BLTY_VOLUME resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_BLTY_VOLUME resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisBltyVolumeCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisBltyVolumeDAO.Update(raw);
