...
namespace MOS.MANAGER.HisDeathCertBook
{
    partial class HisDeathCertBookCreate : BusinessBase
    {
		...
		internal bool CreateList(List<HIS_DEATH_CERT_BOOK> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    if (!DAOWorker.HisDeathCertBookDAO.CreateList(listData))
                    {
                        MOS.MANAGER.Base.BugUtil.SetBugCode(param, MOS.LibraryBug.Bug.Enum.HisDeathCertBook_ThemMoiThatBai);
                        throw new Exception("Them moi thong tin HisDeathCertBook that bai." + LogUtil.TraceData("listData", listData));
                    }
                    this.recentHisDeathCertBooks.AddRange(listData);
                    result = true;
                }
            }
            catch (Exception ex)
