...
{
    partial class HisDeathCertBookLock : BusinessBase
    {
        internal HisDeathCertBookLock()
            : base()
        {

        }

        internal HisDeathCertBookLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_DEATH_CERT_BOOK resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_DEATH_CERT_BOOK data = new HisDeathCertBookGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisDeathCertBookDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_DEATH_CERT_BOOK resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_DEATH_CERT_BOOK resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisDeathCertBookCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisDeathCertBookDAO.Update(raw);
