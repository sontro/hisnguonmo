...
{
    partial class HisKskOccupationalLock : BusinessBase
    {
        internal HisKskOccupationalLock()
            : base()
        {

        }

        internal HisKskOccupationalLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_KSK_OCCUPATIONAL resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_KSK_OCCUPATIONAL data = new HisKskOccupationalGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisKskOccupationalDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_KSK_OCCUPATIONAL resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_KSK_OCCUPATIONAL resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisKskOccupationalCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisKskOccupationalDAO.Update(raw);
