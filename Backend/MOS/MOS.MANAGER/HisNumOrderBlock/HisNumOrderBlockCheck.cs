...
{
    partial class HisNumOrderBlockCheck : BusinessBase
    {
        internal HisNumOrderBlockCheck()
            : base()
        {

        }

        internal HisNumOrderBlockCheck(CommonParam paramCheck)
            : base(paramCheck) 
        {

        }

        internal bool VerifyRequireField(HIS_NUM_ORDER_BLOCK data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisNumOrderBlockGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_NUM_ORDER_BLOCK data)
        {
            bool valid = true;
            try
            {
                data = new HisNumOrderBlockGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                if (listId != null && listId.Count > 0)
                {
                    ...
                    filter.IDs = listId;
                    List<HIS_NUM_ORDER_BLOCK> listData = new HisNumOrderBlockGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_NUM_ORDER_BLOCK> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_NUM_ORDER_BLOCK data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisNumOrderBlockDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_NUM_ORDER_BLOCK> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool CheckConstraint(long id)
        {
            ...
            try
            {
                /*List<XXX> hisXXXs = new HisXXXGet().GetByHisNumOrderBlockId(id);
        ...
        internal bool IsNotExists(HIS_NUM_ORDER_BLOCK data)
        {
            bool valid = true;
            try
            {
                HisNumOrderBlockFilterQuery filter = new HisNumOrderBlockFilterQuery();
                filter.NUM_ORDER = data.NUM_ORDER;
                filter.ROOM_TIME_ID = data.ROOM_TIME_ID;
                filter.ID__NOT_EQUAL = data.ID;

                List<HIS_NUM_ORDER_BLOCK> exists = new HisNumOrderBlockGet().Get(filter);

                if (IsNotNullOrEmpty(exists))
                {
                    MOS.MANAGER.Base.MessageUtil.SetMessage(param, MOS.LibraryMessage.Message.Enum.HisNumOrderBlock_TrungStt, data.NUM_ORDER.ToString());
                    return false;
                }
            }
            catch (Exception ex)
