...
{
    partial class HisMedicineMaterialLock : BusinessBase
    {
        internal HisMedicineMaterialLock()
            : base()
        {

        }

        internal HisMedicineMaterialLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_MEDICINE_MATERIAL resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_MEDICINE_MATERIAL data = new HisMedicineMaterialGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisMedicineMaterialDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_MEDICINE_MATERIAL resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_MEDICINE_MATERIAL resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisMedicineMaterialCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisMedicineMaterialDAO.Update(raw);
