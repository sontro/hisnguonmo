...
{
    partial class HisMedicineMaterialCheck : BusinessBase
    {
        internal HisMedicineMaterialCheck()
            : base()
        {

        }

        internal HisMedicineMaterialCheck(CommonParam paramCheck)
            : base(paramCheck)
        {

        }

        internal bool VerifyRequireField(HIS_MEDICINE_MATERIAL data)
        {
            bool valid = true;
            try
            {
                ...
                if (data.MATERIAL_ID <= 0) throw new ArgumentNullException("data.MATERIAL_ID");
                if (data.MATERIAL_AMOUNT <= 0) throw new ArgumentNullException("data.MATERIAL_AMOUNT");
            }
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisMedicineMaterialGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_MEDICINE_MATERIAL data)
        {
            bool valid = true;
            try
            {
                data = new HisMedicineMaterialGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                ...
                {
                    HisMedicineMaterialFilterQuery filter = new HisMedicineMaterialFilterQuery();
                    filter.IDs = listId;
                    List<HIS_MEDICINE_MATERIAL> listData = new HisMedicineMaterialGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_MEDICINE_MATERIAL> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_MEDICINE_MATERIAL data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisMedicineMaterialDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_MEDICINE_MATERIAL> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool CheckConstraint(long id)
        {
            ...
            try
            {
                /*List<XXX> hisXXXs = new HisXXXGet().GetByHisMedicineMaterialId(id);
