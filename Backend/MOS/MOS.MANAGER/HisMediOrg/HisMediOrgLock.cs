...
{
    partial class HisMediOrgLock : BusinessBase
    {
        internal HisMediOrgLock()
            : base()
        {

        }

        internal HisMediOrgLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool Lock(long id, ref HIS_MEDI_ORG resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_MEDI_ORG data = new HisMediOrgGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisMediOrgDAO.Update(data);
        ...
        internal bool Unlock(long id, ref HIS_MEDI_ORG resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_MEDI_ORG resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisMediOrgCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisMediOrgDAO.Update(raw);
