...
{
    partial class HisProcessingMethodLock : BusinessBase
    {
        internal HisProcessingMethodLock()
            : base()
        {

        }

        internal HisProcessingMethodLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_PROCESSING_METHOD resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_PROCESSING_METHOD data = new HisProcessingMethodGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisProcessingMethodDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_PROCESSING_METHOD resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_PROCESSING_METHOD resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisProcessingMethodCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisProcessingMethodDAO.Update(raw);
