...
namespace MOS.MANAGER.HisProcessingMethod
{
    partial class HisProcessingMethodCreate : BusinessBase
    {
		...
		internal bool CreateList(List<HIS_PROCESSING_METHOD> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    if (!DAOWorker.HisProcessingMethodDAO.CreateList(listData))
                    {
                        MOS.MANAGER.Base.BugUtil.SetBugCode(param, MOS.LibraryBug.Bug.Enum.HisProcessingMethod_ThemMoiThatBai);
                        throw new Exception("Them moi thong tin HisProcessingMethod that bai." + LogUtil.TraceData("listData", listData));
                    }
                    this.recentHisProcessingMethods.AddRange(listData);
                    result = true;
                }
            }
            catch (Exception ex)
