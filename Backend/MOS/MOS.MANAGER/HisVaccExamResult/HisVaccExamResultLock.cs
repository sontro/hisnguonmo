...
{
    partial class HisVaccExamResultLock : BusinessBase
    {
        internal HisVaccExamResultLock()
            : base()
        {

        }

        internal HisVaccExamResultLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_VACC_EXAM_RESULT resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_VACC_EXAM_RESULT data = new HisVaccExamResultGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisVaccExamResultDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_VACC_EXAM_RESULT resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_VACC_EXAM_RESULT resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisVaccExamResultCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisVaccExamResultDAO.Update(raw);
