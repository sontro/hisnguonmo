...
namespace MOS.MANAGER.HisAccidentHurtType
{
    ...
    /// </summary>
    partial class HisAccidentHurtTypeCheck : BusinessBase
    {
        internal HisAccidentHurtTypeCheck()
            : base()
        {

        }

        internal HisAccidentHurtTypeCheck(CommonParam paramCheck)
            : base(paramCheck)
        {

        }

        internal bool VerifyRequireField(HIS_ACCIDENT_HURT_TYPE data)
        {
            bool valid = true;
            try
            {
                if (data == null) throw new ArgumentNullException("data");
                if (!IsNotNullOrEmpty(data.ACCIDENT_HURT_TYPE_CODE)) throw new ArgumentNullException("data.ACCIDENT_HURT_TYPE_CODE");
                if (!IsNotNullOrEmpty(data.ACCIDENT_HURT_TYPE_NAME)) throw new ArgumentNullException("data.ACCIDENT_HURT_TYPE_NAME");
            }
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisAccidentHurtTypeGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_ACCIDENT_HURT_TYPE data)
        {
            bool valid = true;
            try
            {
                data = new HisAccidentHurtTypeGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                ...
                {
                    HisAccidentHurtTypeFilterQuery filter = new HisAccidentHurtTypeFilterQuery();
                    filter.IDs = listId;
                    List<HIS_ACCIDENT_HURT_TYPE> listData = new HisAccidentHurtTypeGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_ACCIDENT_HURT_TYPE> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_ACCIDENT_HURT_TYPE data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisAccidentHurtTypeDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_ACCIDENT_HURT_TYPE> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool CheckConstraint(long accidentHurtTypeId)
        {
            bool valid = true;
            try
            {
                List<HIS_ACCIDENT_HURT> hisAccidentHurts = new HisAccidentHurtGet().GetByAccidentHurtTypeId(accidentHurtTypeId);
                if (IsNotNullOrEmpty(hisAccidentHurts))
                {
                    ...
                    throw new Exception("Ton tai du lieu HIS_ACCIDENT_HURT, khong cho phep xoa" + LogUtil.TraceData("accidentHurtTypeId", accidentHurtTypeId));
                }
            }
            catch (Exception ex)
            {
