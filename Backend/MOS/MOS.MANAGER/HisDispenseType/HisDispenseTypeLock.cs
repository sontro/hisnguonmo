...
{
    partial class HisDispenseTypeLock : BusinessBase
    {
        internal HisDispenseTypeLock()
            : base()
        {

        }

        internal HisDispenseTypeLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_DISPENSE_TYPE resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_DISPENSE_TYPE data = new HisDispenseTypeGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisDispenseTypeDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_DISPENSE_TYPE resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_DISPENSE_TYPE resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisDispenseTypeCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisDispenseTypeDAO.Update(raw);
