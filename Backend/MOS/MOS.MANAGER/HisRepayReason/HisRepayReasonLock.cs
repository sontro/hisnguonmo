...
{
    partial class HisRepayReasonLock : BusinessBase
    {
        internal HisRepayReasonLock()
            : base()
        {

        }

        internal HisRepayReasonLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_REPAY_REASON resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_REPAY_REASON data = new HisRepayReasonGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisRepayReasonDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_REPAY_REASON resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_REPAY_REASON resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisRepayReasonCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisRepayReasonDAO.Update(raw);
