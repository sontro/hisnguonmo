...
{
    partial class HisMediRecordBorrowLock : BusinessBase
    {
        internal HisMediRecordBorrowLock()
            : base()
        {

        }

        internal HisMediRecordBorrowLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_MEDI_RECORD_BORROW resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_MEDI_RECORD_BORROW data = new HisMediRecordBorrowGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisMediRecordBorrowDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_MEDI_RECORD_BORROW resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_MEDI_RECORD_BORROW resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisMediRecordBorrowCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisMediRecordBorrowDAO.Update(raw);
