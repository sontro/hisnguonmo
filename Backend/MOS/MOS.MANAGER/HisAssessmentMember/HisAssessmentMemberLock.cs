...
{
    partial class HisAssessmentMemberLock : BusinessBase
    {
        internal HisAssessmentMemberLock()
            : base()
        {

        }

        internal HisAssessmentMemberLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_ASSESSMENT_MEMBER resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_ASSESSMENT_MEMBER data = new HisAssessmentMemberGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisAssessmentMemberDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_ASSESSMENT_MEMBER resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_ASSESSMENT_MEMBER resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisAssessmentMemberCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisAssessmentMemberDAO.Update(raw);
