...
{
    partial class HisMilitaryRankLock : BusinessBase
    {
        internal HisMilitaryRankLock()
            : base()
        {

        }

        internal HisMilitaryRankLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_MILITARY_RANK data)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisMilitaryRankCheck().VerifyId(data.ID, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisMilitaryRankDAO.Update(raw);
