...
namespace MOS.MANAGER.HisMilitaryRank
{
    ...
    /// </summary>
    partial class HisMilitaryRankCheck : BusinessBase
    {
        internal HisMilitaryRankCheck()
            : base()
        {

        }

        internal HisMilitaryRankCheck(CommonParam paramCheck)
            : base(paramCheck) 
        {

        }

        internal bool VerifyRequireField(HIS_MILITARY_RANK data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisMilitaryRankGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_MILITARY_RANK data)
        {
            bool valid = true;
            try
            {
                data = new HisMilitaryRankGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                if (listId != null && listId.Count > 0)
                {
                    ...
                    filter.IDs = listId;
                    List<HIS_MILITARY_RANK> listData = new HisMilitaryRankGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_MILITARY_RANK> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_MILITARY_RANK data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisMilitaryRankDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_MILITARY_RANK> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool ExistsCode(string code, long? id)
        {
            ...
            try
            {
                if (DAOWorker.HisMilitaryRankDAO.ExistsCode(code, id))
                {
        ...
        internal bool CheckConstraint(HIS_MILITARY_RANK data)
        {
            bool valid = true;
            try
            {
                List<HIS_PATIENT> hisPatients = new HisPatientGet().GetByMilitaryRankId(data.ID);
                if (IsNotNullOrEmpty(hisPatients))
                {
                    ...
                    throw new Exception("Ton tai du lieu HIS_PATIENT, khong cho phep xoa" + LogUtil.TraceData("data", data));
                }
            }
            catch (Exception ex)
            {
