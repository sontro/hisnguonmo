...
{
    partial class HisKskPeriodDriverLock : BusinessBase
    {
        internal HisKskPeriodDriverLock()
            : base()
        {

        }

        internal HisKskPeriodDriverLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_KSK_PERIOD_DRIVER resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_KSK_PERIOD_DRIVER data = new HisKskPeriodDriverGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisKskPeriodDriverDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_KSK_PERIOD_DRIVER resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_KSK_PERIOD_DRIVER resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisKskPeriodDriverCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisKskPeriodDriverDAO.Update(raw);
