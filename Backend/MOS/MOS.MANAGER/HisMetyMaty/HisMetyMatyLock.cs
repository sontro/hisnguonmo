...
{
    partial class HisMetyMatyLock : BusinessBase
    {
        internal HisMetyMatyLock()
            : base()
        {

        }

        internal HisMetyMatyLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_METY_MATY resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_METY_MATY data = new HisMetyMatyGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisMetyMatyDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_METY_MATY resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_METY_MATY resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisMetyMatyCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisMetyMatyDAO.Update(raw);
