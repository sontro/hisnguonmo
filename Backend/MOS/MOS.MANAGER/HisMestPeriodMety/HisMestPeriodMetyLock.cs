...
{
    class HisMestPeriodMetyLock : BusinessBase
    {
        internal HisMestPeriodMetyLock()
            : base()
        {

        }

        internal HisMestPeriodMetyLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_MEST_PERIOD_METY data)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    data = new HisMestPeriodMetyGet().GetById(data.ID);
                    if (data != null)
                    {
