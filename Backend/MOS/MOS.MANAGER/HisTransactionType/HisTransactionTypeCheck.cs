...
{
    class HisTransactionTypeCheck : BusinessBase
    {
        internal HisTransactionTypeCheck()
            : base()
        {

        }

        internal HisTransactionTypeCheck(Inventec.Core.CommonParam paramCheck)
            : base(paramCheck) 
        {

        }

        internal bool VerifyRequireField(HIS_TRANSACTION_TYPE data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool ExistsCode(string code, long? id)
        {
            ...
            try
            {
                if (DAOWorker.HisTransactionTypeDAO.ExistsCode(code, id))
                {
        ...
        internal bool IsUnLock(HIS_TRANSACTION_TYPE data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisTransactionTypeDAO.IsUnLock(id))
                {
                    valid = false;
        ...
        internal bool CheckConstraint(long id)
        {
            ...
            try
            {
                List<HIS_TRANSACTION> hisTransactions = new HisTransactionGet().GetByTransactionTypeId(id);
