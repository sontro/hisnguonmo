...
{
    partial class HisBltyServiceLock : BusinessBase
    {
        internal HisBltyServiceLock()
            : base()
        {

        }

        internal HisBltyServiceLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_BLTY_SERVICE resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_BLTY_SERVICE data = new HisBltyServiceGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisBltyServiceDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_BLTY_SERVICE resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_BLTY_SERVICE resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisBltyServiceCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisBltyServiceDAO.Update(raw);
