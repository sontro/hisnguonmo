...
{
    partial class HisServiceReqMatyCheck : BusinessBase
    {
        internal HisServiceReqMatyCheck()
            : base()
        {

        }

        internal HisServiceReqMatyCheck(CommonParam paramCheck)
            : base(paramCheck) 
        {

        }

        internal bool VerifyRequireField(HIS_SERVICE_REQ_MATY data)
        {
            bool valid = true;
            try
            {
                ...
                if (data.SERVICE_REQ_ID <= 0) throw new ArgumentNullException("data.SERVICE_REQ_ID");
            }
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisServiceReqMatyGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_SERVICE_REQ_MATY data)
        {
            bool valid = true;
            try
            {
                data = new HisServiceReqMatyGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                if (listId != null && listId.Count > 0)
                {
                    ...
                    filter.IDs = listId;
                    List<HIS_SERVICE_REQ_MATY> listData = new HisServiceReqMatyGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_SERVICE_REQ_MATY> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_SERVICE_REQ_MATY data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisServiceReqMatyDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_SERVICE_REQ_MATY> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool CheckConstraint(long id)
        {
            ...
            try
            {
                /*List<XXX> hisXXXs = new HisXXXGet().GetByHisServiceReqMatyId(id);
        ...
        internal bool IsNotExisted(HIS_SERVICE_REQ_MATY data)
        {
            bool valid = true;
            try
            {
                HisServiceReqMatyFilterQuery filter = new HisServiceReqMatyFilterQuery();
                filter.SERVICE_REQ_ID = data.SERVICE_REQ_ID;
                filter.MATERIAL_TYPE_ID = data.MATERIAL_TYPE_ID;
                List<HIS_SERVICE_REQ_MATY> serviceReqMaties = new HisServiceReqMatyGet().Get(filter);
                List<HIS_SERVICE_REQ_MATY> otherExists = IsNotNullOrEmpty(serviceReqMaties) ? serviceReqMaties.Where(o => o.ID != data.ID).ToList() : null;
                if (IsNotNullOrEmpty(otherExists))
                {
                    MOS.MANAGER.Base.MessageUtil.SetMessage(param, MOS.LibraryMessage.Message.Enum.DuLieuDaTonTaiTrenHeThong);
                    throw new Exception("Da ton tai du lieu" + LogUtil.TraceData("data", data));
                }
            }
            catch (Exception ex)
            {
