...
{
    partial class HisServiceReqMatyLock : BusinessBase
    {
        internal HisServiceReqMatyLock()
            : base()
        {

        }

        internal HisServiceReqMatyLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_SERVICE_REQ_MATY resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_SERVICE_REQ_MATY data = new HisServiceReqMatyGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisServiceReqMatyDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_SERVICE_REQ_MATY resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_SERVICE_REQ_MATY resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisServiceReqMatyCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisServiceReqMatyDAO.Update(raw);
