...
{
    class HisServiceRoomLock : BusinessBase
    {
        internal HisServiceRoomLock()
            : base()
        {

        }

        internal HisServiceRoomLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_SERVICE_ROOM data)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    data = new HisServiceRoomGet().GetById(data.ID);
                    if (data != null)
                    {
