...
{
    class HisServiceRoomCheck : BusinessBase
    {
        internal HisServiceRoomCheck()
            : base()
        {

        }

        internal HisServiceRoomCheck(CommonParam paramCheck)
            : base(paramCheck) 
        {

        }
        
        internal bool VerifyRequireField(HIS_SERVICE_ROOM data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(HIS_SERVICE_ROOM data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisServiceRoomDAO.IsUnLock(id))
                {
                    valid = false;
                    MOS.MANAGER.Base.BugUtil.SetBugCode(param, MOS.LibraryBug.Bug.Enum.DuLieuDangBiKhoa);
                }
        ...
        internal bool VerifyIds(List<long> listId, List<HIS_SERVICE_ROOM> listObject)
        {
            bool valid = true;
            try
            {
                if (listId != null && listId.Count > 0)
                {
                    ...
                    filter.IDs = listId;
                    List<HIS_SERVICE_ROOM> listData = new HisServiceRoomGet().Get(filter);
        ...
        internal bool IsUnLock(List<HIS_SERVICE_ROOM> listData)
        {
            bool valid = true;
            try
            {
