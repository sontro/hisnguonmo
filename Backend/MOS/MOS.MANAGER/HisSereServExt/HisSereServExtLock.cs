...
{
    partial class HisSereServExtLock : BusinessBase
    {
        internal HisSereServExtLock()
            : base()
        {

        }

        internal HisSereServExtLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_SERE_SERV_EXT resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_SERE_SERV_EXT data = new HisSereServExtGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisSereServExtDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_SERE_SERV_EXT resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_SERE_SERV_EXT resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisSereServExtCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisSereServExtDAO.Update(raw);
