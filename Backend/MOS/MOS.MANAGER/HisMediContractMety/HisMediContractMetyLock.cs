...
{
    partial class HisMediContractMetyLock : BusinessBase
    {
        internal HisMediContractMetyLock()
            : base()
        {

        }

        internal HisMediContractMetyLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_MEDI_CONTRACT_METY resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_MEDI_CONTRACT_METY data = new HisMediContractMetyGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisMediContractMetyDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_MEDI_CONTRACT_METY resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_MEDI_CONTRACT_METY resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisMediContractMetyCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisMediContractMetyDAO.Update(raw);
