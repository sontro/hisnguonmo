...
{
    partial class HisExpMestReasonLock : BusinessBase
    {
        internal HisExpMestReasonLock()
            : base()
        {

        }

        internal HisExpMestReasonLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_EXP_MEST_REASON resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_EXP_MEST_REASON data = new HisExpMestReasonGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisExpMestReasonDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_EXP_MEST_REASON resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_EXP_MEST_REASON resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisExpMestReasonCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisExpMestReasonDAO.Update(raw);
