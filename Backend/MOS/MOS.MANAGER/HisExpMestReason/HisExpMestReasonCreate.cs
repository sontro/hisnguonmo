...
namespace MOS.MANAGER.HisExpMestReason
{
    partial class HisExpMestReasonCreate : BusinessBase
    {
		...
		internal bool CreateList(List<HIS_EXP_MEST_REASON> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    if (!DAOWorker.HisExpMestReasonDAO.CreateList(listData))
                    {
                        MOS.MANAGER.Base.BugUtil.SetBugCode(param, MOS.LibraryBug.Bug.Enum.HisExpMestReason_ThemMoiThatBai);
                        throw new Exception("Them moi thong tin HisExpMestReason that bai." + LogUtil.TraceData("listData", listData));
                    }
                    this.recentHisExpMestReasons.AddRange(listData);
                    result = true;
                }
            }
            catch (Exception ex)
