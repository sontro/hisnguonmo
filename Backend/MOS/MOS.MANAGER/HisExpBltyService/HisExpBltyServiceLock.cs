...
{
    partial class HisExpBltyServiceLock : BusinessBase
    {
        internal HisExpBltyServiceLock()
            : base()
        {

        }

        internal HisExpBltyServiceLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_EXP_BLTY_SERVICE resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_EXP_BLTY_SERVICE data = new HisExpBltyServiceGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisExpBltyServiceDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_EXP_BLTY_SERVICE resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_EXP_BLTY_SERVICE resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisExpBltyServiceCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisExpBltyServiceDAO.Update(raw);
