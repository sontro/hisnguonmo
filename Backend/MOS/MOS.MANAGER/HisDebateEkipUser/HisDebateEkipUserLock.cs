...
{
    partial class HisDebateEkipUserLock : BusinessBase
    {
        internal HisDebateEkipUserLock()
            : base()
        {

        }

        internal HisDebateEkipUserLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_DEBATE_EKIP_USER resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_DEBATE_EKIP_USER data = new HisDebateEkipUserGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisDebateEkipUserDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_DEBATE_EKIP_USER resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_DEBATE_EKIP_USER resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisDebateEkipUserCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisDebateEkipUserDAO.Update(raw);
