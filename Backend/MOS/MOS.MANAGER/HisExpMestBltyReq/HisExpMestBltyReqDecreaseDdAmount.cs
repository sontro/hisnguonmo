...
{
    partial class HisExpMestBltyReqDecreaseDdAmount : BusinessBase
    {
        private Dictionary<long, decimal> recentDecreaseDic;

        internal HisExpMestBltyReqDecreaseDdAmount()
            : base()
        {

        }

        internal HisExpMestBltyReqDecreaseDdAmount(CommonParam paramUpdate)
            : base(paramUpdate)
        {
        ...
        internal bool Run(Dictionary<long, decimal> decreaseDic)
        {
            ...
            try
            {
                if (decreaseDic != null && decreaseDic.Count > 0)
                {
                    ...
                    {
                        string sql = string.Format("UPDATE HIS_EXP_MEST_BLTY_REQ SET DD_AMOUNT = NVL(DD_AMOUNT, 0) - {0} WHERE ID = {1}",  CommonUtil.ToString(decreaseDic[k]), k);
                        sqls.Add(sql);
                    }
        ...
        internal void Rollback()
        {
            try
            {
                if (this.recentDecreaseDic != null && this.recentDecreaseDic.Count > 0)
                {
                    ...
                    {
                        sqlBuilder.Append(string.Format("UPDATE HIS_EXP_MEST_BLTY_REQ SET DD_AMOUNT = NVL(DD_AMOUNT, 0) + {0} WHERE ID = {1};", CommonUtil.ToString(this.recentDecreaseDic[k]), k));
                    }
