...
{
    partial class HisRefectoryCheck : BusinessBase
    {
        internal HisRefectoryCheck()
            : base()
        {

        }

        internal HisRefectoryCheck(CommonParam paramCheck)
            : base(paramCheck)
        {

        }

        internal bool VerifyRequireField(HIS_REFECTORY data)
        {
            bool valid = true;
            try
            {
                ...
                if (String.IsNullOrWhiteSpace(data.REFECTORY_CODE)) throw new ArgumentNullException("data.REFECTORY_CODE");
                if (String.IsNullOrWhiteSpace(data.REFECTORY_NAME)) throw new ArgumentNullException("data.REFECTORY_NAME");
            }
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisRefectoryGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_REFECTORY data)
        {
            bool valid = true;
            try
            {
                data = new HisRefectoryGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                ...
                {
                    HisRefectoryFilterQuery filter = new HisRefectoryFilterQuery();
                    filter.IDs = listId;
                    List<HIS_REFECTORY> listData = new HisRefectoryGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_REFECTORY> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_REFECTORY data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisRefectoryDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_REFECTORY> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool CheckConstraint(long id)
        {
            ...
            try
            {
                /*List<XXX> hisXXXs = new HisXXXGet().GetByHisRefectoryId(id);
