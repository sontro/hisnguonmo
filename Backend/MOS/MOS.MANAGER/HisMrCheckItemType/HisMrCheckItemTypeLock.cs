...
{
    partial class HisMrCheckItemTypeLock : BusinessBase
    {
        internal HisMrCheckItemTypeLock()
            : base()
        {

        }

        internal HisMrCheckItemTypeLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_MR_CHECK_ITEM_TYPE resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_MR_CHECK_ITEM_TYPE data = new HisMrCheckItemTypeGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisMrCheckItemTypeDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_MR_CHECK_ITEM_TYPE resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_MR_CHECK_ITEM_TYPE resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisMrCheckItemTypeCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisMrCheckItemTypeDAO.Update(raw);
