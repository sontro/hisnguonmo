...
{
    partial class HisQcTypeLock : BusinessBase
    {
        internal HisQcTypeLock()
            : base()
        {

        }

        internal HisQcTypeLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_QC_TYPE resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_QC_TYPE data = new HisQcTypeGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisQcTypeDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_QC_TYPE resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_QC_TYPE resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisQcTypeCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisQcTypeDAO.Update(raw);
