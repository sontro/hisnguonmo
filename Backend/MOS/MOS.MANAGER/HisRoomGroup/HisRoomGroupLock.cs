...
{
    partial class HisRoomGroupLock : BusinessBase
    {
        internal HisRoomGroupLock()
            : base()
        {

        }

        internal HisRoomGroupLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_ROOM_GROUP resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_ROOM_GROUP data = new HisRoomGroupGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisRoomGroupDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_ROOM_GROUP resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_ROOM_GROUP resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisRoomGroupCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisRoomGroupDAO.Update(raw);
