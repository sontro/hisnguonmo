...
namespace MOS.MANAGER.HisRoomGroup
{
    partial class HisRoomGroupCreate : BusinessBase
    {
		...
		internal bool CreateList(List<HIS_ROOM_GROUP> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    if (!DAOWorker.HisRoomGroupDAO.CreateList(listData))
                    {
                        MOS.MANAGER.Base.BugUtil.SetBugCode(param, MOS.LibraryBug.Bug.Enum.HisRoomGroup_ThemMoiThatBai);
                        throw new Exception("Them moi thong tin HisRoomGroup that bai." + LogUtil.TraceData("listData", listData));
                    }
                    this.recentHisRoomGroups.AddRange(listData);
                    result = true;
                }
            }
            catch (Exception ex)
