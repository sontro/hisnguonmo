...
{
    partial class HisInfusionSumLock : BusinessBase
    {
        internal HisInfusionSumLock()
            : base()
        {

        }

        internal HisInfusionSumLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_INFUSION_SUM resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_INFUSION_SUM data = new HisInfusionSumGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisInfusionSumDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_INFUSION_SUM resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_INFUSION_SUM resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisInfusionSumCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisInfusionSumDAO.Update(raw);
