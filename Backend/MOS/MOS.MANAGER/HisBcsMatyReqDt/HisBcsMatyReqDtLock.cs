...
{
    partial class HisBcsMatyReqDtLock : BusinessBase
    {
        internal HisBcsMatyReqDtLock()
            : base()
        {

        }

        internal HisBcsMatyReqDtLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_BCS_MATY_REQ_DT resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_BCS_MATY_REQ_DT data = new HisBcsMatyReqDtGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisBcsMatyReqDtDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_BCS_MATY_REQ_DT resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_BCS_MATY_REQ_DT resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisBcsMatyReqDtCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisBcsMatyReqDtDAO.Update(raw);
