...
namespace MOS.MANAGER.HisImpMest.Manu.Update
{
    class MaterialProcessor : BusinessBase
    {
        ...
        private void ProcessMaterial(HIS_IMP_MEST impMest, List<HisMaterialWithPatySDO> manuMaterials, List<HIS_MATERIAL> hisMaterials, List<HIS_MATERIAL_TYPE> hisMaterialTypes, ref List<string> sqls)
        {
            ...
            List<HIS_MEDI_CONTRACT_MATY> mediContractMatys = this.GetMediContractMaty(manuMaterials);
            ...
            foreach (var imp in manuMaterials)
            {
                ...
                {
                    HIS_MEDI_CONTRACT_MATY contractMaty = mediContractMatys.FirstOrDefault(o => o.MEDICAL_CONTRACT_ID == material.MEDICAL_CONTRACT_ID.Value && o.MATERIAL_TYPE_ID == material.MATERIAL_TYPE_ID && o.CONTRACT_PRICE == material.CONTRACT_PRICE);
                        ...
                        && (!contractMaty.IMP_VAT_RATIO.HasValue || contractMaty.IMP_VAT_RATIO.Value == 0))
                    {
                        ...
                        contractMaty.IMP_VAT_RATIO = material.IMP_VAT_RATIO;
                        if (contractMaty.CONTRACT_PRICE.HasValue)
                            contractMaty.IMP_PRICE = contractMaty.CONTRACT_PRICE.Value / (decimal)(1 + contractMaty.IMP_VAT_RATIO.Value);
                        updates.Add(contractMaty);
                        befores.Add(before);
                    }
                }
                ...
                foreach (HisMaterialWithPatySDO sdo in this.outputs)
                {
                    ...
                    if (IsNotNullOrEmpty(oldMestMaterials) && oldMestMaterials.Count == 1)
                    {
                        ...
                        if (impMestMaterial.AMOUNT != sdo.Material.AMOUNT
                            ...
                            || impMestMaterial.CONTRACT_PRICE != sdo.Material.CONTRACT_PRICE)
                        {
                            ...
                            impMestMaterial.DOCUMENT_PRICE = sdo.Material.DOCUMENT_PRICE;
                            impMestMaterial.IMP_UNIT_AMOUNT = sdo.Material.IMP_UNIT_AMOUNT;
                            impMestMaterial.IMP_UNIT_PRICE = sdo.Material.IMP_UNIT_PRICE;
                            impMestMaterial.TDL_IMP_UNIT_CONVERT_RATIO = sdo.Material.TDL_IMP_UNIT_CONVERT_RATIO;
                            impMestMaterial.TDL_IMP_UNIT_ID = sdo.Material.TDL_IMP_UNIT_ID;
        ...
        private List<HIS_MEDI_CONTRACT_MATY> GetMediContractMaty(List<HisMaterialWithPatySDO> manuMaterials)
        {
            ...
            return new HisMediContractMatyGet().GetByMedicalContractIds(medicalContractIds);
        }
