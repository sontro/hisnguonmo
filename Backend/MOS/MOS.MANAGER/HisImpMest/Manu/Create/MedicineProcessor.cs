...
namespace MOS.MANAGER.HisImpMest.Manu.Create
{
    class MedicineProcessor : BusinessBase
    {
        ...
        internal bool Run(List<HisMedicineWithPatySDO> ManuMedicines, HIS_IMP_MEST impMest, ref  List<HisMedicineWithPatySDO> impMedicineSDOs)
        {
            bool result = false;
            try
            {
                if (IsNotNullOrEmpty(ManuMedicines))
                {
                    ...
                    bool valid = true;
                    valid = valid && checker.IsValidImpMedicines(ManuMedicines);
                    //valid = valid && checker.IsValidMedicineExpiredDates(ManuMedicines);
                    valid = valid && checker.IsNotExistStopImpMedicineType(ManuMedicines);
                    ...
                    List<V_HIS_MEDI_CONTRACT_METY_1> mediContractMetys = this.GetMediContractMety(ManuMedicines);
                    ...
                    Mapper.CreateMap<V_HIS_MEDI_CONTRACT_METY_1, HIS_MEDI_CONTRACT_METY>();

                    foreach (HisMedicineWithPatySDO impMedicine in ManuMedicines)
                    {
                        ...
                        medicine.ACTIVE_INGR_BHYT_CODE = !string.IsNullOrWhiteSpace(impMedicine.Medicine.ACTIVE_INGR_BHYT_CODE) ? impMedicine.Medicine.ACTIVE_INGR_BHYT_CODE : medicineType.ACTIVE_INGR_BHYT_CODE;
                        medicine.ACTIVE_INGR_BHYT_NAME = !string.IsNullOrWhiteSpace(impMedicine.Medicine.ACTIVE_INGR_BHYT_NAME) ? impMedicine.Medicine.ACTIVE_INGR_BHYT_NAME : medicineType.ACTIVE_INGR_BHYT_NAME;
                        ...
                        {
                            V_HIS_MEDI_CONTRACT_METY_1 contractMety = mediContractMetys.FirstOrDefault(o => o.MEDICAL_CONTRACT_ID == medicine.MEDICAL_CONTRACT_ID.Value && o.MEDICINE_TYPE_ID == medicine.MEDICINE_TYPE_ID && (o.CONTRACT_PRICE ?? 0) == medicine.CONTRACT_PRICE && (o.BID_GROUP_CODE ?? "") == (medicine.TDL_BID_GROUP_CODE ?? ""));
                                ...
                                && (!contractMety.IMP_VAT_RATIO.HasValue || contractMety.IMP_VAT_RATIO.Value == 0))
                            {
                                ...
                                HIS_MEDI_CONTRACT_METY up = Mapper.Map<HIS_MEDI_CONTRACT_METY>(contractMety);
                                up.IMP_VAT_RATIO = medicine.IMP_VAT_RATIO;
                        ...
                        impMestMedicine.DOCUMENT_PRICE = medicine.DOCUMENT_PRICE;
                        ...
                        impMestMedicine.CONTRACT_PRICE = medicine.CONTRACT_PRICE;
                        impMestMedicine.TEMPERATURE = impMedicine.Temperature;
        ...
        private List<V_HIS_MEDI_CONTRACT_METY_1> GetMediContractMety(List<HisMedicineWithPatySDO> manuMedicines)
        {
            ...
            return new HisMediContractMetyGet().GetView1ByMedicalContractIds(medicalContractIds);
        }
