...
namespace MOS.MANAGER.HisImpMest.Other.Donation.Update
{
    class BloodProcessor : BusinessBase
    {
        ...
        internal bool Run(HIS_IMP_MEST impMest, List<HIS_BLOOD> donationBloods, ref List<HIS_BLOOD> outBloods, ref List<string> sqls)
        {
            bool result = false;
            try
            {
                ...
                this.ProcessBlood(impMest, donationBloods);
        ...
        private void ProcessBlood(HIS_IMP_MEST impMest, List<HIS_BLOOD> donationBloods)
        {
            if (IsNotNullOrEmpty(donationBloods))
            {
                HisImpMestCheck checker = new HisImpMestCheck(param);
                bool valid = true;
                valid = valid && !donationBloods.Exists(o => !o.BLOOD_GIVE_ID.HasValue);
                valid = valid && checker.IsValidBloodExpiredDates(donationBloods);
                if (!valid)
                {
                ...
                List<long> bloodTypeIds = donationBloods.Select(o => o.BLOOD_TYPE_ID).ToList();
                ...
                foreach (HIS_BLOOD blood in donationBloods)
                {
