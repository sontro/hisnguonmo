...
namespace MOS.MANAGER.HisImpMest.Other.Other.Create
{
    class MedicineProcessor : BusinessBase
    {
        ...
        internal bool Run(List<HisMedicineWithPatySDO> otherMedicines, HIS_IMP_MEST impMest, ref  List<HisMedicineWithPatySDO> impMedicineSDOs)
        {
            bool result = false;
            try
            {
                if (IsNotNullOrEmpty(otherMedicines))
                {
                    HisImpMestCheck checker = new HisImpMestCheck(param);
                    bool valid = true;
                    valid = valid && checker.IsValidImpMedicines(otherMedicines);
                    //valid = valid && checker.IsValidMedicineExpiredDates(otherMedicines);
                    valid = valid && checker.IsNotExistStopImpMedicineType(otherMedicines);
                    ...
                    List<long> medicineTypeIds = otherMedicines.Select(o => o.Medicine.MEDICINE_TYPE_ID).ToList();
                    ...
                    foreach (HisMedicineWithPatySDO impMedicine in otherMedicines)
                    {
