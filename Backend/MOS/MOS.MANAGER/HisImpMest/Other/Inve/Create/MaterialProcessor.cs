...
namespace MOS.MANAGER.HisImpMest.Other.Inve.Create
{
    class MaterialProcessor : BusinessBase
    {
        ...
        internal bool Run(List<HisMaterialWithPatySDO> inveMaterials, HIS_IMP_MEST impMest, List<HIS_MATERIAL_TYPE> hisMaterialTypes, ref  List<HisMaterialWithPatySDO> impMaterialSDOs)
        {
            bool result = false;
            try
            {
                if (IsNotNullOrEmpty(inveMaterials))
                {
                    ...
                    bool valid = true;
                    valid = valid && checker.IsValidImpMaterials(inveMaterials);
