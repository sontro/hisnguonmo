...
namespace MOS.MANAGER.HisImpMest.Other.Inve.Update
{
    class BloodProcessor : BusinessBase
    {
        ...
        internal bool Run(HIS_IMP_MEST impMest, List<HIS_BLOOD> inveBloods, ref List<HIS_BLOOD> outBloods, ref List<string> sqls)
        {
            bool result = false;
            try
            {
                ...
                this.ProcessBlood(impMest, inveBloods);
        ...
        private void ProcessBlood(HIS_IMP_MEST impMest, List<HIS_BLOOD> inveBloods)
        {
            if (IsNotNullOrEmpty(inveBloods))
            {
                HisImpMestCheck checker = new HisImpMestCheck(param);
                bool valid = true;
                valid = valid && checker.IsValidBloodExpiredDates(inveBloods);
                if (!valid)
                {
                ...
                List<long> bloodTypeIds = inveBloods.Select(o => o.BLOOD_TYPE_ID).ToList();
                ...
                foreach (HIS_BLOOD blood in inveBloods)
                {
