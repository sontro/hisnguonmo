...
namespace MOS.MANAGER.HisImpMest.Other.Init.Create
{
    class MedicineProcessor : BusinessBase
    {
        ...
        internal bool Run(List<HisMedicineWithPatySDO> initMedicines, HIS_IMP_MEST impMest, ref  List<HisMedicineWithPatySDO> impMedicineSDOs)
        {
            bool result = false;
            try
            {
                if (IsNotNullOrEmpty(initMedicines))
                {
                    HisImpMestCheck checker = new HisImpMestCheck(param);
                    bool valid = true;
                    valid = valid && checker.IsValidImpMedicines(initMedicines);
                    //valid = valid && checker.IsValidMedicineExpiredDates(initMedicines);
                    valid = valid && checker.IsNotExistStopImpMedicineType(initMedicines);
                    ...
                    List<long> medicineTypeIds = initMedicines.Select(o => o.Medicine.MEDICINE_TYPE_ID).ToList();
                    ...
                    foreach (HisMedicineWithPatySDO impMedicine in initMedicines)
                    {
