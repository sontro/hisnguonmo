...
namespace MOS.MANAGER.HisImpMest.Other.Init.Create
{
    class MaterialProcessor : BusinessBase
    {
        ...
        internal bool Run(List<HisMaterialWithPatySDO> initMaterials, HIS_IMP_MEST impMest, List<HIS_MATERIAL_TYPE> hisMaterialTypes, ref  List<HisMaterialWithPatySDO> impMaterialSDOs)
        {
            bool result = false;
            try
            {
                if (IsNotNullOrEmpty(initMaterials))
                {
                    HisImpMestCheck checker = new HisImpMestCheck(param);
                    bool valid = true;
                    valid = valid && checker.IsValidImpMaterials(initMaterials);
                    valid = valid && checker.IsNotExistStopImpMaterialType(initMaterials);
                    if (!valid)
                    {
                        return false;
                    }
                    ...
                    foreach (HisMaterialWithPatySDO impMaterial in initMaterials)
                    {
