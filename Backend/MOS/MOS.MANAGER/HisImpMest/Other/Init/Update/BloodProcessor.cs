...
namespace MOS.MANAGER.HisImpMest.Other.Init.Update
{
    class BloodProcessor : BusinessBase
    {
        ...
        internal bool Run(HIS_IMP_MEST impMest, List<HIS_BLOOD> initBloods, ref List<HIS_BLOOD> outBloods, ref List<string> sqls)
        {
            bool result = false;
            try
            {
                ...
                this.ProcessBlood(impMest, initBloods);
        ...
        private void ProcessBlood(HIS_IMP_MEST impMest, List<HIS_BLOOD> initBloods)
        {
            if (IsNotNullOrEmpty(initBloods))
            {
                HisImpMestCheck checker = new HisImpMestCheck(param);
                bool valid = true;
                valid = valid && checker.IsValidBloodExpiredDates(initBloods);
                if (!valid)
                {
                ...
                List<long> bloodTypeIds = initBloods.Select(o => o.BLOOD_TYPE_ID).ToList();
                ...
                foreach (HIS_BLOOD blood in initBloods)
                {
