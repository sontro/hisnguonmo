...
{
    class HisImpMestMobaPresCabinetCheck : BusinessBase
    {
        ...
        internal HisImpMestMobaPresCabinetCheck()
            : base()
        {

        }

        internal HisImpMestMobaPresCabinetCheck(CommonParam paramCheck)
            : base(paramCheck)
        {

        }

        internal bool VerifyRequireField(HisImpMestMobaPresCabinetSDO data)
        {
            bool valid = true;
            try
            {
                ...
                if (!IsNotNullOrEmpty(data.MobaPresMaterials) && !IsNotNullOrEmpty(data.MobaPresMedicines)) throw new ArgumentNullException("data.MobaPresMaterials && data.MobaPresMedicines");
            }
        ...
        internal bool VerifyExpMestId(long expMestId, ref HIS_EXP_MEST expMest)
        {
            ...
            try
            {
                ...
                {
                    MANAGER.Base.MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.HisExpMest_PhieuChuaKhongPhaiDonTuTruc);
                    return false;
                }
        ...
        internal bool VerifyAllowMoba(HisImpMestMobaPresCabinetSDO data, HIS_EXP_MEST expMest)
        {
            bool valid = true;
            try
            {
                ...
                if (HisImpMestCFG.AUTO_SET_IMP_STOCK_MOBA_PRES_CABINET) return true;

                long mediStockId = data.ImpMediStockId.HasValue ? data.ImpMediStockId.Value : expMest.MEDI_STOCK_ID;
                HIS_MEDI_STOCK mediStock = new HisMediStockGet().GetById(mediStockId);
                if (mediStock == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    throw new Exception("MEDI_STOCK_ID invalid: " + mediStockId);
                }
                ...
                {
                    if (expMest.MEDI_STOCK_ID != mediStockId)
                    {
        ...
        internal bool VerifyWorkingRoom(HIS_EXP_MEST expMest, long workingRoomId, ref WorkPlaceSDO workPlace)
        {
            bool valid = true;
            try
            {
                workPlace = TokenManager.GetWorkPlace(workingRoomId);
                if (workPlace == null)
                {
        ...
        internal bool ValidateDataPres(HisImpMestMobaPresCabinetSDO data, HIS_EXP_MEST expMest, ref List<HIS_EXP_MEST_MEDICINE> medicines, ref List<HIS_EXP_MEST_MATERIAL> materials)
        {
            bool valid = true;
            try
            {
