...
namespace MOS.MANAGER.HisImpMest.Moba.PresCabinet
{
    class MedicineProcessor : BusinessBase
    {
        ...
        internal bool Run(List<HIS_IMP_MEST> impMests, Dictionary<long, MobaData> dicImpMest, List<HIS_EXP_MEST_MEDICINE> expMedicines, List<HIS_SERE_SERV> hisSereServs, ref List<HIS_IMP_MEST_MEDICINE> impMedicines, ref List<HIS_SERE_SERV> sereServUpdates)
        {
            bool result = false;
            try
            {
                ...
                {
                    hisMedicines = new HisMedicineGet().GetByIds(expMedicines.Select(s => s.MEDICINE_ID.Value).Distinct().ToList());
                }

                foreach (HIS_IMP_MEST imp in impMests)
                {
                    MobaData d = dicImpMest[imp.MEDI_STOCK_ID];
                    if (IsNotNullOrEmpty(d.MobaMedicines))
                    {
                        foreach (var mobaItem in d.MobaMedicines)
                        {
                            HIS_EXP_MEST_MEDICINE expMestMedicine = expMedicines.FirstOrDefault(o => o.ID == mobaItem.ExpMestMedicineId);
                            ...
                            impMestMedicine.IMP_MEST_ID = imp.ID;
