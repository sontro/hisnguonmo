...
{
    partial class HisDebateTypeLock : BusinessBase
    {
        internal HisDebateTypeLock()
            : base()
        {

        }

        internal HisDebateTypeLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_DEBATE_TYPE resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_DEBATE_TYPE data = new HisDebateTypeGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisDebateTypeDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_DEBATE_TYPE resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_DEBATE_TYPE resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisDebateTypeCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisDebateTypeDAO.Update(raw);
