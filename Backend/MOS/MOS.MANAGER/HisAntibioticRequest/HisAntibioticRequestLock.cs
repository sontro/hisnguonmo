...
{
    partial class HisAntibioticRequestLock : BusinessBase
    {
        internal HisAntibioticRequestLock()
            : base()
        {

        }

        internal HisAntibioticRequestLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_ANTIBIOTIC_REQUEST resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_ANTIBIOTIC_REQUEST data = new HisAntibioticRequestGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisAntibioticRequestDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_ANTIBIOTIC_REQUEST resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_ANTIBIOTIC_REQUEST resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisAntibioticRequestCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisAntibioticRequestDAO.Update(raw);
