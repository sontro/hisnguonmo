...
namespace MOS.MANAGER.HisAntibioticRequest
{
    partial class HisAntibioticRequestCreate : BusinessBase
    {
		...
		internal bool CreateList(List<HIS_ANTIBIOTIC_REQUEST> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    if (!DAOWorker.HisAntibioticRequestDAO.CreateList(listData))
                    {
                        MOS.MANAGER.Base.BugUtil.SetBugCode(param, MOS.LibraryBug.Bug.Enum.HisAntibioticRequest_ThemMoiThatBai);
                        throw new Exception("Them moi thong tin HisAntibioticRequest that bai." + LogUtil.TraceData("listData", listData));
                    }
                    this.recentHisAntibioticRequests.AddRange(listData);
                    result = true;
                }
            }
            catch (Exception ex)
