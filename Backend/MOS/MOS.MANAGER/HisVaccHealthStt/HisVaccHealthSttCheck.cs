...
{
    partial class HisVaccHealthSttCheck : BusinessBase
    {
        internal HisVaccHealthSttCheck()
            : base()
        {

        }

        internal HisVaccHealthSttCheck(CommonParam paramCheck)
            : base(paramCheck)
        {

        }

        internal bool VerifyRequireField(HIS_VACC_HEALTH_STT data)
        {
            bool valid = true;
            try
            {
                if (data == null) throw new ArgumentNullException("data");
                if (String.IsNullOrWhiteSpace(data.VACC_HEALTH_STT_CODE)) throw new ArgumentNullException("data.VACC_HEALTH_STT_CODE");
                if (String.IsNullOrWhiteSpace(data.VACC_HEALTH_STT_NAME)) throw new ArgumentNullException("data.VACC_HEALTH_STT_NAME");
            }
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisVaccHealthSttGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_VACC_HEALTH_STT data)
        {
            bool valid = true;
            try
            {
                data = new HisVaccHealthSttGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                ...
                {
                    HisVaccHealthSttFilterQuery filter = new HisVaccHealthSttFilterQuery();
                    filter.IDs = listId;
                    List<HIS_VACC_HEALTH_STT> listData = new HisVaccHealthSttGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_VACC_HEALTH_STT> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_VACC_HEALTH_STT data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisVaccHealthSttDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_VACC_HEALTH_STT> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool CheckConstraint(long id)
        {
            ...
            try
            {
                /*List<XXX> hisXXXs = new HisXXXGet().GetByHisVaccHealthSttId(id);
