...
{
    partial class HisVaccHealthSttLock : BusinessBase
    {
        internal HisVaccHealthSttLock()
            : base()
        {

        }

        internal HisVaccHealthSttLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_VACC_HEALTH_STT resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_VACC_HEALTH_STT data = new HisVaccHealthSttGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisVaccHealthSttDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_VACC_HEALTH_STT resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_VACC_HEALTH_STT resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisVaccHealthSttCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisVaccHealthSttDAO.Update(raw);
