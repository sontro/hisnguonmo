...
namespace MOS.MANAGER.HisCareType
{
    ...
    /// </summary>
    partial class HisCareTypeCheck : BusinessBase
    {
        internal HisCareTypeCheck()
            : base()
        {

        }

        internal HisCareTypeCheck(CommonParam paramCheck)
            : base(paramCheck) 
        {

        }

        internal bool VerifyRequireField(HIS_CARE_TYPE data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisCareTypeGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_CARE_TYPE data)
        {
            bool valid = true;
            try
            {
                data = new HisCareTypeGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                ...
                {
                    HisCareTypeFilterQuery filter = new HisCareTypeFilterQuery();
                    filter.IDs = listId;
                    List<HIS_CARE_TYPE> listData = new HisCareTypeGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_CARE_TYPE> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_CARE_TYPE data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisCareTypeDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_CARE_TYPE> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool ExistsCode(string code, long? id)
        {
            ...
            try
            {
                if (DAOWorker.HisCareTypeDAO.ExistsCode(code, id))
                {
        ...
        internal bool CheckConstraint(HIS_CARE_TYPE data)
        {
            bool valid = true;
            try
            {
                List<HIS_CARE_DETAIL> hisCareDetails = new HisCareDetailGet().GetByCareTypeId(data.ID);
                if (IsNotNullOrEmpty(hisCareDetails))
                {
                    MOS.MANAGER.Base.MessageUtil.SetMessage(param, MOS.LibraryMessage.Message.Enum.HisCareDetail_TonTaiDuLieu);
                    throw new Exception("Ton tai du lieu HIS_CARE_DETAIL, khong cho phep xoa" + LogUtil.TraceData("data", data));
                }
            }
            catch (Exception ex)
            {
