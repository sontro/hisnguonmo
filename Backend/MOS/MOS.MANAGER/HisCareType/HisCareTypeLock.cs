...
{
    partial class HisCareTypeLock : BusinessBase
    {
        internal HisCareTypeLock()
            : base()
        {

        }

        internal HisCareTypeLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_CARE_TYPE data)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisCareTypeCheck().VerifyId(data.ID, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisCareTypeDAO.Update(raw);
