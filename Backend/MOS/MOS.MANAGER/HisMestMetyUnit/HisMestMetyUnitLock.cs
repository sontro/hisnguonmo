...
{
    partial class HisMestMetyUnitLock : BusinessBase
    {
        internal HisMestMetyUnitLock()
            : base()
        {

        }

        internal HisMestMetyUnitLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_MEST_METY_UNIT resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_MEST_METY_UNIT data = new HisMestMetyUnitGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisMestMetyUnitDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_MEST_METY_UNIT resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_MEST_METY_UNIT resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisMestMetyUnitCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisMestMetyUnitDAO.Update(raw);
