...
{
    partial class HisAnticipateLock : BusinessBase
    {
        internal HisAnticipateLock()
            : base()
        {

        }

        internal HisAnticipateLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_ANTICIPATE data)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisAnticipateCheck().VerifyId(data.ID, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisAnticipateDAO.Update(raw);
