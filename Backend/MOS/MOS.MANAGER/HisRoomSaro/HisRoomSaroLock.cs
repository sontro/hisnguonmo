...
{
    partial class HisRoomSaroLock : BusinessBase
    {
        internal HisRoomSaroLock()
            : base()
        {

        }

        internal HisRoomSaroLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_ROOM_SARO resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_ROOM_SARO data = new HisRoomSaroGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisRoomSaroDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_ROOM_SARO resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_ROOM_SARO resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisRoomSaroCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisRoomSaroDAO.Update(raw);
