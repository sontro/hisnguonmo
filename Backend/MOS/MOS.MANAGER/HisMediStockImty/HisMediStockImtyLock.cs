...
{
    partial class HisMediStockImtyLock : BusinessBase
    {
        internal HisMediStockImtyLock()
            : base()
        {

        }

        internal HisMediStockImtyLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_MEDI_STOCK_IMTY resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_MEDI_STOCK_IMTY data = new HisMediStockImtyGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisMediStockImtyDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_MEDI_STOCK_IMTY resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_MEDI_STOCK_IMTY resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisMediStockImtyCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisMediStockImtyDAO.Update(raw);
