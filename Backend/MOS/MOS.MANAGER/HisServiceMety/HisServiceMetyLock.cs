...
{
    partial class HisServiceMetyLock : BusinessBase
    {
        internal HisServiceMetyLock()
            : base()
        {

        }

        internal HisServiceMetyLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_SERVICE_METY data)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisServiceMetyCheck().VerifyId(data.ID, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisServiceMetyDAO.Update(raw);
