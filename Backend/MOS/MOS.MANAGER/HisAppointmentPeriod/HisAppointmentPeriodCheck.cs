...
{
    partial class HisAppointmentPeriodCheck : BusinessBase
    {
        internal HisAppointmentPeriodCheck()
            : base()
        {

        }

        internal HisAppointmentPeriodCheck(CommonParam paramCheck)
            : base(paramCheck) 
        {

        }

        internal bool VerifyRequireField(HIS_APPOINTMENT_PERIOD data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisAppointmentPeriodGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_APPOINTMENT_PERIOD data)
        {
            bool valid = true;
            try
            {
                data = new HisAppointmentPeriodGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                ...
                {
                    HisAppointmentPeriodFilterQuery filter = new HisAppointmentPeriodFilterQuery();
                    filter.IDs = listId;
                    List<HIS_APPOINTMENT_PERIOD> listData = new HisAppointmentPeriodGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_APPOINTMENT_PERIOD> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_APPOINTMENT_PERIOD data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisAppointmentPeriodDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_APPOINTMENT_PERIOD> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool CheckConstraint(long id)
        {
            ...
            try
            {
                List<HIS_TREATMENT> hisTreatments = new HisTreatmentGet().GetByAppointmentPeriodId(id);
