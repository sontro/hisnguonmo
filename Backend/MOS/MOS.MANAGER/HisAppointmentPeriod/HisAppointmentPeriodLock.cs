...
{
    partial class HisAppointmentPeriodLock : BusinessBase
    {
        internal HisAppointmentPeriodLock()
            : base()
        {

        }

        internal HisAppointmentPeriodLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_APPOINTMENT_PERIOD resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_APPOINTMENT_PERIOD data = new HisAppointmentPeriodGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisAppointmentPeriodDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_APPOINTMENT_PERIOD resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_APPOINTMENT_PERIOD resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisAppointmentPeriodCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisAppointmentPeriodDAO.Update(raw);
