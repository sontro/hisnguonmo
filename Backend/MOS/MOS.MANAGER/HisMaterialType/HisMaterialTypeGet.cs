...
namespace MOS.MANAGER.HisMaterialType
{
    partial class HisMaterialTypeGet : GetBase
    {
        ...
        internal List<V_HIS_MATERIAL_TYPE_1> GetView1(HisMaterialTypeView1FilterQuery filter)
        {
            try
            {
                return DAOWorker.HisMaterialTypeDAO.GetView1(filter.Query(), param);
            }
            catch (Exception ex)
            {
                LogSystem.Error(ex);
        ...
        internal List<HIS_MATERIAL_TYPE> GetByMaterialTypeMapId(long materialTypeMapId)
        {
            try
            {
                HisMaterialTypeFilterQuery filter = new HisMaterialTypeFilterQuery();
                filter.MATERIAL_TYPE_MAP_ID = materialTypeMapId;
                return this.Get(filter);
            }
            catch (Exception ex)
            {
