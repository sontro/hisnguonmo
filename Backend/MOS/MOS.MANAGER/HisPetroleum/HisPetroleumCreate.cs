...
namespace MOS.MANAGER.HisPetroleum
{
    partial class HisPetroleumCreate : BusinessBase
    {
		...
		internal bool CreateList(List<HIS_PETROLEUM> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    if (!DAOWorker.HisPetroleumDAO.CreateList(listData))
                    {
                        MOS.MANAGER.Base.BugUtil.SetBugCode(param, MOS.LibraryBug.Bug.Enum.HisPetroleum_ThemMoiThatBai);
                        throw new Exception("Them moi thong tin HisPetroleum that bai." + LogUtil.TraceData("listData", listData));
                    }
                    this.recentHisPetroleums.AddRange(listData);
                    result = true;
                }
            }
            catch (Exception ex)
