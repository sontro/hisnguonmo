...
{
    partial class HisPetroleumLock : BusinessBase
    {
        internal HisPetroleumLock()
            : base()
        {

        }

        internal HisPetroleumLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_PETROLEUM resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_PETROLEUM data = new HisPetroleumGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisPetroleumDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_PETROLEUM resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_PETROLEUM resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisPetroleumCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisPetroleumDAO.Update(raw);
