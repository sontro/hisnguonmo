...
{
    partial class HisExroRoomLock : BusinessBase
    {
        internal HisExroRoomLock()
            : base()
        {

        }

        internal HisExroRoomLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_EXRO_ROOM resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_EXRO_ROOM data = new HisExroRoomGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisExroRoomDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_EXRO_ROOM resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_EXRO_ROOM resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisExroRoomCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisExroRoomDAO.Update(raw);
