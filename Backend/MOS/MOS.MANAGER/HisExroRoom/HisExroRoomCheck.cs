...
{
    partial class HisExroRoomCheck : BusinessBase
    {
        internal HisExroRoomCheck()
            : base()
        {

        }

        internal HisExroRoomCheck(CommonParam paramCheck)
            : base(paramCheck)
        {

        }

        internal bool VerifyRequireField(HIS_EXRO_ROOM data)
        {
            bool valid = true;
            try
            {
                ...
                if (!data.IS_ALLOW_REQUEST.HasValue && !data.IS_HOLD_ORDER.HasValue) throw new ArgumentNullException("data.IS_ALLOW_REQUEST && data.IS_HOLD_ORDER");
            }
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisExroRoomGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_EXRO_ROOM data)
        {
            bool valid = true;
            try
            {
                data = new HisExroRoomGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                ...
                {
                    HisExroRoomFilterQuery filter = new HisExroRoomFilterQuery();
                    filter.IDs = listId;
                    List<HIS_EXRO_ROOM> listData = new HisExroRoomGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_EXRO_ROOM> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_EXRO_ROOM data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisExroRoomDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_EXRO_ROOM> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool CheckConstraint(long id)
        {
            ...
            try
            {
                /*List<XXX> hisXXXs = new HisXXXGet().GetByHisExroRoomId(id);
