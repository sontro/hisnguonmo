...
{
    partial class HisAntigenLock : BusinessBase
    {
        internal HisAntigenLock()
            : base()
        {

        }

        internal HisAntigenLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_ANTIGEN resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_ANTIGEN data = new HisAntigenGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisAntigenDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_ANTIGEN resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_ANTIGEN resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisAntigenCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisAntigenDAO.Update(raw);
