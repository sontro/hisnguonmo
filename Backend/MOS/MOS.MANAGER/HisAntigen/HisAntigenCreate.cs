...
namespace MOS.MANAGER.HisAntigen
{
    partial class HisAntigenCreate : BusinessBase
    {
		...
		internal bool CreateList(List<HIS_ANTIGEN> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    if (!DAOWorker.HisAntigenDAO.CreateList(listData))
                    {
                        MOS.MANAGER.Base.BugUtil.SetBugCode(param, MOS.LibraryBug.Bug.Enum.HisAntigen_ThemMoiThatBai);
                        throw new Exception("Them moi thong tin HisAntigen that bai." + LogUtil.TraceData("listData", listData));
                    }
                    this.recentHisAntigens.AddRange(listData);
                    result = true;
                }
            }
            catch (Exception ex)
