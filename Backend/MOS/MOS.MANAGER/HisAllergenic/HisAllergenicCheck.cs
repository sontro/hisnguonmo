...
{
    partial class HisAllergenicCheck : BusinessBase
    {
        internal HisAllergenicCheck()
            : base()
        {

        }

        internal HisAllergenicCheck(CommonParam paramCheck)
            : base(paramCheck) 
        {

        }

        internal bool VerifyRequireField(HIS_ALLERGENIC data)
        {
            bool valid = true;
            try
            {
                if (data == null) throw new ArgumentNullException("data");
                if (data.ALLERGY_CARD_ID <= 0) throw new ArgumentNullException("data.ALLERGY_CARD_ID");
                if (!data.MEDICINE_TYPE_ID.HasValue && String.IsNullOrWhiteSpace(data.ALLERGENIC_NAME)) throw new ArgumentNullException("data.MEDICINE_TYPE_ID && data.ALLERGENIC_NAME");
                if (data.IS_DOUBT.HasValue && data.IS_SURE.HasValue) throw new ArgumentNullException("data.IS_DOUBT && data.IS_SURE");
                if (!data.IS_DOUBT.HasValue && !data.IS_SURE.HasValue) throw new ArgumentNullException("data.IS_DOUBT && data.IS_SURE");
            }
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisAllergenicGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_ALLERGENIC data)
        {
            bool valid = true;
            try
            {
                data = new HisAllergenicGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                if (listId != null && listId.Count > 0)
                {
                    ...
                    filter.IDs = listId;
                    List<HIS_ALLERGENIC> listData = new HisAllergenicGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_ALLERGENIC> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_ALLERGENIC data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisAllergenicDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_ALLERGENIC> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool CheckConstraint(long id)
        {
            ...
            try
            {
                /*List<XXX> hisXXXs = new HisXXXGet().GetByHisAllergenicId(id);
