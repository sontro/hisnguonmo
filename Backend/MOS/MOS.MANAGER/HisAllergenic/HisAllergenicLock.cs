...
{
    partial class HisAllergenicLock : BusinessBase
    {
        internal HisAllergenicLock()
            : base()
        {

        }

        internal HisAllergenicLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_ALLERGENIC resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_ALLERGENIC data = new HisAllergenicGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisAllergenicDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_ALLERGENIC resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_ALLERGENIC resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisAllergenicCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisAllergenicDAO.Update(raw);
