...
{
    class HisSampleRoomLock : BusinessBase
    {
        internal HisSampleRoomLock()
            : base()
        {

        }

        internal HisSampleRoomLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_SAMPLE_ROOM data)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisSampleRoomCheck().VerifyId(data.ID, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisSampleRoomDAO.Update(raw);
                    if (result)
                    {
