...
{
    partial class HisMedicineTypeTutLock : BusinessBase
    {
        internal HisMedicineTypeTutLock()
            : base()
        {

        }

        internal HisMedicineTypeTutLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_MEDICINE_TYPE_TUT resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_MEDICINE_TYPE_TUT data = new HisMedicineTypeTutGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisMedicineTypeTutDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_MEDICINE_TYPE_TUT resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_MEDICINE_TYPE_TUT resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisMedicineTypeTutCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisMedicineTypeTutDAO.Update(raw);
