...
{
    partial class HisMediContractMatyLock : BusinessBase
    {
        internal HisMediContractMatyLock()
            : base()
        {

        }

        internal HisMediContractMatyLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_MEDI_CONTRACT_MATY resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_MEDI_CONTRACT_MATY data = new HisMediContractMatyGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisMediContractMatyDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_MEDI_CONTRACT_MATY resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_MEDI_CONTRACT_MATY resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisMediContractMatyCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisMediContractMatyDAO.Update(raw);
