...
{
    partial class HisSeseDepoRepayLock : BusinessBase
    {
        internal HisSeseDepoRepayLock()
            : base()
        {

        }

        internal HisSeseDepoRepayLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_SESE_DEPO_REPAY resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_SESE_DEPO_REPAY data = new HisSeseDepoRepayGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisSeseDepoRepayDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_SESE_DEPO_REPAY resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_SESE_DEPO_REPAY resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisSeseDepoRepayCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisSeseDepoRepayDAO.Update(raw);
