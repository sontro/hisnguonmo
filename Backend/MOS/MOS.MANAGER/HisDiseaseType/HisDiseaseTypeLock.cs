...
{
    partial class HisDiseaseTypeLock : BusinessBase
    {
        internal HisDiseaseTypeLock()
            : base()
        {

        }

        internal HisDiseaseTypeLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_DISEASE_TYPE resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_DISEASE_TYPE data = new HisDiseaseTypeGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisDiseaseTypeDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_DISEASE_TYPE resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_DISEASE_TYPE resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisDiseaseTypeCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisDiseaseTypeDAO.Update(raw);
