...
{
    partial class HisEkipLock : BusinessBase
    {
        internal HisEkipLock()
            : base()
        {

        }

        internal HisEkipLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_EKIP resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_EKIP data = new HisEkipGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisEkipDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_EKIP resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(HIS_EKIP data)
        {
            bool result = false;
            try
            {
                bool valid = true;
                HIS_EKIP raw = null;
                valid = valid && new HisEkipCheck().VerifyId(data.ID, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisEkipDAO.Update(raw);
