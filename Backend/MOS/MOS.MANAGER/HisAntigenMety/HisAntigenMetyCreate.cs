...
namespace MOS.MANAGER.HisAntigenMety
{
    partial class HisAntigenMetyCreate : BusinessBase
    {
		...
		internal bool CreateList(List<HIS_ANTIGEN_METY> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    if (!DAOWorker.HisAntigenMetyDAO.CreateList(listData))
                    {
                        MOS.MANAGER.Base.BugUtil.SetBugCode(param, MOS.LibraryBug.Bug.Enum.HisAntigenMety_ThemMoiThatBai);
                        throw new Exception("Them moi thong tin HisAntigenMety that bai." + LogUtil.TraceData("listData", listData));
                    }
                    this.recentHisAntigenMetys.AddRange(listData);
                    result = true;
                }
            }
            catch (Exception ex)
