...
{
    partial class HisMetyProductLock : BusinessBase
    {
        internal HisMetyProductLock()
            : base()
        {

        }

        internal HisMetyProductLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_METY_PRODUCT resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_METY_PRODUCT data = new HisMetyProductGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisMetyProductDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_METY_PRODUCT resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_METY_PRODUCT resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisMetyProductCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisMetyProductDAO.Update(raw);
