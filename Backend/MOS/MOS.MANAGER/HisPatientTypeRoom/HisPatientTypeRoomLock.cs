...
{
    partial class HisPatientTypeRoomLock : BusinessBase
    {
        internal HisPatientTypeRoomLock()
            : base()
        {

        }

        internal HisPatientTypeRoomLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_PATIENT_TYPE_ROOM resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_PATIENT_TYPE_ROOM data = new HisPatientTypeRoomGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisPatientTypeRoomDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_PATIENT_TYPE_ROOM resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_PATIENT_TYPE_ROOM resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisPatientTypeRoomCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisPatientTypeRoomDAO.Update(raw);
