...
{
    partial class HisImpSourceLock : BusinessBase
    {
        internal HisImpSourceLock()
            : base()
        {

        }

        internal HisImpSourceLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_IMP_SOURCE resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_IMP_SOURCE data = new HisImpSourceGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisImpSourceDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_IMP_SOURCE resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(HIS_IMP_SOURCE data)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisImpSourceCheck().VerifyId(data.ID, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisImpSourceDAO.Update(raw);
