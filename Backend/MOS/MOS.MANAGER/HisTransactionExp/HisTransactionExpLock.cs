...
{
    partial class HisTransactionExpLock : BusinessBase
    {
        internal HisTransactionExpLock()
            : base()
        {

        }

        internal HisTransactionExpLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_TRANSACTION_EXP resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_TRANSACTION_EXP data = new HisTransactionExpGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisTransactionExpDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_TRANSACTION_EXP resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_TRANSACTION_EXP resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisTransactionExpCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisTransactionExpDAO.Update(raw);
