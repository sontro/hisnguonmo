...
namespace MOS.MANAGER.HisMaterialPaty
{
    class HisMaterialPatyGet : GetBase
    {
        ...
        internal List<HIS_MATERIAL_PATY> GetOfLast(long materialTypeId)
        {
            try
            {
                string sql = "SELECT ID FROM HIS_MATERIAL M WHERE M.IMP_TIME IS NOT NULL AND M.MATERIAL_TYPE_ID = :param0 ORDER BY M.IMP_TIME DESC FETCH FIRST ROWS ONLY";
                long materialId = DAOWorker.SqlDAO.GetSqlSingle<long>(sql, materialTypeId);
                if (materialId > 0)
                {
