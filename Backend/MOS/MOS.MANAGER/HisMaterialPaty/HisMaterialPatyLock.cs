...
{
    class HisMaterialPatyLock : BusinessBase
    {
        internal HisMaterialPatyLock()
            : base()
        {

        }

        internal HisMaterialPatyLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_MATERIAL_PATY data)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    data = new HisMaterialPatyGet().GetById(data.ID);
                    if (data != null)
                    {
                        ...
                        result = DAOWorker.HisMaterialPatyDAO.Update(data);
                    }
                    else
                    {
