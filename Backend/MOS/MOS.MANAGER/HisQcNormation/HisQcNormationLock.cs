...
{
    partial class HisQcNormationLock : BusinessBase
    {
        internal HisQcNormationLock()
            : base()
        {

        }

        internal HisQcNormationLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_QC_NORMATION resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_QC_NORMATION data = new HisQcNormationGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisQcNormationDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_QC_NORMATION resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_QC_NORMATION resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisQcNormationCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisQcNormationDAO.Update(raw);
