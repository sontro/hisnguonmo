...
{
    partial class HisUserGroupTempLock : BusinessBase
    {
        internal HisUserGroupTempLock()
            : base()
        {

        }

        internal HisUserGroupTempLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_USER_GROUP_TEMP resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_USER_GROUP_TEMP data = new HisUserGroupTempGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisUserGroupTempDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_USER_GROUP_TEMP resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_USER_GROUP_TEMP resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisUserGroupTempCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisUserGroupTempDAO.Update(raw);
