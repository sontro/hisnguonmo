...
namespace MOS.MANAGER.HisUserGroupTemp
{
    partial class HisUserGroupTempCreate : BusinessBase
    {
		...
		internal bool CreateList(List<HIS_USER_GROUP_TEMP> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    if (!DAOWorker.HisUserGroupTempDAO.CreateList(listData))
                    {
                        MOS.MANAGER.Base.BugUtil.SetBugCode(param, MOS.LibraryBug.Bug.Enum.HisUserGroupTemp_ThemMoiThatBai);
                        throw new Exception("Them moi thong tin HisUserGroupTemp that bai." + LogUtil.TraceData("listData", listData));
                    }
                    this.recentHisUserGroupTemps.AddRange(listData);
                    result = true;
                }
            }
            catch (Exception ex)
