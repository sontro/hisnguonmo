...
{
    partial class HisDebateTempLock : BusinessBase
    {
        internal HisDebateTempLock()
            : base()
        {

        }

        internal HisDebateTempLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_DEBATE_TEMP resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_DEBATE_TEMP data = new HisDebateTempGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisDebateTempDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_DEBATE_TEMP resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_DEBATE_TEMP resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisDebateTempCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisDebateTempDAO.Update(raw);
