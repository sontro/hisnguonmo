...
namespace MOS.MANAGER.HisSereServFile
{
    ...
    /// </summary>
    class HisSereServFileCheck : BusinessBase
    {
        internal HisSereServFileCheck()
            : base()
        {

        }

        internal HisSereServFileCheck(CommonParam paramCheck)
            : base(paramCheck) 
        {

        }

        internal bool VerifyRequireField(HIS_SERE_SERV_FILE data)
        {
            bool valid = true;
            try
            {
                ...
                if (!IsNotNullOrEmpty(data.URL)) throw new ArgumentNullException("data.URL");
                if (!IsNotNullOrEmpty(data.SERE_SERV_FILE_NAME)) throw new ArgumentNullException("data.SERE_SERV_FILE_NAME");
            }
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisSereServFileGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_SERE_SERV_FILE data)
        {
            bool valid = true;
            try
            {
                data = new HisSereServFileGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                if (listId != null && listId.Count > 0)
                {
                    ...
                    filter.IDs = listId;
                    List<HIS_SERE_SERV_FILE> listData = new HisSereServFileGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_SERE_SERV_FILE> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_SERE_SERV_FILE data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisSereServFileDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_SERE_SERV_FILE> listData)
        {
            bool valid = true;
            try
            {
