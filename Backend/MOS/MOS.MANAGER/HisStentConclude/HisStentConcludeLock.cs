...
{
    partial class HisStentConcludeLock : BusinessBase
    {
        internal HisStentConcludeLock()
            : base()
        {

        }

        internal HisStentConcludeLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_STENT_CONCLUDE resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_STENT_CONCLUDE data = new HisStentConcludeGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisStentConcludeDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_STENT_CONCLUDE resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_STENT_CONCLUDE resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisStentConcludeCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisStentConcludeDAO.Update(raw);
