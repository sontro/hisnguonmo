...
{
    partial class HisCaroDepartmentLock : BusinessBase
    {
        internal HisCaroDepartmentLock()
            : base()
        {

        }

        internal HisCaroDepartmentLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_CARO_DEPARTMENT resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_CARO_DEPARTMENT data = new HisCaroDepartmentGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisCaroDepartmentDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_CARO_DEPARTMENT resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(HIS_CARO_DEPARTMENT data)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisCaroDepartmentCheck().VerifyId(data.ID, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisCaroDepartmentDAO.Update(raw);
