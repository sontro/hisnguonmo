...
{
    partial class HisPtttHighTechLock : BusinessBase
    {
        internal HisPtttHighTechLock()
            : base()
        {

        }

        internal HisPtttHighTechLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_PTTT_HIGH_TECH resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_PTTT_HIGH_TECH data = new HisPtttHighTechGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisPtttHighTechDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_PTTT_HIGH_TECH resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_PTTT_HIGH_TECH resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisPtttHighTechCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisPtttHighTechDAO.Update(raw);
