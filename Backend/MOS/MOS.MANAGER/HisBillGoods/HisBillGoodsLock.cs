...
{
    partial class HisBillGoodsLock : BusinessBase
    {
        internal HisBillGoodsLock()
            : base()
        {

        }

        internal HisBillGoodsLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_BILL_GOODS resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_BILL_GOODS data = new HisBillGoodsGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisBillGoodsDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_BILL_GOODS resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_BILL_GOODS resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisBillGoodsCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisBillGoodsDAO.Update(raw);
