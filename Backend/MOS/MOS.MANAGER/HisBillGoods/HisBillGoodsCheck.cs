...
{
    partial class HisBillGoodsCheck : BusinessBase
    {
        internal HisBillGoodsCheck()
            : base()
        {

        }

        internal HisBillGoodsCheck(CommonParam paramCheck)
            : base(paramCheck)
        {

        }

        internal bool VerifyRequireField(HIS_BILL_GOODS data)
        {
            bool valid = true;
            try
            {
                if (data == null) throw new ArgumentNullException("data");
                if (data.BILL_ID <= 0) throw new ArgumentNullException("data.BILL_ID");
                if (data.AMOUNT < 0) throw new ArgumentNullException("data.AMOUNT");
                if (data.PRICE < 0) throw new ArgumentNullException("data.PRICE");
                if (String.IsNullOrWhiteSpace(data.GOODS_NAME)) throw new ArgumentNullException("data.GOODS_NAME");
                if (data.DISCOUNT.HasValue && data.DISCOUNT.Value < 0) throw new ArgumentNullException("data.DISCOUNT");
            }
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisBillGoodsGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_BILL_GOODS data)
        {
            bool valid = true;
            try
            {
                data = new HisBillGoodsGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                ...
                {
                    HisBillGoodsFilterQuery filter = new HisBillGoodsFilterQuery();
                    filter.IDs = listId;
                    List<HIS_BILL_GOODS> listData = new HisBillGoodsGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_BILL_GOODS> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_BILL_GOODS data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisBillGoodsDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_BILL_GOODS> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool CheckConstraint(long id)
        {
            ...
            try
            {
                /*List<XXX> hisXXXs = new HisXXXGet().GetByHisBillGoodsId(id);
