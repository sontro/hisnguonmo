...
{
    partial class HisMaterialTypeMapLock : BusinessBase
    {
        internal HisMaterialTypeMapLock()
            : base()
        {

        }

        internal HisMaterialTypeMapLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_MATERIAL_TYPE_MAP resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_MATERIAL_TYPE_MAP data = new HisMaterialTypeMapGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisMaterialTypeMapDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_MATERIAL_TYPE_MAP resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_MATERIAL_TYPE_MAP resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisMaterialTypeMapCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisMaterialTypeMapDAO.Update(raw);
