...
{
    partial class HisSereServTempLock : BusinessBase
    {
        internal HisSereServTempLock()
            : base()
        {

        }

        internal HisSereServTempLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_SERE_SERV_TEMP resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_SERE_SERV_TEMP data = new HisSereServTempGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisSereServTempDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_SERE_SERV_TEMP resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_SERE_SERV_TEMP resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisSereServTempCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisSereServTempDAO.Update(raw);
