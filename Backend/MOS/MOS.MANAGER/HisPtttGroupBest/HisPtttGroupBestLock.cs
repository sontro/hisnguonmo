...
{
    partial class HisPtttGroupBestLock : BusinessBase
    {
        internal HisPtttGroupBestLock()
            : base()
        {

        }

        internal HisPtttGroupBestLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_PTTT_GROUP_BEST resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_PTTT_GROUP_BEST data = new HisPtttGroupBestGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisPtttGroupBestDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_PTTT_GROUP_BEST resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_PTTT_GROUP_BEST resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisPtttGroupBestCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisPtttGroupBestDAO.Update(raw);
