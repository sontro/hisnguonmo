...
{
    partial class HisDeskLock : BusinessBase
    {
        internal HisDeskLock()
            : base()
        {

        }

        internal HisDeskLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_DESK resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_DESK data = new HisDeskGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisDeskDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_DESK resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_DESK resultData)
        {
            bool result = false;
            try
            {
                bool valid = true;
                HIS_DESK raw = null;
                valid = valid && new HisDeskCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisDeskDAO.Update(raw);
