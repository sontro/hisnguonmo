...
namespace MOS.MANAGER.HisDesk
{
    partial class HisDeskCreate : BusinessBase
    {
		...
		internal bool CreateList(List<HIS_DESK> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    if (!DAOWorker.HisDeskDAO.CreateList(listData))
                    {
                        MOS.MANAGER.Base.BugUtil.SetBugCode(param, MOS.LibraryBug.Bug.Enum.HisDesk_ThemMoiThatBai);
                        throw new Exception("Them moi thong tin HisDesk that bai." + LogUtil.TraceData("listData", listData));
                    }
                    this.recentHisDesks.AddRange(listData);
                    result = true;
                }
            }
            catch (Exception ex)
