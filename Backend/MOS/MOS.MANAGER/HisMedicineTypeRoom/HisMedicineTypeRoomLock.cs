...
{
    partial class HisMedicineTypeRoomLock : BusinessBase
    {
        internal HisMedicineTypeRoomLock()
            : base()
        {

        }

        internal HisMedicineTypeRoomLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_MEDICINE_TYPE_ROOM resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_MEDICINE_TYPE_ROOM data = new HisMedicineTypeRoomGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisMedicineTypeRoomDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_MEDICINE_TYPE_ROOM resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_MEDICINE_TYPE_ROOM resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisMedicineTypeRoomCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisMedicineTypeRoomDAO.Update(raw);
