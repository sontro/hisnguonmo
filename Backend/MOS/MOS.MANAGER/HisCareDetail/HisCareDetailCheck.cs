...
namespace MOS.MANAGER.HisCareDetail
{
    ...
    /// </summary>
    partial class HisCareDetailCheck : BusinessBase
    {
        internal HisCareDetailCheck()
            : base()
        {

        }

        internal HisCareDetailCheck(CommonParam paramCheck)
            : base(paramCheck)
        {

        }

        internal bool VerifyRequireField(HIS_CARE_DETAIL data)
        {
            bool valid = true;
            try
            {
                if (data == null) throw new ArgumentNullException("data");
                if (!IsGreaterThanZero(data.CARE_ID)) throw new ArgumentNullException("data.CARE_ID");
                if (!IsGreaterThanZero(data.CARE_TYPE_ID)) throw new ArgumentNullException("data.CARE_ID");
                if (!IsNotNullOrEmpty(data.CONTENT)) throw new ArgumentNullException("data.CONTENT");
            }
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisCareDetailGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_CARE_DETAIL data)
        {
            bool valid = true;
            try
            {
                data = new HisCareDetailGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                if (listId != null && listId.Count > 0)
                {
                    ...
                    filter.IDs = listId;
                    List<HIS_CARE_DETAIL> listData = new HisCareDetailGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_CARE_DETAIL> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_CARE_DETAIL data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisCareDetailDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_CARE_DETAIL> listData)
        {
            bool valid = true;
            try
            {
