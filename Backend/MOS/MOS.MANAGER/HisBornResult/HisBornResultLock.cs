...
{
    partial class HisBornResultLock : BusinessBase
    {
        internal HisBornResultLock()
            : base()
        {

        }

        internal HisBornResultLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_BORN_RESULT resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_BORN_RESULT data = new HisBornResultGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisBornResultDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_BORN_RESULT resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_BORN_RESULT resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisBornResultCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisBornResultDAO.Update(raw);
