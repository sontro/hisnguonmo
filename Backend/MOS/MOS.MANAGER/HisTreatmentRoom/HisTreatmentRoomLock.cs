...
{
    partial class HisTreatmentRoomLock : BusinessBase
    {
        internal HisTreatmentRoomLock()
            : base()
        {

        }

        internal HisTreatmentRoomLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_TREATMENT_ROOM resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_TREATMENT_ROOM data = new HisTreatmentRoomGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisTreatmentRoomDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_TREATMENT_ROOM resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_TREATMENT_ROOM resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisTreatmentRoomCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisTreatmentRoomDAO.Update(raw);
