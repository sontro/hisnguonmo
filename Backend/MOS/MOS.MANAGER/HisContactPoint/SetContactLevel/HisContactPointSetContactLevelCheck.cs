...
{
    partial class HisContactPointSetContactLevelCheck : BusinessBase
    {		
        internal HisContactPointSetContactLevelCheck()
            : base()
        {

        }

        internal HisContactPointSetContactLevelCheck(CommonParam param)
            : base(param)
        {
        ...
        internal bool IsValidData(HIS_CONTACT_POINT data)
        {
            ...
            try
            {
                ...
                if (data.CONTACT_TYPE == IMSys.DbConfig.HIS_RS.HIS_CONTACT_POINT.CONTACT_TYPE__KHAC
                    && (string.IsNullOrWhiteSpace(data.LAST_NAME) || !data.DOB.HasValue || !data.GENDER_ID.HasValue))
                {
