...
{
    partial class HisContactPointLock : BusinessBase
    {
        internal HisContactPointLock()
            : base()
        {

        }

        internal HisContactPointLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_CONTACT_POINT resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_CONTACT_POINT data = new HisContactPointGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisContactPointDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_CONTACT_POINT resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_CONTACT_POINT resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisContactPointCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisContactPointDAO.Update(raw);
