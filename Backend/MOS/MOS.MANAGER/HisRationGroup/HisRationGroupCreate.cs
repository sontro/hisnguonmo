...
namespace MOS.MANAGER.HisRationGroup
{
    partial class HisRationGroupCreate : BusinessBase
    {
		...
		internal bool CreateList(List<HIS_RATION_GROUP> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    if (!DAOWorker.HisRationGroupDAO.CreateList(listData))
                    {
                        MOS.MANAGER.Base.BugUtil.SetBugCode(param, MOS.LibraryBug.Bug.Enum.HisRationGroup_ThemMoiThatBai);
                        throw new Exception("Them moi thong tin HisRationGroup that bai." + LogUtil.TraceData("listData", listData));
                    }
                    this.recentHisRationGroups.AddRange(listData);
                    result = true;
                }
            }
            catch (Exception ex)
