...
{
    partial class HisDebateReasonLock : BusinessBase
    {
        internal HisDebateReasonLock()
            : base()
        {

        }

        internal HisDebateReasonLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_DEBATE_REASON resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_DEBATE_REASON data = new HisDebateReasonGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisDebateReasonDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_DEBATE_REASON resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_DEBATE_REASON resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisDebateReasonCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisDebateReasonDAO.Update(raw);
