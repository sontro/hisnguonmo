...
namespace MOS.MANAGER.HisDebateReason
{
    partial class HisDebateReasonCreate : BusinessBase
    {
		...
		internal bool CreateList(List<HIS_DEBATE_REASON> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    if (!DAOWorker.HisDebateReasonDAO.CreateList(listData))
                    {
                        MOS.MANAGER.Base.BugUtil.SetBugCode(param, MOS.LibraryBug.Bug.Enum.HisDebateReason_ThemMoiThatBai);
                        throw new Exception("Them moi thong tin HisDebateReason that bai." + LogUtil.TraceData("listData", listData));
                    }
                    this.recentHisDebateReasons.AddRange(listData);
                    result = true;
                }
            }
            catch (Exception ex)
