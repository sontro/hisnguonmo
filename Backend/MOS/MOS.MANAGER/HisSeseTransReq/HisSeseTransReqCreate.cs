...
namespace MOS.MANAGER.HisSeseTransReq
{
    partial class HisSeseTransReqCreate : BusinessBase
    {
		...
		internal bool CreateList(List<HIS_SESE_TRANS_REQ> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    if (!DAOWorker.HisSeseTransReqDAO.CreateList(listData))
                    {
                        MOS.MANAGER.Base.BugUtil.SetBugCode(param, MOS.LibraryBug.Bug.Enum.HisSeseTransReq_ThemMoiThatBai);
                        throw new Exception("Them moi thong tin HisSeseTransReq that bai." + LogUtil.TraceData("listData", listData));
                    }
                    this.recentHisSeseTransReqs.AddRange(listData);
                    result = true;
                }
            }
            catch (Exception ex)
