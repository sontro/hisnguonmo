...
{
    partial class HisSeseTransReqLock : BusinessBase
    {
        internal HisSeseTransReqLock()
            : base()
        {

        }

        internal HisSeseTransReqLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_SESE_TRANS_REQ resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_SESE_TRANS_REQ data = new HisSeseTransReqGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisSeseTransReqDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_SESE_TRANS_REQ resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_SESE_TRANS_REQ resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisSeseTransReqCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisSeseTransReqDAO.Update(raw);
