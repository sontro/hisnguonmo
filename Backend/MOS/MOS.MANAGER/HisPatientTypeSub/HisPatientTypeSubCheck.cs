...
namespace MOS.MANAGER.HisPatientTypeSub
{
    ...
    /// </summary>
    partial class HisPatientTypeSubCheck : BusinessBase
    {
        internal HisPatientTypeSubCheck()
            : base()
        {

        }

        internal HisPatientTypeSubCheck(CommonParam paramCheck)
            : base(paramCheck) 
        {

        }

        internal bool VerifyRequireField(HIS_PATIENT_TYPE_SUB data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisPatientTypeSubGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_PATIENT_TYPE_SUB data)
        {
            bool valid = true;
            try
            {
                data = new HisPatientTypeSubGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                ...
                {
                    HisPatientTypeSubFilterQuery filter = new HisPatientTypeSubFilterQuery();
                    filter.IDs = listId;
                    List<HIS_PATIENT_TYPE_SUB> listData = new HisPatientTypeSubGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_PATIENT_TYPE_SUB> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_PATIENT_TYPE_SUB data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisPatientTypeSubDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_PATIENT_TYPE_SUB> listData)
        {
            bool valid = true;
            try
            {
