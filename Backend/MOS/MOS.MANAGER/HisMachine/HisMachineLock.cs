...
{
    partial class HisMachineLock : BusinessBase
    {
        internal HisMachineLock()
            : base()
        {

        }

        internal HisMachineLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_MACHINE resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_MACHINE data = new HisMachineGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisMachineDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_MACHINE resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_MACHINE resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisMachineCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisMachineDAO.Update(raw);
