...
{
    partial class HisAdrMedicineTypeLock : BusinessBase
    {
        internal HisAdrMedicineTypeLock()
            : base()
        {

        }

        internal HisAdrMedicineTypeLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_ADR_MEDICINE_TYPE resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_ADR_MEDICINE_TYPE data = new HisAdrMedicineTypeGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisAdrMedicineTypeDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_ADR_MEDICINE_TYPE resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_ADR_MEDICINE_TYPE resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisAdrMedicineTypeCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisAdrMedicineTypeDAO.Update(raw);
