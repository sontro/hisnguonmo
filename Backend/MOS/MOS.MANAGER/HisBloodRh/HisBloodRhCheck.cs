...
namespace MOS.MANAGER.HisBloodRh
{
    ...
    /// </summary>
    partial class HisBloodRhCheck : BusinessBase
    {
        internal HisBloodRhCheck()
            : base()
        {

        }

        internal HisBloodRhCheck(CommonParam paramCheck)
            : base(paramCheck) 
        {

        }

        internal bool VerifyRequireField(HIS_BLOOD_RH data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisBloodRhGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_BLOOD_RH data)
        {
            bool valid = true;
            try
            {
                data = new HisBloodRhGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                ...
                {
                    HisBloodRhFilterQuery filter = new HisBloodRhFilterQuery();
                    filter.IDs = listId;
                    List<HIS_BLOOD_RH> listData = new HisBloodRhGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_BLOOD_RH> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_BLOOD_RH data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisBloodRhDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_BLOOD_RH> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool ExistsCode(string code, long? id)
        {
            ...
            try
            {
                if (DAOWorker.HisBloodRhDAO.ExistsCode(code, id))
                {
        ...
        internal bool CheckConstraint(HIS_BLOOD_RH data)
        {
            bool valid = true;
            try
            {
                List<HIS_SERE_SERV_PTTT> hisSereServPttts = new HisSereServPtttGet().GetByBloodRhId(data.ID);
                if (IsNotNullOrEmpty(hisSereServPttts))
                {
                    ...
                    throw new Exception("Ton tai du lieu HIS_SERE_SERV_PTTT, khong cho phep xoa" + LogUtil.TraceData("data", data));
                }
            }
            catch (Exception ex)
            {
