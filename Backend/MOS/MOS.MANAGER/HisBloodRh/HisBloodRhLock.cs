...
{
    partial class HisBloodRhLock : BusinessBase
    {
        internal HisBloodRhLock()
            : base()
        {

        }

        internal HisBloodRhLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_BLOOD_RH data)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisBloodRhCheck().VerifyId(data.ID, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisBloodRhDAO.Update(raw);
