...
{
    partial class HisInvoiceDetailCheck : BusinessBase
    {
        internal HisInvoiceDetailCheck()
            : base()
        {

        }

        internal HisInvoiceDetailCheck(CommonParam paramCheck)
            : base(paramCheck)
        {

        }

        internal bool VerifyRequireField(HIS_INVOICE_DETAIL data)
        {
            bool valid = true;
            try
            {
                if (data == null) throw new ArgumentNullException("data");
                if (!IsGreaterThanZero(data.INVOICE_ID)) throw new ArgumentNullException("data.INVOICE_ID");
                if (data.AMOUNT < 0) throw new ArgumentNullException("data.AMOUNT");
                if (data.PRICE < 0) throw new ArgumentNullException("data.PRICE");
                if (!IsNotNullOrEmpty(data.GOODS_NAME)) throw new ArgumentNullException("data.GOODS_NAME");
            }
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisInvoiceDetailGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_INVOICE_DETAIL data)
        {
            bool valid = true;
            try
            {
                data = new HisInvoiceDetailGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                if (listId != null && listId.Count > 0)
                {
                    ...
                    filter.IDs = listId;
                    List<HIS_INVOICE_DETAIL> listData = new HisInvoiceDetailGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_INVOICE_DETAIL> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_INVOICE_DETAIL data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisInvoiceDetailDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_INVOICE_DETAIL> listData)
        {
            bool valid = true;
            try
            {
