...
namespace MOS.MANAGER.HisTranPatiTemp
{
    partial class HisTranPatiTempCreate : BusinessBase
    {
		...
		internal bool CreateList(List<HIS_TRAN_PATI_TEMP> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    if (!DAOWorker.HisTranPatiTempDAO.CreateList(listData))
                    {
                        MOS.MANAGER.Base.BugUtil.SetBugCode(param, MOS.LibraryBug.Bug.Enum.HisTranPatiTemp_ThemMoiThatBai);
                        throw new Exception("Them moi thong tin HisTranPatiTemp that bai." + LogUtil.TraceData("listData", listData));
                    }
                    this.recentHisTranPatiTemps.AddRange(listData);
                    result = true;
                }
            }
            catch (Exception ex)
