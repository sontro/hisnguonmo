...
{
    partial class HisTranPatiTempLock : BusinessBase
    {
        internal HisTranPatiTempLock()
            : base()
        {

        }

        internal HisTranPatiTempLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_TRAN_PATI_TEMP resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_TRAN_PATI_TEMP data = new HisTranPatiTempGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisTranPatiTempDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_TRAN_PATI_TEMP resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_TRAN_PATI_TEMP resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisTranPatiTempCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisTranPatiTempDAO.Update(raw);
