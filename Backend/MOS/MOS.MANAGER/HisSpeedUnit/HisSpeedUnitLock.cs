...
{
    partial class HisSpeedUnitLock : BusinessBase
    {
        internal HisSpeedUnitLock()
            : base()
        {

        }

        internal HisSpeedUnitLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_SPEED_UNIT resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_SPEED_UNIT data = new HisSpeedUnitGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisSpeedUnitDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_SPEED_UNIT resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_SPEED_UNIT resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisSpeedUnitCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisSpeedUnitDAO.Update(raw);
