...
{
    partial class HisSpeedUnitCheck : BusinessBase
    {
        internal HisSpeedUnitCheck()
            : base()
        {

        }

        internal HisSpeedUnitCheck(CommonParam paramCheck)
            : base(paramCheck) 
        {

        }

        internal bool VerifyRequireField(HIS_SPEED_UNIT data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisSpeedUnitGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_SPEED_UNIT data)
        {
            bool valid = true;
            try
            {
                data = new HisSpeedUnitGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                ...
                {
                    HisSpeedUnitFilterQuery filter = new HisSpeedUnitFilterQuery();
                    filter.IDs = listId;
                    List<HIS_SPEED_UNIT> listData = new HisSpeedUnitGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_SPEED_UNIT> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_SPEED_UNIT data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisSpeedUnitDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_SPEED_UNIT> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool CheckConstraint(long id)
        {
            ...
            try
            {
                /*List<XXX> hisXXXs = new HisXXXGet().GetByHisSpeedUnitId(id);
