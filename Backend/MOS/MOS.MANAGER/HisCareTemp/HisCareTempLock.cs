...
{
    partial class HisCareTempLock : BusinessBase
    {
        internal HisCareTempLock()
            : base()
        {

        }

        internal HisCareTempLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_CARE_TEMP resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_CARE_TEMP data = new HisCareTempGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisCareTempDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_CARE_TEMP resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_CARE_TEMP resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisCareTempCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisCareTempDAO.Update(raw);
