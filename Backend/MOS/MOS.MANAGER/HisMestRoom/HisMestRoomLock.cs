...
{
    class HisMestRoomLock : BusinessBase
    {
        internal HisMestRoomLock()
            : base()
        {

        }

        internal HisMestRoomLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_MEST_ROOM data)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    data = new HisMestRoomGet().GetById(data.ID);
                    if (data != null)
                    {
