...
{
    partial class HisExmeReasonCfgLock : BusinessBase
    {
        internal HisExmeReasonCfgLock()
            : base()
        {

        }

        internal HisExmeReasonCfgLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_EXME_REASON_CFG resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_EXME_REASON_CFG data = new HisExmeReasonCfgGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisExmeReasonCfgDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_EXME_REASON_CFG resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_EXME_REASON_CFG resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisExmeReasonCfgCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisExmeReasonCfgDAO.Update(raw);
