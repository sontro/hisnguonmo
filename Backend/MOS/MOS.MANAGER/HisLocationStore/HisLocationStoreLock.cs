...
{
    partial class HisLocationStoreLock : BusinessBase
    {
        internal HisLocationStoreLock()
            : base()
        {

        }

        internal HisLocationStoreLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_LOCATION_STORE resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_LOCATION_STORE data = new HisLocationStoreGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisLocationStoreDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_LOCATION_STORE resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_LOCATION_STORE resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisLocationStoreCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisLocationStoreDAO.Update(raw);
