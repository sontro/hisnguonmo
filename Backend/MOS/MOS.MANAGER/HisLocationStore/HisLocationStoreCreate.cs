...
namespace MOS.MANAGER.HisLocationStore
{
    partial class HisLocationStoreCreate : BusinessBase
    {
		...
		internal bool CreateList(List<HIS_LOCATION_STORE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    if (!DAOWorker.HisLocationStoreDAO.CreateList(listData))
                    {
                        MOS.MANAGER.Base.BugUtil.SetBugCode(param, MOS.LibraryBug.Bug.Enum.HisLocationStore_ThemMoiThatBai);
                        throw new Exception("Them moi thong tin HisLocationStore that bai." + LogUtil.TraceData("listData", listData));
                    }
                    this.recentHisLocationStores.AddRange(listData);
                    result = true;
                }
            }
            catch (Exception ex)
