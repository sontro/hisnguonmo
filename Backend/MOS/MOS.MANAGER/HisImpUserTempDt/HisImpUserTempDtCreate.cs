...
namespace MOS.MANAGER.HisImpUserTempDt
{
    partial class HisImpUserTempDtCreate : BusinessBase
    {
		...
		internal bool CreateList(List<HIS_IMP_USER_TEMP_DT> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    if (!DAOWorker.HisImpUserTempDtDAO.CreateList(listData))
                    {
                        MOS.MANAGER.Base.BugUtil.SetBugCode(param, MOS.LibraryBug.Bug.Enum.HisImpUserTempDt_ThemMoiThatBai);
                        throw new Exception("Them moi thong tin HisImpUserTempDt that bai." + LogUtil.TraceData("listData", listData));
                    }
                    this.recentHisImpUserTempDts.AddRange(listData);
                    result = true;
                }
            }
            catch (Exception ex)
