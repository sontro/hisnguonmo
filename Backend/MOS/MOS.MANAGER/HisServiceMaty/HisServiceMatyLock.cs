...
{
    partial class HisServiceMatyLock : BusinessBase
    {
        internal HisServiceMatyLock()
            : base()
        {

        }

        internal HisServiceMatyLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_SERVICE_MATY data)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisServiceMatyCheck().VerifyId(data.ID, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisServiceMatyDAO.Update(raw);
