...
{
    partial class HisRejectAlertLock : BusinessBase
    {
        internal HisRejectAlertLock()
            : base()
        {

        }

        internal HisRejectAlertLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_REJECT_ALERT resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_REJECT_ALERT data = new HisRejectAlertGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisRejectAlertDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_REJECT_ALERT resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_REJECT_ALERT resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisRejectAlertCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisRejectAlertDAO.Update(raw);
