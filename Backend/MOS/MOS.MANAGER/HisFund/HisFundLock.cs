...
{
    partial class HisFundLock : BusinessBase
    {
        internal HisFundLock()
            : base()
        {

        }

        internal HisFundLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_FUND data)
        {
            bool result = false;
            try
            {
                bool valid = true;
                HIS_FUND raw = null;
                valid = valid && new HisFundCheck().VerifyId(data.ID, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisFundDAO.Update(raw);
