...
namespace MOS.MANAGER.HisBidMaterialType
{
    partial class HisBidMaterialTypeCreate : BusinessBase
    {
        ...
        internal bool CreateList(List<HIS_BID_MATERIAL_TYPE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    if (!DAOWorker.HisBidMaterialTypeDAO.CreateList(listData))
                    {
                        MOS.MANAGER.Base.BugUtil.SetBugCode(param, MOS.LibraryBug.Bug.Enum.HisBidMaterialType_ThemMoiThatBai);
                        throw new Exception("Them moi thong tin HisBidMaterialType that bai." + LogUtil.TraceData("listData", listData));
                    }
                    this.recentHisBidMaterialTypes.AddRange(listData);
                    result = true;
                }
            }
            catch (Exception ex)
