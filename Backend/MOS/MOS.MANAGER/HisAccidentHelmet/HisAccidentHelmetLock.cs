...
{
    partial class HisAccidentHelmetLock : BusinessBase
    {
        internal HisAccidentHelmetLock()
            : base()
        {

        }

        internal HisAccidentHelmetLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_ACCIDENT_HELMET data)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisAccidentHelmetCheck().VerifyId(data.ID, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisAccidentHelmetDAO.Update(raw);
