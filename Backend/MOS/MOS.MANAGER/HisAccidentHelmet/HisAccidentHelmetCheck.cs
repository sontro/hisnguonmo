...
namespace MOS.MANAGER.HisAccidentHelmet
{
    ...
    /// </summary>
    partial class HisAccidentHelmetCheck : BusinessBase
    {
        internal HisAccidentHelmetCheck()
            : base()
        {

        }

        internal HisAccidentHelmetCheck(CommonParam paramCheck)
            : base(paramCheck) 
        {

        }

        internal bool VerifyRequireField(HIS_ACCIDENT_HELMET data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisAccidentHelmetGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_ACCIDENT_HELMET data)
        {
            bool valid = true;
            try
            {
                data = new HisAccidentHelmetGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                ...
                {
                    HisAccidentHelmetFilterQuery filter = new HisAccidentHelmetFilterQuery();
                    filter.IDs = listId;
                    List<HIS_ACCIDENT_HELMET> listData = new HisAccidentHelmetGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_ACCIDENT_HELMET> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_ACCIDENT_HELMET data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisAccidentHelmetDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_ACCIDENT_HELMET> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool CheckConstraint(long id)
        {
            ...
            try
            {
                List<HIS_ACCIDENT_HURT> accidentHurts = new HisAccidentHurtGet().GetByAccidentHelmetId(id);
                if (IsNotNullOrEmpty(accidentHurts))
                {
                    ...
                    throw new Exception("Ton tai du lieu HIS_ACCIDENT_HURT, khong cho phep xoa" + LogUtil.TraceData("accidentHurts", accidentHurts));
                }
            }
            catch (Exception ex)
            {
