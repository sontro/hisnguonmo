...
{
    partial class HisPackingTypeLock : BusinessBase
    {
        internal HisPackingTypeLock()
            : base()
        {

        }

        internal HisPackingTypeLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_PACKING_TYPE data)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisPackingTypeCheck().VerifyId(data.ID, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisPackingTypeDAO.Update(raw);
