...
namespace MOS.MANAGER.HisImpMestTypeUser
{
    partial class HisImpMestTypeUserCreate : BusinessBase
    {
		...
		internal bool CreateList(List<HIS_IMP_MEST_TYPE_USER> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    if (!DAOWorker.HisImpMestTypeUserDAO.CreateList(listData))
                    {
                        MOS.MANAGER.Base.BugUtil.SetBugCode(param, MOS.LibraryBug.Bug.Enum.HisImpMestTypeUser_ThemMoiThatBai);
                        throw new Exception("Them moi thong tin HisImpMestTypeUser that bai." + LogUtil.TraceData("listData", listData));
                    }
                    this.recentHisImpMestTypeUsers.AddRange(listData);
                    result = true;
                }
            }
            catch (Exception ex)
