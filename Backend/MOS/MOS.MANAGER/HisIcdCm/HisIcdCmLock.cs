...
{
    partial class HisIcdCmLock : BusinessBase
    {
        internal HisIcdCmLock()
            : base()
        {

        }

        internal HisIcdCmLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_ICD_CM resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_ICD_CM data = new HisIcdCmGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisIcdCmDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_ICD_CM resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_ICD_CM resultData)
        {
            bool result = false;
            try
            {
                bool valid = true;
                HIS_ICD_CM raw = null;
                valid = valid && new HisIcdCmCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisIcdCmDAO.Update(raw);
