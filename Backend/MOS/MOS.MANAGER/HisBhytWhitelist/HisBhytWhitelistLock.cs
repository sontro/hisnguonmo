...
{
    partial class HisBhytWhitelistLock : BusinessBase
    {
        internal HisBhytWhitelistLock()
            : base()
        {

        }

        internal HisBhytWhitelistLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_BHYT_WHITELIST resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_BHYT_WHITELIST data = new HisBhytWhitelistGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisBhytWhitelistDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_BHYT_WHITELIST resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_BHYT_WHITELIST resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisBhytWhitelistCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisBhytWhitelistDAO.Update(raw);
