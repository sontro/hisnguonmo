...
{
    partial class HisPrepareMatyLock : BusinessBase
    {
        internal HisPrepareMatyLock()
            : base()
        {

        }

        internal HisPrepareMatyLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_PREPARE_MATY resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_PREPARE_MATY data = new HisPrepareMatyGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisPrepareMatyDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_PREPARE_MATY resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_PREPARE_MATY resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisPrepareMatyCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisPrepareMatyDAO.Update(raw);
