...
{
    partial class HisPrepareMatyCheck : BusinessBase
    {
        internal HisPrepareMatyCheck()
            : base()
        {

        }

        internal HisPrepareMatyCheck(CommonParam paramCheck)
            : base(paramCheck) 
        {

        }

        internal bool VerifyRequireField(HIS_PREPARE_MATY data)
        {
            bool valid = true;
            try
            {
                ...
                if (data.PREPARE_ID <= 0) throw new ArgumentNullException("data.PREPARE_ID");
                if (data.TDL_TREATMENT_ID <= 0) throw new ArgumentNullException("data.TDL_TREATMENT_ID");
                if (data.REQ_AMOUNT <= 0) throw new ArgumentNullException("data.REQ_AMOUNT");
            }
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisPrepareMatyGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_PREPARE_MATY data)
        {
            bool valid = true;
            try
            {
                data = new HisPrepareMatyGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                if (listId != null && listId.Count > 0)
                {
                    ...
                    filter.IDs = listId;
                    List<HIS_PREPARE_MATY> listData = new HisPrepareMatyGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_PREPARE_MATY> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_PREPARE_MATY data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisPrepareMatyDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_PREPARE_MATY> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool CheckConstraint(long id)
        {
            ...
            try
            {
                /*List<XXX> hisXXXs = new HisXXXGet().GetByHisPrepareMatyId(id);
