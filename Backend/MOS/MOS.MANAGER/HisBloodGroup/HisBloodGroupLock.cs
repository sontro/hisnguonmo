...
{
    partial class HisBloodGroupLock : BusinessBase
    {
        internal HisBloodGroupLock()
            : base()
        {

        }

        internal HisBloodGroupLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_BLOOD_GROUP resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_BLOOD_GROUP data = new HisBloodGroupGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisBloodGroupDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_BLOOD_GROUP resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(HIS_BLOOD_GROUP data)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisBloodGroupCheck().VerifyId(data.ID, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisBloodGroupDAO.Update(raw);
