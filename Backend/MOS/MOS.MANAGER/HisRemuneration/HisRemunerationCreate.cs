...
namespace MOS.MANAGER.HisRemuneration
{
    partial class HisRemunerationCreate : BusinessBase
    {
		...
		internal bool CreateList(List<HIS_REMUNERATION> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    if (!DAOWorker.HisRemunerationDAO.CreateList(listData))
                    {
                        MOS.MANAGER.Base.BugUtil.SetBugCode(param, MOS.LibraryBug.Bug.Enum.HisRemuneration_ThemMoiThatBai);
                        throw new Exception("Them moi thong tin HisRemuneration that bai." + LogUtil.TraceData("listData", listData));
                    }
                    this.recentHisRemunerations.AddRange(listData);
                    result = true;
                }
            }
            catch (Exception ex)
