...
namespace MOS.MANAGER.HisAccidentBodyPart
{
    ...
    /// </summary>
    partial class HisAccidentBodyPartCheck : BusinessBase
    {
        internal HisAccidentBodyPartCheck()
            : base()
        {

        }

        internal HisAccidentBodyPartCheck(CommonParam paramCheck)
            : base(paramCheck)
        {

        }

        internal bool VerifyRequireField(HIS_ACCIDENT_BODY_PART data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisAccidentBodyPartGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_ACCIDENT_BODY_PART data)
        {
            bool valid = true;
            try
            {
                data = new HisAccidentBodyPartGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                ...
                {
                    HisAccidentBodyPartFilterQuery filter = new HisAccidentBodyPartFilterQuery();
                    filter.IDs = listId;
                    List<HIS_ACCIDENT_BODY_PART> listData = new HisAccidentBodyPartGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_ACCIDENT_BODY_PART> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_ACCIDENT_BODY_PART data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisAccidentBodyPartDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_ACCIDENT_BODY_PART> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool CheckConstraint(long id)
        {
            ...
            try
            {
                List<HIS_ACCIDENT_HURT> accidentHurts = new HisAccidentHurtGet().GetByAccidentBodyPartId(id);
                if (IsNotNullOrEmpty(accidentHurts))
                {
                    MANAGER.Base.MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.HisAccidentHurt_TonTaiDuLieu);
                    return false;
                }
            }
            catch (Exception ex)
