...
{
    partial class HisCaroAccountBookLock : BusinessBase
    {
        internal HisCaroAccountBookLock()
            : base()
        {

        }

        internal HisCaroAccountBookLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_CARO_ACCOUNT_BOOK resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_CARO_ACCOUNT_BOOK data = new HisCaroAccountBookGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisCaroAccountBookDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_CARO_ACCOUNT_BOOK resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_CARO_ACCOUNT_BOOK resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisCaroAccountBookCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisCaroAccountBookDAO.Update(raw);
