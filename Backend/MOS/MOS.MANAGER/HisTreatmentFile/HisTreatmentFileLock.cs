...
{
    partial class HisTreatmentFileLock : BusinessBase
    {
        internal HisTreatmentFileLock()
            : base()
        {

        }

        internal HisTreatmentFileLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_TREATMENT_FILE resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_TREATMENT_FILE data = new HisTreatmentFileGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisTreatmentFileDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_TREATMENT_FILE resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_TREATMENT_FILE resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisTreatmentFileCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisTreatmentFileDAO.Update(raw);
