...
{
    class HisPatientLock : BusinessBase
    {
        internal HisPatientLock()
            : base()
        {

        }

        internal HisPatientLock(Inventec.Core.CommonParam paramLock)
            : base(paramLock)
        {
        ...
        internal bool ChangeLock(long id)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    HIS_PATIENT data = new HisPatientGet().GetById(id);
                    if (data != null)
                    {
                        ...
                        result = DAOWorker.HisPatientDAO.Update(data);
                    }
                    else
                    {
        ...
        internal bool ChangeLock(HIS_PATIENT data)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    data = new HisPatientGet().GetById(data.ID);
                    if (data != null)
                    {
                        if (data.IS_ACTIVE.HasValue && data.IS_ACTIVE == IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__TRUE)
                        {
