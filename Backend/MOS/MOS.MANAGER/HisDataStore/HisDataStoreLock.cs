...
{
    class HisDataStoreLock : BusinessBase
    {
        internal HisDataStoreLock()
            : base()
        {

        }

        internal HisDataStoreLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_DATA_STORE data)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    data = new HisDataStoreGet().GetById(data.ID);
                    if (data != null)
                    {
