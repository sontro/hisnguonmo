...
{
    partial class HisMestInventoryLock : BusinessBase
    {
        internal HisMestInventoryLock()
            : base()
        {

        }

        internal HisMestInventoryLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_MEST_INVENTORY resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_MEST_INVENTORY data = new HisMestInventoryGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisMestInventoryDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_MEST_INVENTORY resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_MEST_INVENTORY resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisMestInventoryCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisMestInventoryDAO.Update(raw);
