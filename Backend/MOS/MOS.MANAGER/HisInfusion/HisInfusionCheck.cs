...
namespace MOS.MANAGER.HisInfusion
{
    ...
    /// </summary>
    partial class HisInfusionCheck : BusinessBase
    {
        internal HisInfusionCheck()
            : base()
        {

        }

        internal HisInfusionCheck(CommonParam paramCheck)
            : base(paramCheck) 
        {

        }

        internal bool VerifyRequireField(HIS_INFUSION data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisInfusionGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_INFUSION data)
        {
            bool valid = true;
            try
            {
                data = new HisInfusionGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                if (listId != null && listId.Count > 0)
                {
                    ...
                    filter.IDs = listId;
                    List<HIS_INFUSION> listData = new HisInfusionGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_INFUSION> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_INFUSION data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisInfusionDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_INFUSION> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool ValidateData(HIS_INFUSION data)
        {
            bool valid = true;
            try
            {
                if (data == null) throw new ArgumentException("data");
                if (data.FINISH_TIME < 0) throw new ArgumentNullException("data.FINISH_TIME");
                ...
                if (data.START_TIME > data.FINISH_TIME) throw new ArgumentNullException("data.START_TIME ko duoc lon hon data.FINISH_TIME");
