...
{
    partial class HisInfusionLock : BusinessBase
    {
        internal HisInfusionLock()
            : base()
        {

        }

        internal HisInfusionLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_INFUSION data)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisInfusionCheck().VerifyId(data.ID, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisInfusionDAO.Update(raw);
