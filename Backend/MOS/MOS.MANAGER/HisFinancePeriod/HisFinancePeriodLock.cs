...
{
    partial class HisFinancePeriodLock : BusinessBase
    {
        internal HisFinancePeriodLock()
            : base()
        {

        }

        internal HisFinancePeriodLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_FINANCE_PERIOD resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_FINANCE_PERIOD data = new HisFinancePeriodGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisFinancePeriodDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_FINANCE_PERIOD resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_FINANCE_PERIOD resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisFinancePeriodCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisFinancePeriodDAO.Update(raw);
