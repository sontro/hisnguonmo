...
{
    partial class HisAlertLock : BusinessBase
    {
        internal HisAlertLock()
            : base()
        {

        }

        internal HisAlertLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_ALERT resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_ALERT data = new HisAlertGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisAlertDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_ALERT resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_ALERT resultData)
        {
            bool result = false;
            try
            {
                bool valid = true;
                HIS_ALERT raw = null;
                valid = valid && new HisAlertCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisAlertDAO.Update(raw);
