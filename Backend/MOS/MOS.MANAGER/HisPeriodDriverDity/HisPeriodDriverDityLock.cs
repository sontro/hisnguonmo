...
{
    partial class HisPeriodDriverDityLock : BusinessBase
    {
        internal HisPeriodDriverDityLock()
            : base()
        {

        }

        internal HisPeriodDriverDityLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_PERIOD_DRIVER_DITY resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_PERIOD_DRIVER_DITY data = new HisPeriodDriverDityGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisPeriodDriverDityDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_PERIOD_DRIVER_DITY resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_PERIOD_DRIVER_DITY resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisPeriodDriverDityCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisPeriodDriverDityDAO.Update(raw);
