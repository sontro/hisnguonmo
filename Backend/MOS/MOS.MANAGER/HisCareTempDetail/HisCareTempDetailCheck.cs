...
{
    partial class HisCareTempDetailCheck : BusinessBase
    {
        internal HisCareTempDetailCheck()
            : base()
        {

        }

        internal HisCareTempDetailCheck(CommonParam paramCheck)
            : base(paramCheck)
        {

        }

        internal bool VerifyRequireField(HIS_CARE_TEMP_DETAIL data)
        {
            bool valid = true;
            try
            {
                if (data == null) throw new ArgumentNullException("data");
                if (data.CARE_TEMP_ID <= 0) throw new ArgumentNullException("data.CARE_TEMP_ID");
                if (data.CARE_TYPE_ID <= 0) throw new ArgumentNullException("data.CARE_TYPE_ID");
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisCareTempDetailGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_CARE_TEMP_DETAIL data)
        {
            bool valid = true;
            try
            {
                data = new HisCareTempDetailGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                if (listId != null && listId.Count > 0)
                {
                    ...
                    filter.IDs = listId;
                    List<HIS_CARE_TEMP_DETAIL> listData = new HisCareTempDetailGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_CARE_TEMP_DETAIL> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_CARE_TEMP_DETAIL data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisCareTempDetailDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_CARE_TEMP_DETAIL> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool CheckConstraint(long id)
        {
            ...
            try
            {
                /*List<XXX> hisXXXs = new HisXXXGet().GetByHisCareTempDetailId(id);
