...
{
    partial class HisBillFundLock : BusinessBase
    {
        internal HisBillFundLock()
            : base()
        {

        }

        internal HisBillFundLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_BILL_FUND data)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisBillFundCheck().VerifyId(data.ID, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisBillFundDAO.Update(raw);
