...
{
    class HisUserRoomLock : BusinessBase
    {
        internal HisUserRoomLock()
            : base()
        {

        }

        internal HisUserRoomLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_USER_ROOM data)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    data = new HisUserRoomGet().GetById(data.ID);
                    if (data != null)
                    {
