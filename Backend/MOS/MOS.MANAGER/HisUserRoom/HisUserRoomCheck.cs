...
{
    class HisUserRoomCheck : BusinessBase
    {
        internal HisUserRoomCheck()
            : base()
        {

        }

        internal HisUserRoomCheck(CommonParam paramCheck)
            : base(paramCheck) 
        {

        }
        
        internal bool VerifyRequireField(HIS_USER_ROOM data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(HIS_USER_ROOM data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisUserRoomDAO.IsUnLock(id))
                {
                    valid = false;
                    MOS.MANAGER.Base.BugUtil.SetBugCode(param, MOS.LibraryBug.Bug.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_USER_ROOM data)
        {
            bool valid = true;
            try
            {
                data = new HisUserRoomGet().GetById(id);
                if (data == null)
                {
        ...
        internal bool VerifyIds(List<long> listId, List<HIS_USER_ROOM> listObject)
        {
            bool valid = true;
            try
            {
                if (listId != null && listId.Count > 0)
                {
                    ...
                    filter.IDs = listId;
                    List<HIS_USER_ROOM> listData = new HisUserRoomGet().Get(filter);
        ...
        internal bool IsUnLock(List<HIS_USER_ROOM> listData)
        {
            bool valid = true;
            try
            {
