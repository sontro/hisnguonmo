...
{
    partial class HisMrCheckItemLock : BusinessBase
    {
        internal HisMrCheckItemLock()
            : base()
        {

        }

        internal HisMrCheckItemLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_MR_CHECK_ITEM resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_MR_CHECK_ITEM data = new HisMrCheckItemGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisMrCheckItemDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_MR_CHECK_ITEM resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_MR_CHECK_ITEM resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisMrCheckItemCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisMrCheckItemDAO.Update(raw);
