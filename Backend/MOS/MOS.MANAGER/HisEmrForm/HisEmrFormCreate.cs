...
namespace MOS.MANAGER.HisEmrForm
{
    partial class HisEmrFormCreate : BusinessBase
    {
		...
		internal bool CreateList(List<HIS_EMR_FORM> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    if (!DAOWorker.HisEmrFormDAO.CreateList(listData))
                    {
                        MOS.MANAGER.Base.BugUtil.SetBugCode(param, MOS.LibraryBug.Bug.Enum.HisEmrForm_ThemMoiThatBai);
                        throw new Exception("Them moi thong tin HisEmrForm that bai." + LogUtil.TraceData("listData", listData));
                    }
                    this.recentHisEmrForms.AddRange(listData);
                    result = true;
                }
            }
            catch (Exception ex)
