...
{
    partial class HisEmrFormLock : BusinessBase
    {
        internal HisEmrFormLock()
            : base()
        {

        }

        internal HisEmrFormLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_EMR_FORM resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_EMR_FORM data = new HisEmrFormGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisEmrFormDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_EMR_FORM resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_EMR_FORM resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisEmrFormCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisEmrFormDAO.Update(raw);
