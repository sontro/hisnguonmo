...
{
    class HisCashierRoomLock : BusinessBase
    {
        internal HisCashierRoomLock()
            : base()
        {

        }

        internal HisCashierRoomLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_CASHIER_ROOM data)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisCashierRoomCheck().VerifyId(data.ID, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisCashierRoomDAO.Update(raw);
                    if (result)
                    {
