...
{
    partial class HisReportTypeCatCheck : BusinessBase
    {
        internal HisReportTypeCatCheck()
            : base()
        {

        }

        internal HisReportTypeCatCheck(CommonParam paramCheck)
            : base(paramCheck) 
        {

        }

        internal bool VerifyRequireField(HIS_REPORT_TYPE_CAT data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisReportTypeCatGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_REPORT_TYPE_CAT data)
        {
            bool valid = true;
            try
            {
                data = new HisReportTypeCatGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                if (listId != null && listId.Count > 0)
                {
                    ...
                    filter.IDs = listId;
                    List<HIS_REPORT_TYPE_CAT> listData = new HisReportTypeCatGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_REPORT_TYPE_CAT> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_REPORT_TYPE_CAT data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisReportTypeCatDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_REPORT_TYPE_CAT> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsNoExisted(HIS_REPORT_TYPE_CAT data)
        {
            bool valid = true;
            try
            {
                ...
                {
                    HisReportTypeCatFilterQuery filter = new HisReportTypeCatFilterQuery();
                    filter.REPORT_TYPE_CODE__EXACT = data.REPORT_TYPE_CODE;
                    filter.CATEGORY_CODE__EXACT = data.CATEGORY_CODE;
                    List<HIS_REPORT_TYPE_CAT> exists = new HisReportTypeCatGet().Get(filter);
                    List<HIS_REPORT_TYPE_CAT> others = IsNotNullOrEmpty(exists) ? exists.Where(o => o.ID != data.ID).ToList() : null;
