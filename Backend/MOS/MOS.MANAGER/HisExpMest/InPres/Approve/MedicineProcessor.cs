...
namespace MOS.MANAGER.HisExpMest.InPres.Approve
{
    class MedicineProcessor : BusinessBase
    {
        ...
        internal bool Run(HIS_EXP_MEST expMest, string loginname, string username, long? approvalTime, ref List<string> sqls)
        {
            try
            {
                string sql = string.Format("UPDATE HIS_EXP_MEST_MEDICINE SET APPROVAL_TIME = {0}, APPROVAL_LOGINNAME = '{1}', APPROVAL_USERNAME ='{2}' WHERE EXP_MEST_ID = {3} ", approvalTime, loginname, username, expMest.ID);
                sqls.Add(sql);
                return true;
            }
            catch (Exception ex)
