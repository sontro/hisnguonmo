...
namespace MOS.MANAGER.HisExpMest.Other.Create
{
    class BloodProcessor : BusinessBase
    {
        ...
        internal bool Run(List<ExpBloodSDO> bloods, HIS_EXP_MEST expMest, ref List<HIS_EXP_MEST_BLOOD> resultData, ref List<string> sqls)
        {
            try
            {
                if (IsNotNullOrEmpty(bloods) && expMest != null)
                {
                    ...
                    {
                        foreach (var expMestBlood in data)
                        {
                            string sql = string.Format("UPDATE HIS_BLOOD SET LAST_EXP_MEST_BLOOD_ID = {0} WHERE ID = {1}", expMestBlood.ID, expMestBlood.BLOOD_ID);
                            sqls.Add(sql);
                        }
                    }
                    resultData = data;
