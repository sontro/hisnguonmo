...
namespace MOS.MANAGER.HisExpMest.Other.Update
{
    class BloodProcessor : BusinessBase
    {
        ...
        internal bool Run(HisExpMestOtherSDO sdo, HIS_EXP_MEST expMest, ref List<HIS_EXP_MEST_BLOOD> resultData, ref List<string> sqls)
        {
            try
            {
                if (expMest != null)
                {
                    ...
                    List<long> bloodIdOfDeleteExpMestBloods = IsNotNullOrEmpty(deletes) ? deletes.Select(o => o.BLOOD_ID).ToList() : null;
                    //Cap nhat thong tin bean
                    this.SqlUpdateBlood(inserts, bloodIdOfDeleteExpMestBloods, ref sqls);

                    //Xoa cac exp_mest_blood ko dung.
                    //Luu y: can thuc hien xoa exp_mest_blood sau khi da cap nhat bean (tranh bi loi fk)
                    this.SqlDeleteExpMestBlood(deleteIds, ref sqls);
        ...
        private void SqlUpdateBlood(List<HIS_EXP_MEST_BLOOD> newExpMestBloods, List<long> bloodIdOfDeleteExpMestBloods, ref List<string> sqls)
        {
            if (IsNotNullOrEmpty(bloodIdOfDeleteExpMestBloods))
            {
                //cap nhat danh sach cac bean ko dung
                string query2 = DAOWorker.SqlDAO.AddInClause(bloodIdOfDeleteExpMestBloods, "UPDATE HIS_BLOOD SET IS_ACTIVE = 1 WHERE %IN_CLAUSE% ", "ID");
                sqls.Add(query2);
            }
        ...
        private List<HIS_EXP_MEST_BLOOD> MakeData(HisExpMestOtherSDO sdo, HIS_EXP_MEST expMest, List<HIS_EXP_MEST_BLOOD> oldExpMestBloods)
        {
