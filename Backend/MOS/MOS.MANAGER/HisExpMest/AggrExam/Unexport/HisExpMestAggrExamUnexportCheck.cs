...
{
    partial class HisExpMestAggrExamUnexportCheck : BusinessBase
    {
        internal HisExpMestAggrExamUnexportCheck()
            : base()
        {

        }

        internal HisExpMestAggrExamUnexportCheck(CommonParam paramCreate)
            : base(paramCreate)
        {
        ...
        internal bool IsAllowed(HisExpMestSDO data, ref HIS_EXP_MEST aggrExpMest, ref List<HIS_EXP_MEST> children)
        {
            try
            {
                ...
                if (tmpAggr == null || tmpAggr.EXP_MEST_TYPE_ID != IMSys.DbConfig.HIS_RS.HIS_EXP_MEST_TYPE.ID__THPK)
                {
                    ...
                    LogSystem.Warn("exp_mest_id ko hop le hoac loai ko phai la tong hop kham");
                    return false;
                }
        ...
        internal bool UnlockTreatment(long aggrExpMestId, ref List<HIS_EXP_MEST> children, ref List<HIS_TREATMENT> treatments)
        {
            try
            {
                children = new HisExpMestGet().GetByAggrExpMestId(aggrExpMestId);

                treatments = new List<HIS_TREATMENT>();

                List<long> treatmentIds = children
