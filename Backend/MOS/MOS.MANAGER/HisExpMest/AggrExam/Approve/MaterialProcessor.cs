...
namespace MOS.MANAGER.HisExpMest.AggrExam.Approve
{
    class MaterialProcessor : BusinessBase
    {
        ...
        internal bool Run(HIS_EXP_MEST aggrExpMest, string loginname, string username, long? approvalTime, ref List<string> sqls)
        {
            try
            {
                string sql = string.Format("UPDATE HIS_EXP_MEST_MATERIAL SET APPROVAL_TIME = {0}, APPROVAL_LOGINNAME = '{1}', APPROVAL_USERNAME ='{2}' WHERE TDL_AGGR_EXP_MEST_ID = {3} ", approvalTime, loginname, username, aggrExpMest.ID);
                sqls.Add(sql);
                return true;
            }
            catch (Exception ex)
