...
namespace MOS.MANAGER.HisExpMest.AggrExam.Delete
{
    /// <summary>
    /// Xóa phiếu tong hop:
    ...
    /// </summary>
    class HisExpMestAggrExamTruncate : BusinessBase
    {
        private HisExpMestUpdate hisExpMestUpdate;

        internal HisExpMestAggrExamTruncate()
            : base()
        {
            this.hisExpMestUpdate = new HisExpMestUpdate(param);
        }

        internal HisExpMestAggrExamTruncate(CommonParam paramTruncate)
            : base(paramTruncate)
        {
            this.hisExpMestUpdate = new HisExpMestUpdate(param);
        }
        ...
        internal bool Truncate(long id)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    if (raw.EXP_MEST_TYPE_ID != IMSys.DbConfig.HIS_RS.HIS_EXP_MEST_TYPE.ID__THPK)
                    {
                        BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.DuLieuDauVaoKhongHopLe);
                        throw new Exception("Chi co phieu tong hop kham moi duoc thuc hien chuc nang nay" + LogUtil.TraceData("expMest", raw));
                    }
