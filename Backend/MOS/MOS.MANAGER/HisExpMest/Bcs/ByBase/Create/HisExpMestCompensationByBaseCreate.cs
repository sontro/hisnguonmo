...
{
    class HisExpMestCompensationByBaseCreate : BusinessBase
    {
        ...
        internal HisExpMestCompensationByBaseCreate()
            : base()
        {
            this.Init();
        }

        internal HisExpMestCompensationByBaseCreate(CommonParam param)
            : base(param)
        {
            this.Init();
        }
        ...
        internal bool Run(CabinetBaseCompensationSDO data, ref  List<HIS_EXP_MEST> resultData)
        {
            ...
            try
            {
                ...
                valid = valid && stockChecker.IsCabinetManageOptionBase(cabinetStock);
                ...
                valid = valid && checker.IsHasNotBaseExpMest(data);
                valid = valid && checker.IsHasNotBaseImpMest(data);
                valid = valid && checker.ValidData(data, ref stockMetys, ref stockMatys);
                valid = valid && checker.VerifyData(data, stockMetys, stockMatys);
                ...
                if (valid)
                {
                    ...
                    if (!this.materialProcessor.Run(dicExpMest))
                    {
                        throw new Exception("materialProcessor. Ket thuc nghiep vu");
                    }

                    if (!this.medicineProcessor.Run(dicExpMest))
                    {
        ...
        private void PrepareData(CabinetBaseCompensationSDO data, List<HIS_MEDI_STOCK_METY> stockMetys, List<HIS_MEDI_STOCK_MATY> stockMatys, WorkPlaceSDO workPlace, ref Dictionary<HIS_EXP_MEST, ExpMestDetail> dicExpMest)
        {
            ...
            foreach (var d in dic)
            {
                ...
                exp.BCS_TYPE_ID = IMSys.DbConfig.HIS_RS.HIS_EXP_MEST.BCS_TYPE__ID__BASE;
