...
namespace MOS.MANAGER.HisExpMest.Aggr.Export
{
    class ChildProcessor : BusinessBase
    {
        ...
        private void UnfinishServiceReq(List<long> serviceReqIds)
        {
            if (IsNotNullOrEmpty(serviceReqIds))
            {
                string query = DAOWorker.SqlDAO.AddInClause(serviceReqIds, "UPDATE HIS_SERVICE_REQ SET SERVICE_REQ_STT_ID = :param1, FINISH_TIME = NULL, EXECUTE_LOGINNAME = NULL, EXECUTE_USERNAME = NULL, EXECUTE_USER_TITLE = NULL WHERE %IN_CLAUSE% ", "ID");
