...
namespace MOS.MANAGER.HisExpMest.Aggr.Unapprove
{
    class ChildrenProcessor : BusinessBase
    {
        ...
        internal bool Run(long parentId, ref List<string> sqls)
        {
            try
            {
                //Cap nhat trang thai cua service-req tuong ung voi cac don noi tru
                ...
                sqls.Add(string.Format(sqlServiceReq, IMSys.DbConfig.HIS_RS.HIS_SERVICE_REQ_STT.ID__CXL, parentId, IMSys.DbConfig.HIS_RS.HIS_EXP_MEST_TYPE.ID__DDT));

                //Cap nhat trang thai cua cac don noi tru
                string sqlExpMest = "UPDATE HIS_EXP_MEST SET EXP_MEST_STT_ID = {0}, "
                                    + " LAST_APPROVAL_TIME = NULL, LAST_APPROVAL_LOGINNAME = NULL, LAST_APPROVAL_USERNAME = NULL, LAST_APPROVAL_DATE = NULL  "
                                    + " WHERE AGGR_EXP_MEST_ID = {1} AND EXP_MEST_TYPE_ID = {2} ";

                sqls.Add(string.Format(sqlExpMest, IMSys.DbConfig.HIS_RS.HIS_EXP_MEST_STT.ID__REQUEST, parentId, IMSys.DbConfig.HIS_RS.HIS_EXP_MEST_TYPE.ID__DDT));
