...
{
    partial class HisBhytBlacklistLock : BusinessBase
    {
        internal HisBhytBlacklistLock()
            : base()
        {

        }

        internal HisBhytBlacklistLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_BHYT_BLACKLIST resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_BHYT_BLACKLIST data = new HisBhytBlacklistGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisBhytBlacklistDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_BHYT_BLACKLIST resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_BHYT_BLACKLIST resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisBhytBlacklistCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisBhytBlacklistDAO.Update(raw);
