...
{
    partial class HisAllergyCardLock : BusinessBase
    {
        internal HisAllergyCardLock()
            : base()
        {

        }

        internal HisAllergyCardLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_ALLERGY_CARD resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_ALLERGY_CARD data = new HisAllergyCardGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisAllergyCardDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_ALLERGY_CARD resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_ALLERGY_CARD resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisAllergyCardCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisAllergyCardDAO.Update(raw);
