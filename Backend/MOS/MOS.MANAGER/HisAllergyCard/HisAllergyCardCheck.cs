...
{
    partial class HisAllergyCardCheck : BusinessBase
    {
        internal HisAllergyCardCheck()
            : base()
        {

        }

        internal HisAllergyCardCheck(CommonParam paramCheck)
            : base(paramCheck)
        {

        }

        internal bool VerifyRequireField(HIS_ALLERGY_CARD data)
        {
            bool valid = true;
            try
            {
                ...
                if (data.ISSUE_TIME <= 0) throw new ArgumentNullException("data.ISSUE_TIME");
                if (String.IsNullOrWhiteSpace(data.DIAGNOSE_LOGINNAME)) throw new ArgumentNullException("data.DIAGNOSE_LOGINNAME");
            }
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisAllergyCardGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_ALLERGY_CARD data)
        {
            bool valid = true;
            try
            {
                data = new HisAllergyCardGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                ...
                {
                    HisAllergyCardFilterQuery filter = new HisAllergyCardFilterQuery();
                    filter.IDs = listId;
                    List<HIS_ALLERGY_CARD> listData = new HisAllergyCardGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_ALLERGY_CARD> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_ALLERGY_CARD data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisAllergyCardDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_ALLERGY_CARD> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool CheckConstraint(long id)
        {
            ...
            try
            {
                /*List<XXX> hisXXXs = new HisXXXGet().GetByHisAllergyCardId(id);
        ...
        internal bool AllowUpdateOrDeleteLoginname(HIS_ALLERGY_CARD raw, ref List<HIS_ALLERGENIC> allergenics)
        {
            bool valid = true;
            try
            {
                string loginname = Inventec.Token.ResourceSystem.ResourceTokenManager.GetLoginName();
                if (raw != null && (raw.CREATOR != loginname && raw.DIAGNOSE_LOGINNAME != loginname))
                {
                    MANAGER.Base.MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDoNguoiKhacTao);
                    return false;
                }
                allergenics = new HisAllergenicGet().GetByAllergyCardId(raw.ID);
            }
            catch (Exception ex)
            {
