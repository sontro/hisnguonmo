...
{
    partial class HisCoTreatmentLock : BusinessBase
    {
        internal HisCoTreatmentLock()
            : base()
        {

        }

        internal HisCoTreatmentLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_CO_TREATMENT resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_CO_TREATMENT data = new HisCoTreatmentGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisCoTreatmentDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_CO_TREATMENT resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_CO_TREATMENT resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisCoTreatmentCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisCoTreatmentDAO.Update(raw);
