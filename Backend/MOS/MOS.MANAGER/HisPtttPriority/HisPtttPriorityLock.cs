...
{
    partial class HisPtttPriorityLock : BusinessBase
    {
        internal HisPtttPriorityLock()
            : base()
        {

        }

        internal HisPtttPriorityLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_PTTT_PRIORITY resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_PTTT_PRIORITY data = new HisPtttPriorityGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisPtttPriorityDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_PTTT_PRIORITY resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_PTTT_PRIORITY resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisPtttPriorityCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisPtttPriorityDAO.Update(raw);
