...
{
    partial class HisVareVartLock : BusinessBase
    {
        internal HisVareVartLock()
            : base()
        {

        }

        internal HisVareVartLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_VARE_VART resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_VARE_VART data = new HisVareVartGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisVareVartDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_VARE_VART resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_VARE_VART resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisVareVartCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisVareVartDAO.Update(raw);
