...
{
    partial class HisPtttMethodLock : BusinessBase
    {
        internal HisPtttMethodLock()
            : base()
        {

        }

        internal HisPtttMethodLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_PTTT_METHOD data)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisPtttMethodCheck().VerifyId(data.ID, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisPtttMethodDAO.Update(raw);
