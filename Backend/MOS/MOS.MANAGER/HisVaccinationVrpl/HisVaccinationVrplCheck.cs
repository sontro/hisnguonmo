...
{
    partial class HisVaccinationVrplCheck : BusinessBase
    {
        internal HisVaccinationVrplCheck()
            : base()
        {

        }

        internal HisVaccinationVrplCheck(CommonParam paramCheck)
            : base(paramCheck) 
        {

        }

        internal bool VerifyRequireField(HIS_VACCINATION_VRPL data)
        {
            bool valid = true;
            try
            {
                if (data == null) throw new ArgumentNullException("data");
                if (data.VACC_REACT_PLACE_ID <= 0) throw new ArgumentNullException("data.VACC_REACT_PLACE_ID");
                if (data.VACCINATION_ID <= 0) throw new ArgumentNullException("data.VACCINATION_ID");
            }
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisVaccinationVrplGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_VACCINATION_VRPL data)
        {
            bool valid = true;
            try
            {
                data = new HisVaccinationVrplGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                if (listId != null && listId.Count > 0)
                {
                    ...
                    filter.IDs = listId;
                    List<HIS_VACCINATION_VRPL> listData = new HisVaccinationVrplGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_VACCINATION_VRPL> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_VACCINATION_VRPL data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisVaccinationVrplDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_VACCINATION_VRPL> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool CheckConstraint(long id)
        {
            ...
            try
            {
                /*List<XXX> hisXXXs = new HisXXXGet().GetByHisVaccinationVrplId(id);
