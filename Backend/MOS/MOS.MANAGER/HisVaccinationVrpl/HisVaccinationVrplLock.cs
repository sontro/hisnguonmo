...
{
    partial class HisVaccinationVrplLock : BusinessBase
    {
        internal HisVaccinationVrplLock()
            : base()
        {

        }

        internal HisVaccinationVrplLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_VACCINATION_VRPL resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_VACCINATION_VRPL data = new HisVaccinationVrplGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisVaccinationVrplDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_VACCINATION_VRPL resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_VACCINATION_VRPL resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisVaccinationVrplCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisVaccinationVrplDAO.Update(raw);
