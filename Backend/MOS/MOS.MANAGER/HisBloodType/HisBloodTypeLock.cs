...
{
    partial class HisBloodTypeLock : BusinessBase
    {
        internal HisBloodTypeLock()
            : base()
        {

        }

        internal HisBloodTypeLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_BLOOD_TYPE resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_BLOOD_TYPE data = new HisBloodTypeGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisBloodTypeDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_BLOOD_TYPE resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_BLOOD_TYPE resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisBloodTypeCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisBloodTypeDAO.Update(raw);
