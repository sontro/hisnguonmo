...
{
    partial class HisHoreHandoverLock : BusinessBase
    {
        internal HisHoreHandoverLock()
            : base()
        {

        }

        internal HisHoreHandoverLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_HORE_HANDOVER resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_HORE_HANDOVER data = new HisHoreHandoverGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisHoreHandoverDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_HORE_HANDOVER resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_HORE_HANDOVER resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisHoreHandoverCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisHoreHandoverDAO.Update(raw);
