...
{
    class HisDiseaseRelationLock : BusinessBase
    {
        internal HisDiseaseRelationLock()
            : base()
        {

        }

        internal HisDiseaseRelationLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_DISEASE_RELATION data)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    data = new HisDiseaseRelationGet().GetById(data.ID);
                    if (data != null)
                    {
