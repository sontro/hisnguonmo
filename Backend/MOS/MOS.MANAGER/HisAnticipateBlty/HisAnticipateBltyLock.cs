...
{
    partial class HisAnticipateBltyLock : BusinessBase
    {
        internal HisAnticipateBltyLock()
            : base()
        {

        }

        internal HisAnticipateBltyLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_ANTICIPATE_BLTY resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_ANTICIPATE_BLTY data = new HisAnticipateBltyGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisAnticipateBltyDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_ANTICIPATE_BLTY resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_ANTICIPATE_BLTY resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisAnticipateBltyCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisAnticipateBltyDAO.Update(raw);
