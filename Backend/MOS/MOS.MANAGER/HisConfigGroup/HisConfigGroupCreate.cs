...
namespace MOS.MANAGER.HisConfigGroup
{
    partial class HisConfigGroupCreate : BusinessBase
    {
		...
		internal bool CreateList(List<HIS_CONFIG_GROUP> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    if (!DAOWorker.HisConfigGroupDAO.CreateList(listData))
                    {
                        MOS.MANAGER.Base.BugUtil.SetBugCode(param, MOS.LibraryBug.Bug.Enum.HisConfigGroup_ThemMoiThatBai);
                        throw new Exception("Them moi thong tin HisConfigGroup that bai." + LogUtil.TraceData("listData", listData));
                    }
                    this.recentHisConfigGroups.AddRange(listData);
                    result = true;
                }
            }
            catch (Exception ex)
