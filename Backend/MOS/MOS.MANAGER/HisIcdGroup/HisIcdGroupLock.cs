...
{
    class HisIcdGroupLock : BusinessBase
    {
        internal HisIcdGroupLock()
            : base()
        {

        }

        internal HisIcdGroupLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_ICD_GROUP data)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    data = new HisIcdGroupGet().GetById(data.ID);
                    if (data != null)
                    {
