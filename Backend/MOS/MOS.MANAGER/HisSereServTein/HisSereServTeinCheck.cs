...
{
    class HisSereServTeinCheck : BusinessBase
    {
        internal HisSereServTeinCheck()
            : base()
        {

        }

        internal HisSereServTeinCheck(Inventec.Core.CommonParam paramCheck)
            : base(paramCheck) 
        {

        }

        internal bool VerifyRequireField(HIS_SERE_SERV_TEIN data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(HIS_SERE_SERV_TEIN data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisSereServTeinDAO.IsUnLock(id))
                {
                    valid = false;
        ...
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_SERE_SERV_TEIN> listObject)
        {
            bool valid = true;
            try
            {
                if (listId != null && listId.Count > 0)
                {
                    ...
                    filter.IDs = listId;
                    List<HIS_SERE_SERV_TEIN> listData = new HisSereServTeinGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_SERE_SERV_TEIN> listData)
        {
            bool valid = true;
            try
            {
