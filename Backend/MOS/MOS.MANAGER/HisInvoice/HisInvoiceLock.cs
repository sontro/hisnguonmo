...
{
    partial class HisInvoiceLock : BusinessBase
    {
        internal HisInvoiceLock()
            : base()
        {

        }

        internal HisInvoiceLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_INVOICE data)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisInvoiceCheck().VerifyId(data.ID, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisInvoiceDAO.Update(raw);
