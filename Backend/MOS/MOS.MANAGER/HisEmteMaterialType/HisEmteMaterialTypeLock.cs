...
{
    class HisEmteMaterialTypeLock : BusinessBase
    {
        internal HisEmteMaterialTypeLock()
            : base()
        {

        }

        internal HisEmteMaterialTypeLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_EMTE_MATERIAL_TYPE data)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    data = new HisEmteMaterialTypeGet().GetById(data.ID);
                    if (data != null)
                    {
