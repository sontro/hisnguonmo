...
namespace MOS.MANAGER.HisRehaTrain
{
    ...
    /// </summary>
    class HisRehaTrainCheck : BusinessBase
    {
        internal HisRehaTrainCheck()
            : base()
        {

        }

        internal HisRehaTrainCheck(CommonParam paramCheck)
            : base(paramCheck)
        {

        }

        internal bool VerifyRequireField(HIS_REHA_TRAIN data)
        {
            bool valid = true;
            try
            {
                if (data == null) throw new ArgumentNullException("data");
                if (!IsGreaterThanZero(data.SERE_SERV_REHA_ID)) throw new ArgumentNullException("data.SERE_SERV_REHA_ID");
                if (!IsGreaterThanZero(data.TRAIN_TIME)) throw new ArgumentNullException("data.TRAIN_TIME");
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisRehaTrainGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_REHA_TRAIN data)
        {
            bool valid = true;
            try
            {
                data = new HisRehaTrainGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                if (listId != null && listId.Count > 0)
                {
                    ...
                    filter.IDs = listId;
                    List<HIS_REHA_TRAIN> listData = new HisRehaTrainGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_REHA_TRAIN> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_REHA_TRAIN data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisRehaTrainDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_REHA_TRAIN> listData)
        {
            bool valid = true;
            try
            {
