...
{
    partial class HisTreatmentLoggingLock : BusinessBase
    {
        internal HisTreatmentLoggingLock()
            : base()
        {

        }

        internal HisTreatmentLoggingLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_TREATMENT_LOGGING resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_TREATMENT_LOGGING data = new HisTreatmentLoggingGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisTreatmentLoggingDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_TREATMENT_LOGGING resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_TREATMENT_LOGGING resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisTreatmentLoggingCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisTreatmentLoggingDAO.Update(raw);
