...
{
    partial class HisMedicineServiceCheck : BusinessBase
    {
        internal HisMedicineServiceCheck()
            : base()
        {

        }

        internal HisMedicineServiceCheck(CommonParam paramCheck)
            : base(paramCheck) 
        {

        }

        internal bool VerifyRequireField(HIS_MEDICINE_SERVICE data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisMedicineServiceGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_MEDICINE_SERVICE data)
        {
            bool valid = true;
            try
            {
                data = new HisMedicineServiceGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                if (listId != null && listId.Count > 0)
                {
                    ...
                    filter.IDs = listId;
                    List<HIS_MEDICINE_SERVICE> listData = new HisMedicineServiceGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_MEDICINE_SERVICE> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_MEDICINE_SERVICE data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisMedicineServiceDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_MEDICINE_SERVICE> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool CheckConstraint(long id)
        {
            ...
            try
            {
                /*List<XXX> hisXXXs = new HisXXXGet().GetByHisMedicineServiceId(id);
        ...
        internal bool IsValidDataTypeAndTestIndexId(List<HIS_MEDICINE_SERVICE> listData)
        {
            bool valid = true;
            try
            {
                ...
                {
                    long? testIndexId = listData.FirstOrDefault().TEST_INDEX_ID;
                    List<HIS_MEDICINE_SERVICE> medicineServices = listData.Where(o => o.DATA_TYPE != IMSys.DbConfig.HIS_RS.HIS_MEDICINE_SERVICE.DATA_TYPE__SERVICE && o.TEST_INDEX_ID != testIndexId).ToList();
                    if (IsNotNullOrEmpty(medicineServices))
                    {
