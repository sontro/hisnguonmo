...
{
    partial class HisMedicineServiceLock : BusinessBase
    {
        internal HisMedicineServiceLock()
            : base()
        {

        }

        internal HisMedicineServiceLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_MEDICINE_SERVICE resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_MEDICINE_SERVICE data = new HisMedicineServiceGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisMedicineServiceDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_MEDICINE_SERVICE resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_MEDICINE_SERVICE resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisMedicineServiceCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisMedicineServiceDAO.Update(raw);
