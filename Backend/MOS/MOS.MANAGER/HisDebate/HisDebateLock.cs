...
{
    partial class HisDebateLock : BusinessBase
    {
        internal HisDebateLock()
            : base()
        {

        }

        internal HisDebateLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_DEBATE data)
        {
            bool result = false;
            try
            {
                bool valid = true;
                HIS_DEBATE raw = null;
                valid = valid && new HisDebateCheck().VerifyId(data.ID, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisDebateDAO.Update(raw);
