...
{
    partial class HisServiceRereTimeLock : BusinessBase
    {
        internal HisServiceRereTimeLock()
            : base()
        {

        }

        internal HisServiceRereTimeLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_SERVICE_RERE_TIME resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_SERVICE_RERE_TIME data = new HisServiceRereTimeGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisServiceRereTimeDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_SERVICE_RERE_TIME resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_SERVICE_RERE_TIME resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisServiceRereTimeCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisServiceRereTimeDAO.Update(raw);
