...
{
    partial class HisImpUserTempLock : BusinessBase
    {
        internal HisImpUserTempLock()
            : base()
        {

        }

        internal HisImpUserTempLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_IMP_USER_TEMP resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_IMP_USER_TEMP data = new HisImpUserTempGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisImpUserTempDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_IMP_USER_TEMP resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_IMP_USER_TEMP resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisImpUserTempCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisImpUserTempDAO.Update(raw);
