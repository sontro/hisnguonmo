...
namespace MOS.MANAGER.HisImpUserTemp
{
    partial class HisImpUserTempCreate : BusinessBase
    {
		...
		internal bool CreateList(List<HIS_IMP_USER_TEMP> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    if (!DAOWorker.HisImpUserTempDAO.CreateList(listData))
                    {
                        MOS.MANAGER.Base.BugUtil.SetBugCode(param, MOS.LibraryBug.Bug.Enum.HisImpUserTemp_ThemMoiThatBai);
                        throw new Exception("Them moi thong tin HisImpUserTemp that bai." + LogUtil.TraceData("listData", listData));
                    }
                    this.recentHisImpUserTemps.AddRange(listData);
                    result = true;
                }
            }
            catch (Exception ex)
