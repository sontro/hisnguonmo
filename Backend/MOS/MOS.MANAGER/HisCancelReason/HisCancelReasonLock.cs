...
{
    partial class HisCancelReasonLock : BusinessBase
    {
        internal HisCancelReasonLock()
            : base()
        {

        }

        internal HisCancelReasonLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_CANCEL_REASON resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_CANCEL_REASON data = new HisCancelReasonGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisCancelReasonDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_CANCEL_REASON resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_CANCEL_REASON resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisCancelReasonCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisCancelReasonDAO.Update(raw);
