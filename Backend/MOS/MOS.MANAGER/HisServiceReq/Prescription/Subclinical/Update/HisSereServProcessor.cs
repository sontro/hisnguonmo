...
namespace MOS.MANAGER.HisServiceReq.Prescription.Subclinical.Update
{
    class HisSereServProcessor : BusinessBase
    {
        ...
        internal bool Run(HIS_TREATMENT treatment, List<HIS_PATIENT_TYPE_ALTER> ptas, HIS_SERVICE_REQ serviceReq, HIS_EXP_MEST expMest, List<HIS_EXP_MEST_MEDICINE> insertMedicines, List<HIS_EXP_MEST_MEDICINE> deleteMedicines, List<HIS_EXP_MEST_MATERIAL> insertMaterials, List<HIS_EXP_MEST_MATERIAL> deleteMaterials, List<HIS_SERE_SERV> existSereServs, ref List<HIS_SERE_SERV> resultData)
        {
            try
            {
                if (IsNotNullOrEmpty(insertMedicines) || IsNotNullOrEmpty(deleteMedicines) || IsNotNullOrEmpty(insertMaterials) || IsNotNullOrEmpty(deleteMaterials))
                {
                    ...
                    this.DataToInsert(treatment, priceAdder, serviceReq, expMest, insertMedicines, ref insertData);
                ...
                if (IsNotNullOrEmpty(deleteMedicines))
                {
                    foreach (HIS_EXP_MEST_MEDICINE medicine in deleteMedicines)
                    {
                        ...
                        else if (exists.Exists(e => e.MEDICINE_ID.HasValue && !e.EXP_MEST_MEDICINE_ID.HasValue))
                        {
                            ...
                            && t.PARENT_ID == medicine.SERE_SERV_PARENT_ID
        ...
        private void DataToInsert(HIS_TREATMENT treatment, HisSereServSetPrice priceAdder, HIS_SERVICE_REQ serviceReq, HIS_EXP_MEST expMest, List<HIS_EXP_MEST_MEDICINE> insertMedicines, ref List<HIS_SERE_SERV> sereServs)
        {
            if (IsNotNullOrEmpty(insertMedicines))
            {
                List<long> medicineIds = insertMedicines.Select(o => o.MEDICINE_ID.Value).ToList();
