...
namespace MOS.MANAGER.HisServiceReq.Prescription.Subclinical.Create
{
    class HisSereServMaker : BusinessBase
    {
        ...
        private SubclinicalPresSDO prescription;
        private HIS_TREATMENT treatment;

        internal HisSereServMaker(CommonParam paramCreate, HIS_TREATMENT t, SubclinicalPresSDO pres, List<HIS_SERVICE_REQ> serviceReqs, List<HIS_EXP_MEST> exps, List<HIS_EXP_MEST_MEDICINE> medicines, List<HIS_EXP_MEST_MATERIAL> materials)
            : base(paramCreate)
        {
            this.expMests = exps;
        ...
        private void MakeDataByMedicine(HisSereServSetPrice priceAdder, ref List<HIS_SERE_SERV> sereServs)
        {
            ...
            {
                List<long> medicineIds = this.expMestMedicines.Select(o => o.MEDICINE_ID.Value).ToList();
