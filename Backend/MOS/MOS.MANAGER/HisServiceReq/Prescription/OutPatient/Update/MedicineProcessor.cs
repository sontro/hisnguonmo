...
namespace MOS.MANAGER.HisServiceReq.Prescription.OutPatient.Update
{
    class MedicineProcessor : BusinessBase
    {
        ...
        /// </summary>
        /// <returns></returns>
        internal bool Run(OutPatientPresSDO sdo, HisSereServPackage37 processPackage37, HisSereServPackageBirth processPackageBirth, HisSereServPackagePttm processPackagePttm, HIS_EXP_MEST expMest, ref List<HIS_EXP_MEST_MEDICINE> inserts, ref List<HIS_EXP_MEST_MEDICINE> deletes, ref List<HIS_EXP_MEST_MEDICINE> resultData, ref List<V_HIS_MEDICINE_2> newsMedicines, HIS_TREATMENT treatment, ref List<string> sqls)
        {
            try
            {
                if (expMest != null)
                {
                    ...
                    List<HIS_EXP_MEST_MEDICINE> news = this.MakeData(sdo, expMest, olds, sessionKey, ref medicineDic, ref medicinePaties, treatment);
                ...
                foreach (HIS_EXP_MEST_MEDICINE newMedicine in news)
                {
                    HIS_EXP_MEST_MEDICINE old = olds
                        .Where(t => !IsDiff(newMedicine, t, newMedicineDic, oldBeans)
                            ...
                            && t.SERVICE_CONDITION_ID == newMedicine.SERVICE_CONDITION_ID
                    ...
                    else if (old.TUTORIAL != newMedicine.TUTORIAL || old.NUM_ORDER != newMedicine.NUM_ORDER || old.USE_TIME_TO != newMedicine.USE_TIME_TO
                        ...
                        || old.OVER_RESULT_TEST_REASON != newMedicine.OVER_RESULT_TEST_REASON)
                    {
                        ...
                        old.HTU_ID = newMedicine.HTU_ID;
                        old.BREATH_SPEED = newMedicine.BREATH_SPEED;
                        old.BREATH_TIME = newMedicine.BREATH_TIME;
        ...
        private List<HIS_EXP_MEST_MEDICINE> MakeData(OutPatientPresSDO sdo, HIS_EXP_MEST expMest, List<HIS_EXP_MEST_MEDICINE> olds, string sessionKey, ref Dictionary<HIS_EXP_MEST_MEDICINE, List<long>> medicineDic, ref List<HIS_MEDICINE_PATY> medicinePaties, HIS_TREATMENT treatment)
        {
            if (IsNotNullOrEmpty(sdo.Medicines) && expMest != null)
            {
                ...
                foreach (PresMedicineSDO s in sdo.Medicines)
                {
                    ...
                    foreach (var medicine in groupMedicines)
                    {
                        ...
                        exp.SERVICE_CONDITION_ID = s.ServiceConditionId;
                        exp.OTHER_PAY_SOURCE_ID = s.OtherPaySourceId;
                        ...
                        exp.TDL_AGGR_EXP_MEST_ID = expMest.AGGR_EXP_MEST_ID; //de tranh truong hop don phong kham da duoc tong hop
                        ...
                        HIS_PATIENT_TYPE hisPatientType = HisPatientTypeCFG.DATA.Where(o => o.ID == s.PatientTypeId).FirstOrDefault();
                        if (isVaccine)
                        {
                            ...
                            V_HIS_SERVICE_PATY paty = MOS.ServicePaty.ServicePatyUtil.GetApplied(HisServicePatyCFG.DATA, room.BRANCH_ID, room.ID, expMest.REQ_ROOM_ID, expMest.REQ_DEPARTMENT_ID, expMest.TDL_INTRUCTION_TIME ?? 0, treatment.IN_TIME, medicine.TDL_SERVICE_ID, s.PatientTypeId, null, null, null, s.ServiceConditionId, treatment.TDL_PATIENT_CLASSIFY_ID, null);
                        ...
                        exp.MORNING = s.Morning;
                        exp.NOON = s.Noon;
                        exp.AFTERNOON = s.Afternoon;
                        exp.EVENING = s.Evening;
                        exp.HTU_ID = s.HtuId;
                        exp.BREATH_SPEED = s.BreathSpeed;
                        exp.BREATH_TIME = s.BreathTime;
                        ...
                        exp.MIXED_INFUSION = s.MixedInfusion;
                        exp.IS_MIXED_MAIN = s.IsMixedMain;
                        exp.TUTORIAL_INFUSION = s.TutorialInfusion;
                        exp.PRES_AMOUNT = s.PresAmount;
                        exp.EXCEED_LIMIT_IN_PRES_REASON = s.ExceedLimitInPresReason;
                        exp.EXCEED_LIMIT_IN_DAY_REASON = s.ExceedLimitInDayReason;
                        exp.ODD_PRES_REASON = s.OddPresReason;
                        if (IsNotNullOrEmpty(s.MedicineInfoSdos))
                        {
                            foreach (var item in s.MedicineInfoSdos)
                            {
                                if (!item.IsNoPrescription && item.IntructionTime == sdo.InstructionTime)
                                {
