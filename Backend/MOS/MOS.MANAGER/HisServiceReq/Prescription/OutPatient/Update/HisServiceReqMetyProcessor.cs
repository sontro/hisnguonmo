...
namespace MOS.MANAGER.HisServiceReq.Prescription.OutPatient.Update
{
    class HisServiceReqMetyProcessor: BusinessBase
    {
        ...
        internal bool Run(List<PresOutStockMetySDO> serviceReqMeties, List<PresMedicineSDO> medicines, HIS_SERVICE_REQ serviceReq, long intructionTime, ref List<HIS_SERVICE_REQ_METY> resultData, ref List<string> sqls)
        {
            try
            {
                ...
                List<HIS_SERVICE_REQ_METY> olds = new HisServiceReqMetyGet().GetByServiceReqId(serviceReq.ID);

                //Don ngoai kho
                List<HIS_SERVICE_REQ_METY> oldOutPres = IsNotNullOrEmpty(olds) ? olds.Where(o => o.IS_SUB_PRES != Constant.IS_TRUE).ToList() : null;
                ...
                //Don phu
                List<HIS_SERVICE_REQ_METY> oldSubPres = IsNotNullOrEmpty(olds) ? olds.Where(o => o.IS_SUB_PRES == Constant.IS_TRUE).ToList() : null;
                List<HIS_SERVICE_REQ_METY> newSubPres = new HisServiceReqOutPatientPresUtil().MakeMety(medicines, new List<HIS_SERVICE_REQ>(){serviceReq});
