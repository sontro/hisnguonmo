...
namespace MOS.MANAGER.HisServiceReq.Prescription.OutPatient.Update
{
    class HisExpMestProcessor : BusinessBase
    {
        ...
        internal bool Run(OutPatientPresSDO data, HIS_SERVICE_REQ serviceReq, ref HIS_EXP_MEST expMest, ref List<string> sqls)
        {
            try
            {
                ...
                if (expMest == null && !IsNotNullOrEmpty(materials) && !IsNotNullOrEmpty(medicines))
                {
                    return true;
                }
                else if (expMest != null && !IsNotNullOrEmpty(materials) && !IsNotNullOrEmpty(medicines))
                {
                    if (sqls == null)
                    {
