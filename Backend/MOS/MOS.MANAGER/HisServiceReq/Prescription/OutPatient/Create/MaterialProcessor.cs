...
namespace MOS.MANAGER.HisServiceReq.Prescription.OutPatient.Create
{
    class MaterialProcessor : BusinessBase
    {
        ...
        /// </summary>
        /// <returns></returns>
        internal bool Run(OutPatientPresSDO presSdo, HisSereServPackage37 processPackage37, HisSereServPackageBirth processPackageBirth, HisSereServPackagePttm processPackagePttm, List<HIS_EXP_MEST> expMests, ref List<HIS_EXP_MEST_MATERIAL> resultData, ref List<string> sqls)
        {
            try
            {
                ...
                if (IsNotNullOrEmpty(presSdo.Materials) && IsNotNullOrEmpty(expMests))
                {
                    ...
                    List<long> choosenMaterailIds = data.Select(o => o.MATERIAL_ID.Value).ToList();
