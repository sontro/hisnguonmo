...
{
    partial class HisCashoutLock : BusinessBase
    {
        internal HisCashoutLock()
            : base()
        {

        }

        internal HisCashoutLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_CASHOUT resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_CASHOUT data = new HisCashoutGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisCashoutDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_CASHOUT resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_CASHOUT resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisCashoutCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisCashoutDAO.Update(raw);
