...
{
    partial class HisServiceReqMetyLock : BusinessBase
    {
        internal HisServiceReqMetyLock()
            : base()
        {

        }

        internal HisServiceReqMetyLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_SERVICE_REQ_METY resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_SERVICE_REQ_METY data = new HisServiceReqMetyGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisServiceReqMetyDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_SERVICE_REQ_METY resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_SERVICE_REQ_METY resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisServiceReqMetyCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisServiceReqMetyDAO.Update(raw);
