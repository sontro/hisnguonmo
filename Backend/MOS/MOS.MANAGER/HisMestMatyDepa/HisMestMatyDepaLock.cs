...
{
    partial class HisMestMatyDepaLock : BusinessBase
    {
        internal HisMestMatyDepaLock()
            : base()
        {

        }

        internal HisMestMatyDepaLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_MEST_MATY_DEPA resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_MEST_MATY_DEPA data = new HisMestMatyDepaGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisMestMatyDepaDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_MEST_MATY_DEPA resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_MEST_MATY_DEPA resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisMestMatyDepaCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisMestMatyDepaDAO.Update(raw);
