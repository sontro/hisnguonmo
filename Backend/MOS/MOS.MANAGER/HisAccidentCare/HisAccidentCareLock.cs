...
{
    partial class HisAccidentCareLock : BusinessBase
    {
        internal HisAccidentCareLock()
            : base()
        {

        }

        internal HisAccidentCareLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_ACCIDENT_CARE data)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisAccidentCareCheck().VerifyId(data.ID, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisAccidentCareDAO.Update(raw);
