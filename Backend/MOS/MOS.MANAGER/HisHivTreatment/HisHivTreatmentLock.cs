...
{
    partial class HisHivTreatmentLock : BusinessBase
    {
        internal HisHivTreatmentLock()
            : base()
        {

        }

        internal HisHivTreatmentLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_HIV_TREATMENT resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_HIV_TREATMENT data = new HisHivTreatmentGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisHivTreatmentDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_HIV_TREATMENT resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_HIV_TREATMENT resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisHivTreatmentCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisHivTreatmentDAO.Update(raw);
