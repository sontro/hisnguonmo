...
namespace MOS.MANAGER.HisMedicineLine
{
    ...
    /// </summary>
    partial class HisMedicineLineCheck : BusinessBase
    {
        internal HisMedicineLineCheck()
            : base()
        {

        }

        internal HisMedicineLineCheck(CommonParam paramCheck)
            : base(paramCheck) 
        {

        }

        internal bool VerifyRequireField(HIS_MEDICINE_LINE data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisMedicineLineGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_MEDICINE_LINE data)
        {
            bool valid = true;
            try
            {
                data = new HisMedicineLineGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                if (listId != null && listId.Count > 0)
                {
                    ...
                    filter.IDs = listId;
                    List<HIS_MEDICINE_LINE> listData = new HisMedicineLineGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_MEDICINE_LINE> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_MEDICINE_LINE data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisMedicineLineDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_MEDICINE_LINE> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool ExistsCode(string code, long? id)
        {
            ...
            try
            {
                if (DAOWorker.HisMedicineLineDAO.ExistsCode(code, id))
                {
        ...
        internal bool CheckConstraint(HIS_MEDICINE_LINE data)
        {
            bool valid = true;
            try
            {
                List<HIS_MEDICINE_TYPE> hisMedicineTypes = new HisMedicineTypeGet().GetByMedicineLineId(data.ID);
                if (IsNotNullOrEmpty(hisMedicineTypes))
                {
                    ...
                    throw new Exception("Ton tai du lieu HIS_MEDICINE_TYPE, khong cho phep xoa" + LogUtil.TraceData("data", data));
                }
            }
            catch (Exception ex)
            {
