...
{
    /// <summary>
    /// Nghiep vu tach bean (cho phep chon lo) theo tung session (tinh theo tung lan tao phieu xuat cua nguoi dung) do client yeu cau
    /// </summary>
    class HisMedicineBeanTakeByMedicine : BusinessBase
    {
        private const char OBJECT_SEPARATOR = ';';
        private const char FIELD_SEPARATOR = ':';

        internal HisMedicineBeanTakeByMedicine()
            : base()
        {
        }

        internal HisMedicineBeanTakeByMedicine(CommonParam paramCreate)
            : base(paramCreate)
        {
        }
        ...
        /// <returns></returns>
        internal List<HIS_MEDICINE_BEAN> Take(TakeBeanByMameSDO sdo)
        {
            ...
            try
            {
                if (this.Validate(sdo))
                {
                    ...
                    string storedSql = "PKG_TAKE_MEDICINE_BEAN_BY_MEDI.PRO_EXECUTE";
                    ...
                    OracleParameter medicineTypeIdPar = new OracleParameter("P_MEDICINE_ID", OracleDbType.Int32, sdo.MameId, ParameterDirection.Input);
                    ...
                    if (DAOWorker.SqlDAO.ExecuteStored(OutputHandler, ref resultHolder, storedSql, languageCodePar, messagePar, medicineBeanPar, mediStockIdPar, medicineTypeIdPar, amountPar, sessionKeyPar, orderOptPar, patientTypeIdPar, beanIdsPar, expMestMedicineIdsPar))
                    {
        ...
        /// <returns></returns>
        internal List<TakeMedicineBeanByMedicineListResultSDO> Take(List<TakeBeanByMameSDO> sdos)
        {
            List<TakeMedicineBeanByMedicineListResultSDO> result = null;
            try
            {
                if (IsNotNullOrEmpty(sdos))
                {
                    result = new List<TakeMedicineBeanByMedicineListResultSDO>();
                    foreach (TakeBeanByMameSDO sdo in sdos)
                    {
                        List<HIS_MEDICINE_BEAN> beans = this.Take(sdo);
                        if (beans != null)
                        {
                            TakeMedicineBeanByMedicineListResultSDO tmp = new TakeMedicineBeanByMedicineListResultSDO();
                            tmp.Request = sdo;
                            tmp.Result = beans;
                            result.Add(tmp);
                        }
