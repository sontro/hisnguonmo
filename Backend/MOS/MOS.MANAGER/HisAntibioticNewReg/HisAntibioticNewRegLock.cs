...
{
    partial class HisAntibioticNewRegLock : BusinessBase
    {
        internal HisAntibioticNewRegLock()
            : base()
        {

        }

        internal HisAntibioticNewRegLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_ANTIBIOTIC_NEW_REG resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_ANTIBIOTIC_NEW_REG data = new HisAntibioticNewRegGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisAntibioticNewRegDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_ANTIBIOTIC_NEW_REG resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_ANTIBIOTIC_NEW_REG resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisAntibioticNewRegCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisAntibioticNewRegDAO.Update(raw);
