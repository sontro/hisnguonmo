...
namespace MOS.MANAGER.HisAntibioticNewReg
{
    partial class HisAntibioticNewRegCreate : BusinessBase
    {
		...
		internal bool CreateList(List<HIS_ANTIBIOTIC_NEW_REG> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    if (!DAOWorker.HisAntibioticNewRegDAO.CreateList(listData))
                    {
                        MOS.MANAGER.Base.BugUtil.SetBugCode(param, MOS.LibraryBug.Bug.Enum.HisAntibioticNewReg_ThemMoiThatBai);
                        throw new Exception("Them moi thong tin HisAntibioticNewReg that bai." + LogUtil.TraceData("listData", listData));
                    }
                    this.recentHisAntibioticNewRegs.AddRange(listData);
                    result = true;
                }
            }
            catch (Exception ex)
