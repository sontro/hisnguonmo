...
{
    partial class HisEquipmentSetLock : BusinessBase
    {
        internal HisEquipmentSetLock()
            : base()
        {

        }

        internal HisEquipmentSetLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_EQUIPMENT_SET resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_EQUIPMENT_SET data = new HisEquipmentSetGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisEquipmentSetDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_EQUIPMENT_SET resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_EQUIPMENT_SET resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisEquipmentSetCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisEquipmentSetDAO.Update(raw);
