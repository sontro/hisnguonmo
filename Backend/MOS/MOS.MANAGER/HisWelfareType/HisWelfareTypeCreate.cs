...
namespace MOS.MANAGER.HisWelfareType
{
    partial class HisWelfareTypeCreate : BusinessBase
    {
		...
		internal bool CreateList(List<HIS_WELFARE_TYPE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    if (!DAOWorker.HisWelfareTypeDAO.CreateList(listData))
                    {
                        MOS.MANAGER.Base.BugUtil.SetBugCode(param, MOS.LibraryBug.Bug.Enum.HisWelfareType_ThemMoiThatBai);
                        throw new Exception("Them moi thong tin HisWelfareType that bai." + LogUtil.TraceData("listData", listData));
                    }
                    this.recentHisWelfareTypes.AddRange(listData);
                    result = true;
                }
            }
            catch (Exception ex)
