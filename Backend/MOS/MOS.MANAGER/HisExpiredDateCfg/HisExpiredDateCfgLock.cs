...
{
    partial class HisExpiredDateCfgLock : BusinessBase
    {
        internal HisExpiredDateCfgLock()
            : base()
        {

        }

        internal HisExpiredDateCfgLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_EXPIRED_DATE_CFG resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_EXPIRED_DATE_CFG data = new HisExpiredDateCfgGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisExpiredDateCfgDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_EXPIRED_DATE_CFG resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_EXPIRED_DATE_CFG resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisExpiredDateCfgCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisExpiredDateCfgDAO.Update(raw);
