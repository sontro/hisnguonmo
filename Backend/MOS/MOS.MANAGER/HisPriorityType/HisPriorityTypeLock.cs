...
{
    partial class HisPriorityTypeLock : BusinessBase
    {
        internal HisPriorityTypeLock()
            : base()
        {

        }

        internal HisPriorityTypeLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_PRIORITY_TYPE resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_PRIORITY_TYPE data = new HisPriorityTypeGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisPriorityTypeDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_PRIORITY_TYPE resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_PRIORITY_TYPE resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisPriorityTypeCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisPriorityTypeDAO.Update(raw);
