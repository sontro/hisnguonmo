...
namespace MOS.MANAGER.HisTransfusion
{
    partial class HisTransfusionCreate : BusinessBase
    {
		...
		internal bool CreateList(List<HIS_TRANSFUSION> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    if (!DAOWorker.HisTransfusionDAO.CreateList(listData))
                    {
                        MOS.MANAGER.Base.BugUtil.SetBugCode(param, MOS.LibraryBug.Bug.Enum.HisTransfusion_ThemMoiThatBai);
                        throw new Exception("Them moi thong tin HisTransfusion that bai." + LogUtil.TraceData("listData", listData));
                    }
                    this.recentHisTransfusions.AddRange(listData);
                    result = true;
                }
            }
            catch (Exception ex)
