...
{
    partial class HisMedicalContractLock : BusinessBase
    {
        internal HisMedicalContractLock()
            : base()
        {

        }

        internal HisMedicalContractLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_MEDICAL_CONTRACT resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_MEDICAL_CONTRACT data = new HisMedicalContractGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisMedicalContractDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_MEDICAL_CONTRACT resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_MEDICAL_CONTRACT resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisMedicalContractCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisMedicalContractDAO.Update(raw);
