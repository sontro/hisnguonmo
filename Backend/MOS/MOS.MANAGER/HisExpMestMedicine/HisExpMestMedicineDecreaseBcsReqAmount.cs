...
{
    class HisExpMestMedicineDecreaseBcsReqAmount : BusinessBase
    {
        Dictionary<long, decimal> dicDecrease;
        internal HisExpMestMedicineDecreaseBcsReqAmount()
            : base()
        {

        }

        internal HisExpMestMedicineDecreaseBcsReqAmount(CommonParam param)
            : base(param)
        {
        ...
        internal bool Run(Dictionary<long, decimal> dicData)
        {
            ...
            try
            {
                if (dicData != null && dicData.Count > 0)
                {
                    ...
                    {
                        sb.Append(String.Format("UPDATE HIS_EXP_MEST_MEDICINE SET BCS_REQ_AMOUNT = NVL(BCS_REQ_AMOUNT,0) - {0} WHERE ID = {1};", CommonUtil.ToString(dic.Value), dic.Key));
                    }
        ...
        internal void RollbackData()
        {
            if (this.dicDecrease != null && this.dicDecrease.Count > 0)
            {
                ...
                {
                    sbRollback.Append(String.Format("UPDATE HIS_EXP_MEST_MEDICINE SET BCS_REQ_AMOUNT = NVL(BCS_REQ_AMOUNT,0) + {0} WHERE ID = {1};", CommonUtil.ToString(dic.Value), dic.Key));
                }
