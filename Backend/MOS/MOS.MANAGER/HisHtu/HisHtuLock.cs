...
{
    partial class HisHtuLock : BusinessBase
    {
        internal HisHtuLock()
            : base()
        {
        ...
        internal HisHtuLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_HTU resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_HTU data = new HisHtuGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisHtuDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_HTU resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_HTU resultData)
        {
            bool result = false;
            try
            {
                bool valid = true;
                HIS_HTU raw = null;
                valid = valid && new HisHtuCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisHtuDAO.Update(raw);
