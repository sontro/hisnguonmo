...
{
    partial class HisCacheMonitorLock : BusinessBase
    {
        internal HisCacheMonitorLock()
            : base()
        {

        }

        internal HisCacheMonitorLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_CACHE_MONITOR resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_CACHE_MONITOR data = new HisCacheMonitorGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisCacheMonitorDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_CACHE_MONITOR resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_CACHE_MONITOR resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisCacheMonitorCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisCacheMonitorDAO.Update(raw);
