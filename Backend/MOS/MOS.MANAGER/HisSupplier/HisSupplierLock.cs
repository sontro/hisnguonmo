...
{
    class HisSupplierLock : BusinessBase
    {
        internal HisSupplierLock()
            : base()
        {

        }

        internal HisSupplierLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_SUPPLIER data)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    data = new HisSupplierGet().GetById(data.ID);
                    if (data != null)
                    {
