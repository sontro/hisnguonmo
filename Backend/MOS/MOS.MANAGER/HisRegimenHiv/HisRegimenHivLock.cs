...
{
    partial class HisRegimenHivLock : BusinessBase
    {
        internal HisRegimenHivLock()
            : base()
        {

        }

        internal HisRegimenHivLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_REGIMEN_HIV resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_REGIMEN_HIV data = new HisRegimenHivGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisRegimenHivDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_REGIMEN_HIV resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_REGIMEN_HIV resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisRegimenHivCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisRegimenHivDAO.Update(raw);
