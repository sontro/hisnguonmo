...
{
    partial class HisAwarenessLock : BusinessBase
    {
        internal HisAwarenessLock()
            : base()
        {

        }

        internal HisAwarenessLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_AWARENESS data)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisAwarenessCheck().VerifyId(data.ID, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisAwarenessDAO.Update(raw);
