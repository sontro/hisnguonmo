...
namespace MOS.MANAGER.HisAwareness
{
    ...
    /// </summary>
    partial class HisAwarenessCheck : BusinessBase
    {
        internal HisAwarenessCheck()
            : base()
        {

        }

        internal HisAwarenessCheck(CommonParam paramCheck)
            : base(paramCheck) 
        {

        }

        internal bool VerifyRequireField(HIS_AWARENESS data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisAwarenessGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_AWARENESS data)
        {
            bool valid = true;
            try
            {
                data = new HisAwarenessGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                ...
                {
                    HisAwarenessFilterQuery filter = new HisAwarenessFilterQuery();
                    filter.IDs = listId;
                    List<HIS_AWARENESS> listData = new HisAwarenessGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_AWARENESS> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_AWARENESS data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisAwarenessDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_AWARENESS> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool ExistsCode(string code, long? id)
        {
            ...
            try
            {
                if (DAOWorker.HisAwarenessDAO.ExistsCode(code, id))
                {
        ...
        internal bool CheckConstraint(HIS_AWARENESS data)
        {
            bool valid = true;
            try
            {
                List<HIS_CARE> hisCares = new HisCareGet().GetByAwarenessId(data.ID);
                if (IsNotNullOrEmpty(hisCares))
                {
                    ...
                    throw new Exception("Ton tai du lieu HIS_CARE, khong cho phep xoa" + LogUtil.TraceData("data", data));
                }
            }
            catch (Exception ex)
            {
