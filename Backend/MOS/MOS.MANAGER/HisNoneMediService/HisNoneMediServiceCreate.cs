...
namespace MOS.MANAGER.HisNoneMediService
{
    partial class HisNoneMediServiceCreate : BusinessBase
    {
		...
		internal bool CreateList(List<HIS_NONE_MEDI_SERVICE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    if (!DAOWorker.HisNoneMediServiceDAO.CreateList(listData))
                    {
                        MOS.MANAGER.Base.BugUtil.SetBugCode(param, MOS.LibraryBug.Bug.Enum.HisNoneMediService_ThemMoiThatBai);
                        throw new Exception("Them moi thong tin HisNoneMediService that bai." + LogUtil.TraceData("listData", listData));
                    }
                    this.recentHisNoneMediServices.AddRange(listData);
                    result = true;
                }
            }
            catch (Exception ex)
