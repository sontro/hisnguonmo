...
namespace MOS.MANAGER.HisExpMestTemplate
{
    partial class HisExpMestTemplateCreate : BusinessBase
    {
        ...
        internal HisExpMestTemplateCreate()
            : base()
        {

        }

        internal HisExpMestTemplateCreate(CommonParam paramCreate)
            : base(paramCreate)
        {

        }

        internal bool Create(HisExpMestTemplateSDO data)
        {
            bool result = false;
            try
            {
                if (data == null || data.ExpMestTemplate == null || !(IsNotNullOrEmpty(data.EmteMedicineTypes) || IsNotNullOrEmpty(data.EmteMaterialTypes)))
                {
                    ...
                    throw new Exception("Du lieu ko hop le. Data null hoac (EmteMedicineTypes va EmteMaterialTypes rong)");
                }
        ...
        private void ProcessExpMestTemplate(HisExpMestTemplateSDO data)
        {
            ...
            valid = valid && checker.ExistsCode(data.ExpMestTemplate.EXP_MEST_TEMPLATE_CODE, null);
            if (!valid || !DAOWorker.HisExpMestTemplateDAO.Create(data.ExpMestTemplate))
            {
                throw new Exception("Tao du lieu ExpMestTemplate that bai");
            }
        ...
        private void PassResult(HisExpMestTemplateSDO resultData)
        {
            ...
            resultData.EmteMaterialTypes = this.recentHisEmteMaterialTypes;
        }
    }
}
