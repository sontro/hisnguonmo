...
{
    class HisExpMestTemplateLock : BusinessBase
    {
        internal HisExpMestTemplateLock()
            : base()
        {

        }

        internal HisExpMestTemplateLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_EXP_MEST_TEMPLATE data)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    data = new HisExpMestTemplateGet().GetById(data.ID);
                    if (data != null)
                    {
                        ...
                        result = DAOWorker.HisExpMestTemplateDAO.Update(data);
                    }
                    else
                    {
