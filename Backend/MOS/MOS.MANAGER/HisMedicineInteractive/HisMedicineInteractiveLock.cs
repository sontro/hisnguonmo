...
{
    partial class HisMedicineInteractiveLock : BusinessBase
    {
        internal HisMedicineInteractiveLock()
            : base()
        {

        }

        internal HisMedicineInteractiveLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_MEDICINE_INTERACTIVE resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_MEDICINE_INTERACTIVE data = new HisMedicineInteractiveGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisMedicineInteractiveDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_MEDICINE_INTERACTIVE resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_MEDICINE_INTERACTIVE resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisMedicineInteractiveCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisMedicineInteractiveDAO.Update(raw);
