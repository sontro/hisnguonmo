...
namespace MOS.MANAGER.HisEmployeeSchedule
{
    partial class HisEmployeeScheduleCreate : BusinessBase
    {
		...
		internal bool CreateList(List<HIS_EMPLOYEE_SCHEDULE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    if (!DAOWorker.HisEmployeeScheduleDAO.CreateList(listData))
                    {
                        MOS.MANAGER.Base.BugUtil.SetBugCode(param, MOS.LibraryBug.Bug.Enum.HisEmployeeSchedule_ThemMoiThatBai);
                        throw new Exception("Them moi thong tin HisEmployeeSchedule that bai." + LogUtil.TraceData("listData", listData));
                    }
                    this.recentHisEmployeeSchedules.AddRange(listData);
                    result = true;
                }
            }
            catch (Exception ex)
