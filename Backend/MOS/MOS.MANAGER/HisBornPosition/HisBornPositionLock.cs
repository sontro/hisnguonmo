...
{
    partial class HisBornPositionLock : BusinessBase
    {
        internal HisBornPositionLock()
            : base()
        {

        }

        internal HisBornPositionLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_BORN_POSITION resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_BORN_POSITION data = new HisBornPositionGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisBornPositionDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_BORN_POSITION resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_BORN_POSITION resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisBornPositionCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisBornPositionDAO.Update(raw);
