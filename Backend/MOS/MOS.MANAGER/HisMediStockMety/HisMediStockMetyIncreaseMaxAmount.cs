...
{
    partial class HisMediStockMetyIncreaseMaxAmount : BusinessBase
    {
        private Dictionary<long, decimal> recentIncreaseDic;

        internal HisMediStockMetyIncreaseMaxAmount()
            : base()
        {

        }

        internal HisMediStockMetyIncreaseMaxAmount(CommonParam paramUpdate)
            : base(paramUpdate)
        {
        ...
        internal bool Run(Dictionary<long, decimal> increaseDic)
        {
            ...
            try
            {
                if (increaseDic != null && increaseDic.Count > 0)
                {
                    ...
                    {
                        string sql = string.Format("UPDATE HIS_MEDI_STOCK_METY SET ALERT_MAX_IN_STOCK = NVL(ALERT_MAX_IN_STOCK, 0) + {0} WHERE ID = {1}", CommonUtil.ToString(increaseDic[k]), k);
                        sqls.Add(sql);
                    }
        ...
        internal void Rollback()
        {
            try
            {
                if (this.recentIncreaseDic != null && this.recentIncreaseDic.Count > 0)
                {
                    ...
                    {
                        sqlBuilder.Append(string.Format("UPDATE HIS_MEDI_STOCK_METY SET ALERT_MAX_IN_STOCK = NVL(ALERT_MAX_IN_STOCK, 0) - {0} WHERE ID = {1};", CommonUtil.ToString(this.recentIncreaseDic[k]), k));
                    }
