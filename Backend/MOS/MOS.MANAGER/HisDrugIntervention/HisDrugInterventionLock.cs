...
{
    partial class HisDrugInterventionLock : BusinessBase
    {
        internal HisDrugInterventionLock()
            : base()
        {

        }

        internal HisDrugInterventionLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_DRUG_INTERVENTION resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_DRUG_INTERVENTION data = new HisDrugInterventionGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisDrugInterventionDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_DRUG_INTERVENTION resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_DRUG_INTERVENTION resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisDrugInterventionCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisDrugInterventionDAO.Update(raw);
