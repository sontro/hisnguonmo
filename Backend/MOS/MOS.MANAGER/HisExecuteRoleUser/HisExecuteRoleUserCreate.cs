...
namespace MOS.MANAGER.HisExecuteRoleUser
{
    partial class HisExecuteRoleUserCreate : BusinessBase
    {
        ...
        internal bool CreateList(List<HIS_EXECUTE_ROLE_USER> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    if (!DAOWorker.HisExecuteRoleUserDAO.CreateList(listData))
                    {
                        MOS.MANAGER.Base.BugUtil.SetBugCode(param, MOS.LibraryBug.Bug.Enum.HisExecuteRoleUser_ThemMoiThatBai);
                        throw new Exception("Them moi thong tin HisExecuteRoleUser that bai." + LogUtil.TraceData("listData", listData));
                    }
                    this.recentHisExecuteRoleUsers.AddRange(listData);
                    result = true;
                }
            }
            catch (Exception ex)
