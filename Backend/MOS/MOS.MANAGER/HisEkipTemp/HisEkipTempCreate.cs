...
namespace MOS.MANAGER.HisEkipTemp
{
    partial class HisEkipTempCreate : BusinessBase
    {
		...
		internal bool CreateList(List<HIS_EKIP_TEMP> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    if (!DAOWorker.HisEkipTempDAO.CreateList(listData))
                    {
                        MOS.MANAGER.Base.BugUtil.SetBugCode(param, MOS.LibraryBug.Bug.Enum.HisEkipTemp_ThemMoiThatBai);
                        throw new Exception("Them moi thong tin HisEkipTemp that bai." + LogUtil.TraceData("listData", listData));
                    }
                    this.recentHisEkipTemps.AddRange(listData);
                    result = true;
                }
            }
            catch (Exception ex)
