...
{
    partial class HisBedBstyLock : BusinessBase
    {
        internal HisBedBstyLock()
            : base()
        {

        }

        internal HisBedBstyLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_BED_BSTY resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_BED_BSTY data = new HisBedBstyGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisBedBstyDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_BED_BSTY resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_BED_BSTY resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisBedBstyCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisBedBstyDAO.Update(raw);
