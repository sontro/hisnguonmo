...
{
    partial class HisDocHoldTypeLock : BusinessBase
    {
        internal HisDocHoldTypeLock()
            : base()
        {

        }

        internal HisDocHoldTypeLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_DOC_HOLD_TYPE resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_DOC_HOLD_TYPE data = new HisDocHoldTypeGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisDocHoldTypeDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_DOC_HOLD_TYPE resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_DOC_HOLD_TYPE resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisDocHoldTypeCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisDocHoldTypeDAO.Update(raw);
