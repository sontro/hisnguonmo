...
{
    partial class HisSesePtttMethodLock : BusinessBase
    {
        internal HisSesePtttMethodLock()
            : base()
        {

        }

        internal HisSesePtttMethodLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_SESE_PTTT_METHOD resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_SESE_PTTT_METHOD data = new HisSesePtttMethodGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisSesePtttMethodDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_SESE_PTTT_METHOD resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_SESE_PTTT_METHOD resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisSesePtttMethodCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisSesePtttMethodDAO.Update(raw);
