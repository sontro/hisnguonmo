...
{
    partial class HisTrackingLock : BusinessBase
    {
        internal HisTrackingLock()
            : base()
        {

        }

        internal HisTrackingLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_TRACKING data)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisTrackingCheck().VerifyId(data.ID, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisTrackingDAO.Update(raw);
