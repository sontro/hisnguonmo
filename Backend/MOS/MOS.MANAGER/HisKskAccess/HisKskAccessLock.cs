...
{
    partial class HisKskAccessLock : BusinessBase
    {
        internal HisKskAccessLock()
            : base()
        {

        }

        internal HisKskAccessLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_KSK_ACCESS resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_KSK_ACCESS data = new HisKskAccessGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisKskAccessDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_KSK_ACCESS resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_KSK_ACCESS resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisKskAccessCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisKskAccessDAO.Update(raw);
