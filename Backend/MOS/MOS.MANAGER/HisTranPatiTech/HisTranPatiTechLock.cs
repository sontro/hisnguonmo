...
{
    partial class HisTranPatiTechLock : BusinessBase
    {
        internal HisTranPatiTechLock()
            : base()
        {

        }

        internal HisTranPatiTechLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_TRAN_PATI_TECH resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_TRAN_PATI_TECH data = new HisTranPatiTechGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisTranPatiTechDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_TRAN_PATI_TECH resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_TRAN_PATI_TECH resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisTranPatiTechCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisTranPatiTechDAO.Update(raw);
