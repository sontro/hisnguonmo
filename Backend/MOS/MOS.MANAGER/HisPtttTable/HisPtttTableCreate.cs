    ...
    partial class HisPtttTableCreate : BusinessBase
    {
		...
		internal bool CreateList(List<HIS_PTTT_TABLE> listData)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    if (!DAOWorker.HisPtttTableDAO.CreateList(listData))
                    {
                        MOS.MANAGER.Base.BugUtil.SetBugCode(param, MOS.LibraryBug.Bug.Enum.HisPtttTable_ThemMoiThatBai);
                        throw new Exception("Them moi thong tin HisPtttTable that bai." + LogUtil.TraceData("listData", listData));
                    }
                    this.recentHisPtttTables.AddRange(listData);
                    result = true;
                }
            }
            catch (Exception ex)
