...
{
    partial class HisPtttTableLock : BusinessBase
    {
        internal HisPtttTableLock()
            : base()
        {

        }

        internal HisPtttTableLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_PTTT_TABLE resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_PTTT_TABLE data = new HisPtttTableGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisPtttTableDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_PTTT_TABLE resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_PTTT_TABLE resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisPtttTableCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisPtttTableDAO.Update(raw);
