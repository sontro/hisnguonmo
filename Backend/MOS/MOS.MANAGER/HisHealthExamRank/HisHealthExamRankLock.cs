...
{
    partial class HisHealthExamRankLock : BusinessBase
    {
        internal HisHealthExamRankLock()
            : base()
        {

        }

        internal HisHealthExamRankLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_HEALTH_EXAM_RANK resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_HEALTH_EXAM_RANK data = new HisHealthExamRankGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisHealthExamRankDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_HEALTH_EXAM_RANK resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_HEALTH_EXAM_RANK resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisHealthExamRankCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisHealthExamRankDAO.Update(raw);
