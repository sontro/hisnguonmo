...
{
    partial class HisMrChecklistLock : BusinessBase
    {
        internal HisMrChecklistLock()
            : base()
        {

        }

        internal HisMrChecklistLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_MR_CHECKLIST resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_MR_CHECKLIST data = new HisMrChecklistGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisMrChecklistDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_MR_CHECKLIST resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_MR_CHECKLIST resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisMrChecklistCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisMrChecklistDAO.Update(raw);
