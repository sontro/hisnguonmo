...
{
    partial class HisBodyPartLock : BusinessBase
    {
        internal HisBodyPartLock()
            : base()
        {

        }

        internal HisBodyPartLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_BODY_PART resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_BODY_PART data = new HisBodyPartGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisBodyPartDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_BODY_PART resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_BODY_PART resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisBodyPartCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisBodyPartDAO.Update(raw);
