...
{
    partial class HisAccidentLocationLock : BusinessBase
    {
        internal HisAccidentLocationLock()
            : base()
        {

        }

        internal HisAccidentLocationLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_ACCIDENT_LOCATION data)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisAccidentLocationCheck().VerifyId(data.ID, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisAccidentLocationDAO.Update(raw);
