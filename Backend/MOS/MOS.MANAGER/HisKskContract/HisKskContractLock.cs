...
{
    partial class HisKskContractLock : BusinessBase
    {
        internal HisKskContractLock()
            : base()
        {

        }

        internal HisKskContractLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_KSK_CONTRACT data)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisKskContractCheck().VerifyId(data.ID, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisKskContractDAO.Update(raw);
