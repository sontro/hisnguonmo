...
namespace MOS.MANAGER.HisPtttCatastrophe
{
    ...
    /// </summary>
    partial class HisPtttCatastropheCheck : BusinessBase
    {
        internal HisPtttCatastropheCheck()
            : base()
        {

        }

        internal HisPtttCatastropheCheck(CommonParam paramCheck)
            : base(paramCheck) 
        {

        }

        internal bool VerifyRequireField(HIS_PTTT_CATASTROPHE data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisPtttCatastropheGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_PTTT_CATASTROPHE data)
        {
            bool valid = true;
            try
            {
                data = new HisPtttCatastropheGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                ...
                {
                    HisPtttCatastropheFilterQuery filter = new HisPtttCatastropheFilterQuery();
                    filter.IDs = listId;
                    List<HIS_PTTT_CATASTROPHE> listData = new HisPtttCatastropheGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_PTTT_CATASTROPHE> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_PTTT_CATASTROPHE data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisPtttCatastropheDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_PTTT_CATASTROPHE> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool ExistsCode(string code, long? id)
        {
            ...
            try
            {
                if (DAOWorker.HisPtttCatastropheDAO.ExistsCode(code, id))
                {
        ...
        internal bool CheckConstraint(long id)
        {
            ...
            try
            {
                List<HIS_SERE_SERV_PTTT> hisSereServPttts = new HisSereServPtttGet().GetByPtttCatastropheId(id);
