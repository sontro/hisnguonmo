...
{
    partial class HisServiceFollowLock : BusinessBase
    {
        internal HisServiceFollowLock()
            : base()
        {

        }

        internal HisServiceFollowLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_SERVICE_FOLLOW resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_SERVICE_FOLLOW data = new HisServiceFollowGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisServiceFollowDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_SERVICE_FOLLOW resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_SERVICE_FOLLOW resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisServiceFollowCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisServiceFollowDAO.Update(raw);
