...
namespace MOS.MANAGER.HisDebateInviteUser
{
    partial class HisDebateInviteUserCreate : BusinessBase
    {
		...
		internal bool CreateList(List<HIS_DEBATE_INVITE_USER> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    if (!DAOWorker.HisDebateInviteUserDAO.CreateList(listData))
                    {
                        MOS.MANAGER.Base.BugUtil.SetBugCode(param, MOS.LibraryBug.Bug.Enum.HisDebateInviteUser_ThemMoiThatBai);
                        throw new Exception("Them moi thong tin HisDebateInviteUser that bai." + LogUtil.TraceData("listData", listData));
                    }
                    this.recentHisDebateInviteUsers.AddRange(listData);
                    result = true;
                }
            }
            catch (Exception ex)
