...
namespace MOS.MANAGER.HisBedLog
{
    ...
    /// </summary>
    partial class HisBedLogCheck : BusinessBase
    {
        internal HisBedLogCheck()
            : base()
        {

        }

        internal HisBedLogCheck(CommonParam paramCheck)
            : base(paramCheck) 
        {

        }

        internal bool VerifyRequireField(HIS_BED_LOG data)
        {
            bool valid = true;
            try
            {
                if (data == null) throw new ArgumentNullException("data");
                if (data.BED_ID <= 0) throw new ArgumentNullException("data.BED_ID");
                if (data.TREATMENT_BED_ROOM_ID <= 0) throw new ArgumentNullException("data.TREATMENT_BED_ROOM_ID");
                if (data.START_TIME <= 0) throw new ArgumentNullException("data.START_TIME");
                if (HisSereServCFG.IS_USING_BED_TEMP && (!data.BED_SERVICE_TYPE_ID.HasValue || !data.PATIENT_TYPE_ID.HasValue))
                {
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisBedLogGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_BED_LOG data)
        {
            bool valid = true;
            try
            {
                data = new HisBedLogGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                if (listId != null && listId.Count > 0)
                {
                    ...
                    filter.IDs = listId;
                    List<HIS_BED_LOG> listData = new HisBedLogGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_BED_LOG> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_BED_LOG data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisBedLogDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_BED_LOG> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool CheckConstraint(long id)
        {
            ...
            try
            {
                /*List<XXX> hisXXXs = new HisXXXGet().GetByHisBedLogId(id);
        ...
        internal bool IsValidTime(HIS_BED_LOG data, HIS_TREATMENT_BED_ROOM treatmentBedRoom, HIS_TREATMENT treatment)
        {
            bool valid = true;
            try
            {
                if (data.FINISH_TIME.HasValue && data.FINISH_TIME.Value < data.START_TIME)
                {
                    MOS.MANAGER.Base.MessageUtil.SetMessage(param, MOS.LibraryMessage.Message.Enum.HisBedLog_ThoiGianBatDauKhongDuocLonHonThoiGianKetThuc);
                    return false;
                }

                if (treatment.IN_TIME > data.START_TIME)
                {
                    string time = Inventec.Common.DateTime.Convert.TimeNumberToTimeString(treatment.IN_TIME);
                    MOS.MANAGER.Base.MessageUtil.SetMessage(param, MOS.LibraryMessage.Message.Enum.HisBedLog_ThoiGianBatDauKhongDuocNhoHonThoiGianDenVien, time);
                    return false;
                }

                if (treatment.OUT_TIME.HasValue && treatment.OUT_TIME < data.FINISH_TIME)
                {
                    string time = Inventec.Common.DateTime.Convert.TimeNumberToTimeString(treatment.OUT_TIME.Value);
                    MOS.MANAGER.Base.MessageUtil.SetMessage(param, MOS.LibraryMessage.Message.Enum.HisBedLog_ThoiGianKetThucKhongDuocLonHonThoiGianRaVien, time);
                    return false;
                }
            }
            catch (Exception ex)
