...
{
    partial class HisExecuteRoleLock : BusinessBase
    {
        internal HisExecuteRoleLock()
            : base()
        {

        }

        internal HisExecuteRoleLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_EXECUTE_ROLE resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_EXECUTE_ROLE data = new HisExecuteRoleGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisExecuteRoleDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_EXECUTE_ROLE resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(HIS_EXECUTE_ROLE data)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisExecuteRoleCheck().VerifyId(data.ID, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisExecuteRoleDAO.Update(raw);
