...
{
    class HisMedicinePatyLock : BusinessBase
    {
        internal HisMedicinePatyLock()
            : base()
        {

        }

        internal HisMedicinePatyLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_MEDICINE_PATY data)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    data = new HisMedicinePatyGet().GetById(data.ID);
                    if (data != null)
                    {
                        ...
                        result = DAOWorker.HisMedicinePatyDAO.Update(data);
                    }
                    else
                    {
