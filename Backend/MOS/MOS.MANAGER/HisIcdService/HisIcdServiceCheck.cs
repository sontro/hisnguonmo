...
{
    partial class HisIcdServiceCheck : BusinessBase
    {
        internal HisIcdServiceCheck()
            : base()
        {

        }

        internal HisIcdServiceCheck(CommonParam paramCheck)
            : base(paramCheck)
        {

        }

        internal bool VerifyRequireField(HIS_ICD_SERVICE data)
        {
            bool valid = true;
            try
            {
                ...
                if (String.IsNullOrEmpty(data.ICD_CODE)) throw new ArgumentNullException("data.ICD_CODE");
                if (String.IsNullOrEmpty(data.ICD_NAME)) throw new ArgumentNullException("data.ICD_NAME");
            }
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisIcdServiceGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_ICD_SERVICE data)
        {
            bool valid = true;
            try
            {
                data = new HisIcdServiceGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                ...
                {
                    HisIcdServiceFilterQuery filter = new HisIcdServiceFilterQuery();
                    filter.IDs = listId;
                    List<HIS_ICD_SERVICE> listData = new HisIcdServiceGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_ICD_SERVICE> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_ICD_SERVICE data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisIcdServiceDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_ICD_SERVICE> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool CheckConstraint(long id)
        {
            ...
            try
            {
                /*List<XXX> hisXXXs = new HisXXXGet().GetByHisIcdServiceId(id);
