...
namespace MOS.MANAGER.HisIcdService
{
    partial class HisIcdServiceCreate : BusinessBase
    {
        ...
        internal bool CreateList(List<HIS_ICD_SERVICE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    if (!DAOWorker.HisIcdServiceDAO.CreateList(listData))
                    {
                        MOS.MANAGER.Base.BugUtil.SetBugCode(param, MOS.LibraryBug.Bug.Enum.HisIcdService_ThemMoiThatBai);
                        throw new Exception("Them moi thong tin HisIcdService that bai." + LogUtil.TraceData("listData", listData));
                    }
                    this.recentHisIcdServices.AddRange(listData);
                    result = true;
                }
            }
            catch (Exception ex)
