...
{
    partial class HisSereServPtttTempLock : BusinessBase
    {
        internal HisSereServPtttTempLock()
            : base()
        {

        }

        internal HisSereServPtttTempLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_SERE_SERV_PTTT_TEMP resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_SERE_SERV_PTTT_TEMP data = new HisSereServPtttTempGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisSereServPtttTempDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_SERE_SERV_PTTT_TEMP resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_SERE_SERV_PTTT_TEMP resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisSereServPtttTempCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisSereServPtttTempDAO.Update(raw);
