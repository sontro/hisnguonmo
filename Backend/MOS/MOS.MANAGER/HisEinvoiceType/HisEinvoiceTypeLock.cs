...
{
    partial class HisEinvoiceTypeLock : BusinessBase
    {
        internal HisEinvoiceTypeLock()
            : base()
        {

        }

        internal HisEinvoiceTypeLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_EINVOICE_TYPE resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_EINVOICE_TYPE data = new HisEinvoiceTypeGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisEinvoiceTypeDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_EINVOICE_TYPE resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_EINVOICE_TYPE resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisEinvoiceTypeCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisEinvoiceTypeDAO.Update(raw);
