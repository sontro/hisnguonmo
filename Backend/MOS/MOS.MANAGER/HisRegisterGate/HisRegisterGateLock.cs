...
{
    partial class HisRegisterGateLock : BusinessBase
    {
        internal HisRegisterGateLock()
            : base()
        {

        }

        internal HisRegisterGateLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_REGISTER_GATE resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_REGISTER_GATE data = new HisRegisterGateGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisRegisterGateDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_REGISTER_GATE resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_REGISTER_GATE resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisRegisterGateCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisRegisterGateDAO.Update(raw);
