...
{
    partial class HisBidLock : BusinessBase
    {
        internal HisBidLock()
            : base()
        {
        ...
        internal HisBidLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_BID data)
        {
            bool result = false;
            try
            {
                bool valid = true;
                HIS_BID raw = null;
                valid = valid && new HisBidCheck().VerifyId(data.ID, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisBidDAO.Update(raw);
