...
namespace MOS.MANAGER.HisBid
{
    ...
    /// </summary>
    partial class HisBidCheck : BusinessBase
    {
        internal HisBidCheck()
            : base()
        {

        }

        internal HisBidCheck(CommonParam paramCheck)
            : base(paramCheck) 
        {

        }

        internal bool VerifyRequireField(HIS_BID data)
        {
            bool valid = true;
            try
            {
                if (data == null) throw new ArgumentNullException("data");
                if (!IsNotNullOrEmpty(data.BID_NUMBER)) throw new ArgumentNullException("data.BID_NUMBER");
            }
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisBidGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_BID data)
        {
            bool valid = true;
            try
            {
                data = new HisBidGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                if (listId != null && listId.Count > 0)
                {
                    ...
                    filter.IDs = listId;
                    List<HIS_BID> listData = new HisBidGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_BID> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_BID data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisBidDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_BID> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool CheckConstraint(long id)
        {
            ...
            try
            {
                List<HIS_MEDICINE> hisMedicines = new HisMedicineGet().GetByBidId(id);
                if (IsNotNullOrEmpty(hisMedicines))
                {
                    MANAGER.Base.MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.HisMedicine_TonTaiDuLieu);
                    throw new Exception("Goi thau da ton tai du lieu HIS_MEDICINE, khong cho phep xoa");
                }

                List<HIS_MATERIAL> hisMaterials = new HisMaterialGet().GetByBidId(id);
                if (IsNotNullOrEmpty(hisMaterials))
                {
                    MANAGER.Base.MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.HisMaterial_TonTaiDuLieu);
                    throw new Exception("Goi thau da ton tai du lieu HIS_MATERIAL, khong cho phep xoa");
                }

                List<HIS_BLOOD> hisBloods = new HisBloodGet().GetByBidId(id);
                if (IsNotNullOrEmpty(hisBloods))
                {
                    MANAGER.Base.MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.HisBlood_TonTaiDuLieu);
                    throw new Exception("Goi thau da ton tai du lieu HIS_BLOOD, khong cho phep xoa");
                }
            }
            catch (Exception ex)
            {
