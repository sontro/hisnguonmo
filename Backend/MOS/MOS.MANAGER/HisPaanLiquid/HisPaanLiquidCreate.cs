...
namespace MOS.MANAGER.HisPaanLiquid
{
    partial class HisPaanLiquidCreate : BusinessBase
    {
		...
		internal bool CreateList(List<HIS_PAAN_LIQUID> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    if (!DAOWorker.HisPaanLiquidDAO.CreateList(listData))
                    {
                        MOS.MANAGER.Base.BugUtil.SetBugCode(param, MOS.LibraryBug.Bug.Enum.HisPaanLiquid_ThemMoiThatBai);
                        throw new Exception("Them moi thong tin HisPaanLiquid that bai." + LogUtil.TraceData("listData", listData));
                    }
                    this.recentHisPaanLiquids.AddRange(listData);
                    result = true;
                }
            }
            catch (Exception ex)
