...
{
    partial class HisPaanLiquidLock : BusinessBase
    {
        internal HisPaanLiquidLock()
            : base()
        {

        }

        internal HisPaanLiquidLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_PAAN_LIQUID resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_PAAN_LIQUID data = new HisPaanLiquidGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisPaanLiquidDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_PAAN_LIQUID resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_PAAN_LIQUID resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisPaanLiquidCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisPaanLiquidDAO.Update(raw);
