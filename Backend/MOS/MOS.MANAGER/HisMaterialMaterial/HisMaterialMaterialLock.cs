...
{
    partial class HisMaterialMaterialLock : BusinessBase
    {
        internal HisMaterialMaterialLock()
            : base()
        {

        }

        internal HisMaterialMaterialLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_MATERIAL_MATERIAL resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_MATERIAL_MATERIAL data = new HisMaterialMaterialGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisMaterialMaterialDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_MATERIAL_MATERIAL resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_MATERIAL_MATERIAL resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisMaterialMaterialCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisMaterialMaterialDAO.Update(raw);
