...
{
    class HisRehaTrainUnitLock : BusinessBase
    {
        internal HisRehaTrainUnitLock()
            : base()
        {

        }

        internal HisRehaTrainUnitLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_REHA_TRAIN_UNIT data)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    data = new HisRehaTrainUnitGet().GetById(data.ID);
                    if (data != null)
                    {
