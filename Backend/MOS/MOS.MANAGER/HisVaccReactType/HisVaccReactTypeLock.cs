...
{
    partial class HisVaccReactTypeLock : BusinessBase
    {
        internal HisVaccReactTypeLock()
            : base()
        {

        }

        internal HisVaccReactTypeLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_VACC_REACT_TYPE resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_VACC_REACT_TYPE data = new HisVaccReactTypeGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisVaccReactTypeDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_VACC_REACT_TYPE resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_VACC_REACT_TYPE resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisVaccReactTypeCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisVaccReactTypeDAO.Update(raw);
