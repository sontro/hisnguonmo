...
{
    partial class HisBcsMetyReqDtLock : BusinessBase
    {
        internal HisBcsMetyReqDtLock()
            : base()
        {

        }

        internal HisBcsMetyReqDtLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_BCS_METY_REQ_DT resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_BCS_METY_REQ_DT data = new HisBcsMetyReqDtGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisBcsMetyReqDtDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_BCS_METY_REQ_DT resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_BCS_METY_REQ_DT resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisBcsMetyReqDtCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisBcsMetyReqDtDAO.Update(raw);
