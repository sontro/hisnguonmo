...
{
    class HisServicePatyLock : BusinessBase
    {
        internal HisServicePatyLock()
            : base()
        {

        }

        internal HisServicePatyLock(Inventec.Core.CommonParam paramLock)
            : base(paramLock)
        {
        ...
        internal bool ChangeLock(long id)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    HIS_SERVICE_PATY data = new HisServicePatyGet().GetById(id);
                    if (data != null)
                    {
