...
{
    partial class HisBornTypeLock : BusinessBase
    {
        internal HisBornTypeLock()
            : base()
        {

        }

        internal HisBornTypeLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_BORN_TYPE resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_BORN_TYPE data = new HisBornTypeGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisBornTypeDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_BORN_TYPE resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_BORN_TYPE resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisBornTypeCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisBornTypeDAO.Update(raw);
