...
{
    partial class HisHoreHohaLock : BusinessBase
    {
        internal HisHoreHohaLock()
            : base()
        {

        }

        internal HisHoreHohaLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_HORE_HOHA resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_HORE_HOHA data = new HisHoreHohaGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisHoreHohaDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_HORE_HOHA resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_HORE_HOHA resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisHoreHohaCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisHoreHohaDAO.Update(raw);
