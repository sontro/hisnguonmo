...
{
    partial class HisContactLock : BusinessBase
    {
        internal HisContactLock()
            : base()
        {

        }

        internal HisContactLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_CONTACT resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_CONTACT data = new HisContactGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisContactDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_CONTACT resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_CONTACT resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisContactCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisContactDAO.Update(raw);
