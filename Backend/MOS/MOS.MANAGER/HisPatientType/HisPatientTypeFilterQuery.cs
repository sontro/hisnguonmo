	...
	public class HisPatientTypeFilterQuery : HisPatientTypeFilter
	{
		...
		internal HisPatientTypeSO Query()
		{
			...
			try
			{
                ...
                if (this.IS_CHECK_FINISH_CLS_WHEN_PRES.HasValue)
                {
                    if (this.IS_CHECK_FINISH_CLS_WHEN_PRES.Value)
                    {
                        listHisPatientTypeExpression.Add(o => o.IS_CHECK_FINISH_CLS_WHEN_PRES.HasValue && o.IS_CHECK_FINISH_CLS_WHEN_PRES.Value == Constant.IS_TRUE);
                    }
                    else
                    {
                        listHisPatientTypeExpression.Add(o => !o.IS_CHECK_FINISH_CLS_WHEN_PRES.HasValue || o.IS_CHECK_FINISH_CLS_WHEN_PRES.Value != Constant.IS_TRUE);
