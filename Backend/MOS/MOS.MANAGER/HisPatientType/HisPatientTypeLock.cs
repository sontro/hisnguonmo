...
{
    partial class HisPatientTypeLock : BusinessBase
    {
        internal HisPatientTypeLock()
            : base()
        {

        }

        internal HisPatientTypeLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_PATIENT_TYPE data)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisPatientTypeCheck().VerifyId(data.ID, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisPatientTypeDAO.Update(raw);
