...
{
    partial class HisServiceNumOrderLock : BusinessBase
    {
        internal HisServiceNumOrderLock()
            : base()
        {

        }

        internal HisServiceNumOrderLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_SERVICE_NUM_ORDER resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_SERVICE_NUM_ORDER data = new HisServiceNumOrderGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisServiceNumOrderDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_SERVICE_NUM_ORDER resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_SERVICE_NUM_ORDER resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisServiceNumOrderCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisServiceNumOrderDAO.Update(raw);
