...
{
    class HisMediStockMatyLock : BusinessBase
    {
        internal HisMediStockMatyLock()
            : base()
        {

        }

        internal HisMediStockMatyLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_MEDI_STOCK_MATY data)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    data = new HisMediStockMatyGet().GetById(data.ID);
                    if (data != null)
                    {
                        ...
                        result = DAOWorker.HisMediStockMatyDAO.Update(data);
                    }
                    else
                    {
