...
{
    partial class HisMediStockMatyDecreaseMaxAmount : BusinessBase
    {
        private Dictionary<long, decimal> recentDecreaseDic;

        internal HisMediStockMatyDecreaseMaxAmount()
            : base()
        {

        }

        internal HisMediStockMatyDecreaseMaxAmount(CommonParam paramUpdate)
            : base(paramUpdate)
        {
        ...
        internal bool Run(Dictionary<long, decimal> decreaseDic)
        {
            ...
            try
            {
                if (decreaseDic != null && decreaseDic.Count > 0)
                {
                    ...
                    {
                        string sql = string.Format("UPDATE HIS_MEDI_STOCK_MATY SET ALERT_MAX_IN_STOCK = NVL(ALERT_MAX_IN_STOCK, 0) - {0} WHERE ID = {1}", CommonUtil.ToString(decreaseDic[k]), k);
                        sqls.Add(sql);
                    }
        ...
        internal void Rollback()
        {
            try
            {
                if (this.recentDecreaseDic != null && this.recentDecreaseDic.Count > 0)
                {
                    ...
                    {
                        sqlBuilder.Append(string.Format("UPDATE HIS_MEDI_STOCK_MATY SET ALERT_MAX_IN_STOCK = NVL(ALERT_MAX_IN_STOCK, 0) + {0} WHERE ID = {1};", CommonUtil.ToString(this.recentDecreaseDic[k]), k));
                    }
