...
{
    partial class HisSourceMedicineLock : BusinessBase
    {
        internal HisSourceMedicineLock()
            : base()
        {

        }

        internal HisSourceMedicineLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_SOURCE_MEDICINE resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_SOURCE_MEDICINE data = new HisSourceMedicineGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisSourceMedicineDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_SOURCE_MEDICINE resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_SOURCE_MEDICINE resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisSourceMedicineCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisSourceMedicineDAO.Update(raw);
