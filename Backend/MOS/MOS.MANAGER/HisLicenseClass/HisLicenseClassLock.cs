...
{
    partial class HisLicenseClassLock : BusinessBase
    {
        internal HisLicenseClassLock()
            : base()
        {

        }

        internal HisLicenseClassLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_LICENSE_CLASS resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_LICENSE_CLASS data = new HisLicenseClassGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisLicenseClassDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_LICENSE_CLASS resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_LICENSE_CLASS resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisLicenseClassCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisLicenseClassDAO.Update(raw);
