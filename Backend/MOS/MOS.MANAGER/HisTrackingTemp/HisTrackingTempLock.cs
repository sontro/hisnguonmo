...
{
    partial class HisTrackingTempLock : BusinessBase
    {
        internal HisTrackingTempLock()
            : base()
        {

        }

        internal HisTrackingTempLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_TRACKING_TEMP resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_TRACKING_TEMP data = new HisTrackingTempGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisTrackingTempDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_TRACKING_TEMP resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_TRACKING_TEMP resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisTrackingTempCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisTrackingTempDAO.Update(raw);
