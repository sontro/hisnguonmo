...
{
    partial class HisTreatmentEndTypeExtCheck : BusinessBase
    {
        internal HisTreatmentEndTypeExtCheck()
            : base()
        {

        }

        internal HisTreatmentEndTypeExtCheck(CommonParam paramCheck)
            : base(paramCheck) 
        {

        }

        internal bool VerifyRequireField(HIS_TREATMENT_END_TYPE_EXT data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisTreatmentEndTypeExtGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_TREATMENT_END_TYPE_EXT data)
        {
            bool valid = true;
            try
            {
                data = new HisTreatmentEndTypeExtGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                ...
                {
                    HisTreatmentEndTypeExtFilterQuery filter = new HisTreatmentEndTypeExtFilterQuery();
                    filter.IDs = listId;
                    List<HIS_TREATMENT_END_TYPE_EXT> listData = new HisTreatmentEndTypeExtGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_TREATMENT_END_TYPE_EXT> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_TREATMENT_END_TYPE_EXT data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisTreatmentEndTypeExtDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_TREATMENT_END_TYPE_EXT> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool CheckConstraint(long id)
        {
            ...
            try
            {
                /*List<XXX> hisXXXs = new HisXXXGet().GetByHisTreatmentEndTypeExtId(id);
