...
{
    partial class HisMixedMedicineLock : BusinessBase
    {
        internal HisMixedMedicineLock()
            : base()
        {

        }

        internal HisMixedMedicineLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_MIXED_MEDICINE resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_MIXED_MEDICINE data = new HisMixedMedicineGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisMixedMedicineDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_MIXED_MEDICINE resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_MIXED_MEDICINE resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisMixedMedicineCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisMixedMedicineDAO.Update(raw);
