...
namespace MOS.MANAGER.HisBhytParam
{
    partial class HisBhytParamCreate : BusinessBase
    {
		...
		internal bool CreateList(List<HIS_BHYT_PARAM> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    if (!DAOWorker.HisBhytParamDAO.CreateList(listData))
                    {
                        MOS.MANAGER.Base.BugUtil.SetBugCode(param, MOS.LibraryBug.Bug.Enum.HisBhytParam_ThemMoiThatBai);
                        throw new Exception("Them moi thong tin HisBhytParam that bai." + LogUtil.TraceData("listData", listData));
                    }
                    this.recentHisBhytParams.AddRange(listData);
                    result = true;
                }
            }
            catch (Exception ex)
