...
{
    partial class HisBhytParamLock : BusinessBase
    {
        internal HisBhytParamLock()
            : base()
        {

        }

        internal HisBhytParamLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_BHYT_PARAM resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_BHYT_PARAM data = new HisBhytParamGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisBhytParamDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_BHYT_PARAM resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_BHYT_PARAM resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisBhytParamCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisBhytParamDAO.Update(raw);
