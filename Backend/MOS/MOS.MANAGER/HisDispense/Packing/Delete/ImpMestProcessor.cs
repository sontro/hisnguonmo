...
namespace MOS.MANAGER.HisDispense.Packing.Delete
{
    class ImpMestProcessor : BusinessBase
    {
        ...
        private void ProcessImpMestMaterial(HIS_IMP_MEST impMest, List<HIS_IMP_MEST_MATERIAL> impMestMaterials, ref List<string> sqls)
        {
            ...
            if (!valid)
            {
                throw new Exception("HisImpMestMaterialCheck. Ket thuc nghiep vu.");
            }
            string sqlDelete = String.Format("DELETE HIS_IMP_MEST_MATERIAL WHERE IMP_MEST_ID = {0}", impMest.ID);
            sqls.Add(sqlDelete);
        }

        private void ProcessMaterialBean(List<HIS_IMP_MEST_MATERIAL> impMestMaterials, ref List<string> sqls)
        {
            ...
            sqls.Add(deleteMaterialBean);
        }

        private void ProcessMaterial(List<HIS_IMP_MEST_MATERIAL> impMestMaterials, ref List<string> sqls)
        {
            bool valid = true;
