...
{
    class HisDispenseHandlerTruncate : BusinessBase
    {
        ...
        internal HisDispenseHandlerTruncate()
            : base()
        {
            this.Init();
        }

        internal HisDispenseHandlerTruncate(CommonParam param)
            : base(param)
        {
            this.Init();
        }
        ...
        internal bool Run(HisDispenseDeleteSDO data)
        {
            bool result = false;
            try
            {
                ...
                HisDispenseHandlerTruncateCheck checker = new HisDispenseHandlerTruncateCheck(param);
                ...
                if (valid)
                {
                    ...
                    if (!this.expMestMedicineProcessor.Run(expMest, ref sqls))
                    {
                    ...
                    if (!this.dispenseProcessor.Run(dispense, ref sqls))
                    {
                    ...
                    new EventLogGenerator(EventLog.Enum.HisDispense_XoaPhieuBaoChe).DispenseCode(dispense.DISPENSE_CODE).ExpMestCode(expMest.EXP_MEST_CODE).ImpMestCode(impMest.IMP_MEST_CODE).Run();
