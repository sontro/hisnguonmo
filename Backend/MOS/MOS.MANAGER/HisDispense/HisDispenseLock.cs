...
{
    partial class HisDispenseLock : BusinessBase
    {
        internal HisDispenseLock()
            : base()
        {

        }

        internal HisDispenseLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_DISPENSE resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_DISPENSE data = new HisDispenseGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisDispenseDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_DISPENSE resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_DISPENSE resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisDispenseCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisDispenseDAO.Update(raw);
