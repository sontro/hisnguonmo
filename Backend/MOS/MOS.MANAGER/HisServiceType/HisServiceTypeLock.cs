...
{
    class HisServiceTypeLock : BusinessBase
    {
        internal HisServiceTypeLock()
            : base()
        {

        }

        internal HisServiceTypeLock(Inventec.Core.CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_SERVICE_TYPE data)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    data = new HisServiceTypeGet().GetById(data.ID);
                    if (data != null)
                    {
        ...
        internal bool ChangeLock(long id)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    HIS_SERVICE_TYPE data = new HisServiceTypeGet().GetById(id);
                    if (data != null)
                    {
                        if (data.IS_ACTIVE.HasValue && data.IS_ACTIVE == IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__TRUE)
                        {
