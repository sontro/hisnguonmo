...
{
    partial class HisAreaLock : BusinessBase
    {
        internal HisAreaLock()
            : base()
        {

        }

        internal HisAreaLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_AREA resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_AREA data = new HisAreaGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisAreaDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_AREA resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_AREA resultData)
        {
            bool result = false;
            try
            {
                bool valid = true;
                HIS_AREA raw = null;
                valid = valid && new HisAreaCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisAreaDAO.Update(raw);
