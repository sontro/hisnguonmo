...
{
    partial class HisDepositReqLock : BusinessBase
    {
        internal HisDepositReqLock()
            : base()
        {

        }

        internal HisDepositReqLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_DEPOSIT_REQ data)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisDepositReqCheck().VerifyId(data.ID, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisDepositReqDAO.Update(raw);
