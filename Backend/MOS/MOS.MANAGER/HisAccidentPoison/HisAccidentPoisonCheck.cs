...
namespace MOS.MANAGER.HisAccidentPoison
{
    ...
    /// </summary>
    partial class HisAccidentPoisonCheck : BusinessBase
    {
        internal HisAccidentPoisonCheck()
            : base()
        {

        }

        internal HisAccidentPoisonCheck(CommonParam paramCheck)
            : base(paramCheck) 
        {

        }

        internal bool VerifyRequireField(HIS_ACCIDENT_POISON data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisAccidentPoisonGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_ACCIDENT_POISON data)
        {
            bool valid = true;
            try
            {
                data = new HisAccidentPoisonGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                ...
                {
                    HisAccidentPoisonFilterQuery filter = new HisAccidentPoisonFilterQuery();
                    filter.IDs = listId;
                    List<HIS_ACCIDENT_POISON> listData = new HisAccidentPoisonGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_ACCIDENT_POISON> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_ACCIDENT_POISON data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisAccidentPoisonDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_ACCIDENT_POISON> listData)
        {
            bool valid = true;
            try
            {
