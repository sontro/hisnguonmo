...
{
    partial class HisPatientClassifyLock : BusinessBase
    {
        internal HisPatientClassifyLock()
            : base()
        {

        }

        internal HisPatientClassifyLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_PATIENT_CLASSIFY resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_PATIENT_CLASSIFY data = new HisPatientClassifyGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisPatientClassifyDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_PATIENT_CLASSIFY resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_PATIENT_CLASSIFY resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisPatientClassifyCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisPatientClassifyDAO.Update(raw);
