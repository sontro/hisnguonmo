...
{
    partial class HisPatientClassifyCheck : BusinessBase
    {
        internal HisPatientClassifyCheck()
            : base()
        {

        }

        internal HisPatientClassifyCheck(CommonParam paramCheck)
            : base(paramCheck) 
        {

        }

        internal bool VerifyRequireField(HIS_PATIENT_CLASSIFY data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisPatientClassifyGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_PATIENT_CLASSIFY data)
        {
            bool valid = true;
            try
            {
                data = new HisPatientClassifyGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                ...
                {
                    HisPatientClassifyFilterQuery filter = new HisPatientClassifyFilterQuery();
                    filter.IDs = listId;
                    List<HIS_PATIENT_CLASSIFY> listData = new HisPatientClassifyGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_PATIENT_CLASSIFY> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_PATIENT_CLASSIFY data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisPatientClassifyDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_PATIENT_CLASSIFY> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool CheckConstraint(long id)
        {
            ...
            try
            {
                List<HIS_PATIENT> hisPatients = new HisPatientGet().GetByPatientClassifyId(id);
                ...
                List<HIS_TREATMENT> hisTreatments = new HisTreatmentGet().GetByPatientClassifyId(id);
                ...
                List<HIS_SERVICE_REQ> hisServiceReqs = new HisServiceReqGet().GetByPatientClassifyId(id);
