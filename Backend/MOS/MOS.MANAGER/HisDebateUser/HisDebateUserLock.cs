...
{
    partial class HisDebateUserLock : BusinessBase
    {
        internal HisDebateUserLock()
            : base()
        {

        }

        internal HisDebateUserLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_DEBATE_USER data)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisDebateUserCheck().VerifyId(data.ID, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisDebateUserDAO.Update(raw);
