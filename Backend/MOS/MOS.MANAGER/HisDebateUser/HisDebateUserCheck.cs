...
namespace MOS.MANAGER.HisDebateUser
{
    ...
    /// </summary>
    partial class HisDebateUserCheck : BusinessBase
    {
        internal HisDebateUserCheck()
            : base()
        {

        }

        internal HisDebateUserCheck(CommonParam paramCheck)
            : base(paramCheck)
        {

        }

        internal bool VerifyRequireField(HIS_DEBATE_USER data)
        {
            bool valid = true;
            try
            {
                if (data == null) throw new ArgumentNullException("data");
                //if (!IsGreaterThanZero(data.DEBATE_ID ?? 0)) throw new ArgumentNullException("data.DEBATE_ID");
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisDebateUserGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_DEBATE_USER data)
        {
            bool valid = true;
            try
            {
                data = new HisDebateUserGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                if (listId != null && listId.Count > 0)
                {
                    ...
                    filter.IDs = listId;
                    List<HIS_DEBATE_USER> listData = new HisDebateUserGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_DEBATE_USER> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_DEBATE_USER data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisDebateUserDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_DEBATE_USER> listData)
        {
            bool valid = true;
            try
            {
