...
{
    partial class HisBedLock : BusinessBase
    {
        internal HisBedLock()
            : base()
        {
        ...
        internal HisBedLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_BED resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_BED data = new HisBedGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisBedDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_BED resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(HIS_BED data)
        {
            bool result = false;
            try
            {
                bool valid = true;
                HIS_BED raw = null;
                valid = valid && new HisBedCheck().VerifyId(data.ID, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisBedDAO.Update(raw);
