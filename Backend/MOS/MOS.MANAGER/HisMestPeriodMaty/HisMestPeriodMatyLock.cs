...
{
    class HisMestPeriodMatyLock : BusinessBase
    {
        internal HisMestPeriodMatyLock()
            : base()
        {

        }

        internal HisMestPeriodMatyLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_MEST_PERIOD_MATY data)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    data = new HisMestPeriodMatyGet().GetById(data.ID);
                    if (data != null)
                    {
