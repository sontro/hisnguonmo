...
{
    partial class HisPackageCheck : BusinessBase
    {
        internal HisPackageCheck()
            : base()
        {

        }

        internal HisPackageCheck(CommonParam paramCheck)
            : base(paramCheck) 
        {

        }

        internal bool VerifyRequireField(HIS_PACKAGE data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsValidPackage(HIS_PACKAGE data)
        {
            bool valid = true;
            try
            {
                ...
                {
                    List<HIS_PACKAGE_DETAIL> details = new HisPackageDetailGet().GetByPackageId(data.ID);
                    if (IsNotNullOrEmpty(details))
                    {
                        MOS.MANAGER.Base.MessageUtil.SetMessage(param, MOS.LibraryMessage.Message.Enum.HisPackage_DaTonTaiDichVuTrongGoi, data.PACKAGE_NAME);
                        return false;
                    }
                }
            }
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisPackageGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_PACKAGE data)
        {
            bool valid = true;
            try
            {
                data = new HisPackageGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                if (listId != null && listId.Count > 0)
                {
                    ...
                    filter.IDs = listId;
                    List<HIS_PACKAGE> listData = new HisPackageGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_PACKAGE> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_PACKAGE data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisPackageDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_PACKAGE> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool CheckConstraint(long id)
        {
            ...
            try
            {
                List<HIS_SERE_SERV> hisSereServs = new HisSereServGet().GetByPackageId(id);
                ...
                List<HIS_SERVICE> hisServices = new HisServiceGet().GetByPackageId(id);
