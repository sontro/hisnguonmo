...
{
    partial class HisRationSumLock : BusinessBase
    {
        internal HisRationSumLock()
            : base()
        {

        }

        internal HisRationSumLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_RATION_SUM resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_RATION_SUM data = new HisRationSumGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisRationSumDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_RATION_SUM resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_RATION_SUM resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisRationSumCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisRationSumDAO.Update(raw);
