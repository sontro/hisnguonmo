...
{
    partial class HisMediRecordTypeLock : BusinessBase
    {
        internal HisMediRecordTypeLock()
            : base()
        {

        }

        internal HisMediRecordTypeLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_MEDI_RECORD_TYPE resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_MEDI_RECORD_TYPE data = new HisMediRecordTypeGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisMediRecordTypeDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_MEDI_RECORD_TYPE resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_MEDI_RECORD_TYPE resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisMediRecordTypeCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisMediRecordTypeDAO.Update(raw);
