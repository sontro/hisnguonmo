...
{
    partial class HisMestPeriodBloodLock : BusinessBase
    {
        internal HisMestPeriodBloodLock()
            : base()
        {

        }

        internal HisMestPeriodBloodLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_MEST_PERIOD_BLOOD resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_MEST_PERIOD_BLOOD data = new HisMestPeriodBloodGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisMestPeriodBloodDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_MEST_PERIOD_BLOOD resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(HIS_MEST_PERIOD_BLOOD data)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisMestPeriodBloodCheck().VerifyId(data.ID, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisMestPeriodBloodDAO.Update(raw);
