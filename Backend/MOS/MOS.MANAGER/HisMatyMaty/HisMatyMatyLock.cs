...
{
    partial class HisMatyMatyLock : BusinessBase
    {
        internal HisMatyMatyLock()
            : base()
        {

        }

        internal HisMatyMatyLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_MATY_MATY resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_MATY_MATY data = new HisMatyMatyGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisMatyMatyDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_MATY_MATY resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_MATY_MATY resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisMatyMatyCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisMatyMatyDAO.Update(raw);
