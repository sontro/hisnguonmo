...
{
    partial class HisBranchTimeLock : BusinessBase
    {
        internal HisBranchTimeLock()
            : base()
        {

        }

        internal HisBranchTimeLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_BRANCH_TIME resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_BRANCH_TIME data = new HisBranchTimeGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisBranchTimeDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_BRANCH_TIME resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_BRANCH_TIME resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisBranchTimeCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisBranchTimeDAO.Update(raw);
