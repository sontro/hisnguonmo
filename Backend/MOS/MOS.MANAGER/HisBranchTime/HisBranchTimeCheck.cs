...
{
    partial class HisBranchTimeCheck : BusinessBase
    {
        internal HisBranchTimeCheck()
            : base()
        {

        }

        internal HisBranchTimeCheck(CommonParam paramCheck)
            : base(paramCheck)
        {

        }

        internal bool VerifyRequireField(HIS_BRANCH_TIME data)
        {
            bool valid = true;
            try
            {
                ...
                if (data.DAY <= 0 || data.DAY >= 8) throw new ArgumentNullException("data.DAY");
                if (String.IsNullOrWhiteSpace(data.FROM_TIME)) throw new ArgumentNullException("data.FROM_TIME");
                if (String.IsNullOrWhiteSpace(data.TO_TIME)) throw new ArgumentNullException("data.TO_TIME");
            }
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisBranchTimeGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_BRANCH_TIME data)
        {
            bool valid = true;
            try
            {
                data = new HisBranchTimeGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                ...
                {
                    HisBranchTimeFilterQuery filter = new HisBranchTimeFilterQuery();
                    filter.IDs = listId;
                    List<HIS_BRANCH_TIME> listData = new HisBranchTimeGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_BRANCH_TIME> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_BRANCH_TIME data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisBranchTimeDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_BRANCH_TIME> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool CheckConstraint(long id)
        {
            ...
            try
            {
                /*List<XXX> hisXXXs = new HisXXXGet().GetByHisBranchTimeId(id);
