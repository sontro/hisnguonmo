...
{
    partial class HisEkipUserLock : BusinessBase
    {
        internal HisEkipUserLock()
            : base()
        {

        }

        internal HisEkipUserLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_EKIP_USER resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_EKIP_USER data = new HisEkipUserGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisEkipUserDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_EKIP_USER resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(HIS_EKIP_USER data)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisEkipUserCheck().VerifyId(data.ID, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisEkipUserDAO.Update(raw);
