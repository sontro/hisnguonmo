...
{
    partial class HisMestPeriodBltyLock : BusinessBase
    {
        internal HisMestPeriodBltyLock()
            : base()
        {

        }

        internal HisMestPeriodBltyLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_MEST_PERIOD_BLTY resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_MEST_PERIOD_BLTY data = new HisMestPeriodBltyGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisMestPeriodBltyDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_MEST_PERIOD_BLTY resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(HIS_MEST_PERIOD_BLTY data)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisMestPeriodBltyCheck().VerifyId(data.ID, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisMestPeriodBltyDAO.Update(raw);
