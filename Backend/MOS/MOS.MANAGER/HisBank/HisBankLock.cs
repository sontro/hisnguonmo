...
{
    partial class HisBankLock : BusinessBase
    {
        internal HisBankLock()
            : base()
        {

        }

        internal HisBankLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_BANK resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_BANK data = new HisBankGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisBankDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_BANK resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_BANK resultData)
        {
            bool result = false;
            try
            {
                bool valid = true;
                HIS_BANK raw = null;
                valid = valid && new HisBankCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisBankDAO.Update(raw);
