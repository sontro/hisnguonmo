...
{
    class HisServSegrLock : BusinessBase
    {
        internal HisServSegrLock()
            : base()
        {

        }

        internal HisServSegrLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_SERV_SEGR data)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    data = new HisServSegrGet().GetById(data.ID);
                    if (data != null)
                    {
