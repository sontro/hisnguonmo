...
namespace MOS.MANAGER.HisMaterialBean.Handle
{
    ...
    /// </summary>
    class HisMaterialBeanTake : BusinessBase
    {
        private const char OBJECT_SEPARATOR = ';';
        private const char FIELD_SEPARATOR = ':';

        internal HisMaterialBeanTake()
            : base()
        {
        }

        internal HisMaterialBeanTake(CommonParam paramCreate)
            : base(paramCreate)
        {
        }
        ...
        /// <returns></returns>
        internal List<HIS_MATERIAL_BEAN> Take(TakeBeanSDO sdo)
        {
            ...
            try
            {
                if (this.Validate(sdo))
                {
                    ...
                    string storedSql = "PKG_TAKE_MATERIAL_BEAN.PRO_EXECUTE";
                    ...
                    OracleParameter materialTypeIdPar = new OracleParameter("P_MATERIAL_TYPE_ID", OracleDbType.Int32, sdo.TypeId, ParameterDirection.Input);
                    ...
                    if (DAOWorker.SqlDAO.ExecuteStored(OutputHandler, ref resultHolder, storedSql, languageCodePar, messagePar, materialBeanPar, mediStockIdPar, materialTypeIdPar, amountPar, sessionKeyPar, orderOptPar, patientTypeIdPar, beanIdsPar, expMestMaterialIdsPar, expiredDatePar))
                    {
        ...
        /// <returns></returns>
        internal List<TakeMaterialBeanListResultSDO> Take(List<TakeBeanSDO> sdos)
        {
            List<TakeMaterialBeanListResultSDO> result = null;
            try
            {
                if (IsNotNullOrEmpty(sdos))
                {
                    result = new List<TakeMaterialBeanListResultSDO>();
                    foreach (TakeBeanSDO sdo in sdos)
                    {
                        List<HIS_MATERIAL_BEAN> beans = this.Take(sdo);
                        if (beans != null)
                        {
                            TakeMaterialBeanListResultSDO tmp = new TakeMaterialBeanListResultSDO();
                            tmp.Request = sdo;
                            tmp.Result = beans;
                            result.Add(tmp);
                        }
        ...
        private bool Validate(TakeBeanSDO data)
        {
            ...
            try
            {
                ...
                if (HisMaterialTypeCFG.IsStent(data.TypeId) && data.Amount > 1)
                {
