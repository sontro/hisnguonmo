...
namespace MOS.MANAGER.HisMaterialBean.Handle
{
    ...
    /// </summary>
    class HisMaterialBeanRelease : BusinessBase
    {
        internal HisMaterialBeanRelease()
            : base()
        {
        }

        internal HisMaterialBeanRelease(CommonParam paramCreate)
            : base(paramCreate)
        {
        }

        /// <summary>
        /// Tra bean. Lay toan bo cac bean tuong ung voi medi-stock-id, material-type-id, session-key truyen vao,
        ...
        internal bool Release(ReleaseBeanSDO sdo)
        {
            try
            {
                ...
                {
                    sql = "UPDATE HIS_MATERIAL_BEAN "
                        + " SET SESSION_KEY = NULL, IS_ACTIVE = 1 "
                        + " WHERE MATERIAL_ID = :param1 "
                ...
                else
                {
                    sql = "UPDATE HIS_MATERIAL_BEAN "
                        + " SET SESSION_KEY = NULL, IS_ACTIVE = 1 "
                        + " WHERE TDL_MATERIAL_TYPE_ID = :param1 "
                        + " AND MEDI_STOCK_ID = :param2 "
                        + " AND SESSION_KEY = :param3 "
                        + " AND %IN_CLAUSE% ";
                    id = sdo.TypeId;
        ...
        internal bool Release(string clientSessionKey)
        {
            try
            {
                string sql = "UPDATE HIS_MATERIAL_BEAN "
                            + " SET SESSION_KEY = NULL, IS_ACTIVE = 1 "
