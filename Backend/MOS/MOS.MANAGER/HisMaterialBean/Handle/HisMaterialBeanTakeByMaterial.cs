...
namespace MOS.MANAGER.HisMaterialBean.Handle
{
    ...
    /// </summary>
    class HisMaterialBeanTakeByMaterial : BusinessBase
    {
        private const char OBJECT_SEPARATOR = ';';
        private const char FIELD_SEPARATOR = ':';

        internal HisMaterialBeanTakeByMaterial()
            : base()
        {
        }

        internal HisMaterialBeanTakeByMaterial(CommonParam paramCreate)
            : base(paramCreate)
        {
        }
        ...
        /// lên exp_mest_material_id tuong ung
        /// </summary>
        ...
        /// <returns></returns>
        internal List<HIS_MATERIAL_BEAN> Take(TakeBeanByMameSDO sdo)
        {
            ...
            try
            {
                if (this.Validate(sdo))
                {
                    ...
                    string expMestMaterialIds = IsNotNullOrEmpty(sdo.ExpMestDetailIds) ? string.Join(",", sdo.ExpMestDetailIds) : null;

                    string storedSql = "PKG_TAKE_MATERIAL_BEAN_BY_MATE.PRO_EXECUTE";
                    ...
                    OracleParameter materialBeanPar = new OracleParameter("P_MATERIAL_BEAN", OracleDbType.Varchar2, ParameterDirection.Output);
                    materialBeanPar.Size = 3000;
                    ...
                    OracleParameter materialTypeIdPar = new OracleParameter("P_MATERIAL_ID", OracleDbType.Int32, sdo.MameId, ParameterDirection.Input);
                    ...
                    OracleParameter expMestMaterialIdsPar = new OracleParameter("P_EXP_MEST_MATERIAL_IDS", OracleDbType.Varchar2, 3000, expMestMaterialIds, ParameterDirection.InputOutput);

                    object resultHolder = null;

                    if (DAOWorker.SqlDAO.ExecuteStored(OutputHandler, ref resultHolder, storedSql, languageCodePar, messagePar, materialBeanPar, mediStockIdPar, materialTypeIdPar, amountPar, sessionKeyPar, orderOptPar, patientTypeIdPar, beanIdsPar, expMestMaterialIdsPar))
                    {
                        if (resultHolder != null)
                        {
                            MaterialBeanResultHolder t = (MaterialBeanResultHolder)resultHolder;
                            if (t.Message != null)
                            {
                                param.Messages.Add(t.Message);
                            }
                            result = t.MaterialBeans;
                        }
                    }
                }
            }
        ...
        /// <returns></returns>
        internal List<TakeMaterialBeanByMaterialListResultSDO> Take(List<TakeBeanByMameSDO> sdos)
        {
            List<TakeMaterialBeanByMaterialListResultSDO> result = null;
            try
            {
                if (IsNotNullOrEmpty(sdos))
                {
                    result = new List<TakeMaterialBeanByMaterialListResultSDO>();
                    foreach (TakeBeanByMameSDO sdo in sdos)
                    {
                        List<HIS_MATERIAL_BEAN> beans = this.Take(sdo);
                        if (beans != null)
                        {
                            TakeMaterialBeanByMaterialListResultSDO tmp = new TakeMaterialBeanByMaterialListResultSDO();
                            tmp.Request = sdo;
                            tmp.Result = beans;
                            result.Add(tmp);
                        }
        ...
        private void OutputHandler(ref object resultHolder, OracleParameter[] parameters)
        {
            try
            {
                ...
                {
                    List<HIS_MATERIAL_BEAN> beans = new List<HIS_MATERIAL_BEAN>();
                    ...
                    if (beanStrArr != null && beanStrArr.Length > 0)
                    {
                        foreach (string s in beanStrArr)
                        {
                            ...
                            if (fieldStrArr != null && fieldStrArr.Length > 0)
                            {
                                ...
                                bean.MATERIAL_ID = long.Parse(fieldStrArr[1]);
                                bean.TDL_MATERIAL_IMP_PRICE = decimal.Parse(fieldStrArr[2]);
                                bean.TDL_MATERIAL_IMP_VAT_RATIO = decimal.Parse(fieldStrArr[3]);
                                bean.AMOUNT = decimal.Parse(fieldStrArr[4]);
                                beans.Add(bean);
                            }
                        }
                        rs.MaterialBeans = beans;
                    }
                }
        ...
        private bool Validate(TakeBeanByMameSDO data)
        {
            bool valid = true;
            try
            {
                ...
                if (data.MameId <= 0) throw new ArgumentNullException("thieu data.MameId");
                ...
                if (HisMaterialTypeCFG.IsStent(data.MameId) && data.Amount > 1)
                {
                    MOS.MANAGER.Base.MessageUtil.SetMessage(param, MOS.LibraryMessage.Message.Enum.HisMaterialBean_StentKhongChoPhepYeuCauSoLuongLonHon1);
                    return false;
                }
            }
