...
{
    partial class HisCashierAddConfigLock : BusinessBase
    {
        internal HisCashierAddConfigLock()
            : base()
        {

        }

        internal HisCashierAddConfigLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_CASHIER_ADD_CONFIG resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_CASHIER_ADD_CONFIG data = new HisCashierAddConfigGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisCashierAddConfigDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_CASHIER_ADD_CONFIG resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_CASHIER_ADD_CONFIG resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisCashierAddConfigCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisCashierAddConfigDAO.Update(raw);
