...
{
    partial class HisExamServiceTempLock : BusinessBase
    {
        internal HisExamServiceTempLock()
            : base()
        {

        }

        internal HisExamServiceTempLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_EXAM_SERVICE_TEMP resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_EXAM_SERVICE_TEMP data = new HisExamServiceTempGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisExamServiceTempDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_EXAM_SERVICE_TEMP resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_EXAM_SERVICE_TEMP resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisExamServiceTempCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisExamServiceTempDAO.Update(raw);
