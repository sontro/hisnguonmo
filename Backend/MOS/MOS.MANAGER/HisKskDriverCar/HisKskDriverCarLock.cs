...
{
    partial class HisKskDriverCarLock : BusinessBase
    {
        internal HisKskDriverCarLock()
            : base()
        {

        }

        internal HisKskDriverCarLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_KSK_DRIVER_CAR resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_KSK_DRIVER_CAR data = new HisKskDriverCarGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisKskDriverCarDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_KSK_DRIVER_CAR resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_KSK_DRIVER_CAR resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisKskDriverCarCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisKskDriverCarDAO.Update(raw);
