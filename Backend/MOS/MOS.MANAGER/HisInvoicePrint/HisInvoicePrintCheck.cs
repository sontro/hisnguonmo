...
namespace MOS.MANAGER.HisInvoicePrint
{
    ...
    /// </summary>
    partial class HisInvoicePrintCheck : BusinessBase
    {
        internal HisInvoicePrintCheck()
            : base()
        {

        }

        internal HisInvoicePrintCheck(CommonParam paramCheck)
            : base(paramCheck) 
        {

        }

        internal bool VerifyRequireField(HIS_INVOICE_PRINT data)
        {
            bool valid = true;
            try
            {
                if (data == null) throw new ArgumentNullException("data");
                if (!IsNotNullOrEmpty(data.USERNAME)) throw new ArgumentNullException("data.USERNAME");
                if (!IsNotNullOrEmpty(data.LOGINNAME)) throw new ArgumentNullException("data.LOGINNAME");
                if (data.PRINT_TIME <= 0) throw new ArgumentNullException("data.PRINT_TIME");
                if (data.INVOICE_ID <= 0) throw new ArgumentNullException("data.INVOICE_ID");
            }
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisInvoicePrintGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_INVOICE_PRINT data)
        {
            bool valid = true;
            try
            {
                data = new HisInvoicePrintGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                ...
                {
                    HisInvoicePrintFilterQuery filter = new HisInvoicePrintFilterQuery();
                    filter.IDs = listId;
                    List<HIS_INVOICE_PRINT> listData = new HisInvoicePrintGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_INVOICE_PRINT> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_INVOICE_PRINT data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisInvoicePrintDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_INVOICE_PRINT> listData)
        {
            bool valid = true;
            try
            {
