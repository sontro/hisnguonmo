...
{
    partial class HisFuexTypeLock : BusinessBase
    {
        internal HisFuexTypeLock()
            : base()
        {

        }

        internal HisFuexTypeLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_FUEX_TYPE resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_FUEX_TYPE data = new HisFuexTypeGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisFuexTypeDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_FUEX_TYPE resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_FUEX_TYPE resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisFuexTypeCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisFuexTypeDAO.Update(raw);
