...
{
    partial class HisVaccineTypeLock : BusinessBase
    {
        internal HisVaccineTypeLock()
            : base()
        {

        }

        internal HisVaccineTypeLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_VACCINE_TYPE resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_VACCINE_TYPE data = new HisVaccineTypeGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisVaccineTypeDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_VACCINE_TYPE resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_VACCINE_TYPE resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisVaccineTypeCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisVaccineTypeDAO.Update(raw);
