...
namespace MOS.MANAGER.HisMediReactType
{
    ...
    /// </summary>
    class HisMediReactTypeCheck : BusinessBase
    {
        internal HisMediReactTypeCheck()
            : base()
        {

        }

        internal HisMediReactTypeCheck(CommonParam paramCheck)
            : base(paramCheck) 
        {

        }

        internal bool VerifyRequireField(HIS_MEDI_REACT_TYPE data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisMediReactTypeGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsGreaterThanZero(long id, ref HIS_MEDI_REACT_TYPE data)
        {
            bool valid = true;
            try
            {
                data = new HisMediReactTypeGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                ...
                {
                    HisMediReactTypeFilterQuery filter = new HisMediReactTypeFilterQuery();
                    filter.IDs = listId;
                    List<HIS_MEDI_REACT_TYPE> listData = new HisMediReactTypeGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_MEDI_REACT_TYPE> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_MEDI_REACT_TYPE data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisMediReactTypeDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_MEDI_REACT_TYPE> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool ExistsCode(string code, long? id)
        {
            ...
            try
            {
                if (DAOWorker.HisMediReactTypeDAO.ExistsCode(code, id))
                {
        ...
        internal bool CheckConstraint(long id)
        {
            ...
            try
            {
                List<HIS_MEDI_REACT> hisMediReacts = new HisMediReactGet().GetByMediReactTypeId(id);
