...
{
    class HisReceptionRoomLock : BusinessBase
    {
        internal HisReceptionRoomLock()
            : base()
        {

        }

        internal HisReceptionRoomLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_RECEPTION_ROOM data)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    data = new HisReceptionRoomGet().GetById(data.ID);
                    if (data != null)
                    {
