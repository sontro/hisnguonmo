...
{
    partial class HisEquipmentSetMatyLock : BusinessBase
    {
        internal HisEquipmentSetMatyLock()
            : base()
        {

        }

        internal HisEquipmentSetMatyLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_EQUIPMENT_SET_MATY resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_EQUIPMENT_SET_MATY data = new HisEquipmentSetMatyGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisEquipmentSetMatyDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_EQUIPMENT_SET_MATY resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_EQUIPMENT_SET_MATY resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisEquipmentSetMatyCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisEquipmentSetMatyDAO.Update(raw);
