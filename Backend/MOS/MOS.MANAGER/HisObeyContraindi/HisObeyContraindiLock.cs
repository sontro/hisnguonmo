...
{
    partial class HisObeyContraindiLock : BusinessBase
    {
        internal HisObeyContraindiLock()
            : base()
        {

        }

        internal HisObeyContraindiLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_OBEY_CONTRAINDI resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_OBEY_CONTRAINDI data = new HisObeyContraindiGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisObeyContraindiDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_OBEY_CONTRAINDI resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_OBEY_CONTRAINDI resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisObeyContraindiCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisObeyContraindiDAO.Update(raw);
