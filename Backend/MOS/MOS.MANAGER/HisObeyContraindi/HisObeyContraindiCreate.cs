...
namespace MOS.MANAGER.HisObeyContraindi
{
    partial class HisObeyContraindiCreate : BusinessBase
    {
		...
		internal bool CreateList(List<HIS_OBEY_CONTRAINDI> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    if (!DAOWorker.HisObeyContraindiDAO.CreateList(listData))
                    {
                        MOS.MANAGER.Base.BugUtil.SetBugCode(param, MOS.LibraryBug.Bug.Enum.HisObeyContraindi_ThemMoiThatBai);
                        throw new Exception("Them moi thong tin HisObeyContraindi that bai." + LogUtil.TraceData("listData", listData));
                    }
                    this.recentHisObeyContraindis.AddRange(listData);
                    result = true;
                }
            }
            catch (Exception ex)
