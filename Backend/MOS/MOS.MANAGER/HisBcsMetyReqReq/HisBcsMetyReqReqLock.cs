...
{
    partial class HisBcsMetyReqReqLock : BusinessBase
    {
        internal HisBcsMetyReqReqLock()
            : base()
        {

        }

        internal HisBcsMetyReqReqLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_BCS_METY_REQ_REQ resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_BCS_METY_REQ_REQ data = new HisBcsMetyReqReqGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisBcsMetyReqReqDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_BCS_METY_REQ_REQ resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_BCS_METY_REQ_REQ resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisBcsMetyReqReqCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisBcsMetyReqReqDAO.Update(raw);
