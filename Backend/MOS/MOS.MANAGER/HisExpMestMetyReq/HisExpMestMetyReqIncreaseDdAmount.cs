...
{
    partial class HisExpMestMetyReqIncreaseDdAmount : BusinessBase
    {
        private Dictionary<long, decimal> recentIncreaseDic;

        internal HisExpMestMetyReqIncreaseDdAmount()
            : base()
        {

        }

        internal HisExpMestMetyReqIncreaseDdAmount(CommonParam paramUpdate)
            : base(paramUpdate)
        {
        ...
        internal bool Run(Dictionary<long, decimal> increaseDic)
        {
            ...
            try
            {
                if (increaseDic != null && increaseDic.Count > 0)
                {
                    ...
                    {
                        string sql = string.Format("UPDATE HIS_EXP_MEST_METY_REQ SET DD_AMOUNT = NVL(DD_AMOUNT, 0) + {0} WHERE ID = {1}", CommonUtil.ToString(increaseDic[k]), k);
                        sqls.Add(sql);
                    }
        ...
        internal void Rollback()
        {
            try
            {
                if (this.recentIncreaseDic != null && this.recentIncreaseDic.Count > 0)
                {
                    ...
                    {
                        sqlBuilder.Append(string.Format("UPDATE HIS_EXP_MEST_METY_REQ SET DD_AMOUNT = NVL(DD_AMOUNT, 0) - {0} WHERE ID = {1};", CommonUtil.ToString(this.recentIncreaseDic[k]), k));
                    }
