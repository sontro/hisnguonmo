...
{
    partial class HisExpMestMetyReqDecreaseBcsReqAmount : BusinessBase
    {
        private Dictionary<long, decimal> recentDecreaseDic;

        internal HisExpMestMetyReqDecreaseBcsReqAmount()
            : base()
        {

        }

        internal HisExpMestMetyReqDecreaseBcsReqAmount(CommonParam paramUpdate)
            : base(paramUpdate)
        {
        ...
        internal bool Run(Dictionary<long, decimal> decreaseDic)
        {
            ...
            try
            {
                if (decreaseDic != null && decreaseDic.Count > 0)
                {
                    ...
                    {
                        string sql = string.Format("UPDATE HIS_EXP_MEST_METY_REQ SET BCS_REQ_AMOUNT = NVL(BCS_REQ_AMOUNT, 0) - {0} WHERE ID = {1}", CommonUtil.ToString(decreaseDic[k]), k);
                        sqls.Add(sql);
                    }
        ...
        internal void Rollback()
        {
            try
            {
                if (this.recentDecreaseDic != null && this.recentDecreaseDic.Count > 0)
                {
                    ...
                    {
                        sqlBuilder.Append(string.Format("UPDATE HIS_EXP_MEST_METY_REQ SET BCS_REQ_AMOUNT = NVL(BCS_REQ_AMOUNT, 0) + {0} WHERE ID = {1};", CommonUtil.ToString(this.recentDecreaseDic[k]), k));
                    }
