...
{
    partial class HisServiceSameLock : BusinessBase
    {
        internal HisServiceSameLock()
            : base()
        {

        }

        internal HisServiceSameLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_SERVICE_SAME resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_SERVICE_SAME data = new HisServiceSameGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisServiceSameDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_SERVICE_SAME resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_SERVICE_SAME resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisServiceSameCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisServiceSameDAO.Update(raw);
