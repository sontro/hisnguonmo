...
{
    partial class HisVaccinationVrtyLock : BusinessBase
    {
        internal HisVaccinationVrtyLock()
            : base()
        {

        }

        internal HisVaccinationVrtyLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_VACCINATION_VRTY resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_VACCINATION_VRTY data = new HisVaccinationVrtyGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisVaccinationVrtyDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_VACCINATION_VRTY resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_VACCINATION_VRTY resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisVaccinationVrtyCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisVaccinationVrtyDAO.Update(raw);
