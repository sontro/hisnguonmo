...
{
    partial class HisHoreDhtyLock : BusinessBase
    {
        internal HisHoreDhtyLock()
            : base()
        {

        }

        internal HisHoreDhtyLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_HORE_DHTY resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_HORE_DHTY data = new HisHoreDhtyGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisHoreDhtyDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_HORE_DHTY resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_HORE_DHTY resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisHoreDhtyCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisHoreDhtyDAO.Update(raw);
