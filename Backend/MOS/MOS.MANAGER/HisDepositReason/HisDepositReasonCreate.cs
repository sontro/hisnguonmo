...
namespace MOS.MANAGER.HisDepositReason
{
    partial class HisDepositReasonCreate : BusinessBase
    {
		...
		internal bool CreateList(List<HIS_DEPOSIT_REASON> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    if (!DAOWorker.HisDepositReasonDAO.CreateList(listData))
                    {
                        MOS.MANAGER.Base.BugUtil.SetBugCode(param, MOS.LibraryBug.Bug.Enum.HisDepositReason_ThemMoiThatBai);
                        throw new Exception("Them moi thong tin HisDepositReason that bai." + LogUtil.TraceData("listData", listData));
                    }
                    this.recentHisDepositReasons.AddRange(listData);
                    result = true;
                }
            }
            catch (Exception ex)
