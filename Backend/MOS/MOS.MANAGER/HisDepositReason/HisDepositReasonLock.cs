...
{
    partial class HisDepositReasonLock : BusinessBase
    {
        internal HisDepositReasonLock()
            : base()
        {

        }

        internal HisDepositReasonLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_DEPOSIT_REASON resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_DEPOSIT_REASON data = new HisDepositReasonGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisDepositReasonDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_DEPOSIT_REASON resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_DEPOSIT_REASON resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisDepositReasonCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisDepositReasonDAO.Update(raw);
