...
{
    partial class HisVaccAppointmentLock : BusinessBase
    {
        internal HisVaccAppointmentLock()
            : base()
        {

        }

        internal HisVaccAppointmentLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_VACC_APPOINTMENT resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_VACC_APPOINTMENT data = new HisVaccAppointmentGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisVaccAppointmentDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_VACC_APPOINTMENT resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_VACC_APPOINTMENT resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisVaccAppointmentCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisVaccAppointmentDAO.Update(raw);
