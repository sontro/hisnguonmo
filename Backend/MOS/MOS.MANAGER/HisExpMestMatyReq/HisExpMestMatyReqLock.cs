...
{
    partial class HisExpMestMatyReqLock : BusinessBase
    {
        internal HisExpMestMatyReqLock()
            : base()
        {

        }

        internal HisExpMestMatyReqLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_EXP_MEST_MATY_REQ resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_EXP_MEST_MATY_REQ data = new HisExpMestMatyReqGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisExpMestMatyReqDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_EXP_MEST_MATY_REQ resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_EXP_MEST_MATY_REQ resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisExpMestMatyReqCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisExpMestMatyReqDAO.Update(raw);
