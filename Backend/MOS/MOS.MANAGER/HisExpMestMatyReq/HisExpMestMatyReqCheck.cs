...
{
    partial class HisExpMestMatyReqCheck : BusinessBase
    {
        internal HisExpMestMatyReqCheck()
            : base()
        {

        }

        internal HisExpMestMatyReqCheck(CommonParam paramCheck)
            : base(paramCheck) 
        {

        }

        internal bool VerifyRequireField(HIS_EXP_MEST_MATY_REQ data)
        {
            bool valid = true;
            try
            {
                if (data == null) throw new ArgumentNullException("data");
                if (data.MATERIAL_TYPE_ID <= 0) throw new ArgumentNullException("data.MEDICINE_TYPE_ID");
                if (data.TDL_MEDI_STOCK_ID <= 0) throw new ArgumentNullException("data.TDL_MEDI_STOCK_ID");
                if (data.AMOUNT <= 0) throw new ArgumentNullException("data.AMOUNT");
                if (data.EXP_MEST_ID <= 0) throw new ArgumentNullException("data.EXP_MEST_ID");
            }
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisExpMestMatyReqGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_EXP_MEST_MATY_REQ data)
        {
            bool valid = true;
            try
            {
                data = new HisExpMestMatyReqGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                if (listId != null && listId.Count > 0)
                {
                    ...
                    filter.IDs = listId;
                    List<HIS_EXP_MEST_MATY_REQ> listData = new HisExpMestMatyReqGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_EXP_MEST_MATY_REQ> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_EXP_MEST_MATY_REQ data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisExpMestMatyReqDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_EXP_MEST_MATY_REQ> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool CheckConstraint(long id)
        {
            ...
            try
            {
                /*List<XXX> hisXXXs = new HisXXXGet().GetByHisExpMestMatyReqId(id);
