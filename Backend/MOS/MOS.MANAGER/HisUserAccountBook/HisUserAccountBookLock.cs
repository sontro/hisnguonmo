...
{
    partial class HisUserAccountBookLock : BusinessBase
    {
        internal HisUserAccountBookLock()
            : base()
        {

        }

        internal HisUserAccountBookLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_USER_ACCOUNT_BOOK resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_USER_ACCOUNT_BOOK data = new HisUserAccountBookGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisUserAccountBookDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_USER_ACCOUNT_BOOK resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_USER_ACCOUNT_BOOK resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisUserAccountBookCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisUserAccountBookDAO.Update(raw);
