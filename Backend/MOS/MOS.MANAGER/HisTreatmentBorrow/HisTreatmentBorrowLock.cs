...
{
    partial class HisTreatmentBorrowLock : BusinessBase
    {
        internal HisTreatmentBorrowLock()
            : base()
        {

        }

        internal HisTreatmentBorrowLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_TREATMENT_BORROW resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_TREATMENT_BORROW data = new HisTreatmentBorrowGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisTreatmentBorrowDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_TREATMENT_BORROW resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_TREATMENT_BORROW resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisTreatmentBorrowCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisTreatmentBorrowDAO.Update(raw);
