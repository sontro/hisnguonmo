...
{
    class HisExecuteGroupLock : BusinessBase
    {
        internal HisExecuteGroupLock()
            : base()
        {

        }

        internal HisExecuteGroupLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_EXECUTE_GROUP data)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    data = new HisExecuteGroupGet().GetById(data.ID);
                    if (data != null)
                    {
