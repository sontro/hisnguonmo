...
{
    partial class HisMachineServMatyCheck : BusinessBase
    {
        internal HisMachineServMatyCheck()
            : base()
        {

        }

        internal HisMachineServMatyCheck(CommonParam paramCheck)
            : base(paramCheck) 
        {

        }

        internal bool VerifyRequireField(HIS_MACHINE_SERV_MATY data)
        {
            bool valid = true;
            try
            {
                if (data == null) throw new ArgumentNullException("data");
                if (data.MACHINE_ID <= 0) throw new ArgumentNullException("data.MACHINE_ID");
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisMachineServMatyGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_MACHINE_SERV_MATY data)
        {
            bool valid = true;
            try
            {
                data = new HisMachineServMatyGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                ...
                {
                    HisMachineServMatyFilterQuery filter = new HisMachineServMatyFilterQuery();
                    filter.IDs = listId;
                    List<HIS_MACHINE_SERV_MATY> listData = new HisMachineServMatyGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_MACHINE_SERV_MATY> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_MACHINE_SERV_MATY data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisMachineServMatyDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_MACHINE_SERV_MATY> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool CheckConstraint(long id)
        {
            ...
            try
            {
                /*List<XXX> hisXXXs = new HisXXXGet().GetByHisMachineServMatyId(id);
