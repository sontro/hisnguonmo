...
{
    partial class HisMachineServMatyLock : BusinessBase
    {
        internal HisMachineServMatyLock()
            : base()
        {

        }

        internal HisMachineServMatyLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_MACHINE_SERV_MATY resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_MACHINE_SERV_MATY data = new HisMachineServMatyGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisMachineServMatyDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_MACHINE_SERV_MATY resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_MACHINE_SERV_MATY resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisMachineServMatyCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisMachineServMatyDAO.Update(raw);
