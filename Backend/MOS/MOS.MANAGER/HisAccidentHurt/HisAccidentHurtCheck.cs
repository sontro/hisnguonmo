...
namespace MOS.MANAGER.HisAccidentHurt
{
    ...
    /// </summary>
    partial class HisAccidentHurtCheck : BusinessBase
    {
        internal HisAccidentHurtCheck()
            : base()
        {

        }

        internal HisAccidentHurtCheck(CommonParam paramCheck)
            : base(paramCheck) 
        {

        }

        internal bool VerifyRequireField(HIS_ACCIDENT_HURT data)
        {
            bool valid = true;
            try
            {
                if (data == null) throw new ArgumentNullException("data");
                if (data.ACCIDENT_HURT_TYPE_ID <= 0) throw new ArgumentNullException("data.ACCIDENT_HURT_TYPE_ID");
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisAccidentHurtGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_ACCIDENT_HURT data)
        {
            bool valid = true;
            try
            {
                data = new HisAccidentHurtGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                if (listId != null && listId.Count > 0)
                {
                    ...
                    filter.IDs = listId;
                    List<HIS_ACCIDENT_HURT> listData = new HisAccidentHurtGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_ACCIDENT_HURT> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_ACCIDENT_HURT data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisAccidentHurtDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_ACCIDENT_HURT> listData)
        {
            bool valid = true;
            try
            {
