...
{
    partial class HisMrCheckSummaryLock : BusinessBase
    {
        internal HisMrCheckSummaryLock()
            : base()
        {

        }

        internal HisMrCheckSummaryLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_MR_CHECK_SUMMARY resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_MR_CHECK_SUMMARY data = new HisMrCheckSummaryGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisMrCheckSummaryDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_MR_CHECK_SUMMARY resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_MR_CHECK_SUMMARY resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisMrCheckSummaryCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisMrCheckSummaryDAO.Update(raw);
