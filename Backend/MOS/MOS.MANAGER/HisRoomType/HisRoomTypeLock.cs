...
{
    class HisRoomTypeLock : BusinessBase
    {
        internal HisRoomTypeLock()
            : base()
        {

        }

        internal HisRoomTypeLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_ROOM_TYPE data)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    data = new HisRoomTypeGet().GetById(data.ID);
                    if (data != null)
                    {
