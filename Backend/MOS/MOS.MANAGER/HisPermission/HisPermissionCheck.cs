...
{
    partial class HisPermissionCheck : BusinessBase
    {
        internal HisPermissionCheck()
            : base()
        {

        }

        internal HisPermissionCheck(CommonParam paramCheck)
            : base(paramCheck) 
        {

        }

        internal bool VerifyRequireField(HIS_PERMISSION data)
        {
            bool valid = true;
            try
            {
                ...
                if (data.PERMISSION_TYPE_ID <= 0) throw new ArgumentNullException("data.PERMISSION_TYPE_ID");
                if (data.EFFECTIVE_DATE <= 0) throw new ArgumentNullException("data.EFFECTIVE_DATE");
            }
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisPermissionGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_PERMISSION data)
        {
            bool valid = true;
            try
            {
                data = new HisPermissionGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                ...
                {
                    HisPermissionFilterQuery filter = new HisPermissionFilterQuery();
                    filter.IDs = listId;
                    List<HIS_PERMISSION> listData = new HisPermissionGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_PERMISSION> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_PERMISSION data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisPermissionDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_PERMISSION> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool CheckConstraint(long id)
        {
            ...
            try
            {
                /*List<XXX> hisXXXs = new HisXXXGet().GetByHisPermissionId(id);
