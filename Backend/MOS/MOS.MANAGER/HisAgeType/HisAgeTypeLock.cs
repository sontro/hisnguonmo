...
{
    partial class HisAgeTypeLock : BusinessBase
    {
        internal HisAgeTypeLock()
            : base()
        {

        }

        internal HisAgeTypeLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_AGE_TYPE resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_AGE_TYPE data = new HisAgeTypeGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisAgeTypeDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_AGE_TYPE resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_AGE_TYPE resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisAgeTypeCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisAgeTypeDAO.Update(raw);
