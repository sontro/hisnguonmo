...
namespace MOS.MANAGER.HisAgeType
{
    partial class HisAgeTypeCreate : BusinessBase
    {
		...
		internal bool CreateList(List<HIS_AGE_TYPE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    if (!DAOWorker.HisAgeTypeDAO.CreateList(listData))
                    {
                        MOS.MANAGER.Base.BugUtil.SetBugCode(param, MOS.LibraryBug.Bug.Enum.HisAgeType_ThemMoiThatBai);
                        throw new Exception("Them moi thong tin HisAgeType that bai." + LogUtil.TraceData("listData", listData));
                    }
                    this.recentHisAgeTypes.AddRange(listData);
                    result = true;
                }
            }
            catch (Exception ex)
