...
{
    class HisTranPatiReasonCheck : BusinessBase
    {
        internal HisTranPatiReasonCheck()
            : base()
        {

        }

        internal HisTranPatiReasonCheck(Inventec.Core.CommonParam paramCheck)
            : base(paramCheck) 
        {

        }

        internal bool VerifyRequireField(HIS_TRAN_PATI_REASON data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool ExistsCode(string code, long? id)
        {
            ...
            try
            {
                if (DAOWorker.HisTranPatiReasonDAO.ExistsCode(code, id))
                {
        ...
        internal bool IsUnLock(HIS_TRAN_PATI_REASON data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisTranPatiReasonDAO.IsUnLock(id))
                {
                    valid = false;
        ...
        internal bool CheckConstraint(long id)
        {
            ...
            try
            {
                List<HIS_TREATMENT> hisTreatments = new HisTreatmentGet().GetByTranPatiReasonId(id);
