...
{
    partial class HisServiceMachineLock : BusinessBase
    {
        internal HisServiceMachineLock()
            : base()
        {

        }

        internal HisServiceMachineLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_SERVICE_MACHINE resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_SERVICE_MACHINE data = new HisServiceMachineGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisServiceMachineDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_SERVICE_MACHINE resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_SERVICE_MACHINE resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisServiceMachineCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisServiceMachineDAO.Update(raw);
