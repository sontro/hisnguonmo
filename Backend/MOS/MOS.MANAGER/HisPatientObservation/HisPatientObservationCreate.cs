...
namespace MOS.MANAGER.HisPatientObservation
{
    partial class HisPatientObservationCreate : BusinessBase
    {
		...
		internal bool CreateList(List<HIS_PATIENT_OBSERVATION> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    if (!DAOWorker.HisPatientObservationDAO.CreateList(listData))
                    {
                        MOS.MANAGER.Base.BugUtil.SetBugCode(param, MOS.LibraryBug.Bug.Enum.HisPatientObservation_ThemMoiThatBai);
                        throw new Exception("Them moi thong tin HisPatientObservation that bai." + LogUtil.TraceData("listData", listData));
                    }
                    this.recentHisPatientObservations.AddRange(listData);
                    result = true;
                }
            }
            catch (Exception ex)
