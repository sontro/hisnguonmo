...
{
    partial class HisPatientObservationLock : BusinessBase
    {
        internal HisPatientObservationLock()
            : base()
        {

        }

        internal HisPatientObservationLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_PATIENT_OBSERVATION resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_PATIENT_OBSERVATION data = new HisPatientObservationGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisPatientObservationDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_PATIENT_OBSERVATION resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_PATIENT_OBSERVATION resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisPatientObservationCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisPatientObservationDAO.Update(raw);
