...
{
    partial class HisPaanPositionLock : BusinessBase
    {
        internal HisPaanPositionLock()
            : base()
        {

        }

        internal HisPaanPositionLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_PAAN_POSITION resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_PAAN_POSITION data = new HisPaanPositionGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisPaanPositionDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_PAAN_POSITION resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_PAAN_POSITION resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisPaanPositionCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisPaanPositionDAO.Update(raw);
