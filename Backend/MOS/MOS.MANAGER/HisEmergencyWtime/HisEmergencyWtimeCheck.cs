...
{
    class HisEmergencyWtimeCheck : BusinessBase
    {
        internal HisEmergencyWtimeCheck()
            : base()
        {

        }

        internal HisEmergencyWtimeCheck(CommonParam paramCheck)
            : base(paramCheck) 
        {

        }

        internal bool VerifyRequireField(HIS_EMERGENCY_WTIME data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool ExistsCode(string code, long? id)
        {
            ...
            try
            {
                if (DAOWorker.HisEmergencyWtimeDAO.ExistsCode(code, id))
                {
        ...
        internal bool IsUnLock(HIS_EMERGENCY_WTIME data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisEmergencyWtimeDAO.IsUnLock(id))
                {
                    valid = false;
                    MOS.MANAGER.Base.BugUtil.SetBugCode(param, MOS.LibraryBug.Bug.Enum.DuLieuDangBiKhoa);
                }
        ...
        internal bool CheckConstraint(long id)
        {
            ...
            try
            {
                List<HIS_TREATMENT> hisTreatments = new HisTreatmentGet().GetByEmergencyWtimeId(id);
