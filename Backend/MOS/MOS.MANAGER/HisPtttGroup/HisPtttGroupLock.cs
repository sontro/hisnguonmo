...
{
    partial class HisPtttGroupLock : BusinessBase
    {
        internal HisPtttGroupLock()
            : base()
        {

        }

        internal HisPtttGroupLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_PTTT_GROUP data)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisPtttGroupCheck().VerifyId(data.ID, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisPtttGroupDAO.Update(raw);
