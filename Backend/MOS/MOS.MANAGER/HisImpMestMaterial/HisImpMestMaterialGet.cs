...
namespace MOS.MANAGER.HisImpMestMaterial
{
    partial class HisImpMestMaterialGet : GetBase
    {
        ...
        internal List<HisImpMestMaterialWithInStockAmountSDO> GetViewWithInStockAmount(long impMestId)
        {
            try
            {
                List<HisImpMestMaterialWithInStockAmountSDO> result = null;
                List<V_HIS_IMP_MEST_MATERIAL> impMestMaterials = this.GetViewByImpMestId(impMestId);
                if (IsNotNullOrEmpty(impMestMaterials))
                {
                    Mapper.CreateMap<V_HIS_IMP_MEST_MATERIAL, HisImpMestMaterialWithInStockAmountSDO>();
                    result = Mapper.Map<List<HisImpMestMaterialWithInStockAmountSDO>>(impMestMaterials);

                    HisMaterialStockViewFilter stockViewFilter = new HisMaterialStockViewFilter();
                    //imp_mest_material cung imp_mest_id thi co cung medi_stock_id
                    stockViewFilter.MEDI_STOCK_ID = impMestMaterials[0].MEDI_STOCK_ID;
                    stockViewFilter.IDs = impMestMaterials.Select(o => o.MATERIAL_ID).ToList();
                    List<HisMaterialInStockSDO> inStockMaterials = new HisMaterialGet(param).GetInStockMaterial(stockViewFilter);

                    if (inStockMaterials != null)
                    {
                        foreach (HisImpMestMaterialWithInStockAmountSDO sdo in result)
                        {
                            HisMaterialInStockSDO materialSDO = inStockMaterials.Where(o => o.ID == sdo.MATERIAL_ID).FirstOrDefault();
                            sdo.InStockAmount = materialSDO != null ? materialSDO.TotalAmount : 0;
                            sdo.AvailableAmount = materialSDO != null ? materialSDO.AvailableAmount : 0;
                        }
                    }
                }
                return result;
        ...
        internal List<V_HIS_IMP_MEST_MATERIAL> GetViewByMaterialId(long id)
        {
            HisImpMestMaterialViewFilterQuery filter = new HisImpMestMaterialViewFilterQuery();
            filter.MATERIAL_ID = id;
            return this.GetView(filter);
