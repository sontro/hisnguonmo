...
{
    partial class HisOweTypeLock : BusinessBase
    {
        internal HisOweTypeLock()
            : base()
        {

        }

        internal HisOweTypeLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_OWE_TYPE resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_OWE_TYPE data = new HisOweTypeGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisOweTypeDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_OWE_TYPE resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_OWE_TYPE resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisOweTypeCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisOweTypeDAO.Update(raw);
