...
{
    partial class HisMediRecordLock : BusinessBase
    {
        internal HisMediRecordLock()
            : base()
        {

        }

        internal HisMediRecordLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_MEDI_RECORD resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_MEDI_RECORD data = new HisMediRecordGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisMediRecordDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_MEDI_RECORD resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_MEDI_RECORD resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisMediRecordCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisMediRecordDAO.Update(raw);
