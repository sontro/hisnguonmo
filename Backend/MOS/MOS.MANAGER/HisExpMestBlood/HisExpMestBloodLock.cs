...
{
    partial class HisExpMestBloodLock : BusinessBase
    {
        internal HisExpMestBloodLock()
            : base()
        {

        }

        internal HisExpMestBloodLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_EXP_MEST_BLOOD resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_EXP_MEST_BLOOD data = new HisExpMestBloodGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisExpMestBloodDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_EXP_MEST_BLOOD resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(HIS_EXP_MEST_BLOOD data)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisExpMestBloodCheck().VerifyId(data.ID, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisExpMestBloodDAO.Update(raw);
