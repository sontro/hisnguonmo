...
{
    partial class HisAntibioticMicrobiLock : BusinessBase
    {
        internal HisAntibioticMicrobiLock()
            : base()
        {

        }

        internal HisAntibioticMicrobiLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_ANTIBIOTIC_MICROBI resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_ANTIBIOTIC_MICROBI data = new HisAntibioticMicrobiGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisAntibioticMicrobiDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_ANTIBIOTIC_MICROBI resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_ANTIBIOTIC_MICROBI resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisAntibioticMicrobiCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisAntibioticMicrobiDAO.Update(raw);
