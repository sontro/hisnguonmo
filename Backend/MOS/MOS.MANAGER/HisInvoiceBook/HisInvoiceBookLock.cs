...
{
    partial class HisInvoiceBookLock : BusinessBase
    {
        internal HisInvoiceBookLock()
            : base()
        {

        }

        internal HisInvoiceBookLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_INVOICE_BOOK data)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisInvoiceBookCheck().VerifyId(data.ID, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisInvoiceBookDAO.Update(raw);
