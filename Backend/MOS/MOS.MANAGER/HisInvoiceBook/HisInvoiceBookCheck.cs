...
namespace MOS.MANAGER.HisInvoiceBook
{
    ...
    /// </summary>
    partial class HisInvoiceBookCheck : BusinessBase
    {
        internal HisInvoiceBookCheck()
            : base()
        {

        }

        internal HisInvoiceBookCheck(CommonParam paramCheck)
            : base(paramCheck)
        {

        }

        internal bool VerifyRequireField(HIS_INVOICE_BOOK data)
        {
            bool valid = true;
            try
            {
                ...
                if (!IsNotNullOrEmpty(data.SYMBOL_CODE)) throw new ArgumentNullException("data.SYMBOL_CODE");
                if (!IsNotNullOrEmpty(data.TEMPLATE_CODE)) throw new ArgumentNullException("data.TEMPLATE_CODE");
            }
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisInvoiceBookGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_INVOICE_BOOK data)
        {
            bool valid = true;
            try
            {
                data = new HisInvoiceBookGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                if (listId != null && listId.Count > 0)
                {
                    ...
                    filter.IDs = listId;
                    List<HIS_INVOICE_BOOK> listData = new HisInvoiceBookGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_INVOICE_BOOK> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_INVOICE_BOOK data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisInvoiceBookDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_INVOICE_BOOK> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsNotExistInvoice(HIS_INVOICE_BOOK data)
        {
            bool valid = true;
            try
            {
                List<HIS_INVOICE> existedInvoices = new HisInvoiceGet().GetByInvoiceBookId(data.ID);
                if (IsNotNullOrEmpty(existedInvoices))
                {
                    MOS.MANAGER.Base.MessageUtil.SetMessage(param, MOS.LibraryMessage.Message.Enum.HisInvoiceBook_KhongDuocCapNhatViDaCoGiaoDich);
                    valid = false;
                }
            }
            catch (Exception ex)
        ...
        internal bool CheckConstraint(long id)
        {
            ...
            try
            {
                List<HIS_INVOICE> hisInvoices = new HisInvoiceGet().GetByInvoiceBookId(id);
                if (IsNotNullOrEmpty(hisInvoices))
                {
                    MOS.MANAGER.Base.MessageUtil.SetMessage(param, MOS.LibraryMessage.Message.Enum.HisInvoice_TonTaiDuLieu);
                    throw new Exception("Ton tai du lieu HIS_INVOICE, khong cho phep xoa" + LogUtil.TraceData("id", id));
                }
            }
            catch (Exception ex)
            {
