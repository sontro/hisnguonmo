...
{
    partial class HisWorkPlaceLock : BusinessBase
    {
        internal HisWorkPlaceLock()
            : base()
        {

        }

        internal HisWorkPlaceLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_WORK_PLACE data)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisWorkPlaceCheck().VerifyId(data.ID, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisWorkPlaceDAO.Update(raw);
