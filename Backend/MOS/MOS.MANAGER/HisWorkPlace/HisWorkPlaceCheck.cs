...
namespace MOS.MANAGER.HisWorkPlace
{
    ...
    /// </summary>
    partial class HisWorkPlaceCheck : BusinessBase
    {
        internal HisWorkPlaceCheck()
            : base()
        {

        }

        internal HisWorkPlaceCheck(CommonParam paramCheck)
            : base(paramCheck) 
        {

        }

        internal bool VerifyRequireField(HIS_WORK_PLACE data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisWorkPlaceGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_WORK_PLACE data)
        {
            bool valid = true;
            try
            {
                data = new HisWorkPlaceGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                ...
                {
                    HisWorkPlaceFilterQuery filter = new HisWorkPlaceFilterQuery();
                    filter.IDs = listId;
                    List<HIS_WORK_PLACE> listData = new HisWorkPlaceGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_WORK_PLACE> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_WORK_PLACE data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisWorkPlaceDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_WORK_PLACE> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool ExistsCode(string code, long? id)
        {
            ...
            try
            {
                if (DAOWorker.HisWorkPlaceDAO.ExistsCode(code, id))
                {
        ...
        internal bool CheckConstraint(long id)
        {
            ...
            try
            {
                List<HIS_PATIENT> hisPatients = new HisPatientGet().GetByWorkPlaceId(id);
                if (IsNotNullOrEmpty(hisPatients))
                {
                    ...
                    throw new Exception("Ton tai du lieu HIS_PATIENT, khong cho phep xoa" + LogUtil.TraceData("id", id));
                }
            }
            catch (Exception ex)
            {
