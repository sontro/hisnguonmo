...
{
    partial class HisCarerCardLock : BusinessBase
    {
        internal HisCarerCardLock()
            : base()
        {

        }

        internal HisCarerCardLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_CARER_CARD resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_CARER_CARD data = new HisCarerCardGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisCarerCardDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_CARER_CARD resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_CARER_CARD resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisCarerCardCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisCarerCardDAO.Update(raw);
