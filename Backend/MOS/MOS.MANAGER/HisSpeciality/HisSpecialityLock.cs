...
{
    partial class HisSpecialityLock : BusinessBase
    {
        internal HisSpecialityLock()
            : base()
        {

        }

        internal HisSpecialityLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_SPECIALITY resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_SPECIALITY data = new HisSpecialityGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisSpecialityDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_SPECIALITY resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_SPECIALITY resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisSpecialityCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisSpecialityDAO.Update(raw);
