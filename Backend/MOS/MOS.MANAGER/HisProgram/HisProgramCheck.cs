...
namespace MOS.MANAGER.HisProgram
{
    ...
    /// </summary>
    partial class HisProgramCheck : BusinessBase
    {
        internal HisProgramCheck()
            : base()
        {

        }

        internal HisProgramCheck(CommonParam paramCheck)
            : base(paramCheck) 
        {

        }

        internal bool VerifyRequireField(HIS_PROGRAM data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisProgramGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_PROGRAM data)
        {
            bool valid = true;
            try
            {
                data = new HisProgramGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                ...
                {
                    HisProgramFilterQuery filter = new HisProgramFilterQuery();
                    filter.IDs = listId;
                    List<HIS_PROGRAM> listData = new HisProgramGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_PROGRAM> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_PROGRAM data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisProgramDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_PROGRAM> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool CheckConstraint(long id)
        {
            ...
            try
            {
                List<HIS_MEDI_RECORD> hisMediRecords = new HisMediRecordGet().GetByProgramId(id);
                if (IsNotNullOrEmpty(hisMediRecords))
                {
                    MOS.MANAGER.Base.MessageUtil.SetMessage(param, MOS.LibraryMessage.Message.Enum.HisMediRecord_TonTaiDuLieu);
                    throw new Exception("Ton tai du lieu HIS_MEDI_RECORD, khong cho phep xoa" + LogUtil.TraceData("id", id));
                }
                List<HIS_PATIENT_PROGRAM> hisPatientPrograms = new HisPatientProgramGet().GetByProgramId(id);
                if (IsNotNullOrEmpty(hisPatientPrograms))
                {
                    MOS.MANAGER.Base.MessageUtil.SetMessage(param, MOS.LibraryMessage.Message.Enum.HisPatientProgram_TonTaiDuLieu);
                    throw new Exception("Ton tai du lieu HIS_PATIENT_PROGRAM, khong cho phep xoa" + LogUtil.TraceData("id", id));
                }
            }
            catch (Exception ex)
            {
