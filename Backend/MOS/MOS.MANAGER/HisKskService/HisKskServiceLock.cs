...
{
    partial class HisKskServiceLock : BusinessBase
    {
        internal HisKskServiceLock()
            : base()
        {

        }

        internal HisKskServiceLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_KSK_SERVICE resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_KSK_SERVICE data = new HisKskServiceGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisKskServiceDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_KSK_SERVICE resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_KSK_SERVICE resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisKskServiceCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisKskServiceDAO.Update(raw);
