...
{
    partial class HisVaccinationExamLock : BusinessBase
    {
        internal HisVaccinationExamLock()
            : base()
        {

        }

        internal HisVaccinationExamLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_VACCINATION_EXAM resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_VACCINATION_EXAM data = new HisVaccinationExamGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisVaccinationExamDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_VACCINATION_EXAM resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_VACCINATION_EXAM resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisVaccinationExamCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
