...
{
    partial class HisBloodVolumeLock : BusinessBase
    {
        internal HisBloodVolumeLock()
            : base()
        {

        }

        internal HisBloodVolumeLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_BLOOD_VOLUME resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_BLOOD_VOLUME data = new HisBloodVolumeGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisBloodVolumeDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_BLOOD_VOLUME resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(HIS_BLOOD_VOLUME data)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisBloodVolumeCheck().VerifyId(data.ID, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisBloodVolumeDAO.Update(raw);
