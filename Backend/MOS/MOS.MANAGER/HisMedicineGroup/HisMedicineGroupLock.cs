...
{
    partial class HisMedicineGroupLock : BusinessBase
    {
        internal HisMedicineGroupLock()
            : base()
        {

        }

        internal HisMedicineGroupLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_MEDICINE_GROUP resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_MEDICINE_GROUP data = new HisMedicineGroupGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisMedicineGroupDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_MEDICINE_GROUP resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_MEDICINE_GROUP resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisMedicineGroupCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisMedicineGroupDAO.Update(raw);
