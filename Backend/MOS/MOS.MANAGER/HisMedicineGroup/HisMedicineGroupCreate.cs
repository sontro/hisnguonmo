...
namespace MOS.MANAGER.HisMedicineGroup
{
    partial class HisMedicineGroupCreate : BusinessBase
    {
		...
		internal bool CreateList(List<HIS_MEDICINE_GROUP> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    if (!DAOWorker.HisMedicineGroupDAO.CreateList(listData))
                    {
                        MOS.MANAGER.Base.BugUtil.SetBugCode(param, MOS.LibraryBug.Bug.Enum.HisMedicineGroup_ThemMoiThatBai);
                        throw new Exception("Them moi thong tin HisMedicineGroup that bai." + LogUtil.TraceData("listData", listData));
                    }
                    this.recentHisMedicineGroups.AddRange(listData);
                    result = true;
                }
            }
            catch (Exception ex)
