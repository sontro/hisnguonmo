...
{
    partial class HisMetyMetyLock : BusinessBase
    {
        internal HisMetyMetyLock()
            : base()
        {

        }

        internal HisMetyMetyLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_METY_METY resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_METY_METY data = new HisMetyMetyGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisMetyMetyDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_METY_METY resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_METY_METY resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisMetyMetyCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisMetyMetyDAO.Update(raw);
