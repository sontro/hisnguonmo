...
{
    partial class HisSereServMatyLock : BusinessBase
    {
        internal HisSereServMatyLock()
            : base()
        {

        }

        internal HisSereServMatyLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_SERE_SERV_MATY resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_SERE_SERV_MATY data = new HisSereServMatyGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisSereServMatyDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_SERE_SERV_MATY resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_SERE_SERV_MATY resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisSereServMatyCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisSereServMatyDAO.Update(raw);
