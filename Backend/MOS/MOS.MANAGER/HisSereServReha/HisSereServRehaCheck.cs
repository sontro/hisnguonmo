...
namespace MOS.MANAGER.HisSereServReha
{
    ...
    /// </summary>
    partial class HisSereServRehaCheck : BusinessBase
    {
        internal HisSereServRehaCheck()
            : base()
        {

        }

        internal HisSereServRehaCheck(CommonParam paramCheck)
            : base(paramCheck) 
        {

        }

        internal bool VerifyRequireField(HIS_SERE_SERV_REHA data)
        {
            bool valid = true;
            try
            {
                if (data == null) throw new ArgumentNullException("data");
                if (data.REHA_TRAIN_TYPE_ID <= 0) throw new ArgumentNullException("data.REHA_TRAIN_TYPE_ID");
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisSereServRehaGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_SERE_SERV_REHA data)
        {
            bool valid = true;
            try
            {
                data = new HisSereServRehaGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                if (listId != null && listId.Count > 0)
                {
                    ...
                    filter.IDs = listId;
                    List<HIS_SERE_SERV_REHA> listData = new HisSereServRehaGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_SERE_SERV_REHA> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_SERE_SERV_REHA data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisSereServRehaDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_SERE_SERV_REHA> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsNotAdded(HIS_SERE_SERV_REHA data)
        {
            if (data != null)
            {
                List<HIS_SERE_SERV_REHA> ss = new HisSereServRehaGet().GetBySereServIdAndRehaTrainTypeId(data.SERE_SERV_ID, data.REHA_TRAIN_TYPE_ID);
                return !IsNotNullOrEmpty(ss);
            }
            return true;
        }
    }
