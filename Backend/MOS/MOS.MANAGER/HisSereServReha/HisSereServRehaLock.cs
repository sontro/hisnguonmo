...
{
    partial class HisSereServRehaLock : BusinessBase
    {
        internal HisSereServRehaLock()
            : base()
        {

        }

        internal HisSereServRehaLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_SERE_SERV_REHA data)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisSereServRehaCheck().VerifyId(data.ID, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisSereServRehaDAO.Update(raw);
