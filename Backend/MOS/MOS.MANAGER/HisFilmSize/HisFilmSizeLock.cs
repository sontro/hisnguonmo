...
{
    partial class HisFilmSizeLock : BusinessBase
    {
        internal HisFilmSizeLock()
            : base()
        {

        }

        internal HisFilmSizeLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_FILM_SIZE resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_FILM_SIZE data = new HisFilmSizeGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisFilmSizeDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_FILM_SIZE resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_FILM_SIZE resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisFilmSizeCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisFilmSizeDAO.Update(raw);
