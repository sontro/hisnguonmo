...
{
    partial class HisBedTypeLock : BusinessBase
    {
        internal HisBedTypeLock()
            : base()
        {

        }

        internal HisBedTypeLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_BED_TYPE resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_BED_TYPE data = new HisBedTypeGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisBedTypeDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_BED_TYPE resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(HIS_BED_TYPE data)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisBedTypeCheck().VerifyId(data.ID, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisBedTypeDAO.Update(raw);
