...
{
    partial class HisVaccReactPlaceLock : BusinessBase
    {
        internal HisVaccReactPlaceLock()
            : base()
        {

        }

        internal HisVaccReactPlaceLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_VACC_REACT_PLACE resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_VACC_REACT_PLACE data = new HisVaccReactPlaceGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisVaccReactPlaceDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_VACC_REACT_PLACE resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_VACC_REACT_PLACE resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisVaccReactPlaceCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisVaccReactPlaceDAO.Update(raw);
