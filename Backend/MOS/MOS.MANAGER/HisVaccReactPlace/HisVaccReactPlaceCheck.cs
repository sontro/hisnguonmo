...
{
    partial class HisVaccReactPlaceCheck : BusinessBase
    {
        internal HisVaccReactPlaceCheck()
            : base()
        {

        }

        internal HisVaccReactPlaceCheck(CommonParam paramCheck)
            : base(paramCheck) 
        {

        }

        internal bool VerifyRequireField(HIS_VACC_REACT_PLACE data)
        {
            bool valid = true;
            try
            {
                if (data == null) throw new ArgumentNullException("data");
                if (String.IsNullOrWhiteSpace(data.VACC_REACT_PLACE_CODE)) throw new ArgumentNullException("data.VACC_REACT_PLACE_CODE");
                if (String.IsNullOrWhiteSpace(data.VACC_REACT_PLACE_NAME)) throw new ArgumentNullException("data.VACC_REACT_PLACE_NAME");
            }
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisVaccReactPlaceGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_VACC_REACT_PLACE data)
        {
            bool valid = true;
            try
            {
                data = new HisVaccReactPlaceGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                ...
                {
                    HisVaccReactPlaceFilterQuery filter = new HisVaccReactPlaceFilterQuery();
                    filter.IDs = listId;
                    List<HIS_VACC_REACT_PLACE> listData = new HisVaccReactPlaceGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_VACC_REACT_PLACE> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_VACC_REACT_PLACE data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisVaccReactPlaceDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_VACC_REACT_PLACE> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool CheckConstraint(long id)
        {
            ...
            try
            {
                /*List<XXX> hisXXXs = new HisXXXGet().GetByHisVaccReactPlaceId(id);
