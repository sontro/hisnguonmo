...
{
    partial class HisPositionLock : BusinessBase
    {
        internal HisPositionLock()
            : base()
        {

        }

        internal HisPositionLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_POSITION resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_POSITION data = new HisPositionGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisPositionDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_POSITION resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_POSITION resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisPositionCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisPositionDAO.Update(raw);
