...
{
    partial class HisExeServiceModuleCheck : BusinessBase
    {
        internal HisExeServiceModuleCheck()
            : base()
        {

        }

        internal HisExeServiceModuleCheck(CommonParam paramCheck)
            : base(paramCheck)
        {

        }

        internal bool VerifyRequireField(HIS_EXE_SERVICE_MODULE data)
        {
            bool valid = true;
            try
            {
                if (data == null) throw new ArgumentNullException("data");
                if (String.IsNullOrWhiteSpace(data.EXE_SERVICE_MODULE_NAME)) throw new ArgumentNullException("data.EXE_SERVICE_MODULE_NAME");
                if (String.IsNullOrWhiteSpace(data.MODULE_LINK)) throw new ArgumentNullException("data.MODULE_LINK");
            }
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisExeServiceModuleGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_EXE_SERVICE_MODULE data)
        {
            bool valid = true;
            try
            {
                data = new HisExeServiceModuleGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                ...
                {
                    HisExeServiceModuleFilterQuery filter = new HisExeServiceModuleFilterQuery();
                    filter.IDs = listId;
                    List<HIS_EXE_SERVICE_MODULE> listData = new HisExeServiceModuleGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_EXE_SERVICE_MODULE> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_EXE_SERVICE_MODULE data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisExeServiceModuleDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_EXE_SERVICE_MODULE> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool CheckConstraint(long id)
        {
            ...
            try
            {
                /*List<XXX> hisXXXs = new HisXXXGet().GetByHisExeServiceModuleId(id);
