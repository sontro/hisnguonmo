...
{
    partial class HisPrepareMetyLock : BusinessBase
    {
        internal HisPrepareMetyLock()
            : base()
        {

        }

        internal HisPrepareMetyLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_PREPARE_METY resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_PREPARE_METY data = new HisPrepareMetyGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisPrepareMetyDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_PREPARE_METY resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_PREPARE_METY resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisPrepareMetyCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisPrepareMetyDAO.Update(raw);
