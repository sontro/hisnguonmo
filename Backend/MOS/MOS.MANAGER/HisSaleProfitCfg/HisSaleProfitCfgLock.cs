...
{
    partial class HisSaleProfitCfgLock : BusinessBase
    {
        internal HisSaleProfitCfgLock()
            : base()
        {

        }

        internal HisSaleProfitCfgLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_SALE_PROFIT_CFG resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_SALE_PROFIT_CFG data = new HisSaleProfitCfgGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisSaleProfitCfgDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_SALE_PROFIT_CFG resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_SALE_PROFIT_CFG resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisSaleProfitCfgCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisSaleProfitCfgDAO.Update(raw);
