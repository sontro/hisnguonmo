...
{
    partial class HisAtcLock : BusinessBase
    {
        internal HisAtcLock()
            : base()
        {
        ...
        internal HisAtcLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_ATC resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_ATC data = new HisAtcGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisAtcDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_ATC resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_ATC resultData)
        {
            bool result = false;
            try
            {
                bool valid = true;
                HIS_ATC raw = null;
                valid = valid && new HisAtcCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisAtcDAO.Update(raw);
