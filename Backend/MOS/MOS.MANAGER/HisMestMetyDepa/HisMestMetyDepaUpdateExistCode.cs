    ...
    partial class HisMestMetyDepaUpdate : BusinessBase
    {
        ...
        internal bool Update(HIS_MEST_METY_DEPA data)
        {
            ...
            try
            {
                ...
                valid = valid && checker.ExistsCode(data.MEST_METY_DEPA_CODE, data.ID);
                if (valid)
                {
        ...
        internal bool UpdateList(List<HIS_MEST_METY_DEPA> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
					this.beforeUpdateHisMestMetyDepas.AddRange(listRaw);
		...
		internal void RollbackData()
        {
            ...
            {
                if (!new HisMestMetyDepaUpdate(param).UpdateList(this.beforeUpdateHisMestMetyDepas))
                {
