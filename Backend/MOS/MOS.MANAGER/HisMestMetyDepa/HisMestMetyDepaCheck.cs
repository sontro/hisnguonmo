...
{
    partial class HisMestMetyDepaCheck : BusinessBase
    {
        internal HisMestMetyDepaCheck()
            : base()
        {

        }

        internal HisMestMetyDepaCheck(CommonParam paramCheck)
            : base(paramCheck)
        {

        }

        internal bool VerifyRequireField(HIS_MEST_METY_DEPA data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool VerifyRequireField(HisMestMetyDepaByMedicineSDO data)
        {
            bool valid = true;
            try
            {
                if (data == null) throw new ArgumentNullException("data");
                if (data.MedicineTypeId <= 0) throw new ArgumentNullException("data.MedicineTypeId");
            }
            catch (ArgumentNullException ex)
            {
                BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.ThieuThongTinBatBuoc);
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisMestMetyDepaGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_MEST_METY_DEPA data)
        {
            bool valid = true;
            try
            {
                data = new HisMestMetyDepaGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                if (listId != null && listId.Count > 0)
                {
                    ...
                    filter.IDs = listId;
                    List<HIS_MEST_METY_DEPA> listData = new HisMestMetyDepaGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_MEST_METY_DEPA> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_MEST_METY_DEPA data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisMestMetyDepaDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_MEST_METY_DEPA> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool CheckConstraint(long id)
        {
            ...
            try
            {
                /*List<XXX> hisXXXs = new HisXXXGet().GetByHisMestMetyDepaId(id);
        ...
        internal bool IsNotExists(HIS_MEST_METY_DEPA data, long? id)
        {
            bool valid = true;
            try
            {
                if (data != null)
                {
                    ...
                    List<HIS_MEST_METY_DEPA> exists = new HisMestMetyDepaGet().Get(filter);
                    if (IsNotNullOrEmpty(exists))
                    {
                        MANAGER.Base.MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.HisMestMetyDepa_TonTaiDuLieu);
                        return false;
                    }
                }
            }
