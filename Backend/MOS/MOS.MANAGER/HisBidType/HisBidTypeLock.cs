...
{
    partial class HisBidTypeLock : BusinessBase
    {
        internal HisBidTypeLock()
            : base()
        {

        }

        internal HisBidTypeLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_BID_TYPE resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_BID_TYPE data = new HisBidTypeGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisBidTypeDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_BID_TYPE resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_BID_TYPE resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisBidTypeCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisBidTypeDAO.Update(raw);
