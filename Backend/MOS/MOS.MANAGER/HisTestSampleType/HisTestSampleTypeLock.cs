...
{
    partial class HisTestSampleTypeLock : BusinessBase
    {
        internal HisTestSampleTypeLock()
            : base()
        {

        }

        internal HisTestSampleTypeLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_TEST_SAMPLE_TYPE resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_TEST_SAMPLE_TYPE data = new HisTestSampleTypeGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisTestSampleTypeDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_TEST_SAMPLE_TYPE resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_TEST_SAMPLE_TYPE resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisTestSampleTypeCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisTestSampleTypeDAO.Update(raw);
