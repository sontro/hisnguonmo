...
{
    class HisPatientTypeAllowLock : BusinessBase
    {
        internal HisPatientTypeAllowLock()
            : base()
        {

        }

        internal HisPatientTypeAllowLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_PATIENT_TYPE_ALLOW data)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    data = new HisPatientTypeAllowGet().GetById(data.ID);
                    if (data != null)
                    {
