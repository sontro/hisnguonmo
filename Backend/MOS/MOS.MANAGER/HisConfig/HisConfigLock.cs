...
{
    partial class HisConfigLock : BusinessBase
    {
        internal HisConfigLock()
            : base()
        {

        }

        internal HisConfigLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_CONFIG resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_CONFIG data = new HisConfigGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisConfigDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_CONFIG resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_CONFIG resultData)
        {
            bool result = false;
            try
            {
                bool valid = true;
                HIS_CONFIG raw = null;
                valid = valid && new HisConfigCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisConfigDAO.Update(raw);
