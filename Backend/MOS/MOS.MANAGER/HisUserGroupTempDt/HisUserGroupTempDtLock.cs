...
{
    partial class HisUserGroupTempDtLock : BusinessBase
    {
        internal HisUserGroupTempDtLock()
            : base()
        {

        }

        internal HisUserGroupTempDtLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_USER_GROUP_TEMP_DT resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_USER_GROUP_TEMP_DT data = new HisUserGroupTempDtGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisUserGroupTempDtDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_USER_GROUP_TEMP_DT resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_USER_GROUP_TEMP_DT resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisUserGroupTempDtCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisUserGroupTempDtDAO.Update(raw);
