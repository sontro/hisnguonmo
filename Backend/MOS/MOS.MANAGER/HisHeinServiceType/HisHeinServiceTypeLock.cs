...
{
    class HisHeinServiceTypeLock : BusinessBase
    {
        internal HisHeinServiceTypeLock()
            : base()
        {

        }

        internal HisHeinServiceTypeLock(Inventec.Core.CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_HEIN_SERVICE_TYPE data)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    data = new HisHeinServiceTypeGet().GetById(data.ID);
                    if (data != null)
                    {
                        ...
                        result = DAOWorker.HisHeinServiceTypeDAO.Update(data);
                    }
                    else
                    {
        ...
        internal bool ChangeLock(long id)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    HIS_HEIN_SERVICE_TYPE data = new HisHeinServiceTypeGet().GetById(id);
                    if (data != null)
                    {
                        if (data.IS_ACTIVE.HasValue && data.IS_ACTIVE == IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__TRUE)
                        {
