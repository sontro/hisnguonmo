...
{
    class HisTransactionUnrejectCancellationRequestCheck : BusinessBase
    {
        internal HisTransactionUnrejectCancellationRequestCheck()
            : base()
        {

        }

        internal HisTransactionUnrejectCancellationRequestCheck(CommonParam param)
            : base(param)
        {
        ...
        /// <returns></returns>
        internal bool IsAllow(HisTransactionUnrejectCancellationRequestSDO sdo, V_HIS_TRANSACTION transaction)
        {
            bool valid = true;
            try
            {
                ...
                if (!HisEmployeeUtil.IsAdmin() && Inventec.Token.ResourceSystem.ResourceTokenManager.GetLoginName() != transaction.CANCEL_REQ_REJECT_LOGINNAME)
                {
                    MOS.MANAGER.Base.MessageUtil.SetMessage(param, MOS.LibraryMessage.Message.Enum.BanKhongPhaiNguoiTuChoiYeuCauHoacQuanTriHeThong);
                    return false;
                }
        ...
        internal bool VerifyRequireField(HisTransactionUnrejectCancellationRequestSDO data)
        {
            bool valid = true;
            try
            {
