...
{
    class HisCarerCardBorrowUnLost : BusinessBase
    {
        ...
        internal HisCarerCardBorrowUnLost()
            : base()
        {
            this.Init();
        }

        internal HisCarerCardBorrowUnLost(CommonParam param)
            : base(param)
        {
            this.Init();
        }
        ...
        internal bool UnLost(HisCarerCardBorrowUnLostSDO data, ref HIS_CARER_CARD_BORROW resultData)
        {
            bool result = false;
            try
            {
                ...
                HisCarerCardBorrowUnLostCheck checker = new HisCarerCardBorrowUnLostCheck(param);
                ...
                valid = valid && commonChecker.IsLost(cardBorrowed);
        ...
        private void ProcessServiceReq(long carerCardBorrowId, ref List<HIS_SERVICE_REQ> reqs)
        {
            ...
            if (IsNotNullOrEmpty(reqs))
            {
                ...
                if (!DAOWorker.HisServiceReqDAO.UpdateList(reqs))
                {
                    ...
                    throw new Exception("Cap nhat thong tin HisServiceReqs khi huy bao mat the that bai." + LogUtil.TraceData("data", reqs));
                }
            }
        }
        ...
        private void ProcessCarerCardBorrow(HIS_CARER_CARD_BORROW cardBorrowed)
        {
            ...
            if (!DAOWorker.HisCarerCardBorrowDAO.Update(cardBorrowed))
            {
                ...
                throw new Exception("Cap nhat thong tin HisCarerCardBorrow khi huy bao mat the that bai." + LogUtil.TraceData("data", cardBorrowed));
            }
        }
