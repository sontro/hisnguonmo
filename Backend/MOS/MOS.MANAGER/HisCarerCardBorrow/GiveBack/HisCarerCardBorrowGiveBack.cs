...
{
    class HisCarerCardBorrowGiveBack : BusinessBase
    {
        ...
        internal HisCarerCardBorrowGiveBack()
            : base()
        {
            this.Init();
        }

        internal HisCarerCardBorrowGiveBack(CommonParam param)
            : base(param)
        {
            this.Init();
        }
        ...
        internal bool GiveBack(HisCarerCardBorrowGiveBackSDO data, ref HIS_CARER_CARD_BORROW resultData)
        {
            bool result = false;
            try
            {
                ...
                HisCarerCardBorrowGiveBackCheck checker = new HisCarerCardBorrowGiveBackCheck(param);
                ...
                valid = valid && commonChecker.IsValidGiveTime(data.GiveBackTime);

                if (valid)
                {
                    this.ProcessCarerCardBorrow(cardBorrowed, data.GiveBackTime, data.ReceivingLoginName, data.ReceivingUserName);
                    this.ProcessServiceReq(cardBorrowed.ID, data.ReceivingLoginName, data.ReceivingUserName, ref reqs);
        ...
        private void ProcessServiceReq(long carerCardBorrowId, string receivingLoginName, string receivingUserName, ref List<HIS_SERVICE_REQ> reqs)
        {
            ...
            if (IsNotNullOrEmpty(reqs))
            {
                ...
                reqs.ForEach(o =>
                {
                    ...
                    o.EXECUTE_LOGINNAME = receivingLoginName;
                    o.EXECUTE_USERNAME = receivingUserName;
                });
                ...
                if (!DAOWorker.HisServiceReqDAO.UpdateList(reqs))
                {
                    ...
                    throw new Exception("Cap nhat thong tin HisServiceReqs khi tra the that bai." + LogUtil.TraceData("data", reqs));
                }
            }
        }

        private void ProcessCarerCardBorrow(HIS_CARER_CARD_BORROW cardBorrowed, long giveBackTime, string receivingLoginName, string receivingUserName)
        {
            ...
            cardBorrowed.RECEIVING_LOGINNAME = receivingLoginName;
            cardBorrowed.RECEIVING_USERNAME = receivingUserName;
            if (!DAOWorker.HisCarerCardBorrowDAO.Update(cardBorrowed))
            {
                ...
                throw new Exception("Cap nhat thong tin HisCarerCardBorrow khi tra the that bai." + LogUtil.TraceData("data", cardBorrowed));
            }
        }
