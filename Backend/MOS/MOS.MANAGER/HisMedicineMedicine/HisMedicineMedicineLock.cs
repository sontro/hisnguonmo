...
{
    partial class HisMedicineMedicineLock : BusinessBase
    {
        internal HisMedicineMedicineLock()
            : base()
        {

        }

        internal HisMedicineMedicineLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_MEDICINE_MEDICINE resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_MEDICINE_MEDICINE data = new HisMedicineMedicineGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisMedicineMedicineDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_MEDICINE_MEDICINE resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_MEDICINE_MEDICINE resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisMedicineMedicineCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisMedicineMedicineDAO.Update(raw);
