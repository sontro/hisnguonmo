...
{
    partial class HisKskLock : BusinessBase
    {
        internal HisKskLock()
            : base()
        {
        ...
        internal HisKskLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_KSK resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_KSK data = new HisKskGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisKskDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_KSK resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_KSK resultData)
        {
            bool result = false;
            try
            {
                bool valid = true;
                HIS_KSK raw = null;
                valid = valid && new HisKskCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisKskDAO.Update(raw);
