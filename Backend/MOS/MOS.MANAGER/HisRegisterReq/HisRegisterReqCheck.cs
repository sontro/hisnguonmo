...
{
    partial class HisRegisterReqCheck : BusinessBase
    {
        internal HisRegisterReqCheck()
            : base()
        {

        }

        internal HisRegisterReqCheck(CommonParam paramCheck)
            : base(paramCheck)
        {

        }

        internal bool VerifyRequireField(HIS_REGISTER_REQ data)
        {
            bool valid = true;
            try
            {
                if (data == null) throw new ArgumentNullException("data");
                if (data.REGISTER_GATE_ID <= 0) throw new ArgumentNullException("data.REGISTER_GATE_ID");
                //if (data.REGISTER_TIME <= 0) throw new ArgumentNullException("data.REGISTER_TIME");
            }
        ...
        internal bool VerifyRequireField(HisRegisterReqSDO data)
        {
            bool valid = true;
            try
            {
                if (data == null) throw new ArgumentNullException("data");
                if (data.RegisterGateId <= 0) throw new ArgumentNullException("data.RegisterGateId");
                //if (data.REGISTER_TIME <= 0) throw new ArgumentNullException("data.REGISTER_TIME");
            }
            catch (ArgumentNullException ex)
            {
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisRegisterReqGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_REGISTER_REQ data)
        {
            bool valid = true;
            try
            {
                data = new HisRegisterReqGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                if (listId != null && listId.Count > 0)
                {
                    ...
                    filter.IDs = listId;
                    List<HIS_REGISTER_REQ> listData = new HisRegisterReqGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_REGISTER_REQ> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_REGISTER_REQ data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisRegisterReqDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_REGISTER_REQ> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool CheckConstraint(long id)
        {
            ...
            try
            {
                /*List<XXX> hisXXXs = new HisXXXGet().GetByHisRegisterReqId(id);
