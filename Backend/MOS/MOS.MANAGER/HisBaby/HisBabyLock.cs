...
{
    partial class HisBabyLock : BusinessBase
    {
        internal HisBabyLock()
            : base()
        {

        }

        internal HisBabyLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_BABY resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_BABY data = new HisBabyGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisBabyDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_BABY resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_BABY resultData)
        {
            bool result = false;
            try
            {
                bool valid = true;
                HIS_BABY raw = null;
                valid = valid && new HisBabyCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisBabyDAO.Update(raw);
