...
{
    partial class HisMestPatyTrtyLock : BusinessBase
    {
        internal HisMestPatyTrtyLock()
            : base()
        {

        }

        internal HisMestPatyTrtyLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_MEST_PATY_TRTY resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_MEST_PATY_TRTY data = new HisMestPatyTrtyGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisMestPatyTrtyDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_MEST_PATY_TRTY resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_MEST_PATY_TRTY resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisMestPatyTrtyCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisMestPatyTrtyDAO.Update(raw);
