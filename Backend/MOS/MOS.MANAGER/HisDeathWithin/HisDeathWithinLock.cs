...
{
    class HisDeathWithinLock : BusinessBase
    {
        internal HisDeathWithinLock()
            : base()
        {

        }

        internal HisDeathWithinLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_DEATH_WITHIN data)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    data = new HisDeathWithinGet().GetById(data.ID);
                    if (data != null)
                    {
