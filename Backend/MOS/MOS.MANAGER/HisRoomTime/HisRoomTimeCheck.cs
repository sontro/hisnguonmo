...
{
    partial class HisRoomTimeCheck : BusinessBase
    {
        internal HisRoomTimeCheck()
            : base()
        {

        }

        internal HisRoomTimeCheck(CommonParam paramCheck)
            : base(paramCheck) 
        {

        }

        internal bool VerifyRequireField(HIS_ROOM_TIME data)
        {
            bool valid = true;
            try
            {
                ...
                if (data.DAY < 0) throw new ArgumentNullException("data.DAY");
            }
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisRoomTimeGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_ROOM_TIME data)
        {
            bool valid = true;
            try
            {
                data = new HisRoomTimeGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                ...
                {
                    HisRoomTimeFilterQuery filter = new HisRoomTimeFilterQuery();
                    filter.IDs = listId;
                    List<HIS_ROOM_TIME> listData = new HisRoomTimeGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_ROOM_TIME> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_ROOM_TIME data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisRoomTimeDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_ROOM_TIME> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool CheckConstraint(long id)
        {
            ...
            try
            {
                /*List<XXX> hisXXXs = new HisXXXGet().GetByHisRoomTimeId(id);
