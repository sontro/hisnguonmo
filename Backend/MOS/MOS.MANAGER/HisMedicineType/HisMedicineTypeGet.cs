...
namespace MOS.MANAGER.HisMedicineType
{
    partial class HisMedicineTypeGet : GetBase
    {
        ...
        internal List<V_HIS_MEDICINE_TYPE_1> GetView1(HisMedicineTypeView1FilterQuery filter)
        {
            try
            {
                List<V_HIS_MEDICINE_TYPE_1> result = DAOWorker.HisMedicineTypeDAO.GetView1(filter.Query(), param);
                return result;
            }
            catch (Exception ex)
            {
        ...
        internal List<HIS_MEDICINE_TYPE> GetByStorageConditionId(long id)
        {
            try
            {
                HisMedicineTypeFilterQuery filter = new HisMedicineTypeFilterQuery();
                filter.STORAGE_CONDITION_ID = id;
                return this.Get(filter);
            }
            catch (Exception ex)
            {
