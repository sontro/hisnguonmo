...
namespace MOS.MANAGER.HisBidBloodType
{
	partial class HisBidBloodTypeCreate : BusinessBase
	{
		...
		internal bool CreateList(List<HIS_BID_BLOOD_TYPE> listData)
		{
			bool result = false;
			try
			{
				...
				if (valid)
				{
					if (!DAOWorker.HisBidBloodTypeDAO.CreateList(listData))
					{
						MOS.MANAGER.Base.BugUtil.SetBugCode(param, MOS.LibraryBug.Bug.Enum.HisBidBloodType_ThemMoiThatBai);
						throw new Exception("Them moi thong tin HisBidBloodType that bai." + LogUtil.TraceData("listData", listData));
					}
					this.recentHisBidBloodTypes.AddRange(listData);
					result = true;
				}
			}
			catch (Exception ex)
