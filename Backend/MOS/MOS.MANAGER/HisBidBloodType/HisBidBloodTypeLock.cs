...
{
    partial class HisBidBloodTypeLock : BusinessBase
    {
        internal HisBidBloodTypeLock()
            : base()
        {

        }

        internal HisBidBloodTypeLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_BID_BLOOD_TYPE resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_BID_BLOOD_TYPE data = new HisBidBloodTypeGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisBidBloodTypeDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_BID_BLOOD_TYPE resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_BID_BLOOD_TYPE resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisBidBloodTypeCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisBidBloodTypeDAO.Update(raw);
