...
{
    partial class HisExpMestUserLock : BusinessBase
    {
        internal HisExpMestUserLock()
            : base()
        {

        }

        internal HisExpMestUserLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_EXP_MEST_USER resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_EXP_MEST_USER data = new HisExpMestUserGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisExpMestUserDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_EXP_MEST_USER resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_EXP_MEST_USER resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisExpMestUserCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisExpMestUserDAO.Update(raw);
