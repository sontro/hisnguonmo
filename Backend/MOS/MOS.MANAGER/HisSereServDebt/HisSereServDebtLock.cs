...
{
    partial class HisSereServDebtLock : BusinessBase
    {
        internal HisSereServDebtLock()
            : base()
        {

        }

        internal HisSereServDebtLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_SERE_SERV_DEBT resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_SERE_SERV_DEBT data = new HisSereServDebtGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisSereServDebtDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_SERE_SERV_DEBT resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_SERE_SERV_DEBT resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisSereServDebtCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisSereServDebtDAO.Update(raw);
