...
namespace MOS.MANAGER.HisAssessmentObject
{
    partial class HisAssessmentObjectCreate : BusinessBase
    {
		...
		internal bool CreateList(List<HIS_ASSESSMENT_OBJECT> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    if (!DAOWorker.HisAssessmentObjectDAO.CreateList(listData))
                    {
                        MOS.MANAGER.Base.BugUtil.SetBugCode(param, MOS.LibraryBug.Bug.Enum.HisAssessmentObject_ThemMoiThatBai);
                        throw new Exception("Them moi thong tin HisAssessmentObject that bai." + LogUtil.TraceData("listData", listData));
                    }
                    this.recentHisAssessmentObjects.AddRange(listData);
                    result = true;
                }
            }
            catch (Exception ex)
