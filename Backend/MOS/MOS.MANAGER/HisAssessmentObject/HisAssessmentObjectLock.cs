...
{
    partial class HisAssessmentObjectLock : BusinessBase
    {
        internal HisAssessmentObjectLock()
            : base()
        {

        }

        internal HisAssessmentObjectLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_ASSESSMENT_OBJECT resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_ASSESSMENT_OBJECT data = new HisAssessmentObjectGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisAssessmentObjectDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_ASSESSMENT_OBJECT resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_ASSESSMENT_OBJECT resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisAssessmentObjectCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisAssessmentObjectDAO.Update(raw);
