...
{
    partial class HisAssessmentObjectCheck : BusinessBase
    {
        internal HisAssessmentObjectCheck()
            : base()
        {

        }

        internal HisAssessmentObjectCheck(CommonParam paramCheck)
            : base(paramCheck) 
        {

        }

        internal bool VerifyRequireField(HIS_ASSESSMENT_OBJECT data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool VerifyId(long id)
        {
            ...
            try
            {
                if (new HisAssessmentObjectGet().GetById(id) == null)
                {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref HIS_ASSESSMENT_OBJECT data)
        {
            bool valid = true;
            try
            {
                data = new HisAssessmentObjectGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => id), id), LogType.Error);
        ...
        internal bool VerifyIds(List<long> listId)
        {
            ...
            try
            {
                ...
                {
                    HisAssessmentObjectFilterQuery filter = new HisAssessmentObjectFilterQuery();
                    filter.IDs = listId;
                    List<HIS_ASSESSMENT_OBJECT> listData = new HisAssessmentObjectGet().Get(filter);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyIds(List<long> listId, List<HIS_ASSESSMENT_OBJECT> listObject)
        {
            bool valid = true;
            try
            {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(HIS_ASSESSMENT_OBJECT data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.HisAssessmentObjectDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<HIS_ASSESSMENT_OBJECT> listData)
        {
            bool valid = true;
            try
            {
        ...
        internal bool CheckConstraint(long id)
        {
            ...
            try
            {
                /*List<XXX> hisXXXs = new HisXXXGet().GetByHisAssessmentObjectId(id);
        ...
        internal bool ExistsCode(string code, long? id)
        {
            ...
            try
            {
                if (DAOWorker.HisAssessmentObjectDAO.ExistsCode(code, id))
                {
