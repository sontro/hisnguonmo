...
{
    partial class HisAnticipateMatyLock : BusinessBase
    {
        internal HisAnticipateMatyLock()
            : base()
        {

        }

        internal HisAnticipateMatyLock(CommonParam paramLock)
            : base(paramLock)
        {

        }

        internal bool ChangeLock(HIS_ANTICIPATE_MATY data)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisAnticipateMatyCheck().VerifyId(data.ID, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisAnticipateMatyDAO.Update(raw);
