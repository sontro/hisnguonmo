...
{
    partial class HisRoomTypeModuleLock : BusinessBase
    {
        internal HisRoomTypeModuleLock()
            : base()
        {

        }

        internal HisRoomTypeModuleLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_ROOM_TYPE_MODULE resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_ROOM_TYPE_MODULE data = new HisRoomTypeModuleGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisRoomTypeModuleDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_ROOM_TYPE_MODULE resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_ROOM_TYPE_MODULE resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisRoomTypeModuleCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisRoomTypeModuleDAO.Update(raw);
