...
{
    partial class HisDosageFormLock : BusinessBase
    {
        internal HisDosageFormLock()
            : base()
        {

        }

        internal HisDosageFormLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_DOSAGE_FORM resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_DOSAGE_FORM data = new HisDosageFormGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisDosageFormDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_DOSAGE_FORM resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_DOSAGE_FORM resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisDosageFormCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisDosageFormDAO.Update(raw);
