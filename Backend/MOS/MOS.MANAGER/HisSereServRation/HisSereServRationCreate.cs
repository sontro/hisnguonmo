...
namespace MOS.MANAGER.HisSereServRation
{
    partial class HisSereServRationCreate : BusinessBase
    {
		...
		internal bool CreateList(List<HIS_SERE_SERV_RATION> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    if (!DAOWorker.HisSereServRationDAO.CreateList(listData))
                    {
                        MOS.MANAGER.Base.BugUtil.SetBugCode(param, MOS.LibraryBug.Bug.Enum.HisSereServRation_ThemMoiThatBai);
                        throw new Exception("Them moi thong tin HisSereServRation that bai." + LogUtil.TraceData("listData", listData));
                    }
                    this.recentHisSereServRations.AddRange(listData);
                    result = true;
                }
            }
            catch (Exception ex)
