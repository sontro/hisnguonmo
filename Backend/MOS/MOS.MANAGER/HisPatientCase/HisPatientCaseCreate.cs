...
namespace MOS.MANAGER.HisPatientCase
{
    partial class HisPatientCaseCreate : BusinessBase
    {
		...
		internal bool CreateList(List<HIS_PATIENT_CASE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    if (!DAOWorker.HisPatientCaseDAO.CreateList(listData))
                    {
                        MOS.MANAGER.Base.BugUtil.SetBugCode(param, MOS.LibraryBug.Bug.Enum.HisPatientCase_ThemMoiThatBai);
                        throw new Exception("Them moi thong tin HisPatientCase that bai." + LogUtil.TraceData("listData", listData));
                    }
                    this.recentHisPatientCases.AddRange(listData);
                    result = true;
                }
            }
            catch (Exception ex)
