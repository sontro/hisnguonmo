...
{
    partial class HisStationLock : BusinessBase
    {
        internal HisStationLock()
            : base()
        {

        }

        internal HisStationLock(CommonParam paramLock)
            : base(paramLock)
        {

        }
		
		internal bool Lock(long id, ref HIS_STATION resultData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    HIS_STATION data = new HisStationGet().GetById(id);
                    if (data != null)
                    {
                        data.IS_ACTIVE = IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__FALSE;
                        result = DAOWorker.HisStationDAO.Update(data);
		...
		internal bool Unlock(long id, ref HIS_STATION resultData)
        {
            bool result = false;
            try
            {
        ...
        internal bool ChangeLock(long id, ref HIS_STATION resultData)
        {
            bool result = false;
            try
            {
                ...
                valid = valid && new HisStationCheck().VerifyId(id, ref raw);
                if (valid && raw != null)
                {
                    ...
                    result = DAOWorker.HisStationDAO.Update(raw);
