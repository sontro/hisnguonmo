...
namespace MOS.MANAGER.HisImpMestMedicine
{
    partial class HisImpMestMedicineGet : GetBase
    {
        ...
        internal List<HisImpMestMedicineWithInStockAmountSDO> GetViewWithInStockAmount(long impMestId)
        {
            try
            {
                List<HisImpMestMedicineWithInStockAmountSDO> result = null;
                List<V_HIS_IMP_MEST_MEDICINE> impMestMedicineDTOs = this.GetViewByImpMestId(impMestId);
                if (IsNotNullOrEmpty(impMestMedicineDTOs))
                {
                    Mapper.CreateMap<V_HIS_IMP_MEST_MEDICINE, HisImpMestMedicineWithInStockAmountSDO>();
                    result = Mapper.Map<List<HisImpMestMedicineWithInStockAmountSDO>>(impMestMedicineDTOs);

                    HisMedicineStockViewFilter stockViewFilter = new HisMedicineStockViewFilter();
                    //imp_mest_medicine cung imp_mest_id thi co cung medi_stock_id
                    stockViewFilter.MEDI_STOCK_ID = impMestMedicineDTOs[0].MEDI_STOCK_ID;
                    stockViewFilter.IDs = impMestMedicineDTOs.Select(o => o.MEDICINE_ID).ToList();
                    List<HisMedicineInStockSDO> inStockMedicines = new HisMedicineGet(param).GetInStockMedicine(stockViewFilter);
                    
                    if (inStockMedicines != null)
                    {
                        foreach (HisImpMestMedicineWithInStockAmountSDO sdo in result)
                        {
                            HisMedicineInStockSDO medicineSDO = inStockMedicines.Where(o => o.ID == sdo.MEDICINE_ID).FirstOrDefault();
                            sdo.InStockAmount = medicineSDO != null ? medicineSDO.TotalAmount : 0;
                            sdo.AvailableAmount = medicineSDO != null ? medicineSDO.AvailableAmount : 0;
                        }
                    }
                }
                return result;
        ...
        internal List<V_HIS_IMP_MEST_MEDICINE> GetViewByMedicineId(long id)
        {
            HisImpMestMedicineViewFilterQuery filter = new HisImpMestMedicineViewFilterQuery();
                filter.MEDICINE_ID = id;
                return this.GetView(filter);
