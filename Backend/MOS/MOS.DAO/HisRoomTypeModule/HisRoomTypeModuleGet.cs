...
namespace MOS.DAO.HisRoomTypeModule
{
    partial class HisRoomTypeModuleGet : EntityBase
    {
        ...
        {
            List<HIS_ROOM_TYPE_MODULE> list = new List<HIS_ROOM_TYPE_MODULE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_ROOM_TYPE_MODULE.AsNoTracking().AsQueryable();
        ...
        public HIS_ROOM_TYPE_MODULE GetById(long id, HisRoomTypeModuleSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_ROOM_TYPE_MODULE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisRoomTypeModuleExpression != null && search.listHisRoomTypeModuleExpression.Count > 0)
                        {
                            foreach (var item in search.listHisRoomTypeModuleExpression)
                            {
