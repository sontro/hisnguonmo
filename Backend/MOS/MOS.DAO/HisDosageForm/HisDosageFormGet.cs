...
namespace MOS.DAO.HisDosageForm
{
    partial class HisDosageFormGet : EntityBase
    {
        ...
        {
            List<HIS_DOSAGE_FORM> list = new List<HIS_DOSAGE_FORM>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_DOSAGE_FORM.AsNoTracking().AsQueryable();
        ...
        public HIS_DOSAGE_FORM GetById(long id, HisDosageFormSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_DOSAGE_FORM.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisDosageFormExpression != null && search.listHisDosageFormExpression.Count > 0)
                        {
                            foreach (var item in search.listHisDosageFormExpression)
                            {
