...
{
    partial class HisDbLogDelete : EntityBase
    {
        public HisDbLogDelete()
            : base()
        {
        ...
        public bool Delete(HIS_DB_LOG data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_DB_LOG> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_DB_LOG> listIdNotFound = new List<HIS_DB_LOG>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_DB_LOG> listRaw = ctx.HIS_DB_LOG.Where(o => listId.Contains(o.ID)).ToList();
