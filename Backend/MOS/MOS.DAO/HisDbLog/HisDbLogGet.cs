...
{
    partial class HisDbLogGet : EntityBase
    {
        public List<HIS_DB_LOG> Get(HisDbLogSO search, CommonParam param)
        {
            List<HIS_DB_LOG> list = new List<HIS_DB_LOG>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_DB_LOG.AsNoTracking().AsQueryable();
                        if (search.listHisDbLogExpression != null && search.listHisDbLogExpression.Count > 0)
                        {
                            foreach (var item in search.listHisDbLogExpression)
                            {
        ...
        public HIS_DB_LOG GetById(long id, HisDbLogSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_DB_LOG.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisDbLogExpression != null && search.listHisDbLogExpression.Count > 0)
                        {
                            foreach (var item in search.listHisDbLogExpression)
                            {
