...
{
    partial class HisBedDelete : EntityBase
    {
        public HisBedDelete()
            : base()
        {
        ...
        public bool Delete(HIS_BED data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_BED> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_BED> listIdNotFound = new List<HIS_BED>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_BED> listRaw = ctx.HIS_BED.Where(o => listId.Contains(o.ID)).ToList();
