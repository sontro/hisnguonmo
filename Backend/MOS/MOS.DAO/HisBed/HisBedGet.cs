...
{
    partial class HisBedGet : EntityBase
    {
        public List<HIS_BED> Get(HisBedSO search, CommonParam param)
        {
            List<HIS_BED> list = new List<HIS_BED>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_BED.AsNoTracking().AsQueryable();
                        if (search.listHisBedExpression != null && search.listHisBedExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBedExpression)
                            {
        ...
        public HIS_BED GetById(long id, HisBedSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_BED.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisBedExpression != null && search.listHisBedExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBedExpression)
                            {
