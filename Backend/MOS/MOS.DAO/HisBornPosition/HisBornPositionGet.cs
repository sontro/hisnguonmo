...
namespace MOS.DAO.HisBornPosition
{
    partial class HisBornPositionGet : EntityBase
    {
        ...
        {
            List<HIS_BORN_POSITION> list = new List<HIS_BORN_POSITION>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_BORN_POSITION.AsNoTracking().AsQueryable();
                        if (search.listHisBornPositionExpression != null && search.listHisBornPositionExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBornPositionExpression)
                            {
        ...
        public HIS_BORN_POSITION GetById(long id, HisBornPositionSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_BORN_POSITION.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisBornPositionExpression != null && search.listHisBornPositionExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBornPositionExpression)
                            {
