...
namespace MOS.DAO.HisBornPosition
{
    partial class HisBornPositionGet : EntityBase
    {
        ...
        {
            List<V_HIS_BORN_POSITION> list = new List<V_HIS_BORN_POSITION>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_BORN_POSITION.AsQueryable();
                        if (search.listVHisBornPositionExpression != null && search.listVHisBornPositionExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisBornPositionExpression)
                            {
