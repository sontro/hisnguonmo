...
namespace MOS.DAO.HisMediRecordType
{
    partial class HisMediRecordTypeGet : EntityBase
    {
        ...
        {
            List<V_HIS_MEDI_RECORD_TYPE> list = new List<V_HIS_MEDI_RECORD_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_MEDI_RECORD_TYPE.AsNoTracking().AsQueryable();
                        if (search.listVHisMediRecordTypeExpression != null && search.listVHisMediRecordTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisMediRecordTypeExpression)
                            {
