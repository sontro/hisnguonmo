...
{
    partial class HisMediRecordTypeGet : EntityBase
    {
        public List<HIS_MEDI_RECORD_TYPE> Get(HisMediRecordTypeSO search, CommonParam param)
        {
            List<HIS_MEDI_RECORD_TYPE> list = new List<HIS_MEDI_RECORD_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MEDI_RECORD_TYPE.AsNoTracking().AsQueryable();
                        if (search.listHisMediRecordTypeExpression != null && search.listHisMediRecordTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMediRecordTypeExpression)
                            {
        ...
        public HIS_MEDI_RECORD_TYPE GetById(long id, HisMediRecordTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_MEDI_RECORD_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMediRecordTypeExpression != null && search.listHisMediRecordTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMediRecordTypeExpression)
                            {
