...
{
    partial class HisPrepareMatyGet : EntityBase
    {
        public List<HIS_PREPARE_MATY> Get(HisPrepareMatySO search, CommonParam param)
        {
            List<HIS_PREPARE_MATY> list = new List<HIS_PREPARE_MATY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_PREPARE_MATY.AsNoTracking().AsQueryable();
                        if (search.listHisPrepareMatyExpression != null && search.listHisPrepareMatyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisPrepareMatyExpression)
                            {
        ...
        public HIS_PREPARE_MATY GetById(long id, HisPrepareMatySO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_PREPARE_MATY.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisPrepareMatyExpression != null && search.listHisPrepareMatyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisPrepareMatyExpression)
                            {
