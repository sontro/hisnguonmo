...
namespace MOS.DAO.HisPrepareMaty
{
    partial class HisPrepareMatyGet : EntityBase
    {
        ...
        {
            List<V_HIS_PREPARE_MATY> list = new List<V_HIS_PREPARE_MATY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_PREPARE_MATY.AsNoTracking().AsQueryable();
                        if (search.listVHisPrepareMatyExpression != null && search.listVHisPrepareMatyExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisPrepareMatyExpression)
                            {
