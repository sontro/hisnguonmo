...
namespace MOS.DAO.HisExpMestUser
{
    partial class HisExpMestUserGet : EntityBase
    {
        ...
        {
            List<HIS_EXP_MEST_USER> list = new List<HIS_EXP_MEST_USER>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_EXP_MEST_USER.AsNoTracking().AsQueryable();
        ...
        public HIS_EXP_MEST_USER GetById(long id, HisExpMestUserSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_EXP_MEST_USER.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisExpMestUserExpression != null && search.listHisExpMestUserExpression.Count > 0)
                        {
                            foreach (var item in search.listHisExpMestUserExpression)
                            {
