...
{
    partial class HisExpMestUserDelete : EntityBase
    {
        public HisExpMestUserDelete()
            : base()
        {
        ...
        public bool Delete(HIS_EXP_MEST_USER data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_EXP_MEST_USER> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_EXP_MEST_USER> listIdNotFound = new List<HIS_EXP_MEST_USER>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_EXP_MEST_USER> listRaw = ctx.HIS_EXP_MEST_USER.Where(o => listId.Contains(o.ID)).ToList();
