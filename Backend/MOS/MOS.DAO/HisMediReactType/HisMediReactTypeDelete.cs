...
{
    partial class HisMediReactTypeDelete : EntityBase
    {
        public HisMediReactTypeDelete()
            : base()
        {
        ...
        public bool Delete(HIS_MEDI_REACT_TYPE data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_MEDI_REACT_TYPE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_MEDI_REACT_TYPE> listIdNotFound = new List<HIS_MEDI_REACT_TYPE>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_MEDI_REACT_TYPE> listRaw = ctx.HIS_MEDI_REACT_TYPE.Where(o => listId.Contains(o.ID)).ToList();
