...
namespace MOS.DAO.HisMediContractMety
{
    partial class HisMediContractMetyGet : EntityBase
    {
        ...
        {
            List<V_HIS_MEDI_CONTRACT_METY_1> list = new List<V_HIS_MEDI_CONTRACT_METY_1>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_MEDI_CONTRACT_METY_1.AsNoTracking().AsQueryable();
                        if (search.listV1HisMediContractMetyExpression != null && search.listV1HisMediContractMetyExpression.Count > 0)
                        {
                            foreach (var item in search.listV1HisMediContractMetyExpression)
                            {
