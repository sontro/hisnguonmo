...
{
    partial class HisMediContractMetyGet : EntityBase
    {
        public List<HIS_MEDI_CONTRACT_METY> Get(HisMediContractMetySO search, CommonParam param)
        {
            List<HIS_MEDI_CONTRACT_METY> list = new List<HIS_MEDI_CONTRACT_METY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MEDI_CONTRACT_METY.AsNoTracking().AsQueryable();
                        if (search.listHisMediContractMetyExpression != null && search.listHisMediContractMetyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMediContractMetyExpression)
                            {
        ...
        public HIS_MEDI_CONTRACT_METY GetById(long id, HisMediContractMetySO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_MEDI_CONTRACT_METY.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMediContractMetyExpression != null && search.listHisMediContractMetyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMediContractMetyExpression)
                            {
