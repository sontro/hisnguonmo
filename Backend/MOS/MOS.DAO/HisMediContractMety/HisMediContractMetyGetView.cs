...
namespace MOS.DAO.HisMediContractMety
{
    partial class HisMediContractMetyGet : EntityBase
    {
        ...
        {
            List<V_HIS_MEDI_CONTRACT_METY> list = new List<V_HIS_MEDI_CONTRACT_METY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_MEDI_CONTRACT_METY.AsNoTracking().AsQueryable();
                        if (search.listVHisMediContractMetyExpression != null && search.listVHisMediContractMetyExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisMediContractMetyExpression)
                            {
