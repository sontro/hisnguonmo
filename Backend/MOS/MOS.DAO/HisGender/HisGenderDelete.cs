...
{
    partial class HisGenderDelete : EntityBase
    {
        public HisGenderDelete()
            : base()
        {
        ...
        public bool Delete(HIS_GENDER data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_GENDER> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_GENDER> listIdNotFound = new List<HIS_GENDER>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_GENDER> listRaw = ctx.HIS_GENDER.Where(o => listId.Contains(o.ID)).ToList();
