...
{
    partial class HisDispenseGet : EntityBase
    {
        public List<V_HIS_DISPENSE> GetView(HisDispenseSO search, CommonParam param)
        {
            List<V_HIS_DISPENSE> list = new List<V_HIS_DISPENSE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_DISPENSE.AsNoTracking().AsQueryable();
                        if (search.listVHisDispenseExpression != null && search.listVHisDispenseExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisDispenseExpression)
                            {
