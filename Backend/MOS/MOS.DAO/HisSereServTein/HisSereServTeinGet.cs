...
namespace MOS.DAO.HisSereServTein
{
    partial class HisSereServTeinGet : EntityBase
    {
        ...
        {
            List<HIS_SERE_SERV_TEIN> list = new List<HIS_SERE_SERV_TEIN>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SERE_SERV_TEIN.AsNoTracking().AsQueryable();
        ...
        public HIS_SERE_SERV_TEIN GetById(long id, HisSereServTeinSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_SERE_SERV_TEIN.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisSereServTeinExpression != null && search.listHisSereServTeinExpression.Count > 0)
                        {
                            foreach (var item in search.listHisSereServTeinExpression)
                            {
