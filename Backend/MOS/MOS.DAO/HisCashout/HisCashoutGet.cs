...
{
    partial class HisCashoutGet : EntityBase
    {
        public List<HIS_CASHOUT> Get(HisCashoutSO search, CommonParam param)
        {
            List<HIS_CASHOUT> list = new List<HIS_CASHOUT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_CASHOUT.AsNoTracking().AsQueryable();
                        if (search.listHisCashoutExpression != null && search.listHisCashoutExpression.Count > 0)
                        {
                            foreach (var item in search.listHisCashoutExpression)
                            {
        ...
        public HIS_CASHOUT GetById(long id, HisCashoutSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_CASHOUT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisCashoutExpression != null && search.listHisCashoutExpression.Count > 0)
                        {
                            foreach (var item in search.listHisCashoutExpression)
                            {
