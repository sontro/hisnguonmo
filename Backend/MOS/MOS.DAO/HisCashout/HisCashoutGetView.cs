...
namespace MOS.DAO.HisCashout
{
    partial class HisCashoutGet : EntityBase
    {
        ...
        {
            List<V_HIS_CASHOUT> list = new List<V_HIS_CASHOUT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_CASHOUT.AsQueryable();
                        if (search.listVHisCashoutExpression != null && search.listVHisCashoutExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisCashoutExpression)
                            {
