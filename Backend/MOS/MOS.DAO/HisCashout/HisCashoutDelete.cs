...
{
    partial class HisCashoutDelete : EntityBase
    {
        public HisCashoutDelete()
            : base()
        {
        ...
        public bool Delete(HIS_CASHOUT data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_CASHOUT> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_CASHOUT> listIdNotFound = new List<HIS_CASHOUT>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_CASHOUT> listRaw = ctx.HIS_CASHOUT.Where(o => listId.Contains(o.ID)).ToList();
