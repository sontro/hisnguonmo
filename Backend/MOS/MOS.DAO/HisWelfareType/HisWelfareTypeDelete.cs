...
{
    partial class HisWelfareTypeDelete : EntityBase
    {
        public HisWelfareTypeDelete()
            : base()
        {
        ...
        public bool Delete(HIS_WELFARE_TYPE data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_WELFARE_TYPE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_WELFARE_TYPE> listIdNotFound = new List<HIS_WELFARE_TYPE>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_WELFARE_TYPE> listRaw = ctx.HIS_WELFARE_TYPE.Where(o => listId.Contains(o.ID)).ToList();
