...
{
    partial class HisWelfareTypeGet : EntityBase
    {
        public List<HIS_WELFARE_TYPE> Get(HisWelfareTypeSO search, CommonParam param)
        {
            List<HIS_WELFARE_TYPE> list = new List<HIS_WELFARE_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_WELFARE_TYPE.AsNoTracking().AsQueryable();
                        if (search.listHisWelfareTypeExpression != null && search.listHisWelfareTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisWelfareTypeExpression)
                            {
        ...
        public HIS_WELFARE_TYPE GetById(long id, HisWelfareTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_WELFARE_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisWelfareTypeExpression != null && search.listHisWelfareTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisWelfareTypeExpression)
                            {
