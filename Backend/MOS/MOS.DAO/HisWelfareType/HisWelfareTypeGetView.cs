...
namespace MOS.DAO.HisWelfareType
{
    partial class HisWelfareTypeGet : EntityBase
    {
        ...
        {
            List<V_HIS_WELFARE_TYPE> list = new List<V_HIS_WELFARE_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_WELFARE_TYPE.AsNoTracking().AsQueryable();
                        if (search.listVHisWelfareTypeExpression != null && search.listVHisWelfareTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisWelfareTypeExpression)
                            {
