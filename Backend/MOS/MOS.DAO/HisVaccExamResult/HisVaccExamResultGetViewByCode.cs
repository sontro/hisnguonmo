...
namespace MOS.DAO.HisVaccExamResult
{
    ...
    {
        public V_HIS_VACC_EXAM_RESULT GetViewByCode(string code, HisVaccExamResultSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_VACC_EXAM_RESULT.AsQueryable().Where(p => p.VACC_EXAM_RESULT_CODE == code);
