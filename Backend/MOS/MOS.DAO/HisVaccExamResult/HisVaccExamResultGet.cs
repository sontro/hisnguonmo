...
namespace MOS.DAO.HisVaccExamResult
{
    partial class HisVaccExamResultGet : EntityBase
    {
        ...
        {
            List<HIS_VACC_EXAM_RESULT> list = new List<HIS_VACC_EXAM_RESULT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_VACC_EXAM_RESULT.AsNoTracking().AsQueryable();
        ...
        public HIS_VACC_EXAM_RESULT GetById(long id, HisVaccExamResultSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_VACC_EXAM_RESULT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisVaccExamResultExpression != null && search.listHisVaccExamResultExpression.Count > 0)
                        {
                            foreach (var item in search.listHisVaccExamResultExpression)
                            {
