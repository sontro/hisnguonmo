...
{
    partial class HisBankGet : EntityBase
    {
        public List<HIS_BANK> Get(HisBankSO search, CommonParam param)
        {
            List<HIS_BANK> list = new List<HIS_BANK>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_BANK.AsNoTracking().AsQueryable();
                        if (search.listHisBankExpression != null && search.listHisBankExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBankExpression)
                            {
        ...
        public HIS_BANK GetById(long id, HisBankSO search)
        {
            HIS_BANK result = null;
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_BANK.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisBankExpression != null && search.listHisBankExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBankExpression)
                            {
