...
{
    partial class HisBankDelete : EntityBase
    {
        public HisBankDelete()
            : base()
        {
        ...
        public bool Delete(HIS_BANK data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_BANK> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_BANK> listIdNotFound = new List<HIS_BANK>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_BANK> listRaw = ctx.HIS_BANK.Where(o => listId.Contains(o.ID)).ToList();
