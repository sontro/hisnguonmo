...
namespace MOS.DAO.HisMedicineMaterial
{
    partial class HisMedicineMaterialGet : EntityBase
    {
        ...
        {
            List<HIS_MEDICINE_MATERIAL> list = new List<HIS_MEDICINE_MATERIAL>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MEDICINE_MATERIAL.AsNoTracking().AsQueryable();
                        if (search.listHisMedicineMaterialExpression != null && search.listHisMedicineMaterialExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMedicineMaterialExpression)
                            {
        ...
        public HIS_MEDICINE_MATERIAL GetById(long id, HisMedicineMaterialSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_MEDICINE_MATERIAL.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMedicineMaterialExpression != null && search.listHisMedicineMaterialExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMedicineMaterialExpression)
                            {
