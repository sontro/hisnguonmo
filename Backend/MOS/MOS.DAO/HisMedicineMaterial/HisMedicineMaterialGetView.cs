...
namespace MOS.DAO.HisMedicineMaterial
{
    partial class HisMedicineMaterialGet : EntityBase
    {
        ...
        {
            List<V_HIS_MEDICINE_MATERIAL> list = new List<V_HIS_MEDICINE_MATERIAL>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_MEDICINE_MATERIAL.AsNoTracking().AsQueryable();
                        if (search.listVHisMedicineMaterialExpression != null && search.listVHisMedicineMaterialExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisMedicineMaterialExpression)
                            {
