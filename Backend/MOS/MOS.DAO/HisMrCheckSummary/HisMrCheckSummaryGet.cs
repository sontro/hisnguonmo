...
namespace MOS.DAO.HisMrCheckSummary
{
    partial class HisMrCheckSummaryGet : EntityBase
    {
        ...
        {
            List<HIS_MR_CHECK_SUMMARY> list = new List<HIS_MR_CHECK_SUMMARY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MR_CHECK_SUMMARY.AsNoTracking().AsQueryable();
        ...
        public HIS_MR_CHECK_SUMMARY GetById(long id, HisMrCheckSummarySO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_MR_CHECK_SUMMARY.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMrCheckSummaryExpression != null && search.listHisMrCheckSummaryExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMrCheckSummaryExpression)
                            {
