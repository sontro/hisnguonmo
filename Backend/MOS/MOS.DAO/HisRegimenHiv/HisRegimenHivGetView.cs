...
namespace MOS.DAO.HisRegimenHiv
{
    partial class HisRegimenHivGet : EntityBase
    {
        ...
        {
            List<V_HIS_REGIMEN_HIV> list = new List<V_HIS_REGIMEN_HIV>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_REGIMEN_HIV.AsNoTracking().AsQueryable();
                        if (search.listVHisRegimenHivExpression != null && search.listVHisRegimenHivExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisRegimenHivExpression)
                            {
