...
namespace MOS.DAO.HisRegimenHiv
{
    partial class HisRegimenHivGet : EntityBase
    {
        ...
        {
            List<HIS_REGIMEN_HIV> list = new List<HIS_REGIMEN_HIV>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_REGIMEN_HIV.AsNoTracking().AsQueryable();
        ...
        public HIS_REGIMEN_HIV GetById(long id, HisRegimenHivSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_REGIMEN_HIV.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisRegimenHivExpression != null && search.listHisRegimenHivExpression.Count > 0)
                        {
                            foreach (var item in search.listHisRegimenHivExpression)
                            {
