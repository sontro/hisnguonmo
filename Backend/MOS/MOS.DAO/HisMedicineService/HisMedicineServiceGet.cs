...
namespace MOS.DAO.HisMedicineService
{
    partial class HisMedicineServiceGet : EntityBase
    {
        ...
        {
            List<HIS_MEDICINE_SERVICE> list = new List<HIS_MEDICINE_SERVICE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MEDICINE_SERVICE.AsNoTracking().AsQueryable();
                        if (search.listHisMedicineServiceExpression != null && search.listHisMedicineServiceExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMedicineServiceExpression)
                            {
        ...
        public HIS_MEDICINE_SERVICE GetById(long id, HisMedicineServiceSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_MEDICINE_SERVICE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMedicineServiceExpression != null && search.listHisMedicineServiceExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMedicineServiceExpression)
                            {
