...
{
    partial class HisKskUnderEighteenGet : EntityBase
    {
        public List<HIS_KSK_UNDER_EIGHTEEN> Get(HisKskUnderEighteenSO search, CommonParam param)
        {
            List<HIS_KSK_UNDER_EIGHTEEN> list = new List<HIS_KSK_UNDER_EIGHTEEN>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_KSK_UNDER_EIGHTEEN.AsNoTracking().AsQueryable();
                        if (search.listHisKskUnderEighteenExpression != null && search.listHisKskUnderEighteenExpression.Count > 0)
                        {
                            foreach (var item in search.listHisKskUnderEighteenExpression)
                            {
        ...
        public HIS_KSK_UNDER_EIGHTEEN GetById(long id, HisKskUnderEighteenSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_KSK_UNDER_EIGHTEEN.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisKskUnderEighteenExpression != null && search.listHisKskUnderEighteenExpression.Count > 0)
                        {
                            foreach (var item in search.listHisKskUnderEighteenExpression)
                            {
