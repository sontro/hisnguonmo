...
{
    partial class HisMedicineDelete : EntityBase
    {
        public HisMedicineDelete()
            : base()
        {
        ...
        public bool Delete(HIS_MEDICINE data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_MEDICINE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_MEDICINE> listIdNotFound = new List<HIS_MEDICINE>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_MEDICINE> listRaw = ctx.HIS_MEDICINE.Where(o => listId.Contains(o.ID)).ToList();
