        ...
        public List<HIS_MEDICINE> Get(HisMedicineSO search, CommonParam param)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MEDICINE.AsNoTracking().AsQueryable();
                        if (search.listHisMedicineExpression != null && search.listHisMedicineExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMedicineExpression)
                            {
        ...
        public HIS_MEDICINE GetById(long id, HisMedicineSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_MEDICINE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMedicineExpression != null && search.listHisMedicineExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMedicineExpression)
                            {
