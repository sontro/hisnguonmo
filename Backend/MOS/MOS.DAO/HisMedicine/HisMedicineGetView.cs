...
namespace MOS.DAO.HisMedicine
{
    partial class HisMedicineGet : EntityBase
    {
        ...
        {
            List<V_HIS_MEDICINE> list = new List<V_HIS_MEDICINE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_MEDICINE.AsNoTracking().AsQueryable();
                        if (search.listVHisMedicineExpression != null && search.listVHisMedicineExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisMedicineExpression)
                            {
