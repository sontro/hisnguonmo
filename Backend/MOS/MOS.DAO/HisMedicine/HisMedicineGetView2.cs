...
namespace MOS.DAO.HisMedicine
{
    partial class HisMedicineGet : EntityBase
    {
        ...
        {
            List<V_HIS_MEDICINE_2> list = new List<V_HIS_MEDICINE_2>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_MEDICINE_2.AsNoTracking().AsQueryable();
                        if (search.listVHisMedicine2Expression != null && search.listVHisMedicine2Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisMedicine2Expression)
                            {
