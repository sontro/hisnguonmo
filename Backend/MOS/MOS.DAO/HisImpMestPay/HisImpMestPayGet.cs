...
namespace MOS.DAO.HisImpMestPay
{
    partial class HisImpMestPayGet : EntityBase
    {
        ...
        {
            List<HIS_IMP_MEST_PAY> list = new List<HIS_IMP_MEST_PAY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_IMP_MEST_PAY.AsNoTracking().AsQueryable();
                        if (search.listHisImpMestPayExpression != null && search.listHisImpMestPayExpression.Count > 0)
                        {
                            foreach (var item in search.listHisImpMestPayExpression)
                            {
        ...
        public HIS_IMP_MEST_PAY GetById(long id, HisImpMestPaySO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_IMP_MEST_PAY.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisImpMestPayExpression != null && search.listHisImpMestPayExpression.Count > 0)
                        {
                            foreach (var item in search.listHisImpMestPayExpression)
                            {
