...
{
    partial class HisImpMestPayGet : EntityBase
    {
        public List<V_HIS_IMP_MEST_PAY> GetView(HisImpMestPaySO search, CommonParam param)
        {
            List<V_HIS_IMP_MEST_PAY> list = new List<V_HIS_IMP_MEST_PAY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_IMP_MEST_PAY.AsNoTracking().AsQueryable();
                        if (search.listVHisImpMestPayExpression != null && search.listVHisImpMestPayExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisImpMestPayExpression)
                            {
