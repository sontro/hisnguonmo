...
{
    partial class HisSupplierGet : EntityBase
    {
        public List<HIS_SUPPLIER> Get(HisSupplierSO search, CommonParam param)
        {
            List<HIS_SUPPLIER> list = new List<HIS_SUPPLIER>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SUPPLIER.AsNoTracking().AsQueryable();
                        if (search.listHisSupplierExpression != null && search.listHisSupplierExpression.Count > 0)
                        {
                            foreach (var item in search.listHisSupplierExpression)
                            {
        ...
        public HIS_SUPPLIER GetById(long id, HisSupplierSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_SUPPLIER.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisSupplierExpression != null && search.listHisSupplierExpression.Count > 0)
                        {
                            foreach (var item in search.listHisSupplierExpression)
                            {
