...
{
    partial class HisSupplierDelete : EntityBase
    {
        public HisSupplierDelete()
            : base()
        {
        ...
        public bool Delete(HIS_SUPPLIER data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_SUPPLIER> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_SUPPLIER> listIdNotFound = new List<HIS_SUPPLIER>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_SUPPLIER> listRaw = ctx.HIS_SUPPLIER.Where(o => listId.Contains(o.ID)).ToList();
