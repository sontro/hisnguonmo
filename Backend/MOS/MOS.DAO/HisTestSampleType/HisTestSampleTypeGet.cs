...
namespace MOS.DAO.HisTestSampleType
{
    partial class HisTestSampleTypeGet : EntityBase
    {
        ...
        {
            List<HIS_TEST_SAMPLE_TYPE> list = new List<HIS_TEST_SAMPLE_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_TEST_SAMPLE_TYPE.AsNoTracking().AsQueryable();
        ...
        public HIS_TEST_SAMPLE_TYPE GetById(long id, HisTestSampleTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_TEST_SAMPLE_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisTestSampleTypeExpression != null && search.listHisTestSampleTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisTestSampleTypeExpression)
                            {
