...
{
    partial class HisBranchDelete : EntityBase
    {
        public HisBranchDelete()
            : base()
        {
        ...
        public bool Delete(HIS_BRANCH data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_BRANCH> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_BRANCH> listIdNotFound = new List<HIS_BRANCH>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_BRANCH> listRaw = ctx.HIS_BRANCH.Where(o => listId.Contains(o.ID)).ToList();
