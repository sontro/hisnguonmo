...
namespace MOS.DAO.HisExpMestStt
{
    partial class HisExpMestSttGet : EntityBase
    {
        ...
        {
            List<HIS_EXP_MEST_STT> list = new List<HIS_EXP_MEST_STT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_EXP_MEST_STT.AsNoTracking().AsQueryable();
        ...
        public HIS_EXP_MEST_STT GetById(long id, HisExpMestSttSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_EXP_MEST_STT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisExpMestSttExpression != null && search.listHisExpMestSttExpression.Count > 0)
                        {
                            foreach (var item in search.listHisExpMestSttExpression)
                            {
