...
namespace MOS.DAO.HisExpMestStt
{
    partial class HisExpMestSttGet : EntityBase
    {
        ...
        {
            List<V_HIS_EXP_MEST_STT> list = new List<V_HIS_EXP_MEST_STT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_EXP_MEST_STT.AsQueryable();
                        if (search.listVHisExpMestSttExpression != null && search.listVHisExpMestSttExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisExpMestSttExpression)
                            {
