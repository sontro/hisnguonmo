...
{
    partial class HisIcdDelete : EntityBase
    {
        public HisIcdDelete()
            : base()
        {
        ...
        public bool Delete(HIS_ICD data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_ICD> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_ICD> listIdNotFound = new List<HIS_ICD>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_ICD> listRaw = ctx.HIS_ICD.Where(o => listId.Contains(o.ID)).ToList();
