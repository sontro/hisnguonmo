...
{
    partial class HisIcdGet : EntityBase
    {
        public List<HIS_ICD> Get(HisIcdSO search, CommonParam param)
        {
            List<HIS_ICD> list = new List<HIS_ICD>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_ICD.AsNoTracking().AsQueryable();
                        if (search.listHisIcdExpression != null && search.listHisIcdExpression.Count > 0)
                        {
                            foreach (var item in search.listHisIcdExpression)
                            {
        ...
        public HIS_ICD GetById(long id, HisIcdSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_ICD.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisIcdExpression != null && search.listHisIcdExpression.Count > 0)
                        {
                            foreach (var item in search.listHisIcdExpression)
                            {
