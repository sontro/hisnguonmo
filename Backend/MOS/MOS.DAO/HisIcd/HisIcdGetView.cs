...
namespace MOS.DAO.HisIcd
{
    partial class HisIcdGet : EntityBase
    {
        ...
        {
            List<V_HIS_ICD> list = new List<V_HIS_ICD>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_ICD.AsNoTracking().AsQueryable();
                        if (search.listVHisIcdExpression != null && search.listVHisIcdExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisIcdExpression)
                            {
