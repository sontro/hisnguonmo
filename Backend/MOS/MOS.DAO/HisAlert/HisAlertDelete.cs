...
{
    partial class HisAlertDelete : EntityBase
    {
        public HisAlertDelete()
            : base()
        {
        ...
        public bool Delete(HIS_ALERT data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_ALERT> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_ALERT> listIdNotFound = new List<HIS_ALERT>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_ALERT> listRaw = ctx.HIS_ALERT.Where(o => listId.Contains(o.ID)).ToList();
