...
namespace MOS.DAO.HisAlert
{
    ...
    {
        public List<V_HIS_ALERT> GetView(HisAlertSO search, CommonParam param)
        {
            List<V_HIS_ALERT> list = new List<V_HIS_ALERT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_ALERT.AsNoTracking().AsQueryable();
                        if (search.listVHisAlertExpression != null && search.listVHisAlertExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisAlertExpression)
                            {
