...
namespace MOS.DAO.HisExpMestMetyReq
{
    partial class HisExpMestMetyReqGet : EntityBase
    {
        ...
        {
            List<L_HIS_EXP_MEST_METY_REQ> list = new List<L_HIS_EXP_MEST_METY_REQ>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.L_HIS_EXP_MEST_METY_REQ.AsNoTracking().AsQueryable();
                        if (search.listLHisExpMestMetyReqExpression != null && search.listLHisExpMestMetyReqExpression.Count > 0)
                        {
                            foreach (var item in search.listLHisExpMestMetyReqExpression)
                            {
