...
namespace MOS.DAO.HisExpMestMetyReq
{
    ...
    {
        public List<V_HIS_EXP_MEST_METY_REQ_1> GetView1(HisExpMestMetyReqSO search, CommonParam param)
        {
            List<V_HIS_EXP_MEST_METY_REQ_1> list = new List<V_HIS_EXP_MEST_METY_REQ_1>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_EXP_MEST_METY_REQ_1.AsNoTracking().AsQueryable();
                        if (search.listVHisExpMestMetyReq1Expression != null && search.listVHisExpMestMetyReq1Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisExpMestMetyReq1Expression)
                            {
