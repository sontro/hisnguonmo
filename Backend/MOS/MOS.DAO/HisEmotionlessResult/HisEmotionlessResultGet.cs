...
namespace MOS.DAO.HisEmotionlessResult
{
    partial class HisEmotionlessResultGet : EntityBase
    {
        ...
        {
            List<HIS_EMOTIONLESS_RESULT> list = new List<HIS_EMOTIONLESS_RESULT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_EMOTIONLESS_RESULT.AsNoTracking().AsQueryable();
        ...
        public HIS_EMOTIONLESS_RESULT GetById(long id, HisEmotionlessResultSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_EMOTIONLESS_RESULT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisEmotionlessResultExpression != null && search.listHisEmotionlessResultExpression.Count > 0)
                        {
                            foreach (var item in search.listHisEmotionlessResultExpression)
                            {
