...
namespace MOS.DAO.HisMestPatientType
{
    partial class HisMestPatientTypeGet : EntityBase
    {
        ...
        {
            List<HIS_MEST_PATIENT_TYPE> list = new List<HIS_MEST_PATIENT_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MEST_PATIENT_TYPE.AsNoTracking().AsQueryable();
        ...
        public HIS_MEST_PATIENT_TYPE GetById(long id, HisMestPatientTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_MEST_PATIENT_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMestPatientTypeExpression != null && search.listHisMestPatientTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMestPatientTypeExpression)
                            {
