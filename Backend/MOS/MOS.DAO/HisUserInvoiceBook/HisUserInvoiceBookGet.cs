...
namespace MOS.DAO.HisUserInvoiceBook
{
    partial class HisUserInvoiceBookGet : EntityBase
    {
        ...
        {
            List<HIS_USER_INVOICE_BOOK> list = new List<HIS_USER_INVOICE_BOOK>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_USER_INVOICE_BOOK.AsNoTracking().AsQueryable();
        ...
        public HIS_USER_INVOICE_BOOK GetById(long id, HisUserInvoiceBookSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_USER_INVOICE_BOOK.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisUserInvoiceBookExpression != null && search.listHisUserInvoiceBookExpression.Count > 0)
                        {
                            foreach (var item in search.listHisUserInvoiceBookExpression)
                            {
