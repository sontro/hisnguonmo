...
namespace MOS.DAO.HisMatyMaty
{
    partial class HisMatyMatyGet : EntityBase
    {
        ...
        {
            List<HIS_MATY_MATY> list = new List<HIS_MATY_MATY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MATY_MATY.AsNoTracking().AsQueryable();
        ...
        public HIS_MATY_MATY GetById(long id, HisMatyMatySO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_MATY_MATY.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMatyMatyExpression != null && search.listHisMatyMatyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMatyMatyExpression)
                            {
