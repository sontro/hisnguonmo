...
namespace MOS.DAO.HisMediRecord
{
    partial class HisMediRecordGet : EntityBase
    {
        ...
        {
            List<HIS_MEDI_RECORD> list = new List<HIS_MEDI_RECORD>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MEDI_RECORD.AsNoTracking().AsQueryable();
        ...
        public HIS_MEDI_RECORD GetById(long id, HisMediRecordSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_MEDI_RECORD.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMediRecordExpression != null && search.listHisMediRecordExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMediRecordExpression)
                            {
