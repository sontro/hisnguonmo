...
{
    partial class HisMediRecordDelete : EntityBase
    {
        public HisMediRecordDelete()
            : base()
        {
        ...
        public bool Delete(HIS_MEDI_RECORD data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_MEDI_RECORD> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_MEDI_RECORD> listIdNotFound = new List<HIS_MEDI_RECORD>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_MEDI_RECORD> listRaw = ctx.HIS_MEDI_RECORD.Where(o => listId.Contains(o.ID)).ToList();
