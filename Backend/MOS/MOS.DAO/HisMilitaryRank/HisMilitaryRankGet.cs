...
namespace MOS.DAO.HisMilitaryRank
{
    partial class HisMilitaryRankGet : EntityBase
    {
        ...
        {
            List<HIS_MILITARY_RANK> list = new List<HIS_MILITARY_RANK>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MILITARY_RANK.AsNoTracking().AsQueryable();
        ...
        public HIS_MILITARY_RANK GetById(long id, HisMilitaryRankSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_MILITARY_RANK.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMilitaryRankExpression != null && search.listHisMilitaryRankExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMilitaryRankExpression)
                            {
