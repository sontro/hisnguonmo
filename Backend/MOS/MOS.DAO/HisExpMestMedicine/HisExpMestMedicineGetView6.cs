...
namespace MOS.DAO.HisExpMestMedicine
{
    partial class HisExpMestMedicineGet : EntityBase
    {
        ...
        {
            List<V_HIS_EXP_MEST_MEDICINE_6> list = new List<V_HIS_EXP_MEST_MEDICINE_6>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_EXP_MEST_MEDICINE_6.AsNoTracking().AsQueryable();
                        if (search.listVHisExpMestMedicine6Expression != null && search.listVHisExpMestMedicine6Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisExpMestMedicine6Expression)
                            {
