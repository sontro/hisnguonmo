...
namespace MOS.DAO.HisExpMestMedicine
{
    partial class HisExpMestMedicineGet : EntityBase
    {
        ...
        {
            List<V_HIS_EXP_MEST_MEDICINE_1> list = new List<V_HIS_EXP_MEST_MEDICINE_1>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_EXP_MEST_MEDICINE_1.AsNoTracking().AsQueryable();
                        if (search.listVHisExpMestMedicine1Expression != null && search.listVHisExpMestMedicine1Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisExpMestMedicine1Expression)
                            {
