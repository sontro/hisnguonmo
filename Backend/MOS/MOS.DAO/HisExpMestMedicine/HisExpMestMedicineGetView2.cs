...
namespace MOS.DAO.HisExpMestMedicine
{
    partial class HisExpMestMedicineGet : EntityBase
    {
        ...
        {
            List<V_HIS_EXP_MEST_MEDICINE_2> list = new List<V_HIS_EXP_MEST_MEDICINE_2>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_EXP_MEST_MEDICINE_2.AsNoTracking().AsQueryable();
                        if (search.listVHisExpMestMedicine2Expression != null && search.listVHisExpMestMedicine2Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisExpMestMedicine2Expression)
                            {
