...
namespace MOS.DAO.HisExpMestMedicine
{
    partial class HisExpMestMedicineGet : EntityBase
    {
        ...
        {
            List<V_HIS_EXP_MEST_MEDICINE_4> list = new List<V_HIS_EXP_MEST_MEDICINE_4>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_EXP_MEST_MEDICINE_4.AsNoTracking().AsQueryable();
                        if (search.listVHisExpMestMedicine4Expression != null && search.listVHisExpMestMedicine4Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisExpMestMedicine4Expression)
                            {
