...
namespace MOS.DAO.HisVaccReactPlace
{
    partial class HisVaccReactPlaceGet : EntityBase
    {
        ...
        {
            List<HIS_VACC_REACT_PLACE> list = new List<HIS_VACC_REACT_PLACE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_VACC_REACT_PLACE.AsNoTracking().AsQueryable();
        ...
        public HIS_VACC_REACT_PLACE GetById(long id, HisVaccReactPlaceSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_VACC_REACT_PLACE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisVaccReactPlaceExpression != null && search.listHisVaccReactPlaceExpression.Count > 0)
                        {
                            foreach (var item in search.listHisVaccReactPlaceExpression)
                            {
