...
namespace MOS.DAO.HisUserGroupTempDt
{
    partial class HisUserGroupTempDtGet : EntityBase
    {
        ...
        {
            List<HIS_USER_GROUP_TEMP_DT> list = new List<HIS_USER_GROUP_TEMP_DT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_USER_GROUP_TEMP_DT.AsNoTracking().AsQueryable();
        ...
        public HIS_USER_GROUP_TEMP_DT GetById(long id, HisUserGroupTempDtSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_USER_GROUP_TEMP_DT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisUserGroupTempDtExpression != null && search.listHisUserGroupTempDtExpression.Count > 0)
                        {
                            foreach (var item in search.listHisUserGroupTempDtExpression)
                            {
