...
namespace MOS.DAO.HisMestPeriodMety
{
    partial class HisMestPeriodMetyGet : EntityBase
    {
        ...
        {
            List<HIS_MEST_PERIOD_METY> list = new List<HIS_MEST_PERIOD_METY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MEST_PERIOD_METY.AsNoTracking().AsQueryable();
        ...
        public HIS_MEST_PERIOD_METY GetById(long id, HisMestPeriodMetySO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_MEST_PERIOD_METY.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMestPeriodMetyExpression != null && search.listHisMestPeriodMetyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMestPeriodMetyExpression)
                            {
