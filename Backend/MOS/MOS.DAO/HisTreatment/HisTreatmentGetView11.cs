...
namespace MOS.DAO.HisTreatment
{
    ...
    {
        public List<V_HIS_TREATMENT_11> GetView11(HisTreatmentSO search, CommonParam param)
        {
            List<V_HIS_TREATMENT_11> list = new List<V_HIS_TREATMENT_11>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_TREATMENT_11.AsQueryable();
                        if (search.listVHisTreatment11Expression != null && search.listVHisTreatment11Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisTreatment11Expression)
                            {
