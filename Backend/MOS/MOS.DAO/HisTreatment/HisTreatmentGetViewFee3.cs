...
namespace MOS.DAO.HisTreatment
{
    ...
    {
        public List<V_HIS_TREATMENT_FEE_3> GetFeeView3(HisTreatmentSO search, CommonParam param)
        {
            List<V_HIS_TREATMENT_FEE_3> list = new List<V_HIS_TREATMENT_FEE_3>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_TREATMENT_FEE_3.AsNoTracking().AsQueryable();
                        if (search.listVHisTreatmentFee3Expression != null && search.listVHisTreatmentFee3Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisTreatmentFee3Expression)
                            {
        ...
        public V_HIS_TREATMENT_FEE_3 GetFeeView3ById(long id, HisTreatmentSO search)
        {
            V_HIS_TREATMENT_FEE_3 result = null;
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.V_HIS_TREATMENT_FEE_3.AsQueryable().Where(p => p.ID == id);
                        if (search.listVHisTreatmentFee3Expression != null && search.listVHisTreatmentFee3Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisTreatmentFee3Expression)
                            {
