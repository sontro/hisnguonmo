...
namespace MOS.DAO.HisTreatment
{
    ...
    {
        public List<V_HIS_TREATMENT_10> GetView10(HisTreatmentSO search, CommonParam param)
        {
            List<V_HIS_TREATMENT_10> list = new List<V_HIS_TREATMENT_10>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_TREATMENT_10.AsQueryable();
                        if (search.listVHisTreatment10Expression != null && search.listVHisTreatment10Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisTreatment10Expression)
                            {
