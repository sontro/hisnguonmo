...
namespace MOS.DAO.HisTreatment
{
    ...
    {
        public List<V_HIS_TREATMENT_2> GetView2(HisTreatmentSO search, CommonParam param)
        {
            List<V_HIS_TREATMENT_2> list = new List<V_HIS_TREATMENT_2>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_TREATMENT_2.AsNoTracking().AsQueryable();
                        if (search.listVHisTreatment2Expression != null && search.listVHisTreatment2Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisTreatment2Expression)
                            {
