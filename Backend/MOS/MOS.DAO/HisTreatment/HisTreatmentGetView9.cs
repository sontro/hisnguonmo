...
namespace MOS.DAO.HisTreatment
{
    ...
    {
        public List<V_HIS_TREATMENT_9> GetView9(HisTreatmentSO search, CommonParam param)
        {
            List<V_HIS_TREATMENT_9> list = new List<V_HIS_TREATMENT_9>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_TREATMENT_9.AsQueryable();
                        if (search.listVHisTreatment9Expression != null && search.listVHisTreatment9Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisTreatment9Expression)
                            {
