...
{
    partial class HisTreatmentDelete : EntityBase
    {
        public HisTreatmentDelete()
            : base()
        {
        ...
        public bool Delete(HIS_TREATMENT data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_TREATMENT> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_TREATMENT> listIdNotFound = new List<HIS_TREATMENT>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_TREATMENT> listRaw = ctx.HIS_TREATMENT.Where(o => listId.Contains(o.ID)).ToList();
