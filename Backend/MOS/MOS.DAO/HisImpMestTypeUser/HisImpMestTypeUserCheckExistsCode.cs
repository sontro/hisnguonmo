    ...
    partial class HisImpMestTypeUserCheck : EntityBase
    {
        public bool ExistsCode(string code, long? id)
        {
            ...
            try
            {
                ...
                {
                    count = ctx.HIS_IMP_MEST_TYPE_USER.AsQueryable().Where(p => p.IMP_MEST_TYPE_USER_CODE.Equals(code) && p.ID != id && (p.IS_DELETE.HasValue || p.IS_DELETE.Value != (short)1)).Count();
                }
                result = (count > 0);
            }
            catch (Exception)
