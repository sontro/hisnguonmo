...
namespace MOS.DAO.HisTrackingTemp
{
    partial class HisTrackingTempGet : EntityBase
    {
        ...
        {
            List<HIS_TRACKING_TEMP> list = new List<HIS_TRACKING_TEMP>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_TRACKING_TEMP.AsNoTracking().AsQueryable();
                        if (search.listHisTrackingTempExpression != null && search.listHisTrackingTempExpression.Count > 0)
                        {
                            foreach (var item in search.listHisTrackingTempExpression)
                            {
        ...
        public HIS_TRACKING_TEMP GetById(long id, HisTrackingTempSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_TRACKING_TEMP.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisTrackingTempExpression != null && search.listHisTrackingTempExpression.Count > 0)
                        {
                            foreach (var item in search.listHisTrackingTempExpression)
                            {
