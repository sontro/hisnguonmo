...
{
    partial class HisTrackingTempGet : EntityBase
    {
        public List<V_HIS_TRACKING_TEMP> GetView(HisTrackingTempSO search, CommonParam param)
        {
            List<V_HIS_TRACKING_TEMP> list = new List<V_HIS_TRACKING_TEMP>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_TRACKING_TEMP.AsNoTracking().AsQueryable();
                        if (search.listVHisTrackingTempExpression != null && search.listVHisTrackingTempExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisTrackingTempExpression)
                            {
