...
namespace MOS.DAO.HisVaccinationStt
{
    partial class HisVaccinationSttGet : EntityBase
    {
        ...
        {
            List<HIS_VACCINATION_STT> list = new List<HIS_VACCINATION_STT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_VACCINATION_STT.AsNoTracking().AsQueryable();
        ...
        public HIS_VACCINATION_STT GetById(long id, HisVaccinationSttSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_VACCINATION_STT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisVaccinationSttExpression != null && search.listHisVaccinationSttExpression.Count > 0)
                        {
                            foreach (var item in search.listHisVaccinationSttExpression)
                            {
