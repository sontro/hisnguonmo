...
namespace MOS.DAO.HisLicenseClass
{
    partial class HisLicenseClassGet : EntityBase
    {
        ...
        {
            List<HIS_LICENSE_CLASS> list = new List<HIS_LICENSE_CLASS>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_LICENSE_CLASS.AsNoTracking().AsQueryable();
                        if (search.listHisLicenseClassExpression != null && search.listHisLicenseClassExpression.Count > 0)
                        {
                            foreach (var item in search.listHisLicenseClassExpression)
                            {
        ...
        public HIS_LICENSE_CLASS GetById(long id, HisLicenseClassSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_LICENSE_CLASS.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisLicenseClassExpression != null && search.listHisLicenseClassExpression.Count > 0)
                        {
                            foreach (var item in search.listHisLicenseClassExpression)
                            {
