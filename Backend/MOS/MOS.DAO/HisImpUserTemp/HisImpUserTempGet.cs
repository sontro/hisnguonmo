...
namespace MOS.DAO.HisImpUserTemp
{
    partial class HisImpUserTempGet : EntityBase
    {
        ...
        {
            List<HIS_IMP_USER_TEMP> list = new List<HIS_IMP_USER_TEMP>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_IMP_USER_TEMP.AsNoTracking().AsQueryable();
        ...
        public HIS_IMP_USER_TEMP GetById(long id, HisImpUserTempSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_IMP_USER_TEMP.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisImpUserTempExpression != null && search.listHisImpUserTempExpression.Count > 0)
                        {
                            foreach (var item in search.listHisImpUserTempExpression)
                            {
