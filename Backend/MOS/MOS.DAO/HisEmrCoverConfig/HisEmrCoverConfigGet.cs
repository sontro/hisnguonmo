...
namespace MOS.DAO.HisEmrCoverConfig
{
    partial class HisEmrCoverConfigGet : EntityBase
    {
        ...
        {
            List<HIS_EMR_COVER_CONFIG> list = new List<HIS_EMR_COVER_CONFIG>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_EMR_COVER_CONFIG.AsNoTracking().AsQueryable();
        ...
        public HIS_EMR_COVER_CONFIG GetById(long id, HisEmrCoverConfigSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_EMR_COVER_CONFIG.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisEmrCoverConfigExpression != null && search.listHisEmrCoverConfigExpression.Count > 0)
                        {
                            foreach (var item in search.listHisEmrCoverConfigExpression)
                            {
