...
namespace MOS.DAO.HisArea
{
    partial class HisAreaGet : EntityBase
    {
        ...
        {
            List<V_HIS_AREA> list = new List<V_HIS_AREA>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_AREA.AsNoTracking().AsQueryable();
                        if (search.listVHisAreaExpression != null && search.listVHisAreaExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisAreaExpression)
                            {
