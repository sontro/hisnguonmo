...
namespace MOS.DAO.HisServicePackage
{
    partial class HisServicePackageGet : EntityBase
    {
        ...
        {
            List<HIS_SERVICE_PACKAGE> list = new List<HIS_SERVICE_PACKAGE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SERVICE_PACKAGE.AsNoTracking().AsQueryable();
                        if (search.listHisServicePackageExpression != null && search.listHisServicePackageExpression.Count > 0)
                        {
                            foreach (var item in search.listHisServicePackageExpression)
                            {
        ...
        public HIS_SERVICE_PACKAGE GetById(long id, HisServicePackageSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_SERVICE_PACKAGE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisServicePackageExpression != null && search.listHisServicePackageExpression.Count > 0)
                        {
                            foreach (var item in search.listHisServicePackageExpression)
                            {
