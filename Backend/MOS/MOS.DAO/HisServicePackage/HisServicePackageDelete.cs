...
{
    partial class HisServicePackageDelete : EntityBase
    {
        public HisServicePackageDelete()
            : base()
        {
        ...
        public bool Delete(HIS_SERVICE_PACKAGE data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_SERVICE_PACKAGE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_SERVICE_PACKAGE> listIdNotFound = new List<HIS_SERVICE_PACKAGE>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_SERVICE_PACKAGE> listRaw = ctx.HIS_SERVICE_PACKAGE.Where(o => listId.Contains(o.ID)).ToList();
