...
namespace MOS.DAO.HisServiceRetyCat
{
    ...
    {
        public V_HIS_SERVICE_RETY_CAT GetViewByCode(string code, HisServiceRetyCatSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_SERVICE_RETY_CAT.AsQueryable().Where(p => p.SERVICE_RETY_CAT_CODE == code);
