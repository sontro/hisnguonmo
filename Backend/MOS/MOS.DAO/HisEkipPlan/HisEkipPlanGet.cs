...
namespace MOS.DAO.HisEkipPlan
{
    partial class HisEkipPlanGet : EntityBase
    {
        ...
        {
            List<HIS_EKIP_PLAN> list = new List<HIS_EKIP_PLAN>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_EKIP_PLAN.AsNoTracking().AsQueryable();
        ...
        public HIS_EKIP_PLAN GetById(long id, HisEkipPlanSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_EKIP_PLAN.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisEkipPlanExpression != null && search.listHisEkipPlanExpression.Count > 0)
                        {
                            foreach (var item in search.listHisEkipPlanExpression)
                            {
