...
namespace MOS.DAO.HisEkipPlan
{
    partial class HisEkipPlanGet : EntityBase
    {
        ...
        {
            List<V_HIS_EKIP_PLAN> list = new List<V_HIS_EKIP_PLAN>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_EKIP_PLAN.AsNoTracking().AsQueryable();
                        if (search.listVHisEkipPlanExpression != null && search.listVHisEkipPlanExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisEkipPlanExpression)
                            {
