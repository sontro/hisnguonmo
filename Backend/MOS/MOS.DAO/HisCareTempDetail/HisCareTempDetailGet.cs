...
{
    partial class HisCareTempDetailGet : EntityBase
    {
        public List<HIS_CARE_TEMP_DETAIL> Get(HisCareTempDetailSO search, CommonParam param)
        {
            List<HIS_CARE_TEMP_DETAIL> list = new List<HIS_CARE_TEMP_DETAIL>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_CARE_TEMP_DETAIL.AsNoTracking().AsQueryable();
                        if (search.listHisCareTempDetailExpression != null && search.listHisCareTempDetailExpression.Count > 0)
                        {
                            foreach (var item in search.listHisCareTempDetailExpression)
                            {
        ...
        public HIS_CARE_TEMP_DETAIL GetById(long id, HisCareTempDetailSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_CARE_TEMP_DETAIL.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisCareTempDetailExpression != null && search.listHisCareTempDetailExpression.Count > 0)
                        {
                            foreach (var item in search.listHisCareTempDetailExpression)
                            {
