...
namespace MOS.DAO.HisNumOrderBlock
{
    partial class HisNumOrderBlockGet : EntityBase
    {
        ...
        {
            List<HIS_NUM_ORDER_BLOCK> list = new List<HIS_NUM_ORDER_BLOCK>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_NUM_ORDER_BLOCK.AsNoTracking().AsQueryable();
        ...
        public HIS_NUM_ORDER_BLOCK GetById(long id, HisNumOrderBlockSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_NUM_ORDER_BLOCK.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisNumOrderBlockExpression != null && search.listHisNumOrderBlockExpression.Count > 0)
                        {
                            foreach (var item in search.listHisNumOrderBlockExpression)
                            {
