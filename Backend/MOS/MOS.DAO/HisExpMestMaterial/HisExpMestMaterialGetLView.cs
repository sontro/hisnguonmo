...
namespace MOS.DAO.HisExpMestMaterial
{
    partial class HisExpMestMaterialGet : EntityBase
    {
        ...
        {
            List<L_HIS_EXP_MEST_MATERIAL> list = new List<L_HIS_EXP_MEST_MATERIAL>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.L_HIS_EXP_MEST_MATERIAL.AsNoTracking().AsQueryable();
                        if (search.listLHisExpMestMaterialExpression != null && search.listLHisExpMestMaterialExpression.Count > 0)
                        {
                            foreach (var item in search.listLHisExpMestMaterialExpression)
                            {
