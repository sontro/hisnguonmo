...
namespace MOS.DAO.HisExpMestMaterial
{
    partial class HisExpMestMaterialGet : EntityBase
    {
        ...
        {
            List<L_HIS_EXP_MEST_MATERIAL_2> list = new List<L_HIS_EXP_MEST_MATERIAL_2>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.L_HIS_EXP_MEST_MATERIAL_2.AsNoTracking().AsQueryable();
                        if (search.listLHisExpMestMaterial2Expression != null && search.listLHisExpMestMaterial2Expression.Count > 0)
                        {
                            foreach (var item in search.listLHisExpMestMaterial2Expression)
                            {
