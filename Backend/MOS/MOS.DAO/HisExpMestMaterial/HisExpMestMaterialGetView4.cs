...
namespace MOS.DAO.HisExpMestMaterial
{
    partial class HisExpMestMaterialGet : EntityBase
    {
        ...
        {
            List<V_HIS_EXP_MEST_MATERIAL_4> list = new List<V_HIS_EXP_MEST_MATERIAL_4>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_EXP_MEST_MATERIAL_4.AsNoTracking().AsQueryable();
                        if (search.listVHisExpMestMaterial4Expression != null && search.listVHisExpMestMaterial4Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisExpMestMaterial4Expression)
                            {
