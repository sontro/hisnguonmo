...
namespace MOS.DAO.HisVaccineType
{
    ...
    {
        public List<V_HIS_VACCINE_TYPE> GetView(HisVaccineTypeSO search, CommonParam param)
        {
            List<V_HIS_VACCINE_TYPE> list = new List<V_HIS_VACCINE_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_VACCINE_TYPE.AsNoTracking().AsQueryable();
