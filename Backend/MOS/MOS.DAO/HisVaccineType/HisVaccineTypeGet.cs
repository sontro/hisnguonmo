...
namespace MOS.DAO.HisVaccineType
{
    partial class HisVaccineTypeGet : EntityBase
    {
        ...
        {
            List<HIS_VACCINE_TYPE> list = new List<HIS_VACCINE_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_VACCINE_TYPE.AsNoTracking().AsQueryable();
                        if (search.listHisVaccineTypeExpression != null && search.listHisVaccineTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisVaccineTypeExpression)
                            {
        ...
        public HIS_VACCINE_TYPE GetById(long id, HisVaccineTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_VACCINE_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisVaccineTypeExpression != null && search.listHisVaccineTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisVaccineTypeExpression)
                            {
