...
namespace MOS.DAO.HisDepositReq
{
    partial class HisDepositReqGet : EntityBase
    {
        ...
        {
            List<V_HIS_DEPOSIT_REQ> list = new List<V_HIS_DEPOSIT_REQ>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_DEPOSIT_REQ.AsNoTracking().AsQueryable();
                        if (search.listVHisDepositReqExpression != null && search.listVHisDepositReqExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisDepositReqExpression)
                            {
