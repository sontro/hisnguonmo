...
namespace MOS.DAO.HisKskOther
{
    partial class HisKskOtherGet : EntityBase
    {
        ...
        {
            List<HIS_KSK_OTHER> list = new List<HIS_KSK_OTHER>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_KSK_OTHER.AsNoTracking().AsQueryable();
        ...
        public HIS_KSK_OTHER GetById(long id, HisKskOtherSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_KSK_OTHER.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisKskOtherExpression != null && search.listHisKskOtherExpression.Count > 0)
                        {
                            foreach (var item in search.listHisKskOtherExpression)
                            {
