...
namespace MOS.DAO.HisUnlimitReason
{
    partial class HisUnlimitReasonGet : EntityBase
    {
        ...
        {
            List<V_HIS_UNLIMIT_REASON> list = new List<V_HIS_UNLIMIT_REASON>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_UNLIMIT_REASON.AsNoTracking().AsQueryable();
                        if (search.listVHisUnlimitReasonExpression != null && search.listVHisUnlimitReasonExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisUnlimitReasonExpression)
                            {
