...
namespace MOS.DAO.HisUnlimitReason
{
    partial class HisUnlimitReasonGet : EntityBase
    {
        ...
        {
            List<HIS_UNLIMIT_REASON> list = new List<HIS_UNLIMIT_REASON>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_UNLIMIT_REASON.AsNoTracking().AsQueryable();
        ...
        public HIS_UNLIMIT_REASON GetById(long id, HisUnlimitReasonSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_UNLIMIT_REASON.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisUnlimitReasonExpression != null && search.listHisUnlimitReasonExpression.Count > 0)
                        {
                            foreach (var item in search.listHisUnlimitReasonExpression)
                            {
