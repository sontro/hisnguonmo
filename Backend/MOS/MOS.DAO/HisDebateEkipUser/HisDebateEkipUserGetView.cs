...
{
    partial class HisDebateEkipUserGet : EntityBase
    {
        public List<V_HIS_DEBATE_EKIP_USER> GetView(HisDebateEkipUserSO search, CommonParam param)
        {
            List<V_HIS_DEBATE_EKIP_USER> list = new List<V_HIS_DEBATE_EKIP_USER>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_DEBATE_EKIP_USER.AsNoTracking().AsQueryable();
                        if (search.listVHisDebateEkipUserExpression != null && search.listVHisDebateEkipUserExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisDebateEkipUserExpression)
                            {
