        ...
        public List<HIS_DEBATE_EKIP_USER> Get(HisDebateEkipUserSO search, CommonParam param)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_DEBATE_EKIP_USER.AsNoTracking().AsQueryable();
                        if (search.listHisDebateEkipUserExpression != null && search.listHisDebateEkipUserExpression.Count > 0)
                        {
                            foreach (var item in search.listHisDebateEkipUserExpression)
                            {
        ...
        public HIS_DEBATE_EKIP_USER GetById(long id, HisDebateEkipUserSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_DEBATE_EKIP_USER.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisDebateEkipUserExpression != null && search.listHisDebateEkipUserExpression.Count > 0)
                        {
                            foreach (var item in search.listHisDebateEkipUserExpression)
                            {
