...
namespace MOS.DAO.HisServiceNumOrder
{
    partial class HisServiceNumOrderGet : EntityBase
    {
        ...
        {
            List<V_HIS_SERVICE_NUM_ORDER> list = new List<V_HIS_SERVICE_NUM_ORDER>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_SERVICE_NUM_ORDER.AsNoTracking().AsQueryable();
                        if (search.listVHisServiceNumOrderExpression != null && search.listVHisServiceNumOrderExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisServiceNumOrderExpression)
                            {
