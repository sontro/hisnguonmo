...
{
    partial class HisServiceNumOrderDelete : EntityBase
    {
        public HisServiceNumOrderDelete()
            : base()
        {
        ...
        public bool Delete(HIS_SERVICE_NUM_ORDER data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_SERVICE_NUM_ORDER> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_SERVICE_NUM_ORDER> listIdNotFound = new List<HIS_SERVICE_NUM_ORDER>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_SERVICE_NUM_ORDER> listRaw = ctx.HIS_SERVICE_NUM_ORDER.Where(o => listId.Contains(o.ID)).ToList();
