...
namespace MOS.DAO.HisSpeedUnit
{
    partial class HisSpeedUnitGet : EntityBase
    {
        ...
        {
            List<V_HIS_SPEED_UNIT> list = new List<V_HIS_SPEED_UNIT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_SPEED_UNIT.AsNoTracking().AsQueryable();
                        if (search.listVHisSpeedUnitExpression != null && search.listVHisSpeedUnitExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisSpeedUnitExpression)
                            {
