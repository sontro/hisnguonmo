...
namespace MOS.DAO.HisSpeedUnit
{
    partial class HisSpeedUnitGet : EntityBase
    {
        ...
        {
            List<HIS_SPEED_UNIT> list = new List<HIS_SPEED_UNIT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SPEED_UNIT.AsNoTracking().AsQueryable();
        ...
        public HIS_SPEED_UNIT GetById(long id, HisSpeedUnitSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_SPEED_UNIT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisSpeedUnitExpression != null && search.listHisSpeedUnitExpression.Count > 0)
                        {
                            foreach (var item in search.listHisSpeedUnitExpression)
                            {
