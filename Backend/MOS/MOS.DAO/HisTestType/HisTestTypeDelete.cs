...
{
    partial class HisTestTypeDelete : EntityBase
    {
        public HisTestTypeDelete()
            : base()
        {
        ...
        public bool Delete(HIS_TEST_TYPE data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_TEST_TYPE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_TEST_TYPE> listIdNotFound = new List<HIS_TEST_TYPE>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_TEST_TYPE> listRaw = ctx.HIS_TEST_TYPE.Where(o => listId.Contains(o.ID)).ToList();
