...
namespace MOS.DAO.HisServiceReqMety
{
    partial class HisServiceReqMetyGet : EntityBase
    {
        ...
        {
            List<V_HIS_SERVICE_REQ_METY> list = new List<V_HIS_SERVICE_REQ_METY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_SERVICE_REQ_METY.AsQueryable();
                        if (search.listVHisServiceReqMetyExpression != null && search.listVHisServiceReqMetyExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisServiceReqMetyExpression)
                            {
