...
{
    partial class HisPatientTypeRoomGet : EntityBase
    {
        public List<HIS_PATIENT_TYPE_ROOM> Get(HisPatientTypeRoomSO search, CommonParam param)
        {
            List<HIS_PATIENT_TYPE_ROOM> list = new List<HIS_PATIENT_TYPE_ROOM>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_PATIENT_TYPE_ROOM.AsNoTracking().AsQueryable();
                        if (search.listHisPatientTypeRoomExpression != null && search.listHisPatientTypeRoomExpression.Count > 0)
                        {
                            foreach (var item in search.listHisPatientTypeRoomExpression)
                            {
        ...
        public HIS_PATIENT_TYPE_ROOM GetById(long id, HisPatientTypeRoomSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_PATIENT_TYPE_ROOM.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisPatientTypeRoomExpression != null && search.listHisPatientTypeRoomExpression.Count > 0)
                        {
                            foreach (var item in search.listHisPatientTypeRoomExpression)
                            {
