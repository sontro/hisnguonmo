...
namespace MOS.DAO.HisPatientTypeRoom
{
    partial class HisPatientTypeRoomGet : EntityBase
    {
        ...
        {
            List<V_HIS_PATIENT_TYPE_ROOM> list = new List<V_HIS_PATIENT_TYPE_ROOM>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_PATIENT_TYPE_ROOM.AsNoTracking().AsQueryable();
                        if (search.listVHisPatientTypeRoomExpression != null && search.listVHisPatientTypeRoomExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisPatientTypeRoomExpression)
                            {
