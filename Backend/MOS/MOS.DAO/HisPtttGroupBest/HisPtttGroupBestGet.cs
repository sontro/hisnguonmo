...
namespace MOS.DAO.HisPtttGroupBest
{
    partial class HisPtttGroupBestGet : EntityBase
    {
        ...
        {
            List<HIS_PTTT_GROUP_BEST> list = new List<HIS_PTTT_GROUP_BEST>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_PTTT_GROUP_BEST.AsNoTracking().AsQueryable();
        ...
        public HIS_PTTT_GROUP_BEST GetById(long id, HisPtttGroupBestSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_PTTT_GROUP_BEST.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisPtttGroupBestExpression != null && search.listHisPtttGroupBestExpression.Count > 0)
                        {
                            foreach (var item in search.listHisPtttGroupBestExpression)
                            {
