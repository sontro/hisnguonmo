...
namespace MOS.DAO.HisPtttGroupBest
{
    ...
    {
        public List<V_HIS_PTTT_GROUP_BEST> GetView(HisPtttGroupBestSO search, CommonParam param)
        {
            List<V_HIS_PTTT_GROUP_BEST> list = new List<V_HIS_PTTT_GROUP_BEST>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_PTTT_GROUP_BEST.AsNoTracking().AsQueryable();
