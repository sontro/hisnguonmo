...
{
    partial class HisAdrDelete : EntityBase
    {
        public HisAdrDelete()
            : base()
        {
        ...
        public bool Delete(HIS_ADR data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_ADR> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_ADR> listIdNotFound = new List<HIS_ADR>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_ADR> listRaw = ctx.HIS_ADR.Where(o => listId.Contains(o.ID)).ToList();
