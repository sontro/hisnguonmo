...
{
    partial class HisAdrGet : EntityBase
    {
        public List<HIS_ADR> Get(HisAdrSO search, CommonParam param)
        {
            List<HIS_ADR> list = new List<HIS_ADR>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_ADR.AsNoTracking().AsQueryable();
                        if (search.listHisAdrExpression != null && search.listHisAdrExpression.Count > 0)
                        {
                            foreach (var item in search.listHisAdrExpression)
                            {
        ...
        public HIS_ADR GetById(long id, HisAdrSO search)
        {
            HIS_ADR result = null;
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_ADR.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisAdrExpression != null && search.listHisAdrExpression.Count > 0)
                        {
                            foreach (var item in search.listHisAdrExpression)
                            {
