...
{
    partial class HisCareTypeGet : EntityBase
    {
        public List<HIS_CARE_TYPE> Get(HisCareTypeSO search, CommonParam param)
        {
            List<HIS_CARE_TYPE> list = new List<HIS_CARE_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_CARE_TYPE.AsNoTracking().AsQueryable();
                        if (search.listHisCareTypeExpression != null && search.listHisCareTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisCareTypeExpression)
                            {
        ...
        public HIS_CARE_TYPE GetById(long id, HisCareTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_CARE_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisCareTypeExpression != null && search.listHisCareTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisCareTypeExpression)
                            {
