...
namespace MOS.DAO.HisMediRecordBorrow
{
    partial class HisMediRecordBorrowGet : EntityBase
    {
        ...
        {
            List<HIS_MEDI_RECORD_BORROW> list = new List<HIS_MEDI_RECORD_BORROW>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MEDI_RECORD_BORROW.AsNoTracking().AsQueryable();
        ...
        public HIS_MEDI_RECORD_BORROW GetById(long id, HisMediRecordBorrowSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_MEDI_RECORD_BORROW.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMediRecordBorrowExpression != null && search.listHisMediRecordBorrowExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMediRecordBorrowExpression)
                            {
