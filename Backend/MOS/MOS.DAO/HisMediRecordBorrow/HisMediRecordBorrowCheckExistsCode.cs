    ...
    partial class HisMediRecordBorrowCheck : EntityBase
    {
        public bool ExistsCode(string code, long? id)
        {
            ...
            try
            {
                ...
                {
                    count = ctx.HIS_MEDI_RECORD_BORROW.AsQueryable().Where(p => p.MEDI_RECORD_BORROW_CODE.Equals(code) && p.ID != id && (p.IS_DELETE.HasValue || p.IS_DELETE.Value != (short)1)).Count();
                }
                result = (count > 0);
            }
            catch (Exception)
