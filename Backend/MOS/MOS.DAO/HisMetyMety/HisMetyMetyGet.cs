...
namespace MOS.DAO.HisMetyMety
{
    partial class HisMetyMetyGet : EntityBase
    {
        ...
        {
            List<HIS_METY_METY> list = new List<HIS_METY_METY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_METY_METY.AsNoTracking().AsQueryable();
                        if (search.listHisMetyMetyExpression != null && search.listHisMetyMetyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMetyMetyExpression)
                            {
        ...
        public HIS_METY_METY GetById(long id, HisMetyMetySO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_METY_METY.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMetyMetyExpression != null && search.listHisMetyMetyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMetyMetyExpression)
                            {
