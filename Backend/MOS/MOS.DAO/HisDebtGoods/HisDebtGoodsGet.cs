...
namespace MOS.DAO.HisDebtGoods
{
    partial class HisDebtGoodsGet : EntityBase
    {
        ...
        {
            List<HIS_DEBT_GOODS> list = new List<HIS_DEBT_GOODS>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_DEBT_GOODS.AsNoTracking().AsQueryable();
        ...
        public HIS_DEBT_GOODS GetById(long id, HisDebtGoodsSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_DEBT_GOODS.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisDebtGoodsExpression != null && search.listHisDebtGoodsExpression.Count > 0)
                        {
                            foreach (var item in search.listHisDebtGoodsExpression)
                            {
