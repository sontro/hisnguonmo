...
namespace MOS.DAO.HisNoneMediService
{
    partial class HisNoneMediServiceGet : EntityBase
    {
        ...
        {
            List<HIS_NONE_MEDI_SERVICE> list = new List<HIS_NONE_MEDI_SERVICE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_NONE_MEDI_SERVICE.AsNoTracking().AsQueryable();
        ...
        public HIS_NONE_MEDI_SERVICE GetById(long id, HisNoneMediServiceSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_NONE_MEDI_SERVICE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisNoneMediServiceExpression != null && search.listHisNoneMediServiceExpression.Count > 0)
                        {
                            foreach (var item in search.listHisNoneMediServiceExpression)
                            {
