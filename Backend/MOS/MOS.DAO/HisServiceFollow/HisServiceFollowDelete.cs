...
{
    partial class HisServiceFollowDelete : EntityBase
    {
        public HisServiceFollowDelete()
            : base()
        {
        ...
        public bool Delete(HIS_SERVICE_FOLLOW data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_SERVICE_FOLLOW> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_SERVICE_FOLLOW> listIdNotFound = new List<HIS_SERVICE_FOLLOW>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_SERVICE_FOLLOW> listRaw = ctx.HIS_SERVICE_FOLLOW.Where(o => listId.Contains(o.ID)).ToList();
