...
namespace MOS.DAO.HisPtttHighTech
{
    partial class HisPtttHighTechGet : EntityBase
    {
        ...
        {
            List<HIS_PTTT_HIGH_TECH> list = new List<HIS_PTTT_HIGH_TECH>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_PTTT_HIGH_TECH.AsNoTracking().AsQueryable();
        ...
        public HIS_PTTT_HIGH_TECH GetById(long id, HisPtttHighTechSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_PTTT_HIGH_TECH.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisPtttHighTechExpression != null && search.listHisPtttHighTechExpression.Count > 0)
                        {
                            foreach (var item in search.listHisPtttHighTechExpression)
                            {
