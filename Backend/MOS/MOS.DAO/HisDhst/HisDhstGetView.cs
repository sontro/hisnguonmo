...
namespace MOS.DAO.HisDhst
{
    partial class HisDhstGet : EntityBase
    {
        ...
        {
            List<V_HIS_DHST> list = new List<V_HIS_DHST>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_DHST.AsQueryable();
                        if (search.listVHisDhstExpression != null && search.listVHisDhstExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisDhstExpression)
                            {
