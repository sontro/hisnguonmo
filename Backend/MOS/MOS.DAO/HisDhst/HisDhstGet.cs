...
{
    partial class HisDhstGet : EntityBase
    {
        public List<HIS_DHST> Get(HisDhstSO search, CommonParam param)
        {
            List<HIS_DHST> list = new List<HIS_DHST>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_DHST.AsNoTracking().AsQueryable();
                        if (search.listHisDhstExpression != null && search.listHisDhstExpression.Count > 0)
                        {
                            foreach (var item in search.listHisDhstExpression)
                            {
        ...
        public HIS_DHST GetById(long id, HisDhstSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_DHST.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisDhstExpression != null && search.listHisDhstExpression.Count > 0)
                        {
                            foreach (var item in search.listHisDhstExpression)
                            {
