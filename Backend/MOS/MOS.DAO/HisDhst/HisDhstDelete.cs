...
{
    partial class HisDhstDelete : EntityBase
    {
        public HisDhstDelete()
            : base()
        {
        ...
        public bool Delete(HIS_DHST data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_DHST> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_DHST> listIdNotFound = new List<HIS_DHST>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_DHST> listRaw = ctx.HIS_DHST.Where(o => listId.Contains(o.ID)).ToList();
