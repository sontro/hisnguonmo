...
namespace MOS.DAO.HisPetroleum
{
    partial class HisPetroleumGet : EntityBase
    {
        ...
        {
            List<V_HIS_PETROLEUM> list = new List<V_HIS_PETROLEUM>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_PETROLEUM.AsNoTracking().AsQueryable();
                        if (search.listVHisPetroleumExpression != null && search.listVHisPetroleumExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisPetroleumExpression)
                            {
