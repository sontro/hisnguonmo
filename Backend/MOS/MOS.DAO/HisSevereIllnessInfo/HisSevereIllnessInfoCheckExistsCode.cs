    ...
    partial class HisSevereIllnessInfoCheck : EntityBase
    {
        public bool ExistsCode(string code, long? id)
        {
            ...
            try
            {
                ...
                {
                    count = ctx.HIS_SEVERE_ILLNESS_INFO.AsQueryable().Where(p => p.SEVERE_ILLNESS_INFO_CODE.Equals(code) && p.ID != id && (p.IS_DELETE.HasValue || p.IS_DELETE.Value != (short)1)).Count();
                }
                result = (count > 0);
            }
            catch (Exception)
