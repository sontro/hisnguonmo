...
namespace MOS.DAO.HisSevereIllnessInfo
{
    partial class HisSevereIllnessInfoGet : EntityBase
    {
        ...
        {
            List<HIS_SEVERE_ILLNESS_INFO> list = new List<HIS_SEVERE_ILLNESS_INFO>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SEVERE_ILLNESS_INFO.AsNoTracking().AsQueryable();
                        if (search.listHisSevereIllnessInfoExpression != null && search.listHisSevereIllnessInfoExpression.Count > 0)
                        {
                            foreach (var item in search.listHisSevereIllnessInfoExpression)
                            {
        ...
        public HIS_SEVERE_ILLNESS_INFO GetById(long id, HisSevereIllnessInfoSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_SEVERE_ILLNESS_INFO.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisSevereIllnessInfoExpression != null && search.listHisSevereIllnessInfoExpression.Count > 0)
                        {
                            foreach (var item in search.listHisSevereIllnessInfoExpression)
                            {
