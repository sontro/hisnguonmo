...
{
    partial class HisRoomGroupDelete : EntityBase
    {
        public HisRoomGroupDelete()
            : base()
        {
        ...
        public bool Delete(HIS_ROOM_GROUP data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_ROOM_GROUP> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_ROOM_GROUP> listIdNotFound = new List<HIS_ROOM_GROUP>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_ROOM_GROUP> listRaw = ctx.HIS_ROOM_GROUP.Where(o => listId.Contains(o.ID)).ToList();
