...
namespace MOS.DAO.HisExpMestTemplate
{
    partial class HisExpMestTemplateGet : EntityBase
    {
        ...
        {
            List<HIS_EXP_MEST_TEMPLATE> list = new List<HIS_EXP_MEST_TEMPLATE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_EXP_MEST_TEMPLATE.AsNoTracking().AsQueryable();
        ...
        public HIS_EXP_MEST_TEMPLATE GetById(long id, HisExpMestTemplateSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_EXP_MEST_TEMPLATE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisExpMestTemplateExpression != null && search.listHisExpMestTemplateExpression.Count > 0)
                        {
                            foreach (var item in search.listHisExpMestTemplateExpression)
                            {
