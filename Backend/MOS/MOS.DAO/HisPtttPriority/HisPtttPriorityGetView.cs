...
namespace MOS.DAO.HisPtttPriority
{
    partial class HisPtttPriorityGet : EntityBase
    {
        ...
        {
            List<V_HIS_PTTT_PRIORITY> list = new List<V_HIS_PTTT_PRIORITY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_PTTT_PRIORITY.AsNoTracking().AsQueryable();
                        if (search.listVHisPtttPriorityExpression != null && search.listVHisPtttPriorityExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisPtttPriorityExpression)
                            {
