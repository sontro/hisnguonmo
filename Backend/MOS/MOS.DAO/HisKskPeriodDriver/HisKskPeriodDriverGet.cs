...
{
    partial class HisKskPeriodDriverGet : EntityBase
    {
        public List<HIS_KSK_PERIOD_DRIVER> Get(HisKskPeriodDriverSO search, CommonParam param)
        {
            List<HIS_KSK_PERIOD_DRIVER> list = new List<HIS_KSK_PERIOD_DRIVER>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_KSK_PERIOD_DRIVER.AsNoTracking().AsQueryable();
                        if (search.listHisKskPeriodDriverExpression != null && search.listHisKskPeriodDriverExpression.Count > 0)
                        {
                            foreach (var item in search.listHisKskPeriodDriverExpression)
                            {
        ...
        public HIS_KSK_PERIOD_DRIVER GetById(long id, HisKskPeriodDriverSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_KSK_PERIOD_DRIVER.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisKskPeriodDriverExpression != null && search.listHisKskPeriodDriverExpression.Count > 0)
                        {
                            foreach (var item in search.listHisKskPeriodDriverExpression)
                            {
