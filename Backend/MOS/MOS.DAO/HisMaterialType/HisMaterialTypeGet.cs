...
namespace MOS.DAO.HisMaterialType
{
    partial class HisMaterialTypeGet : EntityBase
    {
        ...
        {
            List<HIS_MATERIAL_TYPE> list = new List<HIS_MATERIAL_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MATERIAL_TYPE.AsNoTracking().AsQueryable();
                        if (search.listHisMaterialTypeExpression != null && search.listHisMaterialTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMaterialTypeExpression)
                            {
        ...
        public HIS_MATERIAL_TYPE GetById(long id, HisMaterialTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_MATERIAL_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMaterialTypeExpression != null && search.listHisMaterialTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMaterialTypeExpression)
                            {
