...
{
    partial class HisMaterialTypeDelete : EntityBase
    {
        public HisMaterialTypeDelete()
            : base()
        {
        ...
        public bool Delete(HIS_MATERIAL_TYPE data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_MATERIAL_TYPE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_MATERIAL_TYPE> listIdNotFound = new List<HIS_MATERIAL_TYPE>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_MATERIAL_TYPE> listRaw = ctx.HIS_MATERIAL_TYPE.Where(o => listId.Contains(o.ID)).ToList();
