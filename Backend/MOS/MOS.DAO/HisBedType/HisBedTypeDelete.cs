...
{
    partial class HisBedTypeDelete : EntityBase
    {
        public HisBedTypeDelete()
            : base()
        {
        ...
        public bool Delete(HIS_BED_TYPE data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_BED_TYPE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_BED_TYPE> listIdNotFound = new List<HIS_BED_TYPE>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_BED_TYPE> listRaw = ctx.HIS_BED_TYPE.Where(o => listId.Contains(o.ID)).ToList();
