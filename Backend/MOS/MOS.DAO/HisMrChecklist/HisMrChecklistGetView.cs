...
namespace MOS.DAO.HisMrChecklist
{
    partial class HisMrChecklistGet : EntityBase
    {
        ...
        {
            List<V_HIS_MR_CHECKLIST> list = new List<V_HIS_MR_CHECKLIST>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_MR_CHECKLIST.AsNoTracking().AsQueryable();
                        if (search.listVHisMrChecklistExpression != null && search.listVHisMrChecklistExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisMrChecklistExpression)
                            {
