...
namespace MOS.DAO.HisMrChecklist
{
    partial class HisMrChecklistGet : EntityBase
    {
        ...
        {
            List<HIS_MR_CHECKLIST> list = new List<HIS_MR_CHECKLIST>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MR_CHECKLIST.AsNoTracking().AsQueryable();
        ...
        public HIS_MR_CHECKLIST GetById(long id, HisMrChecklistSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_MR_CHECKLIST.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMrChecklistExpression != null && search.listHisMrChecklistExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMrChecklistExpression)
                            {
