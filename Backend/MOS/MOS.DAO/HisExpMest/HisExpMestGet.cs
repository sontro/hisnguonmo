...
namespace MOS.DAO.HisExpMest
{
    partial class HisExpMestGet : EntityBase
    {
        ...
        {
            List<HIS_EXP_MEST> list = new List<HIS_EXP_MEST>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_EXP_MEST.AsNoTracking().AsQueryable();
                        if (search.listHisExpMestExpression != null && search.listHisExpMestExpression.Count > 0)
                        {
                            foreach (var item in search.listHisExpMestExpression)
                            {
        ...
        public HIS_EXP_MEST GetById(long id, HisExpMestSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_EXP_MEST.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisExpMestExpression != null && search.listHisExpMestExpression.Count > 0)
                        {
                            foreach (var item in search.listHisExpMestExpression)
                            {
