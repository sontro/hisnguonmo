...
namespace MOS.DAO.HisExpMest
{
    partial class HisExpMestGet : EntityBase
    {
        ...
        {
            List<V_HIS_EXP_MEST> list = new List<V_HIS_EXP_MEST>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_EXP_MEST.AsNoTracking().AsQueryable();
                        if (search.listVHisExpMestExpression != null && search.listVHisExpMestExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisExpMestExpression)
                            {
