...
namespace MOS.DAO.HisExpMest
{
    partial class HisExpMestGet : EntityBase
    {
        ...
        {
            List<V_HIS_EXP_MEST_4> list = new List<V_HIS_EXP_MEST_4>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_EXP_MEST_4.AsNoTracking().AsQueryable();
                        if (search.listVHisExpMest4Expression != null && search.listVHisExpMest4Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisExpMest4Expression)
                            {
