...
namespace MOS.DAO.HisExpMest
{
    ...
    {
        public List<V_HIS_EXP_MEST_1> GetView1(HisExpMestSO search, CommonParam param)
        {
            List<V_HIS_EXP_MEST_1> list = new List<V_HIS_EXP_MEST_1>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_EXP_MEST_1.AsNoTracking().AsQueryable();
                        if (search.listVHisExpMest1Expression != null && search.listVHisExpMest1Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisExpMest1Expression)
                            {
