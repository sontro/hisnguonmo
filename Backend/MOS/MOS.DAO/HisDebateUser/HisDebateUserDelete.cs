...
{
    partial class HisDebateUserDelete : EntityBase
    {
        public HisDebateUserDelete()
            : base()
        {
        ...
        public bool Delete(HIS_DEBATE_USER data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_DEBATE_USER> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_DEBATE_USER> listIdNotFound = new List<HIS_DEBATE_USER>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_DEBATE_USER> listRaw = ctx.HIS_DEBATE_USER.Where(o => listId.Contains(o.ID)).ToList();
