        ...
        public List<HIS_DEBATE_USER> Get(HisDebateUserSO search, CommonParam param)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_DEBATE_USER.AsNoTracking().AsQueryable();
                        if (search.listHisDebateUserExpression != null && search.listHisDebateUserExpression.Count > 0)
                        {
                            foreach (var item in search.listHisDebateUserExpression)
                            {
        ...
        public HIS_DEBATE_USER GetById(long id, HisDebateUserSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_DEBATE_USER.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisDebateUserExpression != null && search.listHisDebateUserExpression.Count > 0)
                        {
                            foreach (var item in search.listHisDebateUserExpression)
                            {
