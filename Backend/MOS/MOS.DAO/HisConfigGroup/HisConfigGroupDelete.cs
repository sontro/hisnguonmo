...
{
    partial class HisConfigGroupDelete : EntityBase
    {
        public HisConfigGroupDelete()
            : base()
        {
        ...
        public bool Delete(HIS_CONFIG_GROUP data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_CONFIG_GROUP> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_CONFIG_GROUP> listIdNotFound = new List<HIS_CONFIG_GROUP>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_CONFIG_GROUP> listRaw = ctx.HIS_CONFIG_GROUP.Where(o => listId.Contains(o.ID)).ToList();
