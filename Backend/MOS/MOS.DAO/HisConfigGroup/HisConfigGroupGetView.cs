...
namespace MOS.DAO.HisConfigGroup
{
    partial class HisConfigGroupGet : EntityBase
    {
        ...
        {
            List<V_HIS_CONFIG_GROUP> list = new List<V_HIS_CONFIG_GROUP>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_CONFIG_GROUP.AsNoTracking().AsQueryable();
                        if (search.listVHisConfigGroupExpression != null && search.listVHisConfigGroupExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisConfigGroupExpression)
                            {
