...
namespace MOS.DAO.HisBloodGiver
{
    partial class HisBloodGiverGet : EntityBase
    {
        ...
        {
            List<HIS_BLOOD_GIVER> list = new List<HIS_BLOOD_GIVER>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_BLOOD_GIVER.AsNoTracking().AsQueryable();
        ...
        public HIS_BLOOD_GIVER GetById(long id, HisBloodGiverSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_BLOOD_GIVER.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisBloodGiverExpression != null && search.listHisBloodGiverExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBloodGiverExpression)
                            {
