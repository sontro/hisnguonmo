...
namespace MOS.DAO.HisMaterialMaterial
{
    partial class HisMaterialMaterialGet : EntityBase
    {
        ...
        {
            List<HIS_MATERIAL_MATERIAL> list = new List<HIS_MATERIAL_MATERIAL>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MATERIAL_MATERIAL.AsNoTracking().AsQueryable();
        ...
        public HIS_MATERIAL_MATERIAL GetById(long id, HisMaterialMaterialSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_MATERIAL_MATERIAL.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMaterialMaterialExpression != null && search.listHisMaterialMaterialExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMaterialMaterialExpression)
                            {
