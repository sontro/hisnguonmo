...
namespace MOS.DAO.HisExmeReasonCfg
{
    partial class HisExmeReasonCfgGet : EntityBase
    {
        ...
        {
            List<HIS_EXME_REASON_CFG> list = new List<HIS_EXME_REASON_CFG>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_EXME_REASON_CFG.AsNoTracking().AsQueryable();
        ...
        public HIS_EXME_REASON_CFG GetById(long id, HisExmeReasonCfgSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_EXME_REASON_CFG.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisExmeReasonCfgExpression != null && search.listHisExmeReasonCfgExpression.Count > 0)
                        {
                            foreach (var item in search.listHisExmeReasonCfgExpression)
                            {
