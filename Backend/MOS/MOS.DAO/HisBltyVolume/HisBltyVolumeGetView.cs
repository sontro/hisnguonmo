...
namespace MOS.DAO.HisBltyVolume
{
    partial class HisBltyVolumeGet : EntityBase
    {
        ...
        {
            List<V_HIS_BLTY_VOLUME> list = new List<V_HIS_BLTY_VOLUME>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_BLTY_VOLUME.AsNoTracking().AsQueryable();
                        if (search.listVHisBltyVolumeExpression != null && search.listVHisBltyVolumeExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisBltyVolumeExpression)
                            {
