...
{
    partial class HisBltyVolumeGet : EntityBase
    {
        public List<HIS_BLTY_VOLUME> Get(HisBltyVolumeSO search, CommonParam param)
        {
            List<HIS_BLTY_VOLUME> list = new List<HIS_BLTY_VOLUME>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_BLTY_VOLUME.AsNoTracking().AsQueryable();
                        if (search.listHisBltyVolumeExpression != null && search.listHisBltyVolumeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBltyVolumeExpression)
                            {
        ...
        public HIS_BLTY_VOLUME GetById(long id, HisBltyVolumeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_BLTY_VOLUME.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisBltyVolumeExpression != null && search.listHisBltyVolumeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBltyVolumeExpression)
                            {
