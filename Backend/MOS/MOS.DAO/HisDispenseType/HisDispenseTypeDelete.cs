...
{
    partial class HisDispenseTypeDelete : EntityBase
    {
        public HisDispenseTypeDelete()
            : base()
        {
        ...
        public bool Delete(HIS_DISPENSE_TYPE data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_DISPENSE_TYPE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_DISPENSE_TYPE> listIdNotFound = new List<HIS_DISPENSE_TYPE>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_DISPENSE_TYPE> listRaw = ctx.HIS_DISPENSE_TYPE.Where(o => listId.Contains(o.ID)).ToList();
