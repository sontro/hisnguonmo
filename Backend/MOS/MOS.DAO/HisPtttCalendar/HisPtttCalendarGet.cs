...
namespace MOS.DAO.HisPtttCalendar
{
    partial class HisPtttCalendarGet : EntityBase
    {
        ...
        {
            List<HIS_PTTT_CALENDAR> list = new List<HIS_PTTT_CALENDAR>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_PTTT_CALENDAR.AsNoTracking().AsQueryable();
        ...
        public HIS_PTTT_CALENDAR GetById(long id, HisPtttCalendarSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_PTTT_CALENDAR.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisPtttCalendarExpression != null && search.listHisPtttCalendarExpression.Count > 0)
                        {
                            foreach (var item in search.listHisPtttCalendarExpression)
                            {
