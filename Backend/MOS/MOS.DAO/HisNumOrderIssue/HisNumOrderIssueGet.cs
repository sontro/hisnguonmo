...
namespace MOS.DAO.HisNumOrderIssue
{
    partial class HisNumOrderIssueGet : EntityBase
    {
        ...
        {
            List<HIS_NUM_ORDER_ISSUE> list = new List<HIS_NUM_ORDER_ISSUE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_NUM_ORDER_ISSUE.AsNoTracking().AsQueryable();
        ...
        public HIS_NUM_ORDER_ISSUE GetById(long id, HisNumOrderIssueSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_NUM_ORDER_ISSUE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisNumOrderIssueExpression != null && search.listHisNumOrderIssueExpression.Count > 0)
                        {
                            foreach (var item in search.listHisNumOrderIssueExpression)
                            {
