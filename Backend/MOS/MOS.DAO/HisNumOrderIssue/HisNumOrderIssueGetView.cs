...
namespace MOS.DAO.HisNumOrderIssue
{
    partial class HisNumOrderIssueGet : EntityBase
    {
        ...
        {
            List<V_HIS_NUM_ORDER_ISSUE> list = new List<V_HIS_NUM_ORDER_ISSUE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_NUM_ORDER_ISSUE.AsNoTracking().AsQueryable();
                        if (search.listVHisNumOrderIssueExpression != null && search.listVHisNumOrderIssueExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisNumOrderIssueExpression)
                            {
