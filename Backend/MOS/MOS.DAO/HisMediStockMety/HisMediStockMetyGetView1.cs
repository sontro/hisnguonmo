...
namespace MOS.DAO.HisMediStockMety
{
    ...
    {
        public List<V_HIS_MEDI_STOCK_METY_1> GetView1(HisMediStockMetySO search, CommonParam param)
        {
            List<V_HIS_MEDI_STOCK_METY_1> list = new List<V_HIS_MEDI_STOCK_METY_1>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_MEDI_STOCK_METY_1.AsNoTracking().AsQueryable();
