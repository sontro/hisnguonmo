...
{
    partial class HisDebateDelete : EntityBase
    {
        public HisDebateDelete()
            : base()
        {
        ...
        public bool Delete(HIS_DEBATE data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_DEBATE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_DEBATE> listIdNotFound = new List<HIS_DEBATE>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_DEBATE> listRaw = ctx.HIS_DEBATE.Where(o => listId.Contains(o.ID)).ToList();
