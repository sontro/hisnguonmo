...
namespace MOS.DAO.HisDebate
{
    partial class HisDebateGet : EntityBase
    {
        ...
        {
            List<HIS_DEBATE> list = new List<HIS_DEBATE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_DEBATE.AsNoTracking().AsQueryable();
                        if (search.listHisDebateExpression != null && search.listHisDebateExpression.Count > 0)
                        {
                            foreach (var item in search.listHisDebateExpression)
                            {
        ...
        public HIS_DEBATE GetById(long id, HisDebateSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_DEBATE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisDebateExpression != null && search.listHisDebateExpression.Count > 0)
                        {
                            foreach (var item in search.listHisDebateExpression)
                            {
