...
{
    partial class HisDebateGet : EntityBase
    {
        public List<V_HIS_DEBATE> GetView(HisDebateSO search, CommonParam param)
        {
            List<V_HIS_DEBATE> list = new List<V_HIS_DEBATE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_DEBATE.AsQueryable();
                        if (search.listVHisDebateExpression != null && search.listVHisDebateExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisDebateExpression)
                            {
