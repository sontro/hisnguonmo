...
namespace MOS.DAO.HisVaccHealthStt
{
    partial class HisVaccHealthSttGet : EntityBase
    {
        ...
        {
            List<HIS_VACC_HEALTH_STT> list = new List<HIS_VACC_HEALTH_STT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_VACC_HEALTH_STT.AsNoTracking().AsQueryable();
        ...
        public HIS_VACC_HEALTH_STT GetById(long id, HisVaccHealthSttSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_VACC_HEALTH_STT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisVaccHealthSttExpression != null && search.listHisVaccHealthSttExpression.Count > 0)
                        {
                            foreach (var item in search.listHisVaccHealthSttExpression)
                            {
