...
namespace MOS.DAO.HisServiceReqStt
{
    partial class HisServiceReqSttGet : EntityBase
    {
        ...
        {
            List<HIS_SERVICE_REQ_STT> list = new List<HIS_SERVICE_REQ_STT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SERVICE_REQ_STT.AsNoTracking().AsQueryable();
                        if (search.listHisServiceReqSttExpression != null && search.listHisServiceReqSttExpression.Count > 0)
                        {
                            foreach (var item in search.listHisServiceReqSttExpression)
                            {
        ...
        public HIS_SERVICE_REQ_STT GetById(long id, HisServiceReqSttSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_SERVICE_REQ_STT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisServiceReqSttExpression != null && search.listHisServiceReqSttExpression.Count > 0)
                        {
                            foreach (var item in search.listHisServiceReqSttExpression)
                            {
