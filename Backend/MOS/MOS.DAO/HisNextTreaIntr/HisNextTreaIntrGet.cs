...
namespace MOS.DAO.HisNextTreaIntr
{
    partial class HisNextTreaIntrGet : EntityBase
    {
        ...
        {
            List<HIS_NEXT_TREA_INTR> list = new List<HIS_NEXT_TREA_INTR>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_NEXT_TREA_INTR.AsNoTracking().AsQueryable();
                        if (search.listHisNextTreaIntrExpression != null && search.listHisNextTreaIntrExpression.Count > 0)
                        {
                            foreach (var item in search.listHisNextTreaIntrExpression)
                            {
        ...
        public HIS_NEXT_TREA_INTR GetById(long id, HisNextTreaIntrSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_NEXT_TREA_INTR.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisNextTreaIntrExpression != null && search.listHisNextTreaIntrExpression.Count > 0)
                        {
                            foreach (var item in search.listHisNextTreaIntrExpression)
                            {
