...
{
    partial class HisServiceDelete : EntityBase
    {
        public HisServiceDelete()
            : base()
        {
        ...
        public bool Delete(HIS_SERVICE data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_SERVICE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_SERVICE> listIdNotFound = new List<HIS_SERVICE>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_SERVICE> listRaw = ctx.HIS_SERVICE.Where(o => listId.Contains(o.ID)).ToList();
