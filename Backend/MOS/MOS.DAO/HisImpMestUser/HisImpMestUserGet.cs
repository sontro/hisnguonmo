...
namespace MOS.DAO.HisImpMestUser
{
    partial class HisImpMestUserGet : EntityBase
    {
        ...
        {
            List<HIS_IMP_MEST_USER> list = new List<HIS_IMP_MEST_USER>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_IMP_MEST_USER.AsNoTracking().AsQueryable();
                        if (search.listHisImpMestUserExpression != null && search.listHisImpMestUserExpression.Count > 0)
                        {
                            foreach (var item in search.listHisImpMestUserExpression)
                            {
        ...
        public HIS_IMP_MEST_USER GetById(long id, HisImpMestUserSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_IMP_MEST_USER.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisImpMestUserExpression != null && search.listHisImpMestUserExpression.Count > 0)
                        {
                            foreach (var item in search.listHisImpMestUserExpression)
                            {
