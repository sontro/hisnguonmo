...
namespace MOS.DAO.HisTranPatiTech
{
    partial class HisTranPatiTechGet : EntityBase
    {
        ...
        {
            List<HIS_TRAN_PATI_TECH> list = new List<HIS_TRAN_PATI_TECH>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_TRAN_PATI_TECH.AsNoTracking().AsQueryable();
                        if (search.listHisTranPatiTechExpression != null && search.listHisTranPatiTechExpression.Count > 0)
                        {
                            foreach (var item in search.listHisTranPatiTechExpression)
                            {
        ...
        public HIS_TRAN_PATI_TECH GetById(long id, HisTranPatiTechSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_TRAN_PATI_TECH.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisTranPatiTechExpression != null && search.listHisTranPatiTechExpression.Count > 0)
                        {
                            foreach (var item in search.listHisTranPatiTechExpression)
                            {
