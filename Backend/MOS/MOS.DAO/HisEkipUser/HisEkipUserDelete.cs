...
{
    partial class HisEkipUserDelete : EntityBase
    {
        public HisEkipUserDelete()
            : base()
        {
        ...
        public bool Delete(HIS_EKIP_USER data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_EKIP_USER> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_EKIP_USER> listIdNotFound = new List<HIS_EKIP_USER>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_EKIP_USER> listRaw = ctx.HIS_EKIP_USER.Where(o => listId.Contains(o.ID)).ToList();
