...
{
    partial class HisBabyGet : EntityBase
    {
        public List<HIS_BABY> Get(HisBabySO search, CommonParam param)
        {
            List<HIS_BABY> list = new List<HIS_BABY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_BABY.AsNoTracking().AsQueryable();
                        if (search.listHisBabyExpression != null && search.listHisBabyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBabyExpression)
                            {
        ...
        public HIS_BABY GetById(long id, HisBabySO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_BABY.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisBabyExpression != null && search.listHisBabyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBabyExpression)
                            {
