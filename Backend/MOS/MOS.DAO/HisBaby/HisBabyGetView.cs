...
namespace MOS.DAO.HisBaby
{
    partial class HisBabyGet : EntityBase
    {
        ...
        {
            List<V_HIS_BABY> list = new List<V_HIS_BABY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_BABY.AsNoTracking().AsQueryable();
                        if (search.listVHisBabyExpression != null && search.listVHisBabyExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisBabyExpression)
                            {
