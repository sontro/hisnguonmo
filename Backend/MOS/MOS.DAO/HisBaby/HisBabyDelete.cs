...
{
    partial class HisBabyDelete : EntityBase
    {
        public HisBabyDelete()
            : base()
        {
        ...
        public bool Delete(HIS_BABY data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_BABY> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_BABY> listIdNotFound = new List<HIS_BABY>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_BABY> listRaw = ctx.HIS_BABY.Where(o => listId.Contains(o.ID)).ToList();
