...
{
    partial class HisConfigGet : EntityBase
    {
        public List<V_HIS_CONFIG> GetView(HisConfigSO search, CommonParam param)
        {
            List<V_HIS_CONFIG> list = new List<V_HIS_CONFIG>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_CONFIG.AsQueryable();
                        if (search.listVHisConfigExpression != null && search.listVHisConfigExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisConfigExpression)
                            {
