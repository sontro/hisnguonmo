...
namespace MOS.DAO.HisConfig
{
    partial class HisConfigGet : EntityBase
    {
        ...
        {
            List<HIS_CONFIG> list = new List<HIS_CONFIG>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_CONFIG.AsNoTracking().AsQueryable();
                        if (search.listHisConfigExpression != null && search.listHisConfigExpression.Count > 0)
                        {
                            foreach (var item in search.listHisConfigExpression)
                            {
        ...
        public HIS_CONFIG GetById(long id, HisConfigSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_CONFIG.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisConfigExpression != null && search.listHisConfigExpression.Count > 0)
                        {
                            foreach (var item in search.listHisConfigExpression)
                            {
