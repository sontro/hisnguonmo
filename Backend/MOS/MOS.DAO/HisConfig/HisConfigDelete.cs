...
{
    partial class HisConfigDelete : EntityBase
    {
        public HisConfigDelete()
            : base()
        {
        ...
        public bool Delete(HIS_CONFIG data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_CONFIG> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_CONFIG> listIdNotFound = new List<HIS_CONFIG>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_CONFIG> listRaw = ctx.HIS_CONFIG.Where(o => listId.Contains(o.ID)).ToList();
