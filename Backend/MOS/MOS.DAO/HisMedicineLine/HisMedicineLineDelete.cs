...
{
    partial class HisMedicineLineDelete : EntityBase
    {
        public HisMedicineLineDelete()
            : base()
        {
        ...
        public bool Delete(HIS_MEDICINE_LINE data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_MEDICINE_LINE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_MEDICINE_LINE> listIdNotFound = new List<HIS_MEDICINE_LINE>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_MEDICINE_LINE> listRaw = ctx.HIS_MEDICINE_LINE.Where(o => listId.Contains(o.ID)).ToList();
