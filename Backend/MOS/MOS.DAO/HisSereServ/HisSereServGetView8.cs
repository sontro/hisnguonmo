...
namespace MOS.DAO.HisSereServ
{
    ...
    {
        public List<V_HIS_SERE_SERV_8> GetView8(HisSereServSO search, CommonParam param)
        {
            List<V_HIS_SERE_SERV_8> list = new List<V_HIS_SERE_SERV_8>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_SERE_SERV_8.AsNoTracking().AsQueryable();
                        if (search.listVHisSereServ8Expression != null && search.listVHisSereServ8Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisSereServ8Expression)
                            {
