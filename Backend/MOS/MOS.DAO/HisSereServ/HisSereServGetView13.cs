...
namespace MOS.DAO.HisSereServ
{
    ...
    {
        public List<V_HIS_SERE_SERV_13> GetView13(HisSereServSO search, CommonParam param)
        {
            List<V_HIS_SERE_SERV_13> list = new List<V_HIS_SERE_SERV_13>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_SERE_SERV_13.AsNoTracking().AsQueryable();
                        if (search.listVHisSereServ13Expression != null && search.listVHisSereServ13Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisSereServ13Expression)
                            {
