...
namespace MOS.DAO.HisSereServ
{
    ...
    {
        public List<V_HIS_SERE_SERV_14> GetView14(HisSereServSO search, CommonParam param)
        {
            List<V_HIS_SERE_SERV_14> list = new List<V_HIS_SERE_SERV_14>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_SERE_SERV_14.AsNoTracking().AsQueryable();
                        if (search.listVHisSereServ14Expression != null && search.listVHisSereServ14Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisSereServ14Expression)
                            {
