...
namespace MOS.DAO.HisSereServ
{
    ...
    {
        public List<V_HIS_SERE_SERV_15> GetView15(HisSereServSO search, CommonParam param)
        {
            List<V_HIS_SERE_SERV_15> list = new List<V_HIS_SERE_SERV_15>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_SERE_SERV_15.AsNoTracking().AsQueryable();
                        if (search.listVHisSereServ15Expression != null && search.listVHisSereServ15Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisSereServ15Expression)
                            {
