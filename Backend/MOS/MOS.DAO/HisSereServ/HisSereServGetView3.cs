...
namespace MOS.DAO.HisSereServ
{
    ...
    {
        public List<V_HIS_SERE_SERV_3> GetView3(HisSereServSO search, CommonParam param)
        {
            List<V_HIS_SERE_SERV_3> list = new List<V_HIS_SERE_SERV_3>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_SERE_SERV_3.AsNoTracking().AsQueryable();
                        if (search.listVHisSereServ3Expression != null && search.listVHisSereServ3Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisSereServ3Expression)
                            {
