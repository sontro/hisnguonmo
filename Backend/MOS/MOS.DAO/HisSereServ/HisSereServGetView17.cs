...
namespace MOS.DAO.HisSereServ
{
    ...
    {
        public List<V_HIS_SERE_SERV_17> GetView17(HisSereServSO search, CommonParam param)
        {
            List<V_HIS_SERE_SERV_17> list = new List<V_HIS_SERE_SERV_17>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_SERE_SERV_17.AsNoTracking().AsQueryable();
                        if (search.listVHisSereServ17Expression != null && search.listVHisSereServ17Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisSereServ17Expression)
                            {
