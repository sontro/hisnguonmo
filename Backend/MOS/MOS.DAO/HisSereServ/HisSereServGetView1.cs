...
namespace MOS.DAO.HisSereServ
{
    ...
    {
        public List<V_HIS_SERE_SERV_1> GetView1(HisSereServSO search, CommonParam param)
        {
            List<V_HIS_SERE_SERV_1> list = new List<V_HIS_SERE_SERV_1>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_SERE_SERV_1.AsNoTracking().AsQueryable();
                        if (search.listVHisSereServ1Expression != null && search.listVHisSereServ1Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisSereServ1Expression)
                            {
