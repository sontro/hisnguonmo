...
namespace MOS.DAO.HisSereServ
{
    ...
    {
        public List<V_HIS_SERE_SERV_16> GetView16(HisSereServSO search, CommonParam param)
        {
            List<V_HIS_SERE_SERV_16> list = new List<V_HIS_SERE_SERV_16>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_SERE_SERV_16.AsNoTracking().AsQueryable();
                        if (search.listVHisSereServ16Expression != null && search.listVHisSereServ16Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisSereServ16Expression)
                            {
