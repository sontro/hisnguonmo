        ...
        public List<HIS_SERE_SERV> Get(HisSereServSO search, CommonParam param)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SERE_SERV.AsNoTracking().AsQueryable();
                        if (search.listHisSereServExpression != null && search.listHisSereServExpression.Count > 0)
                        {
                            foreach (var item in search.listHisSereServExpression)
                            {
        ...
        public HIS_SERE_SERV GetById(long id, HisSereServSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_SERE_SERV.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisSereServExpression != null && search.listHisSereServExpression.Count > 0)
                        {
                            foreach (var item in search.listHisSereServExpression)
                            {
