...
namespace MOS.DAO.HisHoreHandover
{
    partial class HisHoreHandoverGet : EntityBase
    {
        ...
        {
            List<HIS_HORE_HANDOVER> list = new List<HIS_HORE_HANDOVER>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_HORE_HANDOVER.AsNoTracking().AsQueryable();
                        if (search.listHisHoreHandoverExpression != null && search.listHisHoreHandoverExpression.Count > 0)
                        {
                            foreach (var item in search.listHisHoreHandoverExpression)
                            {
        ...
        public HIS_HORE_HANDOVER GetById(long id, HisHoreHandoverSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_HORE_HANDOVER.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisHoreHandoverExpression != null && search.listHisHoreHandoverExpression.Count > 0)
                        {
                            foreach (var item in search.listHisHoreHandoverExpression)
                            {
