...
namespace MOS.DAO.HisKskDriver
{
    partial class HisKskDriverGet : EntityBase
    {
        ...
        {
            List<HIS_KSK_DRIVER> list = new List<HIS_KSK_DRIVER>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_KSK_DRIVER.AsNoTracking().AsQueryable();
                        if (search.listHisKskDriverExpression != null && search.listHisKskDriverExpression.Count > 0)
                        {
                            foreach (var item in search.listHisKskDriverExpression)
                            {
        ...
        public HIS_KSK_DRIVER GetById(long id, HisKskDriverSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_KSK_DRIVER.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisKskDriverExpression != null && search.listHisKskDriverExpression.Count > 0)
                        {
                            foreach (var item in search.listHisKskDriverExpression)
                            {
