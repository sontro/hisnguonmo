...
namespace MOS.DAO.HisEkipTemp
{
    partial class HisEkipTempGet : EntityBase
    {
        ...
        {
            List<HIS_EKIP_TEMP> list = new List<HIS_EKIP_TEMP>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_EKIP_TEMP.AsNoTracking().AsQueryable();
        ...
        public HIS_EKIP_TEMP GetById(long id, HisEkipTempSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_EKIP_TEMP.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisEkipTempExpression != null && search.listHisEkipTempExpression.Count > 0)
                        {
                            foreach (var item in search.listHisEkipTempExpression)
                            {
