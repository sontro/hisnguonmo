...
namespace MOS.DAO.HisUserGroupTemp
{
    partial class HisUserGroupTempGet : EntityBase
    {
        ...
        {
            List<HIS_USER_GROUP_TEMP> list = new List<HIS_USER_GROUP_TEMP>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_USER_GROUP_TEMP.AsNoTracking().AsQueryable();
        ...
        public HIS_USER_GROUP_TEMP GetById(long id, HisUserGroupTempSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_USER_GROUP_TEMP.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisUserGroupTempExpression != null && search.listHisUserGroupTempExpression.Count > 0)
                        {
                            foreach (var item in search.listHisUserGroupTempExpression)
                            {
