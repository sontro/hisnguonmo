...
namespace MOS.DAO.HisVaccination
{
    ...
    {
        public List<V_HIS_VACCINATION> GetView(HisVaccinationSO search, CommonParam param)
        {
            List<V_HIS_VACCINATION> list = new List<V_HIS_VACCINATION>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_VACCINATION.AsNoTracking().AsQueryable();
                        if (search.listVHisVaccinationExpression != null && search.listVHisVaccinationExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisVaccinationExpression)
                            {
