using MOS.DAO.StagingObject;
using MOS.EFMODEL.DataModels;
using Inventec.Common.Repository;
using Inventec.Core;
using System;
using System.Collections.Generic;

namespace MOS.DAO.HisMestMetyDepa
{
    public partial class HisMestMetyDepaDAO : EntityBase
    {
        private HisMestMetyDepaCreate CreateWorker
        {
            get
            {
                return (HisMestMetyDepaCreate)Worker.Get<HisMestMetyDepaCreate>();
            }
        }
        private HisMestMetyDepaUpdate UpdateWorker
        {
            get
            {
                return (HisMestMetyDepaUpdate)Worker.Get<HisMestMetyDepaUpdate>();
            }
        }
        private HisMestMetyDepaDelete DeleteWorker
        {
            get
            {
                return (HisMestMetyDepaDelete)Worker.Get<HisMestMetyDepaDelete>();
            }
        }
        private HisMestMetyDepaTruncate TruncateWorker
        {
            get
            {
                return (HisMestMetyDepaTruncate)Worker.Get<HisMestMetyDepaTruncate>();
            }
        }
        private HisMestMetyDepaGet GetWorker
        {
            get
            {
                return (HisMestMetyDepaGet)Worker.Get<HisMestMetyDepaGet>();
            }
        }
        private HisMestMetyDepaCheck CheckWorker
        {
            get
            {
                return (HisMestMetyDepaCheck)Worker.Get<HisMestMetyDepaCheck>();
            }
        }

        public bool Create(HIS_MEST_METY_DEPA data)
        {
            bool result = false;
            try
            {
                result = CreateWorker.Create(data);
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
                result = false;
            }
            return result;
        }

        public bool CreateList(List<HIS_MEST_METY_DEPA> listData)
        {
            bool result = false;
            try
            {
                result = CreateWorker.CreateList(listData);
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
                result = false;
            }
            return result;
        }

        public bool Update(HIS_MEST_METY_DEPA data)
        {
            bool result = false;
            try
            {
                result = UpdateWorker.Update(data);
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
                result = false;
            }
            return result;
        }

        public bool UpdateList(List<HIS_MEST_METY_DEPA> listData)
        {
            bool result = false;
            try
            {
                result = UpdateWorker.UpdateList(listData);
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
                result = false;
            }
            return result;
        }

        public bool Delete(HIS_MEST_METY_DEPA data)
        {
            bool result = false;
            try
            {
                result = DeleteWorker.Delete(data);
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
                result = false;
            }
            return result;
        }

        public bool DeleteList(List<HIS_MEST_METY_DEPA> listData)
        {
            bool result = false;

            try
            {
                result = DeleteWorker.DeleteList(listData);
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
                result = false;
            }
            return result;
        }

        public bool Truncate(HIS_MEST_METY_DEPA data)
        {
            bool result = false;
            try
            {
                result = TruncateWorker.Truncate(data);
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
                result = false;
            }
            return result;
        }

        public bool TruncateList(List<HIS_MEST_METY_DEPA> listData)
        {
            bool result = false;
            try
            {
                result = TruncateWorker.TruncateList(listData);
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
                result = false;
            }
            return result;
        }

        public List<HIS_MEST_METY_DEPA> Get(HisMestMetyDepaSO search, CommonParam param)
        {
            List<HIS_MEST_METY_DEPA> result = new List<HIS_MEST_METY_DEPA>();
            try
            {
                result = GetWorker.Get(search, param);
            }
            catch (Exception ex)
            {
                param.HasException = true;
                Inventec.Common.Logging.LogSystem.Error(ex);
                result.Clear();
            }
            return result;
        }

        public HIS_MEST_METY_DEPA GetById(long id, HisMestMetyDepaSO search)
        {
            HIS_MEST_METY_DEPA result = null;
            try
            {
                result = GetWorker.GetById(id, search);
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
                result = null;
            }

            return result;
        }

        public bool IsUnLock(long id)
        {
            try
            {
                return CheckWorker.IsUnLock(id);
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
                throw;
            }
        }
    }
}
