...
namespace MOS.DAO.HisMedicineInteractive
{
    partial class HisMedicineInteractiveGet : EntityBase
    {
        ...
        {
            List<HIS_MEDICINE_INTERACTIVE> list = new List<HIS_MEDICINE_INTERACTIVE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MEDICINE_INTERACTIVE.AsNoTracking().AsQueryable();
        ...
        public HIS_MEDICINE_INTERACTIVE GetById(long id, HisMedicineInteractiveSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_MEDICINE_INTERACTIVE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMedicineInteractiveExpression != null && search.listHisMedicineInteractiveExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMedicineInteractiveExpression)
                            {
