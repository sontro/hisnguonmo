    ...
    partial class HisMedicineInteractiveCheck : EntityBase
    {
        public bool ExistsCode(string code, long? id)
        {
            ...
            try
            {
                ...
                {
                    count = ctx.HIS_MEDICINE_INTERACTIVE.AsQueryable().Where(p => p.MEDICINE_INTERACTIVE_CODE.Equals(code) && p.ID != id && (p.IS_DELETE.HasValue || p.IS_DELETE.Value != (short)1)).Count();
                }
                result = (count > 0);
            }
            catch (Exception)
