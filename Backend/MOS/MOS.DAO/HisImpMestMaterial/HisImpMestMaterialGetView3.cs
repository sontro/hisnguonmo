...
namespace MOS.DAO.HisImpMestMaterial
{
    partial class HisImpMestMaterialGet : EntityBase
    {
        ...
        {
            List<V_HIS_IMP_MEST_MATERIAL_3> list = new List<V_HIS_IMP_MEST_MATERIAL_3>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_IMP_MEST_MATERIAL_3.AsNoTracking().AsQueryable();
                        if (search.listVHisImpMestMaterial3Expression != null && search.listVHisImpMestMaterial3Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisImpMestMaterial3Expression)
                            {
