...
namespace MOS.DAO.HisImpMestMaterial
{
    partial class HisImpMestMaterialGet : EntityBase
    {
        ...
        {
            List<HIS_IMP_MEST_MATERIAL> list = new List<HIS_IMP_MEST_MATERIAL>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_IMP_MEST_MATERIAL.AsNoTracking().AsQueryable();
                        if (search.listHisImpMestMaterialExpression != null && search.listHisImpMestMaterialExpression.Count > 0)
                        {
                            foreach (var item in search.listHisImpMestMaterialExpression)
                            {
        ...
        public HIS_IMP_MEST_MATERIAL GetById(long id, HisImpMestMaterialSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_IMP_MEST_MATERIAL.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisImpMestMaterialExpression != null && search.listHisImpMestMaterialExpression.Count > 0)
                        {
                            foreach (var item in search.listHisImpMestMaterialExpression)
                            {
