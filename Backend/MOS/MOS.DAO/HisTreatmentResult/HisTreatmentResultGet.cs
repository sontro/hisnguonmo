...
namespace MOS.DAO.HisTreatmentResult
{
    partial class HisTreatmentResultGet : EntityBase
    {
        ...
        {
            List<HIS_TREATMENT_RESULT> list = new List<HIS_TREATMENT_RESULT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_TREATMENT_RESULT.AsNoTracking().AsQueryable();
        ...
        public HIS_TREATMENT_RESULT GetById(long id, HisTreatmentResultSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_TREATMENT_RESULT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisTreatmentResultExpression != null && search.listHisTreatmentResultExpression.Count > 0)
                        {
                            foreach (var item in search.listHisTreatmentResultExpression)
                            {
