...
namespace MOS.DAO.HisMaterial
{
    partial class HisMaterialGet : EntityBase
    {
        ...
        {
            List<V_HIS_MATERIAL_2> list = new List<V_HIS_MATERIAL_2>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_MATERIAL_2.AsNoTracking().AsQueryable();
                        if (search.listVHisMaterial2Expression != null && search.listVHisMaterial2Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisMaterial2Expression)
                            {
