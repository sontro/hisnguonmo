...
{
    partial class HisMaterialDelete : EntityBase
    {
        public HisMaterialDelete()
            : base()
        {
        ...
        public bool Delete(HIS_MATERIAL data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_MATERIAL> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_MATERIAL> listIdNotFound = new List<HIS_MATERIAL>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_MATERIAL> listRaw = ctx.HIS_MATERIAL.Where(o => listId.Contains(o.ID)).ToList();
