...
namespace MOS.DAO.HisMaterial
{
    ...
    {
        public List<V_HIS_MATERIAL> GetView(HisMaterialSO search, CommonParam param)
        {
            List<V_HIS_MATERIAL> list = new List<V_HIS_MATERIAL>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_MATERIAL.AsNoTracking().AsQueryable();
                        if (search.listVHisMaterialExpression != null && search.listVHisMaterialExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisMaterialExpression)
                            {
