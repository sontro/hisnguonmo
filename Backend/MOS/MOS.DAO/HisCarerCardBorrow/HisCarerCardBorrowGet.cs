...
namespace MOS.DAO.HisCarerCardBorrow
{
    partial class HisCarerCardBorrowGet : EntityBase
    {
        ...
        {
            List<HIS_CARER_CARD_BORROW> list = new List<HIS_CARER_CARD_BORROW>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_CARER_CARD_BORROW.AsNoTracking().AsQueryable();
                        if (search.listHisCarerCardBorrowExpression != null && search.listHisCarerCardBorrowExpression.Count > 0)
                        {
                            foreach (var item in search.listHisCarerCardBorrowExpression)
                            {
        ...
        public HIS_CARER_CARD_BORROW GetById(long id, HisCarerCardBorrowSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_CARER_CARD_BORROW.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisCarerCardBorrowExpression != null && search.listHisCarerCardBorrowExpression.Count > 0)
                        {
                            foreach (var item in search.listHisCarerCardBorrowExpression)
                            {
