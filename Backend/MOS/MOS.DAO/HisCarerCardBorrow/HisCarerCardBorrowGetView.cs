...
{
    partial class HisCarerCardBorrowGet : EntityBase
    {
        public List<V_HIS_CARER_CARD_BORROW> GetView(HisCarerCardBorrowSO search, CommonParam param)
        {
            List<V_HIS_CARER_CARD_BORROW> list = new List<V_HIS_CARER_CARD_BORROW>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_CARER_CARD_BORROW.AsNoTracking().AsQueryable();
                        if (search.listVHisCarerCardBorrowExpression != null && search.listVHisCarerCardBorrowExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisCarerCardBorrowExpression)
                            {
