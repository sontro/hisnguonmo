...
{
    partial class HisTestIndexDelete : EntityBase
    {
        public HisTestIndexDelete()
            : base()
        {
        ...
        public bool Delete(HIS_TEST_INDEX data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_TEST_INDEX> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_TEST_INDEX> listIdNotFound = new List<HIS_TEST_INDEX>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_TEST_INDEX> listRaw = ctx.HIS_TEST_INDEX.Where(o => listId.Contains(o.ID)).ToList();
