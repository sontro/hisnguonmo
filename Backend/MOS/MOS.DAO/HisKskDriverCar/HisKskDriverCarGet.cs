...
{
    partial class HisKskDriverCarGet : EntityBase
    {
        public List<HIS_KSK_DRIVER_CAR> Get(HisKskDriverCarSO search, CommonParam param)
        {
            List<HIS_KSK_DRIVER_CAR> list = new List<HIS_KSK_DRIVER_CAR>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_KSK_DRIVER_CAR.AsNoTracking().AsQueryable();
                        if (search.listHisKskDriverCarExpression != null && search.listHisKskDriverCarExpression.Count > 0)
                        {
                            foreach (var item in search.listHisKskDriverCarExpression)
                            {
        ...
        public HIS_KSK_DRIVER_CAR GetById(long id, HisKskDriverCarSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_KSK_DRIVER_CAR.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisKskDriverCarExpression != null && search.listHisKskDriverCarExpression.Count > 0)
                        {
                            foreach (var item in search.listHisKskDriverCarExpression)
                            {
