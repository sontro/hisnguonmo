...
{
    partial class HisDiseaseTypeGet : EntityBase
    {
        public List<HIS_DISEASE_TYPE> Get(HisDiseaseTypeSO search, CommonParam param)
        {
            List<HIS_DISEASE_TYPE> list = new List<HIS_DISEASE_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_DISEASE_TYPE.AsNoTracking().AsQueryable();
                        if (search.listHisDiseaseTypeExpression != null && search.listHisDiseaseTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisDiseaseTypeExpression)
                            {
        ...
        public HIS_DISEASE_TYPE GetById(long id, HisDiseaseTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_DISEASE_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisDiseaseTypeExpression != null && search.listHisDiseaseTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisDiseaseTypeExpression)
                            {
