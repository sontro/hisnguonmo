...
namespace MOS.DAO.HisServiceSame
{
    partial class HisServiceSameGet : EntityBase
    {
        ...
        {
            List<V_HIS_SERVICE_SAME> list = new List<V_HIS_SERVICE_SAME>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_SERVICE_SAME.AsNoTracking().AsQueryable();
                        if (search.listVHisServiceSameExpression != null && search.listVHisServiceSameExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisServiceSameExpression)
                            {
