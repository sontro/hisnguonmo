...
{
    partial class HisServiceSameDelete : EntityBase
    {
        public HisServiceSameDelete()
            : base()
        {
        ...
        public bool Delete(HIS_SERVICE_SAME data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_SERVICE_SAME> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_SERVICE_SAME> listIdNotFound = new List<HIS_SERVICE_SAME>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_SERVICE_SAME> listRaw = ctx.HIS_SERVICE_SAME.Where(o => listId.Contains(o.ID)).ToList();
