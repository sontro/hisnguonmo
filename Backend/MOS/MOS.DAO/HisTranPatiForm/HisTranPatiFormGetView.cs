...
namespace MOS.DAO.HisTranPatiForm
{
    partial class HisTranPatiFormGet : EntityBase
    {
        ...
        {
            List<V_HIS_TRAN_PATI_FORM> list = new List<V_HIS_TRAN_PATI_FORM>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_TRAN_PATI_FORM.AsQueryable();
                        if (search.listVHisTranPatiFormExpression != null && search.listVHisTranPatiFormExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisTranPatiFormExpression)
                            {
