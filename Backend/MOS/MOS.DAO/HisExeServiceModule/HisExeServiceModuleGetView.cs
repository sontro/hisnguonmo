...
namespace MOS.DAO.HisExeServiceModule
{
    partial class HisExeServiceModuleGet : EntityBase
    {
        ...
        {
            List<V_HIS_EXE_SERVICE_MODULE> list = new List<V_HIS_EXE_SERVICE_MODULE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_EXE_SERVICE_MODULE.AsNoTracking().AsQueryable();
                        if (search.listVHisExeServiceModuleExpression != null && search.listVHisExeServiceModuleExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisExeServiceModuleExpression)
                            {
