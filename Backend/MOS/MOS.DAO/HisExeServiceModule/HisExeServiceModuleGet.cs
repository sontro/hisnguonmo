...
namespace MOS.DAO.HisExeServiceModule
{
    partial class HisExeServiceModuleGet : EntityBase
    {
        ...
        {
            List<HIS_EXE_SERVICE_MODULE> list = new List<HIS_EXE_SERVICE_MODULE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_EXE_SERVICE_MODULE.AsNoTracking().AsQueryable();
        ...
        public HIS_EXE_SERVICE_MODULE GetById(long id, HisExeServiceModuleSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_EXE_SERVICE_MODULE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisExeServiceModuleExpression != null && search.listHisExeServiceModuleExpression.Count > 0)
                        {
                            foreach (var item in search.listHisExeServiceModuleExpression)
                            {
