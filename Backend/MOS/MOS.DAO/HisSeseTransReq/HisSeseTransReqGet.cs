...
namespace MOS.DAO.HisSeseTransReq
{
    partial class HisSeseTransReqGet : EntityBase
    {
        ...
        {
            List<HIS_SESE_TRANS_REQ> list = new List<HIS_SESE_TRANS_REQ>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SESE_TRANS_REQ.AsNoTracking().AsQueryable();
        ...
        public HIS_SESE_TRANS_REQ GetById(long id, HisSeseTransReqSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_SESE_TRANS_REQ.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisSeseTransReqExpression != null && search.listHisSeseTransReqExpression.Count > 0)
                        {
                            foreach (var item in search.listHisSeseTransReqExpression)
                            {
