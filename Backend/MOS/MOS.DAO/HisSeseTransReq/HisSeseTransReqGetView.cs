...
namespace MOS.DAO.HisSeseTransReq
{
    partial class HisSeseTransReqGet : EntityBase
    {
        ...
        {
            List<V_HIS_SESE_TRANS_REQ> list = new List<V_HIS_SESE_TRANS_REQ>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_SESE_TRANS_REQ.AsNoTracking().AsQueryable();
                        if (search.listVHisSeseTransReqExpression != null && search.listVHisSeseTransReqExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisSeseTransReqExpression)
                            {
