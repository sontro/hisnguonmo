...
{
    partial class HisPtttMethodDelete : EntityBase
    {
        public HisPtttMethodDelete()
            : base()
        {
        ...
        public bool Delete(HIS_PTTT_METHOD data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_PTTT_METHOD> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_PTTT_METHOD> listIdNotFound = new List<HIS_PTTT_METHOD>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_PTTT_METHOD> listRaw = ctx.HIS_PTTT_METHOD.Where(o => listId.Contains(o.ID)).ToList();
