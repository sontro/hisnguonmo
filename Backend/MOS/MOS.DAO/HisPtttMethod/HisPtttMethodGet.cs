...
namespace MOS.DAO.HisPtttMethod
{
    partial class HisPtttMethodGet : EntityBase
    {
        ...
        {
            List<HIS_PTTT_METHOD> list = new List<HIS_PTTT_METHOD>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_PTTT_METHOD.AsNoTracking().AsQueryable();
                        if (search.listHisPtttMethodExpression != null && search.listHisPtttMethodExpression.Count > 0)
                        {
                            foreach (var item in search.listHisPtttMethodExpression)
                            {
        ...
        public HIS_PTTT_METHOD GetById(long id, HisPtttMethodSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_PTTT_METHOD.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisPtttMethodExpression != null && search.listHisPtttMethodExpression.Count > 0)
                        {
                            foreach (var item in search.listHisPtttMethodExpression)
                            {
