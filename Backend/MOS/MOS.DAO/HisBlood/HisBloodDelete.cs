...
{
    partial class HisBloodDelete : EntityBase
    {
        public HisBloodDelete()
            : base()
        {
        ...
        public bool Delete(HIS_BLOOD data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_BLOOD> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_BLOOD> listIdNotFound = new List<HIS_BLOOD>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_BLOOD> listRaw = ctx.HIS_BLOOD.Where(o => listId.Contains(o.ID)).ToList();
