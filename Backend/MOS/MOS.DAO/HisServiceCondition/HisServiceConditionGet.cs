...
namespace MOS.DAO.HisServiceCondition
{
    ...
    {
        public List<HIS_SERVICE_CONDITION> Get(HisServiceConditionSO search, CommonParam param)
        {
            List<HIS_SERVICE_CONDITION> list = new List<HIS_SERVICE_CONDITION>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SERVICE_CONDITION.AsNoTracking().AsQueryable();
                        if (search.listHisServiceConditionExpression != null && search.listHisServiceConditionExpression.Count > 0)
                        {
                            foreach (var item in search.listHisServiceConditionExpression)
                            {
        ...
        public HIS_SERVICE_CONDITION GetById(long id, HisServiceConditionSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_SERVICE_CONDITION.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisServiceConditionExpression != null && search.listHisServiceConditionExpression.Count > 0)
                        {
                            foreach (var item in search.listHisServiceConditionExpression)
                            {
