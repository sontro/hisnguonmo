...
{
    partial class HisServiceConditionDelete : EntityBase
    {
        public HisServiceConditionDelete()
            : base()
        {
        ...
        public bool Delete(HIS_SERVICE_CONDITION data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_SERVICE_CONDITION> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_SERVICE_CONDITION> listIdNotFound = new List<HIS_SERVICE_CONDITION>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_SERVICE_CONDITION> listRaw = ctx.HIS_SERVICE_CONDITION.Where(o => listId.Contains(o.ID)).ToList();
