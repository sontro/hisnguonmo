...
{
    partial class HisDebateTypeDelete : EntityBase
    {
        public HisDebateTypeDelete()
            : base()
        {
        ...
        public bool Delete(HIS_DEBATE_TYPE data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_DEBATE_TYPE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_DEBATE_TYPE> listIdNotFound = new List<HIS_DEBATE_TYPE>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_DEBATE_TYPE> listRaw = ctx.HIS_DEBATE_TYPE.Where(o => listId.Contains(o.ID)).ToList();
