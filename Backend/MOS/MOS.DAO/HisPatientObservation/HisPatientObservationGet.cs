...
namespace MOS.DAO.HisPatientObservation
{
    partial class HisPatientObservationGet : EntityBase
    {
        ...
        {
            List<HIS_PATIENT_OBSERVATION> list = new List<HIS_PATIENT_OBSERVATION>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_PATIENT_OBSERVATION.AsNoTracking().AsQueryable();
        ...
        public HIS_PATIENT_OBSERVATION GetById(long id, HisPatientObservationSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_PATIENT_OBSERVATION.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisPatientObservationExpression != null && search.listHisPatientObservationExpression.Count > 0)
                        {
                            foreach (var item in search.listHisPatientObservationExpression)
                            {
