...
namespace MOS.DAO.HisEmteMedicineType
{
    partial class HisEmteMedicineTypeGet : EntityBase
    {
        ...
        {
            List<V_HIS_EMTE_MEDICINE_TYPE> list = new List<V_HIS_EMTE_MEDICINE_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_EMTE_MEDICINE_TYPE.AsNoTracking().AsQueryable();
                        if (search.listVHisEmteMedicineTypeExpression != null && search.listVHisEmteMedicineTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisEmteMedicineTypeExpression)
                            {
