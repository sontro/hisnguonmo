...
namespace MOS.DAO.HisEmteMedicineType
{
    partial class HisEmteMedicineTypeGet : EntityBase
    {
        ...
        {
            List<HIS_EMTE_MEDICINE_TYPE> list = new List<HIS_EMTE_MEDICINE_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_EMTE_MEDICINE_TYPE.AsNoTracking().AsQueryable();
                        if (search.listHisEmteMedicineTypeExpression != null && search.listHisEmteMedicineTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisEmteMedicineTypeExpression)
                            {
        ...
        public HIS_EMTE_MEDICINE_TYPE GetById(long id, HisEmteMedicineTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_EMTE_MEDICINE_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisEmteMedicineTypeExpression != null && search.listHisEmteMedicineTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisEmteMedicineTypeExpression)
                            {
