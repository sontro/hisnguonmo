...
namespace MOS.DAO.HisAntigenMety
{
    partial class HisAntigenMetyGet : EntityBase
    {
        ...
        {
            List<HIS_ANTIGEN_METY> list = new List<HIS_ANTIGEN_METY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_ANTIGEN_METY.AsNoTracking().AsQueryable();
        ...
        public HIS_ANTIGEN_METY GetById(long id, HisAntigenMetySO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_ANTIGEN_METY.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisAntigenMetyExpression != null && search.listHisAntigenMetyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisAntigenMetyExpression)
                            {
