    ...
    partial class HisEventsCausesDeathCheck : EntityBase
    {
        public bool ExistsCode(string code, long? id)
        {
            ...
            try
            {
                ...
                {
                    count = ctx.HIS_EVENTS_CAUSES_DEATH.AsQueryable().Where(p => p.EVENTS_CAUSES_DEATH_CODE.Equals(code) && p.ID != id && (p.IS_DELETE.HasValue || p.IS_DELETE.Value != (short)1)).Count();
                }
                result = (count > 0);
            }
            catch (Exception)
