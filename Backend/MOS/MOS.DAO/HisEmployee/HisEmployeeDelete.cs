...
{
    partial class HisEmployeeDelete : EntityBase
    {
        public HisEmployeeDelete()
            : base()
        {
        ...
        public bool Delete(HIS_EMPLOYEE data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_EMPLOYEE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_EMPLOYEE> listIdNotFound = new List<HIS_EMPLOYEE>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_EMPLOYEE> listRaw = ctx.HIS_EMPLOYEE.Where(o => listId.Contains(o.ID)).ToList();
