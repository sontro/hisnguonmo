...
namespace MOS.DAO.HisEmployee
{
    partial class HisEmployeeGet : EntityBase
    {
        ...
        {
            List<V_HIS_EMPLOYEE> list = new List<V_HIS_EMPLOYEE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_EMPLOYEE.AsNoTracking().AsQueryable();
                        if (search.listVHisEmployeeExpression != null && search.listVHisEmployeeExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisEmployeeExpression)
                            {
