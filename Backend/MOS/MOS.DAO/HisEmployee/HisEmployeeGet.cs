...
{
    partial class HisEmployeeGet : EntityBase
    {
        public List<HIS_EMPLOYEE> Get(HisEmployeeSO search, CommonParam param)
        {
            List<HIS_EMPLOYEE> list = new List<HIS_EMPLOYEE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_EMPLOYEE.AsNoTracking().AsQueryable();
                        if (search.listHisEmployeeExpression != null && search.listHisEmployeeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisEmployeeExpression)
                            {
        ...
        public HIS_EMPLOYEE GetById(long id, HisEmployeeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_EMPLOYEE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisEmployeeExpression != null && search.listHisEmployeeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisEmployeeExpression)
                            {
