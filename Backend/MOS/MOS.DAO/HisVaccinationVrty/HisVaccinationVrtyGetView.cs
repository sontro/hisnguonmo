...
namespace MOS.DAO.HisVaccinationVrty
{
    partial class HisVaccinationVrtyGet : EntityBase
    {
        ...
        {
            List<V_HIS_VACCINATION_VRTY> list = new List<V_HIS_VACCINATION_VRTY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_VACCINATION_VRTY.AsNoTracking().AsQueryable();
                        if (search.listVHisVaccinationVrtyExpression != null && search.listVHisVaccinationVrtyExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisVaccinationVrtyExpression)
                            {
