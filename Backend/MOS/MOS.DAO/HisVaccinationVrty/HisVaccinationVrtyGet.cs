...
{
    partial class HisVaccinationVrtyGet : EntityBase
    {
        public List<HIS_VACCINATION_VRTY> Get(HisVaccinationVrtySO search, CommonParam param)
        {
            List<HIS_VACCINATION_VRTY> list = new List<HIS_VACCINATION_VRTY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_VACCINATION_VRTY.AsNoTracking().AsQueryable();
                        if (search.listHisVaccinationVrtyExpression != null && search.listHisVaccinationVrtyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisVaccinationVrtyExpression)
                            {
        ...
        public HIS_VACCINATION_VRTY GetById(long id, HisVaccinationVrtySO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_VACCINATION_VRTY.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisVaccinationVrtyExpression != null && search.listHisVaccinationVrtyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisVaccinationVrtyExpression)
                            {
