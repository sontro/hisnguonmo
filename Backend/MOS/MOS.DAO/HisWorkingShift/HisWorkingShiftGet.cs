...
namespace MOS.DAO.HisWorkingShift
{
    partial class HisWorkingShiftGet : EntityBase
    {
        ...
        {
            List<HIS_WORKING_SHIFT> list = new List<HIS_WORKING_SHIFT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_WORKING_SHIFT.AsNoTracking().AsQueryable();
        ...
        public HIS_WORKING_SHIFT GetById(long id, HisWorkingShiftSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_WORKING_SHIFT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisWorkingShiftExpression != null && search.listHisWorkingShiftExpression.Count > 0)
                        {
                            foreach (var item in search.listHisWorkingShiftExpression)
                            {
