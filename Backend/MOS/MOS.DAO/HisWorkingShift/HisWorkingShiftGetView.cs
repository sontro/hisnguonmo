...
namespace MOS.DAO.HisWorkingShift
{
    partial class HisWorkingShiftGet : EntityBase
    {
        ...
        {
            List<V_HIS_WORKING_SHIFT> list = new List<V_HIS_WORKING_SHIFT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_WORKING_SHIFT.AsNoTracking().AsQueryable();
                        if (search.listVHisWorkingShiftExpression != null && search.listVHisWorkingShiftExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisWorkingShiftExpression)
                            {
