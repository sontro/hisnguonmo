...
namespace MOS.DAO.HisVaccinationReact
{
    partial class HisVaccinationReactGet : EntityBase
    {
        ...
        {
            List<HIS_VACCINATION_REACT> list = new List<HIS_VACCINATION_REACT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_VACCINATION_REACT.AsNoTracking().AsQueryable();
                        if (search.listHisVaccinationReactExpression != null && search.listHisVaccinationReactExpression.Count > 0)
                        {
                            foreach (var item in search.listHisVaccinationReactExpression)
                            {
        ...
        public HIS_VACCINATION_REACT GetById(long id, HisVaccinationReactSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_VACCINATION_REACT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisVaccinationReactExpression != null && search.listHisVaccinationReactExpression.Count > 0)
                        {
                            foreach (var item in search.listHisVaccinationReactExpression)
                            {
