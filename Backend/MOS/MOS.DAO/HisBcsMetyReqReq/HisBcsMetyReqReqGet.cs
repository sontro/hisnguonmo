...
{
    partial class HisBcsMetyReqReqGet : EntityBase
    {
        public List<HIS_BCS_METY_REQ_REQ> Get(HisBcsMetyReqReqSO search, CommonParam param)
        {
            List<HIS_BCS_METY_REQ_REQ> list = new List<HIS_BCS_METY_REQ_REQ>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_BCS_METY_REQ_REQ.AsNoTracking().AsQueryable();
                        if (search.listHisBcsMetyReqReqExpression != null && search.listHisBcsMetyReqReqExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBcsMetyReqReqExpression)
                            {
        ...
        public HIS_BCS_METY_REQ_REQ GetById(long id, HisBcsMetyReqReqSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_BCS_METY_REQ_REQ.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisBcsMetyReqReqExpression != null && search.listHisBcsMetyReqReqExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBcsMetyReqReqExpression)
                            {
