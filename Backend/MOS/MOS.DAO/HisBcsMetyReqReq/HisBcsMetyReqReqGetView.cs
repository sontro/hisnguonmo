...
namespace MOS.DAO.HisBcsMetyReqReq
{
    partial class HisBcsMetyReqReqGet : EntityBase
    {
        ...
        {
            List<V_HIS_BCS_METY_REQ_REQ> list = new List<V_HIS_BCS_METY_REQ_REQ>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_BCS_METY_REQ_REQ.AsNoTracking().AsQueryable();
                        if (search.listVHisBcsMetyReqReqExpression != null && search.listVHisBcsMetyReqReqExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisBcsMetyReqReqExpression)
                            {
