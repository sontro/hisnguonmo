        ...
        public List<HIS_MIXED_MEDICINE> Get(HisMixedMedicineSO search, CommonParam param)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MIXED_MEDICINE.AsNoTracking().AsQueryable();
                        if (search.listHisMixedMedicineExpression != null && search.listHisMixedMedicineExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMixedMedicineExpression)
                            {
        ...
        public HIS_MIXED_MEDICINE GetById(long id, HisMixedMedicineSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_MIXED_MEDICINE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMixedMedicineExpression != null && search.listHisMixedMedicineExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMixedMedicineExpression)
                            {
