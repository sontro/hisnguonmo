...
namespace MOS.DAO.HisMixedMedicine
{
    partial class HisMixedMedicineGet : EntityBase
    {
        ...
        {
            List<V_HIS_MIXED_MEDICINE> list = new List<V_HIS_MIXED_MEDICINE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_MIXED_MEDICINE.AsNoTracking().AsQueryable();
                        if (search.listVHisMixedMedicineExpression != null && search.listVHisMixedMedicineExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisMixedMedicineExpression)
                            {
