...
namespace MOS.DAO.HisRationSum
{
    partial class HisRationSumGet : EntityBase
    {
        ...
        {
            List<HIS_RATION_SUM> list = new List<HIS_RATION_SUM>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_RATION_SUM.AsNoTracking().AsQueryable();
        ...
        public HIS_RATION_SUM GetById(long id, HisRationSumSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_RATION_SUM.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisRationSumExpression != null && search.listHisRationSumExpression.Count > 0)
                        {
                            foreach (var item in search.listHisRationSumExpression)
                            {
