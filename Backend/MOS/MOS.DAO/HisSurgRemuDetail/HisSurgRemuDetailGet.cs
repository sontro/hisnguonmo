...
namespace MOS.DAO.HisSurgRemuDetail
{
    partial class HisSurgRemuDetailGet : EntityBase
    {
        ...
        {
            List<HIS_SURG_REMU_DETAIL> list = new List<HIS_SURG_REMU_DETAIL>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SURG_REMU_DETAIL.AsNoTracking().AsQueryable();
        ...
        public HIS_SURG_REMU_DETAIL GetById(long id, HisSurgRemuDetailSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_SURG_REMU_DETAIL.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisSurgRemuDetailExpression != null && search.listHisSurgRemuDetailExpression.Count > 0)
                        {
                            foreach (var item in search.listHisSurgRemuDetailExpression)
                            {
