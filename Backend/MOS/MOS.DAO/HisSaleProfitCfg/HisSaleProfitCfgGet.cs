...
namespace MOS.DAO.HisSaleProfitCfg
{
    partial class HisSaleProfitCfgGet : EntityBase
    {
        ...
        {
            List<HIS_SALE_PROFIT_CFG> list = new List<HIS_SALE_PROFIT_CFG>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SALE_PROFIT_CFG.AsNoTracking().AsQueryable();
        ...
        public HIS_SALE_PROFIT_CFG GetById(long id, HisSaleProfitCfgSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_SALE_PROFIT_CFG.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisSaleProfitCfgExpression != null && search.listHisSaleProfitCfgExpression.Count > 0)
                        {
                            foreach (var item in search.listHisSaleProfitCfgExpression)
                            {
