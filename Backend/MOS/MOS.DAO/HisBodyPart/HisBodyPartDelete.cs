...
{
    partial class HisBodyPartDelete : EntityBase
    {
        public HisBodyPartDelete()
            : base()
        {
        ...
        public bool Delete(HIS_BODY_PART data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_BODY_PART> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_BODY_PART> listIdNotFound = new List<HIS_BODY_PART>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_BODY_PART> listRaw = ctx.HIS_BODY_PART.Where(o => listId.Contains(o.ID)).ToList();
