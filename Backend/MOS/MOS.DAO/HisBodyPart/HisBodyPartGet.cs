...
namespace MOS.DAO.HisBodyPart
{
    partial class HisBodyPartGet : EntityBase
    {
        ...
        {
            List<HIS_BODY_PART> list = new List<HIS_BODY_PART>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_BODY_PART.AsNoTracking().AsQueryable();
                        if (search.listHisBodyPartExpression != null && search.listHisBodyPartExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBodyPartExpression)
                            {
        ...
        public HIS_BODY_PART GetById(long id, HisBodyPartSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_BODY_PART.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisBodyPartExpression != null && search.listHisBodyPartExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBodyPartExpression)
                            {
