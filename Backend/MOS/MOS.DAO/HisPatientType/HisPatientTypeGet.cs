...
namespace MOS.DAO.HisPatientType
{
    partial class HisPatientTypeGet : EntityBase
    {
        ...
        {
            List<HIS_PATIENT_TYPE> list = new List<HIS_PATIENT_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_PATIENT_TYPE.AsNoTracking().AsQueryable();
                        if (search.listHisPatientTypeExpression != null && search.listHisPatientTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisPatientTypeExpression)
                            {
        ...
        public HIS_PATIENT_TYPE GetById(long id, HisPatientTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_PATIENT_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisPatientTypeExpression != null && search.listHisPatientTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisPatientTypeExpression)
                            {
