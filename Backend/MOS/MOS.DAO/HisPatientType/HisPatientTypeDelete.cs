...
{
    partial class HisPatientTypeDelete : EntityBase
    {
        public HisPatientTypeDelete()
            : base()
        {
        ...
        public bool Delete(HIS_PATIENT_TYPE data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_PATIENT_TYPE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_PATIENT_TYPE> listIdNotFound = new List<HIS_PATIENT_TYPE>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_PATIENT_TYPE> listRaw = ctx.HIS_PATIENT_TYPE.Where(o => listId.Contains(o.ID)).ToList();
