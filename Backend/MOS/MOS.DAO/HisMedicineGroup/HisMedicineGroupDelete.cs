...
{
    partial class HisMedicineGroupDelete : EntityBase
    {
        public HisMedicineGroupDelete()
            : base()
        {
        ...
        public bool Delete(HIS_MEDICINE_GROUP data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_MEDICINE_GROUP> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_MEDICINE_GROUP> listIdNotFound = new List<HIS_MEDICINE_GROUP>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_MEDICINE_GROUP> listRaw = ctx.HIS_MEDICINE_GROUP.Where(o => listId.Contains(o.ID)).ToList();
