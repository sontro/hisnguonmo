...
namespace MOS.DAO.HisCaroAccountBook
{
    partial class HisCaroAccountBookGet : EntityBase
    {
        ...
        {
            List<V_HIS_CARO_ACCOUNT_BOOK> list = new List<V_HIS_CARO_ACCOUNT_BOOK>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_CARO_ACCOUNT_BOOK.AsNoTracking().AsQueryable();
                        if (search.listVHisCaroAccountBookExpression != null && search.listVHisCaroAccountBookExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisCaroAccountBookExpression)
                            {
