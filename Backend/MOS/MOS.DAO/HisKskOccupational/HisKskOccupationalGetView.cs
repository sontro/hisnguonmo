...
namespace MOS.DAO.HisKskOccupational
{
    partial class HisKskOccupationalGet : EntityBase
    {
        ...
        {
            List<V_HIS_KSK_OCCUPATIONAL> list = new List<V_HIS_KSK_OCCUPATIONAL>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_KSK_OCCUPATIONAL.AsNoTracking().AsQueryable();
                        if (search.listVHisKskOccupationalExpression != null && search.listVHisKskOccupationalExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisKskOccupationalExpression)
                            {
