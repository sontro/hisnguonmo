...
{
    partial class HisKskOccupationalGet : EntityBase
    {
        public List<HIS_KSK_OCCUPATIONAL> Get(HisKskOccupationalSO search, CommonParam param)
        {
            List<HIS_KSK_OCCUPATIONAL> list = new List<HIS_KSK_OCCUPATIONAL>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_KSK_OCCUPATIONAL.AsNoTracking().AsQueryable();
                        if (search.listHisKskOccupationalExpression != null && search.listHisKskOccupationalExpression.Count > 0)
                        {
                            foreach (var item in search.listHisKskOccupationalExpression)
                            {
        ...
        public HIS_KSK_OCCUPATIONAL GetById(long id, HisKskOccupationalSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_KSK_OCCUPATIONAL.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisKskOccupationalExpression != null && search.listHisKskOccupationalExpression.Count > 0)
                        {
                            foreach (var item in search.listHisKskOccupationalExpression)
                            {
