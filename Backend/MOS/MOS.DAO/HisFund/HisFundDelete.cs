...
{
    partial class HisFundDelete : EntityBase
    {
        public HisFundDelete()
            : base()
        {
        ...
        public bool Delete(HIS_FUND data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_FUND> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_FUND> listIdNotFound = new List<HIS_FUND>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_FUND> listRaw = ctx.HIS_FUND.Where(o => listId.Contains(o.ID)).ToList();
