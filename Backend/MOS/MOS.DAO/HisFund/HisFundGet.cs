...
namespace MOS.DAO.HisFund
{
    partial class HisFundGet : EntityBase
    {
        ...
        {
            List<HIS_FUND> list = new List<HIS_FUND>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_FUND.AsNoTracking().AsQueryable();
                        if (search.listHisFundExpression != null && search.listHisFundExpression.Count > 0)
                        {
                            foreach (var item in search.listHisFundExpression)
                            {
        ...
        public HIS_FUND GetById(long id, HisFundSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_FUND.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisFundExpression != null && search.listHisFundExpression.Count > 0)
                        {
                            foreach (var item in search.listHisFundExpression)
                            {
