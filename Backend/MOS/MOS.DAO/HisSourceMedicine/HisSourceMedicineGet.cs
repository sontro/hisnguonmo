...
namespace MOS.DAO.HisSourceMedicine
{
    partial class HisSourceMedicineGet : EntityBase
    {
        ...
        {
            List<HIS_SOURCE_MEDICINE> list = new List<HIS_SOURCE_MEDICINE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SOURCE_MEDICINE.AsNoTracking().AsQueryable();
                        if (search.listHisSourceMedicineExpression != null && search.listHisSourceMedicineExpression.Count > 0)
                        {
                            foreach (var item in search.listHisSourceMedicineExpression)
                            {
        ...
        public HIS_SOURCE_MEDICINE GetById(long id, HisSourceMedicineSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_SOURCE_MEDICINE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisSourceMedicineExpression != null && search.listHisSourceMedicineExpression.Count > 0)
                        {
                            foreach (var item in search.listHisSourceMedicineExpression)
                            {
