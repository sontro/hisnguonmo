...
namespace MOS.DAO.HisEyeSurgryDesc
{
    partial class HisEyeSurgryDescGet : EntityBase
    {
        ...
        {
            List<HIS_EYE_SURGRY_DESC> list = new List<HIS_EYE_SURGRY_DESC>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_EYE_SURGRY_DESC.AsNoTracking().AsQueryable();
        ...
        public HIS_EYE_SURGRY_DESC GetById(long id, HisEyeSurgryDescSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_EYE_SURGRY_DESC.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisEyeSurgryDescExpression != null && search.listHisEyeSurgryDescExpression.Count > 0)
                        {
                            foreach (var item in search.listHisEyeSurgryDescExpression)
                            {
