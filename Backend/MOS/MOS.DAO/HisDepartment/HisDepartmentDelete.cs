...
{
    partial class HisDepartmentDelete : EntityBase
    {
        public HisDepartmentDelete()
            : base()
        {
        ...
        public bool Delete(HIS_DEPARTMENT data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_DEPARTMENT> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_DEPARTMENT> listIdNotFound = new List<HIS_DEPARTMENT>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_DEPARTMENT> listRaw = ctx.HIS_DEPARTMENT.Where(o => listId.Contains(o.ID)).ToList();
