...
{
    partial class HisImpSourceDelete : EntityBase
    {
        public HisImpSourceDelete()
            : base()
        {
        ...
        public bool Delete(HIS_IMP_SOURCE data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_IMP_SOURCE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_IMP_SOURCE> listIdNotFound = new List<HIS_IMP_SOURCE>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_IMP_SOURCE> listRaw = ctx.HIS_IMP_SOURCE.Where(o => listId.Contains(o.ID)).ToList();
