...
namespace MOS.DAO.HisMemaGroup
{
    partial class HisMemaGroupGet : EntityBase
    {
        ...
        {
            List<HIS_MEMA_GROUP> list = new List<HIS_MEMA_GROUP>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MEMA_GROUP.AsNoTracking().AsQueryable();
                        if (search.listHisMemaGroupExpression != null && search.listHisMemaGroupExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMemaGroupExpression)
                            {
        ...
        public HIS_MEMA_GROUP GetById(long id, HisMemaGroupSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_MEMA_GROUP.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMemaGroupExpression != null && search.listHisMemaGroupExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMemaGroupExpression)
                            {
