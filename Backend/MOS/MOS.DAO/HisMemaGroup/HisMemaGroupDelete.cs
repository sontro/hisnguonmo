...
{
    partial class HisMemaGroupDelete : EntityBase
    {
        public HisMemaGroupDelete()
            : base()
        {
        ...
        public bool Delete(HIS_MEMA_GROUP data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_MEMA_GROUP> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_MEMA_GROUP> listIdNotFound = new List<HIS_MEMA_GROUP>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_MEMA_GROUP> listRaw = ctx.HIS_MEMA_GROUP.Where(o => listId.Contains(o.ID)).ToList();
