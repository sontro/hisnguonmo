...
namespace MOS.DAO.HisUnlimitType
{
    ...
    {
        public List<HIS_UNLIMIT_TYPE> Get(HisUnlimitTypeSO search, CommonParam param)
        {
            List<HIS_UNLIMIT_TYPE> list = new List<HIS_UNLIMIT_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_UNLIMIT_TYPE.AsNoTracking().AsQueryable();
        ...
        public HIS_UNLIMIT_TYPE GetById(long id, HisUnlimitTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_UNLIMIT_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisUnlimitTypeExpression != null && search.listHisUnlimitTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisUnlimitTypeExpression)
                            {
