...
{
    partial class HisUnlimitTypeDelete : EntityBase
    {
        public HisUnlimitTypeDelete()
            : base()
        {
        ...
        public bool Delete(HIS_UNLIMIT_TYPE data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_UNLIMIT_TYPE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_UNLIMIT_TYPE> listIdNotFound = new List<HIS_UNLIMIT_TYPE>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_UNLIMIT_TYPE> listRaw = ctx.HIS_UNLIMIT_TYPE.Where(o => listId.Contains(o.ID)).ToList();
