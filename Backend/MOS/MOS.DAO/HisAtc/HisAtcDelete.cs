...
{
    partial class HisAtcDelete : EntityBase
    {
        public HisAtcDelete()
            : base()
        {
        ...
        public bool Delete(HIS_ATC data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_ATC> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_ATC> listIdNotFound = new List<HIS_ATC>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_ATC> listRaw = ctx.HIS_ATC.Where(o => listId.Contains(o.ID)).ToList();
