...
{
    partial class HisAtcGet : EntityBase
    {
        public List<HIS_ATC> Get(HisAtcSO search, CommonParam param)
        {
            List<HIS_ATC> list = new List<HIS_ATC>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_ATC.AsNoTracking().AsQueryable();
                        if (search.listHisAtcExpression != null && search.listHisAtcExpression.Count > 0)
                        {
                            foreach (var item in search.listHisAtcExpression)
                            {
        ...
        public HIS_ATC GetById(long id, HisAtcSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_ATC.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisAtcExpression != null && search.listHisAtcExpression.Count > 0)
                        {
                            foreach (var item in search.listHisAtcExpression)
                            {
