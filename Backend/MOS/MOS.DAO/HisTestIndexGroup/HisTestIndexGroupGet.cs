...
namespace MOS.DAO.HisTestIndexGroup
{
    partial class HisTestIndexGroupGet : EntityBase
    {
        ...
        {
            List<HIS_TEST_INDEX_GROUP> list = new List<HIS_TEST_INDEX_GROUP>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_TEST_INDEX_GROUP.AsNoTracking().AsQueryable();
        ...
        public HIS_TEST_INDEX_GROUP GetById(long id, HisTestIndexGroupSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_TEST_INDEX_GROUP.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisTestIndexGroupExpression != null && search.listHisTestIndexGroupExpression.Count > 0)
                        {
                            foreach (var item in search.listHisTestIndexGroupExpression)
                            {
