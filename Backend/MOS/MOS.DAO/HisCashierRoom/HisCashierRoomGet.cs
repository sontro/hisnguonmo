...
namespace MOS.DAO.HisCashierRoom
{
    partial class HisCashierRoomGet : EntityBase
    {
        ...
        {
            List<HIS_CASHIER_ROOM> list = new List<HIS_CASHIER_ROOM>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_CASHIER_ROOM.AsNoTracking().AsQueryable();
        ...
        public HIS_CASHIER_ROOM GetById(long id, HisCashierRoomSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_CASHIER_ROOM.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisCashierRoomExpression != null && search.listHisCashierRoomExpression.Count > 0)
                        {
                            foreach (var item in search.listHisCashierRoomExpression)
                            {
