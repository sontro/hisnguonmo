...
namespace MOS.DAO.HisCashierRoom
{
    partial class HisCashierRoomGet : EntityBase
    {
        ...
        {
            List<V_HIS_CASHIER_ROOM> list = new List<V_HIS_CASHIER_ROOM>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_CASHIER_ROOM.AsNoTracking().AsQueryable();
                        if (search.listVHisCashierRoomExpression != null && search.listVHisCashierRoomExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisCashierRoomExpression)
                            {
