...
namespace MOS.DAO.HisHealthExamRank
{
    partial class HisHealthExamRankGet : EntityBase
    {
        ...
        {
            List<HIS_HEALTH_EXAM_RANK> list = new List<HIS_HEALTH_EXAM_RANK>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_HEALTH_EXAM_RANK.AsNoTracking().AsQueryable();
        ...
        public HIS_HEALTH_EXAM_RANK GetById(long id, HisHealthExamRankSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_HEALTH_EXAM_RANK.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisHealthExamRankExpression != null && search.listHisHealthExamRankExpression.Count > 0)
                        {
                            foreach (var item in search.listHisHealthExamRankExpression)
                            {
