...
namespace MOS.DAO.HisSkinSurgeryDesc
{
    partial class HisSkinSurgeryDescGet : EntityBase
    {
        ...
        {
            List<HIS_SKIN_SURGERY_DESC> list = new List<HIS_SKIN_SURGERY_DESC>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SKIN_SURGERY_DESC.AsNoTracking().AsQueryable();
        ...
        public HIS_SKIN_SURGERY_DESC GetById(long id, HisSkinSurgeryDescSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_SKIN_SURGERY_DESC.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisSkinSurgeryDescExpression != null && search.listHisSkinSurgeryDescExpression.Count > 0)
                        {
                            foreach (var item in search.listHisSkinSurgeryDescExpression)
                            {
