...
namespace MOS.DAO.HisCaroDepartment
{
    partial class HisCaroDepartmentGet : EntityBase
    {
        ...
        {
            List<HIS_CARO_DEPARTMENT> list = new List<HIS_CARO_DEPARTMENT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_CARO_DEPARTMENT.AsNoTracking().AsQueryable();
        ...
        public HIS_CARO_DEPARTMENT GetById(long id, HisCaroDepartmentSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_CARO_DEPARTMENT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisCaroDepartmentExpression != null && search.listHisCaroDepartmentExpression.Count > 0)
                        {
                            foreach (var item in search.listHisCaroDepartmentExpression)
                            {
