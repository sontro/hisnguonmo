...
namespace MOS.DAO.HisCaroDepartment
{
    partial class HisCaroDepartmentGet : EntityBase
    {
        ...
        {
            List<V_HIS_CARO_DEPARTMENT> list = new List<V_HIS_CARO_DEPARTMENT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_CARO_DEPARTMENT.AsQueryable();
                        if (search.listVHisCaroDepartmentExpression != null && search.listVHisCaroDepartmentExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisCaroDepartmentExpression)
                            {
