...
namespace MOS.DAO.HisBcsMatyReqReq
{
    partial class HisBcsMatyReqReqGet : EntityBase
    {
        ...
        {
            List<V_HIS_BCS_MATY_REQ_REQ> list = new List<V_HIS_BCS_MATY_REQ_REQ>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_BCS_MATY_REQ_REQ.AsNoTracking().AsQueryable();
                        if (search.listVHisBcsMatyReqReqExpression != null && search.listVHisBcsMatyReqReqExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisBcsMatyReqReqExpression)
                            {
