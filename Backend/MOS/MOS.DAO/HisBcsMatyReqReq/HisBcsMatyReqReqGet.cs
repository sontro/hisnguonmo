...
{
    partial class HisBcsMatyReqReqGet : EntityBase
    {
        public List<HIS_BCS_MATY_REQ_REQ> Get(HisBcsMatyReqReqSO search, CommonParam param)
        {
            List<HIS_BCS_MATY_REQ_REQ> list = new List<HIS_BCS_MATY_REQ_REQ>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_BCS_MATY_REQ_REQ.AsNoTracking().AsQueryable();
                        if (search.listHisBcsMatyReqReqExpression != null && search.listHisBcsMatyReqReqExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBcsMatyReqReqExpression)
                            {
        ...
        public HIS_BCS_MATY_REQ_REQ GetById(long id, HisBcsMatyReqReqSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_BCS_MATY_REQ_REQ.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisBcsMatyReqReqExpression != null && search.listHisBcsMatyReqReqExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBcsMatyReqReqExpression)
                            {
