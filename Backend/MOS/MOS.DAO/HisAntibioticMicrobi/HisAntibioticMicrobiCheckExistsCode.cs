    ...
    partial class HisAntibioticMicrobiCheck : EntityBase
    {
        public bool ExistsCode(string code, long? id)
        {
            ...
            try
            {
                ...
                {
                    count = ctx.HIS_ANTIBIOTIC_MICROBI.AsQueryable().Where(p => p.ANTIBIOTIC_MICROBI_CODE.Equals(code) && p.ID != id && (p.IS_DELETE.HasValue || p.IS_DELETE.Value != (short)1)).Count();
                }
                result = (count > 0);
            }
            catch (Exception)
