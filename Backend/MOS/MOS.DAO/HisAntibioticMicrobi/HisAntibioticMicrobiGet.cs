...
namespace MOS.DAO.HisAntibioticMicrobi
{
    partial class HisAntibioticMicrobiGet : EntityBase
    {
        ...
        {
            List<HIS_ANTIBIOTIC_MICROBI> list = new List<HIS_ANTIBIOTIC_MICROBI>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_ANTIBIOTIC_MICROBI.AsNoTracking().AsQueryable();
        ...
        public HIS_ANTIBIOTIC_MICROBI GetById(long id, HisAntibioticMicrobiSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_ANTIBIOTIC_MICROBI.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisAntibioticMicrobiExpression != null && search.listHisAntibioticMicrobiExpression.Count > 0)
                        {
                            foreach (var item in search.listHisAntibioticMicrobiExpression)
                            {
