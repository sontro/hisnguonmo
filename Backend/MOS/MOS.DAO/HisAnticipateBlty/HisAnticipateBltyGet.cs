...
namespace MOS.DAO.HisAnticipateBlty
{
    partial class HisAnticipateBltyGet : EntityBase
    {
        ...
        {
            List<HIS_ANTICIPATE_BLTY> list = new List<HIS_ANTICIPATE_BLTY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_ANTICIPATE_BLTY.AsNoTracking().AsQueryable();
        ...
        public HIS_ANTICIPATE_BLTY GetById(long id, HisAnticipateBltySO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_ANTICIPATE_BLTY.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisAnticipateBltyExpression != null && search.listHisAnticipateBltyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisAnticipateBltyExpression)
                            {
