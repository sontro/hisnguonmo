...
{
    partial class HisKskUneiVatyGet : EntityBase
    {
        public List<HIS_KSK_UNEI_VATY> Get(HisKskUneiVatySO search, CommonParam param)
        {
            List<HIS_KSK_UNEI_VATY> list = new List<HIS_KSK_UNEI_VATY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_KSK_UNEI_VATY.AsNoTracking().AsQueryable();
                        if (search.listHisKskUneiVatyExpression != null && search.listHisKskUneiVatyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisKskUneiVatyExpression)
                            {
        ...
        public HIS_KSK_UNEI_VATY GetById(long id, HisKskUneiVatySO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_KSK_UNEI_VATY.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisKskUneiVatyExpression != null && search.listHisKskUneiVatyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisKskUneiVatyExpression)
                            {
