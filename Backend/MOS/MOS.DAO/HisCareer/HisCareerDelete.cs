...
{
    partial class HisCareerDelete : EntityBase
    {
        public HisCareerDelete()
            : base()
        {
        ...
        public bool Delete(HIS_CAREER data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_CAREER> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_CAREER> listIdNotFound = new List<HIS_CAREER>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_CAREER> listRaw = ctx.HIS_CAREER.Where(o => listId.Contains(o.ID)).ToList();
