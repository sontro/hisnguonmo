...
namespace MOS.DAO.HisServiceReq
{
    partial class HisServiceReqGet : EntityBase
    {
        ...
        {
            List<V_HIS_SERVICE_REQ_9> list = new List<V_HIS_SERVICE_REQ_9>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_SERVICE_REQ_9.AsNoTracking().AsQueryable();
                        if (search.listVHisServiceReq9Expression != null && search.listVHisServiceReq9Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisServiceReq9Expression)
                            {
