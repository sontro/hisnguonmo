...
namespace MOS.DAO.HisServiceReq
{
    partial class HisServiceReqGet : EntityBase
    {
        ...
        {
            List<V_HIS_SERVICE_REQ_10> list = new List<V_HIS_SERVICE_REQ_10>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_SERVICE_REQ_10.AsNoTracking().AsQueryable();
                        if (search.listVHisServiceReq10Expression != null && search.listVHisServiceReq10Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisServiceReq10Expression)
                            {
