...
namespace MOS.DAO.HisServiceReq
{
    partial class HisServiceReqGet : EntityBase
    {
        ...
        {
            List<HisServiceReqViewDTO> list = new List<HisServiceReqViewDTO>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        ...
                        var selector = new DynamicQueryProcessor<V_HIS_SERVICE_REQ>();
                        selector.SetMemberInfo(search.DynamicColumns);
                        var q = selector.Select<HisServiceReqViewDTO>(query);
