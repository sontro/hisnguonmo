...
namespace MOS.DAO.HisServiceReq
{
    partial class HisServiceReqGet : EntityBase
    {
        ...
        {
            List<V_HIS_SERVICE_REQ_11> list = new List<V_HIS_SERVICE_REQ_11>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_SERVICE_REQ_11.AsNoTracking().AsQueryable();
                        if (search.listVHisServiceReq11Expression != null && search.listVHisServiceReq11Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisServiceReq11Expression)
                            {
