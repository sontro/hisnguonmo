...
namespace MOS.DAO.HisServiceReq
{
    partial class HisServiceReqGet : EntityBase
    {
        ...
        {
            List<L_HIS_SERVICE_REQ> list = new List<L_HIS_SERVICE_REQ>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.L_HIS_SERVICE_REQ.AsNoTracking().AsQueryable();
                        if (search.listLHisServiceReqExpression != null && search.listLHisServiceReqExpression.Count > 0)
                        {
                            foreach (var item in search.listLHisServiceReqExpression)
                            {
