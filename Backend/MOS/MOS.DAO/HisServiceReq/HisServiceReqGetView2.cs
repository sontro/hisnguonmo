...
namespace MOS.DAO.HisServiceReq
{
    partial class HisServiceReqGet : EntityBase
    {
        ...
        {
            List<V_HIS_SERVICE_REQ_2> list = new List<V_HIS_SERVICE_REQ_2>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_SERVICE_REQ_2.AsNoTracking().AsQueryable();
                        if (search.listVHisServiceReq2Expression != null && search.listVHisServiceReq2Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisServiceReq2Expression)
                            {
