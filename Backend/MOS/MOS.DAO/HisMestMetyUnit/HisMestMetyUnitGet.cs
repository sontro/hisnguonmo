...
namespace MOS.DAO.HisMestMetyUnit
{
    partial class HisMestMetyUnitGet : EntityBase
    {
        ...
        {
            List<HIS_MEST_METY_UNIT> list = new List<HIS_MEST_METY_UNIT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MEST_METY_UNIT.AsNoTracking().AsQueryable();
        ...
        public HIS_MEST_METY_UNIT GetById(long id, HisMestMetyUnitSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_MEST_METY_UNIT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMestMetyUnitExpression != null && search.listHisMestMetyUnitExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMestMetyUnitExpression)
                            {
