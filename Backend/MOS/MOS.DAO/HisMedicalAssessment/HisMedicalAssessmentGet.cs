...
namespace MOS.DAO.HisMedicalAssessment
{
    partial class HisMedicalAssessmentGet : EntityBase
    {
        ...
        {
            List<HIS_MEDICAL_ASSESSMENT> list = new List<HIS_MEDICAL_ASSESSMENT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MEDICAL_ASSESSMENT.AsNoTracking().AsQueryable();
        ...
        public HIS_MEDICAL_ASSESSMENT GetById(long id, HisMedicalAssessmentSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_MEDICAL_ASSESSMENT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMedicalAssessmentExpression != null && search.listHisMedicalAssessmentExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMedicalAssessmentExpression)
                            {
