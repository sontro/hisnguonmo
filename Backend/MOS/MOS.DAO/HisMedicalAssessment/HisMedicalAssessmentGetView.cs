...
namespace MOS.DAO.HisMedicalAssessment
{
    partial class HisMedicalAssessmentGet : EntityBase
    {
        ...
        {
            List<V_HIS_MEDICAL_ASSESSMENT> list = new List<V_HIS_MEDICAL_ASSESSMENT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_MEDICAL_ASSESSMENT.AsNoTracking().AsQueryable();
                        if (search.listVHisMedicalAssessmentExpression != null && search.listVHisMedicalAssessmentExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisMedicalAssessmentExpression)
                            {
