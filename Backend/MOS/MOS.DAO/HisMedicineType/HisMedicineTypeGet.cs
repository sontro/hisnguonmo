...
namespace MOS.DAO.HisMedicineType
{
    partial class HisMedicineTypeGet : EntityBase
    {
        ...
        {
            List<HIS_MEDICINE_TYPE> list = new List<HIS_MEDICINE_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MEDICINE_TYPE.AsNoTracking().AsQueryable();
                        if (search.listHisMedicineTypeExpression != null && search.listHisMedicineTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMedicineTypeExpression)
                            {
        ...
        public HIS_MEDICINE_TYPE GetById(long id, HisMedicineTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_MEDICINE_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMedicineTypeExpression != null && search.listHisMedicineTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMedicineTypeExpression)
                            {
