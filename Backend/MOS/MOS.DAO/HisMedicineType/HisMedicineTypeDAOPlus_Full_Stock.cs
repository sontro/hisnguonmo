...
namespace MOS.DAO.HisMedicineType
{
    ...
    {
        public List<V_HIS_MEDICINE_TYPE_STOCK> GetStockView(HisMedicineTypeSO search, CommonParam param)
        {
            List<V_HIS_MEDICINE_TYPE_STOCK> result = new List<V_HIS_MEDICINE_TYPE_STOCK>();
            ...
            try
            {
                result = GetWorker.GetStockView(search, param);
            }
            catch (Exception ex)
            {
