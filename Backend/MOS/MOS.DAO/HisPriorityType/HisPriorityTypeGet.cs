...
namespace MOS.DAO.HisPriorityType
{
    partial class HisPriorityTypeGet : EntityBase
    {
        ...
        {
            List<HIS_PRIORITY_TYPE> list = new List<HIS_PRIORITY_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_PRIORITY_TYPE.AsNoTracking().AsQueryable();
        ...
        public HIS_PRIORITY_TYPE GetById(long id, HisPriorityTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_PRIORITY_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisPriorityTypeExpression != null && search.listHisPriorityTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisPriorityTypeExpression)
                            {
