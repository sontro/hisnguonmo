...
namespace MOS.DAO.HisDiseaseRelation
{
    partial class HisDiseaseRelationGet : EntityBase
    {
        ...
        {
            List<HIS_DISEASE_RELATION> list = new List<HIS_DISEASE_RELATION>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_DISEASE_RELATION.AsNoTracking().AsQueryable();
        ...
        public HIS_DISEASE_RELATION GetById(long id, HisDiseaseRelationSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_DISEASE_RELATION.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisDiseaseRelationExpression != null && search.listHisDiseaseRelationExpression.Count > 0)
                        {
                            foreach (var item in search.listHisDiseaseRelationExpression)
                            {
