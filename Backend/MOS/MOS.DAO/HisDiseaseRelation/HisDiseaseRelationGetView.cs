...
namespace MOS.DAO.HisDiseaseRelation
{
    partial class HisDiseaseRelationGet : EntityBase
    {
        ...
        {
            List<V_HIS_DISEASE_RELATION> list = new List<V_HIS_DISEASE_RELATION>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_DISEASE_RELATION.AsQueryable();
                        if (search.listVHisDiseaseRelationExpression != null && search.listVHisDiseaseRelationExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisDiseaseRelationExpression)
                            {
