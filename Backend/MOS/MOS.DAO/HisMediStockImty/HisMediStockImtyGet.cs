...
{
    partial class HisMediStockImtyGet : EntityBase
    {
        public List<HIS_MEDI_STOCK_IMTY> Get(HisMediStockImtySO search, CommonParam param)
        {
            List<HIS_MEDI_STOCK_IMTY> list = new List<HIS_MEDI_STOCK_IMTY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MEDI_STOCK_IMTY.AsNoTracking().AsQueryable();
                        if (search.listHisMediStockImtyExpression != null && search.listHisMediStockImtyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMediStockImtyExpression)
                            {
        ...
        public HIS_MEDI_STOCK_IMTY GetById(long id, HisMediStockImtySO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_MEDI_STOCK_IMTY.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMediStockImtyExpression != null && search.listHisMediStockImtyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMediStockImtyExpression)
                            {
