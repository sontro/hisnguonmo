...
namespace MOS.DAO.HisMediStockImty
{
    partial class HisMediStockImtyGet : EntityBase
    {
        ...
        {
            List<V_HIS_MEDI_STOCK_IMTY> list = new List<V_HIS_MEDI_STOCK_IMTY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_MEDI_STOCK_IMTY.AsNoTracking().AsQueryable();
                        if (search.listVHisMediStockImtyExpression != null && search.listVHisMediStockImtyExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisMediStockImtyExpression)
                            {
