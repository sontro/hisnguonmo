...
namespace MOS.DAO.HisBltyService
{
    partial class HisBltyServiceGet : EntityBase
    {
        ...
        {
            List<V_HIS_BLTY_SERVICE> list = new List<V_HIS_BLTY_SERVICE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_BLTY_SERVICE.AsNoTracking().AsQueryable();
                        if (search.listVHisBltyServiceExpression != null && search.listVHisBltyServiceExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisBltyServiceExpression)
                            {
