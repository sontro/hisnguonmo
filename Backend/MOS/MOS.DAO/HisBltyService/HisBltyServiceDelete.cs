...
{
    partial class HisBltyServiceDelete : EntityBase
    {
        public HisBltyServiceDelete()
            : base()
        {
        ...
        public bool Delete(HIS_BLTY_SERVICE data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_BLTY_SERVICE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_BLTY_SERVICE> listIdNotFound = new List<HIS_BLTY_SERVICE>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_BLTY_SERVICE> listRaw = ctx.HIS_BLTY_SERVICE.Where(o => listId.Contains(o.ID)).ToList();
