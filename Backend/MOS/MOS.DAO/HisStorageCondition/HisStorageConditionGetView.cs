...
namespace MOS.DAO.HisStorageCondition
{
    partial class HisStorageConditionGet : EntityBase
    {
        ...
        {
            List<V_HIS_STORAGE_CONDITION> list = new List<V_HIS_STORAGE_CONDITION>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_STORAGE_CONDITION.AsNoTracking().AsQueryable();
                        if (search.listVHisStorageConditionExpression != null && search.listVHisStorageConditionExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisStorageConditionExpression)
                            {
