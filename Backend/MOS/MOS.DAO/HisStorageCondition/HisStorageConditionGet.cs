...
namespace MOS.DAO.HisStorageCondition
{
    partial class HisStorageConditionGet : EntityBase
    {
        ...
        {
            List<HIS_STORAGE_CONDITION> list = new List<HIS_STORAGE_CONDITION>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_STORAGE_CONDITION.AsNoTracking().AsQueryable();
        ...
        public HIS_STORAGE_CONDITION GetById(long id, HisStorageConditionSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_STORAGE_CONDITION.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisStorageConditionExpression != null && search.listHisStorageConditionExpression.Count > 0)
                        {
                            foreach (var item in search.listHisStorageConditionExpression)
                            {
