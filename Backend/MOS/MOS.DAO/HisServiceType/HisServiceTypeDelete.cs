...
{
    partial class HisServiceTypeDelete : EntityBase
    {
        public HisServiceTypeDelete()
            : base()
        {
        ...
        public bool Delete(HIS_SERVICE_TYPE data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_SERVICE_TYPE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_SERVICE_TYPE> listIdNotFound = new List<HIS_SERVICE_TYPE>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_SERVICE_TYPE> listRaw = ctx.HIS_SERVICE_TYPE.Where(o => listId.Contains(o.ID)).ToList();
