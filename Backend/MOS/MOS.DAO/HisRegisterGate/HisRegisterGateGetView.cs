...
namespace MOS.DAO.HisRegisterGate
{
    partial class HisRegisterGateGet : EntityBase
    {
        ...
        {
            List<V_HIS_REGISTER_GATE> list = new List<V_HIS_REGISTER_GATE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_REGISTER_GATE.AsQueryable();
                        if (search.listVHisRegisterGateExpression != null && search.listVHisRegisterGateExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisRegisterGateExpression)
                            {
