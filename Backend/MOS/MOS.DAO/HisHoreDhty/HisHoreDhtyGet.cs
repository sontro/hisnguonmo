...
namespace MOS.DAO.HisHoreDhty
{
    partial class HisHoreDhtyGet : EntityBase
    {
        ...
        {
            List<HIS_HORE_DHTY> list = new List<HIS_HORE_DHTY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_HORE_DHTY.AsNoTracking().AsQueryable();
        ...
        public HIS_HORE_DHTY GetById(long id, HisHoreDhtySO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_HORE_DHTY.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisHoreDhtyExpression != null && search.listHisHoreDhtyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisHoreDhtyExpression)
                            {
