...
namespace MOS.DAO.HisAcinInteractive
{
    partial class HisAcinInteractiveGet : EntityBase
    {
        ...
        {
            List<HIS_ACIN_INTERACTIVE> list = new List<HIS_ACIN_INTERACTIVE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_ACIN_INTERACTIVE.AsNoTracking().AsQueryable();
                        if (search.listHisAcinInteractiveExpression != null && search.listHisAcinInteractiveExpression.Count > 0)
                        {
                            foreach (var item in search.listHisAcinInteractiveExpression)
                            {
        ...
        public HIS_ACIN_INTERACTIVE GetById(long id, HisAcinInteractiveSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_ACIN_INTERACTIVE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisAcinInteractiveExpression != null && search.listHisAcinInteractiveExpression.Count > 0)
                        {
                            foreach (var item in search.listHisAcinInteractiveExpression)
                            {
