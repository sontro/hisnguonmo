...
namespace MOS.DAO.HisRejectAlert
{
    partial class HisRejectAlertGet : EntityBase
    {
        ...
        {
            List<V_HIS_REJECT_ALERT> list = new List<V_HIS_REJECT_ALERT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_REJECT_ALERT.AsNoTracking().AsQueryable();
                        if (search.listVHisRejectAlertExpression != null && search.listVHisRejectAlertExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisRejectAlertExpression)
                            {
