...
{
    partial class HisRejectAlertGet : EntityBase
    {
        public List<HIS_REJECT_ALERT> Get(HisRejectAlertSO search, CommonParam param)
        {
            List<HIS_REJECT_ALERT> list = new List<HIS_REJECT_ALERT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_REJECT_ALERT.AsNoTracking().AsQueryable();
                        if (search.listHisRejectAlertExpression != null && search.listHisRejectAlertExpression.Count > 0)
                        {
                            foreach (var item in search.listHisRejectAlertExpression)
                            {
        ...
        public HIS_REJECT_ALERT GetById(long id, HisRejectAlertSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_REJECT_ALERT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisRejectAlertExpression != null && search.listHisRejectAlertExpression.Count > 0)
                        {
                            foreach (var item in search.listHisRejectAlertExpression)
                            {
