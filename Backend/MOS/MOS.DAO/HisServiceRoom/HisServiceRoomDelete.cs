...
{
    partial class HisServiceRoomDelete : EntityBase
    {
        public HisServiceRoomDelete()
            : base()
        {
        ...
        public bool Delete(HIS_SERVICE_ROOM data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_SERVICE_ROOM> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_SERVICE_ROOM> listIdNotFound = new List<HIS_SERVICE_ROOM>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_SERVICE_ROOM> listRaw = ctx.HIS_SERVICE_ROOM.Where(o => listId.Contains(o.ID)).ToList();
