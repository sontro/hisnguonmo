...
{
    partial class HisEkipTempUserGet : EntityBase
    {
        public List<HIS_EKIP_TEMP_USER> Get(HisEkipTempUserSO search, CommonParam param)
        {
            List<HIS_EKIP_TEMP_USER> list = new List<HIS_EKIP_TEMP_USER>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_EKIP_TEMP_USER.AsNoTracking().AsQueryable();
                        if (search.listHisEkipTempUserExpression != null && search.listHisEkipTempUserExpression.Count > 0)
                        {
                            foreach (var item in search.listHisEkipTempUserExpression)
                            {
        ...
        public HIS_EKIP_TEMP_USER GetById(long id, HisEkipTempUserSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_EKIP_TEMP_USER.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisEkipTempUserExpression != null && search.listHisEkipTempUserExpression.Count > 0)
                        {
                            foreach (var item in search.listHisEkipTempUserExpression)
                            {
