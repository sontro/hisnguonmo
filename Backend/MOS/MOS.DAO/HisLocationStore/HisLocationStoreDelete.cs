...
{
    partial class HisLocationStoreDelete : EntityBase
    {
        public HisLocationStoreDelete()
            : base()
        {
        ...
        public bool Delete(HIS_LOCATION_STORE data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_LOCATION_STORE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_LOCATION_STORE> listIdNotFound = new List<HIS_LOCATION_STORE>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_LOCATION_STORE> listRaw = ctx.HIS_LOCATION_STORE.Where(o => listId.Contains(o.ID)).ToList();
