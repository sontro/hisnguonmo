...
namespace MOS.DAO.HisLocationStore
{
    ...
    {
        public List<HIS_LOCATION_STORE> Get(HisLocationStoreSO search, CommonParam param)
        {
            List<HIS_LOCATION_STORE> list = new List<HIS_LOCATION_STORE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_LOCATION_STORE.AsNoTracking().AsQueryable();
                        if (search.listHisLocationStoreExpression != null && search.listHisLocationStoreExpression.Count > 0)
                        {
                            foreach (var item in search.listHisLocationStoreExpression)
                            {
        ...
        public HIS_LOCATION_STORE GetById(long id, HisLocationStoreSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_LOCATION_STORE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisLocationStoreExpression != null && search.listHisLocationStoreExpression.Count > 0)
                        {
                            foreach (var item in search.listHisLocationStoreExpression)
                            {
