...
namespace MOS.DAO.HisDocHoldType
{
    partial class HisDocHoldTypeGet : EntityBase
    {
        ...
        {
            List<HIS_DOC_HOLD_TYPE> list = new List<HIS_DOC_HOLD_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_DOC_HOLD_TYPE.AsNoTracking().AsQueryable();
        ...
        public HIS_DOC_HOLD_TYPE GetById(long id, HisDocHoldTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_DOC_HOLD_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisDocHoldTypeExpression != null && search.listHisDocHoldTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisDocHoldTypeExpression)
                            {
