...
{
    partial class HisKskAccessDelete : EntityBase
    {
        public HisKskAccessDelete()
            : base()
        {
        ...
        public bool Delete(HIS_KSK_ACCESS data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_KSK_ACCESS> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_KSK_ACCESS> listIdNotFound = new List<HIS_KSK_ACCESS>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_KSK_ACCESS> listRaw = ctx.HIS_KSK_ACCESS.Where(o => listId.Contains(o.ID)).ToList();
