...
namespace MOS.DAO.HisKskAccess
{
    ...
    {
        public List<HIS_KSK_ACCESS> Get(HisKskAccessSO search, CommonParam param)
        {
            List<HIS_KSK_ACCESS> list = new List<HIS_KSK_ACCESS>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_KSK_ACCESS.AsNoTracking().AsQueryable();
                        if (search.listHisKskAccessExpression != null && search.listHisKskAccessExpression.Count > 0)
                        {
                            foreach (var item in search.listHisKskAccessExpression)
                            {
        ...
        public HIS_KSK_ACCESS GetById(long id, HisKskAccessSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_KSK_ACCESS.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisKskAccessExpression != null && search.listHisKskAccessExpression.Count > 0)
                        {
                            foreach (var item in search.listHisKskAccessExpression)
                            {
