...
namespace MOS.DAO.HisMestPeriodBlood
{
    partial class HisMestPeriodBloodGet : EntityBase
    {
        ...
        {
            List<HIS_MEST_PERIOD_BLOOD> list = new List<HIS_MEST_PERIOD_BLOOD>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MEST_PERIOD_BLOOD.AsNoTracking().AsQueryable();
        ...
        public HIS_MEST_PERIOD_BLOOD GetById(long id, HisMestPeriodBloodSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_MEST_PERIOD_BLOOD.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMestPeriodBloodExpression != null && search.listHisMestPeriodBloodExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMestPeriodBloodExpression)
                            {
