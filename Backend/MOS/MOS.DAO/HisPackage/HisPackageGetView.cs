...
namespace MOS.DAO.HisPackage
{
    partial class HisPackageGet : EntityBase
    {
        ...
        {
            List<V_HIS_PACKAGE> list = new List<V_HIS_PACKAGE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_PACKAGE.AsQueryable();
                        if (search.listVHisPackageExpression != null && search.listVHisPackageExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisPackageExpression)
                            {
