...
{
    partial class HisPackageDelete : EntityBase
    {
        public HisPackageDelete()
            : base()
        {
        ...
        public bool Delete(HIS_PACKAGE data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_PACKAGE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_PACKAGE> listIdNotFound = new List<HIS_PACKAGE>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_PACKAGE> listRaw = ctx.HIS_PACKAGE.Where(o => listId.Contains(o.ID)).ToList();
