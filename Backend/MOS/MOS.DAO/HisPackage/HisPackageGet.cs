...
{
    partial class HisPackageGet : EntityBase
    {
        public List<HIS_PACKAGE> Get(HisPackageSO search, CommonParam param)
        {
            List<HIS_PACKAGE> list = new List<HIS_PACKAGE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_PACKAGE.AsNoTracking().AsQueryable();
                        if (search.listHisPackageExpression != null && search.listHisPackageExpression.Count > 0)
                        {
                            foreach (var item in search.listHisPackageExpression)
                            {
        ...
        public HIS_PACKAGE GetById(long id, HisPackageSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_PACKAGE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisPackageExpression != null && search.listHisPackageExpression.Count > 0)
                        {
                            foreach (var item in search.listHisPackageExpression)
                            {
