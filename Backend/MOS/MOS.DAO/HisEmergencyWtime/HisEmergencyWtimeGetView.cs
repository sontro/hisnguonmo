...
namespace MOS.DAO.HisEmergencyWtime
{
    partial class HisEmergencyWtimeGet : EntityBase
    {
        ...
        {
            List<V_HIS_EMERGENCY_WTIME> list = new List<V_HIS_EMERGENCY_WTIME>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_EMERGENCY_WTIME.AsQueryable();
                        if (search.listVHisEmergencyWtimeExpression != null && search.listVHisEmergencyWtimeExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisEmergencyWtimeExpression)
                            {
