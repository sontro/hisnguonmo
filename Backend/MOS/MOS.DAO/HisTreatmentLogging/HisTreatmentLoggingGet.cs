...
namespace MOS.DAO.HisTreatmentLogging
{
    partial class HisTreatmentLoggingGet : EntityBase
    {
        ...
        {
            List<HIS_TREATMENT_LOGGING> list = new List<HIS_TREATMENT_LOGGING>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_TREATMENT_LOGGING.AsNoTracking().AsQueryable();
        ...
        public HIS_TREATMENT_LOGGING GetById(long id, HisTreatmentLoggingSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_TREATMENT_LOGGING.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisTreatmentLoggingExpression != null && search.listHisTreatmentLoggingExpression.Count > 0)
                        {
                            foreach (var item in search.listHisTreatmentLoggingExpression)
                            {
