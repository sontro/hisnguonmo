...
namespace MOS.DAO.HisTreatmentLogging
{
    partial class HisTreatmentLoggingGet : EntityBase
    {
        ...
        {
            List<V_HIS_TREATMENT_LOGGING> list = new List<V_HIS_TREATMENT_LOGGING>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_TREATMENT_LOGGING.AsNoTracking().AsQueryable();
                        if (search.listVHisTreatmentLoggingExpression != null && search.listVHisTreatmentLoggingExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisTreatmentLoggingExpression)
                            {
