...
{
    partial class HisRationTimeDelete : EntityBase
    {
        public HisRationTimeDelete()
            : base()
        {
        ...
        public bool Delete(HIS_RATION_TIME data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_RATION_TIME> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_RATION_TIME> listIdNotFound = new List<HIS_RATION_TIME>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_RATION_TIME> listRaw = ctx.HIS_RATION_TIME.Where(o => listId.Contains(o.ID)).ToList();
