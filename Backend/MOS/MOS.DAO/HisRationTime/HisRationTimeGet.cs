...
namespace MOS.DAO.HisRationTime
{
    ...
    {
        public List<HIS_RATION_TIME> Get(HisRationTimeSO search, CommonParam param)
        {
            List<HIS_RATION_TIME> list = new List<HIS_RATION_TIME>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_RATION_TIME.AsNoTracking().AsQueryable();
                        if (search.listHisRationTimeExpression != null && search.listHisRationTimeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisRationTimeExpression)
                            {
        ...
        public HIS_RATION_TIME GetById(long id, HisRationTimeSO search)
        {
            HIS_RATION_TIME result = null;
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_RATION_TIME.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisRationTimeExpression != null && search.listHisRationTimeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisRationTimeExpression)
                            {
