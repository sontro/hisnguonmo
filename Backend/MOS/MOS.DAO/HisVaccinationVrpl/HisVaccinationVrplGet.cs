...
{
    partial class HisVaccinationVrplGet : EntityBase
    {
        public List<HIS_VACCINATION_VRPL> Get(HisVaccinationVrplSO search, CommonParam param)
        {
            List<HIS_VACCINATION_VRPL> list = new List<HIS_VACCINATION_VRPL>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_VACCINATION_VRPL.AsNoTracking().AsQueryable();
                        if (search.listHisVaccinationVrplExpression != null && search.listHisVaccinationVrplExpression.Count > 0)
                        {
                            foreach (var item in search.listHisVaccinationVrplExpression)
                            {
        ...
        public HIS_VACCINATION_VRPL GetById(long id, HisVaccinationVrplSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_VACCINATION_VRPL.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisVaccinationVrplExpression != null && search.listHisVaccinationVrplExpression.Count > 0)
                        {
                            foreach (var item in search.listHisVaccinationVrplExpression)
                            {
