...
namespace MOS.DAO.HisRationSchedule
{
    partial class HisRationScheduleGet : EntityBase
    {
        ...
        {
            List<HIS_RATION_SCHEDULE> list = new List<HIS_RATION_SCHEDULE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_RATION_SCHEDULE.AsNoTracking().AsQueryable();
        ...
        public HIS_RATION_SCHEDULE GetById(long id, HisRationScheduleSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_RATION_SCHEDULE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisRationScheduleExpression != null && search.listHisRationScheduleExpression.Count > 0)
                        {
                            foreach (var item in search.listHisRationScheduleExpression)
                            {
