...
namespace MOS.DAO.HisSeseDepoRepay
{
    partial class HisSeseDepoRepayGet : EntityBase
    {
        ...
        {
            List<V_HIS_SESE_DEPO_REPAY> list = new List<V_HIS_SESE_DEPO_REPAY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_SESE_DEPO_REPAY.AsQueryable();
                        if (search.listVHisSeseDepoRepayExpression != null && search.listVHisSeseDepoRepayExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisSeseDepoRepayExpression)
                            {
