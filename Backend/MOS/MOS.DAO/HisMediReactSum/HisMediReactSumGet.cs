...
namespace MOS.DAO.HisMediReactSum
{
    partial class HisMediReactSumGet : EntityBase
    {
        ...
        {
            List<HIS_MEDI_REACT_SUM> list = new List<HIS_MEDI_REACT_SUM>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MEDI_REACT_SUM.AsNoTracking().AsQueryable();
                        if (search.listHisMediReactSumExpression != null && search.listHisMediReactSumExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMediReactSumExpression)
                            {
        ...
        public HIS_MEDI_REACT_SUM GetById(long id, HisMediReactSumSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_MEDI_REACT_SUM.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMediReactSumExpression != null && search.listHisMediReactSumExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMediReactSumExpression)
                            {
