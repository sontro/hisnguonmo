...
{
    partial class HisAwarenessDelete : EntityBase
    {
        public HisAwarenessDelete()
            : base()
        {
        ...
        public bool Delete(HIS_AWARENESS data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_AWARENESS> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_AWARENESS> listIdNotFound = new List<HIS_AWARENESS>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_AWARENESS> listRaw = ctx.HIS_AWARENESS.Where(o => listId.Contains(o.ID)).ToList();
