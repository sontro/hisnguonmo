...
namespace MOS.DAO.HisImpMestStt
{
    partial class HisImpMestSttGet : EntityBase
    {
        ...
        {
            List<V_HIS_IMP_MEST_STT> list = new List<V_HIS_IMP_MEST_STT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_IMP_MEST_STT.AsQueryable();
                        if (search.listVHisImpMestSttExpression != null && search.listVHisImpMestSttExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisImpMestSttExpression)
                            {
