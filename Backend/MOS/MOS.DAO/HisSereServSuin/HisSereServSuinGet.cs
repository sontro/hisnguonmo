...
namespace MOS.DAO.HisSereServSuin
{
    partial class HisSereServSuinGet : EntityBase
    {
        ...
        {
            List<HIS_SERE_SERV_SUIN> list = new List<HIS_SERE_SERV_SUIN>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SERE_SERV_SUIN.AsNoTracking().AsQueryable();
        ...
        public HIS_SERE_SERV_SUIN GetById(long id, HisSereServSuinSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_SERE_SERV_SUIN.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisSereServSuinExpression != null && search.listHisSereServSuinExpression.Count > 0)
                        {
                            foreach (var item in search.listHisSereServSuinExpression)
                            {
