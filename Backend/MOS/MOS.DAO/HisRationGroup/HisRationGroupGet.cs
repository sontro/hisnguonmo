...
namespace MOS.DAO.HisRationGroup
{
    partial class HisRationGroupGet : EntityBase
    {
        ...
        {
            List<HIS_RATION_GROUP> list = new List<HIS_RATION_GROUP>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_RATION_GROUP.AsNoTracking().AsQueryable();
                        if (search.listHisRationGroupExpression != null && search.listHisRationGroupExpression.Count > 0)
                        {
                            foreach (var item in search.listHisRationGroupExpression)
                            {
        ...
        public HIS_RATION_GROUP GetById(long id, HisRationGroupSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_RATION_GROUP.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisRationGroupExpression != null && search.listHisRationGroupExpression.Count > 0)
                        {
                            foreach (var item in search.listHisRationGroupExpression)
                            {
