...
{
    partial class HisRoomDelete : EntityBase
    {
        public HisRoomDelete()
            : base()
        {
        ...
        public bool Delete(HIS_ROOM data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_ROOM> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_ROOM> listIdNotFound = new List<HIS_ROOM>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_ROOM> listRaw = ctx.HIS_ROOM.Where(o => listId.Contains(o.ID)).ToList();
