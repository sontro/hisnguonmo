...
namespace MOS.DAO.HisRoom
{
    partial class HisRoomGet : EntityBase
    {
        ...
        {
            List<HIS_ROOM> list = new List<HIS_ROOM>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_ROOM.AsNoTracking().AsQueryable();
                        if (search.listHisRoomExpression != null && search.listHisRoomExpression.Count > 0)
                        {
                            foreach (var item in search.listHisRoomExpression)
                            {
        ...
        public HIS_ROOM GetById(long id, HisRoomSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_ROOM.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisRoomExpression != null && search.listHisRoomExpression.Count > 0)
                        {
                            foreach (var item in search.listHisRoomExpression)
                            {
