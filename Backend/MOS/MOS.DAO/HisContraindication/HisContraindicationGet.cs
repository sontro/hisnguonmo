...
namespace MOS.DAO.HisContraindication
{
    partial class HisContraindicationGet : EntityBase
    {
        ...
        {
            List<HIS_CONTRAINDICATION> list = new List<HIS_CONTRAINDICATION>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_CONTRAINDICATION.AsNoTracking().AsQueryable();
        ...
        public HIS_CONTRAINDICATION GetById(long id, HisContraindicationSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_CONTRAINDICATION.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisContraindicationExpression != null && search.listHisContraindicationExpression.Count > 0)
                        {
                            foreach (var item in search.listHisContraindicationExpression)
                            {
