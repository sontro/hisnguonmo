...
namespace MOS.DAO.HisCarerCard
{
    partial class HisCarerCardGet : EntityBase
    {
        ...
        {
            List<HIS_CARER_CARD> list = new List<HIS_CARER_CARD>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_CARER_CARD.AsNoTracking().AsQueryable();
        ...
        public HIS_CARER_CARD GetById(long id, HisCarerCardSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_CARER_CARD.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisCarerCardExpression != null && search.listHisCarerCardExpression.Count > 0)
                        {
                            foreach (var item in search.listHisCarerCardExpression)
                            {
