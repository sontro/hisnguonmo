...
namespace MOS.DAO.HisCarerCard
{
    partial class HisCarerCardGet : EntityBase
    {
        ...
        {
            List<V_HIS_CARER_CARD> list = new List<V_HIS_CARER_CARD>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_CARER_CARD.AsNoTracking().AsQueryable();
                        if (search.listVHisCarerCardExpression != null && search.listVHisCarerCardExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisCarerCardExpression)
                            {
