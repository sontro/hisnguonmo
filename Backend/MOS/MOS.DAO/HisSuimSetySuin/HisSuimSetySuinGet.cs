...
namespace MOS.DAO.HisSuimSetySuin
{
    partial class HisSuimSetySuinGet : EntityBase
    {
        ...
        {
            List<HIS_SUIM_SETY_SUIN> list = new List<HIS_SUIM_SETY_SUIN>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SUIM_SETY_SUIN.AsNoTracking().AsQueryable();
        ...
        public HIS_SUIM_SETY_SUIN GetById(long id, HisSuimSetySuinSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_SUIM_SETY_SUIN.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisSuimSetySuinExpression != null && search.listHisSuimSetySuinExpression.Count > 0)
                        {
                            foreach (var item in search.listHisSuimSetySuinExpression)
                            {
