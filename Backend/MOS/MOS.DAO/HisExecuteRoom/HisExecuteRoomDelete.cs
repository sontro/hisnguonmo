...
{
    partial class HisExecuteRoomDelete : EntityBase
    {
        public HisExecuteRoomDelete()
            : base()
        {
        ...
        public bool Delete(HIS_EXECUTE_ROOM data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_EXECUTE_ROOM> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_EXECUTE_ROOM> listIdNotFound = new List<HIS_EXECUTE_ROOM>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_EXECUTE_ROOM> listRaw = ctx.HIS_EXECUTE_ROOM.Where(o => listId.Contains(o.ID)).ToList();
