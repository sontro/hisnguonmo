...
namespace MOS.DAO.HisInvoiceBook
{
    partial class HisInvoiceBookGet : EntityBase
    {
        ...
        {
            List<HIS_INVOICE_BOOK> list = new List<HIS_INVOICE_BOOK>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_INVOICE_BOOK.AsNoTracking().AsQueryable();
                        if (search.listHisInvoiceBookExpression != null && search.listHisInvoiceBookExpression.Count > 0)
                        {
                            foreach (var item in search.listHisInvoiceBookExpression)
                            {
        ...
        public HIS_INVOICE_BOOK GetById(long id, HisInvoiceBookSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_INVOICE_BOOK.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisInvoiceBookExpression != null && search.listHisInvoiceBookExpression.Count > 0)
                        {
                            foreach (var item in search.listHisInvoiceBookExpression)
                            {
