...
namespace MOS.DAO.HisPtttCondition
{
    partial class HisPtttConditionGet : EntityBase
    {
        ...
        {
            List<HIS_PTTT_CONDITION> list = new List<HIS_PTTT_CONDITION>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_PTTT_CONDITION.AsNoTracking().AsQueryable();
        ...
        public HIS_PTTT_CONDITION GetById(long id, HisPtttConditionSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_PTTT_CONDITION.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisPtttConditionExpression != null && search.listHisPtttConditionExpression.Count > 0)
                        {
                            foreach (var item in search.listHisPtttConditionExpression)
                            {
