...
namespace MOS.DAO.HisSereServFile
{
    partial class HisSereServFileGet : EntityBase
    {
        ...
        {
            List<HIS_SERE_SERV_FILE> list = new List<HIS_SERE_SERV_FILE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SERE_SERV_FILE.AsNoTracking().AsQueryable();
        ...
        public HIS_SERE_SERV_FILE GetById(long id, HisSereServFileSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_SERE_SERV_FILE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisSereServFileExpression != null && search.listHisSereServFileExpression.Count > 0)
                        {
                            foreach (var item in search.listHisSereServFileExpression)
                            {
