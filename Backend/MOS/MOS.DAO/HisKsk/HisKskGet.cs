...
{
    partial class HisKskGet : EntityBase
    {
        public List<HIS_KSK> Get(HisKskSO search, CommonParam param)
        {
            List<HIS_KSK> list = new List<HIS_KSK>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_KSK.AsNoTracking().AsQueryable();
                        if (search.listHisKskExpression != null && search.listHisKskExpression.Count > 0)
                        {
                            foreach (var item in search.listHisKskExpression)
                            {
        ...
        public HIS_KSK GetById(long id, HisKskSO search)
        {
            HIS_KSK result = null;
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_KSK.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisKskExpression != null && search.listHisKskExpression.Count > 0)
                        {
                            foreach (var item in search.listHisKskExpression)
                            {
