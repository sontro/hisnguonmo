...
{
    partial class HisKskDelete : EntityBase
    {
        public HisKskDelete()
            : base()
        {
        ...
        public bool Delete(HIS_KSK data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_KSK> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_KSK> listIdNotFound = new List<HIS_KSK>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_KSK> listRaw = ctx.HIS_KSK.Where(o => listId.Contains(o.ID)).ToList();
