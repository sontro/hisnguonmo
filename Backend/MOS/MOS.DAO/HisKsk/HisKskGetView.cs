...
namespace MOS.DAO.HisKsk
{
    partial class HisKskGet : EntityBase
    {
        ...
        {
            List<V_HIS_KSK> list = new List<V_HIS_KSK>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_KSK.AsNoTracking().AsQueryable();
                        if (search.listVHisKskExpression != null && search.listVHisKskExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisKskExpression)
                            {
