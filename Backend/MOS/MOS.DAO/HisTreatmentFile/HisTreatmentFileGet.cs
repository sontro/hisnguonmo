...
{
    partial class HisTreatmentFileGet : EntityBase
    {
        public List<HIS_TREATMENT_FILE> Get(HisTreatmentFileSO search, CommonParam param)
        {
            List<HIS_TREATMENT_FILE> list = new List<HIS_TREATMENT_FILE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_TREATMENT_FILE.AsNoTracking().AsQueryable();
                        if (search.listHisTreatmentFileExpression != null && search.listHisTreatmentFileExpression.Count > 0)
                        {
                            foreach (var item in search.listHisTreatmentFileExpression)
                            {
        ...
        public HIS_TREATMENT_FILE GetById(long id, HisTreatmentFileSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_TREATMENT_FILE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisTreatmentFileExpression != null && search.listHisTreatmentFileExpression.Count > 0)
                        {
                            foreach (var item in search.listHisTreatmentFileExpression)
                            {
