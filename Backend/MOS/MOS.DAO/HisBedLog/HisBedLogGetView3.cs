...
namespace MOS.DAO.HisBedLog
{
    partial class HisBedLogGet : EntityBase
    {
        ...
        {
            List<V_HIS_BED_LOG_3> list = new List<V_HIS_BED_LOG_3>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_BED_LOG_3.AsNoTracking().AsQueryable();
                        if (search.listVHisBedLog3Expression != null && search.listVHisBedLog3Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisBedLog3Expression)
                            {
