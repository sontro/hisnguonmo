...
namespace MOS.DAO.HisBedLog
{
    partial class HisBedLogGet : EntityBase
    {
        ...
        {
            List<V_HIS_BED_LOG_4> list = new List<V_HIS_BED_LOG_4>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_BED_LOG_4.AsNoTracking().AsQueryable();
                        if (search.listVHisBedLog4Expression != null && search.listVHisBedLog4Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisBedLog4Expression)
                            {
