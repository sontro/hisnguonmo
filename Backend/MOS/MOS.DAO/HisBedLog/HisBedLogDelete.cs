...
{
    partial class HisBedLogDelete : EntityBase
    {
        public HisBedLogDelete()
            : base()
        {
        ...
        public bool Delete(HIS_BED_LOG data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_BED_LOG> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_BED_LOG> listIdNotFound = new List<HIS_BED_LOG>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_BED_LOG> listRaw = ctx.HIS_BED_LOG.Where(o => listId.Contains(o.ID)).ToList();
