...
{
    partial class HisPatientCaseDelete : EntityBase
    {
        public HisPatientCaseDelete()
            : base()
        {
        ...
        public bool Delete(HIS_PATIENT_CASE data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_PATIENT_CASE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_PATIENT_CASE> listIdNotFound = new List<HIS_PATIENT_CASE>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_PATIENT_CASE> listRaw = ctx.HIS_PATIENT_CASE.Where(o => listId.Contains(o.ID)).ToList();
