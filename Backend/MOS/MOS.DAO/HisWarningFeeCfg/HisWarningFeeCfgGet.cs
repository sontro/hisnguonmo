...
namespace MOS.DAO.HisWarningFeeCfg
{
    partial class HisWarningFeeCfgGet : EntityBase
    {
        ...
        {
            List<HIS_WARNING_FEE_CFG> list = new List<HIS_WARNING_FEE_CFG>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_WARNING_FEE_CFG.AsNoTracking().AsQueryable();
        ...
        public HIS_WARNING_FEE_CFG GetById(long id, HisWarningFeeCfgSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_WARNING_FEE_CFG.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisWarningFeeCfgExpression != null && search.listHisWarningFeeCfgExpression.Count > 0)
                        {
                            foreach (var item in search.listHisWarningFeeCfgExpression)
                            {
