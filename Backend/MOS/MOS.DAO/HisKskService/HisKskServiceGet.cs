...
namespace MOS.DAO.HisKskService
{
    partial class HisKskServiceGet : EntityBase
    {
        ...
        {
            List<HIS_KSK_SERVICE> list = new List<HIS_KSK_SERVICE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_KSK_SERVICE.AsNoTracking().AsQueryable();
                        if (search.listHisKskServiceExpression != null && search.listHisKskServiceExpression.Count > 0)
                        {
                            foreach (var item in search.listHisKskServiceExpression)
                            {
        ...
        public HIS_KSK_SERVICE GetById(long id, HisKskServiceSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_KSK_SERVICE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisKskServiceExpression != null && search.listHisKskServiceExpression.Count > 0)
                        {
                            foreach (var item in search.listHisKskServiceExpression)
                            {
