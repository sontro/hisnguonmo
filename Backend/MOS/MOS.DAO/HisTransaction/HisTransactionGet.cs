        ...
        public List<HIS_TRANSACTION> Get(HisTransactionSO search, CommonParam param)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_TRANSACTION.AsNoTracking().AsQueryable();
                        if (search.listHisTransactionExpression != null && search.listHisTransactionExpression.Count > 0)
                        {
                            foreach (var item in search.listHisTransactionExpression)
                            {
        ...
        public HIS_TRANSACTION GetById(long id, HisTransactionSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_TRANSACTION.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisTransactionExpression != null && search.listHisTransactionExpression.Count > 0)
                        {
                            foreach (var item in search.listHisTransactionExpression)
                            {
