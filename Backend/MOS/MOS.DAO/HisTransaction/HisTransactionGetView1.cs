...
namespace MOS.DAO.HisTransaction
{
    partial class HisTransactionGet : EntityBase
    {
        ...
        {
            List<V_HIS_TRANSACTION_1> list = new List<V_HIS_TRANSACTION_1>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_TRANSACTION_1.AsNoTracking().AsQueryable();
                        if (search.listVHisTransaction1Expression != null && search.listVHisTransaction1Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisTransaction1Expression)
                            {
