...
namespace MOS.DAO.HisFormTypeCfg
{
    partial class HisFormTypeCfgGet : EntityBase
    {
        ...
        {
            List<HIS_FORM_TYPE_CFG> list = new List<HIS_FORM_TYPE_CFG>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_FORM_TYPE_CFG.AsNoTracking().AsQueryable();
        ...
        public HIS_FORM_TYPE_CFG GetById(long id, HisFormTypeCfgSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_FORM_TYPE_CFG.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisFormTypeCfgExpression != null && search.listHisFormTypeCfgExpression.Count > 0)
                        {
                            foreach (var item in search.listHisFormTypeCfgExpression)
                            {
