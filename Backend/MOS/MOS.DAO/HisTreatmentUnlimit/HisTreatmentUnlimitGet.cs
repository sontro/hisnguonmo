...
namespace MOS.DAO.HisTreatmentUnlimit
{
    partial class HisTreatmentUnlimitGet : EntityBase
    {
        ...
        {
            List<HIS_TREATMENT_UNLIMIT> list = new List<HIS_TREATMENT_UNLIMIT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_TREATMENT_UNLIMIT.AsNoTracking().AsQueryable();
                        if (search.listHisTreatmentUnlimitExpression != null && search.listHisTreatmentUnlimitExpression.Count > 0)
                        {
                            foreach (var item in search.listHisTreatmentUnlimitExpression)
                            {
        ...
        public HIS_TREATMENT_UNLIMIT GetById(long id, HisTreatmentUnlimitSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_TREATMENT_UNLIMIT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisTreatmentUnlimitExpression != null && search.listHisTreatmentUnlimitExpression.Count > 0)
                        {
                            foreach (var item in search.listHisTreatmentUnlimitExpression)
                            {
