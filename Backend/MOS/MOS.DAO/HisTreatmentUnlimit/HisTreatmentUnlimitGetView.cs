...
{
    partial class HisTreatmentUnlimitGet : EntityBase
    {
        public List<V_HIS_TREATMENT_UNLIMIT> GetView(HisTreatmentUnlimitSO search, CommonParam param)
        {
            List<V_HIS_TREATMENT_UNLIMIT> list = new List<V_HIS_TREATMENT_UNLIMIT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_TREATMENT_UNLIMIT.AsNoTracking().AsQueryable();
                        if (search.listVHisTreatmentUnlimitExpression != null && search.listVHisTreatmentUnlimitExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisTreatmentUnlimitExpression)
                            {
