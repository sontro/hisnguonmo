...
{
    partial class HisCareSumDelete : EntityBase
    {
        public HisCareSumDelete()
            : base()
        {
        ...
        public bool Delete(HIS_CARE_SUM data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_CARE_SUM> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_CARE_SUM> listIdNotFound = new List<HIS_CARE_SUM>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_CARE_SUM> listRaw = ctx.HIS_CARE_SUM.Where(o => listId.Contains(o.ID)).ToList();
