...
{
    partial class HisCareSumGet : EntityBase
    {
        public List<HIS_CARE_SUM> Get(HisCareSumSO search, CommonParam param)
        {
            List<HIS_CARE_SUM> list = new List<HIS_CARE_SUM>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_CARE_SUM.AsNoTracking().AsQueryable();
                        if (search.listHisCareSumExpression != null && search.listHisCareSumExpression.Count > 0)
                        {
                            foreach (var item in search.listHisCareSumExpression)
                            {
        ...
        public HIS_CARE_SUM GetById(long id, HisCareSumSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_CARE_SUM.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisCareSumExpression != null && search.listHisCareSumExpression.Count > 0)
                        {
                            foreach (var item in search.listHisCareSumExpression)
                            {
