...
namespace MOS.DAO.HisAppointmentPeriod
{
    partial class HisAppointmentPeriodGet : EntityBase
    {
        ...
        {
            List<HIS_APPOINTMENT_PERIOD> list = new List<HIS_APPOINTMENT_PERIOD>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_APPOINTMENT_PERIOD.AsNoTracking().AsQueryable();
                        if (search.listHisAppointmentPeriodExpression != null && search.listHisAppointmentPeriodExpression.Count > 0)
                        {
                            foreach (var item in search.listHisAppointmentPeriodExpression)
                            {
        ...
        public HIS_APPOINTMENT_PERIOD GetById(long id, HisAppointmentPeriodSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_APPOINTMENT_PERIOD.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisAppointmentPeriodExpression != null && search.listHisAppointmentPeriodExpression.Count > 0)
                        {
                            foreach (var item in search.listHisAppointmentPeriodExpression)
                            {
