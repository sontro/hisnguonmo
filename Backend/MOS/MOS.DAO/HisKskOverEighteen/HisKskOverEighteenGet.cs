...
{
    partial class HisKskOverEighteenGet : EntityBase
    {
        public List<HIS_KSK_OVER_EIGHTEEN> Get(HisKskOverEighteenSO search, CommonParam param)
        {
            List<HIS_KSK_OVER_EIGHTEEN> list = new List<HIS_KSK_OVER_EIGHTEEN>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_KSK_OVER_EIGHTEEN.AsNoTracking().AsQueryable();
                        if (search.listHisKskOverEighteenExpression != null && search.listHisKskOverEighteenExpression.Count > 0)
                        {
                            foreach (var item in search.listHisKskOverEighteenExpression)
                            {
        ...
        public HIS_KSK_OVER_EIGHTEEN GetById(long id, HisKskOverEighteenSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_KSK_OVER_EIGHTEEN.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisKskOverEighteenExpression != null && search.listHisKskOverEighteenExpression.Count > 0)
                        {
                            foreach (var item in search.listHisKskOverEighteenExpression)
                            {
