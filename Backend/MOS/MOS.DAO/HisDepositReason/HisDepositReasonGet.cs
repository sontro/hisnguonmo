...
namespace MOS.DAO.HisDepositReason
{
    partial class HisDepositReasonGet : EntityBase
    {
        ...
        {
            List<HIS_DEPOSIT_REASON> list = new List<HIS_DEPOSIT_REASON>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_DEPOSIT_REASON.AsNoTracking().AsQueryable();
        ...
        public HIS_DEPOSIT_REASON GetById(long id, HisDepositReasonSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_DEPOSIT_REASON.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisDepositReasonExpression != null && search.listHisDepositReasonExpression.Count > 0)
                        {
                            foreach (var item in search.listHisDepositReasonExpression)
                            {
