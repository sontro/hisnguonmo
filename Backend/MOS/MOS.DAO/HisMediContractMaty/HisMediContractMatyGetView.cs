...
namespace MOS.DAO.HisMediContractMaty
{
    partial class HisMediContractMatyGet : EntityBase
    {
        ...
        {
            List<V_HIS_MEDI_CONTRACT_MATY> list = new List<V_HIS_MEDI_CONTRACT_MATY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_MEDI_CONTRACT_MATY.AsNoTracking().AsQueryable();
                        if (search.listVHisMediContractMatyExpression != null && search.listVHisMediContractMatyExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisMediContractMatyExpression)
                            {
