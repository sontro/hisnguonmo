...
namespace MOS.DAO.HisMediContractMaty
{
    partial class HisMediContractMatyGet : EntityBase
    {
        ...
        {
            List<HIS_MEDI_CONTRACT_MATY> list = new List<HIS_MEDI_CONTRACT_MATY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MEDI_CONTRACT_MATY.AsNoTracking().AsQueryable();
                        if (search.listHisMediContractMatyExpression != null && search.listHisMediContractMatyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMediContractMatyExpression)
                            {
        ...
        public HIS_MEDI_CONTRACT_MATY GetById(long id, HisMediContractMatySO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_MEDI_CONTRACT_MATY.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMediContractMatyExpression != null && search.listHisMediContractMatyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMediContractMatyExpression)
                            {
