...
namespace MOS.DAO.HisBranchTime
{
    partial class HisBranchTimeGet : EntityBase
    {
        ...
        {
            List<V_HIS_BRANCH_TIME> list = new List<V_HIS_BRANCH_TIME>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_BRANCH_TIME.AsNoTracking().AsQueryable();
                        if (search.listVHisBranchTimeExpression != null && search.listVHisBranchTimeExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisBranchTimeExpression)
                            {
