...
{
    partial class HisBranchTimeGet : EntityBase
    {
        public List<HIS_BRANCH_TIME> Get(HisBranchTimeSO search, CommonParam param)
        {
            List<HIS_BRANCH_TIME> list = new List<HIS_BRANCH_TIME>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_BRANCH_TIME.AsNoTracking().AsQueryable();
                        if (search.listHisBranchTimeExpression != null && search.listHisBranchTimeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBranchTimeExpression)
                            {
        ...
        public HIS_BRANCH_TIME GetById(long id, HisBranchTimeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_BRANCH_TIME.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisBranchTimeExpression != null && search.listHisBranchTimeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBranchTimeExpression)
                            {
