...
namespace MOS.DAO.HisInteractiveGrade
{
    partial class HisInteractiveGradeGet : EntityBase
    {
        ...
        {
            List<V_HIS_INTERACTIVE_GRADE> list = new List<V_HIS_INTERACTIVE_GRADE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_INTERACTIVE_GRADE.AsNoTracking().AsQueryable();
                        if (search.listVHisInteractiveGradeExpression != null && search.listVHisInteractiveGradeExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisInteractiveGradeExpression)
                            {
