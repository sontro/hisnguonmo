...
namespace MOS.DAO.HisInteractiveGrade
{
    partial class HisInteractiveGradeGet : EntityBase
    {
        ...
        {
            List<HIS_INTERACTIVE_GRADE> list = new List<HIS_INTERACTIVE_GRADE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_INTERACTIVE_GRADE.AsNoTracking().AsQueryable();
        ...
        public HIS_INTERACTIVE_GRADE GetById(long id, HisInteractiveGradeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_INTERACTIVE_GRADE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisInteractiveGradeExpression != null && search.listHisInteractiveGradeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisInteractiveGradeExpression)
                            {
