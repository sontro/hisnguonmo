    ...
    partial class HisTreatmentLogTypeCheck : EntityBase
    {
        public bool ExistsCode(string code, long? id)
        {
            ...
            try
            {
                ...
                {
                    count = ctx.HIS_TREATMENT_LOG_TYPE.AsQueryable().Where(p => p.TREATMENT_LOG_TYPE_CODE.Equals(code) && p.ID != id && (!p.IS_DELETE.HasValue || p.IS_DELETE.Value != (short)1)).Count();
                }
                result = (count > 0);
            }
            catch (Exception)
