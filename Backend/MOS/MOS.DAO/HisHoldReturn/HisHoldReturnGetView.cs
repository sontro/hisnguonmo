...
namespace MOS.DAO.HisHoldReturn
{
    partial class HisHoldReturnGet : EntityBase
    {
        ...
        {
            List<V_HIS_HOLD_RETURN> list = new List<V_HIS_HOLD_RETURN>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_HOLD_RETURN.AsNoTracking().AsQueryable();
                        if (search.listVHisHoldReturnExpression != null && search.listVHisHoldReturnExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisHoldReturnExpression)
                            {
