...
{
    partial class HisHoldReturnGet : EntityBase
    {
        public List<HIS_HOLD_RETURN> Get(HisHoldReturnSO search, CommonParam param)
        {
            List<HIS_HOLD_RETURN> list = new List<HIS_HOLD_RETURN>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_HOLD_RETURN.AsNoTracking().AsQueryable();
                        if (search.listHisHoldReturnExpression != null && search.listHisHoldReturnExpression.Count > 0)
                        {
                            foreach (var item in search.listHisHoldReturnExpression)
                            {
        ...
        public HIS_HOLD_RETURN GetById(long id, HisHoldReturnSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_HOLD_RETURN.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisHoldReturnExpression != null && search.listHisHoldReturnExpression.Count > 0)
                        {
                            foreach (var item in search.listHisHoldReturnExpression)
                            {
