...
namespace MOS.DAO.HisBirthCertBook
{
    partial class HisBirthCertBookGet : EntityBase
    {
        ...
        {
            List<HIS_BIRTH_CERT_BOOK> list = new List<HIS_BIRTH_CERT_BOOK>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_BIRTH_CERT_BOOK.AsNoTracking().AsQueryable();
        ...
        public HIS_BIRTH_CERT_BOOK GetById(long id, HisBirthCertBookSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_BIRTH_CERT_BOOK.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisBirthCertBookExpression != null && search.listHisBirthCertBookExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBirthCertBookExpression)
                            {
