...
namespace MOS.DAO.HisActiveIngredient
{
    partial class HisActiveIngredientGet : EntityBase
    {
        ...
        {
            List<HIS_ACTIVE_INGREDIENT> list = new List<HIS_ACTIVE_INGREDIENT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_ACTIVE_INGREDIENT.AsNoTracking().AsQueryable();
        ...
        public HIS_ACTIVE_INGREDIENT GetById(long id, HisActiveIngredientSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_ACTIVE_INGREDIENT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisActiveIngredientExpression != null && search.listHisActiveIngredientExpression.Count > 0)
                        {
                            foreach (var item in search.listHisActiveIngredientExpression)
                            {
