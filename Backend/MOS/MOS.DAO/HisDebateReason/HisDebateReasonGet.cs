...
namespace MOS.DAO.HisDebateReason
{
    partial class HisDebateReasonGet : EntityBase
    {
        ...
        {
            List<HIS_DEBATE_REASON> list = new List<HIS_DEBATE_REASON>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_DEBATE_REASON.AsNoTracking().AsQueryable();
        ...
        public HIS_DEBATE_REASON GetById(long id, HisDebateReasonSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_DEBATE_REASON.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisDebateReasonExpression != null && search.listHisDebateReasonExpression.Count > 0)
                        {
                            foreach (var item in search.listHisDebateReasonExpression)
                            {
