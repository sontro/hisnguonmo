...
namespace MOS.DAO.HisDebateReason
{
    partial class HisDebateReasonGet : EntityBase
    {
        ...
        {
            List<V_HIS_DEBATE_REASON> list = new List<V_HIS_DEBATE_REASON>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_DEBATE_REASON.AsNoTracking().AsQueryable();
                        if (search.listVHisDebateReasonExpression != null && search.listVHisDebateReasonExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisDebateReasonExpression)
                            {
