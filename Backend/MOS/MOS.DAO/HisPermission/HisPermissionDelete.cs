...
{
    partial class HisPermissionDelete : EntityBase
    {
        public HisPermissionDelete()
            : base()
        {
        ...
        public bool Delete(HIS_PERMISSION data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_PERMISSION> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_PERMISSION> listIdNotFound = new List<HIS_PERMISSION>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_PERMISSION> listRaw = ctx.HIS_PERMISSION.Where(o => listId.Contains(o.ID)).ToList();
