...
namespace MOS.DAO.HisInvoiceDetail
{
    partial class HisInvoiceDetailGet : EntityBase
    {
        ...
        {
            List<V_HIS_INVOICE_DETAIL> list = new List<V_HIS_INVOICE_DETAIL>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_INVOICE_DETAIL.AsQueryable();
                        if (search.listVHisInvoiceDetailExpression != null && search.listVHisInvoiceDetailExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisInvoiceDetailExpression)
                            {
