...
{
    partial class HisIcdServiceDelete : EntityBase
    {
        public HisIcdServiceDelete()
            : base()
        {
        ...
        public bool Delete(HIS_ICD_SERVICE data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_ICD_SERVICE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_ICD_SERVICE> listIdNotFound = new List<HIS_ICD_SERVICE>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_ICD_SERVICE> listRaw = ctx.HIS_ICD_SERVICE.Where(o => listId.Contains(o.ID)).ToList();
