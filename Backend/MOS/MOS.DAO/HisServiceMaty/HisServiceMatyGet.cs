...
namespace MOS.DAO.HisServiceMaty
{
    partial class HisServiceMatyGet : EntityBase
    {
        ...
        {
            List<HIS_SERVICE_MATY> list = new List<HIS_SERVICE_MATY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SERVICE_MATY.AsNoTracking().AsQueryable();
                        if (search.listHisServiceMatyExpression != null && search.listHisServiceMatyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisServiceMatyExpression)
                            {
        ...
        public HIS_SERVICE_MATY GetById(long id, HisServiceMatySO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_SERVICE_MATY.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisServiceMatyExpression != null && search.listHisServiceMatyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisServiceMatyExpression)
                            {
