...
namespace MOS.DAO.HisDeathWithin
{
    partial class HisDeathWithinGet : EntityBase
    {
        ...
        {
            List<HIS_DEATH_WITHIN> list = new List<HIS_DEATH_WITHIN>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_DEATH_WITHIN.AsNoTracking().AsQueryable();
        ...
        public HIS_DEATH_WITHIN GetById(long id, HisDeathWithinSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_DEATH_WITHIN.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisDeathWithinExpression != null && search.listHisDeathWithinExpression.Count > 0)
                        {
                            foreach (var item in search.listHisDeathWithinExpression)
                            {
