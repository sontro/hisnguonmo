...
{
    partial class HisCareGet : EntityBase
    {
        public List<HIS_CARE> Get(HisCareSO search, CommonParam param)
        {
            List<HIS_CARE> list = new List<HIS_CARE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_CARE.AsNoTracking().AsQueryable();
                        if (search.listHisCareExpression != null && search.listHisCareExpression.Count > 0)
                        {
                            foreach (var item in search.listHisCareExpression)
                            {
        ...
        public HIS_CARE GetById(long id, HisCareSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_CARE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisCareExpression != null && search.listHisCareExpression.Count > 0)
                        {
                            foreach (var item in search.listHisCareExpression)
                            {
