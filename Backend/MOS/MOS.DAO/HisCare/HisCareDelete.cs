...
{
    partial class HisCareDelete : EntityBase
    {
        public HisCareDelete()
            : base()
        {
        ...
        public bool Delete(HIS_CARE data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_CARE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_CARE> listIdNotFound = new List<HIS_CARE>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_CARE> listRaw = ctx.HIS_CARE.Where(o => listId.Contains(o.ID)).ToList();
