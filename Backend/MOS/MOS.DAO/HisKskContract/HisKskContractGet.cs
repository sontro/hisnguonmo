...
namespace MOS.DAO.HisKskContract
{
    partial class HisKskContractGet : EntityBase
    {
        ...
        {
            List<HIS_KSK_CONTRACT> list = new List<HIS_KSK_CONTRACT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_KSK_CONTRACT.AsNoTracking().AsQueryable();
                        if (search.listHisKskContractExpression != null && search.listHisKskContractExpression.Count > 0)
                        {
                            foreach (var item in search.listHisKskContractExpression)
                            {
        ...
        public HIS_KSK_CONTRACT GetById(long id, HisKskContractSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_KSK_CONTRACT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisKskContractExpression != null && search.listHisKskContractExpression.Count > 0)
                        {
                            foreach (var item in search.listHisKskContractExpression)
                            {
