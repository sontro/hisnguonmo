...
namespace MOS.DAO.HisPrepare
{
    partial class HisPrepareGet : EntityBase
    {
        ...
        {
            List<V_HIS_PREPARE> list = new List<V_HIS_PREPARE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_PREPARE.AsNoTracking().AsQueryable();
                        if (search.listVHisPrepareExpression != null && search.listVHisPrepareExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisPrepareExpression)
                            {
