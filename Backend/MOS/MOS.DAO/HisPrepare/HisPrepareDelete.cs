...
{
    partial class HisPrepareDelete : EntityBase
    {
        public HisPrepareDelete()
            : base()
        {
        ...
        public bool Delete(HIS_PREPARE data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_PREPARE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_PREPARE> listIdNotFound = new List<HIS_PREPARE>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_PREPARE> listRaw = ctx.HIS_PREPARE.Where(o => listId.Contains(o.ID)).ToList();
