...
namespace MOS.DAO.HisEmrCoverType
{
    partial class HisEmrCoverTypeGet : EntityBase
    {
        ...
        {
            List<V_HIS_EMR_COVER_TYPE> list = new List<V_HIS_EMR_COVER_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_EMR_COVER_TYPE.AsNoTracking().AsQueryable();
                        if (search.listVHisEmrCoverTypeExpression != null && search.listVHisEmrCoverTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisEmrCoverTypeExpression)
                            {
