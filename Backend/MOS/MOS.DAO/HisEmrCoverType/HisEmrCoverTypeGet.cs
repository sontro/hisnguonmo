...
{
    partial class HisEmrCoverTypeGet : EntityBase
    {
        public List<HIS_EMR_COVER_TYPE> Get(HisEmrCoverTypeSO search, CommonParam param)
        {
            List<HIS_EMR_COVER_TYPE> list = new List<HIS_EMR_COVER_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_EMR_COVER_TYPE.AsNoTracking().AsQueryable();
                        if (search.listHisEmrCoverTypeExpression != null && search.listHisEmrCoverTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisEmrCoverTypeExpression)
                            {
        ...
        public HIS_EMR_COVER_TYPE GetById(long id, HisEmrCoverTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_EMR_COVER_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisEmrCoverTypeExpression != null && search.listHisEmrCoverTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisEmrCoverTypeExpression)
                            {
