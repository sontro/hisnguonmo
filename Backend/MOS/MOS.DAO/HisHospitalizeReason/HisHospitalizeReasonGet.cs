...
namespace MOS.DAO.HisHospitalizeReason
{
    partial class HisHospitalizeReasonGet : EntityBase
    {
        ...
        {
            List<HIS_HOSPITALIZE_REASON> list = new List<HIS_HOSPITALIZE_REASON>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_HOSPITALIZE_REASON.AsNoTracking().AsQueryable();
        ...
        public HIS_HOSPITALIZE_REASON GetById(long id, HisHospitalizeReasonSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_HOSPITALIZE_REASON.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisHospitalizeReasonExpression != null && search.listHisHospitalizeReasonExpression.Count > 0)
                        {
                            foreach (var item in search.listHisHospitalizeReasonExpression)
                            {
