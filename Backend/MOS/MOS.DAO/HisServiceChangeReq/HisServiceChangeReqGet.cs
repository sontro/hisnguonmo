...
namespace MOS.DAO.HisServiceChangeReq
{
    partial class HisServiceChangeReqGet : EntityBase
    {
        ...
        {
            List<HIS_SERVICE_CHANGE_REQ> list = new List<HIS_SERVICE_CHANGE_REQ>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SERVICE_CHANGE_REQ.AsNoTracking().AsQueryable();
        ...
        public HIS_SERVICE_CHANGE_REQ GetById(long id, HisServiceChangeReqSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_SERVICE_CHANGE_REQ.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisServiceChangeReqExpression != null && search.listHisServiceChangeReqExpression.Count > 0)
                        {
                            foreach (var item in search.listHisServiceChangeReqExpression)
                            {
