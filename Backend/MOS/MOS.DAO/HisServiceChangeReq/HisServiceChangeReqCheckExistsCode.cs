    ...
    partial class HisServiceChangeReqCheck : EntityBase
    {
        public bool ExistsCode(string code, long? id)
        {
            ...
            try
            {
                ...
                {
                    count = ctx.HIS_SERVICE_CHANGE_REQ.AsQueryable().Where(p => p.SERVICE_CHANGE_REQ_CODE.Equals(code) && p.ID != id && (p.IS_DELETE.HasValue || p.IS_DELETE.Value != (short)1)).Count();
                }
                result = (count > 0);
            }
            catch (Exception)
