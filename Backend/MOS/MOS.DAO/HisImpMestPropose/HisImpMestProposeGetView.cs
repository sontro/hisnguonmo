...
namespace MOS.DAO.HisImpMestPropose
{
    partial class HisImpMestProposeGet : EntityBase
    {
        ...
        {
            List<V_HIS_IMP_MEST_PROPOSE> list = new List<V_HIS_IMP_MEST_PROPOSE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_IMP_MEST_PROPOSE.AsNoTracking().AsQueryable();
                        if (search.listVHisImpMestProposeExpression != null && search.listVHisImpMestProposeExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisImpMestProposeExpression)
                            {
