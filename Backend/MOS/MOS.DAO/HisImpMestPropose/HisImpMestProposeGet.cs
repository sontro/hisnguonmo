...
namespace MOS.DAO.HisImpMestPropose
{
    partial class HisImpMestProposeGet : EntityBase
    {
        ...
        {
            List<HIS_IMP_MEST_PROPOSE> list = new List<HIS_IMP_MEST_PROPOSE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_IMP_MEST_PROPOSE.AsNoTracking().AsQueryable();
        ...
        public HIS_IMP_MEST_PROPOSE GetById(long id, HisImpMestProposeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_IMP_MEST_PROPOSE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisImpMestProposeExpression != null && search.listHisImpMestProposeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisImpMestProposeExpression)
                            {
