...
namespace MOS.DAO.HisBloodVolume
{
    partial class HisBloodVolumeGet : EntityBase
    {
        ...
        {
            List<V_HIS_BLOOD_VOLUME> list = new List<V_HIS_BLOOD_VOLUME>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_BLOOD_VOLUME.AsQueryable();
                        if (search.listVHisBloodVolumeExpression != null && search.listVHisBloodVolumeExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisBloodVolumeExpression)
                            {
