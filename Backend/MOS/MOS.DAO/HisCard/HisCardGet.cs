...
{
    partial class HisCardGet : EntityBase
    {
        public List<HIS_CARD> Get(HisCardSO search, CommonParam param)
        {
            List<HIS_CARD> list = new List<HIS_CARD>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_CARD.AsNoTracking().AsQueryable();
                        if (search.listHisCardExpression != null && search.listHisCardExpression.Count > 0)
                        {
                            foreach (var item in search.listHisCardExpression)
                            {
        ...
        public HIS_CARD GetById(long id, HisCardSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_CARD.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisCardExpression != null && search.listHisCardExpression.Count > 0)
                        {
                            foreach (var item in search.listHisCardExpression)
                            {
