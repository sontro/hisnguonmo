...
{
    partial class HisCardDelete : EntityBase
    {
        public HisCardDelete()
            : base()
        {
        ...
        public bool Delete(HIS_CARD data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_CARD> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_CARD> listIdNotFound = new List<HIS_CARD>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_CARD> listRaw = ctx.HIS_CARD.Where(o => listId.Contains(o.ID)).ToList();
