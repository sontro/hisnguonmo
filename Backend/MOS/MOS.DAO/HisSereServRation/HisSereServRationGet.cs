...
namespace MOS.DAO.HisSereServRation
{
    partial class HisSereServRationGet : EntityBase
    {
        ...
        {
            List<HIS_SERE_SERV_RATION> list = new List<HIS_SERE_SERV_RATION>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SERE_SERV_RATION.AsNoTracking().AsQueryable();
        ...
        public HIS_SERE_SERV_RATION GetById(long id, HisSereServRationSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_SERE_SERV_RATION.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisSereServRationExpression != null && search.listHisSereServRationExpression.Count > 0)
                        {
                            foreach (var item in search.listHisSereServRationExpression)
                            {
