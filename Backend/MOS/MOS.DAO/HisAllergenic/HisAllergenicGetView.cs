...
{
    partial class HisAllergenicGet : EntityBase
    {
        public List<V_HIS_ALLERGENIC> GetView(HisAllergenicSO search, CommonParam param)
        {
            List<V_HIS_ALLERGENIC> list = new List<V_HIS_ALLERGENIC>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_ALLERGENIC.AsNoTracking().AsQueryable();
                        if (search.listVHisAllergenicExpression != null && search.listVHisAllergenicExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisAllergenicExpression)
                            {
