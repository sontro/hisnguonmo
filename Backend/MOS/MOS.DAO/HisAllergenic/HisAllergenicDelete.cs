...
{
    partial class HisAllergenicDelete : EntityBase
    {
        public HisAllergenicDelete()
            : base()
        {
        ...
        public bool Delete(HIS_ALLERGENIC data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_ALLERGENIC> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_ALLERGENIC> listIdNotFound = new List<HIS_ALLERGENIC>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_ALLERGENIC> listRaw = ctx.HIS_ALLERGENIC.Where(o => listId.Contains(o.ID)).ToList();
