...
{
    partial class HisHtuDelete : EntityBase
    {
        public HisHtuDelete()
            : base()
        {
        ...
        public bool Delete(HIS_HTU data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_HTU> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_HTU> listIdNotFound = new List<HIS_HTU>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_HTU> listRaw = ctx.HIS_HTU.Where(o => listId.Contains(o.ID)).ToList();
