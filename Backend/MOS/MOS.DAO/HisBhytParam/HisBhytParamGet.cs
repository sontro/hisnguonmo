...
namespace MOS.DAO.HisBhytParam
{
    partial class HisBhytParamGet : EntityBase
    {
        ...
        {
            List<HIS_BHYT_PARAM> list = new List<HIS_BHYT_PARAM>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_BHYT_PARAM.AsNoTracking().AsQueryable();
        ...
        public HIS_BHYT_PARAM GetById(long id, HisBhytParamSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_BHYT_PARAM.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisBhytParamExpression != null && search.listHisBhytParamExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBhytParamExpression)
                            {
