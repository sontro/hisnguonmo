...
{
    partial class HisTransactionTypeDelete : EntityBase
    {
        public HisTransactionTypeDelete()
            : base()
        {
        ...
        public bool Delete(HIS_TRANSACTION_TYPE data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_TRANSACTION_TYPE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_TRANSACTION_TYPE> listIdNotFound = new List<HIS_TRANSACTION_TYPE>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_TRANSACTION_TYPE> listRaw = ctx.HIS_TRANSACTION_TYPE.Where(o => listId.Contains(o.ID)).ToList();
