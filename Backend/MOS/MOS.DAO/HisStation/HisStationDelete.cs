...
{
    partial class HisStationDelete : EntityBase
    {
        public HisStationDelete()
            : base()
        {
        ...
        public bool Delete(HIS_STATION data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_STATION> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_STATION> listIdNotFound = new List<HIS_STATION>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_STATION> listRaw = ctx.HIS_STATION.Where(o => listId.Contains(o.ID)).ToList();
