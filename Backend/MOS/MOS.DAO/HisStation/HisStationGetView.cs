...
{
    partial class HisStationGet : EntityBase
    {
        public List<V_HIS_STATION> GetView(HisStationSO search, CommonParam param)
        {
            List<V_HIS_STATION> list = new List<V_HIS_STATION>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_STATION.AsNoTracking().AsQueryable();
                        if (search.listVHisStationExpression != null && search.listVHisStationExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisStationExpression)
                            {
