...
namespace MOS.DAO.HisTransfusion
{
    partial class HisTransfusionGet : EntityBase
    {
        ...
        {
            List<HIS_TRANSFUSION> list = new List<HIS_TRANSFUSION>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_TRANSFUSION.AsNoTracking().AsQueryable();
                        if (search.listHisTransfusionExpression != null && search.listHisTransfusionExpression.Count > 0)
                        {
                            foreach (var item in search.listHisTransfusionExpression)
                            {
        ...
        public HIS_TRANSFUSION GetById(long id, HisTransfusionSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_TRANSFUSION.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisTransfusionExpression != null && search.listHisTransfusionExpression.Count > 0)
                        {
                            foreach (var item in search.listHisTransfusionExpression)
                            {
