...
namespace MOS.DAO.HisRehaSum
{
    partial class HisRehaSumGet : EntityBase
    {
        ...
        {
            List<V_HIS_REHA_SUM> list = new List<V_HIS_REHA_SUM>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_REHA_SUM.AsQueryable();
                        if (search.listVHisRehaSumExpression != null && search.listVHisRehaSumExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisRehaSumExpression)
                            {
