...
namespace MOS.DAO.HisServiceRati
{
    partial class HisServiceRatiGet : EntityBase
    {
        ...
        {
            List<HIS_SERVICE_RATI> list = new List<HIS_SERVICE_RATI>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SERVICE_RATI.AsNoTracking().AsQueryable();
                        if (search.listHisServiceRatiExpression != null && search.listHisServiceRatiExpression.Count > 0)
                        {
                            foreach (var item in search.listHisServiceRatiExpression)
                            {
        ...
        public HIS_SERVICE_RATI GetById(long id, HisServiceRatiSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_SERVICE_RATI.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisServiceRatiExpression != null && search.listHisServiceRatiExpression.Count > 0)
                        {
                            foreach (var item in search.listHisServiceRatiExpression)
                            {
