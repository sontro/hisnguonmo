...
{
    partial class HisServiceRatiDelete : EntityBase
    {
        public HisServiceRatiDelete()
            : base()
        {
        ...
        public bool Delete(HIS_SERVICE_RATI data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_SERVICE_RATI> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_SERVICE_RATI> listIdNotFound = new List<HIS_SERVICE_RATI>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_SERVICE_RATI> listRaw = ctx.HIS_SERVICE_RATI.Where(o => listId.Contains(o.ID)).ToList();
