...
namespace MOS.DAO.HisMaterialBean
{
    partial class HisMaterialBeanGet : EntityBase
    {
        ...
        {
            List<V_HIS_MATERIAL_BEAN_2> list = new List<V_HIS_MATERIAL_BEAN_2>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_MATERIAL_BEAN_2.AsNoTracking().AsQueryable();
                        if (search.listVHisMaterialBean2Expression != null && search.listVHisMaterialBean2Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisMaterialBean2Expression)
                            {
