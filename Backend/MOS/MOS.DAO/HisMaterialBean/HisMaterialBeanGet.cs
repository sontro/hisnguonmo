...
namespace MOS.DAO.HisMaterialBean
{
    partial class HisMaterialBeanGet : EntityBase
    {
        ...
        {
            List<HIS_MATERIAL_BEAN> list = new List<HIS_MATERIAL_BEAN>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MATERIAL_BEAN.AsNoTracking().AsQueryable();
        ...
        public HIS_MATERIAL_BEAN GetById(long id, HisMaterialBeanSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_MATERIAL_BEAN.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMaterialBeanExpression != null && search.listHisMaterialBeanExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMaterialBeanExpression)
                            {
