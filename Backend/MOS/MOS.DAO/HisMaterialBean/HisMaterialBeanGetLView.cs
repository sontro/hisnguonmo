...
namespace MOS.DAO.HisMaterialBean
{
    partial class HisMaterialBeanGet : EntityBase
    {
        ...
        {
            List<L_HIS_MATERIAL_BEAN> list = new List<L_HIS_MATERIAL_BEAN>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.L_HIS_MATERIAL_BEAN.AsNoTracking().AsQueryable();
                        if (search.listLHisMaterialBeanExpression != null && search.listLHisMaterialBeanExpression.Count > 0)
                        {
                            foreach (var item in search.listLHisMaterialBeanExpression)
                            {
