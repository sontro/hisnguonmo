...
namespace MOS.DAO.HisVitaminA
{
    partial class HisVitaminAGet : EntityBase
    {
        ...
        {
            List<HIS_VITAMIN_A> list = new List<HIS_VITAMIN_A>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_VITAMIN_A.AsNoTracking().AsQueryable();
        ...
        public HIS_VITAMIN_A GetById(long id, HisVitaminASO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_VITAMIN_A.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisVitaminAExpression != null && search.listHisVitaminAExpression.Count > 0)
                        {
                            foreach (var item in search.listHisVitaminAExpression)
                            {
