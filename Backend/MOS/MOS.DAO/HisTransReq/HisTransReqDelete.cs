...
{
    partial class HisTransReqDelete : EntityBase
    {
        public HisTransReqDelete()
            : base()
        {
        ...
        public bool Delete(HIS_TRANS_REQ data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_TRANS_REQ> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_TRANS_REQ> listIdNotFound = new List<HIS_TRANS_REQ>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_TRANS_REQ> listRaw = ctx.HIS_TRANS_REQ.Where(o => listId.Contains(o.ID)).ToList();
