...
namespace MOS.DAO.HisTransReq
{
    ...
    {
        public List<HIS_TRANS_REQ> Get(HisTransReqSO search, CommonParam param)
        {
            List<HIS_TRANS_REQ> list = new List<HIS_TRANS_REQ>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_TRANS_REQ.AsNoTracking().AsQueryable();
                        if (search.listHisTransReqExpression != null && search.listHisTransReqExpression.Count > 0)
                        {
                            foreach (var item in search.listHisTransReqExpression)
                            {
        ...
        public HIS_TRANS_REQ GetById(long id, HisTransReqSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_TRANS_REQ.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisTransReqExpression != null && search.listHisTransReqExpression.Count > 0)
                        {
                            foreach (var item in search.listHisTransReqExpression)
                            {
