...
{
    partial class HisBloodGroupDelete : EntityBase
    {
        public HisBloodGroupDelete()
            : base()
        {
        ...
        public bool Delete(HIS_BLOOD_GROUP data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_BLOOD_GROUP> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_BLOOD_GROUP> listIdNotFound = new List<HIS_BLOOD_GROUP>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_BLOOD_GROUP> listRaw = ctx.HIS_BLOOD_GROUP.Where(o => listId.Contains(o.ID)).ToList();
