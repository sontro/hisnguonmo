...
namespace MOS.DAO.HisRoomSaro
{
    partial class HisRoomSaroGet : EntityBase
    {
        ...
        {
            List<HIS_ROOM_SARO> list = new List<HIS_ROOM_SARO>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_ROOM_SARO.AsNoTracking().AsQueryable();
        ...
        public HIS_ROOM_SARO GetById(long id, HisRoomSaroSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_ROOM_SARO.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisRoomSaroExpression != null && search.listHisRoomSaroExpression.Count > 0)
                        {
                            foreach (var item in search.listHisRoomSaroExpression)
                            {
