...
namespace MOS.DAO.HisBcsMetyReqDt
{
    partial class HisBcsMetyReqDtGet : EntityBase
    {
        ...
        {
            List<HIS_BCS_METY_REQ_DT> list = new List<HIS_BCS_METY_REQ_DT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_BCS_METY_REQ_DT.AsNoTracking().AsQueryable();
                        if (search.listHisBcsMetyReqDtExpression != null && search.listHisBcsMetyReqDtExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBcsMetyReqDtExpression)
                            {
        ...
        public HIS_BCS_METY_REQ_DT GetById(long id, HisBcsMetyReqDtSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_BCS_METY_REQ_DT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisBcsMetyReqDtExpression != null && search.listHisBcsMetyReqDtExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBcsMetyReqDtExpression)
                            {
