...
namespace MOS.DAO.HisTreatmentEndTypeExt
{
    partial class HisTreatmentEndTypeExtGet : EntityBase
    {
        ...
        {
            List<HIS_TREATMENT_END_TYPE_EXT> list = new List<HIS_TREATMENT_END_TYPE_EXT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_TREATMENT_END_TYPE_EXT.AsNoTracking().AsQueryable();
                        if (search.listHisTreatmentEndTypeExtExpression != null && search.listHisTreatmentEndTypeExtExpression.Count > 0)
                        {
                            foreach (var item in search.listHisTreatmentEndTypeExtExpression)
                            {
        ...
        public HIS_TREATMENT_END_TYPE_EXT GetById(long id, HisTreatmentEndTypeExtSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_TREATMENT_END_TYPE_EXT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisTreatmentEndTypeExtExpression != null && search.listHisTreatmentEndTypeExtExpression.Count > 0)
                        {
                            foreach (var item in search.listHisTreatmentEndTypeExtExpression)
                            {
