    ...
    partial class HisEquipmentSetMatyCheck : EntityBase
    {
        public bool ExistsCode(string code, long? id)
        {
            ...
            try
            {
                ...
                {
                    count = ctx.HIS_EQUIPMENT_SET_MATY.AsQueryable().Where(p => p.EQUIPMENT_SET_MATY_CODE.Equals(code) && p.ID != id && (p.IS_DELETE.HasValue || p.IS_DELETE.Value != (short)1)).Count();
                }
                result = (count > 0);
            }
            catch (Exception)
