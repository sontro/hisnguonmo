...
namespace MOS.DAO.HisEquipmentSetMaty
{
    partial class HisEquipmentSetMatyGet : EntityBase
    {
        ...
        {
            List<HIS_EQUIPMENT_SET_MATY> list = new List<HIS_EQUIPMENT_SET_MATY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_EQUIPMENT_SET_MATY.AsNoTracking().AsQueryable();
        ...
        public HIS_EQUIPMENT_SET_MATY GetById(long id, HisEquipmentSetMatySO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_EQUIPMENT_SET_MATY.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisEquipmentSetMatyExpression != null && search.listHisEquipmentSetMatyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisEquipmentSetMatyExpression)
                            {
