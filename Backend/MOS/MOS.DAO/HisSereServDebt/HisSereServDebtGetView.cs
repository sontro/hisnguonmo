...
namespace MOS.DAO.HisSereServDebt
{
    partial class HisSereServDebtGet : EntityBase
    {
        ...
        {
            List<V_HIS_SERE_SERV_DEBT> list = new List<V_HIS_SERE_SERV_DEBT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_SERE_SERV_DEBT.AsNoTracking().AsQueryable();
                        if (search.listVHisSereServDebtExpression != null && search.listVHisSereServDebtExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisSereServDebtExpression)
                            {
