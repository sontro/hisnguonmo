...
namespace MOS.DAO.HisSereServDebt
{
    partial class HisSereServDebtGet : EntityBase
    {
        ...
        {
            List<HIS_SERE_SERV_DEBT> list = new List<HIS_SERE_SERV_DEBT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SERE_SERV_DEBT.AsNoTracking().AsQueryable();
        ...
        public HIS_SERE_SERV_DEBT GetById(long id, HisSereServDebtSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_SERE_SERV_DEBT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisSereServDebtExpression != null && search.listHisSereServDebtExpression.Count > 0)
                        {
                            foreach (var item in search.listHisSereServDebtExpression)
                            {
