...
{
    partial class HisInvoiceDelete : EntityBase
    {
        public HisInvoiceDelete()
            : base()
        {
        ...
        public bool Delete(HIS_INVOICE data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_INVOICE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_INVOICE> listIdNotFound = new List<HIS_INVOICE>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_INVOICE> listRaw = ctx.HIS_INVOICE.Where(o => listId.Contains(o.ID)).ToList();
