...
{
    partial class HisInvoiceGet : EntityBase
    {
        public List<V_HIS_INVOICE> GetView(HisInvoiceSO search, CommonParam param)
        {
            List<V_HIS_INVOICE> list = new List<V_HIS_INVOICE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_INVOICE.AsNoTracking().AsQueryable();
                        if (search.listVHisInvoiceExpression != null && search.listVHisInvoiceExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisInvoiceExpression)
                            {
