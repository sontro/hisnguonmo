...
{
    partial class HisPositionDelete : EntityBase
    {
        public HisPositionDelete()
            : base()
        {
        ...
        public bool Delete(HIS_POSITION data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_POSITION> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_POSITION> listIdNotFound = new List<HIS_POSITION>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_POSITION> listRaw = ctx.HIS_POSITION.Where(o => listId.Contains(o.ID)).ToList();
