...
namespace MOS.DAO.HisPosition
{
    partial class HisPositionGet : EntityBase
    {
        ...
        {
            List<V_HIS_POSITION> list = new List<V_HIS_POSITION>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_POSITION.AsNoTracking().AsQueryable();
                        if (search.listVHisPositionExpression != null && search.listVHisPositionExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisPositionExpression)
                            {
