...
namespace MOS.DAO.HisExpMestMatyReq
{
    partial class HisExpMestMatyReqGet : EntityBase
    {
        ...
        {
            List<HIS_EXP_MEST_MATY_REQ> list = new List<HIS_EXP_MEST_MATY_REQ>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_EXP_MEST_MATY_REQ.AsNoTracking().AsQueryable();
                        if (search.listHisExpMestMatyReqExpression != null && search.listHisExpMestMatyReqExpression.Count > 0)
                        {
                            foreach (var item in search.listHisExpMestMatyReqExpression)
                            {
        ...
        public HIS_EXP_MEST_MATY_REQ GetById(long id, HisExpMestMatyReqSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_EXP_MEST_MATY_REQ.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisExpMestMatyReqExpression != null && search.listHisExpMestMatyReqExpression.Count > 0)
                        {
                            foreach (var item in search.listHisExpMestMatyReqExpression)
                            {
