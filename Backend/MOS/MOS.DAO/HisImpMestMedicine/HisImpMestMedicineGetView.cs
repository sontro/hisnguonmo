...
namespace MOS.DAO.HisImpMestMedicine
{
    partial class HisImpMestMedicineGet : EntityBase
    {
        ...
        {
            List<V_HIS_IMP_MEST_MEDICINE> list = new List<V_HIS_IMP_MEST_MEDICINE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_IMP_MEST_MEDICINE.AsNoTracking().AsQueryable();
                        if (search.listVHisImpMestMedicineExpression != null && search.listVHisImpMestMedicineExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisImpMestMedicineExpression)
                            {
