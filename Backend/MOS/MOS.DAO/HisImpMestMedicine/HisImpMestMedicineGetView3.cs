...
namespace MOS.DAO.HisImpMestMedicine
{
    ...
    {
        public List<V_HIS_IMP_MEST_MEDICINE_3> GetView3(HisImpMestMedicineSO search, CommonParam param)
        {
            List<V_HIS_IMP_MEST_MEDICINE_3> list = new List<V_HIS_IMP_MEST_MEDICINE_3>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_IMP_MEST_MEDICINE_3.AsNoTracking().AsQueryable();
