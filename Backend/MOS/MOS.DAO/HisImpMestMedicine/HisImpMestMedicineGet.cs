...
namespace MOS.DAO.HisImpMestMedicine
{
    partial class HisImpMestMedicineGet : EntityBase
    {
        ...
        {
            List<HIS_IMP_MEST_MEDICINE> list = new List<HIS_IMP_MEST_MEDICINE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_IMP_MEST_MEDICINE.AsNoTracking().AsQueryable();
        ...
        public HIS_IMP_MEST_MEDICINE GetById(long id, HisImpMestMedicineSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_IMP_MEST_MEDICINE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisImpMestMedicineExpression != null && search.listHisImpMestMedicineExpression.Count > 0)
                        {
                            foreach (var item in search.listHisImpMestMedicineExpression)
                            {
