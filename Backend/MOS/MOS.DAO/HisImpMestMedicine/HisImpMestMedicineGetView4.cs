...
namespace MOS.DAO.HisImpMestMedicine
{
    partial class HisImpMestMedicineGet : EntityBase
    {
        ...
        {
            List<V_HIS_IMP_MEST_MEDICINE_4> list = new List<V_HIS_IMP_MEST_MEDICINE_4>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_IMP_MEST_MEDICINE_4.AsNoTracking().AsQueryable();
                        if (search.listVHisImpMestMedicine4Expression != null && search.listVHisImpMestMedicine4Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisImpMestMedicine4Expression)
                            {
