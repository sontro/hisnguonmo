    ...
    partial class HisDebateInviteUserCheck : EntityBase
    {
        public bool ExistsCode(string code, long? id)
        {
            ...
            try
            {
                ...
                {
                    count = ctx.HIS_DEBATE_INVITE_USER.AsQueryable().Where(p => p.DEBATE_INVITE_USER_CODE.Equals(code) && p.ID != id && (p.IS_DELETE.HasValue || p.IS_DELETE.Value != (short)1)).Count();
                }
                result = (count > 0);
            }
            catch (Exception)
