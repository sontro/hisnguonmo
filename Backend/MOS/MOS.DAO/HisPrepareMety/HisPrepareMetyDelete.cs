...
{
    partial class HisPrepareMetyDelete : EntityBase
    {
        public HisPrepareMetyDelete()
            : base()
        {
        ...
        public bool Delete(HIS_PREPARE_METY data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_PREPARE_METY> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_PREPARE_METY> listIdNotFound = new List<HIS_PREPARE_METY>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_PREPARE_METY> listRaw = ctx.HIS_PREPARE_METY.Where(o => listId.Contains(o.ID)).ToList();
