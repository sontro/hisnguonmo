...
{
    partial class HisMediStockDelete : EntityBase
    {
        public HisMediStockDelete()
            : base()
        {
        ...
        public bool Delete(HIS_MEDI_STOCK data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_MEDI_STOCK> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_MEDI_STOCK> listIdNotFound = new List<HIS_MEDI_STOCK>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_MEDI_STOCK> listRaw = ctx.HIS_MEDI_STOCK.Where(o => listId.Contains(o.ID)).ToList();
