...
namespace MOS.DAO.HisExpMestBltyReq
{
    partial class HisExpMestBltyReqGet : EntityBase
    {
        ...
        {
            List<HIS_EXP_MEST_BLTY_REQ> list = new List<HIS_EXP_MEST_BLTY_REQ>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_EXP_MEST_BLTY_REQ.AsNoTracking().AsQueryable();
                        if (search.listHisExpMestBltyReqExpression != null && search.listHisExpMestBltyReqExpression.Count > 0)
                        {
                            foreach (var item in search.listHisExpMestBltyReqExpression)
                            {
        ...
        public HIS_EXP_MEST_BLTY_REQ GetById(long id, HisExpMestBltyReqSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_EXP_MEST_BLTY_REQ.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisExpMestBltyReqExpression != null && search.listHisExpMestBltyReqExpression.Count > 0)
                        {
                            foreach (var item in search.listHisExpMestBltyReqExpression)
                            {
