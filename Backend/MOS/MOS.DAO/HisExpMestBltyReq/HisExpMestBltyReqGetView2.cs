...
namespace MOS.DAO.HisExpMestBltyReq
{
    partial class HisExpMestBltyReqGet : EntityBase
    {
        ...
        {
            List<V_HIS_EXP_MEST_BLTY_REQ_2> list = new List<V_HIS_EXP_MEST_BLTY_REQ_2>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_EXP_MEST_BLTY_REQ_2.AsNoTracking().AsQueryable();
                        if (search.listVHisExpMestBltyReq2Expression != null && search.listVHisExpMestBltyReq2Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisExpMestBltyReq2Expression)
                            {
