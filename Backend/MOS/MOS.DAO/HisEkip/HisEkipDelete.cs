...
{
    partial class HisEkipDelete : EntityBase
    {
        public HisEkipDelete()
            : base()
        {
        ...
        public bool Delete(HIS_EKIP data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_EKIP> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_EKIP> listIdNotFound = new List<HIS_EKIP>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_EKIP> listRaw = ctx.HIS_EKIP.Where(o => listId.Contains(o.ID)).ToList();
