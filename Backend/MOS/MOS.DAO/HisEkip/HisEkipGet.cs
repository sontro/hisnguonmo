...
namespace MOS.DAO.HisEkip
{
    partial class HisEkipGet : EntityBase
    {
        ...
        {
            List<HIS_EKIP> list = new List<HIS_EKIP>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_EKIP.AsNoTracking().AsQueryable();
                        if (search.listHisEkipExpression != null && search.listHisEkipExpression.Count > 0)
                        {
                            foreach (var item in search.listHisEkipExpression)
                            {
        ...
        public HIS_EKIP GetById(long id, HisEkipSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_EKIP.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisEkipExpression != null && search.listHisEkipExpression.Count > 0)
                        {
                            foreach (var item in search.listHisEkipExpression)
                            {
