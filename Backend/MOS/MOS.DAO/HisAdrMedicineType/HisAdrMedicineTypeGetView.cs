...
namespace MOS.DAO.HisAdrMedicineType
{
    partial class HisAdrMedicineTypeGet : EntityBase
    {
        ...
        {
            List<V_HIS_ADR_MEDICINE_TYPE> list = new List<V_HIS_ADR_MEDICINE_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_ADR_MEDICINE_TYPE.AsNoTracking().AsQueryable();
                        if (search.listVHisAdrMedicineTypeExpression != null && search.listVHisAdrMedicineTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisAdrMedicineTypeExpression)
                            {
