...
{
    partial class HisAdrMedicineTypeGet : EntityBase
    {
        public List<HIS_ADR_MEDICINE_TYPE> Get(HisAdrMedicineTypeSO search, CommonParam param)
        {
            List<HIS_ADR_MEDICINE_TYPE> list = new List<HIS_ADR_MEDICINE_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_ADR_MEDICINE_TYPE.AsNoTracking().AsQueryable();
                        if (search.listHisAdrMedicineTypeExpression != null && search.listHisAdrMedicineTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisAdrMedicineTypeExpression)
                            {
        ...
        public HIS_ADR_MEDICINE_TYPE GetById(long id, HisAdrMedicineTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_ADR_MEDICINE_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisAdrMedicineTypeExpression != null && search.listHisAdrMedicineTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisAdrMedicineTypeExpression)
                            {
