...
namespace MOS.DAO.HisExpiredDateCfg
{
    partial class HisExpiredDateCfgGet : EntityBase
    {
        ...
        {
            List<HIS_EXPIRED_DATE_CFG> list = new List<HIS_EXPIRED_DATE_CFG>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_EXPIRED_DATE_CFG.AsNoTracking().AsQueryable();
                        if (search.listHisExpiredDateCfgExpression != null && search.listHisExpiredDateCfgExpression.Count > 0)
                        {
                            foreach (var item in search.listHisExpiredDateCfgExpression)
                            {
        ...
        public HIS_EXPIRED_DATE_CFG GetById(long id, HisExpiredDateCfgSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_EXPIRED_DATE_CFG.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisExpiredDateCfgExpression != null && search.listHisExpiredDateCfgExpression.Count > 0)
                        {
                            foreach (var item in search.listHisExpiredDateCfgExpression)
                            {
