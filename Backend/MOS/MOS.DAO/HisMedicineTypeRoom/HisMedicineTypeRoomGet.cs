...
namespace MOS.DAO.HisMedicineTypeRoom
{
    partial class HisMedicineTypeRoomGet : EntityBase
    {
        ...
        {
            List<HIS_MEDICINE_TYPE_ROOM> list = new List<HIS_MEDICINE_TYPE_ROOM>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MEDICINE_TYPE_ROOM.AsNoTracking().AsQueryable();
        ...
        public HIS_MEDICINE_TYPE_ROOM GetById(long id, HisMedicineTypeRoomSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_MEDICINE_TYPE_ROOM.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMedicineTypeRoomExpression != null && search.listHisMedicineTypeRoomExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMedicineTypeRoomExpression)
                            {
