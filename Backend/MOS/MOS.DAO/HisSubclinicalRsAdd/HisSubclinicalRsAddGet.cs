...
namespace MOS.DAO.HisSubclinicalRsAdd
{
    partial class HisSubclinicalRsAddGet : EntityBase
    {
        ...
        {
            List<HIS_SUBCLINICAL_RS_ADD> list = new List<HIS_SUBCLINICAL_RS_ADD>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SUBCLINICAL_RS_ADD.AsNoTracking().AsQueryable();
                        if (search.listHisSubclinicalRsAddExpression != null && search.listHisSubclinicalRsAddExpression.Count > 0)
                        {
                            foreach (var item in search.listHisSubclinicalRsAddExpression)
                            {
        ...
        public HIS_SUBCLINICAL_RS_ADD GetById(long id, HisSubclinicalRsAddSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_SUBCLINICAL_RS_ADD.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisSubclinicalRsAddExpression != null && search.listHisSubclinicalRsAddExpression.Count > 0)
                        {
                            foreach (var item in search.listHisSubclinicalRsAddExpression)
                            {
