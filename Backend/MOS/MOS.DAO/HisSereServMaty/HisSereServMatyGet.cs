...
namespace MOS.DAO.HisSereServMaty
{
    partial class HisSereServMatyGet : EntityBase
    {
        ...
        {
            List<HIS_SERE_SERV_MATY> list = new List<HIS_SERE_SERV_MATY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SERE_SERV_MATY.AsNoTracking().AsQueryable();
        ...
        public HIS_SERE_SERV_MATY GetById(long id, HisSereServMatySO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_SERE_SERV_MATY.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisSereServMatyExpression != null && search.listHisSereServMatyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisSereServMatyExpression)
                            {
