...
namespace MOS.DAO.HisSereServMaty
{
    partial class HisSereServMatyGet : EntityBase
    {
        ...
        {
            List<V_HIS_SERE_SERV_MATY> list = new List<V_HIS_SERE_SERV_MATY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_SERE_SERV_MATY.AsNoTracking().AsQueryable();
                        if (search.listVHisSereServMatyExpression != null && search.listVHisSereServMatyExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisSereServMatyExpression)
                            {
