...
{
    partial class HisServiceUnitDelete : EntityBase
    {
        public HisServiceUnitDelete()
            : base()
        {
        ...
        public bool Delete(HIS_SERVICE_UNIT data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_SERVICE_UNIT> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_SERVICE_UNIT> listIdNotFound = new List<HIS_SERVICE_UNIT>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_SERVICE_UNIT> listRaw = ctx.HIS_SERVICE_UNIT.Where(o => listId.Contains(o.ID)).ToList();
