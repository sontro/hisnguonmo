...
namespace MOS.DAO.HisExroRoom
{
    partial class HisExroRoomGet : EntityBase
    {
        ...
        {
            List<HIS_EXRO_ROOM> list = new List<HIS_EXRO_ROOM>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_EXRO_ROOM.AsNoTracking().AsQueryable();
                        if (search.listHisExroRoomExpression != null && search.listHisExroRoomExpression.Count > 0)
                        {
                            foreach (var item in search.listHisExroRoomExpression)
                            {
        ...
        public HIS_EXRO_ROOM GetById(long id, HisExroRoomSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_EXRO_ROOM.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisExroRoomExpression != null && search.listHisExroRoomExpression.Count > 0)
                        {
                            foreach (var item in search.listHisExroRoomExpression)
                            {
