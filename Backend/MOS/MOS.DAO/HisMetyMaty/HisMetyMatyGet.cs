...
namespace MOS.DAO.HisMetyMaty
{
    ...
    {
        public List<HIS_METY_MATY> Get(HisMetyMatySO search, CommonParam param)
        {
            List<HIS_METY_MATY> list = new List<HIS_METY_MATY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_METY_MATY.AsNoTracking().AsQueryable();
        ...
        public HIS_METY_MATY GetById(long id, HisMetyMatySO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_METY_MATY.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMetyMatyExpression != null && search.listHisMetyMatyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMetyMatyExpression)
                            {
