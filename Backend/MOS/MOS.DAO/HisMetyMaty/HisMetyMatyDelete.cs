...
{
    partial class HisMetyMatyDelete : EntityBase
    {
        public HisMetyMatyDelete()
            : base()
        {
        ...
        public bool Delete(HIS_METY_MATY data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_METY_MATY> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_METY_MATY> listIdNotFound = new List<HIS_METY_MATY>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_METY_MATY> listRaw = ctx.HIS_METY_MATY.Where(o => listId.Contains(o.ID)).ToList();
