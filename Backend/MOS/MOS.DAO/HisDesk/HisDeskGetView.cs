...
namespace MOS.DAO.HisDesk
{
    partial class HisDeskGet : EntityBase
    {
        ...
        {
            List<V_HIS_DESK> list = new List<V_HIS_DESK>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_DESK.AsNoTracking().AsQueryable();
                        if (search.listVHisDeskExpression != null && search.listVHisDeskExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisDeskExpression)
                            {
