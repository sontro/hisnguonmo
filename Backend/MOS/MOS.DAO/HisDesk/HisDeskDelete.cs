...
{
    partial class HisDeskDelete : EntityBase
    {
        public HisDeskDelete()
            : base()
        {
        ...
        public bool Delete(HIS_DESK data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_DESK> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_DESK> listIdNotFound = new List<HIS_DESK>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_DESK> listRaw = ctx.HIS_DESK.Where(o => listId.Contains(o.ID)).ToList();
