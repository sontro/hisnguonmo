...
{
    partial class HisDeskGet : EntityBase
    {
        public List<HIS_DESK> Get(HisDeskSO search, CommonParam param)
        {
            List<HIS_DESK> list = new List<HIS_DESK>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_DESK.AsNoTracking().AsQueryable();
                        if (search.listHisDeskExpression != null && search.listHisDeskExpression.Count > 0)
                        {
                            foreach (var item in search.listHisDeskExpression)
                            {
        ...
        public HIS_DESK GetById(long id, HisDeskSO search)
        {
            HIS_DESK result = null;
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_DESK.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisDeskExpression != null && search.listHisDeskExpression.Count > 0)
                        {
                            foreach (var item in search.listHisDeskExpression)
                            {
