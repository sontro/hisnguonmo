...
namespace MOS.DAO.HisBhytBlacklist
{
    partial class HisBhytBlacklistGet : EntityBase
    {
        ...
        {
            List<HIS_BHYT_BLACKLIST> list = new List<HIS_BHYT_BLACKLIST>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_BHYT_BLACKLIST.AsNoTracking().AsQueryable();
        ...
        public HIS_BHYT_BLACKLIST GetById(long id, HisBhytBlacklistSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_BHYT_BLACKLIST.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisBhytBlacklistExpression != null && search.listHisBhytBlacklistExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBhytBlacklistExpression)
                            {
