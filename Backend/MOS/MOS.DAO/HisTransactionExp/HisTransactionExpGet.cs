...
namespace MOS.DAO.HisTransactionExp
{
    partial class HisTransactionExpGet : EntityBase
    {
        ...
        {
            List<HIS_TRANSACTION_EXP> list = new List<HIS_TRANSACTION_EXP>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_TRANSACTION_EXP.AsNoTracking().AsQueryable();
        ...
        public HIS_TRANSACTION_EXP GetById(long id, HisTransactionExpSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_TRANSACTION_EXP.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisTransactionExpExpression != null && search.listHisTransactionExpExpression.Count > 0)
                        {
                            foreach (var item in search.listHisTransactionExpExpression)
                            {
