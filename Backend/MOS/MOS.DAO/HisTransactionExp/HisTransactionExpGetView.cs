...
namespace MOS.DAO.HisTransactionExp
{
    partial class HisTransactionExpGet : EntityBase
    {
        ...
        {
            List<V_HIS_TRANSACTION_EXP> list = new List<V_HIS_TRANSACTION_EXP>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_TRANSACTION_EXP.AsNoTracking().AsQueryable();
                        if (search.listVHisTransactionExpExpression != null && search.listVHisTransactionExpExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisTransactionExpExpression)
                            {
