...
{
    partial class HisProcessingMethodDelete : EntityBase
    {
        public HisProcessingMethodDelete()
            : base()
        {
        ...
        public bool Delete(HIS_PROCESSING_METHOD data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_PROCESSING_METHOD> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_PROCESSING_METHOD> listIdNotFound = new List<HIS_PROCESSING_METHOD>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_PROCESSING_METHOD> listRaw = ctx.HIS_PROCESSING_METHOD.Where(o => listId.Contains(o.ID)).ToList();
