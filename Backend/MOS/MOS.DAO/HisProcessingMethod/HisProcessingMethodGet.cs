...
namespace MOS.DAO.HisProcessingMethod
{
    ...
    {
        public List<HIS_PROCESSING_METHOD> Get(HisProcessingMethodSO search, CommonParam param)
        {
            List<HIS_PROCESSING_METHOD> list = new List<HIS_PROCESSING_METHOD>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_PROCESSING_METHOD.AsNoTracking().AsQueryable();
                        if (search.listHisProcessingMethodExpression != null && search.listHisProcessingMethodExpression.Count > 0)
                        {
                            foreach (var item in search.listHisProcessingMethodExpression)
                            {
        ...
        public HIS_PROCESSING_METHOD GetById(long id, HisProcessingMethodSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_PROCESSING_METHOD.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisProcessingMethodExpression != null && search.listHisProcessingMethodExpression.Count > 0)
                        {
                            foreach (var item in search.listHisProcessingMethodExpression)
                            {
