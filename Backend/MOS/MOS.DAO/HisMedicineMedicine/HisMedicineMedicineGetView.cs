...
{
    partial class HisMedicineMedicineGet : EntityBase
    {
        public List<V_HIS_MEDICINE_MEDICINE> GetView(HisMedicineMedicineSO search, CommonParam param)
        {
            List<V_HIS_MEDICINE_MEDICINE> list = new List<V_HIS_MEDICINE_MEDICINE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_MEDICINE_MEDICINE.AsNoTracking().AsQueryable();
                        if (search.listVHisMedicineMedicineExpression != null && search.listVHisMedicineMedicineExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisMedicineMedicineExpression)
                            {
