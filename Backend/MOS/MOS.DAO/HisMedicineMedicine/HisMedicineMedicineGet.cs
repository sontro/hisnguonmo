...
namespace MOS.DAO.HisMedicineMedicine
{
    partial class HisMedicineMedicineGet : EntityBase
    {
        ...
        {
            List<HIS_MEDICINE_MEDICINE> list = new List<HIS_MEDICINE_MEDICINE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MEDICINE_MEDICINE.AsNoTracking().AsQueryable();
                        if (search.listHisMedicineMedicineExpression != null && search.listHisMedicineMedicineExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMedicineMedicineExpression)
                            {
        ...
        public HIS_MEDICINE_MEDICINE GetById(long id, HisMedicineMedicineSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_MEDICINE_MEDICINE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMedicineMedicineExpression != null && search.listHisMedicineMedicineExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMedicineMedicineExpression)
                            {
