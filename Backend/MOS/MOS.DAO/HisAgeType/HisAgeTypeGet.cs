...
namespace MOS.DAO.HisAgeType
{
    ...
    {
        public List<HIS_AGE_TYPE> Get(HisAgeTypeSO search, CommonParam param)
        {
            List<HIS_AGE_TYPE> list = new List<HIS_AGE_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_AGE_TYPE.AsNoTracking().AsQueryable();
                        if (search.listHisAgeTypeExpression != null && search.listHisAgeTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisAgeTypeExpression)
                            {
        ...
        public HIS_AGE_TYPE GetById(long id, HisAgeTypeSO search)
        {
            HIS_AGE_TYPE result = null;
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_AGE_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisAgeTypeExpression != null && search.listHisAgeTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisAgeTypeExpression)
                            {
