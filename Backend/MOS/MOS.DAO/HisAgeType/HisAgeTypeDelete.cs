...
{
    partial class HisAgeTypeDelete : EntityBase
    {
        public HisAgeTypeDelete()
            : base()
        {
        ...
        public bool Delete(HIS_AGE_TYPE data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_AGE_TYPE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_AGE_TYPE> listIdNotFound = new List<HIS_AGE_TYPE>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_AGE_TYPE> listRaw = ctx.HIS_AGE_TYPE.Where(o => listId.Contains(o.ID)).ToList();
