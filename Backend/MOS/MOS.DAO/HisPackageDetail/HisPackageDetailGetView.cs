...
namespace MOS.DAO.HisPackageDetail
{
    ...
    {
        public List<V_HIS_PACKAGE_DETAIL> GetView(HisPackageDetailSO search, CommonParam param)
        {
            List<V_HIS_PACKAGE_DETAIL> list = new List<V_HIS_PACKAGE_DETAIL>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_PACKAGE_DETAIL.AsNoTracking().AsQueryable();
                        if (search.listVHisPackageDetailExpression != null && search.listVHisPackageDetailExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisPackageDetailExpression)
                            {
