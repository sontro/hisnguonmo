...
{
    partial class HisPackageDetailDelete : EntityBase
    {
        public HisPackageDetailDelete()
            : base()
        {
        ...
        public bool Delete(HIS_PACKAGE_DETAIL data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_PACKAGE_DETAIL> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_PACKAGE_DETAIL> listIdNotFound = new List<HIS_PACKAGE_DETAIL>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_PACKAGE_DETAIL> listRaw = ctx.HIS_PACKAGE_DETAIL.Where(o => listId.Contains(o.ID)).ToList();
