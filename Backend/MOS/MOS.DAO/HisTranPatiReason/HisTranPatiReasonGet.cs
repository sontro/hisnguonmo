...
namespace MOS.DAO.HisTranPatiReason
{
    partial class HisTranPatiReasonGet : EntityBase
    {
        ...
        {
            List<HIS_TRAN_PATI_REASON> list = new List<HIS_TRAN_PATI_REASON>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_TRAN_PATI_REASON.AsNoTracking().AsQueryable();
        ...
        public HIS_TRAN_PATI_REASON GetById(long id, HisTranPatiReasonSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_TRAN_PATI_REASON.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisTranPatiReasonExpression != null && search.listHisTranPatiReasonExpression.Count > 0)
                        {
                            foreach (var item in search.listHisTranPatiReasonExpression)
                            {
