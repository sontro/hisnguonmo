...
namespace MOS.DAO.HisReportTypeCat
{
    partial class HisReportTypeCatGet : EntityBase
    {
        ...
        {
            List<HIS_REPORT_TYPE_CAT> list = new List<HIS_REPORT_TYPE_CAT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_REPORT_TYPE_CAT.AsNoTracking().AsQueryable();
                        if (search.listHisReportTypeCatExpression != null && search.listHisReportTypeCatExpression.Count > 0)
                        {
                            foreach (var item in search.listHisReportTypeCatExpression)
                            {
        ...
        public HIS_REPORT_TYPE_CAT GetById(long id, HisReportTypeCatSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_REPORT_TYPE_CAT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisReportTypeCatExpression != null && search.listHisReportTypeCatExpression.Count > 0)
                        {
                            foreach (var item in search.listHisReportTypeCatExpression)
                            {
