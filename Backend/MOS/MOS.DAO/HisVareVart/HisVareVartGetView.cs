...
namespace MOS.DAO.HisVareVart
{
    partial class HisVareVartGet : EntityBase
    {
        ...
        {
            List<V_HIS_VARE_VART> list = new List<V_HIS_VARE_VART>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_VARE_VART.AsNoTracking().AsQueryable();
                        if (search.listVHisVareVartExpression != null && search.listVHisVareVartExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisVareVartExpression)
                            {
