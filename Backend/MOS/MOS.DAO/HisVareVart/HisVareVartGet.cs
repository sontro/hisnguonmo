...
{
    partial class HisVareVartGet : EntityBase
    {
        public List<HIS_VARE_VART> Get(HisVareVartSO search, CommonParam param)
        {
            List<HIS_VARE_VART> list = new List<HIS_VARE_VART>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_VARE_VART.AsNoTracking().AsQueryable();
                        if (search.listHisVareVartExpression != null && search.listHisVareVartExpression.Count > 0)
                        {
                            foreach (var item in search.listHisVareVartExpression)
                            {
        ...
        public HIS_VARE_VART GetById(long id, HisVareVartSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_VARE_VART.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisVareVartExpression != null && search.listHisVareVartExpression.Count > 0)
                        {
                            foreach (var item in search.listHisVareVartExpression)
                            {
