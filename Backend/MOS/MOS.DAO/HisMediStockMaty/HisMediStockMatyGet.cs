...
namespace MOS.DAO.HisMediStockMaty
{
    partial class HisMediStockMatyGet : EntityBase
    {
        ...
        {
            List<HIS_MEDI_STOCK_MATY> list = new List<HIS_MEDI_STOCK_MATY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MEDI_STOCK_MATY.AsNoTracking().AsQueryable();
                        if (search.listHisMediStockMatyExpression != null && search.listHisMediStockMatyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMediStockMatyExpression)
                            {
        ...
        public HIS_MEDI_STOCK_MATY GetById(long id, HisMediStockMatySO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_MEDI_STOCK_MATY.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMediStockMatyExpression != null && search.listHisMediStockMatyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMediStockMatyExpression)
                            {
