...
namespace MOS.DAO.HisMediStockMaty
{
    partial class HisMediStockMatyGet : EntityBase
    {
        ...
        {
            List<V_HIS_MEDI_STOCK_MATY_1> list = new List<V_HIS_MEDI_STOCK_MATY_1>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_MEDI_STOCK_MATY_1.AsNoTracking().AsQueryable();
                        if (search.listVHisMediStockMaty1Expression != null && search.listVHisMediStockMaty1Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisMediStockMaty1Expression)
                            {
