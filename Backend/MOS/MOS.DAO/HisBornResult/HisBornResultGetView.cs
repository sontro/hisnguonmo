...
namespace MOS.DAO.HisBornResult
{
    partial class HisBornResultGet : EntityBase
    {
        ...
        {
            List<V_HIS_BORN_RESULT> list = new List<V_HIS_BORN_RESULT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_BORN_RESULT.AsQueryable();
                        if (search.listVHisBornResultExpression != null && search.listVHisBornResultExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisBornResultExpression)
                            {
