...
{
    partial class HisBornResultDelete : EntityBase
    {
        public HisBornResultDelete()
            : base()
        {
        ...
        public bool Delete(HIS_BORN_RESULT data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_BORN_RESULT> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_BORN_RESULT> listIdNotFound = new List<HIS_BORN_RESULT>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_BORN_RESULT> listRaw = ctx.HIS_BORN_RESULT.Where(o => listId.Contains(o.ID)).ToList();
