...
namespace MOS.DAO.HisObeyContraindi
{
    ...
    {
        public List<V_HIS_OBEY_CONTRAINDI> GetView(HisObeyContraindiSO search, CommonParam param)
        {
            List<V_HIS_OBEY_CONTRAINDI> list = new List<V_HIS_OBEY_CONTRAINDI>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_OBEY_CONTRAINDI.AsNoTracking().AsQueryable();
