...
namespace MOS.DAO.HisObeyContraindi
{
    partial class HisObeyContraindiGet : EntityBase
    {
        ...
        {
            List<HIS_OBEY_CONTRAINDI> list = new List<HIS_OBEY_CONTRAINDI>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_OBEY_CONTRAINDI.AsNoTracking().AsQueryable();
        ...
        public HIS_OBEY_CONTRAINDI GetById(long id, HisObeyContraindiSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_OBEY_CONTRAINDI.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisObeyContraindiExpression != null && search.listHisObeyContraindiExpression.Count > 0)
                        {
                            foreach (var item in search.listHisObeyContraindiExpression)
                            {
