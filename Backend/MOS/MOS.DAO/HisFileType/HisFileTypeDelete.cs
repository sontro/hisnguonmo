...
{
    partial class HisFileTypeDelete : EntityBase
    {
        public HisFileTypeDelete()
            : base()
        {
        ...
        public bool Delete(HIS_FILE_TYPE data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_FILE_TYPE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_FILE_TYPE> listIdNotFound = new List<HIS_FILE_TYPE>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_FILE_TYPE> listRaw = ctx.HIS_FILE_TYPE.Where(o => listId.Contains(o.ID)).ToList();
