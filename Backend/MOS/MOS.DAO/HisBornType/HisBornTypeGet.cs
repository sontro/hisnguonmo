...
{
    partial class HisBornTypeGet : EntityBase
    {
        public List<HIS_BORN_TYPE> Get(HisBornTypeSO search, CommonParam param)
        {
            List<HIS_BORN_TYPE> list = new List<HIS_BORN_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_BORN_TYPE.AsNoTracking().AsQueryable();
                        if (search.listHisBornTypeExpression != null && search.listHisBornTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBornTypeExpression)
                            {
        ...
        public HIS_BORN_TYPE GetById(long id, HisBornTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_BORN_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisBornTypeExpression != null && search.listHisBornTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBornTypeExpression)
                            {
