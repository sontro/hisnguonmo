...
{
    partial class HisUserRoomDelete : EntityBase
    {
        public HisUserRoomDelete()
            : base()
        {
        ...
        public bool Delete(HIS_USER_ROOM data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_USER_ROOM> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_USER_ROOM> listIdNotFound = new List<HIS_USER_ROOM>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_USER_ROOM> listRaw = ctx.HIS_USER_ROOM.Where(o => listId.Contains(o.ID)).ToList();
