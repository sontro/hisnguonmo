...
namespace MOS.DAO.HisCancelReason
{
    partial class HisCancelReasonGet : EntityBase
    {
        ...
        {
            List<HIS_CANCEL_REASON> list = new List<HIS_CANCEL_REASON>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_CANCEL_REASON.AsNoTracking().AsQueryable();
        ...
        public HIS_CANCEL_REASON GetById(long id, HisCancelReasonSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_CANCEL_REASON.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisCancelReasonExpression != null && search.listHisCancelReasonExpression.Count > 0)
                        {
                            foreach (var item in search.listHisCancelReasonExpression)
                            {
