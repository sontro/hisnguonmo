...
{
    partial class HisDataStoreDelete : EntityBase
    {
        public HisDataStoreDelete()
            : base()
        {
        ...
        public bool Delete(HIS_DATA_STORE data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_DATA_STORE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_DATA_STORE> listIdNotFound = new List<HIS_DATA_STORE>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_DATA_STORE> listRaw = ctx.HIS_DATA_STORE.Where(o => listId.Contains(o.ID)).ToList();
