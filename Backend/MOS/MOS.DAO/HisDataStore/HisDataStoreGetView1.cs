...
namespace MOS.DAO.HisDataStore
{
    partial class HisDataStoreGet : EntityBase
    {
        ...
        {
            List<V_HIS_DATA_STORE_1> list = new List<V_HIS_DATA_STORE_1>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_DATA_STORE_1.AsNoTracking().AsQueryable();
                        if (search.listVHisDataStore1Expression != null && search.listVHisDataStore1Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisDataStore1Expression)
                            {
