...
namespace MOS.DAO.HisDataStore
{
    partial class HisDataStoreGet : EntityBase
    {
        ...
        {
            List<HIS_DATA_STORE> list = new List<HIS_DATA_STORE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_DATA_STORE.AsNoTracking().AsQueryable();
                        if (search.listHisDataStoreExpression != null && search.listHisDataStoreExpression.Count > 0)
                        {
                            foreach (var item in search.listHisDataStoreExpression)
                            {
        ...
        public HIS_DATA_STORE GetById(long id, HisDataStoreSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_DATA_STORE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisDataStoreExpression != null && search.listHisDataStoreExpression.Count > 0)
                        {
                            foreach (var item in search.listHisDataStoreExpression)
                            {
