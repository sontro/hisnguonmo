...
namespace MOS.DAO.HisDeathCertBook
{
    partial class HisDeathCertBookGet : EntityBase
    {
        ...
        {
            List<HIS_DEATH_CERT_BOOK> list = new List<HIS_DEATH_CERT_BOOK>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_DEATH_CERT_BOOK.AsNoTracking().AsQueryable();
        ...
        public HIS_DEATH_CERT_BOOK GetById(long id, HisDeathCertBookSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_DEATH_CERT_BOOK.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisDeathCertBookExpression != null && search.listHisDeathCertBookExpression.Count > 0)
                        {
                            foreach (var item in search.listHisDeathCertBookExpression)
                            {
