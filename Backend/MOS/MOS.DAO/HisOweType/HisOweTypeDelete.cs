...
{
    partial class HisOweTypeDelete : EntityBase
    {
        public HisOweTypeDelete()
            : base()
        {
        ...
        public bool Delete(HIS_OWE_TYPE data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_OWE_TYPE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_OWE_TYPE> listIdNotFound = new List<HIS_OWE_TYPE>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_OWE_TYPE> listRaw = ctx.HIS_OWE_TYPE.Where(o => listId.Contains(o.ID)).ToList();
