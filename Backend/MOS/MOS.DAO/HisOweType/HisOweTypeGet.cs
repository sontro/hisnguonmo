...
namespace MOS.DAO.HisOweType
{
    partial class HisOweTypeGet : EntityBase
    {
        ...
        {
            List<HIS_OWE_TYPE> list = new List<HIS_OWE_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_OWE_TYPE.AsNoTracking().AsQueryable();
                        if (search.listHisOweTypeExpression != null && search.listHisOweTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisOweTypeExpression)
                            {
        ...
        public HIS_OWE_TYPE GetById(long id, HisOweTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_OWE_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisOweTypeExpression != null && search.listHisOweTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisOweTypeExpression)
                            {
