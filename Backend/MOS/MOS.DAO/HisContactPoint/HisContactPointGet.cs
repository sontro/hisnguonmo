...
namespace MOS.DAO.HisContactPoint
{
    partial class HisContactPointGet : EntityBase
    {
        ...
        {
            List<HIS_CONTACT_POINT> list = new List<HIS_CONTACT_POINT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_CONTACT_POINT.AsNoTracking().AsQueryable();
                        if (search.listHisContactPointExpression != null && search.listHisContactPointExpression.Count > 0)
                        {
                            foreach (var item in search.listHisContactPointExpression)
                            {
        ...
        public HIS_CONTACT_POINT GetById(long id, HisContactPointSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_CONTACT_POINT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisContactPointExpression != null && search.listHisContactPointExpression.Count > 0)
                        {
                            foreach (var item in search.listHisContactPointExpression)
                            {
