...
{
    partial class HisContactPointDelete : EntityBase
    {
        public HisContactPointDelete()
            : base()
        {
        ...
        public bool Delete(HIS_CONTACT_POINT data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_CONTACT_POINT> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_CONTACT_POINT> listIdNotFound = new List<HIS_CONTACT_POINT>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_CONTACT_POINT> listRaw = ctx.HIS_CONTACT_POINT.Where(o => listId.Contains(o.ID)).ToList();
