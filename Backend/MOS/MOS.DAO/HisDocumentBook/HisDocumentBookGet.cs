...
namespace MOS.DAO.HisDocumentBook
{
    partial class HisDocumentBookGet : EntityBase
    {
        ...
        {
            List<HIS_DOCUMENT_BOOK> list = new List<HIS_DOCUMENT_BOOK>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_DOCUMENT_BOOK.AsNoTracking().AsQueryable();
                        if (search.listHisDocumentBookExpression != null && search.listHisDocumentBookExpression.Count > 0)
                        {
                            foreach (var item in search.listHisDocumentBookExpression)
                            {
        ...
        public HIS_DOCUMENT_BOOK GetById(long id, HisDocumentBookSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_DOCUMENT_BOOK.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisDocumentBookExpression != null && search.listHisDocumentBookExpression.Count > 0)
                        {
                            foreach (var item in search.listHisDocumentBookExpression)
                            {
