...
{
    partial class HisDocumentBookDelete : EntityBase
    {
        public HisDocumentBookDelete()
            : base()
        {
        ...
        public bool Delete(HIS_DOCUMENT_BOOK data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_DOCUMENT_BOOK> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_DOCUMENT_BOOK> listIdNotFound = new List<HIS_DOCUMENT_BOOK>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_DOCUMENT_BOOK> listRaw = ctx.HIS_DOCUMENT_BOOK.Where(o => listId.Contains(o.ID)).ToList();
