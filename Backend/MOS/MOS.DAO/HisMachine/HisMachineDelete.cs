...
{
    partial class HisMachineDelete : EntityBase
    {
        public HisMachineDelete()
            : base()
        {
        ...
        public bool Delete(HIS_MACHINE data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_MACHINE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_MACHINE> listIdNotFound = new List<HIS_MACHINE>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_MACHINE> listRaw = ctx.HIS_MACHINE.Where(o => listId.Contains(o.ID)).ToList();
