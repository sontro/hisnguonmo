...
namespace MOS.DAO.HisServiceMachine
{
    partial class HisServiceMachineGet : EntityBase
    {
        ...
        {
            List<HIS_SERVICE_MACHINE> list = new List<HIS_SERVICE_MACHINE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SERVICE_MACHINE.AsNoTracking().AsQueryable();
                        if (search.listHisServiceMachineExpression != null && search.listHisServiceMachineExpression.Count > 0)
                        {
                            foreach (var item in search.listHisServiceMachineExpression)
                            {
        ...
        public HIS_SERVICE_MACHINE GetById(long id, HisServiceMachineSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_SERVICE_MACHINE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisServiceMachineExpression != null && search.listHisServiceMachineExpression.Count > 0)
                        {
                            foreach (var item in search.listHisServiceMachineExpression)
                            {
