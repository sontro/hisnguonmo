...
namespace MOS.DAO.HisAntibioticRequest
{
    partial class HisAntibioticRequestGet : EntityBase
    {
        ...
        {
            List<HIS_ANTIBIOTIC_REQUEST> list = new List<HIS_ANTIBIOTIC_REQUEST>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_ANTIBIOTIC_REQUEST.AsNoTracking().AsQueryable();
                        if (search.listHisAntibioticRequestExpression != null && search.listHisAntibioticRequestExpression.Count > 0)
                        {
                            foreach (var item in search.listHisAntibioticRequestExpression)
                            {
        ...
        public HIS_ANTIBIOTIC_REQUEST GetById(long id, HisAntibioticRequestSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_ANTIBIOTIC_REQUEST.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisAntibioticRequestExpression != null && search.listHisAntibioticRequestExpression.Count > 0)
                        {
                            foreach (var item in search.listHisAntibioticRequestExpression)
                            {
