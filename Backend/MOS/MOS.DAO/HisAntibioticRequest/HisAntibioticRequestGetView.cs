...
namespace MOS.DAO.HisAntibioticRequest
{
    partial class HisAntibioticRequestGet : EntityBase
    {
        ...
        {
            List<V_HIS_ANTIBIOTIC_REQUEST> list = new List<V_HIS_ANTIBIOTIC_REQUEST>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_ANTIBIOTIC_REQUEST.AsNoTracking().AsQueryable();
                        if (search.listVHisAntibioticRequestExpression != null && search.listVHisAntibioticRequestExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisAntibioticRequestExpression)
                            {
