...
namespace MOS.DAO.HisQcType
{
    partial class HisQcTypeGet : EntityBase
    {
        ...
        {
            List<V_HIS_QC_TYPE> list = new List<V_HIS_QC_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_QC_TYPE.AsNoTracking().AsQueryable();
                        if (search.listVHisQcTypeExpression != null && search.listVHisQcTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisQcTypeExpression)
                            {
