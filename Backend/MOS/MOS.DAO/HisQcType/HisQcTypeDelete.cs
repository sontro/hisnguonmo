...
{
    partial class HisQcTypeDelete : EntityBase
    {
        public HisQcTypeDelete()
            : base()
        {
        ...
        public bool Delete(HIS_QC_TYPE data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_QC_TYPE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_QC_TYPE> listIdNotFound = new List<HIS_QC_TYPE>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_QC_TYPE> listRaw = ctx.HIS_QC_TYPE.Where(o => listId.Contains(o.ID)).ToList();
