...
{
    partial class HisQcTypeGet : EntityBase
    {
        public List<HIS_QC_TYPE> Get(HisQcTypeSO search, CommonParam param)
        {
            List<HIS_QC_TYPE> list = new List<HIS_QC_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_QC_TYPE.AsNoTracking().AsQueryable();
                        if (search.listHisQcTypeExpression != null && search.listHisQcTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisQcTypeExpression)
                            {
        ...
        public HIS_QC_TYPE GetById(long id, HisQcTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_QC_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisQcTypeExpression != null && search.listHisQcTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisQcTypeExpression)
                            {
