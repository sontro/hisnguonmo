...
{
    partial class HisTrackingDelete : EntityBase
    {
        public HisTrackingDelete()
            : base()
        {
        ...
        public bool Delete(HIS_TRACKING data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_TRACKING> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_TRACKING> listIdNotFound = new List<HIS_TRACKING>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_TRACKING> listRaw = ctx.HIS_TRACKING.Where(o => listId.Contains(o.ID)).ToList();
