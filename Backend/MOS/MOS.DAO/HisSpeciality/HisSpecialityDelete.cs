...
{
    partial class HisSpecialityDelete : EntityBase
    {
        public HisSpecialityDelete()
            : base()
        {
        ...
        public bool Delete(HIS_SPECIALITY data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_SPECIALITY> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_SPECIALITY> listIdNotFound = new List<HIS_SPECIALITY>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_SPECIALITY> listRaw = ctx.HIS_SPECIALITY.Where(o => listId.Contains(o.ID)).ToList();
