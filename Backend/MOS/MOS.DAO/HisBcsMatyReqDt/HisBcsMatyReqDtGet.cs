...
{
    partial class HisBcsMatyReqDtGet : EntityBase
    {
        public List<HIS_BCS_MATY_REQ_DT> Get(HisBcsMatyReqDtSO search, CommonParam param)
        {
            List<HIS_BCS_MATY_REQ_DT> list = new List<HIS_BCS_MATY_REQ_DT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_BCS_MATY_REQ_DT.AsNoTracking().AsQueryable();
                        if (search.listHisBcsMatyReqDtExpression != null && search.listHisBcsMatyReqDtExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBcsMatyReqDtExpression)
                            {
        ...
        public HIS_BCS_MATY_REQ_DT GetById(long id, HisBcsMatyReqDtSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_BCS_MATY_REQ_DT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisBcsMatyReqDtExpression != null && search.listHisBcsMatyReqDtExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBcsMatyReqDtExpression)
                            {
