...
namespace MOS.DAO.HisBcsMatyReqDt
{
    partial class HisBcsMatyReqDtGet : EntityBase
    {
        ...
        {
            List<V_HIS_BCS_MATY_REQ_DT> list = new List<V_HIS_BCS_MATY_REQ_DT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_BCS_MATY_REQ_DT.AsNoTracking().AsQueryable();
                        if (search.listVHisBcsMatyReqDtExpression != null && search.listVHisBcsMatyReqDtExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisBcsMatyReqDtExpression)
                            {
