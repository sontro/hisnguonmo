...
{
    partial class HisMestMatyDepaGet : EntityBase
    {
        public List<V_HIS_MEST_MATY_DEPA> GetView(HisMestMatyDepaSO search, CommonParam param)
        {
            List<V_HIS_MEST_MATY_DEPA> list = new List<V_HIS_MEST_MATY_DEPA>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_MEST_MATY_DEPA.AsNoTracking().AsQueryable();
                        if (search.listVHisMestMatyDepaExpression != null && search.listVHisMestMatyDepaExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisMestMatyDepaExpression)
                            {
