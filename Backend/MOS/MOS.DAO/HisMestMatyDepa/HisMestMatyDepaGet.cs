...
namespace MOS.DAO.HisMestMatyDepa
{
    partial class HisMestMatyDepaGet : EntityBase
    {
        ...
        {
            List<HIS_MEST_MATY_DEPA> list = new List<HIS_MEST_MATY_DEPA>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MEST_MATY_DEPA.AsNoTracking().AsQueryable();
                        if (search.listHisMestMatyDepaExpression != null && search.listHisMestMatyDepaExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMestMatyDepaExpression)
                            {
        ...
        public HIS_MEST_MATY_DEPA GetById(long id, HisMestMatyDepaSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_MEST_MATY_DEPA.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMestMatyDepaExpression != null && search.listHisMestMatyDepaExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMestMatyDepaExpression)
                            {
