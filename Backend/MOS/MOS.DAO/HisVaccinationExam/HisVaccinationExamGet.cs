...
namespace MOS.DAO.HisVaccinationExam
{
    partial class HisVaccinationExamGet : EntityBase
    {
        ...
        {
            List<HIS_VACCINATION_EXAM> list = new List<HIS_VACCINATION_EXAM>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_VACCINATION_EXAM.AsNoTracking().AsQueryable();
        ...
        public HIS_VACCINATION_EXAM GetById(long id, HisVaccinationExamSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_VACCINATION_EXAM.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisVaccinationExamExpression != null && search.listHisVaccinationExamExpression.Count > 0)
                        {
                            foreach (var item in search.listHisVaccinationExamExpression)
                            {
