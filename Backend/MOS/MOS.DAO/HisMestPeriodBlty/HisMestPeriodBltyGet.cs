...
namespace MOS.DAO.HisMestPeriodBlty
{
    partial class HisMestPeriodBltyGet : EntityBase
    {
        ...
        {
            List<HIS_MEST_PERIOD_BLTY> list = new List<HIS_MEST_PERIOD_BLTY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MEST_PERIOD_BLTY.AsNoTracking().AsQueryable();
                        if (search.listHisMestPeriodBltyExpression != null && search.listHisMestPeriodBltyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMestPeriodBltyExpression)
                            {
        ...
        public HIS_MEST_PERIOD_BLTY GetById(long id, HisMestPeriodBltySO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_MEST_PERIOD_BLTY.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMestPeriodBltyExpression != null && search.listHisMestPeriodBltyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMestPeriodBltyExpression)
                            {
