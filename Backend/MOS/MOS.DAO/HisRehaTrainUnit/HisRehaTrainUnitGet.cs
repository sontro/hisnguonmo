...
namespace MOS.DAO.HisRehaTrainUnit
{
    partial class HisRehaTrainUnitGet : EntityBase
    {
        ...
        {
            List<HIS_REHA_TRAIN_UNIT> list = new List<HIS_REHA_TRAIN_UNIT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_REHA_TRAIN_UNIT.AsNoTracking().AsQueryable();
        ...
        public HIS_REHA_TRAIN_UNIT GetById(long id, HisRehaTrainUnitSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_REHA_TRAIN_UNIT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisRehaTrainUnitExpression != null && search.listHisRehaTrainUnitExpression.Count > 0)
                        {
                            foreach (var item in search.listHisRehaTrainUnitExpression)
                            {
