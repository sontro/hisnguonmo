...
namespace MOS.DAO.HisRehaTrainUnit
{
    partial class HisRehaTrainUnitGet : EntityBase
    {
        ...
        {
            List<V_HIS_REHA_TRAIN_UNIT> list = new List<V_HIS_REHA_TRAIN_UNIT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_REHA_TRAIN_UNIT.AsQueryable();
                        if (search.listVHisRehaTrainUnitExpression != null && search.listVHisRehaTrainUnitExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisRehaTrainUnitExpression)
                            {
