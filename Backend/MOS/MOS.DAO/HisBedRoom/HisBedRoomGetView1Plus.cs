...
namespace MOS.DAO.HisBedRoom
{
    ...
    {
        public List<V_HIS_BED_ROOM_1> GetView1(HisBedRoomSO search, CommonParam param)
        {
            List<V_HIS_BED_ROOM_1> list = new List<V_HIS_BED_ROOM_1>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_BED_ROOM_1.AsNoTracking().AsQueryable();
