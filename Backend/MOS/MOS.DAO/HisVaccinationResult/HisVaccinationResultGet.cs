...
{
    partial class HisVaccinationResultGet : EntityBase
    {
        public List<HIS_VACCINATION_RESULT> Get(HisVaccinationResultSO search, CommonParam param)
        {
            List<HIS_VACCINATION_RESULT> list = new List<HIS_VACCINATION_RESULT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_VACCINATION_RESULT.AsNoTracking().AsQueryable();
                        if (search.listHisVaccinationResultExpression != null && search.listHisVaccinationResultExpression.Count > 0)
                        {
                            foreach (var item in search.listHisVaccinationResultExpression)
                            {
        ...
        public HIS_VACCINATION_RESULT GetById(long id, HisVaccinationResultSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_VACCINATION_RESULT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisVaccinationResultExpression != null && search.listHisVaccinationResultExpression.Count > 0)
                        {
                            foreach (var item in search.listHisVaccinationResultExpression)
                            {
