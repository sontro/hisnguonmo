...
namespace MOS.DAO.HisVaccinationResult
{
    partial class HisVaccinationResultGet : EntityBase
    {
        ...
        {
            List<V_HIS_VACCINATION_RESULT> list = new List<V_HIS_VACCINATION_RESULT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_VACCINATION_RESULT.AsNoTracking().AsQueryable();
                        if (search.listVHisVaccinationResultExpression != null && search.listVHisVaccinationResultExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisVaccinationResultExpression)
                            {
