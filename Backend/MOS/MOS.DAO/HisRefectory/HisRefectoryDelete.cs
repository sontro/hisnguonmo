...
{
    partial class HisRefectoryDelete : EntityBase
    {
        public HisRefectoryDelete()
            : base()
        {
        ...
        public bool Delete(HIS_REFECTORY data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_REFECTORY> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_REFECTORY> listIdNotFound = new List<HIS_REFECTORY>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_REFECTORY> listRaw = ctx.HIS_REFECTORY.Where(o => listId.Contains(o.ID)).ToList();
