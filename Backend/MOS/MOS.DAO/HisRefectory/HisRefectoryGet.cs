...
namespace MOS.DAO.HisRefectory
{
    ...
    {
        public List<HIS_REFECTORY> Get(HisRefectorySO search, CommonParam param)
        {
            List<HIS_REFECTORY> list = new List<HIS_REFECTORY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_REFECTORY.AsNoTracking().AsQueryable();
                        if (search.listHisRefectoryExpression != null && search.listHisRefectoryExpression.Count > 0)
                        {
                            foreach (var item in search.listHisRefectoryExpression)
                            {
        ...
        public HIS_REFECTORY GetById(long id, HisRefectorySO search)
        {
            HIS_REFECTORY result = null;
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_REFECTORY.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisRefectoryExpression != null && search.listHisRefectoryExpression.Count > 0)
                        {
                            foreach (var item in search.listHisRefectoryExpression)
                            {
