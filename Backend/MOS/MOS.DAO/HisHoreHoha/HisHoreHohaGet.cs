...
namespace MOS.DAO.HisHoreHoha
{
    partial class HisHoreHohaGet : EntityBase
    {
        ...
        {
            List<HIS_HORE_HOHA> list = new List<HIS_HORE_HOHA>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_HORE_HOHA.AsNoTracking().AsQueryable();
        ...
        public HIS_HORE_HOHA GetById(long id, HisHoreHohaSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_HORE_HOHA.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisHoreHohaExpression != null && search.listHisHoreHohaExpression.Count > 0)
                        {
                            foreach (var item in search.listHisHoreHohaExpression)
                            {
