...
namespace MOS.DAO.HisHoreHoha
{
    partial class HisHoreHohaGet : EntityBase
    {
        ...
        {
            List<V_HIS_HORE_HOHA> list = new List<V_HIS_HORE_HOHA>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_HORE_HOHA.AsNoTracking().AsQueryable();
                        if (search.listVHisHoreHohaExpression != null && search.listVHisHoreHohaExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisHoreHohaExpression)
                            {
