...
namespace MOS.DAO.HisAntibioticOldReg
{
    partial class HisAntibioticOldRegGet : EntityBase
    {
        ...
        {
            List<HIS_ANTIBIOTIC_OLD_REG> list = new List<HIS_ANTIBIOTIC_OLD_REG>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_ANTIBIOTIC_OLD_REG.AsNoTracking().AsQueryable();
                        if (search.listHisAntibioticOldRegExpression != null && search.listHisAntibioticOldRegExpression.Count > 0)
                        {
                            foreach (var item in search.listHisAntibioticOldRegExpression)
                            {
        ...
        public HIS_ANTIBIOTIC_OLD_REG GetById(long id, HisAntibioticOldRegSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_ANTIBIOTIC_OLD_REG.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisAntibioticOldRegExpression != null && search.listHisAntibioticOldRegExpression.Count > 0)
                        {
                            foreach (var item in search.listHisAntibioticOldRegExpression)
                            {
