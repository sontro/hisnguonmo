    ...
    partial class HisAntibioticOldRegCheck : EntityBase
    {
        public bool ExistsCode(string code, long? id)
        {
            ...
            try
            {
                ...
                {
                    count = ctx.HIS_ANTIBIOTIC_OLD_REG.AsQueryable().Where(p => p.ANTIBIOTIC_OLD_REG_CODE.Equals(code) && p.ID != id && (p.IS_DELETE.HasValue || p.IS_DELETE.Value != (short)1)).Count();
                }
                result = (count > 0);
            }
            catch (Exception)
