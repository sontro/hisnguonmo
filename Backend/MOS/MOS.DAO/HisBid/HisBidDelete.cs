...
{
    partial class HisBidDelete : EntityBase
    {
        public HisBidDelete()
            : base()
        {
        ...
        public bool Delete(HIS_BID data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_BID> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_BID> listIdNotFound = new List<HIS_BID>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_BID> listRaw = ctx.HIS_BID.Where(o => listId.Contains(o.ID)).ToList();
