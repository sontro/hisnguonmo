...
namespace MOS.DAO.HisAntigen
{
    partial class HisAntigenGet : EntityBase
    {
        ...
        {
            List<V_HIS_ANTIGEN> list = new List<V_HIS_ANTIGEN>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_ANTIGEN.AsNoTracking().AsQueryable();
                        if (search.listVHisAntigenExpression != null && search.listVHisAntigenExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisAntigenExpression)
                            {
