...
{
    partial class HisAntigenDelete : EntityBase
    {
        public HisAntigenDelete()
            : base()
        {
        ...
        public bool Delete(HIS_ANTIGEN data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_ANTIGEN> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_ANTIGEN> listIdNotFound = new List<HIS_ANTIGEN>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_ANTIGEN> listRaw = ctx.HIS_ANTIGEN.Where(o => listId.Contains(o.ID)).ToList();
