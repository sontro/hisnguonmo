...
{
    partial class HisRoomTimeDelete : EntityBase
    {
        public HisRoomTimeDelete()
            : base()
        {
        ...
        public bool Delete(HIS_ROOM_TIME data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_ROOM_TIME> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_ROOM_TIME> listIdNotFound = new List<HIS_ROOM_TIME>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_ROOM_TIME> listRaw = ctx.HIS_ROOM_TIME.Where(o => listId.Contains(o.ID)).ToList();
