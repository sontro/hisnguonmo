...
namespace MOS.DAO.HisRoomTime
{
    partial class HisRoomTimeGet : EntityBase
    {
        ...
        {
            List<V_HIS_ROOM_TIME> list = new List<V_HIS_ROOM_TIME>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_ROOM_TIME.AsNoTracking().AsQueryable();
                        if (search.listVHisRoomTimeExpression != null && search.listVHisRoomTimeExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisRoomTimeExpression)
                            {
