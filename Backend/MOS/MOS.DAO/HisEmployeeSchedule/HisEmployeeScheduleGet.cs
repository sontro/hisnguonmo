...
namespace MOS.DAO.HisEmployeeSchedule
{
    partial class HisEmployeeScheduleGet : EntityBase
    {
        ...
        {
            List<HIS_EMPLOYEE_SCHEDULE> list = new List<HIS_EMPLOYEE_SCHEDULE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_EMPLOYEE_SCHEDULE.AsNoTracking().AsQueryable();
        ...
        public HIS_EMPLOYEE_SCHEDULE GetById(long id, HisEmployeeScheduleSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_EMPLOYEE_SCHEDULE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisEmployeeScheduleExpression != null && search.listHisEmployeeScheduleExpression.Count > 0)
                        {
                            foreach (var item in search.listHisEmployeeScheduleExpression)
                            {
