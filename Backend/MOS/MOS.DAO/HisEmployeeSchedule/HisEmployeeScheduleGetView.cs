...
namespace MOS.DAO.HisEmployeeSchedule
{
    partial class HisEmployeeScheduleGet : EntityBase
    {
        ...
        {
            List<V_HIS_EMPLOYEE_SCHEDULE> list = new List<V_HIS_EMPLOYEE_SCHEDULE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_EMPLOYEE_SCHEDULE.AsNoTracking().AsQueryable();
                        if (search.listVHisEmployeeScheduleExpression != null && search.listVHisEmployeeScheduleExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisEmployeeScheduleExpression)
                            {
