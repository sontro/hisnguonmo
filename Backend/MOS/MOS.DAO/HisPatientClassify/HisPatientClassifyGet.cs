...
namespace MOS.DAO.HisPatientClassify
{
    partial class HisPatientClassifyGet : EntityBase
    {
        ...
        {
            List<HIS_PATIENT_CLASSIFY> list = new List<HIS_PATIENT_CLASSIFY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_PATIENT_CLASSIFY.AsNoTracking().AsQueryable();
        ...
        public HIS_PATIENT_CLASSIFY GetById(long id, HisPatientClassifySO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_PATIENT_CLASSIFY.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisPatientClassifyExpression != null && search.listHisPatientClassifyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisPatientClassifyExpression)
                            {
