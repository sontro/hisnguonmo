...
{
    partial class HisTextLibDelete : EntityBase
    {
        public HisTextLibDelete()
            : base()
        {
        ...
        public bool Delete(HIS_TEXT_LIB data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_TEXT_LIB> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_TEXT_LIB> listIdNotFound = new List<HIS_TEXT_LIB>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_TEXT_LIB> listRaw = ctx.HIS_TEXT_LIB.Where(o => listId.Contains(o.ID)).ToList();
