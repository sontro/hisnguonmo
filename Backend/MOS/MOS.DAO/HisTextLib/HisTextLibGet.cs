...
{
    partial class HisTextLibGet : EntityBase
    {
        public List<HIS_TEXT_LIB> Get(HisTextLibSO search, CommonParam param)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_TEXT_LIB.AsNoTracking().AsQueryable();
                        if (search.listHisTextLibExpression != null && search.listHisTextLibExpression.Count > 0)
                        {
                            foreach (var item in search.listHisTextLibExpression)
                            {
        ...
        public HIS_TEXT_LIB GetById(long id, HisTextLibSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_TEXT_LIB.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisTextLibExpression != null && search.listHisTextLibExpression.Count > 0)
                        {
                            foreach (var item in search.listHisTextLibExpression)
                            {
