...
namespace MOS.DAO.HisTextLib
{
    partial class HisTextLibGet : EntityBase
    {
        ...
        {
            List<V_HIS_TEXT_LIB> list = new List<V_HIS_TEXT_LIB>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_TEXT_LIB.AsQueryable();
                        if (search.listVHisTextLibExpression != null && search.listVHisTextLibExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisTextLibExpression)
                            {
