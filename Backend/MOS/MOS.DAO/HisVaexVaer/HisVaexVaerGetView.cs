...
namespace MOS.DAO.HisVaexVaer
{
    partial class HisVaexVaerGet : EntityBase
    {
        ...
        {
            List<V_HIS_VAEX_VAER> list = new List<V_HIS_VAEX_VAER>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_VAEX_VAER.AsNoTracking().AsQueryable();
                        if (search.listVHisVaexVaerExpression != null && search.listVHisVaexVaerExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisVaexVaerExpression)
                            {
