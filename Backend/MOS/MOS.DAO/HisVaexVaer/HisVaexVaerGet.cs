...
{
    partial class HisVaexVaerGet : EntityBase
    {
        public List<HIS_VAEX_VAER> Get(HisVaexVaerSO search, CommonParam param)
        {
            List<HIS_VAEX_VAER> list = new List<HIS_VAEX_VAER>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_VAEX_VAER.AsNoTracking().AsQueryable();
                        if (search.listHisVaexVaerExpression != null && search.listHisVaexVaerExpression.Count > 0)
                        {
                            foreach (var item in search.listHisVaexVaerExpression)
                            {
        ...
        public HIS_VAEX_VAER GetById(long id, HisVaexVaerSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_VAEX_VAER.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisVaexVaerExpression != null && search.listHisVaexVaerExpression.Count > 0)
                        {
                            foreach (var item in search.listHisVaexVaerExpression)
                            {
