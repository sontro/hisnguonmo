...
namespace MOS.DAO.HisAssessmentObject
{
    partial class HisAssessmentObjectGet : EntityBase
    {
        ...
        {
            List<HIS_ASSESSMENT_OBJECT> list = new List<HIS_ASSESSMENT_OBJECT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_ASSESSMENT_OBJECT.AsNoTracking().AsQueryable();
        ...
        public HIS_ASSESSMENT_OBJECT GetById(long id, HisAssessmentObjectSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_ASSESSMENT_OBJECT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisAssessmentObjectExpression != null && search.listHisAssessmentObjectExpression.Count > 0)
                        {
                            foreach (var item in search.listHisAssessmentObjectExpression)
                            {
