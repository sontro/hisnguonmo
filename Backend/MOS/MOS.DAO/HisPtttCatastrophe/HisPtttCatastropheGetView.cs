...
namespace MOS.DAO.HisPtttCatastrophe
{
    partial class HisPtttCatastropheGet : EntityBase
    {
        ...
        {
            List<V_HIS_PTTT_CATASTROPHE> list = new List<V_HIS_PTTT_CATASTROPHE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_PTTT_CATASTROPHE.AsQueryable();
                        if (search.listVHisPtttCatastropheExpression != null && search.listVHisPtttCatastropheExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisPtttCatastropheExpression)
                            {
