...
namespace MOS.DAO.HisTranPatiTemp
{
    partial class HisTranPatiTempGet : EntityBase
    {
        ...
        {
            List<HIS_TRAN_PATI_TEMP> list = new List<HIS_TRAN_PATI_TEMP>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_TRAN_PATI_TEMP.AsNoTracking().AsQueryable();
        ...
        public HIS_TRAN_PATI_TEMP GetById(long id, HisTranPatiTempSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_TRAN_PATI_TEMP.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisTranPatiTempExpression != null && search.listHisTranPatiTempExpression.Count > 0)
                        {
                            foreach (var item in search.listHisTranPatiTempExpression)
                            {
