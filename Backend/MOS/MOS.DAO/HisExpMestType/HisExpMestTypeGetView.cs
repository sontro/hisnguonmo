...
namespace MOS.DAO.HisExpMestType
{
    partial class HisExpMestTypeGet : EntityBase
    {
        ...
        {
            List<V_HIS_EXP_MEST_TYPE> list = new List<V_HIS_EXP_MEST_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_EXP_MEST_TYPE.AsQueryable();
                        if (search.listVHisExpMestTypeExpression != null && search.listVHisExpMestTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisExpMestTypeExpression)
                            {
