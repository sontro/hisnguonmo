...
namespace MOS.DAO.HisImpUserTempDt
{
    partial class HisImpUserTempDtGet : EntityBase
    {
        ...
        {
            List<HIS_IMP_USER_TEMP_DT> list = new List<HIS_IMP_USER_TEMP_DT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_IMP_USER_TEMP_DT.AsNoTracking().AsQueryable();
        ...
        public HIS_IMP_USER_TEMP_DT GetById(long id, HisImpUserTempDtSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_IMP_USER_TEMP_DT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisImpUserTempDtExpression != null && search.listHisImpUserTempDtExpression.Count > 0)
                        {
                            foreach (var item in search.listHisImpUserTempDtExpression)
                            {
