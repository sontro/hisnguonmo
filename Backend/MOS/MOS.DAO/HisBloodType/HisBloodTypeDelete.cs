...
{
    partial class HisBloodTypeDelete : EntityBase
    {
        public HisBloodTypeDelete()
            : base()
        {
        ...
        public bool Delete(HIS_BLOOD_TYPE data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_BLOOD_TYPE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_BLOOD_TYPE> listIdNotFound = new List<HIS_BLOOD_TYPE>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_BLOOD_TYPE> listRaw = ctx.HIS_BLOOD_TYPE.Where(o => listId.Contains(o.ID)).ToList();
