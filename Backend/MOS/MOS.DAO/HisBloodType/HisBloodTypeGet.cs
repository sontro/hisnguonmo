...
namespace MOS.DAO.HisBloodType
{
    partial class HisBloodTypeGet : EntityBase
    {
        ...
        {
            List<HIS_BLOOD_TYPE> list = new List<HIS_BLOOD_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_BLOOD_TYPE.AsNoTracking().AsQueryable();
                        if (search.listHisBloodTypeExpression != null && search.listHisBloodTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBloodTypeExpression)
                            {
        ...
        public HIS_BLOOD_TYPE GetById(long id, HisBloodTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_BLOOD_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisBloodTypeExpression != null && search.listHisBloodTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBloodTypeExpression)
                            {
