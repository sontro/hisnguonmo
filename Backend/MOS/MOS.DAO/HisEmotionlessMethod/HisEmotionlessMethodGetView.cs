...
namespace MOS.DAO.HisEmotionlessMethod
{
    partial class HisEmotionlessMethodGet : EntityBase
    {
        ...
        {
            List<V_HIS_EMOTIONLESS_METHOD> list = new List<V_HIS_EMOTIONLESS_METHOD>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_EMOTIONLESS_METHOD.AsQueryable();
                        if (search.listVHisEmotionlessMethodExpression != null && search.listVHisEmotionlessMethodExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisEmotionlessMethodExpression)
                            {
