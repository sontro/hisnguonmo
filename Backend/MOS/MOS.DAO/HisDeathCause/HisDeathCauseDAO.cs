using MOS.DAO.StagingObject;
using MOS.EFMODEL.DataModels;
using Inventec.Common.Repository;
using Inventec.Core;
using System;
using System.Collections.Generic;

namespace MOS.DAO.HisDeathCause
{
    public partial class HisDeathCauseDAO : EntityBase
    {
        private HisDeathCauseCreate CreateWorker
        {
            get
            {
                return (HisDeathCauseCreate)Worker.Get<HisDeathCauseCreate>();
            }
        }
        private HisDeathCauseUpdate UpdateWorker
        {
            get
            {
                return (HisDeathCauseUpdate)Worker.Get<HisDeathCauseUpdate>();
            }
        }
        private HisDeathCauseDelete DeleteWorker
        {
            get
            {
                return (HisDeathCauseDelete)Worker.Get<HisDeathCauseDelete>();
            }
        }
        private HisDeathCauseTruncate TruncateWorker
        {
            get
            {
                return (HisDeathCauseTruncate)Worker.Get<HisDeathCauseTruncate>();
            }
        }
        private HisDeathCauseGet GetWorker
        {
            get
            {
                return (HisDeathCauseGet)Worker.Get<HisDeathCauseGet>();
            }
        }
        private HisDeathCauseCheck CheckWorker
        {
            get
            {
                return (HisDeathCauseCheck)Worker.Get<HisDeathCauseCheck>();
            }
        }

        public bool Create(HIS_DEATH_CAUSE data)
        {
            bool result = false;
            try
            {
                result = CreateWorker.Create(data);
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
                result = false;
            }
            return result;
        }

        public bool CreateList(List<HIS_DEATH_CAUSE> listData)
        {
            bool result = false;
            try
            {
                result = CreateWorker.CreateList(listData);
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
                result = false;
            }
            return result;
        }

        public bool Update(HIS_DEATH_CAUSE data)
        {
            bool result = false;
            try
            {
                result = UpdateWorker.Update(data);
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
                result = false;
            }
            return result;
        }

        public bool UpdateList(List<HIS_DEATH_CAUSE> listData)
        {
            bool result = false;
            try
            {
                result = UpdateWorker.UpdateList(listData);
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
                result = false;
            }
            return result;
        }

        public bool Delete(HIS_DEATH_CAUSE data)
        {
            bool result = false;
            try
            {
                result = DeleteWorker.Delete(data);
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
                result = false;
            }
            return result;
        }

        public bool DeleteList(List<HIS_DEATH_CAUSE> listData)
        {
            bool result = false;

            try
            {
                result = DeleteWorker.DeleteList(listData);
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
                result = false;
            }
            return result;
        }

        public bool Truncate(HIS_DEATH_CAUSE data)
        {
            bool result = false;
            try
            {
                result = TruncateWorker.Truncate(data);
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
                result = false;
            }
            return result;
        }

        public bool TruncateList(List<HIS_DEATH_CAUSE> listData)
        {
            bool result = false;
            try
            {
                result = TruncateWorker.TruncateList(listData);
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
                result = false;
            }
            return result;
        }

        public List<HIS_DEATH_CAUSE> Get(HisDeathCauseSO search, CommonParam param)
        {
            List<HIS_DEATH_CAUSE> result = new List<HIS_DEATH_CAUSE>();
            try
            {
                result = GetWorker.Get(search, param);
            }
            catch (Exception ex)
            {
                param.HasException = true;
                Inventec.Common.Logging.LogSystem.Error(ex);
                result.Clear();
            }
            return result;
        }

        public HIS_DEATH_CAUSE GetById(long id, HisDeathCauseSO search)
        {
            HIS_DEATH_CAUSE result = null;
            try
            {
                result = GetWorker.GetById(id, search);
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
                result = null;
            }

            return result;
        }

        public bool IsUnLock(long id)
        {
            try
            {
                return CheckWorker.IsUnLock(id);
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
                throw;
            }
        }
    }
}
