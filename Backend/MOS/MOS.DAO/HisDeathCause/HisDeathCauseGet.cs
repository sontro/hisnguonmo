...
namespace MOS.DAO.HisDeathCause
{
    partial class HisDeathCauseGet : EntityBase
    {
        ...
        {
            List<HIS_DEATH_CAUSE> list = new List<HIS_DEATH_CAUSE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_DEATH_CAUSE.AsNoTracking().AsQueryable();
        ...
        public HIS_DEATH_CAUSE GetById(long id, HisDeathCauseSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_DEATH_CAUSE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisDeathCauseExpression != null && search.listHisDeathCauseExpression.Count > 0)
                        {
                            foreach (var item in search.listHisDeathCauseExpression)
                            {
