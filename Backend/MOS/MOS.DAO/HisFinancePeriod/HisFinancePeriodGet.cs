...
namespace MOS.DAO.HisFinancePeriod
{
    partial class HisFinancePeriodGet : EntityBase
    {
        ...
        {
            List<HIS_FINANCE_PERIOD> list = new List<HIS_FINANCE_PERIOD>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_FINANCE_PERIOD.AsNoTracking().AsQueryable();
        ...
        public HIS_FINANCE_PERIOD GetById(long id, HisFinancePeriodSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_FINANCE_PERIOD.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisFinancePeriodExpression != null && search.listHisFinancePeriodExpression.Count > 0)
                        {
                            foreach (var item in search.listHisFinancePeriodExpression)
                            {
