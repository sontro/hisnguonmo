...
namespace MOS.DAO.HisServiceMety
{
    partial class HisServiceMetyGet : EntityBase
    {
        ...
        {
            List<HIS_SERVICE_METY> list = new List<HIS_SERVICE_METY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SERVICE_METY.AsNoTracking().AsQueryable();
                        if (search.listHisServiceMetyExpression != null && search.listHisServiceMetyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisServiceMetyExpression)
                            {
        ...
        public HIS_SERVICE_METY GetById(long id, HisServiceMetySO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_SERVICE_METY.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisServiceMetyExpression != null && search.listHisServiceMetyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisServiceMetyExpression)
                            {
