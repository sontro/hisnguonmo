...
{
    partial class HisManufacturerGet : EntityBase
    {
        public List<HIS_MANUFACTURER> Get(HisManufacturerSO search, CommonParam param)
        {
            List<HIS_MANUFACTURER> list = new List<HIS_MANUFACTURER>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MANUFACTURER.AsNoTracking().AsQueryable();
                        if (search.listHisManufacturerExpression != null && search.listHisManufacturerExpression.Count > 0)
                        {
                            foreach (var item in search.listHisManufacturerExpression)
                            {
        ...
        public HIS_MANUFACTURER GetById(long id, HisManufacturerSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_MANUFACTURER.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisManufacturerExpression != null && search.listHisManufacturerExpression.Count > 0)
                        {
                            foreach (var item in search.listHisManufacturerExpression)
                            {
