...
namespace MOS.DAO.HisVaccAppointment
{
    partial class HisVaccAppointmentGet : EntityBase
    {
        ...
        {
            List<HIS_VACC_APPOINTMENT> list = new List<HIS_VACC_APPOINTMENT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_VACC_APPOINTMENT.AsNoTracking().AsQueryable();
                        if (search.listHisVaccAppointmentExpression != null && search.listHisVaccAppointmentExpression.Count > 0)
                        {
                            foreach (var item in search.listHisVaccAppointmentExpression)
                            {
        ...
        public HIS_VACC_APPOINTMENT GetById(long id, HisVaccAppointmentSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_VACC_APPOINTMENT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisVaccAppointmentExpression != null && search.listHisVaccAppointmentExpression.Count > 0)
                        {
                            foreach (var item in search.listHisVaccAppointmentExpression)
                            {
