...
{
    partial class HisKskGeneralGet : EntityBase
    {
        public List<V_HIS_KSK_GENERAL> GetView(HisKskGeneralSO search, CommonParam param)
        {
            List<V_HIS_KSK_GENERAL> list = new List<V_HIS_KSK_GENERAL>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_KSK_GENERAL.AsNoTracking().AsQueryable();
                        if (search.listVHisKskGeneralExpression != null && search.listVHisKskGeneralExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisKskGeneralExpression)
                            {
