...
namespace MOS.DAO.HisKskGeneral
{
    partial class HisKskGeneralGet : EntityBase
    {
        ...
        {
            List<HIS_KSK_GENERAL> list = new List<HIS_KSK_GENERAL>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_KSK_GENERAL.AsNoTracking().AsQueryable();
                        if (search.listHisKskGeneralExpression != null && search.listHisKskGeneralExpression.Count > 0)
                        {
                            foreach (var item in search.listHisKskGeneralExpression)
                            {
        ...
        public HIS_KSK_GENERAL GetById(long id, HisKskGeneralSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_KSK_GENERAL.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisKskGeneralExpression != null && search.listHisKskGeneralExpression.Count > 0)
                        {
                            foreach (var item in search.listHisKskGeneralExpression)
                            {
