...
namespace MOS.DAO.HisPatient
{
    partial class HisPatientGet : EntityBase
    {
        ...
        {
            List<V_HIS_PATIENT> list = new List<V_HIS_PATIENT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_PATIENT.AsNoTracking().AsQueryable();
                        if (search.listVHisPatientExpression != null && search.listVHisPatientExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisPatientExpression)
                            {
