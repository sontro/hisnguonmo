...
{
    partial class HisPatientDelete : EntityBase
    {
        public HisPatientDelete()
            : base()
        {
        ...
        public bool Delete(HIS_PATIENT data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_PATIENT> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_PATIENT> listIdNotFound = new List<HIS_PATIENT>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_PATIENT> listRaw = ctx.HIS_PATIENT.Where(o => listId.Contains(o.ID)).ToList();
