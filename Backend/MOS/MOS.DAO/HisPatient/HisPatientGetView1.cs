...
namespace MOS.DAO.HisPatient
{
    partial class HisPatientGet : EntityBase
    {
        ...
        {
            List<V_HIS_PATIENT_1> list = new List<V_HIS_PATIENT_1>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_PATIENT_1.AsNoTracking().AsQueryable();
                        if (search.listVHisPatient1Expression != null && search.listVHisPatient1Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisPatient1Expression)
                            {
