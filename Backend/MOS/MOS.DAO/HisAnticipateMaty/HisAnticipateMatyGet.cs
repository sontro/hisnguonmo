...
namespace MOS.DAO.HisAnticipateMaty
{
    partial class HisAnticipateMatyGet : EntityBase
    {
        ...
        {
            List<HIS_ANTICIPATE_MATY> list = new List<HIS_ANTICIPATE_MATY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_ANTICIPATE_MATY.AsNoTracking().AsQueryable();
        ...
        public HIS_ANTICIPATE_MATY GetById(long id, HisAnticipateMatySO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_ANTICIPATE_MATY.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisAnticipateMatyExpression != null && search.listHisAnticipateMatyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisAnticipateMatyExpression)
                            {
