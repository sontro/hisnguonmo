...
namespace MOS.DAO.HisImpMest
{
    partial class HisImpMestGet : EntityBase
    {
        ...
        {
            List<V_HIS_IMP_MEST> list = new List<V_HIS_IMP_MEST>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_IMP_MEST.AsNoTracking().AsQueryable();
                        if (search.listVHisImpMestExpression != null && search.listVHisImpMestExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisImpMestExpression)
                            {
