...
{
    partial class HisImpMestCheck : EntityBase
    {
        public bool ExistsCode(string code, long? id)
        {
            ...
            try
            {
                ...
                {
                    count = ctx.HIS_IMP_MEST.AsQueryable().Where(p => p.IMP_MEST_CODE.Equals(code) && p.ID != id && (!p.IS_DELETE.HasValue || p.IS_DELETE.Value != (short)1)).Count();
                }
                result = (count > 0);
            }
            catch (Exception)
