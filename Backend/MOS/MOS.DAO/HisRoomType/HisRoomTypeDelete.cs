...
{
    partial class HisRoomTypeDelete : EntityBase
    {
        public HisRoomTypeDelete()
            : base()
        {
        ...
        public bool Delete(HIS_ROOM_TYPE data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_ROOM_TYPE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_ROOM_TYPE> listIdNotFound = new List<HIS_ROOM_TYPE>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_ROOM_TYPE> listRaw = ctx.HIS_ROOM_TYPE.Where(o => listId.Contains(o.ID)).ToList();
