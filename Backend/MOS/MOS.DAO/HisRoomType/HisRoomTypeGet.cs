...
{
    partial class HisRoomTypeGet : EntityBase
    {
        public List<HIS_ROOM_TYPE> Get(HisRoomTypeSO search, CommonParam param)
        {
            List<HIS_ROOM_TYPE> list = new List<HIS_ROOM_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_ROOM_TYPE.AsNoTracking().AsQueryable();
                        if (search.listHisRoomTypeExpression != null && search.listHisRoomTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisRoomTypeExpression)
                            {
        ...
        public HIS_ROOM_TYPE GetById(long id, HisRoomTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_ROOM_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisRoomTypeExpression != null && search.listHisRoomTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisRoomTypeExpression)
                            {
