...
namespace MOS.DAO.HisRoomType
{
    partial class HisRoomTypeGet : EntityBase
    {
        ...
        {
            List<V_HIS_ROOM_TYPE> list = new List<V_HIS_ROOM_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_ROOM_TYPE.AsQueryable();
                        if (search.listVHisRoomTypeExpression != null && search.listVHisRoomTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisRoomTypeExpression)
                            {
