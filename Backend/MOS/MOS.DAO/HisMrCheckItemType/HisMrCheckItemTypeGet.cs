...
namespace MOS.DAO.HisMrCheckItemType
{
    partial class HisMrCheckItemTypeGet : EntityBase
    {
        ...
        {
            List<HIS_MR_CHECK_ITEM_TYPE> list = new List<HIS_MR_CHECK_ITEM_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MR_CHECK_ITEM_TYPE.AsNoTracking().AsQueryable();
        ...
        public HIS_MR_CHECK_ITEM_TYPE GetById(long id, HisMrCheckItemTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_MR_CHECK_ITEM_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMrCheckItemTypeExpression != null && search.listHisMrCheckItemTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMrCheckItemTypeExpression)
                            {
