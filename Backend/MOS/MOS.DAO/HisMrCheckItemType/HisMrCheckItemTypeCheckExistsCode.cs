    ...
    partial class HisMrCheckItemTypeCheck : EntityBase
    {
        public bool ExistsCode(string code, long? id)
        {
            ...
            try
            {
                ...
                {
                    count = ctx.HIS_MR_CHECK_ITEM_TYPE.AsQueryable().Where(p => p.MR_CHECK_ITEM_TYPE_CODE.Equals(code) && p.ID != id && (p.IS_DELETE.HasValue || p.IS_DELETE.Value != (short)1)).Count();
                }
                result = (count > 0);
            }
            catch (Exception)
