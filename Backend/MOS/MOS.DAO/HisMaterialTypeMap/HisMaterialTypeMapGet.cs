...
namespace MOS.DAO.HisMaterialTypeMap
{
    partial class HisMaterialTypeMapGet : EntityBase
    {
        ...
        {
            List<HIS_MATERIAL_TYPE_MAP> list = new List<HIS_MATERIAL_TYPE_MAP>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MATERIAL_TYPE_MAP.AsNoTracking().AsQueryable();
        ...
        public HIS_MATERIAL_TYPE_MAP GetById(long id, HisMaterialTypeMapSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_MATERIAL_TYPE_MAP.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMaterialTypeMapExpression != null && search.listHisMaterialTypeMapExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMaterialTypeMapExpression)
                            {
