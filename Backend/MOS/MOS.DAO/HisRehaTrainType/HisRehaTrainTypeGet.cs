...
namespace MOS.DAO.HisRehaTrainType
{
    partial class HisRehaTrainTypeGet : EntityBase
    {
        ...
        {
            List<HIS_REHA_TRAIN_TYPE> list = new List<HIS_REHA_TRAIN_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_REHA_TRAIN_TYPE.AsNoTracking().AsQueryable();
        ...
        public HIS_REHA_TRAIN_TYPE GetById(long id, HisRehaTrainTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_REHA_TRAIN_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisRehaTrainTypeExpression != null && search.listHisRehaTrainTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisRehaTrainTypeExpression)
                            {
