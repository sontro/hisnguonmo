...
namespace MOS.DAO.HisTreatmentBorrow
{
    partial class HisTreatmentBorrowGet : EntityBase
    {
        ...
        {
            List<HIS_TREATMENT_BORROW> list = new List<HIS_TREATMENT_BORROW>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_TREATMENT_BORROW.AsNoTracking().AsQueryable();
        ...
        public HIS_TREATMENT_BORROW GetById(long id, HisTreatmentBorrowSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_TREATMENT_BORROW.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisTreatmentBorrowExpression != null && search.listHisTreatmentBorrowExpression.Count > 0)
                        {
                            foreach (var item in search.listHisTreatmentBorrowExpression)
                            {
