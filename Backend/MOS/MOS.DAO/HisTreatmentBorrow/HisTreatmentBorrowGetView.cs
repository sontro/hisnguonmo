...
namespace MOS.DAO.HisTreatmentBorrow
{
    partial class HisTreatmentBorrowGet : EntityBase
    {
        ...
        {
            List<V_HIS_TREATMENT_BORROW> list = new List<V_HIS_TREATMENT_BORROW>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_TREATMENT_BORROW.AsNoTracking().AsQueryable();
                        if (search.listVHisTreatmentBorrowExpression != null && search.listVHisTreatmentBorrowExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisTreatmentBorrowExpression)
                            {
