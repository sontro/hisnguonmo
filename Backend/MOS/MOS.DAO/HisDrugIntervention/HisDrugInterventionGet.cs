...
{
    partial class HisDrugInterventionGet : EntityBase
    {
        public List<HIS_DRUG_INTERVENTION> Get(HisDrugInterventionSO search, CommonParam param)
        {
            List<HIS_DRUG_INTERVENTION> list = new List<HIS_DRUG_INTERVENTION>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_DRUG_INTERVENTION.AsNoTracking().AsQueryable();
                        if (search.listHisDrugInterventionExpression != null && search.listHisDrugInterventionExpression.Count > 0)
                        {
                            foreach (var item in search.listHisDrugInterventionExpression)
                            {
        ...
        public HIS_DRUG_INTERVENTION GetById(long id, HisDrugInterventionSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_DRUG_INTERVENTION.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisDrugInterventionExpression != null && search.listHisDrugInterventionExpression.Count > 0)
                        {
                            foreach (var item in search.listHisDrugInterventionExpression)
                            {
