...
namespace MOS.DAO.HisDrugIntervention
{
    partial class HisDrugInterventionGet : EntityBase
    {
        ...
        {
            List<V_HIS_DRUG_INTERVENTION> list = new List<V_HIS_DRUG_INTERVENTION>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_DRUG_INTERVENTION.AsNoTracking().AsQueryable();
                        if (search.listVHisDrugInterventionExpression != null && search.listVHisDrugInterventionExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisDrugInterventionExpression)
                            {
