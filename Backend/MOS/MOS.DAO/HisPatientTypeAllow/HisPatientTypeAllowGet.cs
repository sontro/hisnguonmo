...
namespace MOS.DAO.HisPatientTypeAllow
{
    partial class HisPatientTypeAllowGet : EntityBase
    {
        ...
        {
            List<HIS_PATIENT_TYPE_ALLOW> list = new List<HIS_PATIENT_TYPE_ALLOW>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_PATIENT_TYPE_ALLOW.AsNoTracking().AsQueryable();
        ...
        public HIS_PATIENT_TYPE_ALLOW GetById(long id, HisPatientTypeAllowSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_PATIENT_TYPE_ALLOW.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisPatientTypeAllowExpression != null && search.listHisPatientTypeAllowExpression.Count > 0)
                        {
                            foreach (var item in search.listHisPatientTypeAllowExpression)
                            {
