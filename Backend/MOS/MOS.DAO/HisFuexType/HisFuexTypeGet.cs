...
namespace MOS.DAO.HisFuexType
{
    partial class HisFuexTypeGet : EntityBase
    {
        ...
        {
            List<HIS_FUEX_TYPE> list = new List<HIS_FUEX_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_FUEX_TYPE.AsNoTracking().AsQueryable();
                        if (search.listHisFuexTypeExpression != null && search.listHisFuexTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisFuexTypeExpression)
                            {
        ...
        public HIS_FUEX_TYPE GetById(long id, HisFuexTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_FUEX_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisFuexTypeExpression != null && search.listHisFuexTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisFuexTypeExpression)
                            {
