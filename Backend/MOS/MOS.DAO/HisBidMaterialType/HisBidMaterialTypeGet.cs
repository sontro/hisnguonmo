...
{
    partial class HisBidMaterialTypeGet : EntityBase
    {
        public List<HIS_BID_MATERIAL_TYPE> Get(HisBidMaterialTypeSO search, CommonParam param)
        {
            List<HIS_BID_MATERIAL_TYPE> list = new List<HIS_BID_MATERIAL_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_BID_MATERIAL_TYPE.AsNoTracking().AsQueryable();
                        if (search.listHisBidMaterialTypeExpression != null && search.listHisBidMaterialTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBidMaterialTypeExpression)
                            {
        ...
        public HIS_BID_MATERIAL_TYPE GetById(long id, HisBidMaterialTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_BID_MATERIAL_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisBidMaterialTypeExpression != null && search.listHisBidMaterialTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBidMaterialTypeExpression)
                            {
