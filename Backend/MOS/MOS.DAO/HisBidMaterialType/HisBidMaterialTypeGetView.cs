...
namespace MOS.DAO.HisBidMaterialType
{
    partial class HisBidMaterialTypeGet : EntityBase
    {
        ...
        {
            List<V_HIS_BID_MATERIAL_TYPE> list = new List<V_HIS_BID_MATERIAL_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_BID_MATERIAL_TYPE.AsNoTracking().AsQueryable();
                        if (search.listVHisBidMaterialTypeExpression != null && search.listVHisBidMaterialTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisBidMaterialTypeExpression)
                            {
