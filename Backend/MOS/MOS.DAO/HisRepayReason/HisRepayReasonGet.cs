...
namespace MOS.DAO.HisRepayReason
{
    partial class HisRepayReasonGet : EntityBase
    {
        ...
        {
            List<HIS_REPAY_REASON> list = new List<HIS_REPAY_REASON>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_REPAY_REASON.AsNoTracking().AsQueryable();
        ...
        public HIS_REPAY_REASON GetById(long id, HisRepayReasonSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_REPAY_REASON.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisRepayReasonExpression != null && search.listHisRepayReasonExpression.Count > 0)
                        {
                            foreach (var item in search.listHisRepayReasonExpression)
                            {
