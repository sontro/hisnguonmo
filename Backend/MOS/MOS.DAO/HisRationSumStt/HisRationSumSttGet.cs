...
namespace MOS.DAO.HisRationSumStt
{
    partial class HisRationSumSttGet : EntityBase
    {
        ...
        {
            List<HIS_RATION_SUM_STT> list = new List<HIS_RATION_SUM_STT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_RATION_SUM_STT.AsNoTracking().AsQueryable();
        ...
        public HIS_RATION_SUM_STT GetById(long id, HisRationSumSttSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_RATION_SUM_STT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisRationSumSttExpression != null && search.listHisRationSumSttExpression.Count > 0)
                        {
                            foreach (var item in search.listHisRationSumSttExpression)
                            {
