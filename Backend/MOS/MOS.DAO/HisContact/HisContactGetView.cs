...
namespace MOS.DAO.HisContact
{
    partial class HisContactGet : EntityBase
    {
        ...
        {
            List<V_HIS_CONTACT> list = new List<V_HIS_CONTACT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_CONTACT.AsNoTracking().AsQueryable();
                        if (search.listVHisContactExpression != null && search.listVHisContactExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisContactExpression)
                            {
