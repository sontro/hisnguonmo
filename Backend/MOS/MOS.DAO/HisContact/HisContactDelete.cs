...
{
    partial class HisContactDelete : EntityBase
    {
        public HisContactDelete()
            : base()
        {
        ...
        public bool Delete(HIS_CONTACT data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_CONTACT> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_CONTACT> listIdNotFound = new List<HIS_CONTACT>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_CONTACT> listRaw = ctx.HIS_CONTACT.Where(o => listId.Contains(o.ID)).ToList();
