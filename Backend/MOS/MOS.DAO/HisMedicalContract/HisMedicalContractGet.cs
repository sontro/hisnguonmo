...
namespace MOS.DAO.HisMedicalContract
{
    partial class HisMedicalContractGet : EntityBase
    {
        ...
        {
            List<HIS_MEDICAL_CONTRACT> list = new List<HIS_MEDICAL_CONTRACT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MEDICAL_CONTRACT.AsNoTracking().AsQueryable();
        ...
        public HIS_MEDICAL_CONTRACT GetById(long id, HisMedicalContractSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_MEDICAL_CONTRACT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMedicalContractExpression != null && search.listHisMedicalContractExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMedicalContractExpression)
                            {
