...
namespace MOS.DAO.HisTestIndexUnit
{
    ...
    {
        public V_HIS_TEST_INDEX_UNIT GetViewByCode(string code, HisTestIndexUnitSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_TEST_INDEX_UNIT.AsQueryable().Where(p => p.TEST_INDEX_UNIT_CODE == code);
