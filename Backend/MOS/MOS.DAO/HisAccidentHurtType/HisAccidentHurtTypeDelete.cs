...
{
    partial class HisAccidentHurtTypeDelete : EntityBase
    {
        public HisAccidentHurtTypeDelete()
            : base()
        {
        ...
        public bool Delete(HIS_ACCIDENT_HURT_TYPE data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_ACCIDENT_HURT_TYPE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_ACCIDENT_HURT_TYPE> listIdNotFound = new List<HIS_ACCIDENT_HURT_TYPE>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_ACCIDENT_HURT_TYPE> listRaw = ctx.HIS_ACCIDENT_HURT_TYPE.Where(o => listId.Contains(o.ID)).ToList();
