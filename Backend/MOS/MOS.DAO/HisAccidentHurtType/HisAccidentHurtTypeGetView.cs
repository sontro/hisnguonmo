...
namespace MOS.DAO.HisAccidentHurtType
{
    partial class HisAccidentHurtTypeGet : EntityBase
    {
        ...
        {
            List<V_HIS_ACCIDENT_HURT_TYPE> list = new List<V_HIS_ACCIDENT_HURT_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_ACCIDENT_HURT_TYPE.AsQueryable();
                        if (search.listVHisAccidentHurtTypeExpression != null && search.listVHisAccidentHurtTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisAccidentHurtTypeExpression)
                            {
