...
namespace MOS.DAO.HisServiceHein
{
    partial class HisServiceHeinGet : EntityBase
    {
        ...
        {
            List<V_HIS_SERVICE_HEIN> list = new List<V_HIS_SERVICE_HEIN>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_SERVICE_HEIN.AsNoTracking().AsQueryable();
                        if (search.listVHisServiceHeinExpression != null && search.listVHisServiceHeinExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisServiceHeinExpression)
                            {
