...
{
    partial class HisServiceHeinDelete : EntityBase
    {
        public HisServiceHeinDelete()
            : base()
        {
        ...
        public bool Delete(HIS_SERVICE_HEIN data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_SERVICE_HEIN> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_SERVICE_HEIN> listIdNotFound = new List<HIS_SERVICE_HEIN>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_SERVICE_HEIN> listRaw = ctx.HIS_SERVICE_HEIN.Where(o => listId.Contains(o.ID)).ToList();
