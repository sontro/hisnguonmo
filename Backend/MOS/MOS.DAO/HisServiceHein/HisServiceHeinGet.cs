...
namespace MOS.DAO.HisServiceHein
{
    ...
    {
        public List<HIS_SERVICE_HEIN> Get(HisServiceHeinSO search, CommonParam param)
        {
            List<HIS_SERVICE_HEIN> list = new List<HIS_SERVICE_HEIN>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SERVICE_HEIN.AsNoTracking().AsQueryable();
                        if (search.listHisServiceHeinExpression != null && search.listHisServiceHeinExpression.Count > 0)
                        {
                            foreach (var item in search.listHisServiceHeinExpression)
                            {
        ...
        public HIS_SERVICE_HEIN GetById(long id, HisServiceHeinSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_SERVICE_HEIN.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisServiceHeinExpression != null && search.listHisServiceHeinExpression.Count > 0)
                        {
                            foreach (var item in search.listHisServiceHeinExpression)
                            {
