...
namespace MOS.DAO.HisMrCheckItem
{
    partial class HisMrCheckItemGet : EntityBase
    {
        ...
        {
            List<HIS_MR_CHECK_ITEM> list = new List<HIS_MR_CHECK_ITEM>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MR_CHECK_ITEM.AsNoTracking().AsQueryable();
        ...
        public HIS_MR_CHECK_ITEM GetById(long id, HisMrCheckItemSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_MR_CHECK_ITEM.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMrCheckItemExpression != null && search.listHisMrCheckItemExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMrCheckItemExpression)
                            {
