...
namespace MOS.DAO.HisMrCheckItem
{
    partial class HisMrCheckItemGet : EntityBase
    {
        ...
        {
            List<V_HIS_MR_CHECK_ITEM> list = new List<V_HIS_MR_CHECK_ITEM>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_MR_CHECK_ITEM.AsNoTracking().AsQueryable();
                        if (search.listVHisMrCheckItemExpression != null && search.listVHisMrCheckItemExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisMrCheckItemExpression)
                            {
