...
namespace MOS.DAO.HisQcNormation
{
    partial class HisQcNormationGet : EntityBase
    {
        ...
        {
            List<HIS_QC_NORMATION> list = new List<HIS_QC_NORMATION>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_QC_NORMATION.AsNoTracking().AsQueryable();
        ...
        public HIS_QC_NORMATION GetById(long id, HisQcNormationSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_QC_NORMATION.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisQcNormationExpression != null && search.listHisQcNormationExpression.Count > 0)
                        {
                            foreach (var item in search.listHisQcNormationExpression)
                            {
