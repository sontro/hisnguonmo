...
namespace MOS.DAO.HisServiceRereTime
{
    partial class HisServiceRereTimeGet : EntityBase
    {
        ...
        {
            List<HIS_SERVICE_RERE_TIME> list = new List<HIS_SERVICE_RERE_TIME>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SERVICE_RERE_TIME.AsNoTracking().AsQueryable();
                        if (search.listHisServiceRereTimeExpression != null && search.listHisServiceRereTimeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisServiceRereTimeExpression)
                            {
        ...
        public HIS_SERVICE_RERE_TIME GetById(long id, HisServiceRereTimeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_SERVICE_RERE_TIME.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisServiceRereTimeExpression != null && search.listHisServiceRereTimeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisServiceRereTimeExpression)
                            {
