...
{
    partial class HisDiimTypeDelete : EntityBase
    {
        public HisDiimTypeDelete()
            : base()
        {
        ...
        public bool Delete(HIS_DIIM_TYPE data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_DIIM_TYPE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_DIIM_TYPE> listIdNotFound = new List<HIS_DIIM_TYPE>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_DIIM_TYPE> listRaw = ctx.HIS_DIIM_TYPE.Where(o => listId.Contains(o.ID)).ToList();
