...
namespace MOS.DAO.HisDiimType
{
    ...
    {
        public List<HIS_DIIM_TYPE> Get(HisDiimTypeSO search, CommonParam param)
        {
            List<HIS_DIIM_TYPE> list = new List<HIS_DIIM_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_DIIM_TYPE.AsNoTracking().AsQueryable();
                        if (search.listHisDiimTypeExpression != null && search.listHisDiimTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisDiimTypeExpression)
                            {
        ...
        public HIS_DIIM_TYPE GetById(long id, HisDiimTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_DIIM_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisDiimTypeExpression != null && search.listHisDiimTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisDiimTypeExpression)
                            {
