...
namespace MOS.DAO.HisIcdCm
{
    partial class HisIcdCmGet : EntityBase
    {
        ...
        {
            List<HIS_ICD_CM> list = new List<HIS_ICD_CM>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_ICD_CM.AsNoTracking().AsQueryable();
                        if (search.listHisIcdCmExpression != null && search.listHisIcdCmExpression.Count > 0)
                        {
                            foreach (var item in search.listHisIcdCmExpression)
                            {
        ...
        public HIS_ICD_CM GetById(long id, HisIcdCmSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_ICD_CM.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisIcdCmExpression != null && search.listHisIcdCmExpression.Count > 0)
                        {
                            foreach (var item in search.listHisIcdCmExpression)
                            {
