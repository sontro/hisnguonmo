...
namespace MOS.DAO.HisHoreHandoverStt
{
    partial class HisHoreHandoverSttGet : EntityBase
    {
        ...
        {
            List<HIS_HORE_HANDOVER_STT> list = new List<HIS_HORE_HANDOVER_STT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_HORE_HANDOVER_STT.AsNoTracking().AsQueryable();
        ...
        public HIS_HORE_HANDOVER_STT GetById(long id, HisHoreHandoverSttSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_HORE_HANDOVER_STT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisHoreHandoverSttExpression != null && search.listHisHoreHandoverSttExpression.Count > 0)
                        {
                            foreach (var item in search.listHisHoreHandoverSttExpression)
                            {
