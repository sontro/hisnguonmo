...
{
    partial class HisCareTempCheck : EntityBase
    {
        public HisCareTempCheck()
            : base()
        {
        ...
        public bool ExistsCode(string code, long? id)
        {
            ...
            try
            {
                ...
                {
                    count = ctx.HIS_CARE_TEMP.AsQueryable().Where(p => p.CARE_TEMP_CODE.Equals(code) && p.ID != id && (p.IS_DELETE.HasValue || p.IS_DELETE.Value != (short)1)).Count();
                }
                result = (count > 0);
            }
            catch (Exception)
