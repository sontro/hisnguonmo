...
{
    partial class HisCareTempDelete : EntityBase
    {
        public HisCareTempDelete()
            : base()
        {
        ...
        public bool Delete(HIS_CARE_TEMP data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_CARE_TEMP> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_CARE_TEMP> listIdNotFound = new List<HIS_CARE_TEMP>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_CARE_TEMP> listRaw = ctx.HIS_CARE_TEMP.Where(o => listId.Contains(o.ID)).ToList();
