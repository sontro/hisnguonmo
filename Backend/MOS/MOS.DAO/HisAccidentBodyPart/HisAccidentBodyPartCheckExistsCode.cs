    ...
    partial class HisAccidentBodyPartCheck : EntityBase
    {
        public bool ExistsCode(string code, long? id)
        {
            ...
            try
            {
                ...
                {
                    count = ctx.HIS_ACCIDENT_BODY_PART.AsQueryable().Where(p => p.ACCIDENT_BODY_PART_CODE.Equals(code) && p.ID != id && (p.IS_DELETE.HasValue || p.IS_DELETE.Value != (short)1)).Count();
                }
                result = (count > 0);
            }
            catch (Exception)
