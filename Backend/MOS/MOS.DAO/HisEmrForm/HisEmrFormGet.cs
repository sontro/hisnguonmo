...
namespace MOS.DAO.HisEmrForm
{
    partial class HisEmrFormGet : EntityBase
    {
        ...
        {
            List<HIS_EMR_FORM> list = new List<HIS_EMR_FORM>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_EMR_FORM.AsNoTracking().AsQueryable();
                        if (search.listHisEmrFormExpression != null && search.listHisEmrFormExpression.Count > 0)
                        {
                            foreach (var item in search.listHisEmrFormExpression)
                            {
        ...
        public HIS_EMR_FORM GetById(long id, HisEmrFormSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_EMR_FORM.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisEmrFormExpression != null && search.listHisEmrFormExpression.Count > 0)
                        {
                            foreach (var item in search.listHisEmrFormExpression)
                            {
