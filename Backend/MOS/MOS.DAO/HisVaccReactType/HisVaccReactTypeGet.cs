...
{
    partial class HisVaccReactTypeGet : EntityBase
    {
        public List<HIS_VACC_REACT_TYPE> Get(HisVaccReactTypeSO search, CommonParam param)
        {
            List<HIS_VACC_REACT_TYPE> list = new List<HIS_VACC_REACT_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_VACC_REACT_TYPE.AsNoTracking().AsQueryable();
                        if (search.listHisVaccReactTypeExpression != null && search.listHisVaccReactTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisVaccReactTypeExpression)
                            {
        ...
        public HIS_VACC_REACT_TYPE GetById(long id, HisVaccReactTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_VACC_REACT_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisVaccReactTypeExpression != null && search.listHisVaccReactTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisVaccReactTypeExpression)
                            {
