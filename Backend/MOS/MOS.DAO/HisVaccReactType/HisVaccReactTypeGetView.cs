...
namespace MOS.DAO.HisVaccReactType
{
    partial class HisVaccReactTypeGet : EntityBase
    {
        ...
        {
            List<V_HIS_VACC_REACT_TYPE> list = new List<V_HIS_VACC_REACT_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_VACC_REACT_TYPE.AsNoTracking().AsQueryable();
                        if (search.listVHisVaccReactTypeExpression != null && search.listVHisVaccReactTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisVaccReactTypeExpression)
                            {
