...
namespace MOS.DAO.HisMestInventory
{
    partial class HisMestInventoryGet : EntityBase
    {
        ...
        {
            List<V_HIS_MEST_INVENTORY> list = new List<V_HIS_MEST_INVENTORY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_MEST_INVENTORY.AsQueryable();
                        if (search.listVHisMestInventoryExpression != null && search.listVHisMestInventoryExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisMestInventoryExpression)
                            {
