...
namespace MOS.DAO.HisMachineServMaty
{
    partial class HisMachineServMatyGet : EntityBase
    {
        ...
        {
            List<HIS_MACHINE_SERV_MATY> list = new List<HIS_MACHINE_SERV_MATY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MACHINE_SERV_MATY.AsNoTracking().AsQueryable();
        ...
        public HIS_MACHINE_SERV_MATY GetById(long id, HisMachineServMatySO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_MACHINE_SERV_MATY.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMachineServMatyExpression != null && search.listHisMachineServMatyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMachineServMatyExpression)
                            {
