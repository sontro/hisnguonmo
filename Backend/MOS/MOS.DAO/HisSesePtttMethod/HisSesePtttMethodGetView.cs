...
namespace MOS.DAO.HisSesePtttMethod
{
    partial class HisSesePtttMethodGet : EntityBase
    {
        ...
        {
            List<V_HIS_SESE_PTTT_METHOD> list = new List<V_HIS_SESE_PTTT_METHOD>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_SESE_PTTT_METHOD.AsNoTracking().AsQueryable();
                        if (search.listVHisSesePtttMethodExpression != null && search.listVHisSesePtttMethodExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisSesePtttMethodExpression)
                            {
