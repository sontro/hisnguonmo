...
namespace MOS.DAO.HisSesePtttMethod
{
    partial class HisSesePtttMethodGet : EntityBase
    {
        ...
        {
            List<HIS_SESE_PTTT_METHOD> list = new List<HIS_SESE_PTTT_METHOD>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SESE_PTTT_METHOD.AsNoTracking().AsQueryable();
        ...
        public HIS_SESE_PTTT_METHOD GetById(long id, HisSesePtttMethodSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_SESE_PTTT_METHOD.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisSesePtttMethodExpression != null && search.listHisSesePtttMethodExpression.Count > 0)
                        {
                            foreach (var item in search.listHisSesePtttMethodExpression)
                            {
