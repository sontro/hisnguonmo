...
{
    partial class HisBidTypeGet : EntityBase
    {
        public List<HIS_BID_TYPE> Get(HisBidTypeSO search, CommonParam param)
        {
            List<HIS_BID_TYPE> list = new List<HIS_BID_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_BID_TYPE.AsNoTracking().AsQueryable();
                        if (search.listHisBidTypeExpression != null && search.listHisBidTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBidTypeExpression)
                            {
        ...
        public HIS_BID_TYPE GetById(long id, HisBidTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_BID_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisBidTypeExpression != null && search.listHisBidTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBidTypeExpression)
                            {
