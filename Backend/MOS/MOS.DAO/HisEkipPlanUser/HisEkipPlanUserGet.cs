...
namespace MOS.DAO.HisEkipPlanUser
{
    partial class HisEkipPlanUserGet : EntityBase
    {
        ...
        {
            List<HIS_EKIP_PLAN_USER> list = new List<HIS_EKIP_PLAN_USER>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_EKIP_PLAN_USER.AsNoTracking().AsQueryable();
        ...
        public HIS_EKIP_PLAN_USER GetById(long id, HisEkipPlanUserSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_EKIP_PLAN_USER.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisEkipPlanUserExpression != null && search.listHisEkipPlanUserExpression.Count > 0)
                        {
                            foreach (var item in search.listHisEkipPlanUserExpression)
                            {
