...
namespace MOS.DAO.HisCashierAddConfig
{
    partial class HisCashierAddConfigGet : EntityBase
    {
        ...
        {
            List<HIS_CASHIER_ADD_CONFIG> list = new List<HIS_CASHIER_ADD_CONFIG>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_CASHIER_ADD_CONFIG.AsNoTracking().AsQueryable();
                        if (search.listHisCashierAddConfigExpression != null && search.listHisCashierAddConfigExpression.Count > 0)
                        {
                            foreach (var item in search.listHisCashierAddConfigExpression)
                            {
        ...
        public HIS_CASHIER_ADD_CONFIG GetById(long id, HisCashierAddConfigSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_CASHIER_ADD_CONFIG.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisCashierAddConfigExpression != null && search.listHisCashierAddConfigExpression.Count > 0)
                        {
                            foreach (var item in search.listHisCashierAddConfigExpression)
                            {
