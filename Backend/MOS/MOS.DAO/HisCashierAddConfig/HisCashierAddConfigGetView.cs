...
{
    partial class HisCashierAddConfigGet : EntityBase
    {
        public List<V_HIS_CASHIER_ADD_CONFIG> GetView(HisCashierAddConfigSO search, CommonParam param)
        {
            List<V_HIS_CASHIER_ADD_CONFIG> list = new List<V_HIS_CASHIER_ADD_CONFIG>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_CASHIER_ADD_CONFIG.AsNoTracking().AsQueryable();
                        if (search.listVHisCashierAddConfigExpression != null && search.listVHisCashierAddConfigExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisCashierAddConfigExpression)
                            {
