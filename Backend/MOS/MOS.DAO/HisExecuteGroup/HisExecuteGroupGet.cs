...
{
    partial class HisExecuteGroupGet : EntityBase
    {
        public List<HIS_EXECUTE_GROUP> Get(HisExecuteGroupSO search, CommonParam param)
        {
            List<HIS_EXECUTE_GROUP> list = new List<HIS_EXECUTE_GROUP>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_EXECUTE_GROUP.AsNoTracking().AsQueryable();
                        if (search.listHisExecuteGroupExpression != null && search.listHisExecuteGroupExpression.Count > 0)
                        {
                            foreach (var item in search.listHisExecuteGroupExpression)
                            {
        ...
        public HIS_EXECUTE_GROUP GetById(long id, HisExecuteGroupSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_EXECUTE_GROUP.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisExecuteGroupExpression != null && search.listHisExecuteGroupExpression.Count > 0)
                        {
                            foreach (var item in search.listHisExecuteGroupExpression)
                            {
