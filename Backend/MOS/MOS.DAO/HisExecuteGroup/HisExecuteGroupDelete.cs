...
{
    partial class HisExecuteGroupDelete : EntityBase
    {
        public HisExecuteGroupDelete()
            : base()
        {
        ...
        public bool Delete(HIS_EXECUTE_GROUP data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_EXECUTE_GROUP> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_EXECUTE_GROUP> listIdNotFound = new List<HIS_EXECUTE_GROUP>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_EXECUTE_GROUP> listRaw = ctx.HIS_EXECUTE_GROUP.Where(o => listId.Contains(o.ID)).ToList();
