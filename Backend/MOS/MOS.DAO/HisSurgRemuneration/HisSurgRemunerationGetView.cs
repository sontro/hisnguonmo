...
namespace MOS.DAO.HisSurgRemuneration
{
    partial class HisSurgRemunerationGet : EntityBase
    {
        ...
        {
            List<V_HIS_SURG_REMUNERATION> list = new List<V_HIS_SURG_REMUNERATION>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_SURG_REMUNERATION.AsNoTracking().AsQueryable();
                        if (search.listVHisSurgRemunerationExpression != null && search.listVHisSurgRemunerationExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisSurgRemunerationExpression)
                            {
