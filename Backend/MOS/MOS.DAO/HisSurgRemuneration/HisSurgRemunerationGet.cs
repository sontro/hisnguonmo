...
namespace MOS.DAO.HisSurgRemuneration
{
    partial class HisSurgRemunerationGet : EntityBase
    {
        ...
        {
            List<HIS_SURG_REMUNERATION> list = new List<HIS_SURG_REMUNERATION>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SURG_REMUNERATION.AsNoTracking().AsQueryable();
                        if (search.listHisSurgRemunerationExpression != null && search.listHisSurgRemunerationExpression.Count > 0)
                        {
                            foreach (var item in search.listHisSurgRemunerationExpression)
                            {
        ...
        public HIS_SURG_REMUNERATION GetById(long id, HisSurgRemunerationSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_SURG_REMUNERATION.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisSurgRemunerationExpression != null && search.listHisSurgRemunerationExpression.Count > 0)
                        {
                            foreach (var item in search.listHisSurgRemunerationExpression)
                            {
