...
namespace MOS.DAO.HisAccidentCare
{
    partial class HisAccidentCareGet : EntityBase
    {
        ...
        {
            List<HIS_ACCIDENT_CARE> list = new List<HIS_ACCIDENT_CARE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_ACCIDENT_CARE.AsNoTracking().AsQueryable();
        ...
        public HIS_ACCIDENT_CARE GetById(long id, HisAccidentCareSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_ACCIDENT_CARE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisAccidentCareExpression != null && search.listHisAccidentCareExpression.Count > 0)
                        {
                            foreach (var item in search.listHisAccidentCareExpression)
                            {
