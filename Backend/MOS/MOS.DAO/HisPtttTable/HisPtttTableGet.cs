...
namespace MOS.DAO.HisPtttTable
{
    partial class HisPtttTableGet : EntityBase
    {
        ...
        {
            List<HIS_PTTT_TABLE> list = new List<HIS_PTTT_TABLE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_PTTT_TABLE.AsNoTracking().AsQueryable();
                        if (search.listHisPtttTableExpression != null && search.listHisPtttTableExpression.Count > 0)
                        {
                            foreach (var item in search.listHisPtttTableExpression)
                            {
        ...
        public HIS_PTTT_TABLE GetById(long id, HisPtttTableSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_PTTT_TABLE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisPtttTableExpression != null && search.listHisPtttTableExpression.Count > 0)
                        {
                            foreach (var item in search.listHisPtttTableExpression)
                            {
