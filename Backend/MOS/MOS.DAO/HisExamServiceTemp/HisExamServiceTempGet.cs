...
namespace MOS.DAO.HisExamServiceTemp
{
    partial class HisExamServiceTempGet : EntityBase
    {
        ...
        {
            List<HIS_EXAM_SERVICE_TEMP> list = new List<HIS_EXAM_SERVICE_TEMP>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_EXAM_SERVICE_TEMP.AsNoTracking().AsQueryable();
        ...
        public HIS_EXAM_SERVICE_TEMP GetById(long id, HisExamServiceTempSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_EXAM_SERVICE_TEMP.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisExamServiceTempExpression != null && search.listHisExamServiceTempExpression.Count > 0)
                        {
                            foreach (var item in search.listHisExamServiceTempExpression)
                            {
