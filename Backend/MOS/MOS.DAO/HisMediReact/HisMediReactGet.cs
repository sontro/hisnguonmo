...
namespace MOS.DAO.HisMediReact
{
    partial class HisMediReactGet : EntityBase
    {
        ...
        {
            List<HIS_MEDI_REACT> list = new List<HIS_MEDI_REACT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MEDI_REACT.AsNoTracking().AsQueryable();
        ...
        public HIS_MEDI_REACT GetById(long id, HisMediReactSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_MEDI_REACT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMediReactExpression != null && search.listHisMediReactExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMediReactExpression)
                            {
