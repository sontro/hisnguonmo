...
{
    partial class HisMediReactDelete : EntityBase
    {
        public HisMediReactDelete()
            : base()
        {
        ...
        public bool Delete(HIS_MEDI_REACT data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<HIS_MEDI_REACT> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<HIS_MEDI_REACT> listIdNotFound = new List<HIS_MEDI_REACT>();
                    ...
                    else
                    {
                        ...
                        {
                            List<HIS_MEDI_REACT> listRaw = ctx.HIS_MEDI_REACT.Where(o => listId.Contains(o.ID)).ToList();
