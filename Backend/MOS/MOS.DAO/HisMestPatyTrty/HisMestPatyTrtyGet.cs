...
namespace MOS.DAO.HisMestPatyTrty
{
    partial class HisMestPatyTrtyGet : EntityBase
    {
        ...
        {
            List<HIS_MEST_PATY_TRTY> list = new List<HIS_MEST_PATY_TRTY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MEST_PATY_TRTY.AsNoTracking().AsQueryable();
        ...
        public HIS_MEST_PATY_TRTY GetById(long id, HisMestPatyTrtySO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_MEST_PATY_TRTY.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMestPatyTrtyExpression != null && search.listHisMestPatyTrtyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMestPatyTrtyExpression)
                            {
