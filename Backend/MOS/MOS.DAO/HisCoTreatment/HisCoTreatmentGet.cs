...
namespace MOS.DAO.HisCoTreatment
{
    partial class HisCoTreatmentGet : EntityBase
    {
        ...
        {
            List<HIS_CO_TREATMENT> list = new List<HIS_CO_TREATMENT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_CO_TREATMENT.AsNoTracking().AsQueryable();
        ...
        public HIS_CO_TREATMENT GetById(long id, HisCoTreatmentSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_CO_TREATMENT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisCoTreatmentExpression != null && search.listHisCoTreatmentExpression.Count > 0)
                        {
                            foreach (var item in search.listHisCoTreatmentExpression)
                            {
