...
namespace MOS.DAO.HisAllergyCard
{
    partial class HisAllergyCardGet : EntityBase
    {
        ...
        {
            List<HIS_ALLERGY_CARD> list = new List<HIS_ALLERGY_CARD>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_ALLERGY_CARD.AsNoTracking().AsQueryable();
                        if (search.listHisAllergyCardExpression != null && search.listHisAllergyCardExpression.Count > 0)
                        {
                            foreach (var item in search.listHisAllergyCardExpression)
                            {
        ...
        public HIS_ALLERGY_CARD GetById(long id, HisAllergyCardSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_ALLERGY_CARD.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisAllergyCardExpression != null && search.listHisAllergyCardExpression.Count > 0)
                        {
                            foreach (var item in search.listHisAllergyCardExpression)
                            {
