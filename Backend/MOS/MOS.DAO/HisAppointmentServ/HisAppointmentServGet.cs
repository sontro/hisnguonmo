...
namespace MOS.DAO.HisAppointmentServ
{
    partial class HisAppointmentServGet : EntityBase
    {
        ...
        {
            List<HIS_APPOINTMENT_SERV> list = new List<HIS_APPOINTMENT_SERV>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_APPOINTMENT_SERV.AsNoTracking().AsQueryable();
                        if (search.listHisAppointmentServExpression != null && search.listHisAppointmentServExpression.Count > 0)
                        {
                            foreach (var item in search.listHisAppointmentServExpression)
                            {
        ...
        public HIS_APPOINTMENT_SERV GetById(long id, HisAppointmentServSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_APPOINTMENT_SERV.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisAppointmentServExpression != null && search.listHisAppointmentServExpression.Count > 0)
                        {
                            foreach (var item in search.listHisAppointmentServExpression)
                            {
