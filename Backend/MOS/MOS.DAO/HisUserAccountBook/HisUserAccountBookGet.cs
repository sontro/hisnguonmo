...
namespace MOS.DAO.HisUserAccountBook
{
    partial class HisUserAccountBookGet : EntityBase
    {
        ...
        {
            List<HIS_USER_ACCOUNT_BOOK> list = new List<HIS_USER_ACCOUNT_BOOK>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_USER_ACCOUNT_BOOK.AsNoTracking().AsQueryable();
                        if (search.listHisUserAccountBookExpression != null && search.listHisUserAccountBookExpression.Count > 0)
                        {
                            foreach (var item in search.listHisUserAccountBookExpression)
                            {
        ...
        public HIS_USER_ACCOUNT_BOOK GetById(long id, HisUserAccountBookSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_USER_ACCOUNT_BOOK.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisUserAccountBookExpression != null && search.listHisUserAccountBookExpression.Count > 0)
                        {
                            foreach (var item in search.listHisUserAccountBookExpression)
                            {
