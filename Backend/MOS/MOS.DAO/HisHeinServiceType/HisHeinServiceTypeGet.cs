...
namespace MOS.DAO.HisHeinServiceType
{
    partial class HisHeinServiceTypeGet : EntityBase
    {
        ...
        {
            List<HIS_HEIN_SERVICE_TYPE> list = new List<HIS_HEIN_SERVICE_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_HEIN_SERVICE_TYPE.AsNoTracking().AsQueryable();
        ...
        public HIS_HEIN_SERVICE_TYPE GetById(long id, HisHeinServiceTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HIS_HEIN_SERVICE_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisHeinServiceTypeExpression != null && search.listHisHeinServiceTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisHeinServiceTypeExpression)
                            {
