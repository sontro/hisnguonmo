...
{
    class TokenSysLoginByEmailBehaviorEv : BeanObjectBase, ITokenSysLoginByEmail
    {
        LoginByEmailTDO entity;
        string ApplicationCode = "";
        ACS_TOKEN currentToken = null;
        ACS_USER userresult;

        internal TokenSysLoginByEmailBehaviorEv(CommonParam param, LoginByEmailTDO data)
            : base(param)
        {
            entity = data;
        }
        ...
        Inventec.Token.Core.TokenData ITokenSysLoginByEmail.Run()
        {
            ...
            try
            {
                if (Check())
                {
                    var userresults = new AcsUserBO().Get<List<ACS_USER>>(new AcsUserFilterQuery()
                    {
                        EMAIL = entity.EMAIL
                    });
                    userresult = userresults != null ? userresults.FirstOrDefault() : null;
                    if (userresult != null)
                    {
                        ...
                        if (valid)
                        {
                            ...
                            if (tokenBO.Authentication(authenticationSDO))
                            {
                                ...
                                if (InsertTokenDataToDb(userresult, commonParam))
                                {
                                    ...
                                    LogEventLoginByEmailSuccess();
                                }
                                else
                                {
                                    Inventec.Common.Logging.LogSystem.Warn("Call api TokenSysLoginByEmail Authentication thanh cong, tuy nhien InsertTokenDataToDb that bai. Dang nhap that bai. Du lieu dau vao____" + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => entity), entity));
                                }
                            }
                            else
                            {
                                CopyCommonParamInfo(tokenBO);
                                Inventec.Common.Logging.LogSystem.Warn("Call api TokenSysLoginByEmail that bai, Authentication that bai. Du lieu dau vao____" + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => entity), entity));
                                result = null;
                            }
                        }
                        else
                    ...
                    {
                        result = null;
                        Inventec.Common.Logging.LogSystem.Warn("Call api TokenSysLoginByEmail that bai, Khong tim thay user tuong ung voi loginname gui len____" + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => entity), entity));
                ...
                {
                    Inventec.Common.Logging.LogSystem.Warn("Call api TokenSysLoginByEmail that bai, du lieu nguoi dung gui len khong hop le____" + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => entity), entity));
                }
            }
            catch (Exception ex)
            {
        ...
        async Task LogEventLoginByEmailSuccess()
        {
            try
            {
                ...
                eventLog.LogginName = userresult.LOGINNAME;
        ...
        bool Check()
        {
            ...
            try
            {
                ...
                if (String.IsNullOrEmpty(entity.EMAIL)) throw new ArgumentNullException("EMAIL");
                ...
                if (!String.IsNullOrEmpty(entity.APPLICATION_CODE))
                    this.ApplicationCode = entity.APPLICATION_CODE;
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
        ...
        private bool CheckUserIsActive(ACS_USER data)
        {
            ...
            try
            {
                ...
                if (!valid && data != null)
                {
                    ...
                    Inventec.Common.Logging.LogSystem.Warn("Call api TokenSysLoginByEmail that bai____" + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => param), param));
                }
            }
            catch (Exception ex)
            {
