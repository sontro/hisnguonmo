...
{
    class AcsRoleCheckVerifyIsUnlock
    {
        ...
        internal static bool Verify(CommonParam param, ACS_ROLE data)
        {
            bool result = true;
            try
            {
        ...
        internal static bool Verify(CommonParam param, List<long> ids)
        {
            ...
            try
            {
                if (ids != null && ids.Count > 0)
                {
                    ...
                    filter.IDs = ids;
                    List<ACS_ROLE> listData = new AcsRoleBO().Get<List<ACS_ROLE>>(filter);
        ...
        internal static bool Verify(CommonParam param, List<ACS_ROLE> datas)
        {
            bool result = true;
            try
            {
        ...
        private static bool Check(CommonParam param, ACS_ROLE raw)
        {
            bool result = true;
            if (raw == null)
            {
