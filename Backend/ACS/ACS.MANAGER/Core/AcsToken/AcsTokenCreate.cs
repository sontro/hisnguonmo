...
namespace ACS.MANAGER.AcsToken
{
    partial class AcsTokenCreate : BusinessBase
    {
		...
		internal bool CreateList(List<ACS_TOKEN> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    if (!DAOWorker.AcsTokenDAO.CreateList(listData))
                    {
                        ACS.MANAGER.Base.BugUtil.SetBugCode(param, ACS.LibraryBug.Bug.Enum.AcsToken_ThemMoiThatBai);
                        throw new Exception("Them moi thong tin AcsToken that bai." + LogUtil.TraceData("listData", listData));
                    }
                    this.recentAcsTokens.AddRange(listData);
                    result = true;
                }
            }
            catch (Exception ex)
