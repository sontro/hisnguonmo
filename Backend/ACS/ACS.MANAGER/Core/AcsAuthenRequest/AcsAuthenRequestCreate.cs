...
namespace ACS.MANAGER.AcsAuthenRequest
{
    partial class AcsAuthenRequestCreate : BusinessBase
    {
		...
		internal bool CreateList(List<ACS_AUTHEN_REQUEST> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    if (!DAOWorker.AcsAuthenRequestDAO.CreateList(listData))
                    {
                        ACS.MANAGER.Base.BugUtil.SetBugCode(param, ACS.LibraryBug.Bug.Enum.AcsAuthenRequest_ThemMoiThatBai);
                        throw new Exception("Them moi thong tin AcsAuthenRequest that bai." + LogUtil.TraceData("listData", listData));
                    }
                    this.recentAcsAuthenRequests.AddRange(listData);
                    result = true;
                }
            }
            catch (Exception ex)
