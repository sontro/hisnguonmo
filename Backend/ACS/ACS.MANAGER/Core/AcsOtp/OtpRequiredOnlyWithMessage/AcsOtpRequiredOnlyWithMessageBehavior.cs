...
{
    class AcsOtpOtpRequiredOnlyWithMessageBehaviorEv : BeanObjectBase, IAcsOtpOtpRequiredOnlyWithMessage
    {
        OtpRequiredOnlyWithMessageSDO entity;
        ACS_APPLICATION application;
        ACS_APP_OTP_TYPE currentAppOtpType;
        ACS_OTP_TYPE currentOtpType;

        internal AcsOtpOtpRequiredOnlyWithMessageBehaviorEv(CommonParam param, OtpRequiredOnlyWithMessageSDO data)
            : base(param)
        {
            entity = data;
        }
        ...
        /// </summary>
        /// <returns></returns>
        bool IAcsOtpOtpRequiredOnlyWithMessage.Run()
        {
            bool result = false;
            try
            {
                if (Valid())
                {
                    ...
                    {
                        Inventec.Common.Logging.LogSystem.Warn("Yeu cau doi xac thuc qua so dien thoai that bai. Cap nhat thong tin ma xac thuc va thoi han xac thuc vao bang acs_otp that bai. Can kiem tra lai. " + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => entity), entity));
                    }
                }
            }
            catch (Exception ex)
            ...
            try
            {
                if (!String.IsNullOrEmpty(mobile))
                {
                    ...
                    else if (!String.IsNullOrEmpty(entity.Message_Format))
                    {
                        ...
                        smsMtSDO.TemplateCode = "Otp dang ky kham: {OTP_CODE}. Hieu luc den: {EXPIRE_TIME}.";
        ...
        double GetActivationMinitePlus()
        {
            ...
            try
            {
                if (currentOtpType != null && currentOtpType.EXPIRE_TIME_NUMBER.HasValue)
                {
                    ...
                    minutePlus = int.Parse(System.Configuration.ConfigurationSettings.AppSettings["ACS.AcsSystem.OtpRequiredOnlyWithMessage.Timeout"] ?? "5");
            }
            catch (Exception ex)
            {
                minutePlus = 60;
