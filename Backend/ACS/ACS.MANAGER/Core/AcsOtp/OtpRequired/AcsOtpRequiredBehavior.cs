...
{
    class AcsOtpOtpRequiredBehaviorEv : BeanObjectBase, IAcsOtpOtpRequired
    {
        OtpRequiredSDO entity;
        ACS_APPLICATION application;
        ACS_APP_OTP_TYPE currentAppOtpType;
        ACS_OTP_TYPE currentOtpType;

        internal AcsOtpOtpRequiredBehaviorEv(CommonParam param, OtpRequiredSDO data)
            : base(param)
        {
            entity = data;
        }
        ...
        bool IAcsOtpOtpRequired.Run()
        {
            ...
            try
            {
                if (Valid())
                {
                    ...
                    if (user != null && GetPrefixMobileNumber(user.MOBILE) != GetPrefixMobileNumber(entity.Mobile))
                    {
                    ...
                    else
                    {
                        if (!String.IsNullOrEmpty(mobile))
                        {
                            ...
                            {
                                Inventec.Common.Logging.LogSystem.Warn("Yeu cau doi mat khau tai khoan that bai. " + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => entity), entity));
                            ...
                            {
                                Inventec.Common.Logging.LogSystem.Warn("Yeu cau doi mat khau tai khoan thanh cong. tuy nhien gui ma kich hoat va thoi han kich hoat sms den so dien thoai " + mobile + " that bai. Can kiem tra lai. " + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => entity), entity));
                                ACS.MANAGER.Base.BugUtil.SetBugCode(param, ACS.LibraryBug.Bug.Enum.Common__CapNhatThatBai);
                                MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.Core_AcsOtp_GuiSmsMaDoiMatKhauDenSDTThatBai);
                            }
                        }
                        else
                        {
