...
{
    class AcsOtpOtpRequiredOnlyBehaviorEv : BeanObjectBase, IAcsOtpOtpRequiredOnly
    {
        OtpRequiredOnlySDO entity;
        ACS_APPLICATION application;
        ACS_APP_OTP_TYPE currentAppOtpType;
        ACS_OTP_TYPE currentOtpType;

        internal AcsOtpOtpRequiredOnlyBehaviorEv(CommonParam param, OtpRequiredOnlySDO data)
            : base(param)
        {
            entity = data;
        }
        ...
        /// </summary>
        /// <returns></returns>
        bool IAcsOtpOtpRequiredOnly.Run()
        {
            bool result = false;
            try
            {
                if (Valid())
                {
                    ...
                    {
                        Inventec.Common.Logging.LogSystem.Warn("Tao yeu cau cap otp xac thuc that bai. Can kiem tra lai. " + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => entity), entity));
                    ...
                    {
                        Inventec.Common.Logging.LogSystem.Warn("Tao yeu cau cap otp xac thuc thanh cong. Tuy nhien gui sms xac thuc qua so dien thoai that bai. Can kiem tra lai. " + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => entity), entity));                        
                    }
                }
            }
            catch (Exception ex)
            ...
            try
            {
                if (!String.IsNullOrEmpty(mobile))
                {
                    ...
                    if (currentAppOtpType != null)
                    {
                        ...
                        smsMtSDO.TemplateCode = "Otp dat ve su kien: {OTP_CODE}. Hieu luc den: {EXPIRE_TIME}.";
        ...
        double GetActivationMinitePlus()
        {
            ...
            try
            {
                if (currentOtpType != null && currentOtpType.EXPIRE_TIME_NUMBER.HasValue)
                {
                    ...
                    minutePlus = int.Parse(System.Configuration.ConfigurationSettings.AppSettings["ACS.AcsSystem.OtpRequiredOnly.Timeout"] ?? "5");
            }
            catch (Exception ex)
            {
                minutePlus = 60;
