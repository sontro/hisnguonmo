...
{
    public class AcsControlViewFilterQuery : AcsControlViewFilter
    {
        public AcsControlViewFilterQuery()
            : base()
        {

        }

        internal List<System.Linq.Expressions.Expression<Func<V_ACS_CONTROL, bool>>> listExpression = new List<System.Linq.Expressions.Expression<Func<V_ACS_CONTROL, bool>>>();
            ...
            try
            {
                ...
                if (!String.IsNullOrEmpty(this.KEY_WORD))
                {
                    ...
                    listExpression.Add(o =>
                        ...
                        (o.MODULE_NAME != null && o.MODULE_NAME.ToLower().Contains(this.KEY_WORD)) ||
                        o.CONTROL_CODE.ToLower().Contains(this.KEY_WORD) ||
                        o.CONTROL_NAME.ToLower().Contains(this.KEY_WORD)
                        );
                }

                search.listVAcsControlExpression.AddRange(listExpression);
                search.OrderField = OrderProcess.GetOrderField<V_ACS_CONTROL>(ORDER_FIELD);
                search.OrderDirection = OrderProcess.GetOrderDirection(ORDER_DIRECTION);
            }
            catch (Exception ex)
            {
                LogSystem.Error(ex);
                search.listVAcsControlExpression.Clear();
                search.listVAcsControlExpression.Add(o => o.ID == NEGATIVE_ID);
            }
            return search;
        }
    }
