...
using ACS.MANAGER.Core.AcsModuleGroup.Get.ListV;
using ACS.MANAGER.Core.AcsModuleGroup.Get.V;
using Inventec.Core;
using System;
    ...
    partial class AcsModuleGroupGet : BeanObjectBase, IDelegacyT
    {
        ...
        T IDelegacyT.Execute<T>()
        {
            ...
            try
            {
                ...
                else if (typeof(T) == typeof(List<V_ACS_MODULE_GROUP>))
                {
                    IAcsModuleGroupGetListV behavior = AcsModuleGroupGetListVBehaviorFactory.MakeIAcsModuleGroupGetListV(param, entity);
                    result = behavior != null ? (T)System.Convert.ChangeType(behavior.Run(), typeof(T)) : default(T);
                }
                else if (typeof(T) == typeof(V_ACS_MODULE_GROUP))
                {
                    IAcsModuleGroupGetV behavior = AcsModuleGroupGetVBehaviorFactory.MakeIAcsModuleGroupGetV(param, entity);
                    result = behavior != null ? (T)System.Convert.ChangeType(behavior.Run(), typeof(T)) : default(T);
                }
            }
            catch (Exception ex)
