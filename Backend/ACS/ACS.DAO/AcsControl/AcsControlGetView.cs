...
namespace ACS.DAO.AcsControl
{
    partial class AcsControlGet : EntityBase
    {
        ...
        {
            List<V_ACS_CONTROL> list = new List<V_ACS_CONTROL>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_ACS_CONTROL.AsQueryable();
                        if (search.listVAcsControlExpression != null && search.listVAcsControlExpression.Count > 0)
                        {
                            foreach (var item in search.listVAcsControlExpression)
                            {
