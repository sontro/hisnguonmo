...
{
    partial class AcsControlGet : EntityBase
    {
        public List<ACS_CONTROL> Get(AcsControlSO search, CommonParam param)
        {
            List<ACS_CONTROL> list = new List<ACS_CONTROL>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.ACS_CONTROL.AsQueryable();
                        if (search.listAcsControlExpression != null && search.listAcsControlExpression.Count > 0)
                        {
                            foreach (var item in search.listAcsControlExpression)
                            {
        ...
        public ACS_CONTROL GetById(long id, AcsControlSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.ACS_CONTROL.AsQueryable().Where(p => p.ID == id);
                        if (search.listAcsControlExpression != null && search.listAcsControlExpression.Count > 0)
                        {
                            foreach (var item in search.listAcsControlExpression)
                            {
