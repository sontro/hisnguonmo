...
{
    partial class AcsControlDelete : EntityBase
    {
        public AcsControlDelete()
            : base()
        {
        ...
        public bool Delete(ACS_CONTROL data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<ACS_CONTROL> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<ACS_CONTROL> listIdNotFound = new List<ACS_CONTROL>();
                    ...
                    else
                    {
                        ...
                        {
                            List<ACS_CONTROL> listRaw = ctx.ACS_CONTROL.Where(o => listId.Contains(o.ID)).ToList();
