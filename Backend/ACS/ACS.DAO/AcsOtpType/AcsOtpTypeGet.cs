...
namespace ACS.DAO.AcsOtpType
{
    partial class AcsOtpTypeGet : EntityBase
    {
        ...
        {
            List<ACS_OTP_TYPE> list = new List<ACS_OTP_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.ACS_OTP_TYPE.AsQueryable();
                        if (search.listAcsOtpTypeExpression != null && search.listAcsOtpTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listAcsOtpTypeExpression)
                            {
        ...
        public ACS_OTP_TYPE GetById(long id, AcsOtpTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.ACS_OTP_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listAcsOtpTypeExpression != null && search.listAcsOtpTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listAcsOtpTypeExpression)
                            {
