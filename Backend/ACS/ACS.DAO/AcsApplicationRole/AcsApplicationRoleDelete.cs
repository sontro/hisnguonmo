...
{
    partial class AcsApplicationRoleDelete : EntityBase
    {
        public AcsApplicationRoleDelete()
            : base()
        {
        ...
        public bool Delete(ACS_APPLICATION_ROLE data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<ACS_APPLICATION_ROLE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<ACS_APPLICATION_ROLE> listIdNotFound = new List<ACS_APPLICATION_ROLE>();
                    ...
                    else
                    {
                        ...
                        {
                            List<ACS_APPLICATION_ROLE> listRaw = ctx.ACS_APPLICATION_ROLE.Where(o => listId.Contains(o.ID)).ToList();
