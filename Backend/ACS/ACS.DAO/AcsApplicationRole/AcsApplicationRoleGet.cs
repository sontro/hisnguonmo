...
namespace ACS.DAO.AcsApplicationRole
{
    partial class AcsApplicationRoleGet : EntityBase
    {
        ...
        {
            List<ACS_APPLICATION_ROLE> list = new List<ACS_APPLICATION_ROLE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.ACS_APPLICATION_ROLE.AsQueryable();
                        if (search.listAcsApplicationRoleExpression != null && search.listAcsApplicationRoleExpression.Count > 0)
                        {
                            foreach (var item in search.listAcsApplicationRoleExpression)
                            {
        ...
        public ACS_APPLICATION_ROLE GetById(long id, AcsApplicationRoleSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.ACS_APPLICATION_ROLE.AsQueryable().Where(p => p.ID == id);
                        if (search.listAcsApplicationRoleExpression != null && search.listAcsApplicationRoleExpression.Count > 0)
                        {
                            foreach (var item in search.listAcsApplicationRoleExpression)
                            {
