...
namespace ACS.DAO.AcsActivityLog
{
    ...
    {
        public List<V_ACS_ACTIVITY_LOG> GetView(AcsActivityLogSO search, CommonParam param)
        {
            List<V_ACS_ACTIVITY_LOG> list = new List<V_ACS_ACTIVITY_LOG>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_ACS_ACTIVITY_LOG.AsQueryable();
