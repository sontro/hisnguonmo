...
{
    partial class AcsTokenDelete : EntityBase
    {
        public AcsTokenDelete()
            : base()
        {
        ...
        public bool Delete(ACS_TOKEN data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<ACS_TOKEN> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<ACS_TOKEN> listIdNotFound = new List<ACS_TOKEN>();
                    ...
                    else
                    {
                        ...
                        {
                            List<ACS_TOKEN> listRaw = ctx.ACS_TOKEN.Where(o => listId.Contains(o.ID)).ToList();
