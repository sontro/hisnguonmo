...
namespace ACS.DAO.AcsToken
{
    partial class AcsTokenGet : EntityBase
    {
        ...
        {
            List<ACS_TOKEN> list = new List<ACS_TOKEN>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.ACS_TOKEN.AsNoTracking().AsQueryable();
                        if (search.listAcsTokenExpression != null && search.listAcsTokenExpression.Count > 0)
                        {
                            foreach (var item in search.listAcsTokenExpression)
                            {
        ...
        public ACS_TOKEN GetById(long id, AcsTokenSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.ACS_TOKEN.AsQueryable().Where(p => p.ID == id);
                        if (search.listAcsTokenExpression != null && search.listAcsTokenExpression.Count > 0)
                        {
                            foreach (var item in search.listAcsTokenExpression)
                            {
