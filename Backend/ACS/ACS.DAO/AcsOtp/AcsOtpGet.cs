...
namespace ACS.DAO.AcsOtp
{
    partial class AcsOtpGet : EntityBase
    {
        ...
        {
            List<ACS_OTP> list = new List<ACS_OTP>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.ACS_OTP.AsNoTracking().AsQueryable();
                        if (search.listAcsOtpExpression != null && search.listAcsOtpExpression.Count > 0)
                        {
                            foreach (var item in search.listAcsOtpExpression)
                            {
        ...
        public ACS_OTP GetById(long id, AcsOtpSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.ACS_OTP.AsQueryable().Where(p => p.ID == id);
                        if (search.listAcsOtpExpression != null && search.listAcsOtpExpression.Count > 0)
                        {
                            foreach (var item in search.listAcsOtpExpression)
                            {
