...
{
    partial class AcsOtpGet : EntityBase
    {
        public List<V_ACS_OTP> GetView(AcsOtpSO search, CommonParam param)
        {
            List<V_ACS_OTP> list = new List<V_ACS_OTP>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_ACS_OTP.AsNoTracking().AsQueryable();
                        if (search.listVAcsOtpExpression != null && search.listVAcsOtpExpression.Count > 0)
                        {
                            foreach (var item in search.listVAcsOtpExpression)
                            {
