...
{
    partial class AcsActivityTypeDelete : EntityBase
    {
        public AcsActivityTypeDelete()
            : base()
        {
        ...
        public bool Delete(ACS_ACTIVITY_TYPE data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<ACS_ACTIVITY_TYPE> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<ACS_ACTIVITY_TYPE> listIdNotFound = new List<ACS_ACTIVITY_TYPE>();
                    ...
                    else
                    {
                        ...
                        {
                            List<ACS_ACTIVITY_TYPE> listRaw = ctx.ACS_ACTIVITY_TYPE.Where(o => listId.Contains(o.ID)).ToList();
