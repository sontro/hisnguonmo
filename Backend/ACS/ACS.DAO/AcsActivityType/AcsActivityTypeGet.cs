...
namespace ACS.DAO.AcsActivityType
{
    partial class AcsActivityTypeGet : EntityBase
    {
        ...
        {
            List<ACS_ACTIVITY_TYPE> list = new List<ACS_ACTIVITY_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.ACS_ACTIVITY_TYPE.AsQueryable();
        ...
        public ACS_ACTIVITY_TYPE GetById(long id, AcsActivityTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.ACS_ACTIVITY_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listAcsActivityTypeExpression != null && search.listAcsActivityTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listAcsActivityTypeExpression)
                            {
