...
namespace ACS.DAO.AcsRoleUser
{
    partial class AcsRoleUserGet : EntityBase
    {
        ...
        {
            List<ACS_ROLE_USER> list = new List<ACS_ROLE_USER>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.ACS_ROLE_USER.AsQueryable();
                        if (search.listAcsRoleUserExpression != null && search.listAcsRoleUserExpression.Count > 0)
                        {
                            foreach (var item in search.listAcsRoleUserExpression)
                            {
        ...
        public ACS_ROLE_USER GetById(long id, AcsRoleUserSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.ACS_ROLE_USER.AsQueryable().Where(p => p.ID == id);
                        if (search.listAcsRoleUserExpression != null && search.listAcsRoleUserExpression.Count > 0)
                        {
                            foreach (var item in search.listAcsRoleUserExpression)
                            {
