...
{
    partial class AcsCredentialDataTruncate : EntityBase
    {
        public AcsCredentialDataTruncate()
            : base()
        {
        ...
        public bool Truncate(ACS_CREDENTIAL_DATA data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Truncate(data.ID);
        }

        public bool TruncateList(List<ACS_CREDENTIAL_DATA> listData)
        {
            bool result = false;
            try
            {
