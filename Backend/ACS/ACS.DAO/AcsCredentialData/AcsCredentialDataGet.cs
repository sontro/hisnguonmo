...
namespace ACS.DAO.AcsCredentialData
{
    ...
    {
        public List<ACS_CREDENTIAL_DATA> Get(AcsCredentialDataSO search, CommonParam param)
        {
            List<ACS_CREDENTIAL_DATA> list = new List<ACS_CREDENTIAL_DATA>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.ACS_CREDENTIAL_DATA.AsQueryable();
                        if (search.listAcsCredentialDataExpression != null && search.listAcsCredentialDataExpression.Count > 0)
                        {
                            foreach (var item in search.listAcsCredentialDataExpression)
                            {
        ...
        public ACS_CREDENTIAL_DATA GetById(long id, AcsCredentialDataSO search)
        {
            ACS_CREDENTIAL_DATA result = null;
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.ACS_CREDENTIAL_DATA.AsQueryable().Where(p => p.ID == id);
                        if (search.listAcsCredentialDataExpression != null && search.listAcsCredentialDataExpression.Count > 0)
                        {
                            foreach (var item in search.listAcsCredentialDataExpression)
                            {
