...
{
    partial class AcsApplicationTruncate : EntityBase
    {
        public AcsApplicationTruncate()
            : base()
        {
        ...
        public bool Truncate(ACS_APPLICATION data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Truncate(data.ID);
        }

        public bool TruncateList(List<ACS_APPLICATION> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<ACS_APPLICATION> listIdNotFound = new List<ACS_APPLICATION>();
                    ...
                    else
                    {
                        ...
                        {
                            List<ACS_APPLICATION> listRaw = ctx.ACS_APPLICATION.Where(o => listId.Contains(o.ID)).ToList();
