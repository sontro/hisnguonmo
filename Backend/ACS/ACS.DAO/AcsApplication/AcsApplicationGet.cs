...
namespace ACS.DAO.AcsApplication
{
    ...
    {
        public List<ACS_APPLICATION> Get(AcsApplicationSO search, CommonParam param)
        {
            List<ACS_APPLICATION> list = new List<ACS_APPLICATION>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.ACS_APPLICATION.AsQueryable();
        ...
        public ACS_APPLICATION GetById(long id, AcsApplicationSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.ACS_APPLICATION.AsQueryable().Where(p => p.ID == id);
                        if (search.listAcsApplicationExpression != null && search.listAcsApplicationExpression.Count > 0)
                        {
                            foreach (var item in search.listAcsApplicationExpression)
                            {
