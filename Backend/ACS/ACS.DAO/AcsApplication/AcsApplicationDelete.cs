...
{
    partial class AcsApplicationDelete : EntityBase
    {
        public AcsApplicationDelete()
            : base()
        {
        ...
        public bool Delete(ACS_APPLICATION data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<ACS_APPLICATION> listData)
        {
            bool result = false;
            try
            {
