...
{
    partial class AcsModuleGet : EntityBase
    {
        public List<ACS_MODULE> Get(AcsModuleSO search, CommonParam param)
        {
            List<ACS_MODULE> list = new List<ACS_MODULE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.ACS_MODULE.AsQueryable();
                        if (search.listAcsModuleExpression != null && search.listAcsModuleExpression.Count > 0)
                        {
                            foreach (var item in search.listAcsModuleExpression)
                            {
        ...
        public ACS_MODULE GetById(long id, AcsModuleSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.ACS_MODULE.AsQueryable().Where(p => p.ID == id);
                        if (search.listAcsModuleExpression != null && search.listAcsModuleExpression.Count > 0)
                        {
                            foreach (var item in search.listAcsModuleExpression)
                            {
