...
namespace ACS.DAO.AcsModule
{
    partial class AcsModuleGet : EntityBase
    {
        ...
        {
            List<V_ACS_MODULE> list = new List<V_ACS_MODULE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_ACS_MODULE.AsQueryable();
                        if (search.listVAcsModuleExpression != null && search.listVAcsModuleExpression.Count > 0)
                        {
                            foreach (var item in search.listVAcsModuleExpression)
                            {
