...
namespace ACS.DAO.AcsControlRole
{
    partial class AcsControlRoleGet : EntityBase
    {
        ...
        {
            List<ACS_CONTROL_ROLE> list = new List<ACS_CONTROL_ROLE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.ACS_CONTROL_ROLE.AsQueryable();
        ...
        public ACS_CONTROL_ROLE GetById(long id, AcsControlRoleSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.ACS_CONTROL_ROLE.AsQueryable().Where(p => p.ID == id);
                        if (search.listAcsControlRoleExpression != null && search.listAcsControlRoleExpression.Count > 0)
                        {
                            foreach (var item in search.listAcsControlRoleExpression)
                            {
