...
namespace ACS.DAO.AcsModuleGroup
{
    partial class AcsModuleGroupGet : EntityBase
    {
        ...
        {
            List<V_ACS_MODULE_GROUP> list = new List<V_ACS_MODULE_GROUP>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_ACS_MODULE_GROUP.AsQueryable();
                        if (search.listVAcsModuleGroupExpression != null && search.listVAcsModuleGroupExpression.Count > 0)
                        {
                            foreach (var item in search.listVAcsModuleGroupExpression)
                            {
