...
namespace ACS.DAO.AcsModuleGroup
{
    ...
    {
        public List<ACS_MODULE_GROUP> Get(AcsModuleGroupSO search, CommonParam param)
        {
            List<ACS_MODULE_GROUP> list = new List<ACS_MODULE_GROUP>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.ACS_MODULE_GROUP.AsQueryable();
                        if (search.listAcsModuleGroupExpression != null && search.listAcsModuleGroupExpression.Count > 0)
                        {
                            foreach (var item in search.listAcsModuleGroupExpression)
                            {
        ...
        public ACS_MODULE_GROUP GetById(long id, AcsModuleGroupSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.ACS_MODULE_GROUP.AsQueryable().Where(p => p.ID == id);
                        if (search.listAcsModuleGroupExpression != null && search.listAcsModuleGroupExpression.Count > 0)
                        {
                            foreach (var item in search.listAcsModuleGroupExpression)
                            {
