...
{
    public partial class ManagerQcs : BusinessBase
    {

        public List<System.Data.DataTable> GetSum<Tfilter>(Tfilter filter,Dictionary<string, string> DicTdo, string query)
        {
            ...
            try
            {
                if (!String.IsNullOrWhiteSpace(query))
                {
                    ...
                    result = this.GetSum(dicFilter, DicTdo, query);
                }
            }
            catch (Exception ex)
            {
        ...
        internal List<System.Data.DataTable> GetSum(Dictionary<string, object> filter, Dictionary<string, string> DicTdo, string query)
        {
            List<System.Data.DataTable> result = new List<DataTable>();
            try
            {
                if (!String.IsNullOrWhiteSpace(query))
                {
                    ...
                    string appCode = DicTdo.ContainsKey("APP_CODE") ? DicTdo["APP_CODE"] : "";
                    if (string.IsNullOrEmpty(appCode))
                    {
                        throw new Exception(string.Format("APP_CODE is null"));
                    }
                    string PassWord = DicTdo.ContainsKey("PASSWORD") ? DicTdo["PASSWORD"] : "";
                    if (string.IsNullOrEmpty(PassWord))
                    {
                        throw new Exception(string.Format("PASSWORD is null"));
                    }
                    string LoginName = DicTdo.ContainsKey("LOGINNAME") ? DicTdo["LOGINNAME"] : "";
                    if (string.IsNullOrEmpty(LoginName))
                    {
                        throw new Exception(string.Format("LOGINNAME is null"));
                    }
                    string LinkAcs = DicTdo.ContainsKey("LINK_ACS") ? DicTdo["LINK_ACS"] : "";
                    if (string.IsNullOrEmpty(LinkAcs))
                    {
                        throw new Exception(string.Format("LINK_ACS is null"));
                    }
                    string LinkQcs = DicTdo.ContainsKey("LINK_QCS") ? DicTdo["LINK_QCS"] : "";
                    if (string.IsNullOrEmpty(LinkQcs))
                    {
                        throw new Exception(string.Format("LINK_QCS is null"));
                    }
                    foreach (var item in querys)
                    {
                        QCS.TDO.ExeStringTDO stringTDO = new QCS.TDO.ExeStringTDO();
                        stringTDO.Query = item;
                        stringTDO.IsAll = DicTdo.ContainsKey("IsAll")&&DicTdo["IsAll"].ToLower()=="true";
                        Inventec.Token.ClientSystem.ClientTokenManager qcsClientTokenManager = new Inventec.Token.ClientSystem.ClientTokenManager(appCode, LinkAcs);
                        qcsClientTokenManager.UseRegistry(true);
                        var TokenData = qcsClientTokenManager.Login(param, LoginName, PassWord);
                        var qcsConsumer = new Inventec.Common.WebApiClient.ApiConsumer(LinkQcs, appCode);
                        qcsConsumer.SetTokenCode(TokenData.TokenCode);
                        var resultData = new BackendAdapter(param).Post<QCS.SDO.ExeResultSDO>("api/QcsQueryLog/ExeStringCmd", qcsConsumer, stringTDO, param);

                        if (resultData != null && (param.Messages == null || param.Messages.Count == 0))
                        {
                            result.Add(resultData.QueryResult);
                        }
                        Inventec.Common.Logging.LogSystem.Info(item);
                    }
                }
            }
            catch (Exception ex)
