...
using TYT.MANAGER.Core.TytTuberculosis.Get.ListV;
using TYT.MANAGER.Core.TytTuberculosis.Get.V;
using Inventec.Core;
using System;
    ...
    partial class TytTuberculosisGet : BeanObjectBase, IDelegacyT
    {
        ...
        T IDelegacyT.Execute<T>()
        {
            ...
            try
            {
                ...
                else if (typeof(T) == typeof(List<V_TYT_TUBERCULOSIS>))
                {
                    ITytTuberculosisGetListV behavior = TytTuberculosisGetListVBehaviorFactory.MakeITytTuberculosisGetListV(param, entity);
                    result = behavior != null ? (T)System.Convert.ChangeType(behavior.Run(), typeof(T)) : default(T);
                }
                else if (typeof(T) == typeof(V_TYT_TUBERCULOSIS))
                {
                    ITytTuberculosisGetV behavior = TytTuberculosisGetVBehaviorFactory.MakeITytTuberculosisGetV(param, entity);
                    result = behavior != null ? (T)System.Convert.ChangeType(behavior.Run(), typeof(T)) : default(T);
                }
            }
            catch (Exception ex)
