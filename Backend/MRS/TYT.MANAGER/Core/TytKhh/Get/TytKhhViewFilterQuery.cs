...
namespace TYT.MANAGER.Core.TytKhh.Get
{
    public class TytKhhViewFilterQuery : TytKhhViewFilter
    {
        public TytKhhViewFilterQuery()
            : base()
        {

        }
        
        internal List<System.Linq.Expressions.Expression<Func<V_TYT_KHH, bool>>> listExpression = new List<System.Linq.Expressions.Expression<Func<V_TYT_KHH, bool>>>();

        internal Inventec.Backend.MANAGER.OrderProcessorBase OrderProcess = new Inventec.Backend.MANAGER.OrderProcessorBase();

        internal TytKhhSO Query()
        {
            TytKhhSO search = new TytKhhSO();
            try
            {
                ...
                search.listVTytKhhExpression.AddRange(listExpression);
                search.OrderField = OrderProcess.GetOrderField<V_TYT_KHH>(ORDER_FIELD);
                search.OrderDirection = OrderProcess.GetOrderDirection(ORDER_DIRECTION);
            }
            catch (Exception ex)
            {
                LogSystem.Error(ex);
                search.listVTytKhhExpression.Clear();
                search.listVTytKhhExpression.Add(o => o.ID == NEGATIVE_ID);
            }
            return search;
        }
    }
