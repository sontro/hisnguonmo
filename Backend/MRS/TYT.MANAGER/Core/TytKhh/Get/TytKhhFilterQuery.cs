...
{
    public class TytKhhFilterQuery : TytKhhFilter
    {
        public TytKhhFilterQuery()
            : base()
        {

        }

        internal List<System.Linq.Expressions.Expression<Func<TYT_KHH, bool>>> listExpression = new List<System.Linq.Expressions.Expression<Func<TYT_KHH, bool>>>();
        ...
        internal TytKhhSO Query()
        {
            ...
            try
            {
                ...
                search.listTytKhhExpression.AddRange(listExpression);
                search.OrderField = OrderProcess.GetOrderField<TYT_KHH>(ORDER_FIELD);
                search.OrderDirection = OrderProcess.GetOrderDirection(ORDER_DIRECTION);
            }
            catch (Exception ex)
            {
                LogSystem.Error(ex);
                search.listTytKhhExpression.Clear();
                search.listTytKhhExpression.Add(o => o.ID == NEGATIVE_ID);
            }
            return search;
        }
    }
