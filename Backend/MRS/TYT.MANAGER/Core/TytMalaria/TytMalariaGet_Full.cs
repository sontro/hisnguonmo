...
using TYT.MANAGER.Core.TytMalaria.Get.ListV;
using TYT.MANAGER.Core.TytMalaria.Get.V;
using Inventec.Core;
using System;
    ...
    partial class TytMalariaGet : BeanObjectBase, IDelegacyT
    {
        ...
        T IDelegacyT.Execute<T>()
        {
            ...
            try
            {
                ...
                else if (typeof(T) == typeof(List<V_TYT_MALARIA>))
                {
                    ITytMalariaGetListV behavior = TytMalariaGetListVBehaviorFactory.MakeITytMalariaGetListV(param, entity);
                    result = behavior != null ? (T)System.Convert.ChangeType(behavior.Run(), typeof(T)) : default(T);
                }
                else if (typeof(T) == typeof(V_TYT_MALARIA))
                {
                    ITytMalariaGetV behavior = TytMalariaGetVBehaviorFactory.MakeITytMalariaGetV(param, entity);
                    result = behavior != null ? (T)System.Convert.ChangeType(behavior.Run(), typeof(T)) : default(T);
                }
            }
            catch (Exception ex)
