...
{
    public class TytMalariaViewFilterQuery : TytMalariaViewFilter
    {
        public TytMalariaViewFilterQuery()
            : base()
        {

        }
        
        internal List<System.Linq.Expressions.Expression<Func<V_TYT_MALARIA, bool>>> listExpression = new List<System.Linq.Expressions.Expression<Func<V_TYT_MALARIA, bool>>>();
        ...
        internal TytMalariaSO Query()
        {
            ...
            try
            {
                ...
                search.listVTytMalariaExpression.AddRange(listExpression);
                search.OrderField = OrderProcess.GetOrderField<V_TYT_MALARIA>(ORDER_FIELD);
                search.OrderDirection = OrderProcess.GetOrderDirection(ORDER_DIRECTION);
            }
            catch (Exception ex)
            {
                LogSystem.Error(ex);
                search.listVTytMalariaExpression.Clear();
                search.listVTytMalariaExpression.Add(o => o.ID == NEGATIVE_ID);
            }
            return search;
        }
    }
