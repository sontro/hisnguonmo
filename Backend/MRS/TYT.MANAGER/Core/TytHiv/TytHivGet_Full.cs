...
using TYT.MANAGER.Core.TytHiv.Get.ListV;
using TYT.MANAGER.Core.TytHiv.Get.V;
using Inventec.Core;
using System;
    ...
    partial class TytHivGet : BeanObjectBase, IDelegacyT
    {
        ...
        T IDelegacyT.Execute<T>()
        {
            ...
            try
            {
                ...
                else if (typeof(T) == typeof(List<V_TYT_HIV>))
                {
                    ITytHivGetListV behavior = TytHivGetListVBehaviorFactory.MakeITytHivGetListV(param, entity);
                    result = behavior != null ? (T)System.Convert.ChangeType(behavior.Run(), typeof(T)) : default(T);
                }
                else if (typeof(T) == typeof(V_TYT_HIV))
                {
                    ITytHivGetV behavior = TytHivGetVBehaviorFactory.MakeITytHivGetV(param, entity);
                    result = behavior != null ? (T)System.Convert.ChangeType(behavior.Run(), typeof(T)) : default(T);
                }
            }
            catch (Exception ex)
