...
namespace TYT.MANAGER.Core.TytHiv.Get
{
    public class TytHivViewFilterQuery : TytHivViewFilter
    {
        public TytHivViewFilterQuery()
            : base()
        {

        }
        
        internal List<System.Linq.Expressions.Expression<Func<V_TYT_HIV, bool>>> listExpression = new List<System.Linq.Expressions.Expression<Func<V_TYT_HIV, bool>>>();

        internal Inventec.Backend.MANAGER.OrderProcessorBase OrderProcess = new Inventec.Backend.MANAGER.OrderProcessorBase();

        internal TytHivSO Query()
        {
            TytHivSO search = new TytHivSO();
            try
            {
                ...
                search.listVTytHivExpression.AddRange(listExpression);
                search.OrderField = OrderProcess.GetOrderField<V_TYT_HIV>(ORDER_FIELD);
                search.OrderDirection = OrderProcess.GetOrderDirection(ORDER_DIRECTION);
            }
            catch (Exception ex)
            {
                LogSystem.Error(ex);
                search.listVTytHivExpression.Clear();
                search.listVTytHivExpression.Add(o => o.ID == NEGATIVE_ID);
            }
            return search;
        }
    }
