...
{
    public class TytHivFilterQuery : TytHivFilter
    {
        public TytHivFilterQuery()
            : base()
        {

        }

        internal List<System.Linq.Expressions.Expression<Func<TYT_HIV, bool>>> listExpression = new List<System.Linq.Expressions.Expression<Func<TYT_HIV, bool>>>();
        ...
        internal TytHivSO Query()
        {
            ...
            try
            {
                ...
                search.listTytHivExpression.AddRange(listExpression);
                search.OrderField = OrderProcess.GetOrderField<TYT_HIV>(ORDER_FIELD);
                search.OrderDirection = OrderProcess.GetOrderDirection(ORDER_DIRECTION);
            }
            catch (Exception ex)
            {
                LogSystem.Error(ex);
                search.listTytHivExpression.Clear();
                search.listTytHivExpression.Add(o => o.ID == NEGATIVE_ID);
            }
            return search;
        }
    }
