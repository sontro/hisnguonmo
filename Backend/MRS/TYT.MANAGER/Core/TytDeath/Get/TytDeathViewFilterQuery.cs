...
namespace TYT.MANAGER.Core.TytDeath.Get
{
    public class TytDeathViewFilterQuery : TytDeathViewFilter
    {
        public TytDeathViewFilterQuery()
            : base()
        {

        }
        
        internal List<System.Linq.Expressions.Expression<Func<V_TYT_DEATH, bool>>> listExpression = new List<System.Linq.Expressions.Expression<Func<V_TYT_DEATH, bool>>>();

        internal Inventec.Backend.MANAGER.OrderProcessorBase OrderProcess = new Inventec.Backend.MANAGER.OrderProcessorBase();

        internal TytDeathSO Query()
        {
            TytDeathSO search = new TytDeathSO();
            try
            {
                ...
                search.listVTytDeathExpression.AddRange(listExpression);
                search.OrderField = OrderProcess.GetOrderField<V_TYT_DEATH>(ORDER_FIELD);
                search.OrderDirection = OrderProcess.GetOrderDirection(ORDER_DIRECTION);
            }
            catch (Exception ex)
            {
                LogSystem.Error(ex);
                search.listVTytDeathExpression.Clear();
                search.listVTytDeathExpression.Add(o => o.ID == NEGATIVE_ID);
            }
            return search;
        }
    }
