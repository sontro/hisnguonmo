...
{
    public class TytDeathFilterQuery : TytDeathFilter
    {
        public TytDeathFilterQuery()
            : base()
        {

        }

        internal List<System.Linq.Expressions.Expression<Func<TYT_DEATH, bool>>> listExpression = new List<System.Linq.Expressions.Expression<Func<TYT_DEATH, bool>>>();
        ...
        internal TytDeathSO Query()
        {
            ...
            try
            {
                ...
                search.listTytDeathExpression.AddRange(listExpression);
                search.OrderField = OrderProcess.GetOrderField<TYT_DEATH>(ORDER_FIELD);
                search.OrderDirection = OrderProcess.GetOrderDirection(ORDER_DIRECTION);
            }
            catch (Exception ex)
            {
                LogSystem.Error(ex);
                search.listTytDeathExpression.Clear();
                search.listTytDeathExpression.Add(o => o.ID == NEGATIVE_ID);
            }
            return search;
        }
    }
