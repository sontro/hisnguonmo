...
using TYT.MANAGER.Core.TytUninfectIcdGroup.Get.ListV;
using TYT.MANAGER.Core.TytUninfectIcdGroup.Get.V;
using Inventec.Core;
using System;
    ...
    partial class TytUninfectIcdGroupGet : BeanObjectBase, IDelegacyT
    {
        ...
        T IDelegacyT.Execute<T>()
        {
            ...
            try
            {
                ...
                else if (typeof(T) == typeof(List<V_TYT_UNINFECT_ICD_GROUP>))
                {
                    ITytUninfectIcdGroupGetListV behavior = TytUninfectIcdGroupGetListVBehaviorFactory.MakeITytUninfectIcdGroupGetListV(param, entity);
                    result = behavior != null ? (T)System.Convert.ChangeType(behavior.Run(), typeof(T)) : default(T);
                }
                else if (typeof(T) == typeof(V_TYT_UNINFECT_ICD_GROUP))
                {
                    ITytUninfectIcdGroupGetV behavior = TytUninfectIcdGroupGetVBehaviorFactory.MakeITytUninfectIcdGroupGetV(param, entity);
                    result = behavior != null ? (T)System.Convert.ChangeType(behavior.Run(), typeof(T)) : default(T);
                }
            }
            catch (Exception ex)
