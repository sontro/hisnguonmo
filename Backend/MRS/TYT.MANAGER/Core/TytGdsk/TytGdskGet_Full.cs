...
using TYT.MANAGER.Core.TytGdsk.Get.ListV;
using TYT.MANAGER.Core.TytGdsk.Get.V;
using Inventec.Core;
using System;
    ...
    partial class TytGdskGet : BeanObjectBase, IDelegacyT
    {
        ...
        T IDelegacyT.Execute<T>()
        {
            ...
            try
            {
                ...
                else if (typeof(T) == typeof(List<V_TYT_GDSK>))
                {
                    ITytGdskGetListV behavior = TytGdskGetListVBehaviorFactory.MakeITytGdskGetListV(param, entity);
                    result = behavior != null ? (T)System.Convert.ChangeType(behavior.Run(), typeof(T)) : default(T);
                }
                else if (typeof(T) == typeof(V_TYT_GDSK))
                {
                    ITytGdskGetV behavior = TytGdskGetVBehaviorFactory.MakeITytGdskGetV(param, entity);
                    result = behavior != null ? (T)System.Convert.ChangeType(behavior.Run(), typeof(T)) : default(T);
                }
            }
            catch (Exception ex)
