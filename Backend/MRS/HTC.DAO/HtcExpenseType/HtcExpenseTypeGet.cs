...
{
    partial class HtcExpenseTypeGet : EntityBase
    {
        public List<HTC_EXPENSE_TYPE> Get(HtcExpenseTypeSO search, CommonParam param)
        {
            List<HTC_EXPENSE_TYPE> list = new List<HTC_EXPENSE_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HTC_EXPENSE_TYPE.AsQueryable();
                        if (search.listHtcExpenseTypeExpression != null && search.listHtcExpenseTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHtcExpenseTypeExpression)
                            {
        ...
        public HTC_EXPENSE_TYPE GetById(long id, HtcExpenseTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HTC_EXPENSE_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHtcExpenseTypeExpression != null && search.listHtcExpenseTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHtcExpenseTypeExpression)
                            {
