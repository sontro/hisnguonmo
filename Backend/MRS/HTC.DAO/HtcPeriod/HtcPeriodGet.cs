...
{
    partial class HtcPeriodGet : EntityBase
    {
        public List<HTC_PERIOD> Get(HtcPeriodSO search, CommonParam param)
        {
            List<HTC_PERIOD> list = new List<HTC_PERIOD>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HTC_PERIOD.AsQueryable();
                        if (search.listHtcPeriodExpression != null && search.listHtcPeriodExpression.Count > 0)
                        {
                            foreach (var item in search.listHtcPeriodExpression)
                            {
        ...
        public HTC_PERIOD GetById(long id, HtcPeriodSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HTC_PERIOD.AsQueryable().Where(p => p.ID == id);
                        if (search.listHtcPeriodExpression != null && search.listHtcPeriodExpression.Count > 0)
                        {
                            foreach (var item in search.listHtcPeriodExpression)
                            {
