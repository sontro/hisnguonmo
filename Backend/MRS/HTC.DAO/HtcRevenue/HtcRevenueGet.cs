...
{
    partial class HtcRevenueGet : EntityBase
    {
        public List<HTC_REVENUE> Get(HtcRevenueSO search, CommonParam param)
        {
            List<HTC_REVENUE> list = new List<HTC_REVENUE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HTC_REVENUE.AsQueryable();
                        if (search.listHtcRevenueExpression != null && search.listHtcRevenueExpression.Count > 0)
                        {
                            foreach (var item in search.listHtcRevenueExpression)
                            {
        ...
        public HTC_REVENUE GetById(long id, HtcRevenueSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HTC_REVENUE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHtcRevenueExpression != null && search.listHtcRevenueExpression.Count > 0)
                        {
                            foreach (var item in search.listHtcRevenueExpression)
                            {
