...
namespace HTC.DAO.HtcPeriodDepartment
{
    partial class HtcPeriodDepartmentGet : EntityBase
    {
        ...
        {
            List<HTC_PERIOD_DEPARTMENT> list = new List<HTC_PERIOD_DEPARTMENT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HTC_PERIOD_DEPARTMENT.AsQueryable();
                        if (search.listHtcPeriodDepartmentExpression != null && search.listHtcPeriodDepartmentExpression.Count > 0)
                        {
                            foreach (var item in search.listHtcPeriodDepartmentExpression)
                            {
        ...
        public HTC_PERIOD_DEPARTMENT GetById(long id, HtcPeriodDepartmentSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.HTC_PERIOD_DEPARTMENT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHtcPeriodDepartmentExpression != null && search.listHtcPeriodDepartmentExpression.Count > 0)
                        {
                            foreach (var item in search.listHtcPeriodDepartmentExpression)
                            {
