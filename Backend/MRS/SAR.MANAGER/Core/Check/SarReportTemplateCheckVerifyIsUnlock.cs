...
{
    class SarReportTemplateCheckVerifyIsUnlock
    {
        ...
        internal static bool Verify(CommonParam param, SAR_REPORT_TEMPLATE data)
        {
            bool result = true;
            try
            {
        ...
        internal static bool Verify(CommonParam param, List<long> ids)
        {
            ...
            try
            {
                if (ids != null && ids.Count > 0)
                {
                    ...
                    filter.IDs = ids;
                    List<SAR_REPORT_TEMPLATE> listData = new SarReportTemplateBO().Get<List<SAR_REPORT_TEMPLATE>>(filter);
        ...
        internal static bool Verify(CommonParam param, List<SAR_REPORT_TEMPLATE> datas)
        {
            bool result = true;
            try
            {
        ...
        private static bool Check(CommonParam param, SAR_REPORT_TEMPLATE raw)
        {
            bool result = true;
            if (raw == null)
            {
