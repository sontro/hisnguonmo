...
using SAR.MANAGER.Core.SarReportCalendar.Get.ListV;
using SAR.MANAGER.Core.SarReportCalendar.Get.V;
using Inventec.Core;
using System;
    ...
    partial class SarReportCalendarGet : BeanObjectBase, IDelegacyT
    {
        ...
        T IDelegacyT.Execute<T>()
        {
            ...
            try
            {
                ...
                else if (typeof(T) == typeof(List<V_SAR_REPORT_CALENDAR>))
                {
                    ISarReportCalendarGetListV behavior = SarReportCalendarGetListVBehaviorFactory.MakeISarReportCalendarGetListV(param, entity);
                    result = behavior != null ? (T)System.Convert.ChangeType(behavior.Run(), typeof(T)) : default(T);
                }
                else if (typeof(T) == typeof(V_SAR_REPORT_CALENDAR))
                {
                    ISarReportCalendarGetV behavior = SarReportCalendarGetVBehaviorFactory.MakeISarReportCalendarGetV(param, entity);
                    result = behavior != null ? (T)System.Convert.ChangeType(behavior.Run(), typeof(T)) : default(T);
                }
            }
            catch (Exception ex)
