...
{
    public class SarReportTemplateFilterQuery : SarReportTemplateFilter
    {
        public SarReportTemplateFilterQuery()
            : base()
        {

        }

        internal List<System.Linq.Expressions.Expression<Func<SAR_REPORT_TEMPLATE, bool>>> listExpression = new List<System.Linq.Expressions.Expression<Func<SAR_REPORT_TEMPLATE, bool>>>();
        ...
        internal SarReportTemplateSO Query()
        {
            ...
            try
            {
                ...
                search.listSarReportTemplateExpression.AddRange(listExpression);
                search.OrderField = OrderProcess.GetOrderField<SAR_REPORT_TEMPLATE>(ORDER_FIELD);
                search.OrderDirection = OrderProcess.GetOrderDirection(ORDER_DIRECTION);
            }
            catch (Exception ex)
            {
                LogSystem.Error(ex);
                search.listSarReportTemplateExpression.Clear();
                search.listSarReportTemplateExpression.Add(o => o.ID == NEGATIVE_ID);
            }
            return search;
        }
    }
