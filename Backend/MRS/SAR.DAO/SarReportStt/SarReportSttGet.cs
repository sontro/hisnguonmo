...
namespace SAR.DAO.SarReportStt
{
    partial class SarReportSttGet : EntityBase
    {
        ...
        {
            List<SAR_REPORT_STT> list = new List<SAR_REPORT_STT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.SAR_REPORT_STT.AsQueryable();
                        if (search.listSarReportSttExpression != null && search.listSarReportSttExpression.Count > 0)
                        {
                            foreach (var item in search.listSarReportSttExpression)
                            {
        ...
        public SAR_REPORT_STT GetById(long id, SarReportSttSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.SAR_REPORT_STT.AsQueryable().Where(p => p.ID == id);
                        if (search.listSarReportSttExpression != null && search.listSarReportSttExpression.Count > 0)
                        {
                            foreach (var item in search.listSarReportSttExpression)
                            {
