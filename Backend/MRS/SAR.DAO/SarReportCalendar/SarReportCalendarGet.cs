...
namespace SAR.DAO.SarReportCalendar
{
    partial class SarReportCalendarGet : EntityBase
    {
        ...
        {
            List<SAR_REPORT_CALENDAR> list = new List<SAR_REPORT_CALENDAR>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.SAR_REPORT_CALENDAR.AsQueryable();
        ...
        public SAR_REPORT_CALENDAR GetById(long id, SarReportCalendarSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.SAR_REPORT_CALENDAR.AsQueryable().Where(p => p.ID == id);
                        if (search.listSarReportCalendarExpression != null && search.listSarReportCalendarExpression.Count > 0)
                        {
                            foreach (var item in search.listSarReportCalendarExpression)
                            {
