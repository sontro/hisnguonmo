...
{
    public class SdaLanguageFilterQuery : SdaLanguageFilter
    {
        public SdaLanguageFilterQuery()
            : base()
        {

        }

        internal List<System.Linq.Expressions.Expression<Func<SDA_LANGUAGE, bool>>> listExpression = new List<System.Linq.Expressions.Expression<Func<SDA_LANGUAGE, bool>>>();
        ...
        internal SdaLanguageSO Query()
        {
            ...
            try
            {
                ...
                search.listSdaLanguageExpression.AddRange(listExpression);
                search.OrderField = OrderProcess.GetOrderField<SDA_LANGUAGE>(ORDER_FIELD);
                search.OrderDirection = OrderProcess.GetOrderDirection(ORDER_DIRECTION);
            }
            catch (Exception ex)
            {
                LogSystem.Error(ex);
                search.listSdaLanguageExpression.Clear();
                search.listSdaLanguageExpression.Add(o => o.ID == NEGATIVE_ID);
            }
            return search;
        }
    }
