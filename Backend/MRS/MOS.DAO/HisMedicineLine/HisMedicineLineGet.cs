...
namespace MOS.DAO.HisMedicineLine
{
    partial class HisMedicineLineGet : EntityBase
    {
        ...
        {
            List<HIS_MEDICINE_LINE> list = new List<HIS_MEDICINE_LINE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MEDICINE_LINE.AsNoTracking().AsQueryable();
                        if (search.listHisMedicineLineExpression != null && search.listHisMedicineLineExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMedicineLineExpression)
                            {
        ...
        public HIS_MEDICINE_LINE GetById(long id, HisMedicineLineSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_MEDICINE_LINE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMedicineLineExpression != null && search.listHisMedicineLineExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMedicineLineExpression)
                            {
