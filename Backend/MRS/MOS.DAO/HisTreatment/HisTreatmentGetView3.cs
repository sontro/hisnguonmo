...
namespace MOS.DAO.HisTreatment
{
    ...
    {
        public List<V_HIS_TREATMENT_3> GetView3(HisTreatmentSO search, CommonParam param)
        {
            List<V_HIS_TREATMENT_3> list = new List<V_HIS_TREATMENT_3>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_TREATMENT_3.AsNoTracking().AsQueryable();
                        if (search.listVHisTreatment3Expression != null && search.listVHisTreatment3Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisTreatment3Expression)
                            {
