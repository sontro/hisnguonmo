...
namespace MOS.DAO.HisTreatment
{
    ...
    {
        public List<V_HIS_TREATMENT_FEE_1> GetFeeView1(HisTreatmentSO search, CommonParam param)
        {
            List<V_HIS_TREATMENT_FEE_1> list = new List<V_HIS_TREATMENT_FEE_1>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_TREATMENT_FEE_1.AsNoTracking().AsQueryable();
                        if (search.listVHisTreatmentFee1Expression != null && search.listVHisTreatmentFee1Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisTreatmentFee1Expression)
                            {
