...
namespace MOS.DAO.HisTreatment
{
    ...
    {
        public List<V_HIS_TREATMENT_7> GetView7(HisTreatmentSO search, CommonParam param)
        {
            List<V_HIS_TREATMENT_7> list = new List<V_HIS_TREATMENT_7>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_TREATMENT_7.AsNoTracking().AsQueryable();
                        if (search.listVHisTreatment7Expression != null && search.listVHisTreatment7Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisTreatment7Expression)
                            {
