...
namespace MOS.DAO.HisTreatment
{
    ...
    {
        public List<V_HIS_TREATMENT_1> GetView1(HisTreatmentSO search, CommonParam param)
        {
            List<V_HIS_TREATMENT_1> list = new List<V_HIS_TREATMENT_1>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_TREATMENT_1.AsNoTracking().AsQueryable();
                        if (search.listVHisTreatment1Expression != null && search.listVHisTreatment1Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisTreatment1Expression)
                            {
