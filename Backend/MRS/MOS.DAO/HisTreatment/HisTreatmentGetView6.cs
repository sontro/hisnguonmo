...
namespace MOS.DAO.HisTreatment
{
    ...
    {
        public List<V_HIS_TREATMENT_6> GetView6(HisTreatmentSO search, CommonParam param)
        {
            List<V_HIS_TREATMENT_6> list = new List<V_HIS_TREATMENT_6>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_TREATMENT_6.AsNoTracking().AsQueryable();
