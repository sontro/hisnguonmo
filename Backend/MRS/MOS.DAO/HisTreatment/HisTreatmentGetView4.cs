...
namespace MOS.DAO.HisTreatment
{
    ...
    {
        public List<V_HIS_TREATMENT_4> GetView4(HisTreatmentSO search, CommonParam param)
        {
            List<V_HIS_TREATMENT_4> list = new List<V_HIS_TREATMENT_4>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_TREATMENT_4.AsNoTracking().AsQueryable();
