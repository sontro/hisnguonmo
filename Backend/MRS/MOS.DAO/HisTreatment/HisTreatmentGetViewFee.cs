...
namespace MOS.DAO.HisTreatment
{
    ...
    {
        public List<V_HIS_TREATMENT_FEE> GetFeeView(HisTreatmentSO search, CommonParam param)
        {
            List<V_HIS_TREATMENT_FEE> list = new List<V_HIS_TREATMENT_FEE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_TREATMENT_FEE.AsNoTracking().AsQueryable();
                        if (search.listVHisTreatmentFeeExpression != null && search.listVHisTreatmentFeeExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisTreatmentFeeExpression)
                            {
