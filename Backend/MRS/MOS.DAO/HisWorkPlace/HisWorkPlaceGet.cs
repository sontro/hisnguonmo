...
namespace MOS.DAO.HisWorkPlace
{
    partial class HisWorkPlaceGet : EntityBase
    {
        ...
        {
            List<HIS_WORK_PLACE> list = new List<HIS_WORK_PLACE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_WORK_PLACE.AsNoTracking().AsQueryable();
                        if (search.listHisWorkPlaceExpression != null && search.listHisWorkPlaceExpression.Count > 0)
                        {
                            foreach (var item in search.listHisWorkPlaceExpression)
                            {
        ...
        public HIS_WORK_PLACE GetById(long id, HisWorkPlaceSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_WORK_PLACE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisWorkPlaceExpression != null && search.listHisWorkPlaceExpression.Count > 0)
                        {
                            foreach (var item in search.listHisWorkPlaceExpression)
                            {
