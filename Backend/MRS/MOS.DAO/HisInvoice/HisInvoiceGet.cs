...
namespace MOS.DAO.HisInvoice
{
    partial class HisInvoiceGet : EntityBase
    {
        ...
        {
            List<HIS_INVOICE> list = new List<HIS_INVOICE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_INVOICE.AsNoTracking().AsQueryable();
                        if (search.listHisInvoiceExpression != null && search.listHisInvoiceExpression.Count > 0)
                        {
                            foreach (var item in search.listHisInvoiceExpression)
                            {
        ...
        public HIS_INVOICE GetById(long id, HisInvoiceSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_INVOICE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisInvoiceExpression != null && search.listHisInvoiceExpression.Count > 0)
                        {
                            foreach (var item in search.listHisInvoiceExpression)
                            {
