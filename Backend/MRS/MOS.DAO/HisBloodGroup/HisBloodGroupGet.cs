...
namespace MOS.DAO.HisBloodGroup
{
    partial class HisBloodGroupGet : EntityBase
    {
        ...
        {
            List<HIS_BLOOD_GROUP> list = new List<HIS_BLOOD_GROUP>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_BLOOD_GROUP.AsNoTracking().AsQueryable();
                        if (search.listHisBloodGroupExpression != null && search.listHisBloodGroupExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBloodGroupExpression)
                            {
        ...
        public HIS_BLOOD_GROUP GetById(long id, HisBloodGroupSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_BLOOD_GROUP.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisBloodGroupExpression != null && search.listHisBloodGroupExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBloodGroupExpression)
                            {
