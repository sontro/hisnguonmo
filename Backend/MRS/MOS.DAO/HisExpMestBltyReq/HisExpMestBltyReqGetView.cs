...
namespace MOS.DAO.HisExpMestBltyReq
{
    partial class HisExpMestBltyReqGet : EntityBase
    {
        ...
        {
            List<V_HIS_EXP_MEST_BLTY_REQ> list = new List<V_HIS_EXP_MEST_BLTY_REQ>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_EXP_MEST_BLTY_REQ.AsNoTracking().AsQueryable();
                        if (search.listVHisExpMestBltyReqExpression != null && search.listVHisExpMestBltyReqExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisExpMestBltyReqExpression)
                            {
