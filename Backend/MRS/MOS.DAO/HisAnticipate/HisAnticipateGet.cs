...
namespace MOS.DAO.HisAnticipate
{
    partial class HisAnticipateGet : EntityBase
    {
        ...
        {
            List<HIS_ANTICIPATE> list = new List<HIS_ANTICIPATE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_ANTICIPATE.AsNoTracking().AsQueryable();
                        if (search.listHisAnticipateExpression != null && search.listHisAnticipateExpression.Count > 0)
                        {
                            foreach (var item in search.listHisAnticipateExpression)
                            {
        ...
        public HIS_ANTICIPATE GetById(long id, HisAnticipateSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_ANTICIPATE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisAnticipateExpression != null && search.listHisAnticipateExpression.Count > 0)
                        {
                            foreach (var item in search.listHisAnticipateExpression)
                            {
