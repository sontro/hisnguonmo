        ...
        public List<HIS_TRACKING> Get(HisTrackingSO search, CommonParam param)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_TRACKING.AsNoTracking().AsQueryable();
                        if (search.listHisTrackingExpression != null && search.listHisTrackingExpression.Count > 0)
                        {
                            foreach (var item in search.listHisTrackingExpression)
                            {
        ...
        public HIS_TRACKING GetById(long id, HisTrackingSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_TRACKING.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisTrackingExpression != null && search.listHisTrackingExpression.Count > 0)
                        {
                            foreach (var item in search.listHisTrackingExpression)
                            {
