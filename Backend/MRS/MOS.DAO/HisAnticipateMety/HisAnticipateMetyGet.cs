...
namespace MOS.DAO.HisAnticipateMety
{
    partial class HisAnticipateMetyGet : EntityBase
    {
        ...
        {
            List<HIS_ANTICIPATE_METY> list = new List<HIS_ANTICIPATE_METY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_ANTICIPATE_METY.AsNoTracking().AsQueryable();
                        if (search.listHisAnticipateMetyExpression != null && search.listHisAnticipateMetyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisAnticipateMetyExpression)
                            {
        ...
        public HIS_ANTICIPATE_METY GetById(long id, HisAnticipateMetySO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_ANTICIPATE_METY.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisAnticipateMetyExpression != null && search.listHisAnticipateMetyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisAnticipateMetyExpression)
                            {
