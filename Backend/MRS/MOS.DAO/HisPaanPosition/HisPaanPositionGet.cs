...
namespace MOS.DAO.HisPaanPosition
{
    partial class HisPaanPositionGet : EntityBase
    {
        ...
        {
            List<HIS_PAAN_POSITION> list = new List<HIS_PAAN_POSITION>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_PAAN_POSITION.AsNoTracking().AsQueryable();
        ...
        public HIS_PAAN_POSITION GetById(long id, HisPaanPositionSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_PAAN_POSITION.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisPaanPositionExpression != null && search.listHisPaanPositionExpression.Count > 0)
                        {
                            foreach (var item in search.listHisPaanPositionExpression)
                            {
