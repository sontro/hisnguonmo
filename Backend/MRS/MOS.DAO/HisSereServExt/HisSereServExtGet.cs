...
{
    partial class HisSereServExtGet : EntityBase
    {
        public List<HIS_SERE_SERV_EXT> Get(HisSereServExtSO search, CommonParam param)
        {
            List<HIS_SERE_SERV_EXT> list = new List<HIS_SERE_SERV_EXT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SERE_SERV_EXT.AsNoTracking().AsQueryable();
                        if (search.listHisSereServExtExpression != null && search.listHisSereServExtExpression.Count > 0)
                        {
                            foreach (var item in search.listHisSereServExtExpression)
                            {
        ...
        public HIS_SERE_SERV_EXT GetById(long id, HisSereServExtSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_SERE_SERV_EXT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisSereServExtExpression != null && search.listHisSereServExtExpression.Count > 0)
                        {
                            foreach (var item in search.listHisSereServExtExpression)
                            {
