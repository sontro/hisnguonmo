...
namespace MOS.DAO.HisMedicinePaty
{
    partial class HisMedicinePatyGet : EntityBase
    {
        ...
        {
            List<HIS_MEDICINE_PATY> list = new List<HIS_MEDICINE_PATY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MEDICINE_PATY.AsNoTracking().AsQueryable();
        ...
        public HIS_MEDICINE_PATY GetById(long id, HisMedicinePatySO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_MEDICINE_PATY.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMedicinePatyExpression != null && search.listHisMedicinePatyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMedicinePatyExpression)
                            {
