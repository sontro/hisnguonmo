...
namespace MOS.DAO.HisServiceFollow
{
    partial class HisServiceFollowGet : EntityBase
    {
        ...
        {
            List<HIS_SERVICE_FOLLOW> list = new List<HIS_SERVICE_FOLLOW>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SERVICE_FOLLOW.AsNoTracking().AsQueryable();
                        if (search.listHisServiceFollowExpression != null && search.listHisServiceFollowExpression.Count > 0)
                        {
                            foreach (var item in search.listHisServiceFollowExpression)
                            {
        ...
        public HIS_SERVICE_FOLLOW GetById(long id, HisServiceFollowSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_SERVICE_FOLLOW.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisServiceFollowExpression != null && search.listHisServiceFollowExpression.Count > 0)
                        {
                            foreach (var item in search.listHisServiceFollowExpression)
                            {
