...
namespace MOS.DAO.HisSampleRoom
{
    partial class HisSampleRoomGet : EntityBase
    {
        ...
        {
            List<HIS_SAMPLE_ROOM> list = new List<HIS_SAMPLE_ROOM>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SAMPLE_ROOM.AsNoTracking().AsQueryable();
                        if (search.listHisSampleRoomExpression != null && search.listHisSampleRoomExpression.Count > 0)
                        {
                            foreach (var item in search.listHisSampleRoomExpression)
                            {
        ...
        public HIS_SAMPLE_ROOM GetById(long id, HisSampleRoomSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_SAMPLE_ROOM.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisSampleRoomExpression != null && search.listHisSampleRoomExpression.Count > 0)
                        {
                            foreach (var item in search.listHisSampleRoomExpression)
                            {
