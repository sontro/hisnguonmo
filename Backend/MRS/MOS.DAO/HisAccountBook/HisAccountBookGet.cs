...
namespace MOS.DAO.HisAccountBook
{
    partial class HisAccountBookGet : EntityBase
    {
        ...
        {
            List<HIS_ACCOUNT_BOOK> list = new List<HIS_ACCOUNT_BOOK>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_ACCOUNT_BOOK.AsNoTracking().AsQueryable();
        ...
        public HIS_ACCOUNT_BOOK GetById(long id, HisAccountBookSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_ACCOUNT_BOOK.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisAccountBookExpression != null && search.listHisAccountBookExpression.Count > 0)
                        {
                            foreach (var item in search.listHisAccountBookExpression)
                            {
