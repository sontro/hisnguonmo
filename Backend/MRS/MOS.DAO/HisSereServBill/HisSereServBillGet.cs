...
namespace MOS.DAO.HisSereServBill
{
    partial class HisSereServBillGet : EntityBase
    {
        ...
        {
            List<HIS_SERE_SERV_BILL> list = new List<HIS_SERE_SERV_BILL>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SERE_SERV_BILL.AsNoTracking().AsQueryable();
        ...
        public HIS_SERE_SERV_BILL GetById(long id, HisSereServBillSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_SERE_SERV_BILL.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisSereServBillExpression != null && search.listHisSereServBillExpression.Count > 0)
                        {
                            foreach (var item in search.listHisSereServBillExpression)
                            {
