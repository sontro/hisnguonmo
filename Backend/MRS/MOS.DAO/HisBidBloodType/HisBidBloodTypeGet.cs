...
namespace MOS.DAO.HisBidBloodType
{
    partial class HisBidBloodTypeGet : EntityBase
    {
        ...
        {
            List<HIS_BID_BLOOD_TYPE> list = new List<HIS_BID_BLOOD_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_BID_BLOOD_TYPE.AsNoTracking().AsQueryable();
        ...
        public HIS_BID_BLOOD_TYPE GetById(long id, HisBidBloodTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_BID_BLOOD_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisBidBloodTypeExpression != null && search.listHisBidBloodTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBidBloodTypeExpression)
                            {
