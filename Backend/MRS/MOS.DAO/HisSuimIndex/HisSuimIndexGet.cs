...
namespace MOS.DAO.HisSuimIndex
{
    partial class HisSuimIndexGet : EntityBase
    {
        ...
        {
            List<HIS_SUIM_INDEX> list = new List<HIS_SUIM_INDEX>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SUIM_INDEX.AsNoTracking().AsQueryable();
        ...
        public HIS_SUIM_INDEX GetById(long id, HisSuimIndexSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_SUIM_INDEX.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisSuimIndexExpression != null && search.listHisSuimIndexExpression.Count > 0)
                        {
                            foreach (var item in search.listHisSuimIndexExpression)
                            {
