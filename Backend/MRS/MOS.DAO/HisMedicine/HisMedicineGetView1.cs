...
{
    partial class HisMedicineGet : EntityBase
    {
        public List<V_HIS_MEDICINE_1> GetView1(HisMedicineSO search, CommonParam param)
        {
            List<V_HIS_MEDICINE_1> list = new List<V_HIS_MEDICINE_1>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_MEDICINE_1.AsNoTracking().AsQueryable();
                        if (search.listVHisMedicine1Expression != null && search.listVHisMedicine1Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisMedicine1Expression)
                            {
