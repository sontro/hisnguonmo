...
namespace MOS.DAO.HisMaterialBean
{
    partial class HisMaterialBeanGet : EntityBase
    {
        ...
        {
            List<V_HIS_MATERIAL_BEAN_1> list = new List<V_HIS_MATERIAL_BEAN_1>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_MATERIAL_BEAN_1.AsNoTracking().AsQueryable();
                        if (search.listVHisMaterialBean1Expression != null && search.listVHisMaterialBean1Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisMaterialBean1Expression)
                            {
