...
namespace MOS.DAO.HisFilmSize
{
    partial class HisFilmSizeGet : EntityBase
    {
        ...
        {
            List<HIS_FILM_SIZE> list = new List<HIS_FILM_SIZE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_FILM_SIZE.AsNoTracking().AsQueryable();
        ...
        public HIS_FILM_SIZE GetById(long id, HisFilmSizeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_FILM_SIZE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisFilmSizeExpression != null && search.listHisFilmSizeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisFilmSizeExpression)
                            {
