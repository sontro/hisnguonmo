...
namespace MOS.DAO.HisSeseDepoRepay
{
    partial class HisSeseDepoRepayGet : EntityBase
    {
        ...
        {
            List<HIS_SESE_DEPO_REPAY> list = new List<HIS_SESE_DEPO_REPAY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SESE_DEPO_REPAY.AsNoTracking().AsQueryable();
        ...
        public HIS_SESE_DEPO_REPAY GetById(long id, HisSeseDepoRepaySO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_SESE_DEPO_REPAY.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisSeseDepoRepayExpression != null && search.listHisSeseDepoRepayExpression.Count > 0)
                        {
                            foreach (var item in search.listHisSeseDepoRepayExpression)
                            {
