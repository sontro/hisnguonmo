...
namespace MOS.DAO.HisPaanLiquid
{
    partial class HisPaanLiquidGet : EntityBase
    {
        ...
        {
            List<HIS_PAAN_LIQUID> list = new List<HIS_PAAN_LIQUID>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_PAAN_LIQUID.AsNoTracking().AsQueryable();
        ...
        public HIS_PAAN_LIQUID GetById(long id, HisPaanLiquidSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_PAAN_LIQUID.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisPaanLiquidExpression != null && search.listHisPaanLiquidExpression.Count > 0)
                        {
                            foreach (var item in search.listHisPaanLiquidExpression)
                            {
