...
namespace MOS.DAO.HisExpMestMedicine
{
    partial class HisExpMestMedicineGet : EntityBase
    {
        ...
        {
            List<V_HIS_EXP_MEST_MEDICINE_3> list = new List<V_HIS_EXP_MEST_MEDICINE_3>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_EXP_MEST_MEDICINE_3.AsNoTracking().AsQueryable();
                        if (search.listVHisExpMestMedicine3Expression != null && search.listVHisExpMestMedicine3Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisExpMestMedicine3Expression)
                            {
