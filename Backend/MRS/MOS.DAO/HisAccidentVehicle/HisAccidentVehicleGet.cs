...
namespace MOS.DAO.HisAccidentVehicle
{
    partial class HisAccidentVehicleGet : EntityBase
    {
        ...
        {
            List<HIS_ACCIDENT_VEHICLE> list = new List<HIS_ACCIDENT_VEHICLE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_ACCIDENT_VEHICLE.AsNoTracking().AsQueryable();
                        if (search.listHisAccidentVehicleExpression != null && search.listHisAccidentVehicleExpression.Count > 0)
                        {
                            foreach (var item in search.listHisAccidentVehicleExpression)
                            {
        ...
        public HIS_ACCIDENT_VEHICLE GetById(long id, HisAccidentVehicleSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_ACCIDENT_VEHICLE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisAccidentVehicleExpression != null && search.listHisAccidentVehicleExpression.Count > 0)
                        {
                            foreach (var item in search.listHisAccidentVehicleExpression)
                            {
