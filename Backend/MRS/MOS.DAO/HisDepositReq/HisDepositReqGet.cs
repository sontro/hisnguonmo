...
namespace MOS.DAO.HisDepositReq
{
    partial class HisDepositReqGet : EntityBase
    {
        ...
        {
            List<HIS_DEPOSIT_REQ> list = new List<HIS_DEPOSIT_REQ>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_DEPOSIT_REQ.AsNoTracking().AsQueryable();
        ...
        public HIS_DEPOSIT_REQ GetById(long id, HisDepositReqSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_DEPOSIT_REQ.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisDepositReqExpression != null && search.listHisDepositReqExpression.Count > 0)
                        {
                            foreach (var item in search.listHisDepositReqExpression)
                            {
