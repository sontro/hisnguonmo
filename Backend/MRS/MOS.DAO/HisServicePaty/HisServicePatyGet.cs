...
{
    partial class HisServicePatyGet : EntityBase
    {
        public List<HIS_SERVICE_PATY> Get(HisServicePatySO search, CommonParam param)
        {
            List<HIS_SERVICE_PATY> list = new List<HIS_SERVICE_PATY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SERVICE_PATY.AsNoTracking().AsQueryable();
                        if (search.listHisServicePatyExpression != null && search.listHisServicePatyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisServicePatyExpression)
                            {
        ...
        public HIS_SERVICE_PATY GetById(long id, HisServicePatySO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_SERVICE_PATY.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisServicePatyExpression != null && search.listHisServicePatyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisServicePatyExpression)
                            {
