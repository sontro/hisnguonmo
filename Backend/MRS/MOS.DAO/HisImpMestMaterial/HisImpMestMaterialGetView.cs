...
namespace MOS.DAO.HisImpMestMaterial
{
    ...
    {
        public List<V_HIS_IMP_MEST_MATERIAL> GetView(HisImpMestMaterialSO search, CommonParam param)
        {
            List<V_HIS_IMP_MEST_MATERIAL> list = new List<V_HIS_IMP_MEST_MATERIAL>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_IMP_MEST_MATERIAL.AsNoTracking().AsQueryable();
