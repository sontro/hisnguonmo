...
{
    partial class HisServiceTypeGet : EntityBase
    {
        public List<HIS_SERVICE_TYPE> Get(HisServiceTypeSO search, CommonParam param)
        {
            List<HIS_SERVICE_TYPE> list = new List<HIS_SERVICE_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SERVICE_TYPE.AsNoTracking().AsQueryable();
                        if (search.listHisServiceTypeExpression != null && search.listHisServiceTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisServiceTypeExpression)
                            {
        ...
        public HIS_SERVICE_TYPE GetById(long id, HisServiceTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_SERVICE_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisServiceTypeExpression != null && search.listHisServiceTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisServiceTypeExpression)
                            {
