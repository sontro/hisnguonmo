...
namespace MOS.DAO.HisTranPatiForm
{
    partial class HisTranPatiFormGet : EntityBase
    {
        ...
        {
            List<HIS_TRAN_PATI_FORM> list = new List<HIS_TRAN_PATI_FORM>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_TRAN_PATI_FORM.AsNoTracking().AsQueryable();
        ...
        public HIS_TRAN_PATI_FORM GetById(long id, HisTranPatiFormSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_TRAN_PATI_FORM.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisTranPatiFormExpression != null && search.listHisTranPatiFormExpression.Count > 0)
                        {
                            foreach (var item in search.listHisTranPatiFormExpression)
                            {
