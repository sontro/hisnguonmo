...
namespace MOS.DAO.HisEmergencyWtime
{
    partial class HisEmergencyWtimeGet : EntityBase
    {
        ...
        {
            List<HIS_EMERGENCY_WTIME> list = new List<HIS_EMERGENCY_WTIME>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_EMERGENCY_WTIME.AsNoTracking().AsQueryable();
        ...
        public HIS_EMERGENCY_WTIME GetById(long id, HisEmergencyWtimeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_EMERGENCY_WTIME.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisEmergencyWtimeExpression != null && search.listHisEmergencyWtimeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisEmergencyWtimeExpression)
                            {
