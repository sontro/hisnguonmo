...
namespace MOS.DAO.HisServiceReqMaty
{
    partial class HisServiceReqMatyGet : EntityBase
    {
        ...
        {
            List<HIS_SERVICE_REQ_MATY> list = new List<HIS_SERVICE_REQ_MATY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SERVICE_REQ_MATY.AsNoTracking().AsQueryable();
        ...
        public HIS_SERVICE_REQ_MATY GetById(long id, HisServiceReqMatySO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_SERVICE_REQ_MATY.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisServiceReqMatyExpression != null && search.listHisServiceReqMatyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisServiceReqMatyExpression)
                            {
