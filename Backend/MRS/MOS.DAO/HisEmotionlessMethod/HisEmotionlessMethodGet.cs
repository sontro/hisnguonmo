...
namespace MOS.DAO.HisEmotionlessMethod
{
    partial class HisEmotionlessMethodGet : EntityBase
    {
        ...
        {
            List<HIS_EMOTIONLESS_METHOD> list = new List<HIS_EMOTIONLESS_METHOD>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_EMOTIONLESS_METHOD.AsNoTracking().AsQueryable();
        ...
        public HIS_EMOTIONLESS_METHOD GetById(long id, HisEmotionlessMethodSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_EMOTIONLESS_METHOD.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisEmotionlessMethodExpression != null && search.listHisEmotionlessMethodExpression.Count > 0)
                        {
                            foreach (var item in search.listHisEmotionlessMethodExpression)
                            {
