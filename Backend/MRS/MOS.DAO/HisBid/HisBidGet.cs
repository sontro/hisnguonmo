...
namespace MOS.DAO.HisBid
{
    partial class HisBidGet : EntityBase
    {
        ...
        {
            List<HIS_BID> list = new List<HIS_BID>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_BID.AsNoTracking().AsQueryable();
                        if (search.listHisBidExpression != null && search.listHisBidExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBidExpression)
                            {
        ...
        public HIS_BID GetById(long id, HisBidSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_BID.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisBidExpression != null && search.listHisBidExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBidExpression)
                            {
