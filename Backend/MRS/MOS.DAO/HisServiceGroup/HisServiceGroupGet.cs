        ...
        public List<HIS_SERVICE_GROUP> Get(HisServiceGroupSO search, CommonParam param)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SERVICE_GROUP.AsNoTracking().AsQueryable();
                        if (search.listHisServiceGroupExpression != null && search.listHisServiceGroupExpression.Count > 0)
                        {
                            foreach (var item in search.listHisServiceGroupExpression)
                            {
        ...
        public HIS_SERVICE_GROUP GetById(long id, HisServiceGroupSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_SERVICE_GROUP.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisServiceGroupExpression != null && search.listHisServiceGroupExpression.Count > 0)
                        {
                            foreach (var item in search.listHisServiceGroupExpression)
                            {
