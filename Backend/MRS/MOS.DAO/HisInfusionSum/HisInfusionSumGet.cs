...
namespace MOS.DAO.HisInfusionSum
{
    partial class HisInfusionSumGet : EntityBase
    {
        ...
        {
            List<HIS_INFUSION_SUM> list = new List<HIS_INFUSION_SUM>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_INFUSION_SUM.AsNoTracking().AsQueryable();
        ...
        public HIS_INFUSION_SUM GetById(long id, HisInfusionSumSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_INFUSION_SUM.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisInfusionSumExpression != null && search.listHisInfusionSumExpression.Count > 0)
                        {
                            foreach (var item in search.listHisInfusionSumExpression)
                            {
