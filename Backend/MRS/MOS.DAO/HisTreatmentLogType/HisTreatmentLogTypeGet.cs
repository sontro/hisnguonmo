...
namespace MOS.DAO.HisTreatmentLogType
{
    partial class HisTreatmentLogTypeGet : EntityBase
    {
        ...
        {
            List<HIS_TREATMENT_LOG_TYPE> list = new List<HIS_TREATMENT_LOG_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_TREATMENT_LOG_TYPE.AsNoTracking().AsQueryable();
                        if (search.listHisTreatmentLogTypeExpression != null && search.listHisTreatmentLogTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisTreatmentLogTypeExpression)
                            {
        ...
        public HIS_TREATMENT_LOG_TYPE GetById(long id, HisTreatmentLogTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_TREATMENT_LOG_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisTreatmentLogTypeExpression != null && search.listHisTreatmentLogTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisTreatmentLogTypeExpression)
                            {
