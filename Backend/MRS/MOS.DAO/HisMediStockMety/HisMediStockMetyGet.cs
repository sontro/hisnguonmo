...
namespace MOS.DAO.HisMediStockMety
{
    partial class HisMediStockMetyGet : EntityBase
    {
        ...
        {
            List<HIS_MEDI_STOCK_METY> list = new List<HIS_MEDI_STOCK_METY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MEDI_STOCK_METY.AsNoTracking().AsQueryable();
        ...
        public HIS_MEDI_STOCK_METY GetById(long id, HisMediStockMetySO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_MEDI_STOCK_METY.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMediStockMetyExpression != null && search.listHisMediStockMetyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMediStockMetyExpression)
                            {
