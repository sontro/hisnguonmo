...
namespace MOS.DAO.HisSaroExro
{
    partial class HisSaroExroGet : EntityBase
    {
        ...
        {
            List<HIS_SARO_EXRO> list = new List<HIS_SARO_EXRO>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SARO_EXRO.AsNoTracking().AsQueryable();
                        if (search.listHisSaroExroExpression != null && search.listHisSaroExroExpression.Count > 0)
                        {
                            foreach (var item in search.listHisSaroExroExpression)
                            {
        ...
        public HIS_SARO_EXRO GetById(long id, HisSaroExroSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_SARO_EXRO.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisSaroExroExpression != null && search.listHisSaroExroExpression.Count > 0)
                        {
                            foreach (var item in search.listHisSaroExroExpression)
                            {
