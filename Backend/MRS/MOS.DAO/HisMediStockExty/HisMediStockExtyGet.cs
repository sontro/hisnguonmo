...
namespace MOS.DAO.HisMediStockExty
{
    partial class HisMediStockExtyGet : EntityBase
    {
        ...
        {
            List<HIS_MEDI_STOCK_EXTY> list = new List<HIS_MEDI_STOCK_EXTY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MEDI_STOCK_EXTY.AsNoTracking().AsQueryable();
                        if (search.listHisMediStockExtyExpression != null && search.listHisMediStockExtyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMediStockExtyExpression)
                            {
        ...
        public HIS_MEDI_STOCK_EXTY GetById(long id, HisMediStockExtySO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_MEDI_STOCK_EXTY.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMediStockExtyExpression != null && search.listHisMediStockExtyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMediStockExtyExpression)
                            {
