...
namespace MOS.DAO.HisSereServ
{
    ...
    {
        public List<V_HIS_SERE_SERV_7> GetView7(HisSereServSO search, CommonParam param)
        {
            List<V_HIS_SERE_SERV_7> list = new List<V_HIS_SERE_SERV_7>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_SERE_SERV_7.AsNoTracking().AsQueryable();
                        if (search.listVHisSereServ7Expression != null && search.listVHisSereServ7Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisSereServ7Expression)
                            {
