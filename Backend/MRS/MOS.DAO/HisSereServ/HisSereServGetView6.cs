...
namespace MOS.DAO.HisSereServ
{
    ...
    {
        public List<V_HIS_SERE_SERV_6> GetView6(HisSereServSO search, CommonParam param)
        {
            List<V_HIS_SERE_SERV_6> list = new List<V_HIS_SERE_SERV_6>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_SERE_SERV_6.AsNoTracking().AsQueryable();
                        if (search.listVHisSereServ6Expression != null && search.listVHisSereServ6Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisSereServ6Expression)
                            {
