...
namespace MOS.DAO.HisSereServ
{
    ...
    {
        public List<V_HIS_SERE_SERV_9> GetView9(HisSereServSO search, CommonParam param)
        {
            List<V_HIS_SERE_SERV_9> list = new List<V_HIS_SERE_SERV_9>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_SERE_SERV_9.AsNoTracking().AsQueryable();
                        if (search.listVHisSereServ9Expression != null && search.listVHisSereServ9Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisSereServ9Expression)
                            {
