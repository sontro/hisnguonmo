...
namespace MOS.DAO.HisSereServ
{
    ...
    {
        public List<V_HIS_SERE_SERV_12> GetView12(HisSereServSO search, CommonParam param)
        {
            List<V_HIS_SERE_SERV_12> list = new List<V_HIS_SERE_SERV_12>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_SERE_SERV_12.AsNoTracking().AsQueryable();
