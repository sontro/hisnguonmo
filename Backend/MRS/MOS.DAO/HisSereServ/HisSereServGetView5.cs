...
namespace MOS.DAO.HisSereServ
{
    ...
    {
        public List<V_HIS_SERE_SERV_5> GetView5(HisSereServSO search, CommonParam param)
        {
            List<V_HIS_SERE_SERV_5> list = new List<V_HIS_SERE_SERV_5>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_SERE_SERV_5.AsNoTracking().AsQueryable();
