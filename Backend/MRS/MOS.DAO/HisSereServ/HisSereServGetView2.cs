...
namespace MOS.DAO.HisSereServ
{
    ...
    {
        public List<V_HIS_SERE_SERV_2> GetView2(HisSereServSO search, CommonParam param)
        {
            List<V_HIS_SERE_SERV_2> list = new List<V_HIS_SERE_SERV_2>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_SERE_SERV_2.AsNoTracking().AsQueryable();
                        if (search.listVHisSereServ2Expression != null && search.listVHisSereServ2Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisSereServ2Expression)
                            {
