...
namespace MOS.DAO.HisMestPeriodMate
{
    partial class HisMestPeriodMateGet : EntityBase
    {
        ...
        {
            List<HIS_MEST_PERIOD_MATE> list = new List<HIS_MEST_PERIOD_MATE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MEST_PERIOD_MATE.AsNoTracking().AsQueryable();
        ...
        public HIS_MEST_PERIOD_MATE GetById(long id, HisMestPeriodMateSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_MEST_PERIOD_MATE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMestPeriodMateExpression != null && search.listHisMestPeriodMateExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMestPeriodMateExpression)
                            {
