...
namespace MOS.DAO.HisExamSereDire
{
    partial class HisExamSereDireGet : EntityBase
    {
        ...
        {
            List<HIS_EXAM_SERE_DIRE> list = new List<HIS_EXAM_SERE_DIRE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_EXAM_SERE_DIRE.AsNoTracking().AsQueryable();
                        if (search.listHisExamSereDireExpression != null && search.listHisExamSereDireExpression.Count > 0)
                        {
                            foreach (var item in search.listHisExamSereDireExpression)
                            {
        ...
        public HIS_EXAM_SERE_DIRE GetById(long id, HisExamSereDireSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_EXAM_SERE_DIRE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisExamSereDireExpression != null && search.listHisExamSereDireExpression.Count > 0)
                        {
                            foreach (var item in search.listHisExamSereDireExpression)
                            {
