...
namespace MOS.DAO.HisMestPatySub
{
    partial class HisMestPatySubGet : EntityBase
    {
        ...
        {
            List<HIS_MEST_PATY_SUB> list = new List<HIS_MEST_PATY_SUB>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MEST_PATY_SUB.AsNoTracking().AsQueryable();
        ...
        public HIS_MEST_PATY_SUB GetById(long id, HisMestPatySubSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_MEST_PATY_SUB.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMestPatySubExpression != null && search.listHisMestPatySubExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMestPatySubExpression)
                            {
