...
namespace MOS.DAO.HisEquipmentSet
{
    partial class HisEquipmentSetGet : EntityBase
    {
        ...
        {
            List<HIS_EQUIPMENT_SET> list = new List<HIS_EQUIPMENT_SET>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_EQUIPMENT_SET.AsNoTracking().AsQueryable();
        ...
        public HIS_EQUIPMENT_SET GetById(long id, HisEquipmentSetSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_EQUIPMENT_SET.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisEquipmentSetExpression != null && search.listHisEquipmentSetExpression.Count > 0)
                        {
                            foreach (var item in search.listHisEquipmentSetExpression)
                            {
