...
{
    partial class HisRegisterGateGet : EntityBase
    {
        public List<HIS_REGISTER_GATE> Get(HisRegisterGateSO search, CommonParam param)
        {
            List<HIS_REGISTER_GATE> list = new List<HIS_REGISTER_GATE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_REGISTER_GATE.AsNoTracking().AsQueryable();
                        if (search.listHisRegisterGateExpression != null && search.listHisRegisterGateExpression.Count > 0)
                        {
                            foreach (var item in search.listHisRegisterGateExpression)
                            {
        ...
        public HIS_REGISTER_GATE GetById(long id, HisRegisterGateSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_REGISTER_GATE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisRegisterGateExpression != null && search.listHisRegisterGateExpression.Count > 0)
                        {
                            foreach (var item in search.listHisRegisterGateExpression)
                            {
