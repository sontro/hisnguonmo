...
namespace MOS.DAO.HisTransaction
{
    partial class HisTransactionGet : EntityBase
    {
        ...
        {
            List<V_HIS_TRANSACTION_5> list = new List<V_HIS_TRANSACTION_5>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_TRANSACTION_5.AsNoTracking().AsQueryable();
                        if (search.listVHisTransaction5Expression != null && search.listVHisTransaction5Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisTransaction5Expression)
                            {
