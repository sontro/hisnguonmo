...
namespace MOS.DAO.HisTestIndexRange
{
    partial class HisTestIndexRangeGet : EntityBase
    {
        ...
        {
            List<HIS_TEST_INDEX_RANGE> list = new List<HIS_TEST_INDEX_RANGE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_TEST_INDEX_RANGE.AsNoTracking().AsQueryable();
        ...
        public HIS_TEST_INDEX_RANGE GetById(long id, HisTestIndexRangeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_TEST_INDEX_RANGE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisTestIndexRangeExpression != null && search.listHisTestIndexRangeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisTestIndexRangeExpression)
                            {
