...
namespace MOS.DAO.HisMestPeriodMaty
{
    partial class HisMestPeriodMatyGet : EntityBase
    {
        ...
        {
            List<V_HIS_MEST_PERIOD_MATY> list = new List<V_HIS_MEST_PERIOD_MATY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_MEST_PERIOD_MATY.AsNoTracking().AsQueryable();
                        if (search.listVHisMestPeriodMatyExpression != null && search.listVHisMestPeriodMatyExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisMestPeriodMatyExpression)
                            {
