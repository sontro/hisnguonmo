...
{
    partial class HisMestPeriodMatyGet : EntityBase
    {
        public List<HIS_MEST_PERIOD_MATY> Get(HisMestPeriodMatySO search, CommonParam param)
        {
            List<HIS_MEST_PERIOD_MATY> list = new List<HIS_MEST_PERIOD_MATY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MEST_PERIOD_MATY.AsNoTracking().AsQueryable();
                        if (search.listHisMestPeriodMatyExpression != null && search.listHisMestPeriodMatyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMestPeriodMatyExpression)
                            {
        ...
        public HIS_MEST_PERIOD_MATY GetById(long id, HisMestPeriodMatySO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_MEST_PERIOD_MATY.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMestPeriodMatyExpression != null && search.listHisMestPeriodMatyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMestPeriodMatyExpression)
                            {
