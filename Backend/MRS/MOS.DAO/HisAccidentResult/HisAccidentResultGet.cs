...
namespace MOS.DAO.HisAccidentResult
{
    partial class HisAccidentResultGet : EntityBase
    {
        ...
        {
            List<HIS_ACCIDENT_RESULT> list = new List<HIS_ACCIDENT_RESULT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_ACCIDENT_RESULT.AsNoTracking().AsQueryable();
                        if (search.listHisAccidentResultExpression != null && search.listHisAccidentResultExpression.Count > 0)
                        {
                            foreach (var item in search.listHisAccidentResultExpression)
                            {
        ...
        public HIS_ACCIDENT_RESULT GetById(long id, HisAccidentResultSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_ACCIDENT_RESULT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisAccidentResultExpression != null && search.listHisAccidentResultExpression.Count > 0)
                        {
                            foreach (var item in search.listHisAccidentResultExpression)
                            {
