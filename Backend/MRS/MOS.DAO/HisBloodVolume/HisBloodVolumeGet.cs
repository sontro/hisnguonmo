...
namespace MOS.DAO.HisBloodVolume
{
    partial class HisBloodVolumeGet : EntityBase
    {
        ...
        {
            List<HIS_BLOOD_VOLUME> list = new List<HIS_BLOOD_VOLUME>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_BLOOD_VOLUME.AsNoTracking().AsQueryable();
        ...
        public HIS_BLOOD_VOLUME GetById(long id, HisBloodVolumeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_BLOOD_VOLUME.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisBloodVolumeExpression != null && search.listHisBloodVolumeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBloodVolumeExpression)
                            {
