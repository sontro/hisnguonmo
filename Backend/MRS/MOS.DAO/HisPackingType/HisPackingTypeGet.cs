...
namespace MOS.DAO.HisPackingType
{
    partial class HisPackingTypeGet : EntityBase
    {
        ...
        {
            List<HIS_PACKING_TYPE> list = new List<HIS_PACKING_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_PACKING_TYPE.AsNoTracking().AsQueryable();
                        if (search.listHisPackingTypeExpression != null && search.listHisPackingTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisPackingTypeExpression)
                            {
        ...
        public HIS_PACKING_TYPE GetById(long id, HisPackingTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_PACKING_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisPackingTypeExpression != null && search.listHisPackingTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisPackingTypeExpression)
                            {
