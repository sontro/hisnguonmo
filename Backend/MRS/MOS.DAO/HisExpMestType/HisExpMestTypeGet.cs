...
namespace MOS.DAO.HisExpMestType
{
    partial class HisExpMestTypeGet : EntityBase
    {
        ...
        {
            List<HIS_EXP_MEST_TYPE> list = new List<HIS_EXP_MEST_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_EXP_MEST_TYPE.AsNoTracking().AsQueryable();
        ...
        public HIS_EXP_MEST_TYPE GetById(long id, HisExpMestTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_EXP_MEST_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisExpMestTypeExpression != null && search.listHisExpMestTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisExpMestTypeExpression)
                            {
