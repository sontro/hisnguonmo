...
namespace MOS.DAO.HisHeinApproval
{
    partial class HisHeinApprovalGet : EntityBase
    {
        ...
        {
            List<HIS_HEIN_APPROVAL> list = new List<HIS_HEIN_APPROVAL>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_HEIN_APPROVAL.AsNoTracking().AsQueryable();
        ...
        public HIS_HEIN_APPROVAL GetById(long id, HisHeinApprovalSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_HEIN_APPROVAL.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisHeinApprovalExpression != null && search.listHisHeinApprovalExpression.Count > 0)
                        {
                            foreach (var item in search.listHisHeinApprovalExpression)
                            {
