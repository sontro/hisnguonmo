...
namespace MOS.DAO.HisServiceRoom
{
    ...
    {
        public List<V_HIS_SERVICE_ROOM> GetView(HisServiceRoomSO search, CommonParam param)
        {
            List<V_HIS_SERVICE_ROOM> list = new List<V_HIS_SERVICE_ROOM>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_SERVICE_ROOM.AsNoTracking().AsQueryable();
                        if (search.listVHisServiceRoomExpression != null && search.listVHisServiceRoomExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisServiceRoomExpression)
                            {
