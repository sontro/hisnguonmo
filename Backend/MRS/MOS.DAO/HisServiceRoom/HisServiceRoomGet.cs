...
namespace MOS.DAO.HisServiceRoom
{
    partial class HisServiceRoomGet : EntityBase
    {
        ...
        {
            List<HIS_SERVICE_ROOM> list = new List<HIS_SERVICE_ROOM>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SERVICE_ROOM.AsNoTracking().AsQueryable();
                        if (search.listHisServiceRoomExpression != null && search.listHisServiceRoomExpression.Count > 0)
                        {
                            foreach (var item in search.listHisServiceRoomExpression)
                            {
        ...
        public HIS_SERVICE_ROOM GetById(long id, HisServiceRoomSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_SERVICE_ROOM.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisServiceRoomExpression != null && search.listHisServiceRoomExpression.Count > 0)
                        {
                            foreach (var item in search.listHisServiceRoomExpression)
                            {
