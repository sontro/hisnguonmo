...
namespace MOS.DAO.HisImpMestTypeUser
{
    partial class HisImpMestTypeUserGet : EntityBase
    {
        ...
        {
            List<HIS_IMP_MEST_TYPE_USER> list = new List<HIS_IMP_MEST_TYPE_USER>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_IMP_MEST_TYPE_USER.AsNoTracking().AsQueryable();
        ...
        public HIS_IMP_MEST_TYPE_USER GetById(long id, HisImpMestTypeUserSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_IMP_MEST_TYPE_USER.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisImpMestTypeUserExpression != null && search.listHisImpMestTypeUserExpression.Count > 0)
                        {
                            foreach (var item in search.listHisImpMestTypeUserExpression)
                            {
