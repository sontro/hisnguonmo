...
namespace MOS.DAO.HisPatientCase
{
    partial class HisPatientCaseGet : EntityBase
    {
        ...
        {
            List<HIS_PATIENT_CASE> list = new List<HIS_PATIENT_CASE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_PATIENT_CASE.AsNoTracking().AsQueryable();
        ...
        public HIS_PATIENT_CASE GetById(long id, HisPatientCaseSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_PATIENT_CASE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisPatientCaseExpression != null && search.listHisPatientCaseExpression.Count > 0)
                        {
                            foreach (var item in search.listHisPatientCaseExpression)
                            {
