...
namespace MOS.DAO.HisMestPeriodMedi
{
    partial class HisMestPeriodMediGet : EntityBase
    {
        ...
        {
            List<HIS_MEST_PERIOD_MEDI> list = new List<HIS_MEST_PERIOD_MEDI>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MEST_PERIOD_MEDI.AsNoTracking().AsQueryable();
        ...
        public HIS_MEST_PERIOD_MEDI GetById(long id, HisMestPeriodMediSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_MEST_PERIOD_MEDI.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMestPeriodMediExpression != null && search.listHisMestPeriodMediExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMestPeriodMediExpression)
                            {
