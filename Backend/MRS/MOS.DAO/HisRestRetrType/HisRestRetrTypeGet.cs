...
namespace MOS.DAO.HisRestRetrType
{
    partial class HisRestRetrTypeGet : EntityBase
    {
        ...
        {
            List<HIS_REST_RETR_TYPE> list = new List<HIS_REST_RETR_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_REST_RETR_TYPE.AsNoTracking().AsQueryable();
        ...
        public HIS_REST_RETR_TYPE GetById(long id, HisRestRetrTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_REST_RETR_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisRestRetrTypeExpression != null && search.listHisRestRetrTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisRestRetrTypeExpression)
                            {
