...
namespace MOS.DAO.HisPtttPriority
{
    partial class HisPtttPriorityGet : EntityBase
    {
        ...
        {
            List<HIS_PTTT_PRIORITY> list = new List<HIS_PTTT_PRIORITY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_PTTT_PRIORITY.AsNoTracking().AsQueryable();
        ...
        public HIS_PTTT_PRIORITY GetById(long id, HisPtttPrioritySO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_PTTT_PRIORITY.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisPtttPriorityExpression != null && search.listHisPtttPriorityExpression.Count > 0)
                        {
                            foreach (var item in search.listHisPtttPriorityExpression)
                            {
