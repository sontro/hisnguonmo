...
namespace MOS.DAO.HisImpMestBlood
{
    partial class HisImpMestBloodGet : EntityBase
    {
        ...
        {
            List<HIS_IMP_MEST_BLOOD> list = new List<HIS_IMP_MEST_BLOOD>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_IMP_MEST_BLOOD.AsNoTracking().AsQueryable();
        ...
        public HIS_IMP_MEST_BLOOD GetById(long id, HisImpMestBloodSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_IMP_MEST_BLOOD.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisImpMestBloodExpression != null && search.listHisImpMestBloodExpression.Count > 0)
                        {
                            foreach (var item in search.listHisImpMestBloodExpression)
                            {
