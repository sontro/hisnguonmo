...
namespace MOS.DAO.HisPtttCatastrophe
{
    partial class HisPtttCatastropheGet : EntityBase
    {
        ...
        {
            List<HIS_PTTT_CATASTROPHE> list = new List<HIS_PTTT_CATASTROPHE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_PTTT_CATASTROPHE.AsNoTracking().AsQueryable();
        ...
        public HIS_PTTT_CATASTROPHE GetById(long id, HisPtttCatastropheSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_PTTT_CATASTROPHE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisPtttCatastropheExpression != null && search.listHisPtttCatastropheExpression.Count > 0)
                        {
                            foreach (var item in search.listHisPtttCatastropheExpression)
                            {
