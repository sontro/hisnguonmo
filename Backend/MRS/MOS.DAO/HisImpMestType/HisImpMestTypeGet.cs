...
namespace MOS.DAO.HisImpMestType
{
    partial class HisImpMestTypeGet : EntityBase
    {
        ...
        {
            List<HIS_IMP_MEST_TYPE> list = new List<HIS_IMP_MEST_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_IMP_MEST_TYPE.AsNoTracking().AsQueryable();
        ...
        public HIS_IMP_MEST_TYPE GetById(long id, HisImpMestTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_IMP_MEST_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisImpMestTypeExpression != null && search.listHisImpMestTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisImpMestTypeExpression)
                            {
