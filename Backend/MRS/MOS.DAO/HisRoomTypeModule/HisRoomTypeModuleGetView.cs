...
namespace MOS.DAO.HisRoomTypeModule
{
    partial class HisRoomTypeModuleGet : EntityBase
    {
        ...
        {
            List<V_HIS_ROOM_TYPE_MODULE> list = new List<V_HIS_ROOM_TYPE_MODULE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_ROOM_TYPE_MODULE.AsNoTracking().AsQueryable();
                        if (search.listVHisRoomTypeModuleExpression != null && search.listVHisRoomTypeModuleExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisRoomTypeModuleExpression)
                            {
