...
namespace MOS.DAO.HisMaterialPaty
{
    partial class HisMaterialPatyGet : EntityBase
    {
        ...
        {
            List<V_HIS_MATERIAL_PATY> list = new List<V_HIS_MATERIAL_PATY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_MATERIAL_PATY.AsNoTracking().AsQueryable();
                        if (search.listVHisMaterialPatyExpression != null && search.listVHisMaterialPatyExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisMaterialPatyExpression)
                            {
