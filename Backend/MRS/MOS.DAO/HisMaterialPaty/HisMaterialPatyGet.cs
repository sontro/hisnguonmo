...
namespace MOS.DAO.HisMaterialPaty
{
    partial class HisMaterialPatyGet : EntityBase
    {
        ...
        {
            List<HIS_MATERIAL_PATY> list = new List<HIS_MATERIAL_PATY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MATERIAL_PATY.AsNoTracking().AsQueryable();
        ...
        public HIS_MATERIAL_PATY GetById(long id, HisMaterialPatySO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_MATERIAL_PATY.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMaterialPatyExpression != null && search.listHisMaterialPatyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMaterialPatyExpression)
                            {
