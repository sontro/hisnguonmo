...
namespace MOS.DAO.HisInvoicePrint
{
    partial class HisInvoicePrintGet : EntityBase
    {
        ...
        {
            List<HIS_INVOICE_PRINT> list = new List<HIS_INVOICE_PRINT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_INVOICE_PRINT.AsNoTracking().AsQueryable();
        ...
        public HIS_INVOICE_PRINT GetById(long id, HisInvoicePrintSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_INVOICE_PRINT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisInvoicePrintExpression != null && search.listHisInvoicePrintExpression.Count > 0)
                        {
                            foreach (var item in search.listHisInvoicePrintExpression)
                            {
