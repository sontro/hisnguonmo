...
namespace MOS.DAO.HisMedicineTypeTut
{
    partial class HisMedicineTypeTutGet : EntityBase
    {
        ...
        {
            List<HIS_MEDICINE_TYPE_TUT> list = new List<HIS_MEDICINE_TYPE_TUT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MEDICINE_TYPE_TUT.AsNoTracking().AsQueryable();
        ...
        public HIS_MEDICINE_TYPE_TUT GetById(long id, HisMedicineTypeTutSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_MEDICINE_TYPE_TUT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMedicineTypeTutExpression != null && search.listHisMedicineTypeTutExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMedicineTypeTutExpression)
                            {
