...
namespace MOS.DAO.HisReceptionRoom
{
    partial class HisReceptionRoomGet : EntityBase
    {
        ...
        {
            List<HIS_RECEPTION_ROOM> list = new List<HIS_RECEPTION_ROOM>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_RECEPTION_ROOM.AsNoTracking().AsQueryable();
                        if (search.listHisReceptionRoomExpression != null && search.listHisReceptionRoomExpression.Count > 0)
                        {
                            foreach (var item in search.listHisReceptionRoomExpression)
                            {
        ...
        public HIS_RECEPTION_ROOM GetById(long id, HisReceptionRoomSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_RECEPTION_ROOM.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisReceptionRoomExpression != null && search.listHisReceptionRoomExpression.Count > 0)
                        {
                            foreach (var item in search.listHisReceptionRoomExpression)
                            {
