...
namespace MOS.DAO.HisMediStockPeriod
{
    partial class HisMediStockPeriodGet : EntityBase
    {
        ...
        {
            List<HIS_MEDI_STOCK_PERIOD> list = new List<HIS_MEDI_STOCK_PERIOD>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MEDI_STOCK_PERIOD.AsNoTracking().AsQueryable();
        ...
        public HIS_MEDI_STOCK_PERIOD GetById(long id, HisMediStockPeriodSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_MEDI_STOCK_PERIOD.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMediStockPeriodExpression != null && search.listHisMediStockPeriodExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMediStockPeriodExpression)
                            {
