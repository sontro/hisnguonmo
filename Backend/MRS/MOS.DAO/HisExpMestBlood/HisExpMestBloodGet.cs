...
namespace MOS.DAO.HisExpMestBlood
{
    partial class HisExpMestBloodGet : EntityBase
    {
        ...
        {
            List<HIS_EXP_MEST_BLOOD> list = new List<HIS_EXP_MEST_BLOOD>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_EXP_MEST_BLOOD.AsNoTracking().AsQueryable();
                        if (search.listHisExpMestBloodExpression != null && search.listHisExpMestBloodExpression.Count > 0)
                        {
                            foreach (var item in search.listHisExpMestBloodExpression)
                            {
        ...
        public HIS_EXP_MEST_BLOOD GetById(long id, HisExpMestBloodSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_EXP_MEST_BLOOD.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisExpMestBloodExpression != null && search.listHisExpMestBloodExpression.Count > 0)
                        {
                            foreach (var item in search.listHisExpMestBloodExpression)
                            {
