...
{
    partial class HisServiceGet : EntityBase
    {
        public List<V_HIS_SERVICE> GetView(HisServiceSO search, CommonParam param)
        {
            List<V_HIS_SERVICE> list = new List<V_HIS_SERVICE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_SERVICE.AsNoTracking().AsQueryable();
                        if (search.listVHisServiceExpression != null && search.listVHisServiceExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisServiceExpression)
                            {
        ...
        {
            List<V_HIS_SERVICE_1> list = new List<V_HIS_SERVICE_1>();
            try
            {
                bool valid = true;
                valid = valid && IsNotNull(param);
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.V_HIS_SERVICE_1.AsNoTracking().AsQueryable();
                        if (search.listVHisService1Expression != null && search.listVHisService1Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisService1Expression)
                            {
                                query = query.Where(item);
                            }
                        }
