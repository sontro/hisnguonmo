...
namespace MOS.DAO.HisService
{
    partial class HisServiceGet : EntityBase
    {
        ...
        {
            List<HIS_SERVICE> list = new List<HIS_SERVICE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SERVICE.AsNoTracking().AsQueryable();
                        if (search.listHisServiceExpression != null && search.listHisServiceExpression.Count > 0)
                        {
                            foreach (var item in search.listHisServiceExpression)
                            {
        ...
        public HIS_SERVICE GetById(long id, HisServiceSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_SERVICE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisServiceExpression != null && search.listHisServiceExpression.Count > 0)
                        {
                            foreach (var item in search.listHisServiceExpression)
                            {
