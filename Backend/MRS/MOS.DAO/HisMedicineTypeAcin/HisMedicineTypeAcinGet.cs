...
namespace MOS.DAO.HisMedicineTypeAcin
{
    partial class HisMedicineTypeAcinGet : EntityBase
    {
        ...
        {
            List<HIS_MEDICINE_TYPE_ACIN> list = new List<HIS_MEDICINE_TYPE_ACIN>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MEDICINE_TYPE_ACIN.AsNoTracking().AsQueryable();
        ...
        public HIS_MEDICINE_TYPE_ACIN GetById(long id, HisMedicineTypeAcinSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_MEDICINE_TYPE_ACIN.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMedicineTypeAcinExpression != null && search.listHisMedicineTypeAcinExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMedicineTypeAcinExpression)
                            {
