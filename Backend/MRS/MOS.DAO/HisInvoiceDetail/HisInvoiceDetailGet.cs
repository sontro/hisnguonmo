...
namespace MOS.DAO.HisInvoiceDetail
{
    partial class HisInvoiceDetailGet : EntityBase
    {
        ...
        {
            List<HIS_INVOICE_DETAIL> list = new List<HIS_INVOICE_DETAIL>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_INVOICE_DETAIL.AsNoTracking().AsQueryable();
                        if (search.listHisInvoiceDetailExpression != null && search.listHisInvoiceDetailExpression.Count > 0)
                        {
                            foreach (var item in search.listHisInvoiceDetailExpression)
                            {
        ...
        public HIS_INVOICE_DETAIL GetById(long id, HisInvoiceDetailSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_INVOICE_DETAIL.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisInvoiceDetailExpression != null && search.listHisInvoiceDetailExpression.Count > 0)
                        {
                            foreach (var item in search.listHisInvoiceDetailExpression)
                            {
