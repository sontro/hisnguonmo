...
namespace MOS.DAO.HisMedicineUseForm
{
    partial class HisMedicineUseFormGet : EntityBase
    {
        ...
        {
            List<HIS_MEDICINE_USE_FORM> list = new List<HIS_MEDICINE_USE_FORM>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MEDICINE_USE_FORM.AsNoTracking().AsQueryable();
        ...
        public HIS_MEDICINE_USE_FORM GetById(long id, HisMedicineUseFormSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_MEDICINE_USE_FORM.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMedicineUseFormExpression != null && search.listHisMedicineUseFormExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMedicineUseFormExpression)
                            {
