...
namespace MOS.DAO.HisExpMestMaterial
{
    partial class HisExpMestMaterialGet : EntityBase
    {
        ...
        {
            List<V_HIS_EXP_MEST_MATERIAL_3> list = new List<V_HIS_EXP_MEST_MATERIAL_3>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_EXP_MEST_MATERIAL_3.AsNoTracking().AsQueryable();
                        if (search.listVHisExpMestMaterial3Expression != null && search.listVHisExpMestMaterial3Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisExpMestMaterial3Expression)
                            {
