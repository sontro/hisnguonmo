...
namespace MOS.DAO.HisExpMestMaterial
{
    partial class HisExpMestMaterialGet : EntityBase
    {
        ...
        {
            List<V_HIS_EXP_MEST_MATERIAL> list = new List<V_HIS_EXP_MEST_MATERIAL>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_EXP_MEST_MATERIAL.AsNoTracking().AsQueryable();
                        if (search.listVHisExpMestMaterialExpression != null && search.listVHisExpMestMaterialExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisExpMestMaterialExpression)
                            {
