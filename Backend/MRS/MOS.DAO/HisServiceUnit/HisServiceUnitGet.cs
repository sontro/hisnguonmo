...
namespace MOS.DAO.HisServiceUnit
{
    partial class HisServiceUnitGet : EntityBase
    {
        ...
        {
            List<HIS_SERVICE_UNIT> list = new List<HIS_SERVICE_UNIT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SERVICE_UNIT.AsNoTracking().AsQueryable();
                        if (search.listHisServiceUnitExpression != null && search.listHisServiceUnitExpression.Count > 0)
                        {
                            foreach (var item in search.listHisServiceUnitExpression)
                            {
        ...
        public HIS_SERVICE_UNIT GetById(long id, HisServiceUnitSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_SERVICE_UNIT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisServiceUnitExpression != null && search.listHisServiceUnitExpression.Count > 0)
                        {
                            foreach (var item in search.listHisServiceUnitExpression)
                            {
