...
{
    partial class HisCareerGet : EntityBase
    {
        public List<HIS_CAREER> Get(HisCareerSO search, CommonParam param)
        {
            List<HIS_CAREER> list = new List<HIS_CAREER>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_CAREER.AsNoTracking().AsQueryable();
                        if (search.listHisCareerExpression != null && search.listHisCareerExpression.Count > 0)
                        {
                            foreach (var item in search.listHisCareerExpression)
                            {
        ...
        public HIS_CAREER GetById(long id, HisCareerSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_CAREER.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisCareerExpression != null && search.listHisCareerExpression.Count > 0)
                        {
                            foreach (var item in search.listHisCareerExpression)
                            {
