...
namespace MOS.DAO.HisMestMetyDepa
{
    partial class HisMestMetyDepaGet : EntityBase
    {
        ...
        {
            List<HIS_MEST_METY_DEPA> list = new List<HIS_MEST_METY_DEPA>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MEST_METY_DEPA.AsNoTracking().AsQueryable();
        ...
        public HIS_MEST_METY_DEPA GetById(long id, HisMestMetyDepaSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_MEST_METY_DEPA.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMestMetyDepaExpression != null && search.listHisMestMetyDepaExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMestMetyDepaExpression)
                            {
