...
namespace MOS.DAO.HisMediStock
{
    partial class HisMediStockGet : EntityBase
    {
        ...
        {
            List<HIS_MEDI_STOCK> list = new List<HIS_MEDI_STOCK>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MEDI_STOCK.AsNoTracking().AsQueryable();
        ...
        public HIS_MEDI_STOCK GetById(long id, HisMediStockSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_MEDI_STOCK.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMediStockExpression != null && search.listHisMediStockExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMediStockExpression)
                            {
