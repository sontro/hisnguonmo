...
namespace MOS.DAO.HisRegisterReq
{
    partial class HisRegisterReqGet : EntityBase
    {
        ...
        {
            List<HIS_REGISTER_REQ> list = new List<HIS_REGISTER_REQ>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_REGISTER_REQ.AsNoTracking().AsQueryable();
        ...
        public HIS_REGISTER_REQ GetById(long id, HisRegisterReqSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_REGISTER_REQ.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisRegisterReqExpression != null && search.listHisRegisterReqExpression.Count > 0)
                        {
                            foreach (var item in search.listHisRegisterReqExpression)
                            {
