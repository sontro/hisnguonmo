...
{
    partial class HisPatientGet : EntityBase
    {
        public List<HIS_PATIENT> Get(HisPatientSO search, CommonParam param)
        {
            List<HIS_PATIENT> list = new List<HIS_PATIENT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_PATIENT.AsNoTracking().AsQueryable();
                        if (search.listHisPatientExpression != null && search.listHisPatientExpression.Count > 0)
                        {
                            foreach (var item in search.listHisPatientExpression)
                            {
        ...
        public HIS_PATIENT GetById(long id, HisPatientSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_PATIENT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisPatientExpression != null && search.listHisPatientExpression.Count > 0)
                        {
                            foreach (var item in search.listHisPatientExpression)
                            {
