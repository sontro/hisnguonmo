...
namespace MOS.DAO.HisTreatmentEndType
{
    partial class HisTreatmentEndTypeGet : EntityBase
    {
        ...
        {
            List<HIS_TREATMENT_END_TYPE> list = new List<HIS_TREATMENT_END_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_TREATMENT_END_TYPE.AsNoTracking().AsQueryable();
        ...
        public HIS_TREATMENT_END_TYPE GetById(long id, HisTreatmentEndTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_TREATMENT_END_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisTreatmentEndTypeExpression != null && search.listHisTreatmentEndTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisTreatmentEndTypeExpression)
                            {
