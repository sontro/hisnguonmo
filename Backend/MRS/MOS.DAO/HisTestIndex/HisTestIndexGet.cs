...
{
    partial class HisTestIndexGet : EntityBase
    {
        public List<HIS_TEST_INDEX> Get(HisTestIndexSO search, CommonParam param)
        {
            List<HIS_TEST_INDEX> list = new List<HIS_TEST_INDEX>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_TEST_INDEX.AsNoTracking().AsQueryable();
                        if (search.listHisTestIndexExpression != null && search.listHisTestIndexExpression.Count > 0)
                        {
                            foreach (var item in search.listHisTestIndexExpression)
                            {
        ...
        public HIS_TEST_INDEX GetById(long id, HisTestIndexSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_TEST_INDEX.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisTestIndexExpression != null && search.listHisTestIndexExpression.Count > 0)
                        {
                            foreach (var item in search.listHisTestIndexExpression)
                            {
