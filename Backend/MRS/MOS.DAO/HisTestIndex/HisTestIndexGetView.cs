...
namespace MOS.DAO.HisTestIndex
{
    partial class HisTestIndexGet : EntityBase
    {
        ...
        {
            List<V_HIS_TEST_INDEX> list = new List<V_HIS_TEST_INDEX>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_TEST_INDEX.AsNoTracking().AsQueryable();
                        if (search.listVHisTestIndexExpression != null && search.listVHisTestIndexExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisTestIndexExpression)
                            {
