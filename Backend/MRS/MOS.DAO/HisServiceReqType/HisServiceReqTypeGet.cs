...
namespace MOS.DAO.HisServiceReqType
{
    partial class HisServiceReqTypeGet : EntityBase
    {
        ...
        {
            List<HIS_SERVICE_REQ_TYPE> list = new List<HIS_SERVICE_REQ_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SERVICE_REQ_TYPE.AsNoTracking().AsQueryable();
                        if (search.listHisServiceReqTypeExpression != null && search.listHisServiceReqTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisServiceReqTypeExpression)
                            {
        ...
        public HIS_SERVICE_REQ_TYPE GetById(long id, HisServiceReqTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_SERVICE_REQ_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisServiceReqTypeExpression != null && search.listHisServiceReqTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisServiceReqTypeExpression)
                            {
