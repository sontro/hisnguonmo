...
{
    partial class HisHtuGet : EntityBase
    {
        public List<HIS_HTU> Get(HisHtuSO search, CommonParam param)
        {
            List<HIS_HTU> list = new List<HIS_HTU>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_HTU.AsNoTracking().AsQueryable();
                        if (search.listHisHtuExpression != null && search.listHisHtuExpression.Count > 0)
                        {
                            foreach (var item in search.listHisHtuExpression)
                            {
        ...
        public HIS_HTU GetById(long id, HisHtuSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_HTU.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisHtuExpression != null && search.listHisHtuExpression.Count > 0)
                        {
                            foreach (var item in search.listHisHtuExpression)
                            {
