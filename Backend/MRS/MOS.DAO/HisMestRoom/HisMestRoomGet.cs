...
namespace MOS.DAO.HisMestRoom
{
    partial class HisMestRoomGet : EntityBase
    {
        ...
        {
            List<HIS_MEST_ROOM> list = new List<HIS_MEST_ROOM>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MEST_ROOM.AsNoTracking().AsQueryable();
                        if (search.listHisMestRoomExpression != null && search.listHisMestRoomExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMestRoomExpression)
                            {
        ...
        public HIS_MEST_ROOM GetById(long id, HisMestRoomSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_MEST_ROOM.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMestRoomExpression != null && search.listHisMestRoomExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMestRoomExpression)
                            {
