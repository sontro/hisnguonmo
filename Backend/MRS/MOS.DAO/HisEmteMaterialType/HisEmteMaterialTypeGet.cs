...
namespace MOS.DAO.HisEmteMaterialType
{
    partial class HisEmteMaterialTypeGet : EntityBase
    {
        ...
        {
            List<HIS_EMTE_MATERIAL_TYPE> list = new List<HIS_EMTE_MATERIAL_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_EMTE_MATERIAL_TYPE.AsNoTracking().AsQueryable();
        ...
        public HIS_EMTE_MATERIAL_TYPE GetById(long id, HisEmteMaterialTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_EMTE_MATERIAL_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisEmteMaterialTypeExpression != null && search.listHisEmteMaterialTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisEmteMaterialTypeExpression)
                            {
