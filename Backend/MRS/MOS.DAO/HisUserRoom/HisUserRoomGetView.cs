...
namespace MOS.DAO.HisUserRoom
{
    partial class HisUserRoomGet : EntityBase
    {
        ...
        {
            List<V_HIS_USER_ROOM> list = new List<V_HIS_USER_ROOM>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_USER_ROOM.AsNoTracking().AsQueryable();
                        if (search.listVHisUserRoomExpression != null && search.listVHisUserRoomExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisUserRoomExpression)
                            {
