...
{
    partial class HisUserRoomGet : EntityBase
    {
        public List<HIS_USER_ROOM> Get(HisUserRoomSO search, CommonParam param)
        {
            List<HIS_USER_ROOM> list = new List<HIS_USER_ROOM>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_USER_ROOM.AsNoTracking().AsQueryable();
                        if (search.listHisUserRoomExpression != null && search.listHisUserRoomExpression.Count > 0)
                        {
                            foreach (var item in search.listHisUserRoomExpression)
                            {
        ...
        public HIS_USER_ROOM GetById(long id, HisUserRoomSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_USER_ROOM.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisUserRoomExpression != null && search.listHisUserRoomExpression.Count > 0)
                        {
                            foreach (var item in search.listHisUserRoomExpression)
                            {
