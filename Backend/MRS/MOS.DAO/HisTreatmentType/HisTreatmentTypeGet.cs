...
{
    partial class HisTreatmentTypeGet : EntityBase
    {
        public List<HIS_TREATMENT_TYPE> Get(HisTreatmentTypeSO search, CommonParam param)
        {
            List<HIS_TREATMENT_TYPE> list = new List<HIS_TREATMENT_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_TREATMENT_TYPE.AsNoTracking().AsQueryable();
                        if (search.listHisTreatmentTypeExpression != null && search.listHisTreatmentTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisTreatmentTypeExpression)
                            {
        ...
        public HIS_TREATMENT_TYPE GetById(long id, HisTreatmentTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_TREATMENT_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisTreatmentTypeExpression != null && search.listHisTreatmentTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisTreatmentTypeExpression)
                            {
