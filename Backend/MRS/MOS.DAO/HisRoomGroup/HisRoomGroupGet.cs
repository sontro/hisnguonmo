...
{
    partial class HisRoomGroupGet : EntityBase
    {
        public List<HIS_ROOM_GROUP> Get(HisRoomGroupSO search, CommonParam param)
        {
            List<HIS_ROOM_GROUP> list = new List<HIS_ROOM_GROUP>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_ROOM_GROUP.AsNoTracking().AsQueryable();
                        if (search.listHisRoomGroupExpression != null && search.listHisRoomGroupExpression.Count > 0)
                        {
                            foreach (var item in search.listHisRoomGroupExpression)
                            {
        ...
        public HIS_ROOM_GROUP GetById(long id, HisRoomGroupSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_ROOM_GROUP.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisRoomGroupExpression != null && search.listHisRoomGroupExpression.Count > 0)
                        {
                            foreach (var item in search.listHisRoomGroupExpression)
                            {
