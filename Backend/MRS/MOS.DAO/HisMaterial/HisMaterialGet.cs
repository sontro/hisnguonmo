...
namespace MOS.DAO.HisMaterial
{
    partial class HisMaterialGet : EntityBase
    {
        ...
        {
            List<HIS_MATERIAL> list = new List<HIS_MATERIAL>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MATERIAL.AsNoTracking().AsQueryable();
                        if (search.listHisMaterialExpression != null && search.listHisMaterialExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMaterialExpression)
                            {
        ...
        public HIS_MATERIAL GetById(long id, HisMaterialSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_MATERIAL.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMaterialExpression != null && search.listHisMaterialExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMaterialExpression)
                            {
