...
namespace MOS.DAO.HisAccidentPoison
{
    partial class HisAccidentPoisonGet : EntityBase
    {
        ...
        {
            List<HIS_ACCIDENT_POISON> list = new List<HIS_ACCIDENT_POISON>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_ACCIDENT_POISON.AsNoTracking().AsQueryable();
        ...
        public HIS_ACCIDENT_POISON GetById(long id, HisAccidentPoisonSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_ACCIDENT_POISON.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisAccidentPoisonExpression != null && search.listHisAccidentPoisonExpression.Count > 0)
                        {
                            foreach (var item in search.listHisAccidentPoisonExpression)
                            {
