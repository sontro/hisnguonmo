...
namespace MOS.DAO.HisInfusion
{
    partial class HisInfusionGet : EntityBase
    {
        ...
        {
            List<HIS_INFUSION> list = new List<HIS_INFUSION>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_INFUSION.AsNoTracking().AsQueryable();
                        if (search.listHisInfusionExpression != null && search.listHisInfusionExpression.Count > 0)
                        {
                            foreach (var item in search.listHisInfusionExpression)
                            {
        ...
        public HIS_INFUSION GetById(long id, HisInfusionSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_INFUSION.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisInfusionExpression != null && search.listHisInfusionExpression.Count > 0)
                        {
                            foreach (var item in search.listHisInfusionExpression)
                            {
