...
namespace MOS.DAO.HisCareDetail
{
    partial class HisCareDetailGet : EntityBase
    {
        ...
        {
            List<HIS_CARE_DETAIL> list = new List<HIS_CARE_DETAIL>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_CARE_DETAIL.AsNoTracking().AsQueryable();
                        if (search.listHisCareDetailExpression != null && search.listHisCareDetailExpression.Count > 0)
                        {
                            foreach (var item in search.listHisCareDetailExpression)
                            {
        ...
        public HIS_CARE_DETAIL GetById(long id, HisCareDetailSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_CARE_DETAIL.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisCareDetailExpression != null && search.listHisCareDetailExpression.Count > 0)
                        {
                            foreach (var item in search.listHisCareDetailExpression)
                            {
