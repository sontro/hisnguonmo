...
namespace MOS.DAO.HisProgram
{
    partial class HisProgramGet : EntityBase
    {
        ...
        {
            List<HIS_PROGRAM> list = new List<HIS_PROGRAM>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_PROGRAM.AsNoTracking().AsQueryable();
                        if (search.listHisProgramExpression != null && search.listHisProgramExpression.Count > 0)
                        {
                            foreach (var item in search.listHisProgramExpression)
                            {
        ...
        public HIS_PROGRAM GetById(long id, HisProgramSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_PROGRAM.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisProgramExpression != null && search.listHisProgramExpression.Count > 0)
                        {
                            foreach (var item in search.listHisProgramExpression)
                            {
