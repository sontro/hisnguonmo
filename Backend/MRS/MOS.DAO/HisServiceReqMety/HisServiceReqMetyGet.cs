...
namespace MOS.DAO.HisServiceReqMety
{
    partial class HisServiceReqMetyGet : EntityBase
    {
        ...
        {
            List<HIS_SERVICE_REQ_METY> list = new List<HIS_SERVICE_REQ_METY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SERVICE_REQ_METY.AsNoTracking().AsQueryable();
        ...
        public HIS_SERVICE_REQ_METY GetById(long id, HisServiceReqMetySO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_SERVICE_REQ_METY.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisServiceReqMetyExpression != null && search.listHisServiceReqMetyExpression.Count > 0)
                        {
                            foreach (var item in search.listHisServiceReqMetyExpression)
                            {
