...
namespace MOS.DAO.HisCacheMonitor
{
    partial class HisCacheMonitorGet : EntityBase
    {
        ...
        {
            List<HIS_CACHE_MONITOR> list = new List<HIS_CACHE_MONITOR>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_CACHE_MONITOR.AsNoTracking().AsQueryable();
        ...
        public HIS_CACHE_MONITOR GetById(long id, HisCacheMonitorSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_CACHE_MONITOR.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisCacheMonitorExpression != null && search.listHisCacheMonitorExpression.Count > 0)
                        {
                            foreach (var item in search.listHisCacheMonitorExpression)
                            {
