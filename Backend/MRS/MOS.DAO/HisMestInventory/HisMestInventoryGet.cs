...
namespace MOS.DAO.HisMestInventory
{
    partial class HisMestInventoryGet : EntityBase
    {
        ...
        {
            List<HIS_MEST_INVENTORY> list = new List<HIS_MEST_INVENTORY>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MEST_INVENTORY.AsNoTracking().AsQueryable();
        ...
        public HIS_MEST_INVENTORY GetById(long id, HisMestInventorySO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_MEST_INVENTORY.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMestInventoryExpression != null && search.listHisMestInventoryExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMestInventoryExpression)
                            {
