...
namespace MOS.DAO.HisIcdService
{
    partial class HisIcdServiceGet : EntityBase
    {
        ...
        {
            List<HIS_ICD_SERVICE> list = new List<HIS_ICD_SERVICE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_ICD_SERVICE.AsNoTracking().AsQueryable();
                        if (search.listHisIcdServiceExpression != null && search.listHisIcdServiceExpression.Count > 0)
                        {
                            foreach (var item in search.listHisIcdServiceExpression)
                            {
        ...
        public HIS_ICD_SERVICE GetById(long id, HisIcdServiceSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_ICD_SERVICE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisIcdServiceExpression != null && search.listHisIcdServiceExpression.Count > 0)
                        {
                            foreach (var item in search.listHisIcdServiceExpression)
                            {
