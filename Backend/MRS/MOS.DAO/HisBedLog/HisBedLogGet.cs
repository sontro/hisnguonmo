...
namespace MOS.DAO.HisBedLog
{
    partial class HisBedLogGet : EntityBase
    {
        ...
        {
            List<HIS_BED_LOG> list = new List<HIS_BED_LOG>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_BED_LOG.AsNoTracking().AsQueryable();
                        if (search.listHisBedLogExpression != null && search.listHisBedLogExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBedLogExpression)
                            {
        ...
        public HIS_BED_LOG GetById(long id, HisBedLogSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_BED_LOG.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisBedLogExpression != null && search.listHisBedLogExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBedLogExpression)
                            {
