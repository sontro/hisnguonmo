...
{
    partial class HisBloodGet : EntityBase
    {
        public List<HIS_BLOOD> Get(HisBloodSO search, CommonParam param)
        {
            List<HIS_BLOOD> list = new List<HIS_BLOOD>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_BLOOD.AsNoTracking().AsQueryable();
                        if (search.listHisBloodExpression != null && search.listHisBloodExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBloodExpression)
                            {
        ...
        public HIS_BLOOD GetById(long id, HisBloodSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_BLOOD.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisBloodExpression != null && search.listHisBloodExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBloodExpression)
                            {
