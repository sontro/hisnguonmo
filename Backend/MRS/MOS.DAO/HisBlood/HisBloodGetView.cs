...
namespace MOS.DAO.HisBlood
{
    partial class HisBloodGet : EntityBase
    {
        ...
        {
            List<V_HIS_BLOOD> list = new List<V_HIS_BLOOD>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_BLOOD.AsNoTracking().AsQueryable();
                        if (search.listVHisBloodExpression != null && search.listVHisBloodExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisBloodExpression)
                            {
