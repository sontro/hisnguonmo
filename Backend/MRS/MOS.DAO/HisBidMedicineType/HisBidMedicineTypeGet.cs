...
namespace MOS.DAO.HisBidMedicineType
{
    partial class HisBidMedicineTypeGet : EntityBase
    {
        ...
        {
            List<HIS_BID_MEDICINE_TYPE> list = new List<HIS_BID_MEDICINE_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_BID_MEDICINE_TYPE.AsNoTracking().AsQueryable();
        ...
        public HIS_BID_MEDICINE_TYPE GetById(long id, HisBidMedicineTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_BID_MEDICINE_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisBidMedicineTypeExpression != null && search.listHisBidMedicineTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBidMedicineTypeExpression)
                            {
