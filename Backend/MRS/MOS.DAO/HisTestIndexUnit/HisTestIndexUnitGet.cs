...
namespace MOS.DAO.HisTestIndexUnit
{
    partial class HisTestIndexUnitGet : EntityBase
    {
        ...
        {
            List<HIS_TEST_INDEX_UNIT> list = new List<HIS_TEST_INDEX_UNIT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_TEST_INDEX_UNIT.AsNoTracking().AsQueryable();
        ...
        public HIS_TEST_INDEX_UNIT GetById(long id, HisTestIndexUnitSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_TEST_INDEX_UNIT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisTestIndexUnitExpression != null && search.listHisTestIndexUnitExpression.Count > 0)
                        {
                            foreach (var item in search.listHisTestIndexUnitExpression)
                            {
