...
namespace MOS.DAO.HisBillFund
{
    partial class HisBillFundGet : EntityBase
    {
        ...
        {
            List<HIS_BILL_FUND> list = new List<HIS_BILL_FUND>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_BILL_FUND.AsNoTracking().AsQueryable();
                        if (search.listHisBillFundExpression != null && search.listHisBillFundExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBillFundExpression)
                            {
        ...
        public HIS_BILL_FUND GetById(long id, HisBillFundSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_BILL_FUND.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisBillFundExpression != null && search.listHisBillFundExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBillFundExpression)
                            {
