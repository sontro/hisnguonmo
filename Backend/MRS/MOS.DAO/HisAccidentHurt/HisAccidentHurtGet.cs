...
namespace MOS.DAO.HisAccidentHurt
{
    partial class HisAccidentHurtGet : EntityBase
    {
        ...
        {
            List<HIS_ACCIDENT_HURT> list = new List<HIS_ACCIDENT_HURT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_ACCIDENT_HURT.AsNoTracking().AsQueryable();
                        if (search.listHisAccidentHurtExpression != null && search.listHisAccidentHurtExpression.Count > 0)
                        {
                            foreach (var item in search.listHisAccidentHurtExpression)
                            {
        ...
        public HIS_ACCIDENT_HURT GetById(long id, HisAccidentHurtSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_ACCIDENT_HURT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisAccidentHurtExpression != null && search.listHisAccidentHurtExpression.Count > 0)
                        {
                            foreach (var item in search.listHisAccidentHurtExpression)
                            {
