...
namespace MOS.DAO.HisSereServDeposit
{
    partial class HisSereServDepositGet : EntityBase
    {
        ...
        {
            List<V_HIS_SERE_SERV_DEPOSIT> list = new List<V_HIS_SERE_SERV_DEPOSIT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_SERE_SERV_DEPOSIT.AsQueryable();
                        if (search.listVHisSereServDepositExpression != null && search.listVHisSereServDepositExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisSereServDepositExpression)
                            {
