...
namespace MOS.DAO.HisDepartmentTran
{
    partial class HisDepartmentTranGet : EntityBase
    {
        ...
        {
            List<HIS_DEPARTMENT_TRAN> list = new List<HIS_DEPARTMENT_TRAN>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_DEPARTMENT_TRAN.AsNoTracking().AsQueryable();
        ...
        public HIS_DEPARTMENT_TRAN GetById(long id, HisDepartmentTranSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_DEPARTMENT_TRAN.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisDepartmentTranExpression != null && search.listHisDepartmentTranExpression.Count > 0)
                        {
                            foreach (var item in search.listHisDepartmentTranExpression)
                            {
