...
namespace MOS.DAO.HisExecuteRoleUser
{
    partial class HisExecuteRoleUserGet : EntityBase
    {
        ...
        {
            List<HIS_EXECUTE_ROLE_USER> list = new List<HIS_EXECUTE_ROLE_USER>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_EXECUTE_ROLE_USER.AsNoTracking().AsQueryable();
        ...
        public HIS_EXECUTE_ROLE_USER GetById(long id, HisExecuteRoleUserSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_EXECUTE_ROLE_USER.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisExecuteRoleUserExpression != null && search.listHisExecuteRoleUserExpression.Count > 0)
                        {
                            foreach (var item in search.listHisExecuteRoleUserExpression)
                            {
