...
namespace MOS.DAO.HisMestInveUser
{
    partial class HisMestInveUserGet : EntityBase
    {
        ...
        {
            List<HIS_MEST_INVE_USER> list = new List<HIS_MEST_INVE_USER>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MEST_INVE_USER.AsNoTracking().AsQueryable();
                        if (search.listHisMestInveUserExpression != null && search.listHisMestInveUserExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMestInveUserExpression)
                            {
        ...
        public HIS_MEST_INVE_USER GetById(long id, HisMestInveUserSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_MEST_INVE_USER.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMestInveUserExpression != null && search.listHisMestInveUserExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMestInveUserExpression)
                            {
