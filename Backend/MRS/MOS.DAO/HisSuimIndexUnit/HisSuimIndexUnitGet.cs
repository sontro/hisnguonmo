...
namespace MOS.DAO.HisSuimIndexUnit
{
    partial class HisSuimIndexUnitGet : EntityBase
    {
        ...
        {
            List<HIS_SUIM_INDEX_UNIT> list = new List<HIS_SUIM_INDEX_UNIT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SUIM_INDEX_UNIT.AsNoTracking().AsQueryable();
        ...
        public HIS_SUIM_INDEX_UNIT GetById(long id, HisSuimIndexUnitSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_SUIM_INDEX_UNIT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisSuimIndexUnitExpression != null && search.listHisSuimIndexUnitExpression.Count > 0)
                        {
                            foreach (var item in search.listHisSuimIndexUnitExpression)
                            {
