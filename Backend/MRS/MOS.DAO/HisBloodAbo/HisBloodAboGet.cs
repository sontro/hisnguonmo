...
namespace MOS.DAO.HisBloodAbo
{
    partial class HisBloodAboGet : EntityBase
    {
        ...
        {
            List<HIS_BLOOD_ABO> list = new List<HIS_BLOOD_ABO>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_BLOOD_ABO.AsNoTracking().AsQueryable();
        ...
        public HIS_BLOOD_ABO GetById(long id, HisBloodAboSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_BLOOD_ABO.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisBloodAboExpression != null && search.listHisBloodAboExpression.Count > 0)
                        {
                            foreach (var item in search.listHisBloodAboExpression)
                            {
