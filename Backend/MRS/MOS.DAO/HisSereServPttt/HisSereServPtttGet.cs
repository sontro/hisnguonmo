...
namespace MOS.DAO.HisSereServPttt
{
    partial class HisSereServPtttGet : EntityBase
    {
        ...
        {
            List<HIS_SERE_SERV_PTTT> list = new List<HIS_SERE_SERV_PTTT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SERE_SERV_PTTT.AsNoTracking().AsQueryable();
        ...
        public HIS_SERE_SERV_PTTT GetById(long id, HisSereServPtttSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_SERE_SERV_PTTT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisSereServPtttExpression != null && search.listHisSereServPtttExpression.Count > 0)
                        {
                            foreach (var item in search.listHisSereServPtttExpression)
                            {
