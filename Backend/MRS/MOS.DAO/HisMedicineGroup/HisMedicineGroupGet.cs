...
{
    partial class HisMedicineGroupGet : EntityBase
    {
        public List<HIS_MEDICINE_GROUP> Get(HisMedicineGroupSO search, CommonParam param)
        {
            List<HIS_MEDICINE_GROUP> list = new List<HIS_MEDICINE_GROUP>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MEDICINE_GROUP.AsNoTracking().AsQueryable();
                        if (search.listHisMedicineGroupExpression != null && search.listHisMedicineGroupExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMedicineGroupExpression)
                            {
        ...
        public HIS_MEDICINE_GROUP GetById(long id, HisMedicineGroupSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_MEDICINE_GROUP.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMedicineGroupExpression != null && search.listHisMedicineGroupExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMedicineGroupExpression)
                            {
