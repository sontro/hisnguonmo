...
namespace MOS.DAO.HisAccidentHelmet
{
    partial class HisAccidentHelmetGet : EntityBase
    {
        ...
        {
            List<HIS_ACCIDENT_HELMET> list = new List<HIS_ACCIDENT_HELMET>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_ACCIDENT_HELMET.AsNoTracking().AsQueryable();
        ...
        public HIS_ACCIDENT_HELMET GetById(long id, HisAccidentHelmetSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_ACCIDENT_HELMET.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisAccidentHelmetExpression != null && search.listHisAccidentHelmetExpression.Count > 0)
                        {
                            foreach (var item in search.listHisAccidentHelmetExpression)
                            {
