...
namespace MOS.DAO.HisAwareness
{
    partial class HisAwarenessGet : EntityBase
    {
        ...
        {
            List<HIS_AWARENESS> list = new List<HIS_AWARENESS>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_AWARENESS.AsNoTracking().AsQueryable();
        ...
        public HIS_AWARENESS GetById(long id, HisAwarenessSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_AWARENESS.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisAwarenessExpression != null && search.listHisAwarenessExpression.Count > 0)
                        {
                            foreach (var item in search.listHisAwarenessExpression)
                            {
