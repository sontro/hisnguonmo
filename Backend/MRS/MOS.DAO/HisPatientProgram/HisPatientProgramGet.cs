...
namespace MOS.DAO.HisPatientProgram
{
    partial class HisPatientProgramGet : EntityBase
    {
        ...
        {
            List<HIS_PATIENT_PROGRAM> list = new List<HIS_PATIENT_PROGRAM>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_PATIENT_PROGRAM.AsNoTracking().AsQueryable();
                        if (search.listHisPatientProgramExpression != null && search.listHisPatientProgramExpression.Count > 0)
                        {
                            foreach (var item in search.listHisPatientProgramExpression)
                            {
        ...
        public HIS_PATIENT_PROGRAM GetById(long id, HisPatientProgramSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_PATIENT_PROGRAM.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisPatientProgramExpression != null && search.listHisPatientProgramExpression.Count > 0)
                        {
                            foreach (var item in search.listHisPatientProgramExpression)
                            {
