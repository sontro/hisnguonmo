...
namespace MOS.DAO.HisServSegr
{
    partial class HisServSegrGet : EntityBase
    {
        ...
        {
            List<V_HIS_SERV_SEGR> list = new List<V_HIS_SERV_SEGR>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_SERV_SEGR.AsNoTracking().AsQueryable();
                        if (search.listVHisServSegrExpression != null && search.listVHisServSegrExpression.Count > 0)
                        {
                            foreach (var item in search.listVHisServSegrExpression)
                            {
