...
namespace MOS.DAO.HisServSegr
{
    partial class HisServSegrGet : EntityBase
    {
        ...
        {
            List<HIS_SERV_SEGR> list = new List<HIS_SERV_SEGR>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_SERV_SEGR.AsNoTracking().AsQueryable();
        ...
        public HIS_SERV_SEGR GetById(long id, HisServSegrSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_SERV_SEGR.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisServSegrExpression != null && search.listHisServSegrExpression.Count > 0)
                        {
                            foreach (var item in search.listHisServSegrExpression)
                            {
