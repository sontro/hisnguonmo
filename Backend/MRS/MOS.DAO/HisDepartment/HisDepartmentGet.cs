...
namespace MOS.DAO.HisDepartment
{
    partial class HisDepartmentGet : EntityBase
    {
        ...
        {
            List<HIS_DEPARTMENT> list = new List<HIS_DEPARTMENT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_DEPARTMENT.AsNoTracking().AsQueryable();
        ...
        public HIS_DEPARTMENT GetById(long id, HisDepartmentSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_DEPARTMENT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisDepartmentExpression != null && search.listHisDepartmentExpression.Count > 0)
                        {
                            foreach (var item in search.listHisDepartmentExpression)
                            {
