...
namespace MOS.DAO.HisMedicineBean
{
    partial class HisMedicineBeanGet : EntityBase
    {
        ...
        {
            List<HIS_MEDICINE_BEAN> list = new List<HIS_MEDICINE_BEAN>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_MEDICINE_BEAN.AsNoTracking().AsQueryable();
                        if (search.listHisMedicineBeanExpression != null && search.listHisMedicineBeanExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMedicineBeanExpression)
                            {
        ...
        public HIS_MEDICINE_BEAN GetById(long id, HisMedicineBeanSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_MEDICINE_BEAN.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisMedicineBeanExpression != null && search.listHisMedicineBeanExpression.Count > 0)
                        {
                            foreach (var item in search.listHisMedicineBeanExpression)
                            {
