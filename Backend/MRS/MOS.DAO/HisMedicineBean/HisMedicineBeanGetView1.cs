...
namespace MOS.DAO.HisMedicineBean
{
    partial class HisMedicineBeanGet : EntityBase
    {
        ...
        {
            List<V_HIS_MEDICINE_BEAN_1> list = new List<V_HIS_MEDICINE_BEAN_1>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_HIS_MEDICINE_BEAN_1.AsNoTracking().AsQueryable();
                        if (search.listVHisMedicineBean1Expression != null && search.listVHisMedicineBean1Expression.Count > 0)
                        {
                            foreach (var item in search.listVHisMedicineBean1Expression)
                            {
