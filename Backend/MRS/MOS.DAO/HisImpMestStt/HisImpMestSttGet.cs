...
namespace MOS.DAO.HisImpMestStt
{
    partial class HisImpMestSttGet : EntityBase
    {
        ...
        {
            List<HIS_IMP_MEST_STT> list = new List<HIS_IMP_MEST_STT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_IMP_MEST_STT.AsNoTracking().AsQueryable();
        ...
        public HIS_IMP_MEST_STT GetById(long id, HisImpMestSttSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_IMP_MEST_STT.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisImpMestSttExpression != null && search.listHisImpMestSttExpression.Count > 0)
                        {
                            foreach (var item in search.listHisImpMestSttExpression)
                            {
