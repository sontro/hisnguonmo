...
{
    partial class HisIcdGroupGet : EntityBase
    {
        public List<HIS_ICD_GROUP> Get(HisIcdGroupSO search, CommonParam param)
        {
            List<HIS_ICD_GROUP> list = new List<HIS_ICD_GROUP>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_ICD_GROUP.AsNoTracking().AsQueryable();
                        if (search.listHisIcdGroupExpression != null && search.listHisIcdGroupExpression.Count > 0)
                        {
                            foreach (var item in search.listHisIcdGroupExpression)
                            {
        ...
        public HIS_ICD_GROUP GetById(long id, HisIcdGroupSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_ICD_GROUP.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisIcdGroupExpression != null && search.listHisIcdGroupExpression.Count > 0)
                        {
                            foreach (var item in search.listHisIcdGroupExpression)
                            {
