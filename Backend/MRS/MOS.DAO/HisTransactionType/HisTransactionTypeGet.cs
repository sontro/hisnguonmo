...
namespace MOS.DAO.HisTransactionType
{
    partial class HisTransactionTypeGet : EntityBase
    {
        ...
        {
            List<HIS_TRANSACTION_TYPE> list = new List<HIS_TRANSACTION_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.HIS_TRANSACTION_TYPE.AsNoTracking().AsQueryable();
                        if (search.listHisTransactionTypeExpression != null && search.listHisTransactionTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisTransactionTypeExpression)
                            {
        ...
        public HIS_TRANSACTION_TYPE GetById(long id, HisTransactionTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new MOS.DAO.Base.AppContext())
                    {
                        var query = ctx.HIS_TRANSACTION_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listHisTransactionTypeExpression != null && search.listHisTransactionTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listHisTransactionTypeExpression)
                            {
