...
{
    public class Mrs00244Processor : AbstractProcessor
    {
        Mrs00244Filter castFilter = null;

        List<Mrs00244RDO> ListRdoA = new List<Mrs00244RDO>();
        List<Mrs00244RDO> ListRdoB = new List<Mrs00244RDO>();
        List<Mrs00244RDO> ListRdoC = new List<Mrs00244RDO>();

        Dictionary<short, Mrs00244RDO> dicRdoA = new Dictionary<short, Mrs00244RDO>();
        Dictionary<short, Mrs00244RDO> dicRdoB = new Dictionary<short, Mrs00244RDO>();
        Dictionary<short, Mrs00244RDO> dicRdoC = new Dictionary<short, Mrs00244RDO>();

        List<Mrs00244RDO> ListTotalRdoA = new List<Mrs00244RDO>();
        List<Mrs00244RDO> ListTotalRdoB = new List<Mrs00244RDO>();
        List<Mrs00244RDO> ListTotalRdoC = new List<Mrs00244RDO>();
        ...
        public Mrs00244Processor(CommonParam param, string reportTypeCode)
		...
		protected override bool GetData()
		{
            ...
            try
            {
                castFilter = ((Mrs00244Filter)this.reportFilter);
                ...
                Inventec.Common.Logging.LogSystem.Debug("Bat dau lay du lieu V_HIS_HEIN_APPROVAL, Mrs00244, filter: " + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => castFilter), castFilter));
                ...
                {
                    throw new DataMisalignedException("Co loi xay ra trong qua trinh lay du lieu V_HIS_HEIN_APPROVAL, Mrs00244");
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                if (IsNotNullOrEmpty(ListHeinApproval))
                {
                    ...
                    while (count > 0)
                    {
                        ...
                        if (paramGet.HasException)
                        {
                            throw new DataMisalignedException("Co exception xay ra tai DAOGET trong qua trinh tong hop du lieu Mrs00244.");
                        }
                    ...
                    foreach (var heinApproval in hisHeinApprovals)
                    {
                        if (heinApproval.HEIN_TREATMENT_TYPE_CODE == MOS.LibraryHein.Bhyt.HeinTreatmentType.HeinTreatmentTypeCode.EXAM && CheckHeinCardNumberType(heinApproval.HEIN_CARD_NUMBER))
                        {
                            ...
                            if (this._Branch.ACCEPT_HEIN_MEDI_ORG_CODE.Contains(heinApproval.HEIN_MEDI_ORG_CODE))
                            {
                                ...
                                {
                                    rdo = new Mrs00244RDO(heinApproval);
        ...
        private void ProcessTotalPrice(Mrs00244RDO rdo, List<V_HIS_SERE_SERV_3> hisSereServs, V_HIS_HEIN_APPROVAL heinApproval, V_HIS_TREATMENT treatment)
        {
            try
            {
                if (IsNotNullOrEmpty(hisSereServs))
                {
                    ...
                    foreach (var sereServ in hisSereServs)
                    {
                        ...
                        if (sereServ.TDL_HEIN_SERVICE_TYPE_ID != null)
                        {
                            ...
                            else if (sereServ.TDL_HEIN_SERVICE_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_HEIN_SERVICE_TYPE.ID__GI_NT || sereServ.TDL_HEIN_SERVICE_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_HEIN_SERVICE_TYPE.ID__GI_NGT)
                            {
        ...
        private List<Mrs00244RDO> CheckList(List<Mrs00244RDO> listRdo)
        {
            List<Mrs00244RDO> result = new List<Mrs00244RDO>();
            try
            {
            ...
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
                result = new List<Mrs00244RDO>();
            }
            return result;
        }

        private bool CheckPrice(Mrs00244RDO rdo)
        {
            bool result = false;
            try
            {
        ...
        private void ProcessSumTotal()
        {
            try
            {
                Mrs00244RDO rdoA = new Mrs00244RDO();
                ...
                Mrs00244RDO rdoB = new Mrs00244RDO();
                ...
                Mrs00244RDO rdoC = new Mrs00244RDO();
