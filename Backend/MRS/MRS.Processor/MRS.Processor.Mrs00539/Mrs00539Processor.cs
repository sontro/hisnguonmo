...
{
    public class Mrs00539Processor : AbstractProcessor
    {
        List<Mrs00539RDO> ListRdo = new List<Mrs00539RDO>();
        ...
        Mrs00539Filter castFilter = null;
        string MaterialPriceOption = "";

        HIS_BRANCH _Branch = null;

        public Mrs00539Processor(CommonParam param, string reportTypeCode)
		...
		protected override bool GetData()
		{
            ...
            try
            {
                castFilter = ((Mrs00539Filter)this.reportFilter);
                ...
                {
                    throw new DataMisalignedException("Co loi xay ra trong qua trinh lay du lieu V_HIS_HEIN_APPROVAL, Mrs00539");
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                if (IsNotNullOrEmpty(ListHeinApproval))
                {
                    ...
                    while (count > 0)
                    {
                        ...
                        if (paramGet.HasException)
                        {
                            throw new DataMisalignedException("Co exception xay ra tai DAOGET trong qua trinh tong hop du lieu Mrs00539.");
                        }
                    ...
                    foreach (var heinApproval in hisHeinApprovals)
                    {
                        if (heinApproval.HEIN_TREATMENT_TYPE_CODE == MOS.LibraryHein.Bhyt.HeinTreatmentType.HeinTreatmentTypeCode.EXAM && CheckHeinCardNumberType(heinApproval.HEIN_CARD_NUMBER))
                        {
                            ...
                            if (dicSereServ.ContainsKey(heinApproval.ID))
                            {
                                ...
                                {
                                    Mrs00539RDO rdo = new Mrs00539RDO(heinApproval);
        ...
        private void ProcessTotalPrice(Mrs00539RDO rdo, List<V_HIS_SERE_SERV_3> hisSereServs)
        {
            try
            {
        ...
        private bool CheckPrice(Mrs00539RDO rdo)
        {
            bool result = false;
            try
            {
        ...
        private bool checkBhytNsd(Mrs00539RDO rdo)
        {
            bool result = false;
            try
            {
