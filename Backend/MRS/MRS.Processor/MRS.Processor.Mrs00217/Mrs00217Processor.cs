...
{
    public class Mrs00217Processor : AbstractProcessor
    {
        Mrs00217Filter castFilter = null;
        List<Mrs00217RDO> ListRdo = new List<Mrs00217RDO>();
        ...
        public Mrs00217Processor(CommonParam param, string reportTypeCode)
		...
		protected override bool GetData()
		{
            ...
            try
            {
                castFilter = (Mrs00217Filter)this.reportFilter;
                ...
                Inventec.Common.Logging.LogSystem.Debug("Bat dau lay du lieu V_HIS_HEIN_APPROVAL, MRS00217, filter: " + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => castFilter), castFilter));
                ...
                {
                    throw new DataMisalignedException("Co loi xay ra trong qua trinh lay du lieu V_HIS_HEIN_APPROVAL, MRS00217");
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                if (IsNotNullOrEmpty(ListHeinApproval))
                {
                    ...
                    while (count > 0)
                    {
                        ...
                        if (paramGet.HasException)
                        {
                            throw new DataMisalignedException("Co exception xay ra tai DAOGET trong qua trinh tong hop du lieu, MRS00217.");
                        }
            ...
            try
            {
                ...
                if (IsNotNullOrEmpty(ListSereServ))
                {
                    foreach (var sereServ in ListSereServ)
                    {
                        ...
                        Mrs00217RDO rdo = new Mrs00217RDO();
        ...
        private List<Mrs00217RDO> ProcessListRDO(List<Mrs00217RDO> listRDO)
        {
            List<Mrs00217RDO> listCurrent = new List<Mrs00217RDO>();
            try
            {
                if (listRDO.Count > 0)
                {
                    ...
                    {
                        List<Mrs00217RDO> listsub = group.ToList<Mrs00217RDO>();
