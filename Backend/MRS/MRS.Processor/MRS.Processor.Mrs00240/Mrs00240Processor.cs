...
{
    public class Mrs00240Processor : AbstractProcessor
    {
        List<Mrs00240RDO> ListRdo = new List<Mrs00240RDO>();
        List<V_HIS_HEIN_APPROVAL> ListHeinApproval = new List<V_HIS_HEIN_APPROVAL>();

        Mrs00240Filter castFilter = null;

        HIS_BRANCH _Branch = null;
        string MaterialPriceOption = "";

        public Mrs00240Processor(CommonParam param, string reportTypeCode)
		...
		protected override bool GetData()
		{
            ...
            try
            {
                ...
                castFilter = ((Mrs00240Filter)this.reportFilter);
                ...
                Inventec.Common.Logging.LogSystem.Debug("Bat dau lay du lieu V_HIS_HEIN_APPROVAL, Mrs00240, filter: " + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => castFilter), castFilter));
                ...
                {
                    throw new DataMisalignedException("Co loi xay ra trong qua trinh lay du lieu V_HIS_HEIN_APPROVAL, Mrs00240");
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                if (IsNotNullOrEmpty(ListHeinApproval))
                {
                    ...
                    while (count > 0)
                    {
                        ...
                        if (paramGet.HasException)
                        {
                            throw new DataMisalignedException("Co exception xay ra tai DAOGET trong qua trinh tong hop du lieu Mrs00240.");
                        }
            ...
            try
            {
                if (IsNotNullOrEmpty(hisHeinApprovals))
                {
                    ...
                    foreach (var heinApproval in hisHeinApprovals)
                    {
                        ...
                        {
                            Mrs00240RDO rdo = new Mrs00240RDO(heinApproval);
        ...
        private void ProcessTotalPrice(Mrs00240RDO rdo, List<V_HIS_SERE_SERV_3> hisSereServs)
        {
            try
            {
        ...
        private bool CheckPrice(Mrs00240RDO rdo)
        {
            bool result = false;
            try
            {
        ...
        private bool checkBhytNsd(Mrs00240RDO rdo)
        {
            bool result = false;
            try
            {
