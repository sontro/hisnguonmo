...
namespace MRS.Processor.Mrs00294
{
    public class Mrs00294Processor : AbstractProcessor
    {
        List<Mrs00294RDO> ListRdoA = new List<Mrs00294RDO>();
        List<Mrs00294RDO> ListRdoB = new List<Mrs00294RDO>();
        List<Mrs00294RDO> ListRdoC = new List<Mrs00294RDO>();

        List<Mrs00294RDO> ListSumTotal = new List<Mrs00294RDO>();
        Mrs00294Filter castFilter = new Mrs00294Filter();
        private decimal TotalAmount = 0;
        bool IsBillFood = false;
        List<V_HIS_HEIN_APPROVAL> ListHeinApproval = new List<V_HIS_HEIN_APPROVAL>();

        HIS_BRANCH _Branch = null;

        public Mrs00294Processor(CommonParam param, string reportTypeCode)
		...
		protected override bool GetData()
		{
            ...
            try
            {
                castFilter = ((Mrs00294Filter)this.reportFilter);
            ...
            try
            {
                if (IsNotNullOrEmpty(ListHeinApproval))
                {
                    ...
                    while (count > 0)
                    {
                        ...
                        PatientTypeAlterViewFilter.TREATMENT_IDs = hisHeinApprovals.Select(s => s.TREATMENT_ID).ToList().Distinct().ToList();
                        List<V_HIS_PATIENT_TYPE_ALTER> ListPatyAlterBhyt = new HisPatientTypeAlterManager(paramGet).GetView(PatientTypeAlterViewFilter);

                        if (paramGet.HasException)
                        {
                            throw new DataMisalignedException("Co exception xay ra tai DAOGET trong qua trinh tong hop du lieu MRS00021.");
                        }
                        GeneralDataByListHeinApproval(hisHeinApprovals, ListSereServ, ListTreatment, ListPatyAlterBhyt);
        ...
        private void GeneralDataByListHeinApproval(List<V_HIS_HEIN_APPROVAL> hisHeinApprovals, List<V_HIS_SERE_SERV_3> ListSereServ, List<V_HIS_TREATMENT> ListTreatment, List<V_HIS_PATIENT_TYPE_ALTER> ListPatientTypeAlter)
        {
            try
            {
                if (IsNotNullOrEmpty(hisHeinApprovals))
                {
                    ...
                    if (IsNotNullOrEmpty(ListPatientTypeAlter))
                    {
                        ...
                        {
                            if (!dicPatientTypeAlter.ContainsKey(patientTypeAlter.TREATMENT_ID)) dicPatientTypeAlter[patientTypeAlter.TREATMENT_ID] = patientTypeAlter;
                        }
                    }
                    ...
                    foreach (var heinApproval in hisHeinApprovals)
                    {
                        ...
                        {
                            IsBillFood = CheckHeinCardNumberType(heinApproval.HEIN_CARD_NUMBER);
                            Mrs00294RDO rdo = new Mrs00294RDO(heinApproval);
                            ...
                            {
                                ProcessTotalPrice(rdo, dicSereServHein[heinApproval.ID], dicPatientTypeAlter);
                            }

                            if (IsBillFood && rdo.HNCODE != null)
                                rdo.TOTAL_PRICE_BILL_FOOD = rdo.TOTAL_DATE * 35000 ?? 0;
                        }
                    }
                }
            }
        ...
        private void ProcessTotalPrice(Mrs00294RDO rdo, List<V_HIS_SERE_SERV_3> hisSereServs, Dictionary<long, V_HIS_PATIENT_TYPE_ALTER> dicPatientTypeAlter)
        {
            try
            {
                foreach (var sereServ in hisSereServs)
                {
                    ...
                    rdo.HNCODE = dicPatientTypeAlter.ContainsKey(sereServ.TDL_TREATMENT_ID ?? 0) ? dicPatientTypeAlter[sereServ.TDL_TREATMENT_ID ?? 0].HNCODE : "";

                    //if (sereServ.TRANSACTION_CODE != null) rdo.NUM_OUT = sereServ.TRANSACTION_CODE; 
        ...
        private bool CheckPrice(Mrs00294RDO rdo)
        {
            bool result = false;
            try
            {
        ...
        private bool CheckHeinCardNumberType(string HeinCardNumber)
        {
            ...
            try
            {
                ...
                {
                    if (IsNotNullOrEmpty(MANAGER.Config.HeinCardNumberTypeCFG.HeinCardNumber__HeinType__Bill_Food))
                    {
                        foreach (var type in MANAGER.Config.HeinCardNumberTypeCFG.HeinCardNumber__HeinType__Bill_Food)
                        {
        ...
        private bool checkBhytNsd(Mrs00294RDO rdo)
        {
            bool result = false;
            try
            {
        ...
        private void ProcessSumTotal()
        {
            try
            {
                Mrs00294RDO rdo = new Mrs00294RDO();
