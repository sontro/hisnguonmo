...
namespace MRS.Processor.Mrs00294
{
    public class Mrs00294RDO
    {
        ...
        public Mrs00294RDO(MOS.EFMODEL.DataModels.V_HIS_HEIN_APPROVAL heinApproval)
        {
        ...
        {
            if (this.HEIN_APPROVAL_BHYT.TDL_PATIENT_DOB > 0)
            {
                try
                {
                    if (this.HEIN_APPROVAL_BHYT.TDL_PATIENT_GENDER_ID == IMSys.DbConfig.HIS_RS.HIS_GENDER.ID__MALE)
                    {
                        this.DOB_MALE = long.Parse((this.HEIN_APPROVAL_BHYT.TDL_PATIENT_DOB.ToString()).Substring(0, 4));
                    }
                    else if (this.HEIN_APPROVAL_BHYT.TDL_PATIENT_GENDER_ID == IMSys.DbConfig.HIS_RS.HIS_GENDER.ID__FEMALE)
                    {
                        this.DOB_FEMALE = long.Parse((this.HEIN_APPROVAL_BHYT.TDL_PATIENT_DOB.ToString()).Substring(0, 4));
                    }
                }
                catch (Exception ex)
                {
