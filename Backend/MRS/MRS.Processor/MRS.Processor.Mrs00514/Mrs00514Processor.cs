...
{
    public class Mrs00514Processor : AbstractProcessor
    {
        private List<Mrs00514RDO> ListRdo = new List<Mrs00514RDO>();
        private List<Mrs00514RDO> ListRdoDepartment = new List<Mrs00514RDO>();
        private List<Mrs00514RDO> ListRdoTreatmentRequestRoom = new List<Mrs00514RDO>();
        private List<Mrs00514RDO> ListRdoTreatmentExecuteRoom = new List<Mrs00514RDO>();
        private List<Mrs00514RDO> ListRdoTreatmentBhytEndRoom = new List<Mrs00514RDO>();
        ...
        public Mrs00514Processor(CommonParam param, string reportTypeCode)
        ...
        {
            return typeof(Mrs00514Filter);
        }

        protected override bool GetData()
        {
            var result = true;
            filter = (Mrs00514Filter)this.reportFilter;
            try
            {
                //Danh sach phong
                ...
                listHisSereServ = listHisSereServ.Where(o => o.IS_NO_EXECUTE != IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__TRUE).ToList();
        ...
        protected override bool ProcessData()
        {
            ...
            try
            {
                ListRdo = (from r in listHisSereServ select new Mrs00514RDO(r, dicServiceMety, dicServiceMaty, listServiceRetyCat ?? new List<V_HIS_SERVICE_RETY_CAT>(), this.filter, Treatments, lastHisPatientTypeAlter,listHisRoom)).ToList();
        ...
        private void GroupByTreatmentRequestRoom()
        {
            ...
            try
            {
                ...
                Decimal sum = 0;
                Mrs00514RDO rdo;
                List<Mrs00514RDO> listSub;
                PropertyInfo[] pi = Properties.Get<Mrs00514RDO>();
                foreach (var item in group)
                {
                    rdo = new Mrs00514RDO();
                    listSub = item.ToList<Mrs00514RDO>();
        ...
        private void GroupByRequestRoomID()
        {
            string errorField = "";
            try
            {
                var group = ListRdo.GroupBy(o => new { o.TDL_REQUEST_ROOM_ID }).ToList();
                ListRdo.Clear();
        ...
        private Mrs00514RDO IsMeaningful(List<Mrs00514RDO> listSub, PropertyInfo field)
        {
            return listSub.Where(o => field.GetValue(o) != null && field.GetValue(o).ToString() != "").FirstOrDefault() ?? new Mrs00514RDO();
        }
        ...
        private void AddInfoExecuteRoomAndBhytEndRoom()
        {
            try
            {
                foreach (var item in ListRdo)
                {
                    ...
                    item.COUNT_TREATMENT_BHYT_END_ROOM = ListRdoTreatmentBhytEndRoom.Where(o => o.TREATMENT_BHYT_END_ROOM_ID == item.TDL_REQUEST_ROOM_ID).ToList().Count;
                }
            }
            catch (Exception ex)
            {
