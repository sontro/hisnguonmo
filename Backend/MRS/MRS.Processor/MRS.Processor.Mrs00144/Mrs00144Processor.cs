...
{
    public class Mrs00144Processor : AbstractProcessor
    {
        List<Mrs00144RDO> _listSereServRdo = new List<Mrs00144RDO>(); 
        Mrs00144Filter CastFilter; 
        ...
        public Mrs00144Processor(CommonParam param, string reportTypeCode)
            : base(param, reportTypeCode)
        { }

        public override Type FilterType()
        {
            return typeof(Mrs00144Filter); 
        }
        ...
        protected override bool GetData()
        {
            ...
            try
            {
                this.CastFilter = (Mrs00144Filter)this.reportFilter; 
                var paramGet = new CommonParam(); 
                LogSystem.Debug("Bat dau lay du lieu filter MRS00144: " +
                    LogUtil.TraceData(LogUtil.GetMemberName(() => CastFilter), CastFilter)); 
                LogSystem.Info("Bat dau lay du lieu filter MRS00144 ==============================================================="); 
                //-------------------------------------------------------------------------------------------------- Lấy thông tin Filter hiển thị lên báo cáo
                //Tên kho xuất
                ...
                EXP_MEST_STT_NAME = new MOS.MANAGER.HisExpMestStt.HisExpMestSttManager(paramGet).GetById(CastFilter.EXP_MEST_STT_IDs).EXP_MEST_STT_NAME; 
                //Loại xuất
                EXP_MEST_TYPE_NAME = new MOS.MANAGER.HisExpMestType.HisExpMestTypeManager(paramGet).GetById(CastFilter.EXP_MEST_TYPE_IDs).EXP_MEST_TYPE_NAME; 
                //-------------------------------------------------------------------------------------------------- EXP_MEST
                var metyFilterExpMest = new HisExpMestViewFilterQuery
                {
                    MEDI_STOCK_ID = CastFilter.MEDI_STOCK_ID,
                    EXP_MEST_STT_IDs = new List<long> { CastFilter.EXP_MEST_STT_IDs },//Trạng thái phiếu xuất: đã xuất
                    EXP_MEST_TYPE_IDs = new List<long> { CastFilter.EXP_MEST_TYPE_IDs }//Loại xuất kho là: Xuất cho khoa phòng (xuất sử dụng)
                }; 
                if (CastFilter.EXP_MEST_STT_IDs == IMSys.DbConfig.HIS_RS.HIS_EXP_MEST_STT.ID__DONE)
                {
                ...
                {
                    REPORT_TYPE_CODE__EXACT = "MRS00144"
                }; 
                ...
                //--------------------------------------------------------------------------------------------------
                LogSystem.Info("Ket thuc lay du lieu filter MRS00144 ==============================================================="); 
                if (!paramGet.HasException)
                {
                    ...
                    throw new DataMisalignedException(
                        "Co exception xay ra tai DAOGET trong qua trinh lay du lieu MRS00144." +
                        LogUtil.TraceData(
        ...
        protected override bool ProcessData()
        {
            ...
            try
            {
                ProcessFilterData(listExpMestMedicineViews, listMedicineTypeViews, listServiceRetyCatViews, listImpMestMedicineViews, listMedicineTypeAcinViews); 
                result = true; 
            }
            catch (Exception ex)
            {
        ...
        private void ProcessFilterData(List<V_HIS_EXP_MEST_MEDICINE> listExpMestMedicines, List<V_HIS_MEDICINE_TYPE> listMedicineTypes,
            List<V_HIS_SERVICE_RETY_CAT> listServiceRetyCats, List<V_HIS_IMP_MEST_MEDICINE> listImpMestMedicines, List<V_HIS_MEDICINE_TYPE_ACIN> listMedicineTypeAcins)
        {
            try
            {
                LogSystem.Info("Bat dau xu ly du lieu MRS00144 ==============================================================="); 
                var listServices = listServiceRetyCats.Select(s => s.SERVICE_ID).ToList(); 
                var listExpMestMedicineViews = listExpMestMedicines.Where(s => listServices.Contains(s.SERVICE_ID)).OrderBy(s => s.MEDICINE_TYPE_NAME).ToList(); 
                var listMedicineTypeIds = listExpMestMedicineViews.Select(s => s.MEDICINE_TYPE_ID).ToList(); 
                ...
                LogSystem.Info("Ket thuc xu ly du lieu MRS00144 ==============================================================="); 
            }
            catch (Exception ex)
            {
