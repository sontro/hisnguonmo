...
namespace MRS.Processor.Mrs00495
{
    class Mrs00495Processor : AbstractProcessor
    {
        Mrs00495Filter castFilter = new Mrs00495Filter();
        List<Mrs00495RDO> listRdo = new List<Mrs00495RDO>();
        List<Mrs00495RDO> listRdoParent = new List<Mrs00495RDO>();
        ...
        public Mrs00495Processor(CommonParam param, string reportTypeCode)
        ...
        protected override bool GetData()
        {
            ...
            try
            {
                this.castFilter = (Mrs00495Filter)this.reportFilter;
                CommonParam paramGet = new CommonParam();
                Inventec.Common.Logging.LogSystem.Info("Bat dau lay bao cao Mrs00495: " + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => castFilter), castFilter));
                ...
                listTreatments = TREATMENT_TYPE_ID != 1 ? (TREATMENT_TYPE_ID != 2 ? listTreatments.Where(w => w.TDL_HEIN_CARD_NUMBER != null).ToList() : listTreatments.Where(w => w.TDL_HEIN_CARD_NUMBER != null && !ICD_CODE__CAPDs.Contains(w.ICD_CODE)).ToList()) : listTreatments.Where(w => w.TDL_HEIN_CARD_NUMBER != null && ICD_CODE__CAPDs.Contains(w.ICD_CODE)).ToList();
        ...
        protected override bool ProcessData()
        {
            ...
            try
            {
                ...
                foreach (var treatment in listTreatments)
                {
                    ...
                    {
                        rdo.TOTAL_PRICE = treatmentFee.FirstOrDefault().TOTAL_PRICE ?? 0;
                        rdo.TOTAL_HEIN_PRICE = treatmentFee.FirstOrDefault().TOTAL_HEIN_PRICE ?? 0;
                        rdo.TOTAL_PATIENT_PRICE = treatmentFee.FirstOrDefault().TOTAL_PATIENT_PRICE ?? 0;

                        if (treatmentFee.FirstOrDefault().TOTAL_PRICE > LIMIT_PRICE)
                        {
                            rdo.PARENT_ID = 1;
                ...
                listRdoParent = listRdo.GroupBy(s => s.PARENT_ID).Select(s => new Mrs00495RDO()
                {
        ...
        protected override void SetTag(Dictionary<string, object> dicSingleTag, Inventec.Common.FlexCellExport.ProcessObjectTag objectTag, Inventec.Common.FlexCellExport.Store store)
        {
            try
            {
                ...
                dicSingleTag.Add("TREATMENT_TYPE", this.TREATMENT_TYPE_ID);
                dicSingleTag.Add("LIMIT_PRICE", this.LIMIT_PRICE);
