...
{
    class Mrs00446Processor : AbstractProcessor
    {
        Mrs00446Filter castFilter = null; 
        List<Mrs00446RDO> listRdo = new List<Mrs00446RDO>(); 


        public Mrs00446Processor(CommonParam param, string reportTypeCode)
        ...
        protected override bool GetData()
        {
            ...
            try
            {
                ...
                this.castFilter = (Mrs00446Filter)this.reportFilter; 
                var skip = 0; 
                //lay khoa
                if (IsNotNullOrEmpty(this.castFilter.EXECUTE_DEPARTMENT_IDs))
                {
                    HisDepartmentFilterQuery departmentFilter = new HisDepartmentFilterQuery(); 
                    departmentFilter.IDs = this.castFilter.EXECUTE_DEPARTMENT_IDs; 
                ...
                while (listTreatmentIds.Count - skip > 0)
                {
                    ...
                    if (IsNotNullOrEmpty(this.castFilter.EXECUTE_DEPARTMENT_IDs))
                    {
                        filter.EXECUTE_DEPARTMENT_IDs = this.castFilter.EXECUTE_DEPARTMENT_IDs; 
                    }
            ...
            try
            {
                if(IsNotNullOrEmpty(listTreatments))
                {
                    foreach(var treatment in listTreatments)
                    {
                        ...
                        Mrs00446RDO rdo = new Mrs00446RDO(); 
