...
{
    public class Mrs00239Processor : AbstractProcessor
    {
        List<Mrs00239RDO> ListRdo = new List<Mrs00239RDO>();
        ...
        Mrs00239Filter castFilter = null;

        HIS_BRANCH _Branch = null;

        public Mrs00239Processor(CommonParam param, string reportTypeCode)
		...
		protected override bool GetData()
		{
            ...
            try
            {
                castFilter = ((Mrs00239Filter)this.reportFilter);
                ...
                Inventec.Common.Logging.LogSystem.Debug("Bat dau lay du lieu V_HIS_HEIN_APPROVAL, Mrs00239, filter: " + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => castFilter), castFilter));
                ...
                {
                    throw new DataMisalignedException("Co loi xay ra trong qua trinh lay du lieu V_HIS_HEIN_APPROVAL, Mrs00239");
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                if (IsNotNullOrEmpty(ListHeinApproval))
                {
                    ...
                    while (count > 0)
                    {
                        ...
                        if (paramGet.HasException)
                        {
                            throw new DataMisalignedException("Co exception xay ra tai DAOGET trong qua trinh tong hop du lieu Mrs00239.");
                        }
            ...
            try
            {
                if (IsNotNullOrEmpty(hisHeinApprovals))
                {
                    ...
                    foreach (var heinApproval in hisHeinApprovals)
                    {
                        ...
                        {
                            Mrs00239RDO rdo = new Mrs00239RDO(heinApproval);
                            ...
                            if (dicTreatment.ContainsKey(heinApproval.TREATMENT_ID))
                            {
                                ...
                                else if (treatment.TREATMENT_END_TYPE_ID != null)
                                {
                                    rdo.TREATMENT_END_TYPE_CODE = "1";
                                }
        ...
        private void ProcessTotalPrice(Mrs00239RDO rdo, List<V_HIS_SERE_SERV_3> hisSereServs)
        {
            try
            {
        ...
        private bool CheckPrice(Mrs00239RDO rdo)
        {
            bool result = false;
            try
            {
        ...
        private bool checkBhytNsd(Mrs00239RDO rdo)
        {
            bool result = false;
            try
            {
