...
{
    public class Mrs00241Processor : AbstractProcessor
    {
        List<Mrs00241RDO> ListRdo = new List<Mrs00241RDO>();
        List<V_HIS_HEIN_APPROVAL> ListHeinApproval = new List<V_HIS_HEIN_APPROVAL>();

        Mrs00241Filter castFilter = null;

        HIS_BRANCH _Branch = null;

        public Mrs00241Processor(CommonParam param, string reportTypeCode)
		...
		protected override bool GetData()
		{
            ...
            try
            {
                castFilter = ((Mrs00241Filter)this.reportFilter);
                ...
                Inventec.Common.Logging.LogSystem.Debug("Bat dau lay du lieu V_HIS_HEIN_APPROVAL, Mrs00241, filter: " + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => castFilter), castFilter));
                ...
                {
                    throw new DataMisalignedException("Co loi xay ra trong qua trinh lay du lieu V_HIS_HEIN_APPROVAL, Mrs00241");
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                if (IsNotNullOrEmpty(ListHeinApproval))
                {
                    ...
                    while (count > 0)
                    {
                        ...
                        if (paramGet.HasException)
                        {
                            throw new DataMisalignedException("Co exception xay ra tai DAOGET trong qua trinh tong hop du lieu Mrs00241.");
                        }
            ...
            try
            {
                if (IsNotNullOrEmpty(hisHeinApprovals))
                {
                    ...
                    foreach (var heinApproval in hisHeinApprovals)
                    {
                        ...
                        {
                            Mrs00241RDO rdo = new Mrs00241RDO(heinApproval);
        ...
        private void ProcessTotalPrice(Mrs00241RDO rdo, List<V_HIS_SERE_SERV_3> hisSereServs)
        {
            try
            {
                foreach (var sereServ in hisSereServs)
                {
                    ...
                    if (sereServ.TDL_HEIN_SERVICE_TYPE_ID != null)
                    {
                        ...
                        var total = TotalPriceTreatment;
                        rdo.TOTAL_PRICE += total;
                        ...
                        rdo.TOTAL_HEIN_PRICE += total - patient;
        ...
        private bool CheckPrice(Mrs00241RDO rdo)
        {
            bool result = false;
            try
            {
        ...
        private bool checkBhytNsd(Mrs00241RDO rdo)
        {
            bool result = false;
            try
            {
