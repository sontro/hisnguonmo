...
using System.Data;
using MRS.Processor.Mrs00676;

namespace MRS.Processor.Mrs00676
{
    public partial class ManagerSql : BusinessBase
    {
        public DataTable GetBillAndOtherCancel(Mrs00676Filter filter)
        {
            ...
            try
            {
                ...
                query += "from (select tran.account_book_id,min(tran.num_order) as min_order,max(tran.num_order) as  max_order,sum(tran.amount) as bill_amount,null as amount_bk,null as num_order,null as transaction_code from his_rs.his_transaction tran where 1=1 ";
        ...
        public DataTable GetSampleCancel(Mrs00676Filter filter)
        {
            DataTable result = null;
            try
            {
                ...
                query += "tran.num_order,acc.*,  tran.TRANSACTION_CODE,tran.amount from his_rs.his_transaction tran join his_rs.his_account_book acc on acc.id = tran.account_book_id where tran.is_cancel is not null ";
        ...
        public DataTable GetCancelOtherDetail(Mrs00676Filter filter)
        {
            DataTable result = null;
            try
            {
        ...
        public DataTable GetCancelSampleDetail(Mrs00676Filter filter)
        {
            DataTable result = null;
            try
            {
        ...
        public DataTable GetBillDetail(Mrs00676Filter filter)
        {
            DataTable result = null;
            try
            {
                ...
                query += "Trans.*,acc.*,dp.department_name from(SELECT tran.tdl_patient_code,tran.tdl_patient_name,dpt.treatment_id,tran.account_book_id, tran.num_order, tran.TRANSACTION_CODE, tran.TRANSACTION_TIME,tran.amount,MAX(dpt.department_id) KEEP (DENSE_RANK LAST ORDER BY dpt.id) as curent_department_id from his_rs.his_transaction tran  left join his_rs.his_department_tran dpt on (dpt.department_in_time<tran.transaction_time  and dpt.treatment_id = tran.treatment_id) where 1=1 ";//Lấy cả giao dịch hủy để phù hợp với báo cáo tổng hợp
        ...
        public List<TYPE_PRICE> GetTypePrice(Mrs00676Filter filter)
        {
