...
{
    public class Mrs00216Processor : AbstractProcessor
    {
        Mrs00216Filter castFilter = null;

        List<Mrs00216RDO> ListRdo = new List<Mrs00216RDO>();
        ...
        public Mrs00216Processor(CommonParam param, string reportTypeCode)
		...
		protected override bool GetData()
		{
            ...
            try
            {
                castFilter = (Mrs00216Filter)this.reportFilter;
                ...
                Inventec.Common.Logging.LogSystem.Debug("Bat dau lay du lieu V_HIS_HEIN_APPROVAL, MRS00216, filter: " + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => castFilter), castFilter));
                ...
                {
                    throw new DataMisalignedException("Co loi xay ra trong qua trinh lay du lieu V_HIS_HEIN_APPROVAL, MRS00216");
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                if (IsNotNullOrEmpty(ListHeinApproval))
                {
                    ...
                    while (count > 0)
                    {
                        ...
                        if (paramGet.HasException)
                        {
                            throw new DataMisalignedException("Co exception xay ra tai DAOGET trong qua trinh tong hop du lieu, MRS00216.");
                        }
            ...
            try
            {
                ...
                if (IsNotNullOrEmpty(ListSereServ))
                {
                    foreach (var sereServ in ListSereServ)
                    {
                        ...
                        Mrs00216RDO rdo = new Mrs00216RDO();
        ...
        private List<Mrs00216RDO> ProcessListRDO(List<Mrs00216RDO> listRDO)
        {
            List<Mrs00216RDO> listCurrent = new List<Mrs00216RDO>();
            try
            {
                if (listRDO.Count > 0)
                {
                    ...
                    {
                        List<Mrs00216RDO> listsub = group.ToList<Mrs00216RDO>();
