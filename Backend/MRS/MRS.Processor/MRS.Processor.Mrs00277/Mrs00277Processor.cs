...
{
    public class Mrs00277Processor : AbstractProcessor
    {
        Mrs00277Filter castFilter = null;

        List<Mrs00277RDO> ListRdoA = new List<Mrs00277RDO>();
        List<Mrs00277RDO> ListRdoB = new List<Mrs00277RDO>();
        List<Mrs00277RDO> ListRdoC = new List<Mrs00277RDO>();

        Dictionary<int, Mrs00277RDO> dicRdoA = new Dictionary<int, Mrs00277RDO>();
        Dictionary<int, Mrs00277RDO> dicRdoB = new Dictionary<int, Mrs00277RDO>();
        Dictionary<int, Mrs00277RDO> dicRdoC = new Dictionary<int, Mrs00277RDO>();

        List<Mrs00277RDO> ListTotalRdoA = new List<Mrs00277RDO>();
        List<Mrs00277RDO> ListTotalRdoB = new List<Mrs00277RDO>();
        List<Mrs00277RDO> ListTotalRdoC = new List<Mrs00277RDO>();
        ...
        public Mrs00277Processor(CommonParam param, string reportTypeCode)
		...
		protected override bool GetData()
		{
            ...
            try
            {
                castFilter = ((Mrs00277Filter)this.reportFilter);
                ...
                Inventec.Common.Logging.LogSystem.Debug("Bat dau lay du lieu V_HIS_HEIN_APPROVAL, Mrs00277, filter: " + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => castFilter), castFilter));
                ...
                {
                    throw new DataMisalignedException("Co loi xay ra trong qua trinh lay du lieu V_HIS_HEIN_APPROVAL, Mrs00277");
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                if (IsNotNullOrEmpty(ListHeinApproval))
                {
                    ...
                    while (count > 0)
                    {
                        ...
                        if (paramGet.HasException)
                        {
                            throw new DataMisalignedException("Co exception xay ra tai DAOGET trong qua trinh tong hop du lieu Mrs00277.");
                        }
                    ...
                    foreach (var heinApproval in hisHeinApprovals)
                    {
                        if (heinApproval.HEIN_TREATMENT_TYPE_CODE == MOS.LibraryHein.Bhyt.HeinTreatmentType.HeinTreatmentTypeCode.TREAT)
                        {
                            ...
                            if (!String.IsNullOrWhiteSpace(this._Branch.ACCEPT_HEIN_MEDI_ORG_CODE) && this._Branch.ACCEPT_HEIN_MEDI_ORG_CODE.Contains(heinApproval.HEIN_MEDI_ORG_CODE) && checkBhytProvinceCode(heinApproval.HEIN_CARD_NUMBER))
                            {
                                ...
                                {
                                    rdo = new Mrs00277RDO(heinApproval);
        ...
        private void ProcessTotalPrice(Mrs00277RDO rdo, List<V_HIS_SERE_SERV_3> hisSereServs, V_HIS_HEIN_APPROVAL heinApproval, V_HIS_TREATMENT treatment)
        {
            try
            {
        ...
        private List<Mrs00277RDO> CheckList(List<Mrs00277RDO> listRdo)
        {
            List<Mrs00277RDO> result = new List<Mrs00277RDO>();
            try
            {
            ...
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
                result = new List<Mrs00277RDO>();
            }
            return result;
        }

        private bool CheckPrice(Mrs00277RDO rdo)
        {
            bool result = false;
            try
            {
                result = rdo.BED_PRICE > 0 || rdo.BLOOD_PRICE > 0 || rdo.DIIM_PRICE > 0 || rdo.EXAM_PRICE > 0 ||
                    ...
                    rdo.TOTAL_HEIN_PRICE > 0 || rdo.TOTAL_HEIN_PRICE_NDS > 0 || rdo.TOTAL_PATIENT_PRICE > 0 || rdo.TT_PRICE > 0 || rdo.TOTAL_PRICE > 0 || rdo.TRAN_PRICE > 0;
            }
            catch (Exception ex)
            {
                result = false;
        ...
        private void ProcessSumTotal()
        {
            try
            {
                Mrs00277RDO rdoA = new Mrs00277RDO();
                ...
                Mrs00277RDO rdoB = new Mrs00277RDO();
                ...
                Mrs00277RDO rdoC = new Mrs00277RDO();
