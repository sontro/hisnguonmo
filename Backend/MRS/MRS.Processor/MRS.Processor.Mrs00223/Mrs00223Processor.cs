...
{
    public class Mrs00223Processor : AbstractProcessor
    {
        Mrs00223Filter castFilter = null;
        List<Mrs00223RDO> ListExamRdo = new List<Mrs00223RDO>();
        List<Mrs00223RDO> ListBedRdo = new List<Mrs00223RDO>();
        List<Mrs00223RDO> ListTestRdo = new List<Mrs00223RDO>();
        List<Mrs00223RDO> ListDiimFuexRdo = new List<Mrs00223RDO>();
        List<Mrs00223RDO> ListSurgMisuRdo = new List<Mrs00223RDO>();
        List<Mrs00223RDO> ListOtherRdo = new List<Mrs00223RDO>();

        List<V_HIS_HEIN_APPROVAL> ListHeinApproval = new List<V_HIS_HEIN_APPROVAL>();

        HIS_BRANCH _Branch = null;

        public Mrs00223Processor(CommonParam param, string reportTypeCode)
		...
		protected override bool GetData()
		{
            ...
            try
            {
                castFilter = (Mrs00223Filter)this.reportFilter;
                ...
                Inventec.Common.Logging.LogSystem.Debug("Bat dau lay du lieu V_HIS_HEIN_APPROVAL, MRS00223, filter: " + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => castFilter), castFilter));
                ...
                if (IsNotNullOrEmpty(ListSereServ))
                {
                    foreach (var sere in ListSereServ)
                    {
                        if (sere.IS_EXPEND == IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__TRUE || sere.AMOUNT <= 0 || sere.PRICE == 0 || sere.HEIN_APPROVAL_ID == null || sere.TDL_HEIN_SERVICE_TYPE_ID == null || sere.IS_NO_EXECUTE == IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__TRUE)
                            continue;
                        bool valid = false;
                        Mrs00223RDO rdo = new Mrs00223RDO(sere);
        ...
        private List<Mrs00223RDO> ProcessListRDO(List<Mrs00223RDO> listRdo)
        {
            List<Mrs00223RDO> listCurrent = new List<Mrs00223RDO>();
            try
            {
                if (listRdo.Count > 0)
                {
                    ...
                    {
                        List<Mrs00223RDO> listsub = group.ToList<Mrs00223RDO>();
                        if (listsub != null && listsub.Count > 0)
                        {
                            Mrs00223RDO rdo = new Mrs00223RDO();
