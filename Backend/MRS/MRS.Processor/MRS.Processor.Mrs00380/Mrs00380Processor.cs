...
{
    class Mrs00380Processor : AbstractProcessor
    {
        Mrs00380Filter castFilter = null; 

        List<Mrs00380RDO> listRdo = new List<Mrs00380RDO>(); 
        Dictionary<long, Dictionary<long, Mrs00380RDO>> dicRdo = new Dictionary<long, Dictionary<long, Mrs00380RDO>>(); 

        public Mrs00380Processor(CommonParam param, string reportTypeCode)
        ...
        protected override bool GetData()
        {
            ...
            try
            {
                this.castFilter = (Mrs00380Filter)this.reportFilter; 
                CommonParam paramGet = new CommonParam(); 
                Inventec.Common.Logging.LogSystem.Info("Ba dau lay du lieu bao cao MRS00380: " + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => castFilter), castFilter)); 
                ...
                serviceRetyFilter.REPORT_TYPE_CODE__EXACT = "MRS00380"; 
                ...
                {
                    throw new Exception("Co exception xay ra tai DAOGET trong qua lay du lieu MRS00380"); 
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                if (IsNotNullOrEmpty(listServiceReq) && IsNotNullOrEmpty(listServiceRetyCat))
                {
                    ...
                    while (count > 0)
                    {
                        ...
                        if (paramGet.HasException)
                        {
                            throw new Exception("Co exception xay ra tai DAOGET trong qua trinh lay du lieu MRS00380"); 
                        }
                    ...
                    if (listSereServId.Count > 0)
                    {
                        ...
                        while (count > 0)
                        {
                            ...
                            if (paramGet.HasException)
                            {
                                throw new Exception("Co excetion xay ra tai DAOGET trong qua trinh lay du lieu MRS00380"); 
                            }
        ...
        void ProcessDataDetail()
        {
            foreach (var serviceReq in listServiceReq)
            {
                ...
                {
                    rdo = new Mrs00380RDO(serviceReq); 
                    if (!dicRdo.ContainsKey(DateFinish))
                        dicRdo[DateFinish] = new Dictionary<long, Mrs00380RDO>(); 
                ...
                foreach (var sereServ in dicSereServ[serviceReq.ID])
                {
                    ...
                    foreach (var tein in dicSereServTein[sereServ.ID])
                    {
                        ...
                        System.Reflection.PropertyInfo pi = typeof(Mrs00380RDO).GetProperty("VALUE_" + testIndex.NumOrder); 
