...
{
    public class Mrs00211Processor : AbstractProcessor
    {
        Mrs00211Filter castFilter = null;

        List<Mrs00211RDO> ListRdoA = new List<Mrs00211RDO>();
        List<Mrs00211RDO> ListRdoB = new List<Mrs00211RDO>();
        List<Mrs00211RDO> ListRdoC = new List<Mrs00211RDO>();

        List<Mrs00211RDO> ListSumTotal = new List<Mrs00211RDO>();
        ...
        public Mrs00211Processor(CommonParam param, string reportTypeCode)
		...
		protected override bool GetData()
		{
            ...
            try
            {
                castFilter = (Mrs00211Filter)this.reportFilter;
                ...
                Inventec.Common.Logging.LogSystem.Debug("Bat dau lay du lieu V_HIS_HEIN_APPROVAL, MRS00211, filter: " + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => castFilter), castFilter));
                ...
                {
                    throw new DataMisalignedException("Co loi xay ra trong qua trinh lay du lieu V_HIS_HEIN_APPROVAL, MRS00211");
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                if (IsNotNullOrEmpty(ListHeinApproval))
                {
                    ...
                    while (count > 0)
                    {
                        ...
                        if (paramGet.HasException)
                        {
                            throw new DataMisalignedException("Co exception xay ra tai DAOGET trong qua trinh tong hop du lieu MRS00211.");
                        }
            ...
            try
            {
                if (IsNotNullOrEmpty(hisHeinApprovals))
                {
                    ...
                    foreach (var heinApproval in hisHeinApprovals)
                    {
                        ...
                        {
                            Mrs00211RDO rdo = new Mrs00211RDO(heinApproval);
        ...
        private void ProcessTotalPrice(Mrs00211RDO rdo, List<V_HIS_SERE_SERV_3> hisSereServs)
        {
            try
            {
        ...
        private bool CheckPrice(Mrs00211RDO rdo)
        {
            bool result = false;
            try
            {
        ...
        private void ProcessSumTotal()
        {
            try
            {
                Mrs00211RDO rdo = new Mrs00211RDO();
        ...
        private bool checkBhytNsd(Mrs00211RDO rdo)
        {
            bool result = false;
            try
            {
