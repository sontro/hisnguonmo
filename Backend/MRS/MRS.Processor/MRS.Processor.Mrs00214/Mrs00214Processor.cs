...
{
    public class Mrs00214Processor : AbstractProcessor
    {
        Mrs00214Filter castFilter = null;

        List<Mrs00214RDO> ListRdoA = new List<Mrs00214RDO>();
        List<Mrs00214RDO> ListRdoB = new List<Mrs00214RDO>();
        List<Mrs00214RDO> ListRdoC = new List<Mrs00214RDO>();

        List<Mrs00214RDO> ListSumTotal = new List<Mrs00214RDO>();
        ...
        public Mrs00214Processor(CommonParam param, string reportTypeCode)
		...
		protected override bool GetData()
		{
            ...
            try
            {
                castFilter = (Mrs00214Filter)this.reportFilter;
                ...
                Inventec.Common.Logging.LogSystem.Debug("Bat dau lay du lieu V_HIS_HEIN_APPROVAL, MRS00214, filter: " + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => castFilter), castFilter));
                ...
                {
                    throw new DataMisalignedException("Co loi xay ra trong qua trinh lay du lieu V_HIS_HEIN_APPROVAL, MRS00214");
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                if (IsNotNullOrEmpty(ListHeinApproval))
                {
                    ...
                    while (count > 0)
                    {
                        ...
                        if (paramGet.HasException)
                        {
                            throw new DataMisalignedException("Co exception xay ra tai DAOGET trong qua trinh tong hop du lieu MRS00214.");
                        }
            ...
            try
            {
                if (IsNotNullOrEmpty(hisHeinApprovals))
                {
                    ...
                    foreach (var heinApproval in hisHeinApprovals)
                    {
                        ...
                        {
                            Mrs00214RDO rdo = new Mrs00214RDO(heinApproval);
                            if (dicTreatment.ContainsKey(heinApproval.TREATMENT_ID))
                            {
                                ...
                                {
                                    rdo.MAIN_DAY = DayOfTreatmentWith3360.DayOfTreatmentWithCV3360(treatment.IN_TIME, treatment.OUT_TIME.Value);
                                }
        ...
        private void ProcessTotalPrice(Mrs00214RDO rdo, List<V_HIS_SERE_SERV_3> hisSereServs)
        {
            try
            {
                ...
                //Doi tuong quan nhan va than nhan si quan thi khong check ma tinh
                //ma chi chia thanh ban dau va da tuyen den
                //trong da tuyen dan chia thanh dung tuyen va trai tuyen
                if (this._Branch.ACCEPT_HEIN_MEDI_ORG_CODE.Contains(rdo.HEIN_MEDI_ORG_CODE))
                {
                    ListRdoA.Add(rdo);
                }
                else if (rdo.RIGHT_ROUTE_CODE == MOS.LibraryHein.Bhyt.HeinRightRoute.HeinRightRouteCode.TRUE)
                {
                    ListRdoB.Add(rdo);
                }
                else
        ...
        private bool CheckPrice(Mrs00214RDO rdo)
        {
            bool result = false;
            try
            {
        ...
        private void ProcessSumTotal()
        {
            try
            {
                Mrs00214RDO rdo = new Mrs00214RDO();
        ...
        private bool checkBhytNsd(Mrs00214RDO rdo)
        {
            bool result = false;
            try
            {
