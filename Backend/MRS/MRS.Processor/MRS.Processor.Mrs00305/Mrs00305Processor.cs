...
{
    public class Mrs00305Processor : AbstractProcessor
    {
        List<Mrs00305RDO> ListRdoA = new List<Mrs00305RDO>();
        List<Mrs00305RDO> ListRdoB = new List<Mrs00305RDO>();
        List<Mrs00305RDO> ListRdoC = new List<Mrs00305RDO>();

        List<Mrs00305RDO> ListSumTotal = new List<Mrs00305RDO>();
        Mrs00305Filter castFilter = new Mrs00305Filter();
        ...
        public Mrs00305Processor(CommonParam param, string reportTypeCode)
		...
		protected override bool GetData()
		{
            ...
            try
            {
                castFilter = ((Mrs00305Filter)this.reportFilter);
            ...
            try
            {
                if (IsNotNullOrEmpty(ListHeinApproval))
                {
                    ...
                    while (count > 0)
                    {
                        ...
                        List<V_HIS_PATIENT_TYPE_ALTER> ListPatientTypeAlter = new HisPatientTypeAlterManager(paramGet).GetView(PatientTypeAlterViewFilter);

                        GeneralDataByListHeinApproval(hisHeinApprovals, ListSereServ, ListTreatment, ListPatientTypeAlter);
                ...
                if (IsNotNullOrEmpty(hisHeinApprovals))
                {
                    ...
                    foreach (var heinApproval in hisHeinApprovals)
                    {
                        if (heinApproval.HEIN_TREATMENT_TYPE_CODE == MOS.LibraryHein.Bhyt.HeinTreatmentType.HeinTreatmentTypeCode.EXAM)
                        {
                            ...
                            Mrs00305RDO rdo = new Mrs00305RDO(heinApproval);
        ...
        private void ProcessTotalPrice(Mrs00305RDO rdo, List<V_HIS_SERE_SERV_3> hisSereServs, Dictionary<long, V_HIS_PATIENT_TYPE_ALTER> dicPatientTypeAlter)
        {
            try
            {
        ...
        private bool CheckPrice(Mrs00305RDO rdo)
        {
            bool result = false;
            try
            {
        ...
        private bool checkBhytNsd(Mrs00305RDO rdo)
        {
            bool result = false;
            try
            {
        ...
        private void ProcessSumTotal()
        {
            try
            {
                Mrs00305RDO rdo = new Mrs00305RDO();
