...
namespace MRS.Processor.Mrs00692
{
    public class Mrs00692Processor : AbstractProcessor
    {
        Mrs00692Filter filter = new Mrs00692Filter();
        ...
        public Mrs00692Processor(CommonParam param, string reportTypeCode)
        ...
        protected override bool GetData()
        {
            ...
            try
            {
                filter = (Mrs00692Filter)this.reportFilter;
                //get dữ liệu:
        ...
        protected override void SetTag(Dictionary<string, object> dicSingleTag, ProcessObjectTag objectTag, Store store)
        {
            ...
            objectTag.AddObjectData(store, "Temp", new ManagerSql().GetBillAndOtherCancel(filter));
            objectTag.AddObjectData(store, "Cancel", new ManagerSql().GetSampleCancel(filter));

            objectTag.AddRelationship(store, "Temp", "Cancel", "ACCOUNT_BOOK_CODE", "ACCOUNT_BOOK_CODE");

            objectTag.AddObjectData(store, "CancelDetail", new ManagerSql().GetCancelOtherDetail(filter));
            objectTag.AddObjectData(store, "CancelDetail1", new ManagerSql().GetCancelSampleDetail(filter));
            objectTag.AddObjectData(store, "TransactionDetail", new ManagerSql().GetBillDetail(filter));
        }
