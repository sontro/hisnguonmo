...
namespace MRS.Proccessor.Mrs00530
{
    public class Mrs00530RDO : V_HIS_SERE_SERV
    {
        ...
        public Mrs00530RDO(V_HIS_SERE_SERV r, Dictionary<long, List<HIS_SERVICE_METY>> dicServiceMety, Dictionary<long, List<HIS_SERVICE_MATY>> dicServiceMaty, List<V_HIS_SERVICE_RETY_CAT> listServiceRetyCat, Mrs00530Filter mrs00530Filter, List<V_HIS_TREATMENT_4> treatments, List<HIS_PATIENT_TYPE_ALTER> lastHisPatientTypeAlter, List<V_HIS_ROOM> listHistRoom, List<HIS_EXECUTE_ROOM> listHisExecuteRoom)
        {
            ...
            SetExtendField(dicServiceMety, dicServiceMaty, listServiceRetyCat, mrs00530Filter, treatments, lastHisPatientTypeAlter, listHistRoom, listHisExecuteRoom);
        }
        ...
        private void SetExtendField(Dictionary<long, List<HIS_SERVICE_METY>> dicServiceMety, Dictionary<long, List<HIS_SERVICE_MATY>> dicServiceMaty, List<V_HIS_SERVICE_RETY_CAT> listServiceRetyCat, Mrs00530Filter mrs00530Filter, List<V_HIS_TREATMENT_4> treatments, List<HIS_PATIENT_TYPE_ALTER> lastHisPatientTypeAlter, List<V_HIS_ROOM> listHistRoom, List<HIS_EXECUTE_ROOM> listHisExecuteRoom)
        {
            ...
            var room = listHistRoom.FirstOrDefault(o => o.ID == treatment.END_ROOM_ID) ?? new V_HIS_ROOM();
            var executeRoom = listHisExecuteRoom.FirstOrDefault(o => o.ROOM_ID == treatment.END_ROOM_ID) ?? new HIS_EXECUTE_ROOM();
            ...
            var ecgServiceIds = listServiceRetyCat.Where(o => o.CATEGORY_CODE == mrs00530Filter.CATEGORY_CODE__ECG).Select(q => q.SERVICE_ID).Distinct().ToList();
            var brainBloodServiceIds = listServiceRetyCat.Where(o => o.CATEGORY_CODE == mrs00530Filter.CATEGORY_CODE__BRAIN_BLOOD).Select(q => q.SERVICE_ID).Distinct().ToList();
            var cervicalEndoServiceIds = listServiceRetyCat.Where(o => o.CATEGORY_CODE == mrs00530Filter.CATEGORY_CODE__CERVICAL_ENDO).Select(q => q.SERVICE_ID).Distinct().ToList();
            #endregion

            #region Contructor field
            this.TREATMENT_CODE = this.TDL_TREATMENT_CODE;
            this.END_ROOM_NAME = room.ROOM_NAME;
            ...
            this.ROOM_NUM_ORDER = executeRoom.NUM_ORDER ?? 0;
            this.END_ROOM_ID = treatment.END_ROOM_ID??0;
            #endregion
