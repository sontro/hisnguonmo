...
{
    public class Mrs00206Processor : AbstractProcessor
    {
        private List<Mrs00206RDO> listMrs00206RDOs = new List<Mrs00206RDO>();
        CommonParam paramGet = new CommonParam();
        List<Mrs00206RDO> ListRdo = new List<Mrs00206RDO>();

        List<Mrs00206RDO> listRdoMedicine = new List<Mrs00206RDO>();
        List<Mrs00206RDO> listRdoMaterial = new List<Mrs00206RDO>();
        ...
        private string a = "";
        public Mrs00206Processor(CommonParam param, string reportTypeCode)
        ...
        protected override bool GetData()
        {
            ...
            try
            {
                ...
                if (IsNotNullOrEmpty(((Mrs00206Filter)this.reportFilter).MEDI_STOCK_IDs))
                {
                    HisMediStockViewFilterQuery mediStockFilter = new HisMediStockViewFilterQuery();
                    mediStockFilter.IDs = ((Mrs00206Filter)this.reportFilter).MEDI_STOCK_IDs;
                    ListMediStock = new HisMediStockManager(paramGet).GetView(mediStockFilter);

                    if (IsNotNullOrEmpty(((Mrs00206Filter)this.reportFilter).REPORT_TYPE_CAT_IDs))
                    {
                        HisReportTypeCatFilterQuery reportTypeCatFilter = new HisReportTypeCatFilterQuery();
                        reportTypeCatFilter.IDs = ((Mrs00206Filter)this.reportFilter).REPORT_TYPE_CAT_IDs;
                        ListReportTypeCat = new HisReportTypeCatManager(paramGet).Get(reportTypeCatFilter);
                    }
                    else
                    {
                        throw new DataMisalignedException("Filter khong truyen len REPORT_TYPE_CAT_ID");
                    }

                    if (IsNotNullOrEmpty(((Mrs00206Filter)this.reportFilter).REPORT_TYPE_CAT_IDs))
                    {
                        HisServiceRetyCatFilterQuery serviceRetyCatFilter = new HisServiceRetyCatFilterQuery();
                        serviceRetyCatFilter.REPORT_TYPE_CAT_IDs = ((Mrs00206Filter)this.reportFilter).REPORT_TYPE_CAT_IDs;
                        ListServiceRetyCat = new HisServiceRetyCatManager(paramGet).Get(serviceRetyCatFilter);
                    }
                    else
                    {
                        throw new DataMisalignedException("Filter khong truyen len REPORT_TYPE_CAT_ID");
                    }
                    if (IsNotNullOrEmpty(ListServiceRetyCat)) ProcessData();
                }
                else
                {
            ...
            try
            {
                if (IsNotNullOrEmpty(ListMediStock))
                {
                    ...
                    foreach (var medistock in ListMediStock)
                    {
                        ...
                        impMediFilter.IMP_TIME_FROM = ((Mrs00206Filter)this.reportFilter).TIME_FROM;
                        impMediFilter.IMP_TIME_TO = ((Mrs00206Filter)this.reportFilter).TIME_TO;
                        ...
                        expMediFilter.EXP_TIME_FROM = ((Mrs00206Filter)this.reportFilter).TIME_FROM;
                        expMediFilter.EXP_TIME_TO = ((Mrs00206Filter)this.reportFilter).TIME_TO;
                        ...
                        impMateFilter.IMP_TIME_FROM = ((Mrs00206Filter)this.reportFilter).TIME_FROM;
                        impMateFilter.IMP_TIME_TO = ((Mrs00206Filter)this.reportFilter).TIME_TO;
                        ...
                        expMateFilter.EXP_TIME_FROM = ((Mrs00206Filter)this.reportFilter).TIME_FROM;
                        expMateFilter.EXP_TIME_TO = ((Mrs00206Filter)this.reportFilter).TIME_TO;
                            ...
                            listRdoMedicine = listRdoMedicine.GroupBy(g =>
                                new { g.SERVICE_ID, g.SUPPLIER_ID, g.IMP_PRICE }).Select(s =>
                                    new Mrs00206RDO
                                    {
                                        ...
                                        EXP_SALE_AMOUNT = s.Sum(s11 => s11.EXP_SALE_AMOUNT),
                                        EXP_LIQU_AMOUNT = s.Sum(s12 => s12.EXP_LIQU_AMOUNT),
                                        EXP_OTHER_AMOUNT = s.Sum(s12 => s12.EXP_OTHER_AMOUNT),
                                        MEDI_MATE_IM_ID = s.First().MEDI_MATE_IM_ID
                                    }).ToList();
                            #endregion
                        ...
                        {
                            throw new DataMisalignedException("Co exception xay ra tai DAOGET trong qua trinh tong hop du lieu, MRS00206.");
                        }
                    }
                    if (paramGet.HasException)
                    {
        ...
        protected override void SetTag(Dictionary<string, object> dicSingleTag, ProcessObjectTag objectTag, Store store)
        {
            ...
            dicSingleTag.Add("TIME_FROM", Inventec.Common.DateTime.Convert.TimeNumberToDateString(((Mrs00206Filter)this.reportFilter).TIME_FROM));
            dicSingleTag.Add("TIME_TO", Inventec.Common.DateTime.Convert.TimeNumberToDateString(((Mrs00206Filter)this.reportFilter).TIME_TO));

            objectTag.AddObjectData(store, "MediStocks", ListMediStock);
            objectTag.AddObjectData(store, "ReportTypeCats", ListReportTypeCat);
            objectTag.AddObjectData(store, "Services", ListRdo);
            objectTag.AddRelationship(store, "MediStocks", "Services", "ID", "MEDI_STOCK_ID");
            objectTag.AddRelationship(store, "ReportTypeCats", "Services", "ID", "REPORT_TYPE_CAT_ID");
        ...
        private void ProcessExpAmountMedicine(List<V_HIS_EXP_MEST_MEDICINE> hisExpMestMedicine)
        {
            try
            {
                ...
                {
                    //Xuất đơn thuốc
                    var listExpMestMediPRES = hisExpMestMedicine.Where(o =>
                        o.EXP_MEST_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_EXP_MEST_TYPE.ID__DPK ||
                        o.EXP_MEST_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_EXP_MEST_TYPE.ID__DDT ||
                        o.EXP_MEST_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_EXP_MEST_TYPE.ID__DTT).ToList();
            ...
            try
            {
                if (IsNotNullOrEmpty(listImpMestMediSub))
                {
                    ...
                    foreach (var group in GroupImps)
                    {
                        ...
                        Mrs00206RDO rdo = new Mrs00206RDO();
                        ...
                        rdo.REPORT_TYPE_CAT_ID = ListServiceRetyCat.Where(o => o.SERVICE_ID == listmediSub.First().SERVICE_ID).Select(p => p.REPORT_TYPE_CAT_ID).FirstOrDefault();
        ...
        private void ProcessExpAmountMaterial(List<V_HIS_EXP_MEST_MATERIAL> hisExpMestMaterial)
        {
            try
            {
                ...
                {
                    //Xuất đơn thuốc
                    var listExpMestMatePRES = hisExpMestMaterial.Where(o =>
                        o.EXP_MEST_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_EXP_MEST_TYPE.ID__DPK ||
                        o.EXP_MEST_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_EXP_MEST_TYPE.ID__DDT ||
                        o.EXP_MEST_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_EXP_MEST_TYPE.ID__DTT).ToList();
                    ...
                    //Xuất cho khoa phòng
                    var listExpMestMateDEPA = hisExpMestMaterial.Where(o =>
                        o.EXP_MEST_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_EXP_MEST_TYPE.ID__HPKP).ToList();
                    ...
                    //Xuất chuyển kho
                    var listExpMestMateCHMS = hisExpMestMaterial.Where(o =>
                        o.EXP_MEST_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_EXP_MEST_TYPE.ID__CK).ToList();
                    ...
                    //Xuất trả nhà cung cấp
                    var listExpMestMateMANU = hisExpMestMaterial.Where(o =>
                        o.EXP_MEST_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_EXP_MEST_TYPE.ID__TNCC).ToList();
                    ...
                    //Xuất bán
                    var listExpMestMateSALE = hisExpMestMaterial.Where(o =>
                        o.EXP_MEST_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_EXP_MEST_TYPE.ID__BAN).ToList();
                    ...
                    // Xuất khác
                    var listExpMestMateOTHER = hisExpMestMaterial.Where(o =>
                        o.EXP_MEST_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_EXP_MEST_TYPE.ID__KHAC).ToList();
            ...
            try
            {
                if (IsNotNullOrEmpty(listImpMestMateSub))
                {
                    ...
                    foreach (var group in GroupImps)
                    {
                        ...
                        rdo.MEDI_STOCK_ID = CurrentMediStock.ID;
                        rdo.MEDI_STOCK_NAME = CurrentMediStock.MEDI_STOCK_NAME;
                        rdo.REPORT_TYPE_CAT_ID = ListServiceRetyCat.Where(o => o.SERVICE_ID == listmateSub.First().SERVICE_ID).Select(p => p.REPORT_TYPE_CAT_ID).FirstOrDefault();
        ...
        private void ProcessGetPeriod(CommonParam paramGet)
        {
            try
            {
                ...
                periodFilter.CREATE_TIME_TO = ((Mrs00206Filter)this.reportFilter).TIME_FROM;
            ...
            try
            {
                ...
                if (IsNotNullOrEmpty(hisMestPeriodMedi))
                {
                    foreach (var item in hisMestPeriodMedi)
                    {
                        ...
                        rdo.MEDI_STOCK_ID = CurrentMediStock.ID;
                        rdo.MEDI_STOCK_NAME = CurrentMediStock.MEDI_STOCK_NAME;
                        rdo.REPORT_TYPE_CAT_ID = ListServiceRetyCat.Where(o => o.SERVICE_ID == item.SERVICE_ID).Select(p => p.REPORT_TYPE_CAT_ID).FirstOrDefault();
                ...
                impMediFilter.IMP_TIME_TO = ((Mrs00206Filter)this.reportFilter).TIME_FROM - 1;
                ...
                expMediFilter.EXP_TIME_TO = ((Mrs00206Filter)this.reportFilter).TIME_FROM - 1;
        ...
        private void processBeinAmountMaterialByMediStockPeriod(CommonParam paramGet, V_HIS_MEDI_STOCK_PERIOD neighborPeriod)
        {
            try
            {
                ...
                impMateFilter.IMP_TIME_TO = ((Mrs00206Filter)this.reportFilter).TIME_FROM - 1;
                ...
                expMateFilter.EXP_TIME_TO = ((Mrs00206Filter)this.reportFilter).TIME_FROM - 1;
        ...
        private Mrs00206Filter ProcessCastFilterQuery(Mrs00206Filter Mrs00206Filter)
        {
            Mrs00206Filter castFilter = null;
            try
            {
                if (Mrs00206Filter == null) throw new ArgumentNullException("Input param Mrs00206Filter is null");

                Mapper.CreateMap<Mrs00206Filter, Mrs00206Filter>();
                castFilter = Mapper.Map<Mrs00206Filter, Mrs00206Filter>(Mrs00206Filter);
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
