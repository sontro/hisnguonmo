...
{
    class Mrs00073Processor : AbstractProcessor
    {
        Mrs00073Filter castFilter = null;
        List<Mrs00073RDO> ListRdo = new List<Mrs00073RDO>();
        List<Mrs00073RDO> ListRdoA = new List<Mrs00073RDO>();
        List<Mrs00073RDO> ListRdoB = new List<Mrs00073RDO>();
        List<Mrs00073RDO> ListRdoC = new List<Mrs00073RDO>();

        Dictionary<long, Mrs00073RDO> dicRdo = new Dictionary<long, Mrs00073RDO>();
        Dictionary<short, Mrs00073RDO> dicRdoA = new Dictionary<short, Mrs00073RDO>();
        Dictionary<short, Mrs00073RDO> dicRdoB = new Dictionary<short, Mrs00073RDO>();
        Dictionary<short, Mrs00073RDO> dicRdoC = new Dictionary<short, Mrs00073RDO>();

        List<Mrs00073RDO> ListTotalRdoA = new List<Mrs00073RDO>();
        List<Mrs00073RDO> ListTotalRdoB = new List<Mrs00073RDO>();
        List<Mrs00073RDO> ListTotalRdoC = new List<Mrs00073RDO>();
        List<Mrs00073RDO> ListDetailRdo = new List<Mrs00073RDO>();
        ...
        private decimal TotalAmountBranch = 0;
        ...
        public Mrs00073Processor(CommonParam param, string reportTypeCode)
            : base(param, reportTypeCode)
        { }

        public override Type FilterType()
        {
            return typeof(Mrs00073Filter);
        }
		...
		protected override bool GetData()
		{
            ...
            try
            {
                this.castFilter = (Mrs00073Filter)this.reportFilter;
                ...
                {
                    throw new Exception("Co exception xay ra tai DAOGET trong qua trinh lay du lieu MRS00073");
                }
            }
            catch (Exception ex)
            {
        ...
        protected override bool ProcessData()
        {
            ...
            try
            {
                if (IsNotNullOrEmpty(ListHeinApproval))
                {
                    ...
                    while (count > 0)
                    {
                        ...
                        if (paramGet.HasException)
                        {
                            throw new DataMisalignedException("Co exception xay ra tai DAOGET trong qua trinh tong hop du lieu MRS00073.");
                        }
                    ...
                    //Inventec.Common.Logging.LogSystem.Info("countListSereServ" + countListSereServ);
                    //Inventec.Common.Logging.LogSystem.Info("countListTreatment" + countListTreatment);
                    ListRdo = dicRdo.Select(s => s.Value).OrderBy(o => o.DUNG_TRAI).ToList();
            ...
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
                ListRdo = new List<Mrs00073RDO>();
                ListRdoA = new List<Mrs00073RDO>();
                ListRdoB = new List<Mrs00073RDO>();
                ListRdoC = new List<Mrs00073RDO>();
                ListTotalRdoA = new List<Mrs00073RDO>();
                ListTotalRdoB = new List<Mrs00073RDO>();
                ListTotalRdoC = new List<Mrs00073RDO>();
                result = false;
            }
            return result;
        }
        ...
        private void GeneralDataByListHeinApproval(List<V_HIS_HEIN_APPROVAL> hisHeinApprovals, List<V_HIS_SERE_SERV_3> ListSereServ, List<V_HIS_TREATMENT> ListTreatment)
        {
            try
            {
                if (IsNotNullOrEmpty(hisHeinApprovals))
                {
                    ...
                    {
                        if (!dicSereServ.ContainsKey(heinApproval.ID)) continue;
                        if (heinApproval.HEIN_TREATMENT_TYPE_CODE == MOS.LibraryHein.Bhyt.HeinTreatmentType.HeinTreatmentTypeCode.EXAM)
                        {
                            ...
                            Mrs00073RDO rdo = null;
                            Mrs00073RDO rdoAll = null;
                            short isDungTrai;
                            ...
                            if (!dicRdo.ContainsKey(heinApproval.BRANCH_ID))
                            {
                                rdoAll = new Mrs00073RDO(heinApproval,castFilter);
                                dicRdo[heinApproval.BRANCH_ID] = rdoAll;
                            }
                            else
                            {
                                rdoAll = dicRdo[heinApproval.BRANCH_ID];
                            }
                                    ...
                                    && (this._Branch.ACCEPT_HEIN_MEDI_ORG_CODE ?? "").Contains(heinApproval.HEIN_MEDI_ORG_CODE) && checkBhytProvinceCode(heinApproval.HEIN_CARD_NUMBER))
                            {
                                ...
                                {
                                    rdo = new Mrs00073RDO(heinApproval,castFilter);
                            ...
                            if (IsNotNull(rdoAll) && dicSereServ.ContainsKey(heinApproval.ID) && dicTreatment.ContainsKey(heinApproval.TREATMENT_ID))
                            {
                                ProcessTotalPriceBranch(rdoAll, dicSereServ[heinApproval.ID], heinApproval, dicTreatment[heinApproval.TREATMENT_ID]);
                            }
                        }
                    }
                }
        ...
        private void ProcessDetail(List<V_HIS_SERE_SERV_3> sereServ)
        {
            try
            {
                ...
                if (IsNotNullOrEmpty(sereServ))
                {
                    ...
                    {
                        var treatment = ListTreatment.FirstOrDefault(p => p.ID == item.TDL_TREATMENT_ID && p.TDL_TREATMENT_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_TREATMENT_TYPE.ID__DTNGOAITRU);
                        var serviceReq = ListServiceReq.FirstOrDefault(p => p.ID == item.SERVICE_REQ_ID);
                        Mrs00073RDO rdo = new Mrs00073RDO();
                        if (treatment != null)
                        {
        ...
        private void ProcessTotalPrice(Mrs00073RDO rdo, List<V_HIS_SERE_SERV_3> hisSereServs, V_HIS_HEIN_APPROVAL heinApproval, V_HIS_TREATMENT treatment)
        {
            try
            {
                if (IsNotNullOrEmpty(hisSereServs))
                {
                    ...
                    foreach (var sereServ in hisSereServs)
                    {
                        ...
                        if (sereServ.TDL_HEIN_SERVICE_TYPE_ID != null)
                        {
                            ...
                            treatmentPrice += TotalPriceTreatment;
                    ...
                    if (!treatmentCodes.Contains(treatment.TREATMENT_CODE) && treatmentPrice > 0)
                    {
                        treatmentCodes.Add(treatment.TREATMENT_CODE);
                        rdo.TOTAL_COUNT++;
                    }
                    ...
                    //{
                    //    rdo.TOTAL_DATE += HIS.Treatment.DateTime.Calculation.DayOfTreatment(treatment.CLINICAL_IN_TIME.Value, treatment.OUT_TIME.Value);
                    //}
        ...
        private void ProcessTotalPriceBranch(Mrs00073RDO rdo, List<V_HIS_SERE_SERV_3> hisSereServs, V_HIS_HEIN_APPROVAL heinApproval, V_HIS_TREATMENT treatment)
        {
            try
            {
                if (IsNotNullOrEmpty(hisSereServs))
                {
                    ...
                    foreach (var sereServ in hisSereServs)
                    {
                        ...
                        if (sereServ.TDL_HEIN_SERVICE_TYPE_ID != null)
                        {
                            ...
                            else if (sereServ.TDL_HEIN_SERVICE_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_HEIN_SERVICE_TYPE.ID__KH || sereServ.TDL_HEIN_SERVICE_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_HEIN_SERVICE_TYPE.ID__GI_NT || sereServ.TDL_HEIN_SERVICE_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_HEIN_SERVICE_TYPE.ID__GI_NGT)
                            {
                                rdo.EXAM_PRICE += TotalPriceTreatment;
                            }
                            else if (sereServ.TDL_HEIN_SERVICE_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_HEIN_SERVICE_TYPE.ID__DVKTC)
                    ...
                    TotalAmountBranch += totalHeinPrice;
                }
            }
            catch (Exception ex)
            {
        ...
        private void ProcessSumTotal()
        {
            try
            {
                Mrs00073RDO rdoA = new Mrs00073RDO();
                ...
                Mrs00073RDO rdoB = new Mrs00073RDO();
                ...
                Mrs00073RDO rdoC = new Mrs00073RDO();
        ...
        private List<Mrs00073RDO> CheckList(List<Mrs00073RDO> listRdo)
        {
            List<Mrs00073RDO> result = new List<Mrs00073RDO>();
            try
            {
            ...
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
                result = new List<Mrs00073RDO>();
            }
            return result;
        }

        private bool CheckPrice(Mrs00073RDO rdo)
        {
            bool result = false;
            try
            {
        ...
        protected override void SetTag(Dictionary<string, object> dicSingleTag, Inventec.Common.FlexCellExport.ProcessObjectTag objectTag, Inventec.Common.FlexCellExport.Store store)
        {
            try
            {
                ...
                ListRdo = ListRdo.GroupBy(o => o.BRANCH_ID).Select(p => new Mrs00073RDO(p.ToList())).ToList();
