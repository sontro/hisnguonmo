...
{
    public class Mrs00549Processor : AbstractProcessor
    {
        Mrs00549Filter castFilter = null;

        List<Mrs00549RDO> ListRdoA = new List<Mrs00549RDO>();
        List<Mrs00549RDO> ListRdoB = new List<Mrs00549RDO>();
        List<Mrs00549RDO> ListRdoC = new List<Mrs00549RDO>();

        List<Mrs00549RDO> ListSumTotal = new List<Mrs00549RDO>();
        ...
        public Mrs00549Processor(CommonParam param, string reportTypeCode)
		...
		protected override bool GetData()
		{
            ...
            try
            {
                castFilter = (Mrs00549Filter)this.reportFilter;
                ...
                //}
                //if (!IsNotNullOrEmpty(this._Branch.ACCEPT_HEIN_MEDI_ORG_CODE) || !IsNotNullOrEmpty(this._Branch.HEIN_PROVINCE_CODE))
                //{
                //    throw new NullReferenceException("Chi nhanh khong co ma tinh, ma kcb tuyen duoi: " + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => _Branch), _Branch));
                //}
                ...
                Inventec.Common.Logging.LogSystem.Info("Bat dau lay du lieu V_HIS_HEIN_APPROVAL, MRS00549, filter: " + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => castFilter), castFilter));
                ...
                {
                    throw new DataMisalignedException("Co loi xay ra trong qua trinh lay du lieu V_HIS_HEIN_APPROVAL, MRS00549");
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                if (IsNotNullOrEmpty(ListHeinApproval))
                {
                    ...
                    while (count > 0)
                    {
                        ...
                        if (paramGet.HasException)
                        {
                            throw new DataMisalignedException("Co exception xay ra tai DAOGET trong qua trinh tong hop du lieu MRS00549.");
                        }
            ...
            try
            {
                if (IsNotNullOrEmpty(hisHeinApprovals))
                {
                    ...
                    foreach (var heinApproval in hisHeinApprovals)
                    {
                        ...
                        {
                            Mrs00549RDO rdo = new Mrs00549RDO(heinApproval);
        ...
        private void ProcessTotalPrice(Mrs00549RDO rdo, List<V_HIS_SERE_SERV_3> hisSereServs)
        {
            try
            {
                foreach (var sereServ in hisSereServs)
                {
                    ...
                    if (sereServ.TDL_HEIN_SERVICE_TYPE_ID != null)
                    {
                        ...
                        {
                            TotalPriceTreatment = Mrs.Bhyt.PayRateAndTotalPrice.Caculator.TotalPrice(NumDigits,sereServ, HisBranchCFG.HisBranchs.FirstOrDefault(o => o.ID == ListHeinApproval.FirstOrDefault(p => p.ID == sereServ.HEIN_APPROVAL_ID).BRANCH_ID) ?? new HIS_BRANCH());
        ...
        private bool CheckPrice(Mrs00549RDO rdo)
        {
            bool result = false;
            try
            {
                result = rdo.BED_PRICE > 0 || rdo.BLOOD_PRICE > 0 || rdo.DIIM_PRICE > 0 || rdo.EXAM_PRICE > 0 || rdo.BED_PRICE > 0 ||
        ...
        private void ProcessSumTotal()
        {
            try
            {
                Mrs00549RDO rdo = new Mrs00549RDO();
        ...
        private bool checkBhytNsd(Mrs00549RDO rdo)
        {
            bool result = false;
            try
            {
