...
{
    public class Mrs00220Processor : AbstractProcessor
    {
        Mrs00220Filter castFilter = null;

        List<Mrs00220RDO> ListRdoGeneric = new List<Mrs00220RDO>();
        List<Mrs00220RDO> ListRdoCpyhct = new List<Mrs00220RDO>();
        List<Mrs00220RDO> ListRdoVtyhct = new List<Mrs00220RDO>();
        List<Mrs00220RDO> ListRdoOther = new List<Mrs00220RDO>();
        ...
        public Mrs00220Processor(CommonParam param, string reportTypeCode)
        ...
        protected override bool GetData()
        {
            ...
            try
            {
                castFilter = (Mrs00220Filter)this.reportFilter;
                ...
                Inventec.Common.Logging.LogSystem.Debug("Bat dau lay du lieu V_HIS_HEIN_APPROVAL, V_HIS_MEDICINE_TYPE, MRS00220, filter: " + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => castFilter), castFilter));
                ...
                {
                    throw new DataMisalignedException("Co loi xay ra trong qua trinh lay du lieu V_HIS_HEIN_APPROVAL, MRS00220");
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                if (IsNotNullOrEmpty(ListHeinApproval))
                {
                    ...
                    while (count > 0)
                    {
                        ...
                        if (paramGet.HasException)
                        {
                            throw new DataMisalignedException("Co exception xay ra tai DAOGET trong qua trinh tong hop du lieu, MRS00220.");
                        }
                ...
                if (IsNotNullOrEmpty(ListSereServ))
                {
                    foreach (var sereServ in ListSereServ)
                    {
                        ...
                        if (!listHeinServiceTypeId.Contains(sereServ.TDL_HEIN_SERVICE_TYPE_ID.Value))
                            continue;
                        Mrs00220RDO rdo = new Mrs00220RDO();
        ...
        private List<Mrs00220RDO> ProcessListRDO(List<Mrs00220RDO> listRDO)
        {
            List<Mrs00220RDO> listCurrent = new List<Mrs00220RDO>();
            try
            {
                if (listRDO.Count > 0)
                {
                    ...
                    {
                        List<Mrs00220RDO> listsub = group.ToList<Mrs00220RDO>();
