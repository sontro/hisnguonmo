...
namespace MRS.Processor.Mrs00561
{
    public class Mrs00561Processor : AbstractProcessor
    {
        private List<Mrs00561RDO> ListRdoDetail = new List<Mrs00561RDO>();

        private List<Mrs00561RDO> ListRdoType01 = new List<Mrs00561RDO>();
        private List<Mrs00561RDO> ListRdoType02 = new List<Mrs00561RDO>();
        private List<Mrs00561RDO> ListRdoTypeOt = new List<Mrs00561RDO>();
        
        private Mrs00561Filter filter;
        ...
        public Mrs00561Processor(CommonParam param, string reportTypeCode)
		...
		{
            var result = true;
            filter = (Mrs00561Filter)this.reportFilter;
            Inventec.Common.Logging.LogSystem.Info("Bat dau lay bao cao MRS00561: " + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => filter), filter)); 
            ...
            try
            {
              ...
              //Danh sách HSDT
                ...
                HisHeinApprovalfilter = this.MapFilter<Mrs00561Filter, HisHeinApprovalFilterQuery>(filter, HisHeinApprovalfilter); 
        ...
        protected override bool ProcessData()
        {
            ...
            try
            {
                ...
                ListRdoDetail = (from r in listHisSereServ select new Mrs00561RDO(NumDigits, r, listHisHeinApproval, listHisMaterialType,HeinServiceTypeId_Mate, listHisMaterial, filter)).ToList();

                ListRdoDetail = ListRdoDetail.OrderBy(q => q.HEIN_SERVICE_BHYT_NAME).ToList();
                List<string> keySums = new List<string>() { "SUM"};

                ListRdoType01 = GroupSum(ListRdoDetail.Where(p => p.HEIN_CARD_NUMBER_TYPE_ID == 1).GroupBy(o => new { o.SERVICE_ID, TOTAL_HEIN_PRICE = o.PRICE, o.MATERIAL_SODANGKY_NAME }).ToList().ToDictionary(p => p, p => new Mrs00561RDO()), keySums);

                ListRdoType02 = GroupSum(ListRdoDetail.Where(p => p.HEIN_CARD_NUMBER_TYPE_ID == 2).GroupBy(o => new { o.SERVICE_ID, TOTAL_HEIN_PRICE = o.PRICE, o.MATERIAL_SODANGKY_NAME }).ToList().ToDictionary(p => p, p => new Mrs00561RDO()), keySums);

                ListRdoTypeOt = GroupSum(ListRdoDetail.Where(p => p.HEIN_CARD_NUMBER_TYPE_ID == 3).GroupBy(o => new { o.SERVICE_ID, TOTAL_HEIN_PRICE = o.PRICE, o.MATERIAL_SODANGKY_NAME }).ToList().ToDictionary(p => p, p => new Mrs00561RDO()), keySums);
            ...
            {
                LogSystem.Error(ex);
                ListRdoDetail = new List<Mrs00561RDO>();
                result = false;
            }
            return result;
        }
