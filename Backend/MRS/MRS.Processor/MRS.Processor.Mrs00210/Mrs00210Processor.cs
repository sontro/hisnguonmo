...
{
    public class Mrs00210Processor : AbstractProcessor
    {
        private Mrs00210Filter castFilter;

        List<Mrs00210RDO> ListRdoA = new List<Mrs00210RDO>();
        List<Mrs00210RDO> ListRdoB = new List<Mrs00210RDO>();
        List<Mrs00210RDO> ListRdoC = new List<Mrs00210RDO>();

        List<Mrs00210RDO> ListSumTotal = new List<Mrs00210RDO>();
        ...
        public Mrs00210Processor(CommonParam param, string reportTypeCode)
		...
		protected override bool GetData()
		{
            ...
            try
            {
                castFilter = ((Mrs00210Filter)this.reportFilter);
                ...
                Inventec.Common.Logging.LogSystem.Debug("Bat dau lay du lieu V_HIS_HEIN_APPROVAL, MRS00210, filter: " + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => castFilter), castFilter));
                ...
                {
                    throw new DataMisalignedException("Co loi xay ra trong qua trinh lay du lieu V_HIS_HEIN_APPROVAL, MRS00210");
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                if (IsNotNullOrEmpty(ListHeinApproval))
                {
                    ...
                    while (count > 0)
                    {
                        ...
                        if (paramGet.HasException)
                        {
                            throw new DataMisalignedException("Co exception xay ra tai DAOGET trong qua trinh tong hop du lieu MRS00210.");
                        }
            ...
            try
            {
                if (IsNotNullOrEmpty(hisHeinApprovals))
                {
                    ...
                    foreach (var heinApproval in hisHeinApprovals)
                    {
                        ...
                        {
                            Mrs00210RDO rdo = new Mrs00210RDO(heinApproval);
        ...
        private void ProcessTotalPrice(Mrs00210RDO rdo, List<V_HIS_SERE_SERV_3> hisSereServs)
        {
            try
            {
        ...
        private bool CheckPrice(Mrs00210RDO rdo)
        {
            bool result = false;
            try
            {
        ...
        private void ProcessSumTotal()
        {
            try
            {
                Mrs00210RDO rdo = new Mrs00210RDO();
        ...
        private bool checkBhytNsd(Mrs00210RDO rdo)
        {
            bool result = false;
            try
            {
