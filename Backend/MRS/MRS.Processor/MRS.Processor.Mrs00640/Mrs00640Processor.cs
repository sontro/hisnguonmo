...
{
    public class Mrs00640Processor : AbstractProcessor
    {
        List<Mrs00640RDO> ListRdo = new List<Mrs00640RDO>();
        ...
        Dictionary<long, Mrs00640RDO> dicRdo = new Dictionary<long, Mrs00640RDO>();
        Mrs00640Filter castFilter = null;

        HIS_BRANCH _Branch = null;

        public Mrs00640Processor(CommonParam param, string reportTypeCode)
		...
		protected override bool GetData()
		{
            ...
            try
            {
                castFilter = ((Mrs00640Filter)this.reportFilter);
                ...
                {
                    throw new DataMisalignedException("Co loi xay ra trong qua trinh lay du lieu V_HIS_HEIN_APPROVAL, Mrs00640");
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                if (IsNotNullOrEmpty(ListHeinApproval))
                {
                    ...
                    while (count > 0)
                    {
                        ...
                        if (paramGet.HasException)
                        {
                            throw new DataMisalignedException("Co exception xay ra tai DAOGET trong qua trinh tong hop du lieu Mrs00640.");
                        }
                    ...
                    foreach (var heinApproval in hisHeinApprovals)
                    {
                        if (heinApproval.HEIN_TREATMENT_TYPE_CODE == MOS.LibraryHein.Bhyt.HeinTreatmentType.HeinTreatmentTypeCode.TREAT && CheckHeinCardNumberType(heinApproval.HEIN_CARD_NUMBER))
                        {
                            ...
                            if (dicSereServ.ContainsKey(heinApproval.ID))
                            {
                                ...
                                {
                                    Mrs00640RDO rdo = new Mrs00640RDO(heinApproval);
                                    ...
                                    {
                                        dicRdo[heinApproval.TREATMENT_ID] = new Mrs00640RDO();
                                        Inventec.Common.Mapper.DataObjectMapper.Map<Mrs00640RDO>(dicRdo[heinApproval.TREATMENT_ID], rdo);
                                    }
                                    #endregion
        ...
        private void ProcessTotalPrice(Mrs00640RDO rdo, List<V_HIS_SERE_SERV_3> hisSereServs)
        {
            try
            {
        ...
        private bool CheckPrice(Mrs00640RDO rdo)
        {
            bool result = false;
            try
            {
        ...
        private bool checkBhytNsd(Mrs00640RDO rdo)
        {
            bool result = false;
            try
            {
