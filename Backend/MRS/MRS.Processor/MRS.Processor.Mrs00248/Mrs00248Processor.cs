...
{
    public class Mrs00248Processor : AbstractProcessor
    {
        Mrs00248Filter castFilter = null;

        List<Mrs00248RDO> ListRdoA = new List<Mrs00248RDO>();
        List<Mrs00248RDO> ListRdoB = new List<Mrs00248RDO>();
        List<Mrs00248RDO> ListRdoC = new List<Mrs00248RDO>();

        Dictionary<short, Mrs00248RDO> dicRdoA = new Dictionary<short, Mrs00248RDO>();
        Dictionary<short, Mrs00248RDO> dicRdoB = new Dictionary<short, Mrs00248RDO>();
        Dictionary<short, Mrs00248RDO> dicRdoC = new Dictionary<short, Mrs00248RDO>();

        List<Mrs00248RDO> ListTotalRdoA = new List<Mrs00248RDO>();
        List<Mrs00248RDO> ListTotalRdoB = new List<Mrs00248RDO>();
        List<Mrs00248RDO> ListTotalRdoC = new List<Mrs00248RDO>();
        ...
        public Mrs00248Processor(CommonParam param, string reportTypeCode)
		...
		protected override bool GetData()
		{
            ...
            try
            {
                castFilter = ((Mrs00248Filter)this.reportFilter);
                ...
                Inventec.Common.Logging.LogSystem.Debug("Bat dau lay du lieu V_HIS_HEIN_APPROVAL, Mrs00248, filter: " + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => castFilter), castFilter));
                ...
                {
                    throw new DataMisalignedException("Co loi xay ra trong qua trinh lay du lieu V_HIS_HEIN_APPROVAL, Mrs00248");
                }
            ...
            try
            {
                if (IsNotNullOrEmpty(ListHeinApproval))
                {
                    ...
                    while (count > 0)
                    {
                        ...
                        if (paramGet.HasException)
                        {
                            throw new DataMisalignedException("Co exception xay ra tai DAOGET trong qua trinh tong hop du lieu Mrs00248.");
                        }
        ...
        private void GeneralDataByListHeinApproval(List<V_HIS_HEIN_APPROVAL> hisHeinApprovals, List<V_HIS_SERE_SERV_3> ListSereServ, List<V_HIS_TREATMENT> ListTreatment)
        {
            try
            {
                if (IsNotNullOrEmpty(hisHeinApprovals))
                {
                    ...
                    var groupTreatment = hisHeinApprovals.GroupBy(o => new { o.TREATMENT_ID, o.BRANCH_ID }).ToList();
                    ...
                    foreach (var heinApproval in groupTreatment)
                    {
                        ...
                        if (heinApproval.ToList().Exists(s => s.HEIN_TREATMENT_TYPE_CODE == MOS.LibraryHein.Bhyt.HeinTreatmentType.HeinTreatmentTypeCode.TREAT && CheckHeinCardNumberType(s.HEIN_CARD_NUMBER)))
                        {
                            ...
                            if (this._Branch.ACCEPT_HEIN_MEDI_ORG_CODE.Contains(heinApproval.First().HEIN_MEDI_ORG_CODE) && checkBhytProvinceCode(heinApproval.First().HEIN_CARD_NUMBER))
                            {
                                if (!dicRdoA.ContainsKey(isDungTrai))
                                {
                                    rdo = new Mrs00248RDO(heinApproval.First());
                            ...
                            else if (checkBhytProvinceCode(heinApproval.First().HEIN_CARD_NUMBER))
                            {
                            ...
                            if (IsNotNull(rdo))
                            {
                                ...
                                decimal TotalPriceCheck = 0;
                                foreach (var item in heinApproval)
                                {
                                    if (dicSereServ.ContainsKey(item.ID) && dicTreatment.ContainsKey(item.TREATMENT_ID))
                                    {
                                        ProcessTotalPrice(rdo, dicSereServ[item.ID], item, treatment, ref TotalPriceCheck);
                                    }
                                }
        ...
        private void ProcessTotalPrice(Mrs00248RDO rdo, List<V_HIS_SERE_SERV_3> hisSereServs, V_HIS_HEIN_APPROVAL heinApproval, V_HIS_TREATMENT treatment,ref decimal TotalPriceCheck)
        {
            try
            {
                ...
                if (IsNotNullOrEmpty(hisSereServs))
                {
                    ...
                    foreach (var sereServ in hisSereServs)
                    {
                        ...
                        if (TotalPriceTreatment > 0)
                        {
                            TotalPriceCheck += TotalPriceTreatment;
                        }
                        if (sereServ.TDL_HEIN_SERVICE_TYPE_ID != null)
                        {
                            ...
                            if (patient > 0)
                            {
                                TotalPriceCheck += patient;
                            }
                            if (totalHeinPrice > 0)
                            {
                                TotalPriceCheck += totalHeinPrice;
                            }
        ...
        private List<Mrs00248RDO> CheckList(List<Mrs00248RDO> listRdo)
        {
            List<Mrs00248RDO> result = new List<Mrs00248RDO>();
            try
            {
            ...
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
                result = new List<Mrs00248RDO>();
            }
            return result;
        }

        private bool CheckPrice(Mrs00248RDO rdo)
        {
            bool result = false;
            try
            {
        ...
        private void ProcessSumTotal()
        {
            try
            {
                Mrs00248RDO rdoA = new Mrs00248RDO();
                ...
                Mrs00248RDO rdoB = new Mrs00248RDO();
                ...
                Mrs00248RDO rdoC = new Mrs00248RDO();
