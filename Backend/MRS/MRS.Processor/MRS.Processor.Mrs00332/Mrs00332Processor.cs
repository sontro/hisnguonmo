...
{
    public class Mrs00332Processor : AbstractProcessor
    {
        List<Mrs00332RDO> ListRdoA = new List<Mrs00332RDO>();
        List<Mrs00332RDO> ListRdoB = new List<Mrs00332RDO>();
        List<Mrs00332RDO> ListRdoC = new List<Mrs00332RDO>();

        List<Mrs00332RDO> ListSumTotal = new List<Mrs00332RDO>();
        Mrs00332Filter castFilter = new Mrs00332Filter();
        ...
        public Mrs00332Processor(CommonParam param, string reportTypeCode)
		...
		protected override bool GetData()
		{
            ...
            try
            {
                castFilter = ((Mrs00332Filter)this.reportFilter);
                ...
                if (IsNotNullOrEmpty(hisHeinApprovals))
                {
                    ...
                    foreach (var heinApproval in hisHeinApprovals)
                    {
                        if (heinApproval.HEIN_TREATMENT_TYPE_CODE == MOS.LibraryHein.Bhyt.HeinTreatmentType.HeinTreatmentTypeCode.TREAT)
                        {
                            ...
                            Mrs00332RDO rdo = new Mrs00332RDO(heinApproval);
        ...
        private void ProcessTotalPrice(Mrs00332RDO rdo, List<V_HIS_SERE_SERV_3> hisSereServs)
        {
            try
            {
                foreach (var sereServ in hisSereServs)
                {
                    if (!sereServ.VIR_TOTAL_HEIN_PRICE.HasValue || sereServ.VIR_TOTAL_HEIN_PRICE.Value <= 0)
                        continue;
                    var TotalPriceTreatment = Mrs.Bhyt.PayRateAndTotalPrice.Caculator.TotalPrice(NumDigits,sereServ, HisBranchCFG.HisBranchs.FirstOrDefault(o => o.ID == rdo.HEIN_APPROVAL.BRANCH_ID) ?? new HIS_BRANCH());
        ...
        private bool CheckPrice(Mrs00332RDO rdo)
        {
            bool result = false;
            try
            {
        ...
        private bool checkBhytNsd(Mrs00332RDO rdo)
        {
            bool result = false;
            try
            {
        ...
        private void ProcessSumTotal()
        {
            try
            {
                Mrs00332RDO rdo = new Mrs00332RDO();
