...
{
    class Mrs00463Processor : AbstractProcessor
    {
        Mrs00463Filter castFilter = null;
        List<Mrs00463RDO> ListRdoA = new List<Mrs00463RDO>();
        List<Mrs00463RDO> ListRdoB = new List<Mrs00463RDO>();
        List<Mrs00463RDO> ListRdoC = new List<Mrs00463RDO>();
        List<V_HIS_HEIN_APPROVAL> ListHeinApproval = new List<V_HIS_HEIN_APPROVAL>();
        HIS_BRANCH _Branch = null;

        decimal TotalAmount = 0;

        public Mrs00463Processor(CommonParam param, string reportTypeCode)
		...
		protected override bool GetData()
		{
            ...
            try
            {
                this.castFilter = (Mrs00463Filter)this.reportFilter;
            ...
            try
            {
                ...
                if (IsNotNullOrEmpty(hisHeinApprovals))
                {
                    ...
                    if (IsNotNullOrEmpty(ListSereServ))
                    {
                        ...
                        {
                            if (item.HEIN_APPROVAL_ID == null || item.TDL_HEIN_SERVICE_TYPE_ID == null || item.IS_EXPEND == 1 || item.AMOUNT <= 0)
                                continue;
                    ...
                    foreach (var heinApproval in hisHeinApprovals)
                    {
                        ...
                        {
                            var existsedRdoA = ListRdoA.FirstOrDefault(o => o.TREATMENT_CODE == heinApproval.TREATMENT_CODE && o.HEIN_CARD_NUMBER == heinApproval.HEIN_CARD_NUMBER);
                            var existsedRdoB = ListRdoB.FirstOrDefault(o => o.TREATMENT_CODE == heinApproval.TREATMENT_CODE && o.HEIN_CARD_NUMBER == heinApproval.HEIN_CARD_NUMBER);
                            var existsedRdoC = ListRdoC.FirstOrDefault(o => o.TREATMENT_CODE == heinApproval.TREATMENT_CODE && o.HEIN_CARD_NUMBER == heinApproval.HEIN_CARD_NUMBER);
                            if (existsedRdoA != null)
                            {
                                ...
                                if (existsedRdoB != null)
                                {
                                    ...
                                    {
                                        Mrs00463RDO rdo = new Mrs00463RDO(heinApproval);
                                        ...
                                        Inventec.Common.Logging.LogSystem.Info("TREATMENT_ID" + heinApproval.TREATMENT_ID.ToString() + rdo.IN_CODE);
                                        ...
                                        if (dicTreatment.ContainsKey(heinApproval.TREATMENT_ID))
                                        {
                                            ...
                                            if (treatment.OUT_TIME.HasValue && treatment.CLINICAL_IN_TIME.HasValue)
                                            {
                                                ...
                                                //rdo.TOTAL_DAY = Calculation.DayOfTreatment(treatment.CLINICAL_IN_TIME, treatment.OUT_TIME, treatment.TREATMENT_END_TYPE_ID, treatment.TREATMENT_RESULT_ID, PatientTypeEnum.TYPE.BHYT);
                                                if (treatment.TREATMENT_DAY_COUNT.HasValue)
                                                {
                                            ...
                                            //Ma noi chuyen
                                            if (treatment.IS_TRANSFER_IN == 1)
                                            {
                                                rdo.MEDI_ORG_FROM_CODE = treatment.TRANSFER_IN_MEDI_ORG_CODE;
                                            }
        ...
        private void ProcessTotalPrice(Mrs00463RDO rdo, List<V_HIS_SERE_SERV_3> hisSereServs)
        {
            try
            {
        ...
        private bool CheckPrice(Mrs00463RDO rdo)
        {
            bool result = false;
            try
            {
        ...
        private bool checkBhytNsd(Mrs00463RDO rdo)
        {
            bool result = false;
            try
            {
