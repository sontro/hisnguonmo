...
namespace HTC.MANAGER.Core.PeriodBO.HtcPeriod.Get
{
    public class HtcPeriodFilterQuery : HtcPeriodFilter
    {
        public HtcPeriodFilterQuery()
            : base()
        {

        }

        internal List<System.Linq.Expressions.Expression<Func<HTC_PERIOD, bool>>> listExpression = new List<System.Linq.Expressions.Expression<Func<HTC_PERIOD, bool>>>();

        internal Inventec.Backend.MANAGER.OrderProcessorBase OrderProcess = new Inventec.Backend.MANAGER.OrderProcessorBase();

        internal HtcPeriodSO Query()
        {
            HtcPeriodSO search = new HtcPeriodSO();
            try
            {
                ...
                search.listHtcPeriodExpression.AddRange(listExpression);
                search.OrderField = OrderProcess.GetOrderField<HTC_PERIOD>(ORDER_FIELD);
                search.OrderDirection = OrderProcess.GetOrderDirection(ORDER_DIRECTION);
            }
            catch (Exception ex)
            {
                LogSystem.Error(ex);
                search.listHtcPeriodExpression.Clear();
                search.listHtcPeriodExpression.Add(o => o.ID == NEGATIVE_ID);
            }
            return search;
        }
    }
