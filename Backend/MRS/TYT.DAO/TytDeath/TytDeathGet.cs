...
namespace TYT.DAO.TytDeath
{
    partial class TytDeathGet : EntityBase
    {
        ...
        {
            List<TYT_DEATH> list = new List<TYT_DEATH>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.TYT_DEATH.AsQueryable();
                        if (search.listTytDeathExpression != null && search.listTytDeathExpression.Count > 0)
                        {
                            foreach (var item in search.listTytDeathExpression)
                            {
        ...
        public TYT_DEATH GetById(long id, TytDeathSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.TYT_DEATH.AsQueryable().Where(p => p.ID == id);
                        if (search.listTytDeathExpression != null && search.listTytDeathExpression.Count > 0)
                        {
                            foreach (var item in search.listTytDeathExpression)
                            {
