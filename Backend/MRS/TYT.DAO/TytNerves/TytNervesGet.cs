...
namespace TYT.DAO.TytNerves
{
    partial class TytNervesGet : EntityBase
    {
        ...
        {
            List<TYT_NERVES> list = new List<TYT_NERVES>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.TYT_NERVES.AsQueryable();
                        if (search.listTytNervesExpression != null && search.listTytNervesExpression.Count > 0)
                        {
                            foreach (var item in search.listTytNervesExpression)
                            {
        ...
        public TYT_NERVES GetById(long id, TytNervesSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.TYT_NERVES.AsQueryable().Where(p => p.ID == id);
                        if (search.listTytNervesExpression != null && search.listTytNervesExpression.Count > 0)
                        {
                            foreach (var item in search.listTytNervesExpression)
                            {
