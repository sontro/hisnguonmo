...
{
    partial class TytHivGet : EntityBase
    {
        public List<V_TYT_HIV> GetView(TytHivSO search, CommonParam param)
        {
            List<V_TYT_HIV> list = new List<V_TYT_HIV>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_TYT_HIV.AsQueryable();
                        if (search.listVTytHivExpression != null && search.listVTytHivExpression.Count > 0)
                        {
                            foreach (var item in search.listVTytHivExpression)
                            {
