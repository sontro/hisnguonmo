...
namespace TYT.DAO.TytMalaria
{
    partial class TytMalariaGet : EntityBase
    {
        ...
        {
            List<TYT_MALARIA> list = new List<TYT_MALARIA>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.TYT_MALARIA.AsQueryable();
                        if (search.listTytMalariaExpression != null && search.listTytMalariaExpression.Count > 0)
                        {
                            foreach (var item in search.listTytMalariaExpression)
                            {
        ...
        public TYT_MALARIA GetById(long id, TytMalariaSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.TYT_MALARIA.AsQueryable().Where(p => p.ID == id);
                        if (search.listTytMalariaExpression != null && search.listTytMalariaExpression.Count > 0)
                        {
                            foreach (var item in search.listTytMalariaExpression)
                            {
