...
namespace TYT.DAO.TytUninfectIcdGroup
{
    partial class TytUninfectIcdGroupGet : EntityBase
    {
        ...
        {
            List<TYT_UNINFECT_ICD_GROUP> list = new List<TYT_UNINFECT_ICD_GROUP>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.TYT_UNINFECT_ICD_GROUP.AsQueryable();
        ...
        public TYT_UNINFECT_ICD_GROUP GetById(long id, TytUninfectIcdGroupSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.TYT_UNINFECT_ICD_GROUP.AsQueryable().Where(p => p.ID == id);
                        if (search.listTytUninfectIcdGroupExpression != null && search.listTytUninfectIcdGroupExpression.Count > 0)
                        {
                            foreach (var item in search.listTytUninfectIcdGroupExpression)
                            {
