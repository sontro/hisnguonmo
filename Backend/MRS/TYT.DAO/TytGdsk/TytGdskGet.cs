...
namespace TYT.DAO.TytGdsk
{
    partial class TytGdskGet : EntityBase
    {
        ...
        {
            List<TYT_GDSK> list = new List<TYT_GDSK>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.TYT_GDSK.AsQueryable();
                        if (search.listTytGdskExpression != null && search.listTytGdskExpression.Count > 0)
                        {
                            foreach (var item in search.listTytGdskExpression)
                            {
        ...
        public TYT_GDSK GetById(long id, TytGdskSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.TYT_GDSK.AsQueryable().Where(p => p.ID == id);
                        if (search.listTytGdskExpression != null && search.listTytGdskExpression.Count > 0)
                        {
                            foreach (var item in search.listTytGdskExpression)
                            {
