...
namespace SDA.DAO.SdaCommune
{
    partial class SdaCommuneGet : EntityBase
    {
        ...
        {
            List<V_SDA_COMMUNE> list = new List<V_SDA_COMMUNE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_SDA_COMMUNE.AsQueryable();
                        if (search.listVSdaCommuneExpression != null && search.listVSdaCommuneExpression.Count > 0)
                        {
                            foreach (var item in search.listVSdaCommuneExpression)
                            {
