...
{
    partial class SdaCommuneGet : EntityBase
    {
        public List<SDA_COMMUNE> Get(SdaCommuneSO search, CommonParam param)
        {
            List<SDA_COMMUNE> list = new List<SDA_COMMUNE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.SDA_COMMUNE.AsQueryable();
                        if (search.listSdaCommuneExpression != null && search.listSdaCommuneExpression.Count > 0)
                        {
                            foreach (var item in search.listSdaCommuneExpression)
                            {
        ...
        public SDA_COMMUNE GetById(long id, SdaCommuneSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.SDA_COMMUNE.AsQueryable().Where(p => p.ID == id);
                        if (search.listSdaCommuneExpression != null && search.listSdaCommuneExpression.Count > 0)
                        {
                            foreach (var item in search.listSdaCommuneExpression)
                            {
