...
namespace SDA.DAO.SdaNational
{
    ...
    {
        public List<SDA_NATIONAL> Get(SdaNationalSO search, CommonParam param)
        {
            List<SDA_NATIONAL> list = new List<SDA_NATIONAL>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.SDA_NATIONAL.AsQueryable();
                        if (search.listSdaNationalExpression != null && search.listSdaNationalExpression.Count > 0)
                        {
                            foreach (var item in search.listSdaNationalExpression)
                            {
        ...
        public SDA_NATIONAL GetById(long id, SdaNationalSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.SDA_NATIONAL.AsQueryable().Where(p => p.ID == id);
                        if (search.listSdaNationalExpression != null && search.listSdaNationalExpression.Count > 0)
                        {
                            foreach (var item in search.listSdaNationalExpression)
                            {
