...
{
    partial class SdaConfigGet : EntityBase
    {
        public List<SDA_CONFIG> Get(SdaConfigSO search, CommonParam param)
        {
            List<SDA_CONFIG> list = new List<SDA_CONFIG>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.SDA_CONFIG.AsQueryable();
                        if (search.listSdaConfigExpression != null && search.listSdaConfigExpression.Count > 0)
                        {
                            foreach (var item in search.listSdaConfigExpression)
                            {
        ...
        public SDA_CONFIG GetById(long id, SdaConfigSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.SDA_CONFIG.AsQueryable().Where(p => p.ID == id);
                        if (search.listSdaConfigExpression != null && search.listSdaConfigExpression.Count > 0)
                        {
                            foreach (var item in search.listSdaConfigExpression)
                            {
