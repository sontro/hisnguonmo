...
{
    partial class SdaGroupTypeGet : EntityBase
    {
        public List<SDA_GROUP_TYPE> Get(SdaGroupTypeSO search, CommonParam param)
        {
            List<SDA_GROUP_TYPE> list = new List<SDA_GROUP_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.SDA_GROUP_TYPE.AsQueryable();
                        if (search.listSdaGroupTypeExpression != null && search.listSdaGroupTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listSdaGroupTypeExpression)
                            {
        ...
        public SDA_GROUP_TYPE GetById(long id, SdaGroupTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.SDA_GROUP_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listSdaGroupTypeExpression != null && search.listSdaGroupTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listSdaGroupTypeExpression)
                            {
