...
namespace SDA.DAO.SdaConfigApp
{
    partial class SdaConfigAppGet : EntityBase
    {
        ...
        {
            List<SDA_CONFIG_APP> list = new List<SDA_CONFIG_APP>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.SDA_CONFIG_APP.AsQueryable();
                        if (search.listSdaConfigAppExpression != null && search.listSdaConfigAppExpression.Count > 0)
                        {
                            foreach (var item in search.listSdaConfigAppExpression)
                            {
        ...
        public SDA_CONFIG_APP GetById(long id, SdaConfigAppSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.SDA_CONFIG_APP.AsQueryable().Where(p => p.ID == id);
                        if (search.listSdaConfigAppExpression != null && search.listSdaConfigAppExpression.Count > 0)
                        {
                            foreach (var item in search.listSdaConfigAppExpression)
                            {
