...
namespace ACS.DAO.AcsUser
{
    partial class AcsUserGet : EntityBase
    {
        ...
        {
            List<ACS_USER> list = new List<ACS_USER>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.ACS_USER.AsQueryable();
                        if (search.listAcsUserExpression != null && search.listAcsUserExpression.Count > 0)
                        {
                            foreach (var item in search.listAcsUserExpression)
                            {
        ...
        public ACS_USER GetById(long id, AcsUserSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.ACS_USER.AsQueryable().Where(p => p.ID == id);
                        if (search.listAcsUserExpression != null && search.listAcsUserExpression.Count > 0)
                        {
                            foreach (var item in search.listAcsUserExpression)
                            {
