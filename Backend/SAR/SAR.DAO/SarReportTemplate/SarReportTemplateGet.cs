...
namespace SAR.DAO.SarReportTemplate
{
    partial class SarReportTemplateGet : EntityBase
    {
        ...
        {
            List<SAR_REPORT_TEMPLATE> list = new List<SAR_REPORT_TEMPLATE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.SAR_REPORT_TEMPLATE.AsQueryable();
        ...
        public SAR_REPORT_TEMPLATE GetById(long id, SarReportTemplateSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.SAR_REPORT_TEMPLATE.AsQueryable().Where(p => p.ID == id);
                        if (search.listSarReportTemplateExpression != null && search.listSarReportTemplateExpression.Count > 0)
                        {
                            foreach (var item in search.listSarReportTemplateExpression)
                            {
