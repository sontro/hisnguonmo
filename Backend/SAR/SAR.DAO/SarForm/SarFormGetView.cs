...
namespace SAR.DAO.SarForm
{
    partial class SarFormGet : EntityBase
    {
        ...
        {
            List<V_SAR_FORM> list = new List<V_SAR_FORM>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_SAR_FORM.AsQueryable();
                        if (search.listVSarFormExpression != null && search.listVSarFormExpression.Count > 0)
                        {
                            foreach (var item in search.listVSarFormExpression)
                            {
