...
{
    partial class SarFormDelete : EntityBase
    {
        public SarFormDelete()
            : base()
        {
        ...
        public bool Delete(SAR_FORM data)
        {
            return IsNotNull(data) && IsGreaterThanZero(data.ID) && bridgeDAO.Delete(data.ID);
        }

        public bool DeleteList(List<SAR_FORM> listData)
        {
            bool result = false;
            try
            {
                ...
                if (valid)
                {
                    List<SAR_FORM> listIdNotFound = new List<SAR_FORM>();
                    ...
                    else
                    {
                        ...
                        {
                            List<SAR_FORM> listRaw = ctx.SAR_FORM.Where(o => listId.Contains(o.ID)).ToList();
