...
namespace SAR.DAO.SarFormField
{
    partial class SarFormFieldGet : EntityBase
    {
        ...
        {
            List<SAR_FORM_FIELD> list = new List<SAR_FORM_FIELD>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.SAR_FORM_FIELD.AsQueryable();
                        if (search.listSarFormFieldExpression != null && search.listSarFormFieldExpression.Count > 0)
                        {
                            foreach (var item in search.listSarFormFieldExpression)
                            {
        ...
        public SAR_FORM_FIELD GetById(long id, SarFormFieldSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.SAR_FORM_FIELD.AsQueryable().Where(p => p.ID == id);
                        if (search.listSarFormFieldExpression != null && search.listSarFormFieldExpression.Count > 0)
                        {
                            foreach (var item in search.listSarFormFieldExpression)
                            {
