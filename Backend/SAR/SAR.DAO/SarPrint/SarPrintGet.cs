...
namespace SAR.DAO.SarPrint
{
    partial class SarPrintGet : EntityBase
    {
        ...
        {
            List<SAR_PRINT> list = new List<SAR_PRINT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.SAR_PRINT.AsQueryable();
                        if (search.listSarPrintExpression != null && search.listSarPrintExpression.Count > 0)
                        {
                            foreach (var item in search.listSarPrintExpression)
                            {
        ...
        public SAR_PRINT GetById(long id, SarPrintSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.SAR_PRINT.AsQueryable().Where(p => p.ID == id);
                        if (search.listSarPrintExpression != null && search.listSarPrintExpression.Count > 0)
                        {
                            foreach (var item in search.listSarPrintExpression)
                            {
