...
namespace SAR.DAO.SarPrint
{
    partial class SarPrintGet : EntityBase
    {
        ...
        {
            List<V_SAR_PRINT> list = new List<V_SAR_PRINT>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.V_SAR_PRINT.AsQueryable();
                        if (search.listVSarPrintExpression != null && search.listVSarPrintExpression.Count > 0)
                        {
                            foreach (var item in search.listVSarPrintExpression)
                            {
