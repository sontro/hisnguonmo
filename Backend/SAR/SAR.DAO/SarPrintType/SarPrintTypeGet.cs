...
namespace SAR.DAO.SarPrintType
{
    partial class SarPrintTypeGet : EntityBase
    {
        ...
        {
            List<SAR_PRINT_TYPE> list = new List<SAR_PRINT_TYPE>();
            try
            {
                bool valid = true;
                ...
                if (valid)
                {
                    ...
                    {
                        var query = ctx.SAR_PRINT_TYPE.AsQueryable();
                        if (search.listSarPrintTypeExpression != null && search.listSarPrintTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listSarPrintTypeExpression)
                            {
        ...
        public SAR_PRINT_TYPE GetById(long id, SarPrintTypeSO search)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    using (var ctx = new AppContext())
                    {
                        var query = ctx.SAR_PRINT_TYPE.AsQueryable().Where(p => p.ID == id);
                        if (search.listSarPrintTypeExpression != null && search.listSarPrintTypeExpression.Count > 0)
                        {
                            foreach (var item in search.listSarPrintTypeExpression)
                            {
