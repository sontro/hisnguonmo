...
using SAR.MANAGER.Core.SarUserReportType.Get;
using Inventec.Core;
using System;
...
{
    class SarUserReportTypeCheckVerifyIsUnlock
    {
        ...
        internal static bool Verify(CommonParam param, List<long> ids)
        {
            ...
            try
            {
                ...
                {
                    SarUserReportTypeFilterQuery filter = new SarUserReportTypeFilterQuery();
                    filter.IDs = ids;
                    List<SAR_USER_REPORT_TYPE> listData = new SarUserReportTypeBO().Get<List<SAR_USER_REPORT_TYPE>>(filter);
        ...
        private static bool Check(CommonParam param, SAR_USER_REPORT_TYPE raw)
        {
            bool result = true;
            if (raw == null)
            {
