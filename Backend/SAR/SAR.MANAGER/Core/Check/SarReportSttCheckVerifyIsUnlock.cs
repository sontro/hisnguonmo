...
using SAR.MANAGER.Core.SarReportStt.Get;
using Inventec.Core;
using System;
...
{
    class SarReportSttCheckVerifyIsUnlock
    {
        ...
        internal static bool Verify(CommonParam param, SAR_REPORT_STT data)
        {
            bool result = true;
            try
            {
        ...
        internal static bool Verify(CommonParam param, List<long> ids)
        {
            ...
            try
            {
                ...
                {
                    SarReportSttFilterQuery filter = new SarReportSttFilterQuery();
                    filter.IDs = ids;
                    List<SAR_REPORT_STT> listData = new SarReportSttBO().Get<List<SAR_REPORT_STT>>(filter);
        ...
        internal static bool Verify(CommonParam param, List<SAR_REPORT_STT> datas)
        {
            bool result = true;
            try
            {
        ...
        private static bool Check(CommonParam param, SAR_REPORT_STT raw)
        {
            bool result = true;
            if (raw == null)
            {
