...
{
    class SarFormDataCheckVerifyIsUnlock
    {
        ...
        internal static bool Verify(CommonParam param, List<long> ids)
        {
            ...
            try
            {
                ...
                {
                    SarFormDataFilterQuery filter = new SarFormDataFilterQuery();
                    filter.IDs = ids;
                    List<SAR_FORM_DATA> listData = new SarFormDataBO().Get<List<SAR_FORM_DATA>>(filter);
        ...
        private static bool Check(CommonParam param, SAR_FORM_DATA raw)
        {
            bool result = true;
            if (raw == null)
            {
