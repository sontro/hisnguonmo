...
{
    public class SarFormViewFilterQuery : SarFormViewFilter
    {
        public SarFormViewFilterQuery()
            : base()
        {

        }
        
        internal List<System.Linq.Expressions.Expression<Func<V_SAR_FORM, bool>>> listExpression = new List<System.Linq.Expressions.Expression<Func<V_SAR_FORM, bool>>>();
        ...
        internal SarFormSO Query()
        {
            ...
            try
            {
                ...
                {
                    listExpression.Add(o => this.FORM_TYPE_CODE__EXACT == o.FORM_TYPE_CODE);
                }
                ...
                if (!String.IsNullOrEmpty(this.KEY_WORD))
                {
                    ...
                    listExpression.Add(o =>
                        ...
                        o.FORM_TYPE_NAME.ToLower().Contains(this.KEY_WORD) ||
                        o.DESCRIPTION.ToLower().Contains(this.KEY_WORD));
                }

                search.listVSarFormExpression.AddRange(listExpression);
                search.OrderField = OrderProcess.GetOrderField<V_SAR_FORM>(ORDER_FIELD);
                search.OrderDirection = OrderProcess.GetOrderDirection(ORDER_DIRECTION);
            }
            catch (Exception ex)
            {
                LogSystem.Error(ex);
                search.listVSarFormExpression.Clear();
                search.listVSarFormExpression.Add(o => o.ID == NEGATIVE_ID);
            }
            return search;
        }
    }
