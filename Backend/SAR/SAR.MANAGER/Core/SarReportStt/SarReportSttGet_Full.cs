...
using SAR.MANAGER.Core.SarReportStt.Get.ListV;
using SAR.MANAGER.Core.SarReportStt.Get.V;
using Inventec.Core;
using System;
    ...
    partial class SarReportSttGet : BeanObjectBase, IDelegacyT
    {
        ...
        T IDelegacyT.Execute<T>()
        {
            ...
            try
            {
                ...
                else if (typeof(T) == typeof(List<V_SAR_REPORT_STT>))
                {
                    ISarReportSttGetListV behavior = SarReportSttGetListVBehaviorFactory.MakeISarReportSttGetListV(param, entity);
                    result = behavior != null ? (T)System.Convert.ChangeType(behavior.Run(), typeof(T)) : default(T);
                }
                else if (typeof(T) == typeof(V_SAR_REPORT_STT))
                {
                    ISarReportSttGetV behavior = SarReportSttGetVBehaviorFactory.MakeISarReportSttGetV(param, entity);
                    result = behavior != null ? (T)System.Convert.ChangeType(behavior.Run(), typeof(T)) : default(T);
                }
            }
            catch (Exception ex)
