...
{
    public class SarRetyFofiFilterQuery : SarRetyFofiFilter
    {
        public SarRetyFofiFilterQuery()
            : base()
        {

        }

        internal List<System.Linq.Expressions.Expression<Func<SAR_RETY_FOFI, bool>>> listExpression = new List<System.Linq.Expressions.Expression<Func<SAR_RETY_FOFI, bool>>>();
        ...
        internal SarRetyFofiSO Query()
        {
            ...
            try
            {
                ...
                search.listSarRetyFofiExpression.AddRange(listExpression);
                search.OrderField = OrderProcess.GetOrderField<SAR_RETY_FOFI>(ORDER_FIELD);
                search.OrderDirection = OrderProcess.GetOrderDirection(ORDER_DIRECTION);
            }
            catch (Exception ex)
            {
                LogSystem.Error(ex);
                search.listSarRetyFofiExpression.Clear();
                search.listSarRetyFofiExpression.Add(o => o.ID == NEGATIVE_ID);
            }
            return search;
        }
    }
