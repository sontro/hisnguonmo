...
{
    public class SarPrintLogViewFilterQuery : SarPrintLogViewFilter
    {
        public SarPrintLogViewFilterQuery()
            : base()
        {

        }
        
        internal List<System.Linq.Expressions.Expression<Func<V_SAR_PRINT_LOG, bool>>> listExpression = new List<System.Linq.Expressions.Expression<Func<V_SAR_PRINT_LOG, bool>>>();
        ...
        internal SarPrintLogSO Query()
        {
            ...
            try
            {
                ...
                search.listVSarPrintLogExpression.AddRange(listExpression);
                search.OrderField = OrderProcess.GetOrderField<V_SAR_PRINT_LOG>(ORDER_FIELD);
                search.OrderDirection = OrderProcess.GetOrderDirection(ORDER_DIRECTION);
            }
            catch (Exception ex)
            {
                LogSystem.Error(ex);
                search.listVSarPrintLogExpression.Clear();
                search.listVSarPrintLogExpression.Add(o => o.ID == NEGATIVE_ID);
            }
            return search;
        }
    }
