...
{
    public class SarFormDataViewFilterQuery : SarFormDataViewFilter
    {
        public SarFormDataViewFilterQuery()
            : base()
        {

        }
        
        internal List<System.Linq.Expressions.Expression<Func<V_SAR_FORM_DATA, bool>>> listExpression = new List<System.Linq.Expressions.Expression<Func<V_SAR_FORM_DATA, bool>>>();
        ...
        internal SarFormDataSO Query()
        {
            ...
            try
            {
                ...
                search.listVSarFormDataExpression.AddRange(listExpression);
                search.OrderField = OrderProcess.GetOrderField<V_SAR_FORM_DATA>(ORDER_FIELD);
                search.OrderDirection = OrderProcess.GetOrderDirection(ORDER_DIRECTION);
            }
            catch (Exception ex)
            {
                LogSystem.Error(ex);
                search.listVSarFormDataExpression.Clear();
                search.listVSarFormDataExpression.Add(o => o.ID == NEGATIVE_ID);
            }
            return search;
        }
    }
