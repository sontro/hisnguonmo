...
namespace SAR.MANAGER.Core.SarReport.Send
{
    class SarReportSendBehavior : BeanObjectBase, ISarReportSend
    {
        SAR_REPORT entity;
        SAR_REPORT raw;

        internal SarReportSendBehavior(CommonParam param, SAR_REPORT data)
            : base(param)
        {
            entity = data;
        }

        bool ISarReportSend.Run()
        {
            bool result = false;
            try
            {
                if (Check())
                {
                    raw.JSON_READER = entity.JSON_READER;
                    ...
                    else
                    {
                        ...
                        BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.SarReport_DAOUpdateJsonReaderReportThatBai);
                    }
                }
            }
            catch (Exception ex)
